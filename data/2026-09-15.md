...
- [Remote Sensing](#Remote Sensing) [Total: 100]
- [ISPRS J P&RS](#ISPRS J P&RS) [Total: 11]
- [RSE](#RSE) [Total: 8]
- [GIScience & RS](#GIScience & RS) [Total: 7]
...
Abstract (英文): Anomaly detection in structured images is challenging in small-data settings where deep learning approaches are costly or impractical. Classical template matching is simple and interpretable but lacks robustness to geometric variations such as scale, rotation, and perspective.
...
Abstract (英文): Heat Kernel Textures (HKTex) represent surface appearance with intrinsic anisotropic
...
Abstract (英文): We study the challenge of sign language video mimicking: given a driving video and a single reference frame, synthesize a video where the target signer reproduces the source
...
Abstract (英文): Human motion may be viewed as a combination of action content, style, and body morphology. Existing motion style transfer methods transfer a reference style onto a content motion while assuming a canonical body, whereas shape-aware motion generators adapt motion to a target shape without explicit style control. This separation of motion style and shape (morphology) makes it difficult to generate stylized motions for non-canonical bodies; naively combining a style transfer module with a shape-aware generator often leaks action content from the style reference and disrupts shape-consistent kinematics. We present MorphoStyle, a framework for shape-aware motion style transfer that is built on a shape-conditioned Finite-Scalar-Quantization Variational Auto-Encoder (FSQ-VAE). The key contribution is to pose the desired style transfer as modular latent disentanglement comprising: (i) a contrastive style encoder that extracts content-decoupled style embeddings; (ii) a text-guided style-routing mechanism that locates style-relevant joints in a text-motion feature space; and (iii) a manifold preserving style modulator that injects discriminative style embeddings in content features as a temporally-gated low-rank offset. Extensive experiments on benchmark datasets demonstrate that MorphoStyle outperforms competing baselines in terms of both shape control and motion style transfer, while simultaneously providing quantitative shape control. For more details, please see project website: https://github.com/funkdub/MorphoStyle.
...
Abstract (英文): Vision-language models (VLMs) remain vulnerable to jailbreaks that distribute harmful intent across text and images, making unimodal safety mechanisms insufficient. We investigate whether this vulnerability can be mitigated directly in the multimodal key-value (KV) memory formed during prefill, without modifying model parameters at inference time. We introduce SPARK, a two-stage framework for targeted KV-memory repair. Stage 1 uses a disposable diagnostic adapter to identify harm-associated directions in multimodal key and value representations. Stage 2 projects out these directions, learns a lightweight residual repair, and anchors repaired keys with an image-structural prior to preserve visual grounding. Rather than applying the intervention uniformly, SPARK mixes repaired and original memory using a head-wise coefficient g_h* determined by intervention-relevant subspace energy E_h, requiring no explicit harm classifier at inference.
...
Abstract (英文): Person re-identification (ReID) is essential for multi-camera surveillance and tracking, yet remains difficult due to viewpoint and illumination changes, occlusion, background clutter, and low resolution imagery. We propose a generative AI integrated multimodal ReID framework designed explicitly for robustness under missing cues and low latency deployment. The key idea is a cost aware early-exit cascade that prioritizes inexpensive, high confidence evidence and only triggers expensive modalities for ambiguous cases.
...
Abstract (英文): Purpose: Paired pre- and post-operative photographs are the standard unit of evidence for plastic surgical outcomes, yet no objective metric verifies whether two images of the same patient were captured under conditions consistent for comparison.
...
### [142] [Weakly Supervised Spatial Grounding for Discriminative Attention-Based Ultrasound-Histopathology Alignment in Prostate Cancer Grading](https://arxiv.org/abs/2609.15150)
...
Abstract (英文): Objective: To develop an intelligent framework, termed CUA-Net, for the automated classification of congenital uterine anomalies (CUA) without requiring coronal plane reconstruction, and to evaluate its clinical applicability.
...
### [150] [Closed-form Bayesian homography estimation from noisy point correspondences](https://arxiv.org/abs/2609.15227)
...
### [171] [VideoScout: Learning Agentic Active Exploration with Adaptive Reasoning Pacing for Long Video Understanding](https://arxiv.org/abs/2609.15606)
...
### [185] [ZGCM-1: A Fully Open and Extremely Efficient Foundation Model for Math and Agentic Search](https://arxiv.org/abs/2609.13356)
...
### [190] [Root-Cause Attribution Is a Search Problem: Continual Search for Long-Horizon Agent Failures](https://arxiv.org/abs/2609.13463)
...
### [220] [How Many Thoughts Can a Vector Hold? The Capacity of Reasoning by Superposition](https://arxiv.org/abs/2609.13747)
...
### [225] [Do Not Restart: Residual Completion for Stateful Agent Handoffs](https://arxiv.org/abs/2609.13800)
...
Abstract (英文): Few-shot in-context learning (ICL), the capacity of a model to infer abstract patterns from input-output examples provided in its prompt and apply them to new inputs, has been extensively studied in large language models trained for next-token prediction on human text. Recently, few-shot ICL has been demonstrated in autoregressive genomic models as well. This raises a question: does ICL emerge broadly across domains, and if so, what common structure is shared?
...
### [236] [Schizophrenia Detection from EEG Signals: A Transformer Framework with Spectrogram Representation](https://arxiv.org/abs/2609.14015)
...
Abstract (英文): Artificial intelligence systems are rapidly becoming critical components in healthcare, finance, public services, and other safety-critical domains. Yet the engineering practices used to evaluate these systems remain predominantly model-centric, emphasizing properties such as accuracy, robustness, fairness, and interpretability before deployment. These properties are necessary but insufficient once an AI system operates within an ever changing socio-technical environment characterized by distribution shifts, institutional constraints, human feedback loops, privacy requirements, and interactions among multiple AI agents.
...
### [247] [A note on goal-based hierarchical RL](https://arxiv.org/abs/2609.14605)
*Kevin Murphy*
...
Abstract (英文): The agent-centric general value function (ACGVF) construction of
...
### [248] [DynSTEER: Dynamic Stage-wise Trajectory Evaluation and Execution-time Review for Agents](https://arxiv.org/abs/2609.14637)
...
### [249] [Diffusion-Based Generation of Gait Trajectories](https://arxiv.org/abs/2609.14642)
...
### [254] [AppliedScientist: Automated Scientific Revision Through Iterative AI Reviewing](https://arxiv.org/abs/2609.14738)
...
### [263] [Domain Generalization for Smartphone-Based Human Activity Recognition: A Systematic Analysis of Components and Interactions](https://arxiv.org/abs/2609.14863)
...
### [264] [GGUF-Metadata Prediction of Single-Sequence llama.cpp Throughput Across Three Systems](https://arxiv.org/abs/2609.14864)
...
### [269] [Converting Sequenced Fuzzy Cognitive Maps to Causal Virtual Worlds with Large Video Generators](https://arxiv.org/abs/2609.14985)
...
Abstract (英文): Designing effective memory mechanisms is crucial for advancing LLM-driven Multi-Agent Systems (MAS), helping agents learn together and perform better over time. While recent work has led to strong cooperation skills, most methods still use flat, unstructured memories, which easily get filled with noise and erase differences between agents. To address this, we introduce the concept of collective-individual memory synergy and propose CoMem, an architecture that unifies both private experience and shared knowledge for multi-agent learning. CoMem features:(i) Private Experience Sedimentation, which lets each agent keep and update its own useful memories over time;(ii) Collective Wisdom Curation, which carefully selects only widely proven ideas to be shared among agents;(iii)Parallel Dual-Stream Retrieval, which allows agents to draw both from their own memory and the group's wisdom, using clustering to ensure diversity.Experiments on ALFWorld and PDDL benchmarks show that CoMem achieves strong overall performance and robustly avoids memory pollution.

</details>


### [272] [Semantic-TVM: Structure-Preserving Trustworthy Virtual Memory for Memory-Augmented and Tool-Using Agents](https://arxiv.org/abs/2609.15011)
...
### [277] [BusMA: A Bus Communication Substrate for Multi-Agent Systems](https://arxiv.org/abs/2609.15054)
...
### [280] [OpenAl4S: Code as Action, Science as Sessions](https://arxiv.org/abs/2609.15096)
...
### [290] [ProIQA: A Process-Based Framework for Fine-Grained Math Item Quality Assessment](https://arxiv.org/abs/2609.15292)
...
### [315] [Navigating Sparse Evidence: Agentic Visual RAG via Explicit Context Selection and Consolidation](https://arxiv.org/abs/2609.15800)
...
### [322] [Corrupt Plans, Clean Traces: Evading Chain-of-Thought Monitoring with Plan Injection](https://arxiv.org/abs/2609.15989)
...
### [324] [RGB-Derived Canopy Height Models for Riparian Woody Vegetation Monitoring Using Depth Anything V2](https://doi.org/10.3390/rs18183063)
*Hun Choi,Seonggi An,Chanjoo Lee,Keunhoo Cho,Boram Seong*
...
### [335] [Noise-Adjusted Feature Extraction for Deep Learning-Based Classification of Hyperspectral Imagery](https://doi.org/10.3390/rs18183071)
*Yan Xu,Qian Du*
...
### [346] [Enhancing Our View from Above: A Downscaling Technique to Reveal Finer-Scale Urban Heat Patterns](https://doi.org/10.3390/rs18183091)
*Evan Shea,A. O. Benson,Tijmen Witvliet,Mark Fillo,Melissa R. McHale*
...
Abstract (英文): Winter wheat is a cornerstone of China’s grain production, contributing substantially to national food security and overall cereal output. This study modeled the nonlinear associations between multitemporal remote sensing variables and winter wheat yield. To produce high-spatiotemporal-resolution inputs, we used the Enhanced Spatial and Temporal Adaptive Reflectance Fusion Model (ESTARFM) to integrate Sentinel-2 normalized difference vegetation index (NDVI) data with MODIS NDVI data, generating NDVI composites at 8-day intervals with a 10-m spatial resolution. The NDVI, actual evapotranspiration (ET), land surface temperature (LST), precipitation (PRE), and soil moisture (SM) were selected as predictors for yield estimation because they are closely associated with winter wheat growth and yield formation during primary growth stages. By integrating the local temporal feature-learning capacity of a one-dimensional convolutional neural network (1-D CNN) with the strength of a bidirectional long short-term memory (BiLSTM) model in capturing temporal dependencies within time series, a BiLSTM–CNN model was constructed for wheat yield estimation and prediction. The BiLSTM–CNN model showed higher estimation accuracy than individual BiLSTM and 1-D CNN models, with an R2 of 0.69 and root mean square error (RMSE) of 478.68 kg/hm2. The use of all the parameters produced the best estimation performance among all the parameter combinations. Approximately two months before harvest, the model still provided satisfactory yield prediction accuracy. This study provides an important theoretical basis for high-accuracy regional winter wheat yield estimation and pre-harvest forecasting.

</details>


### [352] [The Capacity of Generative Models to Synthesize Regional Landslide and Non-Landslide Remote Sensing Imagery Under Data-Scarce Scenarios: Insights from Multimodal Foundation Models](https://doi.org/10.3390/rs18183100)
*Yiwei Liu,Ye Tao,Aiguo Xing,Qiankuan Wang,Muhammad Bilal*
...
Abstract (英文): Cropland abandonment directly undermines the regional food supply, requiring accurate monitoring to inform protection and recultivation policies. However, multi-category abandoned-cropland mapping remains challenging in southern China’s double-cropping rice regions because fragmented fields, frequent cloud/rain conditions, and complex phenology can obscure abandonment signals. Focusing on a typical double-cropping rice region in southern China, this study first used Swin-UNet to generate annual land-use maps from 2018 to 2022 and derive interannual trajectories. It then identified non-cultivated cropland within each year using Normalized Difference Vegetation Index (NDVI) data from key phenological periods and block-adaptive Otsu thresholding. Overlaying cultivation status with land-use trajectories further subdivided non-cultivated cropland into construction-occupied abandoned, naturally abandoned cropland, and fallow cropland. Finally, the spatial differentiation of each type and the potential grain-production gains from recultivating naturally abandoned cropland were analyzed. Results showed that annual classification accuracies from 2018 to 2022 ranged from 94.7 to 95.5%, supporting reliable cropland change trajectory extraction. From 2019 to 2022, total abandoned area decreased from 546.60 km2 to 486.73 km2. Naturally abandoned cropland remained dominant but declined mainly because 18.5–21.7% of it was converted back to stable cropland annually, while construction-occupied abandonment increased and fallow cropland remained relatively stable. Natural abandonment concentrated on moderate slopes near settlements and roads, with a theoretical grain-production potential equivalent to 12.88–15.14% of Maoming’s annual grain output. This study advances abandoned-cropland monitoring from area estimation to type-specific process interpretation and recultivation-priority assessment in southern double-cropping rice regions.

</details>


### [364] [Sentinel-2 Forel–Ule Index as a Proxy for Ecological Status in Reservoirs: A Case Study in Southern Portugal](https://doi.org/10.3390/rs18183109)
*Mariana Campista Chagas,Ana Paula Falcão,Rodrigo Proença de Oliveira*
...
### [366] [MTCPNet: A Mamba-Based Registration Network with Tri-Branch Consistency Projection for SAR-Visible Image Registration](https://doi.org/10.3390/rs18183114)
*Mingming Gao,Yiming Xia,Lei Ma,Ling Wan*
...
### [371] [Wildfire Scar Detection in Mediterranean Chile Using Sentinel-1 InSAR Coherence and Machine Learning: The 2017 “Las Máquinas” Megafire](https://doi.org/10.3390/rs18183105)
*Miguel Aguilera,Antonio Cabrera,Paulina Vidal-Páez,Pablo Sarricolea,Francisca Gutiérrez-Cáceres,Rómulo Santelices-Moya*
...
Abstract (英文): Hyperspectral image (HSI) classification is critical for remote sensing but faces challenges in balancing accuracy and inference efficiency. Existing graph-based knowledge distillation (KD) methods are confined to single geometric spaces, ignoring the hierarchical semantics of land-cover categories. Cross-geometry distillation compresses multiple geometries into a single Euclidean student, forcing one geometry to collapse into the other. In this paper, we propose Dual-Space Knowledge Distillation (DSKD), a novel dual-student dual-space KD framework integrating Euclidean (GCN) and Hyperbolic (HGCN) teachers to jointly train a native MLP student and a native HNN student. With cross-geometric feature bridging (CGFB) and output distribution cohesion (ODC), the two students mutually learn each other’s complementary geometry, so DSKD captures complementary spatial-spectral and hierarchical features while enabling graph-free inference without message passing. Extensive experiments on four HSI datasets and four general graph benchmarks demonstrate that DSKD outperforms single-space distillation and single-student cross-geometry baselines in most settings, confirming its effectiveness and generalization capability across diverse graph-structured data.

</details>


### [391] [Study on Nonlinear Driving Mechanisms of Spatiotemporal Evolution in Sanjiang Plain Wetlands Based on Explainable Learning Methods](https://doi.org/10.3390/rs18183132)
*Nan Lin,Yanan Lu,Ruifei Zhu,Menghong Wu,Hao Yu,Zeyue Jing,Chenglong Xu,Botao Zhang,Ranzhe Jiang*
...
### [394] [Dual-Stream Spatial–Spectral Network with Nested Attention for Hyperspectral Image Classification](https://doi.org/10.3390/rs18183126)
*Jianing Wang,Fanghao Li,Liang Chen,Shijie Liu,W. Zhang,Lijun Jiang,Chuanjie Zhang*
...
### [399] [Polar Summer Snow and Ice Albedo Feedbacks Assessed by Satellite Observations and Radiative Kernels](https://doi.org/10.3390/rs18183135)
...
### [400] [A Magnetic Anomaly Detection Method Based on Multi-Feature Classification-Fusion Neural Network in Colored Noise Background](https://doi.org/10.3390/rs18183138)
...
### [401] [Extracting Summer-Harvested Crops in the Baojixia Irrigation District Using CycleGAN and Transfer Learning](https://doi.org/10.3390/rs18183155)
...
### [402] [Detecting Early Successional Stages in a Glacier Forefield Through a Hierarchical Sentinel-2 Classification Framework](https://doi.org/10.3390/rs18183140)
...
### [403] [LMCAN: A Lightweight Multiscale Contextual Attention Network for Hyperspectral and Multispectral Image Fusion](https://doi.org/10.3390/rs18183154)
...
### [404] [Mapping Native Grass Cover with Random Forest Models: Sentinel-2 Versus Sentinel-2 Combined with Sentinel-1 SAR-Derived GLCM Texture Metrics](https://doi.org/10.3390/rs18183150)
...
### [405] [Nonlinear Association and Spatial Heterogeneity Between Urban Vitality and Built Environment: Evidence from the Main Urban Area of Chengdu](https://doi.org/10.3390/rs18183159)
...
### [406] [LMAF-Net: Lightweight Multi-Attention Fusion Landslide Identification Network](https://doi.org/10.3390/rs18183141)
...
### [407] [Integrating Soil Data, Forest Habitat Types and NDVI to Assess Habitat Heterogeneity in Common Ash (Fraxinus excelsior L.) Stands](https://doi.org/10.3390/rs18183147)
...
### [408] [Interpretable Multi-Year Winter Wheat Mapping with Sentinel-1/2 Time Series: SHAP-Based Feature Selection and Bayesian-Optimized Machine Learning](https://doi.org/10.3390/rs18183137)
...
### [409] [From Wood–Foliar Semantics to 3D Fuel Characterization: Transferable Deep Learning for Terrestrial LiDAR Across Global Forest Ecosystems](https://doi.org/10.3390/rs18183158)
...
### [410] [Satellite Mapping of Annual Center Pivot Irrigation Expansion in Africa’s Hyper-Arid Regions from 1972 to 2025](https://doi.org/10.3390/rs18183142)
...
### [411] [Improving Crop-Type Mapping in Fragmented Agricultural Landscapes with Parcel Constraints and HLSS30-Derived Phenological Features](https://doi.org/10.3390/rs18183149)
...
### [412] [Self-Supervised 3D Point Cloud Detection with Depthwise Separable Masked Autoencoders](https://doi.org/10.3390/rs18183148)
...
### [413] [Comparison of EDGAR, ODIAC, and MEIC Grid CO2 Emission Inventories: Historical and Current Versions](https://doi.org/10.3390/rs18183144)
...
### [414] [Multi-Instrumental Evidence of the 2025 Absorbing Aerosol Perturbation at the RADO-Bucharest Observatory](https://doi.org/10.3390/rs18183143)
...
### [415] [Recurrent Landslide-Derived Debris Flows in the Yizhong River Basin: Geomorphic Controls, Structural Predisposition, and Rainfall-Driven Instability](https://doi.org/10.3390/rs18183139)
...
### [416] [Coseismic Slip Distribution and Coulomb Stress Changes of the 2024 Mw 7.0 Wushi Earthquake, Xinjiang, China](https://doi.org/10.3390/rs18183160)
...
### [417] [DMAM: Dynamic Multiscale Adaptive Mechanism-Driven Remote-Sensing Target Detection Network](https://doi.org/10.3390/rs18183145)
...
### [418] [TFSANet: Temporal Fusion and Structure-Aware Network for Remote Sensing Change Detection](https://doi.org/10.3390/rs18183151)
...
### [419] [Estimating Contents of Multiple Biomarkers in Medicago truncatula Under Salt Stress via Multi-Granularity Spectral Segmentation](https://doi.org/10.3390/rs18183146)
...
### [420] [Evidence-Based Reliability Assessment of Spatial Transfer Learning for Satellite-Derived Ground Deformation Monitoring](https://doi.org/10.3390/rs18183136)
...
### [421] [Attention-Enhanced Lightweight YOLO with Evolutionary Architecture Search for Insulator Defect Detection](https://doi.org/10.3390/rs18183152)
*Sheming Fan,Bin Cao*
...
### [422] [A Spectral Domain Interpolation Framework for Robust Hyperspectral Feature Construction Under Spectral Uncertainty](https://doi.org/10.3390/rs18183157)
...
### [423] [Ice Avalanche Hazard Assessment Using Multi-Source Geospatial Data and GWRF Model](https://doi.org/10.3390/rs18183156)
...
### [424] [GBOV-trained Gaussian process models for Sentinel-3 SYN vegetation traits with decomposed uncertainty quantification](https://doi.org/10.1016/j.isprsjprs.2026.08.028)
*Pablo Reyes-Muñoz,Emma De Clerck,Yuxin Zhang,José Luis García-Soria,Luke A. Brown,Miguel Morata,Carolina Tenjo,Jochem Verrelst*
...
Abstract (英文): Accurate and uncertainty-aware retrieval of vegetation biophysical variables is essential for reliable Earth observation-based monitoring. We developed Gaussian Process Regression (GPR) models trained on globally distributed Copernicus Ground-Based Observations for Validation (GBOV) reference data to retrieve leaf area index (LAI), fraction of absorbed photosynthetically active radiation (FAPAR), and fractional vegetation cover (FVC) from Sentinel-3 SYNERGY (SY_2_SYN) surface reflectance. The models are designed for general applicability, and are illustrated here through large-scale application and evaluation over Europe. Over 3650 GBOV samples upscaled to satellite pixel per fundamental vegetation trait (FVT) supported training of the GPR–SYN-GPR models (2020–2021), whilst over 1700 samples were reserved for validation (2022–2024). The models achieved robust performance for most vegetation types, with R 2 and NRMSE over 0.8 and below 13%, respectively, for croplands, while revealing persistent limitations in dense evergreen canopies. Comparison with Copernicus Global Land Service (CGLS) products reveals overall agreement, with biome-dependent differences that guide future refinement. A key feature of the GBOV–SYN–GPR framework is the explicit decomposition of predictive uncertainty into epistemic (model-driven) and aleatoric (observation-driven) components. European-scale mapping for 14–23 July 2024 reveals coherent biogeographical gradients and epistemic uncertainty patterns consistent with expected vegetation dynamics. Overall, epistemic uncertainty dominates and exhibits systematic spatial structures linked to gaps in training representativeness, while aleatoric uncertainty remains generally smaller and is mainly enhanced in mountainous regions and under challenging observation conditions. To contextualize the magnitude of the total uncertainty, integrating both epistemic and aleatoric components, we compared it with GBOV-provided uncertainties and found that GBOV–SYN–GPR total uncertainties were of similar magnitudes to those of the upscaled GBOV land products at three GBOV sites. The proposed framework provides a scalable and traceable approach for generating uncertainty-quantified vegetation trait products from Sentinel-3 and offers a pathway for future optical missions.

</details>


### [425] [PELNet: A general physics-embedded learning framework for polarimetric target decomposition](https://doi.org/10.1016/j.isprsjprs.2026.08.040)
*Shurong Zhang,Di Zhuang,Lamei Zhang,Bin Zou,Jordi J. Mallorqui*
...
### [426] [Synergizing motion and depth: A distilled dual-branch network for moving infrared small target detection](https://doi.org/10.1016/j.isprsjprs.2026.08.038)
*Dengyan Luo,Yanping Xiang,Hu Wang,Luping Ji,Song Tang,Mao Ye*
...
### [427] [Editorial Board](https://doi.org/10.1016/s0924-2716(26)00455-7)
*Unknown Author*
...
### [428] [Four decades of Landsat-based river turbidity reconstruction reveal spatially heterogeneous declines across U.S. rivers](https://doi.org/10.1016/j.isprsjprs.2026.09.007)
*Yang Li,Bo Han,Ge Liu,Kaishan Song,Dongmei Lyu,Kun� Shi,Yue Ma,Shidi Shao,Yujin Zhou,Jipeng Zhang,Heng Lyu*
...
Abstract (英文): Turbidity, a fundamental water quality parameter, profoundly influences aquatic ecosystems by regulating light penetration, primary productivity, and the fate of nutrients and pollutants. Despite prior large-scale optical water-quality studies of U.S. rivers, systematic, multi-decadal analyses integrating Landsat surface reflectance, machine-learning retrieval, and ecoregion-scale driver attribution have remained limited. This study developed and validated a long-term Landsat-based turbidity reconstruction framework for major rivers of the conterminous United States from 1984 to 2025. By integrating harmonized Landsat 5/7/8/9 surface reflectance (Collection 2 Tier 1) with river geometry from the Surface Water and Ocean Topography Mission River Database (SWORD), named after the SWOT satellite mission, and in-situ turbidity measurements from the USGS, we established a robust Random Forest (RF) regression model (validation R 2 = 0.95, RMSE = 21.25 NTU) for turbidity estimation, outperforming three other machine-learning algorithms. The RF model was further evaluated using strict spatiotemporal transferability tests, including chronological hold-out, shared-site, new-site, and leave-one-region-out (LORO) spatial cross-validation. Model interpretability was rigorously validated using SHAP (SHapley Additive exPlanations) analysis, confirming that the model learned physically meaningful relationships. Our study revealed distinct spatial patterns in river turbidity, with generally higher values in the Southern Plains (SPL) and Temperate Plains (TPL) ecoregions and lower values in the Western Mountains (WMT) and Xeric (XER) regions. Moreover, we detected a heterogeneous but overall declining trend in turbidity over time, particularly in historically high-turbidity areas. Model diagnostics indicate that predictive performance is higher in mid- to high-turbidity waters, and the reconstructed dataset should be interpreted primarily at regional and long-term scales. To explore the drivers of these changes, we integrated climatic, land cover, and dam data into the analysis. The results show that anthropogenic factors, including dams (∼33% contribution), forest cover (∼27%, reflecting the absence of disturbance), and urban/built-up areas (∼17%), are the primary contributors to turbidity variance at the national scale, far outweighing the influence of climatic variables (each generally < 5%). Further investigation of ten representative dammed rivers indicated that dam construction is associated with significant downstream reductions in turbidity, with the effect typically stronger for taller dams but modulated by local topography. As an exploratory application, framed as an illustrative case study rather than a primary finding, we further developed a turbidity-linked hydro-morphodynamic sensitivity indicator that combines turbidity decline and velocity-related channel proxies. This indicator is intended to provide a preliminary reference for identifying reaches where sediment reduction may coincide with enhanced hydraulic forcing; independent geomorphic validation is required before any hazard-related interpretations can be drawn.

</details>


### [429] [WheatScoper: A lightweight organ-based framework for multi-view wheat phenotyping using time-series RGB images](https://doi.org/10.1016/j.isprsjprs.2026.08.039)
...
### [430] [An ensemble version of fire potential index (eFPI) integrating seasonal variability in sub-pixel fuel composition](https://doi.org/10.1016/j.isprsjprs.2026.09.010)
*Tuo Feng,Dar A. Roberts,Max A. Moritz,S. Sweeney,Y. Zhan,Alan T. Murray*
...
### [432] [SAAD-SR: A multi-altitude aerial object detection benchmark evaluated using an altitude-conditioned distillation detector](https://doi.org/10.1016/j.isprsjprs.2026.09.006)
...
Abstract (英文): Object detection in aerial imagery is essential for remote sensing applications such as disaster response, traffic monitoring, and security surveillance. However, detection reliability is often degraded by flight height variation, environmental variability, and reduced object pixel support. These challenges are further compounded by the limitations of existing UAV datasets used for training and evaluation. Although existing UAV datasets have advanced aerial perception research, many lack explicit altitude metadata, provide limited semantic diversity, or contain insufficient real-world variability, thereby restricting the systematic evaluation of altitude-induced scale-regime changes. Consequently, the detector behavior under repeatable flight-height regimes remains insufficiently characterized. To address these challenges, the Scale-Aware Aerial Dataset for Scale Robustness (SAAD–SR) is introduced as a large-scale altitude-stratified benchmark comprising 154K UAV frames, of which 104,255 images are fully annotated for object detection, yielding approximately 1.6M labeled object instances across eight classes under diverse illumination, weather, and scene conditions. The dataset is organized into 15 m, 25 m, and 45 m altitude tiers using synchronized altitude metadata to support altitude-stratified training and evaluation, and the benchmark is evaluated using UAV-oriented and transformer-based detectors. Building on SAAD–SR, the YOLO Scale-Robust (YOLO–SR) framework incorporates the corresponding altitude-regime conditioning value into intermediate features through scalar-conditioned feature modulation and altitude-conditioned teacher–student distillation. Furthermore, a Scale Robustness Index (SRI) is used to quantify detection stability across altitude-induced scale regimes. YOLO–SR achieves a pooled overall validation mAP@0.5 of 0.708 and a mean altitude-wise mAP@0.5 of 0.658, with an SRI of 0.991, while reaching 75.5 FPS in offline evaluation and 13 FPS during onboard UAV operation.

</details>


### [433] [Space-time dynamics of rainfed wheat yield in Ethiopia: Insights from remote sensing](https://doi.org/10.1016/j.isprsjprs.2026.08.037)
*Habtamu S. Gelagay,Louise Leroux,Lulseged Tamene,Zewdu Abro,Siyabusa Mkuhlani,Kindie Tesfaye,Wuletawu Abera,João Vasco Silva,Marc Corbeels*
...
### [434] [When multimodal fusion helps or hurts: Exploring modality synergy and cross-scene transferability for mangrove species mapping](https://doi.org/10.1016/j.isprsjprs.2026.08.036)
...
### [435] [A new adaptive approach for detecting crop sowing and harvesting dates from Sentinel-1 time series](https://doi.org/10.1016/j.rse.2026.115649)
*Shuai Xu,Xiaolin Zhu*
...
Abstract (英文): Spatially-explicit crop sowing and harvesting dates are crucial for crop modeling, field management, and yield estimation. Synthetic Aperture Radar (SAR) technology, especially the globally available Sentinel-1 time-series data, offers significant potential to monitor these cropping activity dates (CADs) due to all-weather observation capabilities. Recent studies have highlighted the effectiveness of two Sentinel-1 indicators, the VH and Cross-polarization Ratio (CR), in CAD detection. However, their performance varies significantly depending on crop types and growing environments, making the SAR-based methods unreliable for large-scale applications. Hence, we developed an Adaptive Sentinel-1 Indicator Selection (ASIS) framework for CAD detection by comprehensively investigating the mechanisms influencing the capabilities of Sentinel-1 indicators. Our findings suggested that CR is more effective for CADs detection in rain-fed crops with rough soil, while VH performs better in irrigated or aqua fields. Based on this new knowledge, ASIS first employed an iterative time-series clustering strategy to group Sentinel-1 pixels into clusters which represent different crop types and land surface conditions. Then, a new index, RCGI, integrated VH and CR to reliably delineate the crop growing seasons. Last, VH intensity out of the growing seasons was used to automatically identify land surface conditions for selecting the optimal Sentinel-1 indicator (VH or CR) in sowing and harvesting detection. Results show that ASIS had superior accuracy for sowing (MAE = 7.62 days) and harvesting dates estimation (MAE = 8.48 days), making the estimation error of sowing dates (or harvesting dates) significantly decrease by 32.4% (or 7.5%) compared to using VH only and by 52.7% (or 9.5%) compared to CR only. This study demonstrated the effectiveness of the optimal Sentinel-1 indicator in CAD detection, especially for regions without prior information on crop labels and field boundaries.

</details>


### [436] [Monitoring global mangrove spatial evolution and functional degradation using BTSAD and CBPA methods over a 36-year dense Landsat and HLSv2.0 time series](https://doi.org/10.1016/j.rse.2026.115656)
*Bolin Fu,Weiping Yi,Yeqiao Wang,Hang Yao,Mingming Jia,Ertao Gao,Donglin Fan*
...
### [437] [Editorial Board](https://doi.org/10.1016/s0034-4257(26)00423-2)
*Unknown Author*
...
### [438] [Next frontier of Earth observation: Quantum-enhanced multispectral remote sensing for ultra-low signal environments](https://doi.org/10.1016/j.rse.2026.115665)
...
### [439] [Unified knowledge transfer boosts individual tree crown segmentation without scene-specific labels](https://doi.org/10.1016/j.rse.2026.115646)
...
### [440] [Reconstructing cloud-free Sentinel-2 time series under complex degradations with state-driven spatio-temporal modeling](https://doi.org/10.1016/j.rse.2026.115661)
...
### [441] [Deep learning-based forest disturbance detection for Europe using Landsat time series](https://doi.org/10.1016/j.rse.2026.115670)
...
### [442] [First vegetation optical depth product retrieved from FengYun-3D X-band observations](https://doi.org/10.1016/j.rse.2026.115647)
...
### [443] [Balancing solution quality and runtime in modular mobile hospital location: a hybrid approach of deep reinforcement learning and integer programming](https://doi.org/10.1080/15481603.2026.2730795)
*Suyeon Hwang,Hanseong Chae,Seohyeon Baek,Jinmu Choi*
...
Abstract (英文): In emergency situations such as natural disasters and infectious disease outbreaks, healthcare demand can surge rapidly and become concentrated in specific areas, and existing fixed medical facilities alone may struggle to respond to such sudden increases in demand, giving rise to the need for the rapid deployment of mobile hospitals. To address this Modular Mobile Hospital (MMH) location selection problem—which requires deriving a sufficiently high-quality solution within a limited timeframe—this study proposes a hybrid framework that integrates Integer Programming (IP) with Deep Reinforcement Learning (DRL). This framework leverages the strengths of both methods: the high solution quality offered by IP and the rapid solution exploration capability of DRL. Specifically, DRL is employed to screen candidate locations, after which IP determines the final locations from within this refined candidate set. Our analysis demonstrates that the proposed framework achieved up to a 93% reduction in computation time compared to IP alone, while consistently yielding improved solution quality over DRL alone across all conditions. Notably, under large-scale candidate conditions (p > 300), the framework narrowed the solution quality gap with IP to within 99.7% of IP's performance, while still producing results in a shorter amount of time. This hybrid framework offers a practical, time-efficient alternative for facility location decisions in disaster response, where rapid identification of sufficiently good solutions is essential.

</details>


### [444] [Assessing local and transferable spectral endmembers for disturbance detection in Pinus sylvestris L. forests across Spain](https://doi.org/10.1080/15481603.2026.2728760)
*Saioa Munuera,Cristina Gómez,Beatriz Fernández Águeda,Francisco Rodríguez*
...
Abstract (英文): Differences in forest structure, species composition, and climate across biogeographical regions challenge the reliability of spectral models for forest disturbance monitoring. This study assessed whether region-specific spectral endmembers are required to detect silvicultural interventions in Pinus sylvestris forests across Spain. Sentinel-2 spectral signatures were characterized across 18 Provenance Regions and forest development stages (young, pole, and mature stands). PRs were grouped by spectral similarity using K-means clustering, and local, transferred, and global endmember sets were incorporated into the Fusion Near Real Time (FNRT) algorithm through Linear Spectral Mixture Analysis (LMSA) and the Normalized Difference Fraction Index (NDFI). Detection performance was evaluated using 70 reference polygons representing clear-cutting and thinning intervention. Spectral analysis identified three main spectral-ecological clusters: Atlantic, Continental, and Mediterranean. Detection accuracy differed significantly among clusters, reaching 86.63 ± 15.09% in the Continental, 78.63 ± 20.87% in the Atlantic, and 70.23 ± 17.77% in the Mediterranean cluster. Silvicultural treatment also significantly affected performance, with clear-cutting achieving higher accuracy (89.60 ± 10.76%) than high- (73.02 ± 20.22%) and low-intensity thinning (72.74 ± 19.95%). In contrast, endmember origin had no significant effect on detection accuracy (p = 0.617): local endmembers achieved 80.22 ± 18.47%, compared with 78.44 ± 19.04% for global and 76.82 ± 20.22% for transferred endmembers. These results show that, despite regional spectral variability, global and transferred endmembers maintain comparable detection performance, supporting their use for large-scale operational forest monitoring while reducing the need for region-specific calibration.

</details>


### [445] [A soil-thickness-constrained geospatial framework for reconstructing monthly shallow soil water storage across the Tibetan Plateau](https://doi.org/10.1080/15481603.2026.2728825)
*Xin Zhang,Jianrong Fan*
...
Abstract (英文): Aerial and ground-based laser scanning platforms provide complementary three-dimensional, high-precision data for characterizing forest structure, yet accurate registration of cross-platform point clouds remains challenging in complex forest environments. Existing methods rely either on object-level features (e.g. tree stems), which are often incomplete across platforms, or on local geometric descriptors that are unreliable given self-similarity, occlusion, and density asymmetry inherent in forest point clouds. To address these limitations, we introduce MultiVoxelReg, a multi-scale voxel-based framework for automatic cross-platform forest point cloud registration. Instead of depending on semantic objects or geometric keypoints, MultiVoxelReg leverages voxel-level statistical properties that are more robust to noise and partial data loss. Forest structure is represented using a 12-dimensional voxel descriptor encoding distributional, positional, and geometric characteristics of points within voxels. Multi-scale structural information is captured through hierarchical voxelization, enabling coarse-to-fine correspondence matching. This is further coupled with a dynamic-threshold Iterative Closest Point refinement stage and a global initial alignment module that accommodates large pose discrepancies. We evaluate the proposed method on 15 forest plots, including 10 UAV-terrestrial LiDAR plots from a public dataset and 5 UAV-backpack LiDAR plots collected in this study, spanning diverse forest structures, terrain conditions, and initial misalignments. The results demonstrate consistent robustness and accuracy, achieving mean translation and rotation errors of 5.23 cm and 0.13° for UAV-terrestrial data, and 17.00 cm and 0.29° for UAV-backpack data. These results highlight voxel-based statistical representation as an effective and generalizable approach for cross-platform point cloud registration in structurally complex environments.

</details>


### [447] [Aboveground woody biomass regression and uncertainty estimation from spaceborne lidar and optical datasets](https://doi.org/10.1080/15481603.2026.2707801)
*H. Schmidt,Javier M. Osorio Leyton,Sorin Popescu,Horia G. Olariu,X. Ben Wu*
...
### [448] [Nonlinear responses of net ecosystem productivity to heatwaves over the Qinghai–Tibetan Plateau: insights from climate and vegetation](https://doi.org/10.1080/15481603.2026.2728311)
*Hongzhe Li,Yi He,Qing Zhu,Lifeng Zhang,Li Quan*
...
Abstract (英文): Global climate warming has intensified heatwave disasters over the Qinghai-Tibetan Plateau (QTP), exerting persistent and far-reaching impacts on the regional carbon cycle, which has attracted extensive attention from the scientific community. However, the spatiotemporal variation patterns of heatwaves over the QTP and their impacts on net ecosystem productivity (NEP) remain unclear. In this study, three heatwave indices – heatwave magnitude index (HWMI), heatwave days (HWD), and heatwave frequency (HWF) – were constructed to develop a three-level analytical framework across interannual, climatic, and vegetation dimensions. This framework was used to investigate the spatiotemporal characteristics of heatwaves over the QTP from 2003 to 2022 and their effects on the NEP. The results show that (1) heatwaves over the QTP exhibit an overall increasing trend with interannual fluctuations, with semi-arid meadow and steppe ecosystems in the southwestern QTP identified as core regions of heatwave intensification. However, QTP ecosystems appear to maintain relatively stable responses under short-duration, high-frequency heatwave disturbances. (2) The NEP over the QTP exhibited decreasing gradients from north to south and from east to west. NEP responses to heatwaves exhibited distinct spatial patterns across vegetation types, with stronger declines observed in the western arid meadow and steppe regions and the southern humid forest regions. Both extreme short-term heatwaves and prolonged heatwave events are strongly associated with NEP reductions. (3) HWMI shows a significant nonlinear negative relationship with NEP and exhibits higher sensitivity to NEP variations compared with HWD and HWF. Along climatic and vegetation gradients, the relative importance of heatwave metrics in explaining NEP variability shifts from HWMI to HWD and HWF. Therefore, attention should be paid to the potential threats of abrupt warming events to the carbon sink function of alpine ecosystems, with enhanced monitoring of abnormal high air temperatures and the implementation of heat mitigation measures. This study provides a theoretical basis and technical support for ecological conservation and disaster risk mitigation on the QTP.

</details>


### [449] [MambaMPD: a mamba-driven segmentation framework for marine pollution detection from remote sensing imagery](https://doi.org/10.1080/15481603.2026.2730893)
