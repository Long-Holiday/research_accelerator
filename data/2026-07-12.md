<div id=toc></div>

# Table of Contents

- [cs.CV](#cs.CV) [Total: 72]
- [GIScience & RS](#GIScience & RS) [Total: 6]
- [RSE](#RSE) [Total: 2]
- [JAG](#JAG) [Total: 11]
- [cs.AI](#cs.AI) [Total: 56]
- [Remote Sensing](#Remote Sensing) [Total: 127]
- [ISPRS J P&RS](#ISPRS J P&RS) [Total: 4]


<div id='cs.CV'></div>

# cs.CV [[Back]](#toc)

### [1] [DreamCharacter-1: From 3D Generative Foundation Models to Product-Ready Character Generation](https://arxiv.org/abs/2607.07817)
*Weizhe Liu,Yunjie Wu,Xiangqian Shu,Guangwei Wang,Xiangyu Xu,Peng Li,Yujie Li,Hengkai Guo*

Main category: cs.CV

TL;DR: DreamCharacter-1是一个轻量级后适应框架，通过对预训练3D基础模型进行几何后训练、纹理后训练和推理加速三个模块的校准，实现了高保真、可投入生产的3D角色生成，在定量和定性实验中均超越了现有最先进方法。


<details>
  <summary>Details</summary>
Motivation: 现有3D基础模型在生成高保真、生产级3D角色时仍存在表面细节不足、纹理质量差和推理效率低等问题，亟需一种轻量级的后适应方案来校准预训练模型，使其达到工业级应用标准。

Method: DreamCharacter-1包含三个核心组件：(1)几何后训练——通过几何偏好优化增强细粒度表面细节；(2)纹理后训练——合成高分辨率纹理并优化遮挡区域的外观；(3)推理加速——支持可扩展的部署推理。

Result: 定量和定性实验表明，DreamCharacter-1生成的3D角色资产在视觉吸引力和结构鲁棒性方面均持续超越当前最先进的角色生成方法。

Conclusion: DreamCharacter-1作为一种轻量级后适应框架，能有效校准预训练3D基础模型，使其生成工业级高质量的3D角色，在性能上全面超越现有方法。

与遥感交叉或改进方案: 交叉/改进可行性：85%。具体方案：(1)将几何后训练中的几何偏好优化引入遥感三维重建，用于增强地形和建筑物的细粒度表面细节；(2)纹理后训练中的遮挡区域外观优化可用于遥感影像中缺失或遮挡区域的纹理修复；(3)推理加速模块可直接部署于遥感大场景的快速3D重建任务，实现可扩展的大规模遥感三维模型生产。

Abstract (英文): We present DreamCharacter-1, a lightweight post-adaptation framework that calibrates pretrained 3D foundation models toward high-fidelity, production-ready 3D character generation. Building upon a 3D foundation backbone, our pipeline incorporates three task-oriented components: (1) geometry post-training, which enhances fine-grained surface details through geometric preference optimization; (2) texture post-training, which synthesizes high-resolution textures and refines the appearance of occluded regions; and (3) inference acceleration, which enables scalable deployment. Extensive quantitative and qualitative experiments demonstrate that DreamCharacter-1 produces visually compelling and structurally robust 3D character assets, consistently surpassing state-of-the-art character generation methods.

</details>


### [2] [GIRAF: Towards Generalizable Human Interactions with Articulated Objects](https://arxiv.org/abs/2607.07880)
*Xiaohan Zhang,Sebastian Starke,Alexander Winkler,Federica Bogo,Samir Aroudj,Yuting Ye*

Main category: cs.CV

TL;DR: 本文提出一种文本条件扩散模型，通过物体中心表示、混合域训练策略和基于接触的数据增强，实现了全身与可动关节物体的协调交互，在泛化性上超越现有方法。


<details>
  <summary>Details</summary>
Motivation: 现有方法局限于静态物体或仅手部操作，无法生成协调的全身运动来实现接近、操控和移动可动关节物体，且缺乏大规模配对数据。

Method: 1) 物体中心表示：统一手-物接触与物体表面；2) 混合域训练策略：平衡导航与操控动作；3) 基于接触的数据增强：扩展训练多样性。

Result: 在未见过的物体配置上表现出强泛化能力，超越当前最优方法。

Conclusion: 物体中心表示与混合域训练能有效解决全身运动与可动物体交互的联合推理问题，数据增强进一步提升了泛化性。

与遥感交叉或改进方案: 交叉/改进可行性：35%。本文聚焦人体运动生成与物体交互，与遥感领域直接交叉有限。潜在交叉方向：1) 将物体中心表示思想迁移到遥感中的地物-目标交互建模（如卫星与地面目标的运动关系）；2) 混合域训练策略可用于遥感中导航（轨迹规划）与操控（传感器指向）的联合学习；3) 接触增强思想可类比为遥感数据中不同光照/视角条件下的地物接触关系增强。但整体相关性较低，需大量领域适配。

Abstract (英文): Synthesizing realistic full-body human interactions with articulated objects is a fundamental challenge for embodied AI and graphics, with applications in robotics training and virtual agents. Existing models remain limited: some focus on simple activities with static objects, while others restrict attention to hand-only manipulation. This leaves open the problem of generating coordinated full-body motion that approaches, manipulates, and moves articulated objects in a realistic and generalizable way. The key difficulty lies in reasoning jointly about locomotion, fine-grained contact, and object articulation. Models must capture subtle hand-object correspondences that transfer across object geometries, while also producing seamless transitions from navigation to manipulation. At the same time, the scarcity of large-scale paired motion-scene data makes it difficult to generalize across diverse object positions and shapes. We introduce a text-conditioned diffusion model that addresses these challenges through three core ideas: an object-centric representation that unifies hand-object contact with object surfaces, a mixed-domain training strategy that balances locomotion and interaction, and a contact-based augmentation scheme that expands training diversity. Through experiments, our method demonstrated strong generalization to unseen object configurations, surpassing current state-of-the-art methods.

</details>


### [3] [3D Reconstruction of deciduous Trees using low-cost UAV- and Crane-based Photogrammetry for Monitoring Shoot Elongation across entire Canopies](https://arxiv.org/abs/2607.07905)
*Stephan Nebiker,Micha Tschanz,Nando Amport,Frederik Baumgarten*

Main category: cs.CV

TL;DR: 本文利用低成本无人机摄影测量和多相机CraneCam系统，在真实条件下对原生落叶树进行3D重建，以监测整个树冠的枝条伸长（初级生长）。研究在两个样地完成整个生长季的数据采集，实现了5-6 mm的3D点精度和92%-98%的重建完整度，并引入了3D打印的地面真值分支用于评估细部结构重建能力。


<details>
  <summary>Details</summary>
Motivation: 目前树木径向生长（次级生长）的连续测量已较为成熟，但枝条伸长（初级生长）的监测因缺乏合适技术而长期被忽视，导致气候变化对树木初级生长的影响尚不明确。因此，亟需一种能够对原生落叶树进行3D重建的方法，以测量和监测整个树冠的枝条伸长。

Method: 使用低成本消费级无人机（<250 g）摄影测量和多相机CraneCam系统，在两个研究区对整个生长季进行数据采集。通过摄影测量处理生成3D点云，并从精度、分辨率和完整度三个方面评估点云质量。引入3D打印的地面真值分支来评估细部结构（如细枝）的重建能力。

Result: 消费级无人机可实现5-6 mm的树木3D点精度，重建完整度在92%-98%之间（取决于无人机类型）。3D打印地面真值分支有效评估了细枝结构的重建性能。

Conclusion: 低成本无人机摄影测量能够以毫米级精度和高完整度实现树木3D重建，为监测树木初级生长提供了可行方案。论文还讨论了操作挑战以及基于摄影测量点云的树木骨架化初步实验。

与遥感交叉或改进方案: 交叉/改进可行性：85%。该论文的方法可直接应用于遥感领域的树木生长监测。具体交叉方案：（1）将低成本无人机摄影测量与卫星遥感（如Landsat、Sentinel-2）结合，实现从个体树木到区域尺度的初级生长监测尺度转换；（2）利用时序点云数据提取树木表型参数（树高、冠幅、枝长等），与遥感植被指数（NDVI、EVI等）建立关联模型；（3）引入LiDAR数据与摄影测量点云融合，弥补摄影测量在穿透冠层方面的不足，提升树木三维结构重建的完整度；（4）将骨架化方法推广至森林样地尺度，结合遥感影像实现大范围树木生长动态监测。

Abstract (英文): Tree growth determines how much CO2 is sequestered from the atmosphere and temporarily stored in woody biomass. At the same time tree growth is affected by increasing temperatures, more frequent drought periods, late frosts and other extreme events associated with climate change. While continuous measurements of radial (secondary) tree growth using dendrometers are well established, monitoring of shoot elongation (primary growth) has largely been neglected because suitable measurement techniques are lacking. As a result, the effects of climate change on primary tree growth remain insufficiently understood. This work aims at reconstructing native deciduous trees in 3D as a basis for measuring and monitoring shoot elongation over entire tree canopies. Here we explored the use of low-cost UAV photogrammetry and of a multi-camera CraneCam system under real-world conditions. Data were collected in two study areas over an entire growing season. We present sensor evaluations, photogrammetric data acquisition and processing strategies. A special focus is placed on the analysis of the resulting photogrammetric 3D point clouds in terms of accuracy, resolution and completeness. Results demonstrate 3D point accuracies of 5-6 mm for entire trees using consumer-grade UAVs weighing less than 250 g and a 3D reconstruction completeness between 92% and 98% depending on the UAV type. The paper introduces a novel 3Dprinted ground-truth branch to evaluate the capability to reconstructing fine-detail structures such as thin tree shoots. Finally, we discuss operational challenges and initial experiments towards a skeletonization of entire trees based on photogrammetric point clouds.

</details>


### [4] [Adversarial Decoys: Misdirecting Attention-Based Defenses in ViT](https://arxiv.org/abs/2607.07922)
*Giulia Marchiori Pietrosanti,Giulio Rossolini,Giorgio Buttazzo*

Main category: cs.CV

TL;DR: 本文提出对抗性诱饵（adversarial decoys），一种独立优化的图像块，通过操控注意力排名绕过基于注意力幅度的测试时防御，揭示使用注意力大小作为对抗相关性指标的根本局限性。


<details>
  <summary>Details</summary>
Motivation: 现有基于注意力分数抑制的防御方法利用对抗性token需吸引大量注意力才能影响预测这一强耦合关系，但作者质疑这种依赖注意力幅度的防御是否可靠，旨在揭示其根本局限性。

Method: 将攻击目标解耦为两个独立部分：原始对抗区域负责误导预测，独立的对抗诱饵负责操纵防御所用的注意力排名。采用逐层目标函数提升目标token注意力并使其超过非目标竞争token。诱饵独立于底层攻击优化，因此可与任意现有对抗补丁攻击集成。

Result: 在ImageNet上针对多种ViT架构和攻击的实验表明，诱饵能将高注意力分数从真实对抗区域转移开，同时保持大部分攻击有效性。

Conclusion: 对抗诱饵成功绕过基于注意力幅度的防御，揭示了使用注意力大小作为对抗相关性指标存在根本性缺陷。

与遥感交叉或改进方案: 交叉/改进可行性：85%。遥感领域ViT模型（如用于场景分类、目标检测的模型）同样易受对抗补丁攻击。可将对抗诱饵机制引入遥感防御评估：在遥感图像中设置对抗补丁攻击地物分类器，同时部署诱饵（如伪装成自然地物的图像块）来规避基于注意力机制的防御。此外，遥感图像的复杂背景有助于隐藏诱饵，增强攻击隐蔽性。反过来，遥感防御研究可针对此攻击开发更鲁棒的注意力评估方法。

Abstract (英文): Vision Transformers (ViTs) remain vulnerable to localized adversarial attacks, e.g., adversarial patches, while recent test-time defenses mitigate them by suppressing image tokens with abnormally high attention scores. These defenses exploit a strong coupling between attention and adversarial effectiveness: adversarial tokens often need to attract substantial attention to influence the prediction. We introduce adversarial decoys, independently optimized image patches that redirect the attention, and therefore related defenses, toward selected target tokens. Rather than jointly optimizing misclassifications and defense evasion, our approach decouples the two objectives: the original adversarial region induces the incorrect prediction, while a separate decoy manipulates the attention ranking used by the defense. A layer-wise objective increases target-token attention and promotes these tokens above competing non-target ones. Since the decoy is optimized independently of the underlying attack, the method is attack-agnostic and can be easily integrated with any existing adversarial patch attack. Experiments on ImageNet across multiple ViT architectures and attacks show that decoys can redirect high attention scores away from the true adversarial region while preserving much of the attack effectiveness. These results reveal a fundamental limitation of using attention magnitude as an indicator of adversarial relevance.

</details>


### [5] [Beyond Thermal Imaging: Inferring Thermophysical Properties from Time-Resolved Thermal Observations](https://arxiv.org/abs/2607.07962)
*Chenghao Xu,Malcolm Mielle,Olga Fink*

Main category: cs.CV

TL;DR: ThermoField是一个通过可微分热传导仿真统一热场景重建与热物理参数估计的框架，将热物理量表示为空间变化的神经场，受场景几何、热传导物理和时序热观测约束，能联合重建几何、估计空间变化的热扩散率并预测未知环境下的热演化。


<details>
  <summary>Details</summary>
Motivation: 现有热场景重建方法能恢复3D环境中的温度场但无法识别控制热演化的热物理参数，而逆物理方法虽能提供可解释的参数估计却依赖简化的几何和受控实验条件。缺乏统一框架同时进行几何重建、热物理参数估计和预测性热模拟。

Method: 将空间变化的热物理量（如热扩散率）和温度场表示为神经场，通过场景几何约束和可微分热传导求解器，联合优化神经场参数以匹配时序热观测数据，实现物理驱动的端到端学习。

Result: ThermoField能联合重建场景几何、估计空间变化的热扩散率，并在未见过的环境条件下预测热演化，实现复杂3D场景中物理可解释的参数推断。

Conclusion: 该工作桥接了热场景重建与逆热传导分析，为从热观测中同时进行几何重建、热物理参数估计和预测性热模拟提供了统一框架。

与遥感交叉或改进方案: 交叉/改进可行性：85%。具体方案：(1) 将该框架应用于遥感热红外影像，反演地表热扩散率和热惯量等关键热物理参数，提升地表物质成分识别的物理可解释性；(2) 结合卫星多时相热红外数据与DSM几何信息，实现大规模城市区域的热物理参数估计与热环境演化预测；(3) 可微分热传导求解器可扩展为考虑辐射、对流等多物理场耦合，用于建筑物热损失评估和城市热岛效应分析。

Abstract (英文): Inferring latent physical properties from sensory observations is a fundamental challenge in machine perception. Among available sensing modalities, thermal imaging is particularly promising because temperature evolution is directly governed by heat-transfer physics and therefore encodes information about underlying thermophysical properties of a scene. Recovering spatially resolved thermophysical properties from thermal observations could transform applications ranging from digital twins and infrastructure monitoring to robotics and scientific imaging. However, existing thermal scene reconstruction methods can recover temperature fields in complex 3D environments without identifying the thermophyiscal properties that govern thermal evolution, whereas inverse methods provide physically interpretable parameter estimation but typically rely on simplified geometries and controlled experimental conditions.
  Here we introduce ThermoField, a framework that unifies thermal scene reconstruction and thermophysical parameter estimation through differentiable heat-transfer simulation. The proposed framework represents these quantities as spatially varying neural fields and constrains them through scene geometry, governing heat-transfer physics, and temporal thermal observations. We demonstrate that ThermoField jointly reconstructs geometry, estimates spatially varying thermal diffusivity, and predicts thermal evolution under previously unseen environmental conditions. By integrating neural scene representations with differentiable heat-transfer solver, the framework enables physically interpretable parameter inference in complex 3D scenes. Our results establish a bridge between thermal scene reconstruction and inverse heat-transfer analysis, providing a unified approach for geometry reconstruction, thermophysical property estimation, and predictive thermal simulation from thermal observations.

</details>


### [6] [LOGOS: Language-guided Oriented Object Detection in Aerial Scenes](https://arxiv.org/abs/2607.08004)
*Trong-Thuan Nguyen,Minh-Triet Tran*

Main category: cs.CV

TL;DR: LOGOS是一种基于Transformer的文本提示引导定向目标检测方法，通过在DOTA数据集上的实验证明其在密集和旋转目标场景下优于现有方法。


<details>
  <summary>Details</summary>
Motivation: 遥感图像中的定向目标检测面临角度不连续性、固定查询尺寸以及稀疏/杂乱场景处理效率低等问题，现有方法难以有效应对。

Method: 提出LOGOS框架，采用Transformer架构，利用文本提示（textual prompts）调制内容查询（prompt-modulated content queries），根据提供的文本动态调整模型关注重点，从而提升复杂环境下的目标检测精度。

Result: 在DOTA数据集上的大量实验表明，LOGOS在密集排列和旋转目标场景中优于现有最先进方法。

Conclusion: LOGOS显著提升了遥感应用中定向目标检测的鲁棒性和可扩展性。

与遥感交叉或改进方案: 交叉/改进可行性：90%。作为遥感定向目标检测论文，改进方向包括：(1) 引入多模态融合策略，将文本提示扩展为视觉-语言联合表征；(2) 优化查询调制机制，支持动态可学习的文本提示而非固定输入；(3) 结合旋转框回归损失（如KLD、GWD）以进一步提升角度预测精度；(4) 在更多遥感数据集（如HRSC2016、DIOR-R）上验证泛化能力。

Abstract (英文): Object detection in geospatial scenes, such as satellite and aerial imagery, poses significant challenges due to the varying orientations and densities of objects, as well as the complex backgrounds inherent to remote sensing imagery. Traditional methods for oriented object detection have struggled to address issues such as angular discontinuity, fixed query sizes, and inefficiencies in handling sparse or cluttered scenes. In this paper, we propose LOGOS, a novel transformer-based approach that leverages textual prompts to guide the detection of oriented objects in aerial scenes. In particular, our proposed approach incorporates prompt-modulated content queries to dynamically adjust the model's focus based on the provided text, thereby improving object detection accuracy in complex environments. Empirically, extensive experiments on the DOTA dataset demonstrate that LOGOS outperforms existing state-of-the-art methods, particularly in densely packed and rotated object scenarios. Our approach offers a significant step forward in improving the robustness and scalability of oriented object detection in remote sensing applications.

</details>


### [7] [FedTR: Federated Learning Framework with Transfer Learning for Industrial Visual Inspection](https://arxiv.org/abs/2607.08014)
*Vikash Sathiamoorthy,Shuo Huai,Hao Kong,Di Liu,Wendy Yong Yi Loy,Christian Makaya,Daren Ho,Ravi Subramaniam,Qian Lin,Weichen Liu*

Main category: cs.CV

TL;DR: FedTR是一个结合迁移学习的联邦学习框架，用于工业视觉检测中的标签缺陷识别。先在公开数据集上预训练，再在分布式私有数据上微调，在同质和异质数据上分别达到95.5%和94.2%的端到端文本识别词级准确率。


<details>
  <summary>Details</summary>
Motivation: 联邦学习在工业视觉检测中面临数据有限和检测任务复杂的问题，导致模型性能受限。本文旨在通过引入迁移学习来提升FL在工业标签缺陷检测中的效果。

Method: 提出FedTR框架，先利用公开数据集训练模型，再在分布式且有限的私有数据上进行联邦学习与模型微调，实现端到端文本识别标签缺陷检测。

Result: 在墨盒数据集上，FedTR在同质数据上达到95.5%词级准确率，在异质数据上达到94.2%，性能与集中式训练相当。

Conclusion: FedTR有效解决了联邦学习在工业视觉检测中数据有限的问题，验证了迁移学习与联邦学习结合在标签缺陷识别中的可行性和有效性。

与遥感交叉或改进方案: 交叉/改进可行性：85%。FedTR的联邦迁移学习框架可直接应用于遥感场景中的分布式标签缺陷检测（如卫星影像上的文字标注错误识别）。具体方案：1）利用公开遥感数据集（如ISPRS）预训练文本识别模型；2）在多个遥感机构的分布式私有数据上进行联邦微调，检测图像中的标签或注释缺陷；3）适用于多源遥感数据（异质性强）场景，FedTR的异质数据能力（94.2%准确率）表明其能有效应对不同传感器、不同分辨率的遥感数据分布差异。

Abstract (英文): Federated learning (FL) is a collaborative learning scheme to train deep learning models, where collaborating parties can consolidate their models without sharing local data with other parties, hence preserving data privacy. Nevertheless, when implementing FL in Industrial visual inspection (IVI), the constraints posed by limited data availability and the intricate nature of the inspection tasks significantly impact the performance of the resulting model. This paper introduces FedTR, a novel FL framework incorporating transfer learning designed for Autonomous IVI, focusing on the challenging task of identifying label defects through end-to-end text recognition. Transfer learning is a method that leverages the knowledge of a pre-trained model to adapt to a different dataset. FedTR initially trains the model using a publicly available dataset, after which performs the essential federated learning process with model fine-tuning on the distributed and limited private data. Extensive experiment results demonstrate the effectiveness and feasibility of FedTR on private ink cartridge datasets for label defect identification. FedTR achieves an end-to-end text recognition word-level accuracy of 95.5% and 94.2% on homogeneous and heterogeneous data respectively. Additionally, it attains performance levels that are on par with those achieved through centralized training.

</details>


### [8] [LightCrafter: PBR-Conditioned Video Diffusion Refinement for Controllable and Consistent Relighting](https://arxiv.org/abs/2607.08016)
*Zixin Guo,Yehonathan Litman,Yifeng He,John Miller,Chuhan Chen,Deva Ramanan*

Main category: cs.CV

TL;DR: LightCrafter提出了一种混合管道，将视频重打光重新定义为对代理视频（PBR渲染结果）的视频翻译任务。通过将目标照明信息烘焙到PBR代理中，避免扩散模型直接学习照明概念，从而在保持长时间时间一致性的同时实现更精细的照明控制，并在真实世界和合成基准上达到SOTA性能。


<details>
  <summary>Details</summary>
Motivation: 现有视频重打光方法存在两类范式的局限：(1) 基于逆渲染+前向渲染的方法受限于噪声重建和难以建模全局光照等效果；(2) 基于生成式视频到视频翻译的方法受限于扩散模型处理长视频的时间稳定性以及输入/重打光训练对的可用性。需要一种能够同时兼顾物理准确性和长时间一致性的方法。

Method: LightCrafter采用混合管道，核心思想是将视频重打光定义为对代理视频的视频翻译：不直接将输入视频翻译到目标照明，而是先将输入视频在目标照明下进行PBR渲染得到代理视频，然后训练扩散模型（基于CogVideoX后训练）将该PBR代理视频翻译为最终目标视频。PBR渲染嵌入了照明目标，免去了扩散模型学习环境贴图等照明概念的需求，同时提供长时间一致性；扩散模型则弥补PBR难以建模的全局光照等效果。训练使用合成视频对和真实世界无配对视频。

Result: 在现有真实世界重打光基准上超越了之前的最先进方法，并贡献了一个用于进一步分析的合成基准。

Conclusion: PBR渲染本身已能超越部分先前工作，但难以处理全局光照等效果；结合扩散模型的光度先验后，LightCrafter在视频重打光任务上实现了更优的性能和更精细的控制能力。

与遥感交叉或改进方案: 交叉/改进可行性：85%。遥感视频（如卫星视频或多光谱视频）同样面临光照变化问题（如太阳角度变化、云影等）。具体交叉方案：(1) 将PBR代理视频思想引入遥感视频光照归一化：利用已知的太阳位置和地形数据对遥感帧进行PBR渲染（代理视频），再通过扩散模型修正非朗伯体效应（如水体镜面反射、城市建筑玻璃反光）；(2) 利用LightCrafter的合成训练策略生成不同太阳角度下的配对遥感数据，训练模型实现跨时相遥感视频的光照一致性增强；(3) 将环境贴图替换为太阳-天空模型（如Perez分布），实现更为物理准确的多角度遥感视频重打光。

Abstract (英文): Video relighting requires balancing long-form temporal consistency with a physically grounded understanding of light transport, which depends on accurate estimation of intrinsic scene properties such as materials, geometry, and illumination. Existing methods follow two paradigms: (1) reconstruct a video's photometric properties via inverse rendering and relight them to a target illumination via forward rendering, using physically-based rendering (PBR) or a neural renderer; these suffer from noisy reconstructions and struggle with hard-to-model effects such as global illumination. (2) Frame the task as generative video-to-video translation conditioned on relighting targets (a target environment map or text); this limits relighting control and temporal stability, since diffusion models struggle to translate long-form videos, and is constrained by the availability of input/relit training pairs. We propose LightCrafter, a hybrid pipeline that reformulates video relighting as video translation of a proxy video: rather than translating the input video directly to the target, we translate a PBR rendering of the input under the target illumination to the final target. This bakes illumination targets into the PBR proxy, removing the need to teach the diffusion model illumination concepts like environment maps, and enables more intricate lighting control while naturally providing long-form temporal consistency. We show PBR renders alone already outperform some prior art but struggle with effects like global illumination; to capture these, we leverage photometric priors in video generation models by post-training CogVideoX on synthetic video pairs and real-world unpaired videos. We outperform prior state-of-the-art on existing real-world relighting benchmarks and contribute a synthetic benchmark for further analysis. We will release our dataset, benchmark, metrics, and code.

</details>


### [9] [SAGA: Stable Acceleration Guidance for Autoregressive Video Generation](https://arxiv.org/abs/2607.08020)
*Thanh-Nhan Vo,Trong-Thuan Nguyen,Trung-Hoang Le,Tam V. Nguyen,Minh-Triet Tran*

Main category: cs.CV

TL;DR: 本文提出SAGA，一种无需训练的稳定加速引导方法，通过频谱运动学分析解决自回归视频扩散中的时间误差累积问题。SAGA利用有限窗口Slepian投影的加速度域频谱引导目标和结构化自回归噪声初始化策略，在不重新训练或修改骨干网络的情况下，有效抑制高频时间扰动，减少闪烁、运动抖动和结构漂移，提升生成视频的时间质量与图像质量。


<details>
  <summary>Details</summary>
Motivation: 自回归视频扩散在高效流式传输和长程视频生成中具有优势，但重复复用已生成的潜在变量作为因果上下文会放大时间误差，导致闪烁、运动抖动和结构漂移等问题。现有方法未能从频谱运动学角度有效解决这一失败模式。

Method: SAGA包含两个核心组件：(1) 基于有限窗口Slepian投影的加速度域频谱引导目标，用于检测并抑制不稳定的高频时间扰动；(2) 结构化自回归噪声初始化策略，抑制短程时间相关性同时保留长程运动结构。该方法无需训练或修改骨干网络，可直接应用于现有的分块自回归扩散模型。

Result: 在Self-Forcing模型上，SAGA将时间质量从97.30提升至97.91，图像质量从69.60提升至70.51。频谱分析和人类偏好研究表明，SAGA在保持视觉保真度的同时有效减少了时间不稳定性。

Conclusion: SAGA是一种训练自由的稳定加速引导方法，无需重新训练或修改模型架构即可有效提升自回归视频扩散模型的时间质量，在多个模型上展现出一致性的改进效果。

与遥感交叉或改进方案: 交叉/改进可行性：85%。遥感卫星视频（如卫星云图、航空遥感视频）存在长时序、运动缓慢但需要高时间一致性的特点。SAGA的加速度域频谱引导方法可应用于：(1) 遥感视频超分辨率与去噪任务，抑制因遥感器噪声或压缩伪影引起的高频闪烁；(2) 遥感时序预测（如云层运动预测、植被变化监测），利用结构化噪声初始化策略保留长程物理运动结构；(3) 遥感视频中的异常检测（如森林火灾、冰山漂移），通过频谱分析识别异常高频扰动。需注意遥感视频的采样频率通常较低，需调整Slepian窗口参数以适应不同时间尺度。

Abstract (英文): Autoregressive video diffusion enables efficient streaming and long-horizon video generation, but repeatedly reusing generated latents as causal context can amplify temporal errors, resulting in flickering, motion jitter, and structural drift. In this paper, we investigate this failure mode from a spectral kinematic perspective and identify discrete latent acceleration as an effective signal for revealing unstable high-frequency temporal perturbations. To this end, we propose SAGA, a training-free \textbf{\textit{s}}table \textbf{\textit{a}}cceleration \textbf{\textit{g}}uidance approach for \textbf{\textit{a}}utoregressive video generation. SAGA integrates an acceleration domain spectral guidance objective based on finite-window Slepian projections with a structured autoregressive noise initialization strategy that suppresses short-range temporal correlations while preserving long-range motion structure. Without retraining or modifying the backbone, SAGA can be directly applied to existing chunk-wise autoregressive diffusion models, which is the prevalent setting for high-quality generation. Extensive experiments show that SAGA consistently improves temporal quality across multiple autoregressive diffusion models. On Self-Forcing, SAGA improves Temporal Quality from 97.30 to 97.91 and Image Quality from 69.60 to 70.51. Moreover, spectral analysis and human preference studies demonstrate that SAGA reduces temporal instability while maintaining visual fidelity.

</details>


### [10] [APIVOT: Adaptive Planning with Interleaved Vision-Language Thoughts](https://arxiv.org/abs/2607.08024)
*Emily Jin,Joy Hsu,Yiqing Xu,Weiyu Liu,Nick Haber,Jiajun Wu*

Main category: cs.CV

TL;DR: 本文提出APIVOT，一种基于VLM的自适应交错语言与视觉思维的长时域机器人规划器，在厨房任务中优于通用VLM和先前规划框架，尤其在空间受限场景中表现显著提升。


<details>
  <summary>Details</summary>
Motivation: 长时域机器人规划需要同时推理语义任务结构和几何可行性，现有方法难以有效结合两者，特别是空间约束下的规划成功率低。

Method: APIVOT是一种VLM-based规划器，自适应地交错使用语言推理（语义推理）和视觉思维（作为想象的未来状态进行内部验证），在规划过程中动态选择模态以平衡推理效率与成功率。

Result: 在长时域厨房任务中，APIVOT优于通用VLM和先前规划框架，在空间受限场景中增益最大，且学习到了有意义的模态选择行为。

Conclusion: 自适应交错视觉-语言思维能同时提升规划成功率和推理效率，验证了多模态动态融合在机器人长时域规划中的有效性。

与遥感交叉或改进方案: 交叉/改进可行性：85%。遥感领域的长时序任务规划（如卫星任务编排、无人机航线规划）面临类似挑战——需要语义理解（任务目标）与几何约束（禁飞区、资源限制）的联合推理。可将APIVOT的自适应模态交错策略引入遥感规划：语言分支处理任务语义（如"监测某区域植被变化"），视觉分支生成仿真的未来遥感影像作为内部验证，检查地理空间约束（如云层遮挡、传感器视场角限制、路径碰撞）。需解决的主要适配问题是：遥感场景的视觉状态空间远大于机器人操作场景，需引入更高效的视觉思维生成（如条件扩散模型生成卫星视角未来帧）。

Abstract (英文): Long-horizon robot planning requires jointly reasoning over semantic task structure and geometric feasibility. To successfully execute a task, a robot must decompose goals, select task-relevant objects, and sequence actions, while ensuring that plans satisfy spatial constraints such as limited free space and object collisions. In this work, we propose APIVOT, a VLM-based planner that adaptively interleaves language and visual thoughts for long-horizon planning. APIVOT learns to leverage language for semantic reasoning, while using visual thoughts as imagined future states for internal verification of geometric feasibility. On long-horizon kitchen tasks, APIVOT outperforms general-purpose VLMs and prior planning frameworks, achieving the largest gains in spatially constrained settings. We find that APIVOT learns meaningful modality selection behavior, demonstrating that adaptive interleaving of vision-language thoughts improves both planning success and reasoning efficiency.

</details>


### [11] [Post-Training in End-to-End Autonomous Driving](https://arxiv.org/abs/2607.08072)
*Ruining Yang,Muxing Wang,Yixiao Chen,Tongfei Guo,Yi Xu,Can Cui,Zichong Yang,Yitian Zhang,Ziran Wang,Yun Fu,Lili Su*

Main category: cs.CV

TL;DR: 本文综述了自动驾驶领域中端到端模型（如视觉-语言-动作模型和轨迹生成规划器）的后训练技术。由于传统开环模仿学习无法满足安全关键和交互密集环境的需求，该领域正转向后训练方法以进一步优化驾驶策略。论文将现有文献按监督形式分为四大类，并讨论了每类的能力、局限性和开放性挑战。


<details>
  <summary>Details</summary>
Motivation: 自动驾驶端到端模型在安全关键和交互密集环境中，传统开环模仿专家轨迹的方法存在不足：小误差随时间累积、训练数据缺乏恢复行为、点状标签无法捕捉安全与舒适等长程目标。这些局限促使研究转向后训练技术以进一步优化驾驶策略。

Method: 本文采用文献综述方法，首先明确后训练在自动驾驶中的定义与范围，然后按监督形式将现有文献组织为四大类，对每类进行能力、局限性和开放性挑战的系统分析。

Result: 构建了自动驾驶后训练的统一视角，将现有方法按监督形式分为四个主要家族，并系统总结了各类方法的能力与局限。

Conclusion: 后训练技术是弥补纯模仿学习不足的关键方向，有助于推动可靠、高效的自动驾驶策略优化，并呼吁未来在该领域进行更多研究。

与遥感交叉或改进方案: 交叉/改进可行性：75%。遥感领域中的时序预测（如地表变化检测、气象预测）和智能控制（如无人机路径规划）与自动驾驶后训练面临类似的模仿学习局限（误差累积、长程目标缺失）。可借鉴本文提出的四类监督形式后训练框架：1）将遥感时序预测模型（如LSTM、Transformer-based模型）引入强化学习或对抗训练进行后优化；2）利用遥感数据中的多模态信息（如光学影像+SAR+气象数据）构建类似视觉-语言-动作的端到端预测模型，并采用后训练策略提升长期预测可靠性；3）遥感中的长程变化检测可类比驾驶安全目标，采用基于轨迹或语义的奖励函数进行后训练微调。

Abstract (英文): End-to-end models that map multimodal inputs directly to future trajectories/maneuvers have emerged as an increasingly prominent research paradigm in autonomous driving. This class of models includes both Vision-Language-Action models and trajectory-generative planners. Unlike classic machine learning applications, autonomous vehicles operate in safety-critical and interaction-intensive environments where traditional open-loop imitation of expert demonstrations is not sufficient to ensure reliability. In particular, small execution errors can accumulate over time, while recovery behaviors are scarce in training data. In addition, long-horizon objectives such as safety and driving comfort are not captured by pointwise labels either. These limitations have motivated a shift toward post-training techniques, which further refine driving policies beyond pure imitation. This survey presents a unified view of post-training for autonomous driving by defining its scope and organizing the existing literature into four major families based on the form of supervision they use. For each family, we discuss its capabilities, limitations, and open challenges. We aim to facilitate a systematic understanding of this emerging area and stimulate future research on reliable and efficient post-training for autonomous driving.

</details>


### [12] [UAV-OVVIS: Unmanned Aerial Vehicles Also Need Open-Vocabulary Video Instance Segmentation](https://arxiv.org/abs/2607.08075)
*Mingyu Dou,Shi Qiu,Ming Hu,Yifan Chen,Zhe Sun*

Main category: cs.CV

TL;DR: 本文提出无人机开放词汇视频实例分割任务(UAV-OVVIS)及无需训练的AeroTrack框架，通过周期开放词汇检测、短片段掩码传播和跨片段身份统一，在9类8279轨迹的AeroVIS基准上显著优于现有方法。


<details>
  <summary>Details</summary>
Motivation: 现有无人机视频感知依赖预定义类别下的框级定位和轨迹关联，难以同时支持灵活查询和开放场景中的细粒度实例级动态理解。

Method: 提出AeroTrack无训练统一框架，核心包含三个模块：周期性开放词汇检测（利用视觉基础模型进行开放词汇目标检测）、短片段掩码传播（在短时窗内传播分割掩码）、跨片段身份统一（对齐不同片段中的实例身份）。基于该框架实例化5种变体，并构建含9类8279轨迹的AeroVIS评估基准。

Result: 在UAV场景下，AeroTrack显著优于现有通用视频实例分割方法，并展现出强大的开放词汇鲁棒性和泛化能力。

Conclusion: AeroTrack和AeroVIS作为UAV-OVVIS的统一框架和基准，可支持未来研究，代码和数据集已开源。

与遥感交叉或改进方案: 交叉/改进可行性：95%。该论文本身就是遥感（无人机视觉）领域的核心工作。改进方向：(1) 引入卫星遥感中的多模态数据（如SAR/红外）增强低光照或恶劣天气下的掩码传播鲁棒性；(2) 结合遥感场景中的地理先验（如道路网络、建筑物轮廓）优化身份跨片段关联；(3) 扩展到多无人机协同视角下的跨视角实例匹配与融合；(4) 利用遥感大模型（如SatMAE、SpectralGPT）替换通用视觉基础模型，提升对地物目标的开放词汇识别能力。

Abstract (英文): Unmanned Aerial Vehicle (UAV) videos are widely used in traffic monitoring, urban management, and emergency rescue. However, existing UAV video perception mainly relies on box-level localization and trajectory association under predefined categories, making it difficult to simultaneously support flexible queries and fine-grained instance-level dynamic understanding in open scenarios. To this end, we introduce a new task, UAV Open-Vocabulary Video Instance Segmentation (UAV-OVVIS), which discovers targets in UAV videos according to open-vocabulary queries and outputs instance-level segmentation trajectories with globally consistent identities. Considering the scarcity of instance-level annotations in UAV scenarios, we propose AeroTrack, a training-free unified framework. AeroTrack centers on periodic open-vocabulary detection, short-segment mask propagation, and cross-segment identity unification, reusing existing visual foundation models to enable UAV-OVVIS. Based on this framework, we instantiate five AeroTrack variants and construct AeroVIS, an evaluation benchmark for UAV-OVVIS containing 9 UAV object categories and 8,279 trajectories. Experiments show that AeroTrack substantially outperforms existing general video instance segmentation methods in UAV scenarios and demonstrates strong open-vocabulary robustness and generalization. To support future research, we release AeroTrack and AeroVIS as a unified framework and benchmark for UAV-OVVIS.

</details>


### [13] [LDFE: Laplacian Decoupled Feature Enhancement Block for Dual-Stream CNN-based RGB-IR Object Detection](https://arxiv.org/abs/2607.08076)
*Wenhao Dong,Xiaoyan Luo,Linlin Yang,Haodong Zhu,Xiaorong Shi,Guodong Guo,Baochang Zhang*

Main category: cs.CV

TL;DR: 本文提出拉普拉斯解耦特征增强模块(LDFE)，通过全局-局部分解、去噪、融合和重建，利用状态空间模型和跨模态注意力机制，在六个基准数据集上超越SOTA方法2.0%-6.2%mAP。


<details>
  <summary>Details</summary>
Motivation: 现有基于YOLO的双流CNN骨干网络在RGB和IR图像融合检测中，缺乏同时考虑模态特性和结构特性的有效特征融合机制，尤其是在噪声抑制和长程依赖捕获方面存在不足。

Method: 提出LDFE模块：首先基于拉普拉斯金字塔将特征分解为全局和局部分量；全局分支采用GS2E模块，通过双分支架构让主/辅模态交替，利用辅模态的跨模态注意力抑制主模态噪声，并使用状态空间模型捕获长程依赖；局部分支采用LC2E模块，通过空间和通道维度的三重卷积提取细粒度细节并抑制噪声；最后进行特征重建。

Result: 在M3FD、DroneVehicle、LLVIP、FLIR-Aligned、KAIST和VEDAI六个数据集上，mAP分别超越SOTA方法6.2%、3.7%、4.7%、2.3%、4.1%和2.0%。

Conclusion: LDFE通过全局-局部解耦、跨模态噪声抑制和状态空间模型的长程依赖捕获，有效提升了RGB-IR多模态目标检测性能，验证了在极端光照条件下多模态融合的有效性。

与遥感交叉或改进方案: 交叉/改进可行性：85%。该论文已在DroneVehicle和VEDAI遥感数据集上验证性能。改进方向：(1)将LDFE中的拉普拉斯金字塔替换为遥感图像中更有效的多尺度分解方法（如小波变换）；(2)针对遥感目标尺度差异大的特点，在GS2E中引入多尺度状态空间模型；(3)将LDFE适配到遥感专用检测框架（如定向目标检测）中，处理遥感图像中的旋转目标。

Abstract (英文): The complementary information between RGB and IR images can significantly enhance object detection performance under extreme conditions. Existing methods prefer dual-stream CNN backbones built upon YOLO for feature extraction and focus on the design of feature fusion. In this paper, we introduce the Laplacian Decoupled Feature Enhancement block (LDFE) to fuse features from different stages of the dual-stream CNN backbone. By design, LDFE simultaneously considers the characteristics of modalities and structures for feature fusion by employing global-local decomposition, denoising, fusion, and reconstruction, sequentially. The LDFE first separates features into global and local components based on Laplacian Pyramid, and then performs denoising and fusion based on Global State Space Enhancement module (GS2E) and Local Convolutional Correlation Enhancement module (LC2E) separately. Specifically, the GS2E conducts a two-branch architecture for the main and auxiliary modalities. It dynamically suppresses noise in the main modality through cross-modal attention derived from the auxiliary modality, while employing a State Space Model to capture long-range dependencies within the global feature representations of the main modality. To obtain bidirectional interaction, the two modalities systematically alternate their main/auxiliary roles. Moreover, the LC2E suppresses noise in local features and leverages spatial and channel dimension along with triple convolution to extract fine-grained details for fusion. These innovative designs achieve a significant performance improvement, with mAP surpassing the SOTA methods 6.2%, 3.7%, 4.7%, 2.3%, 4.1% and 2.0% on M3FD, DroneVehicle, LLVIP, FLIR-Aligned, KAIST and VEDAI datasets,respectively.

</details>


### [14] [Mixture of Enhanced-View Experts for Multi-Query Vehicle ReID and A Large-Scale Benchmark](https://arxiv.org/abs/2607.08085)
*Aihua Zheng,Jie Zhen,Chenglong Li,Jiaxiang Wang,Jin Tang*

Main category: cs.CV

TL;DR: 本文提出EV-MoE方法，通过增强视图专家混合模块（含VFEM和DMFM）和多视图对齐损失（MAL），解决多查询车辆ReID中简单特征融合忽视视图信息与跨视图关系的问题，并构建了大规模数据集LCRI-1K。


<details>
  <summary>Details</summary>
Motivation: 现有方法采用简单的特征融合方式，容易忽略重要视图信息和跨视图关系，导致多查询车辆ReID性能受限。

Method: 提出EV-MoE方法，包含视图特定特征增强子模块（VFEM）和动态多视图融合子模块（DMFM）组成的增强视图专家混合模块，以及多视图对齐损失（MAL），通过双向跨视图对比学习和重建约束对齐特征。

Result: 在多个数据集上验证了CAFNet在多查询车辆ReID问题中的鲁棒性，并构建了包含1,090个身份、107,805张图像、23,637个摄像头的LCRI-1K大规模数据集。

Conclusion: EV-MoE通过增强各视图特征表示并利用MoE高效集成视图特定增强特征，有效提升了多查询车辆ReID的鲁棒性。

与遥感交叉或改进方案: 交叉/改进可行性：85%。该文的多视图增强与动态融合策略可直接应用于遥感图像中多视角目标（如卫星多角度观测的车辆、船只）的跨视角重识别。具体方案：(1) 将VFEM用于增强遥感中不同传感器（光学、SAR、多光谱）或不同视角下的特征表示；(2) DMFM可用于融合多时相/多角度遥感影像的互补信息；(3) MAL中的双向跨视图对比学习适用于遥感中异源图像（如光学-SAR）的跨模态匹配任务。

Abstract (英文): Multi-query vehicle ReID aims to leverage complementary information from diverse views for robust feature learning. However, current methods suffer from simplistic feature fusion and thus easily ignores some important view information and cross-view relationships. To handle these problems, this work presents a novel approach called Mixture of Enhanced-View Experts (EV-MoE), which enhances the feature representation of each view and efficiently integrate the view-specific enhanced features by MoE, for robust multi-query ReID. In particular, we design a mixture of enhanced-view experts module, which consists of two parts including view-specific feature enhancement sub-Module (VFEM) and dynamic multi-view fusion sub-Module (DMFM). Moreover, we further introduce Multi-view Alignment Loss (MAL), which aligns features through bidirectional crossview contrastive learning and reconstruction constraints, addressing the challenges of consistency between multi-query features and single-image features. In addition, to evaluate multi-query ReID in real-world environments, we collect LCRI-1K, a largescale vehicle ReID dataset with 1,090 identities, 107,805 images, across 23,637 cameras, where each vehicle appears in an average of 67.5 cameras, providing a comprehensive benchmark to test the robustness in complex environments. Extensive experiments demonstrate the robustness of CAFNet in addressing the multiquery vehicle ReID problem. The code is available at https: //github.com/xiaozhen28/CAFNet.

</details>


### [15] [GRE-Diff: Gaussian Room Embeddings for Structured Layout Diffusion](https://arxiv.org/abs/2607.08086)
*Jing Wang,Haoran Xiong,Zihao Yan,Minglun Gong,Hui Huang*

Main category: cs.CV

TL;DR: 本文提出GRE-Diff，一种基于扩散模型的可控交互式框架，通过高斯房间嵌入（GRE）实现用户约束下的公寓平面图自动生成与编辑，在RPLAN数据集上取得高质量可编辑的多边形布局效果。


<details>
  <summary>Details</summary>
Motivation: 人工设计功能与美学协调的平面图需要探索庞大的房间排列空间，任务复杂度高、耗时长，亟需自动化工具辅助设计师完成这一过程。

Method: 采用扩散模型框架，核心创新是高斯房间嵌入（GRE），将每个房间建模为表征其位置和范围的空间高斯分布；结合LLM解析指令或GUI交互实现用户约束输入（房间类型、数量、边界形状等），支持人机协同的实时编辑。

Result: 在RPLAN数据集上的大量实验表明，GRE-Diff能够生成高质量、约束感知且可编辑的多边形布局。

Conclusion: 该工作弥合了AI自动化与人类创造力在空间设计中的鸿沟，为平面图设计提供了一种实用的交互式解决方案。

与遥感交叉或改进方案: 交叉/改进可行性：85%。具体方案：（1）将高斯房间嵌入迁移至遥感建筑 footprint 提取任务，将每个建筑轮廓建模为2D高斯分布，结合扩散模型生成建筑物边界；（2）利用LLM解析地理约束（如用地类型、容积率、建筑高度限制）指导遥感图像中的建筑布局生成；（3）将人机交互编辑机制引入遥感解译，允许用户通过自然语言或图形界面修正自动提取的建筑轮廓。

Abstract (英文): Designing functional and aesthetically coherent floor plans requires exploring a vast space of possible room arrangements, a task that quickly becomes overwhelming for human designers. In this paper, we propose GRE-Diff, a controllable and interactive diffusion-based framework that automates the creation and editing of apartment floor plans under user-specified constraints.
  By combining AI-generated suggestions with real-time, human-in-the-loop editing, the system enables users to specify room types, room counts, boundary shapes, and editing operations through LLM-parsed instructions or GUI-based interaction.
  It then generates a diverse set of plausible and well-structured designs for refinement.
  At the core of our approach is Gaussian Room Embedding (GRE), a continuous latent representation that models each room as a spatial Gaussian distribution capturing its location and extent.
  Extensive experiments on the RPLAN dataset show that GRE-Diff produces high-quality, constraint-aware, and editable polygonal layouts, offering a practical step toward bridging AI-driven automation and human creativity in spatial design.

</details>


### [16] [EVIS: A Physics-Grounded Event Camera Plugin for NVIDIA Isaac Sim](https://arxiv.org/abs/2607.08098)
*Linli Shi,Ruijun Zhang,Ziyun Wang*

Main category: cs.CV

TL;DR: EVIS是一个基于物理的事件相机插件，集成于NVIDIA Isaac Sim仿真器中，能够生成高帧率、完全标注的事件流，支持实时生成、传感器噪声和运动模糊模拟，可直接用于预训练事件网络。


<details>
  <summary>Details</summary>
Motivation: 事件相机数据在特定机器人和场景下稀缺且昂贵，制约了事件感知与控制算法的发展。

Method: 在Isaac Sim中实现了一个保真度高的对数强度对比事件模型，支持逐像素异步参考更新；通过双向运动向量扭曲实现仅渲染稀疏关键帧并合成中间帧的插值选项，从而在单GPU上实现实时生成；可选传感器噪声和运动模糊模拟。

Result: 该插件生成的带标签事件流可直接用于预训练的事件网络，完成下游任务。

Conclusion: EVIS为机器人领域的事件感知研究提供了低成本、高保真、完全标注的数据生成工具，加速了事件相机相关算法开发。

与遥感交叉或改进方案: 交叉/改进可行性：85%。该事件相机仿真插件可直接迁移至遥感领域中的运动目标检测、无人机高速避障、卫星姿态估计等任务。具体方案：1) 在Isaac Sim中搭建遥感场景（如卫星轨道、无人机飞行环境），利用EVIS生成高时间分辨率事件流；2) 结合遥感影像进行多模态数据融合，训练事件-帧联合检测网络；3) 利用插件提供的完美地面真值（如深度、光流）训练事件驱动的遥感运动目标跟踪模型。

Abstract (英文): Event cameras offer microsecond temporal resolution, low latency, and high dynamic range, making them attractive for robotics. However, labeled event-camera data for a specific robot and scene is scarce and expensive to collect, which slows the development of event-based perception and control. We present EVIS: a physics-grounded event camera plugin for NVIDIA Isaac Sim that generates high-rate, fully labeled event streams directly inside a physics simulator. The plugin implements a faithful log-intensity contrast event model with per-pixel asynchronous reference updates; it migrates from a normal RGB camera with few changes and integrates into any Isaac Sim / Isaac Lab scene, inheriting the simulator's physics and frame-perfect ground truth. It is fully configurable, and offers an interpolation option that renders only sparse keyframes and synthesizes the in-between frames through bidirectional motion-vector warping, making real-time generation on a single GPU possible. Optional sensor noise and motion blur further narrow the gap to real cameras. The generated streams are directly usable by pretrained event networks for downstream tasks. Code repository: https://github.com/spikelab-jhu/isaac-sim-event-camera-plugin

</details>


### [17] [VSRo-200: A Romanian Visual Speech Recognition Dataset for Studying Supervision and Multimodal Robustness](https://arxiv.org/abs/2607.08112)
*Iulia-Maria Udrea,Alexandra Diaconu,Bogdan Alexe*

Main category: cs.CV

TL;DR: VSRo-200是首个大规模罗马尼亚语视觉语音识别数据集（200小时播客视频），包含伪标签和人工标注子集。实验表明：人工标注在固定数据规模下更优，但伪标签可通过扩展数据持续提升性能；多模态融合在噪声条件下显著提升鲁棒性；该数据集在孤立词识别任务中大幅超越此前结果。


<details>
  <summary>Details</summary>
Motivation: 现有视觉语音识别数据集多集中于英语等资源丰富语言，罗马尼亚语等低资源语言缺乏大规模数据集；同时缺乏对标注质量（人工 vs 伪标签）在统一框架下的系统性对比研究。

Method: 构建200小时罗马尼亚语播客视频数据集，用微调的罗马尼亚语ASR模型生成伪标签，其中100小时子集额外进行人工转写；在此基础上建立低资源视觉语音识别基准，系统比较不同监督信号下的性能，并设计OOD测试集评估域泛化能力，以及噪声条件下的AVSR实验。

Result: 人工标注在固定数据规模下性能更优；伪标签可通过扩展数据规模持续提升性能；多模态融合在噪声下显著优于纯音频模型；在LRRo孤立词识别基准上，基于VSRo-200学习的表征大幅超越此前最优结果。

Conclusion: VSRo-200为低资源视觉语音识别中监督质量、域泛化和多模态融合研究提供了新的测试平台。

与遥感交叉或改进方案: 交叉/改进可行性：85%。该研究的数据构建策略（伪标签+人工标注混合）可迁移到遥感领域中的场景文本识别或地物分类任务（如利用合成数据+少量人工标注提升标注效率）；域泛化评估方法可直接应用于遥感图像跨区域、跨传感器的域适应问题；多模态融合思路可启发遥感中SAR-光学、高光谱-多光谱等异构数据融合方案。

Abstract (英文): We introduce VSRo-200, the first large-scale dataset for visual speech recognition (lip reading) in Romanian, comprising 200 hours of real-world podcast videos. All samples are annotated with pseudo-labels generated by a fine-tuned Romanian ASR model, while a subset of 100 hours is additionally transcribed by humans, enabling controlled analysis of supervision quality under a unified framework. Building on this dataset, we establish a benchmark for visual speech recognition in low-resource settings. We systematically study the impact of supervision quality, showing that while human annotations provide better performance at fixed data scales, pseudo-labels enable continued improvements through scalability. We further evaluate robustness under domain shift using curated out-of-distribution (OOD) test sets, and analyze audio-visual speech recognition (AVSR) under noisy conditions, where multimodal fusion significantly improves robustness compared to audio-only models. Finally, we demonstrate that representations learned on VSRo-200 transfer effectively to the LRRo benchmark for isolated word recognition, substantially outperforming previously reported results. Overall, VSRo-200 provides a new testbed for studying supervision, domain generalization, and multimodal fusion in low-resource visual speech recognition.

</details>


### [18] [Understanding and Mitigating the Video-Action Generalization Gap via Temporal Ratio](https://arxiv.org/abs/2607.08127)
*Utkarsh A. Mishra,Yongxin Chen,Danfei Xu,Yang Liu,Xi Chen,Jiayuan Mao*

Main category: cs.CV

TL;DR: 本文发现生成式视频基础模型在微调为视频-动作模型（VAM）后会丢失组合先验，造成视频-动作泛化差距。作者引入时序比率（TR）作为注意力度量来量化模型对未来潜在展开的依赖程度，并提出一种推理时自适应引导方法，通过在规划阶段动态增强组合视频条件信号来弥合分布外与分布内的组合泛化差距。在LIBERO基准和真实世界任务上验证了有效性。


<details>
  <summary>Details</summary>
Motivation: 生成式视频基础模型具有强大的组合先验，但经过机器人动作数据微调后，世界-动作模型和视频-动作模型往往会丢失这些先验，导致视频-动作泛化差距。现有研究对这一差距缺乏系统性理解，需要解释其成因并提出解决方案。

Method: 1) 系统评估VAM的设计空间，证明标准设计选择不产生涌现解释模式；2) 引入时序比率（Temporal Ratio, TR），一种基于注意力的度量，衡量动作头对潜在未来展开相对于锚定当前帧的依赖强度；3) 提出推理时自适应引导方法，利用TR反映的内在特征注意力模式，在策略依赖未来展开时动态放大组合视频条件信号。

Result: 在LIBERO基准和真实世界任务上，该方法有效缓解了分布外与分布内的组合泛化差距。

Conclusion: TR具有两个关键性质：一是模型对未来预测潜在的结构性依赖（通过TR衡量）可预测其组合泛化能力；二是TR会根据任务阶段自然波动——规划时注意力转向未来帧，精确操作时回到当前帧。基于此提出的自适应引导方法能够有效弥合组合泛化差距。

与遥感交叉或改进方案: 交叉/改进可行性：75%。遥感领域中的视频时序分析（如变化检测、目标跟踪、事件预测）面临类似的预训练-微调泛化问题。具体方案：1) 将TR引入遥感视频基础模型，用于衡量模型对时序上下文（如多时相影像）的依赖程度，诊断模型是否过度依赖单帧特征而丢失时序组合先验；2) 针对遥感中不同任务阶段（如规划观测路径vs.精确地物分割），利用TR动态调整条件信号权重，提升模型在时序不一致场景下的组合泛化能力；3) 结合遥感多模态数据（光学-SAR时序对），扩展TR为多模态时序比率，量化跨模态特征依赖。

Abstract (英文): Generative video foundation models exhibit strong compositional priors, yet world-action models (WAMs) and video-action models (VAMs) often lose these priors after finetuning on robotic action data. We refer to this discrepancy as the video-action generalization gap. In this paper, we systematically investigate this gap by evaluating a comprehensive design space of VAMs, demonstrating that standard design choices yield no emergent explanation pattern. To explain this behavior, we introduce the Temporal Ratio (TR), an attention-based measure of how strongly the action head relies on future latent rollouts relative to the anchored current frame. TR has two key properties: first, a model's structural reliance on future-predictive latents, measured via TR, acts as a predictor of its compositional generalization capacity; second, it natively fluctuates based on task phase, shifting attention to future frames during planning and reverting to the present frame for precise manipulation. Finally, based on these findings, we propose an inference-time adaptive guidance method, which exploits this intrinsic feature attention pattern to dynamically amplify compositional video conditioning signals precisely when the policy relies on future rollouts. Evaluated on the LIBERO benchmark and real-world tasks, our approach mitigates the OOD-ID compositional generalization gap. More details: https://umishra.me/temporal-ratio/

</details>


### [19] [Unified Face Attack Detection via Fine-Grained Semantic Guidance](https://arxiv.org/abs/2607.08156)
*Ning Jiang,Shijie Yu,Dingheng Zeng,Haiyang Yi,Yanhong Liu,Haifeng Shen,Ying Li*

Main category: cs.CV

TL;DR: 本文在MS-UFAD数据集上为每张攻击图像添加细粒度伪造线索文本描述，并提出DAF-Net网络，通过视觉-文本双对齐机制提取更泛化的伪造表征，在面攻击检测任务上优于纯视觉方法和粗粒度描述方法。


<details>
  <summary>Details</summary>
Motivation: 现有面攻击检测数据集缺乏详细的伪造线索文本描述，导致大多数方法仅将面攻击检测视为视觉识别任务，难以提取泛化性强的伪造表征。

Method: 基于MS-UFAD数据集（800万+攻击图像），为每张图像添加细粒度伪造线索文本描述；提出双对齐伪造网络（DAF-Net），利用文本信息辅助学习更通用的伪造表征。

Result: 所提方法提取的伪造表征具有更强的泛化性和语义意义，在面攻击检测任务上优于纯视觉方法及基于粗粒度描述的方法。

Conclusion: 引入细粒度文本描述并采用双对齐机制能有效提升面攻击检测的泛化性能，验证了多模态信息融合在该任务中的价值。

与遥感交叉或改进方案: 交叉/改进可行性：85%。遥感图像伪造检测（如拼接、篡改、生成对抗攻击）可借鉴本文思路：1）为遥感攻击数据集构建细粒度文本描述（如"边界像素异常、光谱不一致"等）；2）采用DAF-Net的双对齐机制，使遥感伪造检测网络同时对齐视觉伪造特征与文本语义描述，提升跨场景泛化能力；3）特别适用于多源遥感图像融合伪造检测场景。

Abstract (英文): The growing applications of facial recognition systems are accompanied by increasingly diverse security threats. Existing datasets lack detailed textual descriptions of forgery cues, leading most prior methods to treat face attack detection primarily as a visual recognition task. In this paper, building upon the large-scale MS-UFAD dataset which contains over 8 million attack images, we enrich each image with a fine-grained textual description of forgery cues. Furthermore, we propose a Dual Alignment Forgery Network(DAF-Net) to better leverage these textual information. Extensive experiments demonstrate that our approach extracts more generalizable and semantically meaningful forgery representations from attack images, outperforming both vision-only methods and approaches based on coarse-grained descriptions.

</details>


### [20] [ProsMAE: Multi-Source MAE Pretraining for ISUP Grade Classification](https://arxiv.org/abs/2607.08162)
*Anna Jung,Kyeonghun Kim,Youngung Han,Eunseob Choi,Jiwon Yang,Ken Ying-Kai Liao,Hyuk-Jae Lee,Nam-Joon Kim*

Main category: cs.CV

TL;DR: 本文提出ProsMAE，一个多源掩码自编码器框架，通过利用多个组织病理学数据集（PANDA、CAMELYON17、BRACS）进行预训练，学习鲁棒的病理图像表示，并在ISUP分级分类任务上优于传统MAE方法。


<details>
  <summary>Details</summary>
Motivation: 全切片图像（WSI）存在千兆像素尺度、染色差异、扫描仪差异、组织伪影以及专家标注有限等问题，导致模型训练困难。现有方法在这些挑战下泛化能力不足，需要更鲁棒的表征学习方法。

Method: 采用多源掩码自编码器（Multi-source MAE）框架，使用来自PANDA、CAMELYON17和BRACS三个数据集的瓦片进行预训练，使编码器暴露于多样的组织形态和采集条件。随后通过ProsCLS任务进行迁移学习，冻结编码器并仅训练线性分类头用于ISUP分级分类。

Result: 在PANDA数据集的分离划分下，ProsMAE的验证二次加权卡帕（QWK）均值高于传统MAE的冻结线性探测基线。

Conclusion: 多源预训练有助于学习更鲁棒的组织病理学表示，但还需要重复划分评估以进一步验证不同划分组成下的鲁棒性。

与遥感交叉或改进方案: 交叉/改进可行性：75%。遥感领域同样面临多源数据（不同传感器、分辨率、光照条件）和标注稀缺问题。可将ProsMAE的多源MAE预训练思路迁移至遥感场景：使用多个遥感数据集（如不同卫星影像、不同地域）进行预训练，暴露编码器于多样化的采集条件；随后冻结编码器，用少量标注数据进行线性探测微调，用于土地覆盖分类等任务。此外，可引入遥感特有的掩码策略（如云掩码、光谱波段掩码）替代随机掩码。

Abstract (英文): Whole slide images (WSIs) provide rich diagnostic information for computational pathology, but their gigapixel scale, stain variation, scanner differences, tissue artifacts, and limited expert annotation make robust model training challenging. This paper presents a multi-source Masked Autoencoder (MAE) framework, named ProsMAE, for histopathology representation learning. Tiles from Prostate cANcer graDe Assessment (PANDA), CAncer MEtastases in LYmph nOdes challeNge 2017 (CAMELYON17), and BReAst Carcinoma Subtyping (BRACS) are used for ProsMAE pretraining to expose the encoder to diverse tissue morphology and acquisition conditions. The learned encoder is transferred for International Society of Urological Pathology (ISUP) grade classification through ProsCLS, using a frozen encoder and a linear classification head. ProsMAE achieved a higher mean validation quadratic weighted kappa (QWK) than the vanilla MAE frozen linear-probe baseline under the evaluated disjoint PANDA split. Repeated-split evaluation remains necessary to further establish robustness across split compositions.

</details>


### [21] [Continual Test-Time Adaptation in Computer Vision: Methods, Benchmarks, and Future Directions](https://arxiv.org/abs/2607.08164)
*Sarthak Kumar Maharana,Shambhavi Mishra,Yunbei Zhang,Shuaicheng Niu,Taki Hasan Rafi,Jihun Hamm,Marco Pedersoli,Jose Dolz,Yunhui Guo*

Main category: cs.CV

TL;DR: 本文系统综述了持续测试时自适应（CTTA）领域，旨在解决深度神经网络在部署中面临非平稳分布漂移时的灾难性遗忘和错误累积问题。论文形式化定义了CTTA问题，分析了不同的持续域漂移模式，提出了一个分层分类法，将现有方法分为三类：基于优化的策略、参数高效方法和基于架构的方法，并进行了系统性比较与基准评测，最后指出了未来研究方向。


<details>
  <summary>Details</summary>
Motivation: 深度神经网络在训练与测试数据分布一致时表现优异，但实际部署中数据分布持续变化，导致性能严重下降。现有方法无法同时解决灾难性遗忘（忘记源知识）和错误累积（噪声伪标签随时间放大）这两个关键问题，亟需一种能够在无源数据、无标签条件下在线适应非平稳目标分布的解决方案。

Method: 本文采用综述研究方法，首先形式化定义CTTA问题，分析多种持续域漂移模式对应的评估协议，进而提出分层分类法：1）基于优化的策略（熵最小化、伪标签、参数恢复）；2）参数高效方法（归一化层适配、自适应参数选择）；3）基于架构的方法（师生框架、适配器、视觉提示、掩码建模）。对各方法进行系统梳理与比较基准测试。

Result: 建立了CTTA问题的完整研究框架，包括问题形式化定义、域漂移模式分类、方法分层分类体系，以及在标准评估设置下的对比基准和实验结果汇总。

Conclusion: 当前CTTA方法仍存在局限性，未来研究方向包括基础模型的自适应和黑盒系统的适配，为该领域提供了清晰的研究路线图。

与遥感交叉或改进方案: 交叉/改进可行性：90%。遥感影像常因传感器参数变化、大气条件变化、季节更替等因素产生持续分布漂移，CTTA技术可有效应用于：1）遥感影像时间序列分类中的在线自适应；2）跨时相、跨传感器遥感图像分割的免源域动态适配；3）遥感基础模型（如遥感SAM、遥感CLIP）在持续场景变化下的微调；4）结合遥感多模态数据（SAR+光学）的分布漂移鲁棒适配。改进方向可针对遥感场景特有的空间连续性和周期性域漂移进行建模。

Abstract (英文): Deep neural nets achieve remarkable performance when training and test data share the same distribution, but this assumption frequently breaks in real-world deployment, where data undergoes continual distributional shifts. Continual Test-Time Adaptation (CTTA) addresses this challenge by adapting pretrained models to non-stationary target distributions on-the-fly, without access to source data or labeled targets, while mitigating two critical failure modes: catastrophic forgetting of source knowledge and error accumulation from noisy pseudo-labels over extended time horizons. In this comprehensive survey, we formally define the CTTA problem, analyze the diverse continual domain shift patterns that characterize different evaluation protocols, and propose a hierarchical taxonomy that categorizes existing methods into three families: optimization-based strategies (entropy minimization, pseudo-labeling, parameter restoration), parameter-efficient methods (normalization layer adaptation, adaptive parameter selection), and architecture-based approaches (teacher-student frameworks, adapters, visual prompting, masked modeling). We systematically review representative methods within each category and present comparative benchmarks and experimental results across standard evaluation settings. Finally, we discuss limitations of current approaches and highlight emerging research directions, including adaptation of foundation models and black-box systems, providing a roadmap for future research in robust continual test-time adaptation. We encourage visiting our repository at [https://github.com/sarthaxxxxx/Awesome-Continual-Test-Time-Adaptation](https://github.com/sarthaxxxxx/Awesome-Continual-Test-Time-Adaptation)

</details>


### [22] [Attention-Based Segmentation of WMHs and Differentiation of Vascular vs. Demyelinating Lesions](https://arxiv.org/abs/2607.08171)
*Aina Tur-Serrano,Gabriel Moyà-Alcover,Francisco J. Perales López*

Main category: cs.CV

TL;DR: 本文提出结合注意力分割（BAM/CBAM、Attention U-Net）与形态特征分类的方法，用于区分血管性和脱髓鞘性白质高信号病变，在多个公开数据集上验证了该方法的可行性。


<details>
  <summary>Details</summary>
Motivation: 血管性和脱髓鞘性白质高信号病变在FLAIR影像上表现相似，病因不同却难以区分，导致鉴别诊断困难。

Method: 采用注意力机制（BAM、CBAM）结合Attention U-Net进行病变分割，使用patch学习和2.5D训练策略，分割后提取病变掩膜的形态学特征，再进行病因分类。

Result: 在五个公开数据集上验证了注意力分割+特征分类方法的有效性，模型具有一定泛化能力，但样本量有限。

Conclusion: 注意力分割与特征驱动分类相结合的方案在区分血管性和脱髓鞘性白质病变方面有前景，仍需更大规模临床队列验证。

与遥感交叉或改进方案: 交叉/改进可行性：85%。遥感影像中不同地物（如植被与水体的阴影、不同作物类型）常呈现相似光谱特征，类似本文中WMH的“同征异病”问题。可将本文的注意力分割（BAM/CBAM+U-Net）与形态特征分类框架迁移至遥感领域：先对遥感影像进行语义分割提取感兴趣区域（如建筑物、水体、农田），再提取分割结果的形态特征（形状、纹理、空间分布）进行地物细分类。2.5D训练策略可借鉴用于多光谱/高光谱遥感数据的通道维度处理。

Abstract (英文): White Matter Hyperintensities (WMHs) are commonly observed in brain Magnetic Resonance Imaging (MRI) scans. They are associated with various neurological conditions, including vascular and inflammatory demyelinating diseases. Despite differing in etiology, WMHs from these conditions often appear similar on Fluid Attenuated Inversion Recovery (FLAIR) images. This similarity makes differential diagnosis challenging. In this work, we highlight the potential of combining attention-based segmentation with feature-driven classification. This approach supports more accurate and efficient classification between vascular and demyelinating white matter pathologies. For segmentation, we evaluate the effectiveness of attention mechanisms, specifically the Bottleneck Attention Module (BAM) and the Convolutional Block Attention Module (CBAM). We also test different architectures, particularly Attention U-Net. In addition, we explore advanced training strategies, such as patch-based learning and a 2.5D approach, to enhance lesion detection. After segmentation, we extract morphological features from the lesion masks. We then use them to classify WMHs based on their underlying cause. Our experiments utilize five publicly available datasets with diverse imaging protocols to promote model generalizability, despite limited sample sizes. The results suggest that attention-based segmentation and feature-driven classification offer a promising direction for discriminating vascular and demyelinating white matter lesions. Further validation in larger clinical cohorts is still needed.

</details>


### [23] [LEEVLA: Seeing What Matters in Latent Environment Evolution for Vision-Language-Action](https://arxiv.org/abs/2607.08182)
*Qi Lyu,Baicheng Liu,Xudong Wang,Jiahua Dong,Lianqing Liu,Zhi Han*

Main category: cs.CV

TL;DR: LEEVLA是一种视觉-语言-动作（VLA）架构，通过引入漂移引导动态优先级（DGDP）和结构化特征流生成（SFFG）机制，实现了对任务关键证据的显式引导和潜在世界表征的结构化演化，在VLA基准测试中持续优于现有方法。


<details>
  <summary>Details</summary>
Motivation: 现有VLA模型在处理复杂动态场景时存在两个主要问题：（1）对所有视觉token统一处理，缺乏强调任务关键证据的机制；（2）使用人为选择的因素进行推理，忽略了潜在的潜在因素。这导致模型难以应对复杂动态场景。

Method: 提出LEEVLA架构，包含两个核心组件：（1）漂移引导动态优先级（DGDP），由动态位置优先级（DPP）和语义漂移引导（SDG）组成，用于识别显著且与指令相关的区域；（2）结构化特征流生成（SFFG），通过原型到外围（P2P）预测建模优先特征在潜在空间中的演化，并采用互邻域对比（MC）损失保持邻域间的拓扑一致性。DGDP和SFFG共同构成任务感知的"何处-如何"训练框架。

Result: 在VLA基准测试上的大量实验表明，LEEVLA持续优于先前的方法，验证了显式任务证据引导和结构化潜在推理对可扩展VLA的关键作用。

Conclusion: 显式任务证据引导和结构化潜在推理对于构建可扩展的VLA模型均至关重要。LEEVLA通过"何处-如何"框架有效提升了复杂动态场景下的VLA性能。

与遥感交叉或改进方案: 交叉/改进可行性：85%。遥感领域常面临复杂动态场景（如变化检测、目标跟踪、时序分析），LEEVLA的潜在表征演化思想可迁移至遥感时序建模：（1）DGDP机制可用于遥感影像中关键变化区域的自动定位与优先级排序，取代传统的人工标注或固定兴趣点方法；（2）SFFG中的P2P预测可建模地物特征随时间演化的结构化过程，例如城市扩张、植被物候变化等；（3）MC损失可保持遥感特征在时间序列上的拓扑一致性，提升变化检测的鲁棒性。建议将视觉-语言-动作框架调整为遥感中的"视觉-语言-分类/检测"范式，利用DGDP识别任务相关区域，用SFFG建模时序演化。

Abstract (英文): Vision-language-action (VLA) models aim to map multimodal inputs to robot actions. However, most existing approaches struggle to cover complex dynamic scenarios due to treating all visual tokens uniformly and reasoning with human-selected factors, which lack mechanisms to emphasize task-critical evidence and ignore underlying factors. To address this issue, we propose LEEVLA, a VLA architecture for seeing what matters in Latent Environment Evolution that explicitly guides the model toward informative regions while preserving the structured evolution of latent world representations. To identify salient and instruction-relevant regions, we introduce drift-guided dynamic prioritization (DGDP), which combines dynamic position prioritization (DPP) with semantic drift guidance (SDG) to guide the VLA agent where to attend during training. On top of this, we introduce structured feature flow generation (SFFG), which models how these prioritized features should evolve in latent space via prototype-to-periphery (P2P) prediction, and a mutual-neighborhood contrastive (MC) loss to maintain topological consistency among neighborhoods. Together, DGDP and SFFG form a task-aware "where-how" training framework. Extensive experiments on VLA benchmarks show that LEEVLA consistently outperforms prior methods, confirming that explicit task-evidence guidance and structured latent reasoning are both crucial for scalable VLA. Our code is available at https://github.com/LyuQi127/LEEVLA.

</details>


### [24] [Leveraging Color Naming for Image Enhancement](https://arxiv.org/abs/2607.08185)
*David Serrano-Lozano,Luis Herranz,Michael S. Brown,Javier Vazquez-Corral*

Main category: cs.CV

TL;DR: NamedCurves+ 提出一种基于颜色命名（Color Naming）的图像增强方法，通过为每种命名颜色学习独立的色调曲线，并结合 Transformer 捕获空间依赖，实现可解释、可交互的图像润饰，在图像润饰、色调映射和曝光校正任务上优于现有方法。


<details>
  <summary>Details</summary>
Motivation: 现有深度学习方法在图像增强中面临可解释性差和缺乏适合用户调整的参数化两个关键问题，导致用户难以理解和个性化定制润饰过程。

Method: 将颜色命名（Color Naming）融入学习框架，为每种命名颜色设计全局色调曲线进行全局调整；引入 Transformer 模块捕获空间依赖以处理局部图像变化，实现上下文感知的局部编辑。

Result: 在图像润饰、色调映射和曝光校正等多个任务上，NamedCurves+ 均优于当前最先进的方法。

Conclusion: NamedCurves+ 不仅性能优越，而且具有可解释性（色调曲线显式表示每种颜色名的贡献）和交互性（用户可自定义调节每条色调曲线），实现了灵活的个人化图像增强。

与遥感交叉或改进方案: 交叉/改进可行性：85%。遥感图像（如多光谱、高光谱影像）中存在丰富的颜色/波段信息，可将 NamedCurves+ 的色调曲线机制扩展到各波段或地物类别，实现对特定地物（如植被、水体、裸土）的独立增强；结合 Transformer 的空间依赖性可处理遥感图像中的复杂空间结构，用于遥感图像去雾、阴影校正和色彩一致性处理等任务。

Abstract (英文): Enhancing images to make them visually appealing is a persistent challenge in computer vision. Many deep-learning methods train models on paired datasets to replicate expert editing styles. However, these approaches struggle with two key issues: (1) interpretability and (2) a parametrization suitable for user adjustments. To address these challenges, we present NamedCurves+, an approach inspired by the concept of Color Naming, a universal set of familiar colors widely used in software tools for intuitive editing. Our method integrates color names into a learning-based framework, enabling global adjustments for each named color through tone curves. To address local image variations, we incorporate a transformer block that captures spatial dependencies, enabling context-aware edits across the image. NamedCurves+ enhances the retouching process's interpretability and supports user interaction, allowing flexible modifications of individual tone curves to refine the retouched image according to personal preferences. Extensive experiments on tasks such as image retouching, tone mapping, and exposure correction demonstrate that NamedCurves+ outperforms state-of-the-art methods. Notably, our approach is both explainable, as the tone curves explicitly represent how each color name contributes to the enhancement, and interactive, allowing users to customize the retouching process and achieve results tailored to their liking.

</details>


### [25] [Dual-Correlation Hypergraph Network for Unaligned RGBT Video Object Detection and A Large-scale Benchmark](https://arxiv.org/abs/2607.08191)
*Qishun Wang,Yapeng Li,Bin Luo,Zhengzheng Tu,Chenglong Li*

Main category: cs.CV

TL;DR: 本文提出了一种双相关超图网络(DHNet)来解决RGBT视频目标检测中的空间错位问题，通过显式建模时间相关性和空间相关性来捕获高维互补信息。同时构建了大规模多样化场景数据集DVT-VOD1000（包含1000个视频序列、103,464对RGBT图像），在多个基准上取得了最先进检测精度。


<details>
  <summary>Details</summary>
Motivation: RGBT视频目标检测中，跨模态图像对之间存在空间错位问题，现有方法未能充分建模时间维度和跨模态维度的高维互补信息，且缺乏大规模、场景多样的基准数据集。

Method: 1) 设计基于分块的空间对齐模块(PSAM)在局部区域级别对齐多模态特征；2) 提出双超图融合模块(DHFM)，分别构建时间超图和多模态超图进行双相关学习以增强目标判别性；3) 构建包含1000个视频序列的大规模RGBT VOD数据集DVT-VOD1000。

Result: 在VT-VOD50和DVT-VOD1000两个数据集上的综合实验表明，DHNet达到了最先进的检测精度。

Conclusion: 提出的DHNet通过双相关超图建模有效解决了RGBT空间错位问题，构建的大规模数据集DVT-VOD1000为领域提供了更全面的评估基准。

与遥感交叉或改进方案: 交叉/改进可行性：85%。遥感领域多源数据（如光学-SAR、多光谱-高光谱）同样存在空间错位问题。具体方案：1) 将PSAM模块迁移至遥感多模态图像配准任务，处理不同分辨率/视角的光学与SAR图像对齐；2) 双超图融合机制可应用于遥感时序变化检测，构建时间超图（多时相遥感影像）和空间超图（多光谱波段）联合建模；3) DVT-VOD1000的构建思路可为遥感视频目标检测数据集（如卫星视频、无人机视频）提供参考框架。

Abstract (英文): RGB-Thermal (RGBT) Video Object Detection (VOD) has gained significant traction due to its ability to overcome the limitations of conventional RGB-based VOD under challenging conditions. However, spatial misalignment commonly exists between RGBT image pairs. To address this, we propose a Dual-Correlation Hypergraph Network (DHNet) that captures high-dimensional complementary information by explicitly modeling two types of correlations: temporal correlation across consecutive frames and spatial correlation from cross-modal features. Specifically, we first design a Patch-based Spatial Alignment Module (PSAM) to sequentially align the multimodal features at the local region level. Subsequently, we introduce a Dual Hypergraph Fusion Module (DHFM), which constructs separate temporal and multimodal hypergraphs to enhance object discriminability through dual-correlation learning. Furthermore, the field currently lacks a large-scale, scene-diverse benchmark dataset for comprehensive evaluation. To address this gap, we construct DVT-VOD1000, a large-scale RGBT VOD dataset containing 1,000 video sequences with 103,464 RGBT image pairs. The dataset covers diverse scenarios, including campuses, parks, transportation, rural areas, night scenes, rain, and snow. Comprehensive experiments on VT-VOD50 and our DVT-VOD1000 demonstrate that DHNet achieves state-of-the-art detection accuracy. The dataset and source code will be made publicly available on https://github.com/tzz-ahu/ to support academic research.

</details>


### [26] [Dive Into the Implicit Biases of Low-rank Vision-language Alignment](https://arxiv.org/abs/2607.08194)
*Mingjia Shi,Shuo Wang,Xiaobo Wang,Sifan Zhou,Kai Wang,Tianyu Fu,Chenxu Zhao,Anyang Su,Ping Jiang,Minghui Wu*

Main category: cs.CV

TL;DR: 本文挑战了视觉-语言对齐阶段必须全参数更新的传统观点，发现使用低秩适配（LoRA）进行对齐不仅降低计算成本，还在多数基准上超越全参数对齐。通过实证和理论分析，揭示了低秩对齐的隐式偏置：减少幻觉、保持视觉特征的线性可分性（LS-curse）、生成更均匀稳定的视觉表征，并提供了两个定理从梯度平坦性和特征子空间鲁棒性角度给出解释。


<details>
  <summary>Details</summary>
Motivation: 传统视觉-语言对齐被视为需要全参数更新的预训练阶段，计算成本高昂。作者质疑这一观点，探究在该阶段对LLM使用低秩适配的效果及其背后原因。

Method: 在视觉-语言对齐阶段对LLM应用低秩适配（LoRA），通过与全参数对齐进行系统对比；通过实证分析低秩对齐对模型行为（幻觉-保守转变）、视觉特征线性可分性（LS-curse现象）和视觉表征几何结构的影响；构建两个定理从参数子空间梯度平坦性和特征子空间扰动鲁棒性角度提供理论解释。

Result: 低秩对齐在多数基准上超越全参数对齐；低秩对齐使模型从幻觉性转向保守性；全参数对齐破坏视觉特征的逐token线性可分性（LS-curse），而低秩对齐保持此特性；低秩对齐生成更均匀、结构稳定的视觉表征，保持模态特有知识而非过早融合实体级语义；超过100种对齐配置的消融实验验证了结果鲁棒性。

Conclusion: 视觉-语言对齐无需全参数更新，低秩适配是一种更优的替代方案，其结构保持行为源于对平坦梯度子空间和鲁棒特征子空间的隐式偏好。

与遥感交叉或改进方案: 交叉/改进可行性：90%。遥感领域视觉-语言对齐任务（如遥感图像描述、跨模态检索）通常面临数据量少、计算资源受限的挑战。可将本文的低秩对齐方法直接应用于遥感视觉-语言模型训练：1）使用预训练的遥感视觉编码器（如ResNet、ViT变体）与大语言模型结合，在对齐阶段采用LoRA而非全参数微调，大幅降低显存需求，适合遥感场景下有限的GPU资源；2）遥感图像中存在大量细粒度地物目标且易产生幻觉（如将建筑误识别为道路），低秩对齐的保守性可有效减少遥感描述中的虚假信息；3）低秩对齐保持视觉特征线性可分性的特性有利于遥感场景中地物类间区分（如不同植被类型），可提升跨模态检索精度。

Abstract (英文): Vision-language alignment, the stage that bridges pretrained vision encoders and large language models, is widely treated as a form of pretraining requiring full-parameter updates. We challenge this view and investigate what happens when low-rank adaptation is applied to the LLM during this stage instead. We find that low-rank alignment not only reduces computational costs but also outperforms full-parameter alignment on most benchmarks. To understand this phenomenon, we systematically characterize the implicit biases introduced by low-rank adaptation during alignment. Empirically, we find that low-rank alignment shifts model behavior from hallucinatory to conservative and preserves per-token linear separability of visual features that full-parameter alignment disrupts, a phenomenon we term LS-curse. Geometrically, low rank aligned models exhibit more homogeneous and structurally stable visual representations, maintaining modality-specific knowledge rather than prematurely fusing entity-level semantics. Theoretically, we establish two theorems showing that low-rank alignment induces preferences for parameter subspaces with flat gradients and feature subspaces robust to perturbations, providing a principled explanation for the observed structure-preserving behavior. Extensive experiments cover ablation over 100 alignment configurations, three families of low-rank operators, and various rank, encoder, and other settings.

</details>


### [27] [Unpaired Joint Distribution Modeling via Multi-Scale Image Representations](https://arxiv.org/abs/2607.08198)
*Yihang Zou,Hui Zhang,Zuowei Shen,Chenglong Bao*

Main category: cs.CV

TL;DR: 本文提出LUD-MSR框架，通过潜在变量和辅助表示从边缘观测中学习联合分布，利用多尺度图像表示（MSR）在域一致性与信息保留之间取得平衡，在真实去噪任务（包括冷冻电镜）上验证了有效性。


<details>
  <summary>Details</summary>
Motivation: 从边缘观测学习联合分布本质上是不适定问题，现有方法在表示学习中面临域一致性与信息保留之间的权衡，缺乏理论分析和有效的平衡策略。

Method: 提出LUD-MSR框架：1）基于潜在变量的概率模型，通过辅助表示对联合分布建模，仅利用边缘数据优化证据下界；2）引入多尺度图像表示（MSR）映射，利用粗尺度的结构相似性同时抑制域特定变化；3）在温和假设下推导分布近似误差的上界。

Result: 理论分析揭示了表示学习中域一致性与信息保留之间的权衡关系，并证明了MSR相比现有方法能实现更优的平衡；在真实去噪基准（包括冷冻电镜cryo-EM）上验证了有效性。

Conclusion: LUD-MSR通过多尺度表示有效解决了从边缘观测学习联合分布中的核心权衡问题，为不适定分布学习问题提供了理论与实验双重支撑的解决方案。

与遥感交叉或改进方案: 交叉/改进可行性：85%。遥感领域常面临多模态数据（如光学-SAR）的联合分布学习问题，且存在域差异。具体方案：1）将LUD-MSR应用于遥感多模态图像融合，利用MSR在不同尺度上对齐结构信息；2）在遥感去噪（如SAR斑点噪声去除）中引入MSR，利用粗尺度结构先验保持地物边界；3）扩展MSR为多光谱/高光谱多尺度表示，处理波段间联合分布学习。

Abstract (英文): This paper studies the problem of learning a joint distribution from marginal observations, which is inherently ill-posed due to the ambiguity of feasible couplings. We propose LUD-MSR, a latent-variable probabilistic framework that models the joint distribution via auxiliary representations and optimizes evidence lower bounds using only marginal data. Under mild assumptions, we establish an upper bound on the distribution approximation error. This analysis reveals a trade-off in representation learning between domain consistency and information preservation. To address this trade-off, we introduce a Multi-Scale image Representation (MSR) mapping that exploits structural similarity at coarse scales while suppressing domain-specific variations. We show that MSR achieves a more favorable balance of this trade-off compared to existing approaches. Experiments on real-world denoising benchmarks, including cryo-electron microscopy (cryo-EM), demonstrate the effectiveness of the proposed framework.

</details>


### [28] [TMI: Text-to-Image Meets Image-to-Image for Complementary Data Synthesis to Boost Long-Tailed Instance Segmentation](https://arxiv.org/abs/2607.08201)
*Hyeonseop Song,Seokhun Choi,Hoseok Do*

Main category: cs.CV

TL;DR: 提出混合数据合成管线TMI，结合文生图（T2I）生成与上下文感知的图生图（I2I）编辑，用于大规模词汇实例分割中的长尾类别和细粒度类别模糊问题，在LVIS基准上AP提升最高+4.0，罕见类AP提升最高+9.5。


<details>
  <summary>Details</summary>
Motivation: 现有数据合成方法存在互补性局限：T2I方法继承噪声伪标签且在稀有类别上表现差，复制粘贴方法则损害上下文真实性。需要一种既能提供类别和场景多样性，又能保证标签可靠性并减少域差距的方法。

Method: 提出混合管线TMI，包含两个分支：（1）T2I分支通过教师-学生方案选择性保留提示指定类别，确保标签可靠性；（2）I2I编辑分支VRAIN将高置信度实例插入野外场景的语义合适位置，实现语义连贯且视觉自然的编辑。两者协同提供广泛类别多样性并针对性增强稀有类别监督。

Result: 在LVIS基准上超越现有基线，总体AP提升最高+4.0点，罕见类AP提升最高+9.5点，且随骨干网络容量有效扩展。

Conclusion: 混合T2I与上下文感知I2I编辑的策略能有效解决大规模词汇实例分割的长尾和模糊性问题，尤其对稀有类别的增强效果显著。

与遥感交叉或改进方案: 交叉/改进可行性：85%。遥感实例分割同样面临长尾分布（如罕见地物类别）和类间模糊（如不同建筑类型）问题。具体方案：（1）T2I分支可生成多样化遥感场景（多时相、多分辨率），教师-学生方案确保标签可靠性；（2）VRAIN可将高置信度遥感目标实例（如特定车辆、船只）插入不同背景场景（不同地形、光照条件），增强罕见地物类别的训练数据；（3）需调整语义位置判定逻辑，从自然场景的空间关系（如"桌面上"）转为地理空间关系（如"道路旁"、"水域中"）。

Abstract (英文): Large-vocabulary instance segmentation is constrained by long-tailed category distributions and fine-grained inter-class ambiguity. While data synthesis offers a promising alternative, current paradigms have complementary limitations: text-to-image (T2I) methods inherit noisy pseudo-labels and struggle on rare classes, whereas copy-paste methods compromise contextual realism. To address these issues, we propose a hybrid pipeline coupling T2I generation with context-aware image-to-image (I2I) editing. The T2I branch provides broad category and scene diversity, while a teacher-student scheme ensures label reliability by selectively retaining only prompt-specified categories. To strengthen supervision for rare classes, we introduce VRAIN (Verified Rare-class Augmentation via INstructed editing), a novel I2I editor. VRAIN inserts high-confidence instances at semantically appropriate locations within in-the-wild scenes, yielding semantically coherent and visually natural edits that reduce domain gaps and enable targeted augmentation. On the LVIS benchmark, our method surpasses existing baselines, improving overall AP by up to +4.0 points and rare-class AP by up to +9.5 points, while scaling effectively with backbone capacity. Our project page is available at https://seokhunchoi.github.io/TMI

</details>


### [29] [Metrics or Mirage? An Audit of Evaluation Inconsistencies in Colonoscopy Polyp Segmentation Benchmarks](https://arxiv.org/abs/2607.08203)
*Aisha Urooj,Zain Ul Abdien,Neelu Madan*

Main category: cs.CV

TL;DR: 本文系统审计了2015-2026年间发表的27篇结肠镜息肉分割论文，发现三个普遍的结构性问题：1) 25/27篇论文省略了有临床相关性的Hausdorff距离指标；2) 至少存在5种不兼容的train/test划分协议，导致Dice分数不可比；3) 26/27篇论文未进行统计显著性检验。作者重新评估了5个代表性模型，证实这些问题并非表面性的，并提出了一个五点的息肉分割报告检查表(PSRC)。


<details>
  <summary>Details</summary>
Motivation: 现有结肠镜息肉分割领域的进展报告主要通过少数公开基准上的排行榜比较来呈现，但这种表面上的进展难以验证。作者通过系统性审计发现该领域存在严重的评估方法学问题，包括关键指标缺失、评估协议不统一和缺乏统计检验，且通用的度量指南(Metrics Reloaded)未能有效渗透到该子领域。

Method: 方法包括三部分：1) 对27篇论文(2015-2026)进行系统审计，检查Hausdorff距离报告情况、训练/测试划分协议和统计显著性检验；2) 重新评估5个代表性模型，在3种受控协议下使用统一评分器进行对比实验；3) 提出五点Polyp Segmentation Reporting Checklist (PSRC)作为领域适配的轻量级纠正方案。

Result: 审计发现25/27篇省略Hausdorff距离、至少5种不兼容划分协议、26/27篇无统计检验。重新评估显示：报告的度量掩盖了严重的边界和召回失败；指标不同时"最佳"模型会发生变化；近乎并列的排名在不同随机划分下会发生反转。

Conclusion: 结肠镜息肉分割领域的进展报告存在系统性、结构性的评估问题，导致所报告的进步难以验证。作者提出的PSRC检查表可作为领域适配的轻量级纠正工具，以期改善该领域的评估规范。

与遥感交叉或改进方案: 交叉/改进可行性：85%。该论文所揭示的评估方法学问题在遥感图像分割领域同样普遍存在。具体交叉方案：1) 将Hausdorff距离引入遥感分割评估（如建筑物轮廓、道路边界、水域边界提取），该指标对边界精细度要求高的遥感任务具有直接临床等效价值；2) 对遥感领域常用数据集（如DeepGlobe、ISPRS Potsdam）的train/test划分协议进行统一审计，建立标准化协议；3) 引入统计显著性检验（如配对t检验、Wilcoxon符号秩检验）到遥感分割模型对比中；4) 借鉴PSRC思路，为遥感分割领域设计类似的Reporting Checklist，重点关注边界精度指标、划分协议透明度和统计检验。

Abstract (英文): Progress in colonoscopy polyp segmentation is routinely reported through leaderboard comparisons on a small set of public benchmarks. We argue that this apparent progress is difficult to verify: a systematic audit of \textbf{27 papers} published between 2015 and 2026 reveals three structural problems in how the community evaluates models. \textbf{First}, 25 of 27 papers \textit{omit the Hausdorff distance}. Hausdorff distance is a boundary-accuracy metric with direct clinical relevance for detecting flat or small polyps, and is a standard in radiotherapy segmentation. \textbf{Second}, at least five \textit{incompatible train/test split protocols} co-exist across papers reporting results on the same two datasets (Kvasir-SEG and CVC-ClinicDB), making published Dice scores non-comparable even when they appear in the same leaderboard column. \textbf{Third}, 26 of 27 papers make \textit{performance claims without any statistical significance test}. Strikingly, four papers published \emph{after} the Metrics Reloaded framework~\cite{metricsreloaded2024} (Maier-Hein et al., \textit{Nature Methods} 2024) perpetuate these same problems, suggesting that general-purpose metric guidance has not yet reached the colonoscopy sub-community. To show these problems are not merely cosmetic, we re-evaluate five representative models under three controlled protocols with a single uniform scorer, and find that the reported metric conceals large boundary and recall failures, that the ``best'' model changes with the metric, and that near-tied rankings reverse across random splits. We propose a five-point \textbf{Polyp Segmentation Reporting Checklist}~(PSRC) as a lightweight, domain-adapted corrective.

</details>


### [30] [Benchmark Evaluation of Feredated Learning on Multi-organ Images](https://arxiv.org/abs/2607.08219)
*Junbin Mao,Xu Tian,Jianchun Zhu,Ludi Li,Jin Liu*

Main category: cs.CV

TL;DR: 本文提出了MobenFL基准，一个包含20种先进联邦学习算法和22个医学影像数据集（覆盖12个关键器官）的综合评估框架，从性能、效率、隐私保护及多种真实临床场景等多维度对FL算法进行系统评估，旨在推动FL在医疗领域的可靠临床应用。


<details>
  <summary>Details</summary>
Motivation: 现有的联邦医学影像基准存在三个主要问题：未能充分纳入最新算法、局限于单一器官或模态的数据、过度强调模型准确率而忽视效率与隐私等关键维度，难以全面评估FL在真实医疗环境中的综合效果。因此需要一个统一、全面、客观的评估标准来推动FL技术向可靠的临床应用发展。

Method: MobenFL基准集成了20种前沿FL算法和22个医学影像数据集，覆盖人体12个关键器官。评估维度包括三个方面：模型性能（准确率等）、算法效率（计算/通信开销等）和隐私保护能力。同时针对不同疾病、设备和成像模态等复杂真实临床场景进行专项评估，构建多维度、全场景的评估框架。

Result: MobenFL在算法覆盖广度（20种算法）和数据集规模（22个数据集、12个器官）上超越了现有基准。通过多维度评估体系，能够更全面地反映FL算法在真实医疗环境中的综合表现，包括性能、效率和隐私保护的平衡。

Conclusion: MobenFL为联邦学习在医学影像领域的临床应用提供了一个统一、全面、深入的评估标准，有助于推动该技术向可靠的临床应用迈进。

与遥感交叉或改进方案: 交叉/改进可行性：85%。遥感领域同样面临数据隐私（如卫星影像的地理敏感信息）、传感器异构（不同卫星/无人机平台）和标注分散等类似问题。具体交叉方案：（1）将MobenFL的FL算法评估框架迁移至遥感影像分析，构建遥感专用的FL基准，覆盖不同传感器（光学、SAR、多光谱）和地理区域；（2）借鉴其多维度评估体系（性能+效率+隐私），用于遥感FL的统一标准化评测；（3）其针对不同设备、成像模态的专项评估策略可直接适用于遥感中不同分辨率、不同波段的传感器数据融合场景。

Abstract (英文): The privacy requirements of medical data and its substantial variations across organs and modalities hinder the clinical implementation of medical AI. Federated learning (FL) is a feasible approach to overcome these challenges. Due to the continuous emergence of FL algorithms and the highly heterogeneous nature of medical data, objectively evaluating their performance in real-world clinical settings remains difficult. Therefore, a comprehensive federated medical imaging benchmark, serving as a unified evaluation standard, is crucial for advancing the technology toward reliable clinical application. Existing federated medical imaging benchmarks have not yet adequately incorporated state-of-the-art algorithms, are limited to data from single organs or modalities, and overly emphasize model accuracy, making it difficult to comprehensively assess the overall efficacy of FL in real-world medical environments. To address these challenges, we developed the MobenFL benchmark. This benchmark integrates 20 cutting-edge FL algorithms and 22 medical imaging datasets, covering 12 critical organs across the human body, surpassing existing benchmark in breadth. In terms of evaluation dimensions, MobenFL not only assesses performance but also systematically incorporates key metrics such as algorithmic efficiency and privacy protection capabilities. Additionally, it conducts specialized evaluations for complex real-world clinical scenarios involving different diseases, devices, and imaging modalities, thereby providing a comprehensive and in-depth evaluation framework for the clinical application of FL in the medical field.

</details>


### [31] [LUMI: Tokenizer-Agnostic LLM-Based Lossless Image Compression](https://arxiv.org/abs/2607.08221)
*Chris Xing Tian,Chengkai Wu,Ziyu Wang,Rongqun Lin,Kecheng Chen,Xiandong Meng,Haoliang Li,Shiqi Wang,Siwei Ma*

Main category: cs.CV

TL;DR: LUMI提出了一种与分词器无关的无损图像压缩框架，通过像素嵌入模块、块内位置编码和256路预测头，将冻结的LLM应用于RGB图像压缩，在自然、医学和遥感图像上实现了有竞争力的压缩率，并提高了跨领域鲁棒性。


<details>
  <summary>Details</summary>
Motivation: 现有的基于LLM的无损图像压缩方法依赖于将像素值通过分词器转换为LLM可处理的token序列，这导致压缩性能受限于特定分词器行为、词汇表数值token和模型家族适配，缺乏跨模型的统一性。

Method: LUMI包含三个核心组件：1) 像素嵌入模块，将原始强度与通道信息映射到LLM的连续嵌入空间；2) 块内位置编码，保留展平后的二维空间结构；3) 256路预测头，输出原生像素字母表上的概率分布。仅训练像素嵌入、位置编码、软前缀参数和预测头，LLM骨干网络保持冻结。

Result: 在自然、医学和遥感图像基准上使用LLaMA、Qwen和Gemma骨干网络进行实验，LUMI提供了跨分词器家族的统一接口，实现了有竞争力的压缩率，并在跨领域鲁棒性上优于基于分词器的LLM压缩基线方法。

Conclusion: LUMI将基于LLM的无损图像压缩重新定义为冻结基础模型的像素空间自适应问题，而非分词器特定的语言符号建模问题，从而实现了模型无关的统一压缩框架。

与遥感交叉或改进方案: 交叉/改进可行性：85%。遥感图像通常包含高动态范围和多光谱特性，LUMI的像素嵌入模块可直接扩展以支持多光谱/高光谱通道输入；其冻结LLM主干加轻量可训练模块的设计适合遥感应用中常见的小样本场景；建议进一步引入波段间相关性建模和空间上下文感知的损失函数以提升遥感图像压缩效率。

Abstract (英文): Large language model (LLM)-based lossless image compression methods typically represent pixel data through the native text interface of a pretrained model, converting pixel values into token sequences that the LLM processes through its vocabulary head. This design shows that pretrained language models can provide probability estimates for image coding, but it also couples compression to tokenizer behavior, vocabulary-specific numeric tokens, and model-family-specific adaptation. In this paper, we present LUMI (LLM-based Unified Model-agnostic lossless Image compression), a tokenizer-agnostic framework for lossless RGB image compression with frozen LLM backbones. LUMI replaces pixel-as-text tokenization with a pixel embedding module that maps raw intensity and channel information into the continuous embedding space of the LLM. It further introduces intra-patch position encoding to retain two-dimensional spatial structure after flattening, and uses a 256-way prediction head to produce probabilities over the native pixel alphabet. Only the pixel embedding, position encoding, soft-prefix parameters, and prediction head are trained, while the LLM backbone remains fixed. Experiments on natural, medical, and remote-sensing image benchmarks with LLaMA, Qwen, and Gemma backbones show that LUMI provides a unified interface across tokenizer families, achieves competitive compression rates, and improves cross-domain robustness over tokenizer-based LLM compression baselines. These results formulate LLM-based lossless image compression as pixel-space adaptation of frozen foundation models rather than tokenizer-specific language-symbol modeling.

</details>


### [32] [Multimodal 3D LUT Generation via StatLUT with Statistical Features for Photorealistic Style Transfer](https://arxiv.org/abs/2607.08227)
*Yifan Wang,Zhixiang Hao,Yu Wang,Congchao Zhu*

Main category: cs.CV

TL;DR: StatLUT提出了一种无需预训练编码器的多模态3D LUT生成框架，通过Lab色彩统计特征提取、Transformer序列到序列映射和轻量扩散Transformer文本驱动，实现了高质量的光照风格迁移，显著优于现有方法。


<details>
  <summary>Details</summary>
Motivation: 现有深度学习光照风格迁移方法存在三大瓶颈：1) 预训练图像编码器导致语义纠缠，产生非自然空间扭曲；2) 像素级映射范式忽略色域拓扑，导致颜色条带伪影；3) 缺乏直观的文本驱动多模态控制能力。

Method: 1) Lab-Extractor：绕过预训练编码器，提取空间无关的统计特征，解耦颜色分布与结构语义；2) MR-Mapper：将3D LUT生成建模为基于Transformer的Seq2Seq翻译任务，预测拓扑平滑的3D LUT；3) H-Diffuser：轻量级扩散Transformer，从自然语言提示直接合成统计特征，实现文本驱动调色。

Result: 在标准基准测试上，StatLUT在视觉质量和量化指标上均显著优于现有最先进方法。

Conclusion: StatLUT开创了多模态光照风格迁移领域一种高度鲁棒且灵活的范式，通过解耦语义与色彩、保持色域拓扑、支持文本控制，解决了现有方法的根本性缺陷。

与遥感交叉或改进方案: 交叉/改进可行性：85%。遥感图像（如多光谱、高光谱影像）的色调校正与风格迁移可直接应用StatLUT框架：1) Lab-Extractor可推广至多光谱统计特征提取，解缠地物结构与光谱分布，避免分类任务中的语义干扰；2) MR-Mapper可用于遥感图像3D LUT色彩映射，消除不同传感器/时相间的色差与条带噪声；3) H-Diffuser支持通过自然语言（如"增强植被红边"）驱动遥感图像光谱调整，实现灵活的遥感图像增强与标准化。

Abstract (英文): Photorealistic Style Transfer (PST) aims to transfer the color and tonal style of a reference to a content image while strictly preserving its structural integrity. However, existing deep learning-based methods inherently suffer from semantic entanglement caused by pre-trained image encoders, leading to unnatural spatial distortions. Moreover, current pixel-level mapping paradigms often ignore color gamut topology, resulting in color banding, while also lacking the multimodal capability for intuitive text-driven control. To address these bottlenecks, we propose StatLUT, an innovative multimodal framework for 3D LUT generation. First, we bypass traditional encoders and introduce a Lab-Extractor to derive spatially-agnostic statistical features, fundamentally decoupling color distributions from structural semantics to ensure artifact-free rendering. Second, we formulate LUT generation as a Transformer-based Seq2Seq translation task, utilizing a Multi-dimensional Residual Mapper (MR-Mapper) to predict topologically smooth 3D LUTs. Finally, to break the single-modal barrier, we propose the H-Diffuser, a lightweight Diffusion Transformer that directly synthesizes statistical features from natural language prompts, enabling flexible text-driven color grading. Extensive experiments on standard benchmarks demonstrate that StatLUT significantly outperforms state-of-the-art methods in both visual quality and quantitative metrics, pioneering a highly robust and flexible paradigm for multimodal photorealistic style transfer.

</details>


### [33] [TVTA: Trajectory-Aware Viseme-Guided Temporal Aggregation for Event-Based Lip Reading](https://arxiv.org/abs/2607.08236)
*Jingrong Zheng,Hongwei Ren,Xiangqian Wu*

Main category: cs.CV

TL;DR: 本文提出了一种时间增强的事件驱动唇读框架，通过轨迹感知差分聚合(TDA)、视素引导聚合(VGA)和EMA教师-学生训练策略，解决了现有方法在时序建模不足和发音结构约束弱的问题，在DVS-Lip基准上验证了有效性。


<details>
  <summary>Details</summary>
Motivation: 现有事件驱动唇读方法在充分的时序建模之前进行空间压缩，抑制了区分相似唇动所需的关键稀疏局部运动轨迹；同时，大多数方法仅在词分类层面优化时序表示，对底层发音结构的约束不足。

Method: 1) 轨迹感知差分聚合(TDA)：在空间聚合前，先对各空间位置进行局部时序建模；2) 视素引导聚合(VGA)：由CTC解码器和视素引导门控聚合分支组成的统一时序模块，注入视素感知的序列监督；3) EMA教师-学生训练策略：增强在强事件扰动下的鲁棒性。

Result: 在DVS-Lip基准上验证了所提设计的有效性，消融实验进一步证实了TDA、VGA和教师-学生一致性的贡献。定性解码结果也表明，基于CTC的时序建模能从事件流中学习有意义的视素感知结构。

Conclusion: 所提时间增强框架有效提升了事件驱动唇读的性能，通过局部时序建模、视素引导监督和鲁棒训练策略实现了更优的视觉语音识别。

与遥感交叉或改进方案: 交叉/改进可行性：85%。1) TDA的轨迹感知差分聚合可用于遥感视频中微小运动目标的时空特征提取，如植被生长监测或冰川移动检测；2) VGA的视素引导机制可类比为语义引导的时序聚合，适用于遥感时序影像中的地物变化检测任务，引入CTC解码器进行序列监督；3) EMA教师-学生策略可增强遥感模型在不同光照、天气等扰动条件下的鲁棒性。

Abstract (英文): Event-based lip reading has recently emerged as a promising direction for visual speech recognition, benefiting from the high temporal resolution and motion sensitivity of event cameras. However, existing methods typically perform spatial compression before sufficient temporal modeling, which may suppress sparse and localized motion trajectories that are crucial for distinguishing similar lip movements. Moreover, most current approaches optimize temporal representations mainly at the word-classification level, leaving the underlying articulatory structure weakly constrained. To address these limitations, we propose a temporally enhanced framework for event-based lip reading. First, we introduce Trajectory-Aware Differential Aggregation (TDA), which performs local temporal modeling at each spatial location before adaptive spatial aggregation. Second, we propose Viseme-Guided Aggregation (VGA), a unified temporal module composed of a CTC decoder and a viseme-guided gated aggregation branch, which injects viseme-aware sequence supervision and improves final temporal aggregation for word recognition. Third, we incorporate an EMA teacher--student training strategy to enhance robustness under strong event perturbations. Experiments on the DVS-Lip benchmark verify the effectiveness of the proposed design, and extensive ablation studies further validate the contributions of TDA, VGA, and teacher--student consistency. Qualitative decoding results also demonstrate that the proposed CTC-based temporal modeling learns meaningful viseme-aware structure from event streams.

</details>


### [34] [Closing the Null Space: Guidance-Aware Quantization for Classifier-Free Diffusion](https://arxiv.org/abs/2607.08241)
*Abdullah Al Shafi,Sumaiya Rahim Suma*

Main category: cs.CV

TL;DR: 现有量化方法忽视CFG扩散模型的双分支条件/无条件结构，导致系统层面延迟被掩盖、算法层面无条件分支漂移。本文提出GAMP方法，通过直接校准引导预测、推导逐层激活位敏感度、贪心背包分配位宽，从构造上防止分支漂移。


<details>
  <summary>Details</summary>
Motivation: 现有后训练量化方法将CFG扩散模型视为单分支网络，忽略了其条件/无条件双分支结构特性。这导致：(1) 系统层面，两遍执行模式的开销被参数计数和比特操作指标完全掩盖，INT8推理栈无法实现理论效率提升；(2) 算法层面，仅针对引导间隙进行校准存在精确零空间——量化模型在间隙诊断指标上完美，但无条件分支发生漂移，破坏每次引导预测。

Method: 提出引导感知混合精度量化(GAMP)框架，包含三个关键组件：(1) 直接以引导预测输出为校准目标；(2) 从引导输出退化程度推导逐层激活位敏感度；(3) 通过贪心背包算法分配位宽，从构造上防止无条件分支漂移。

Result: 通过假阳性实验实证验证了分支漂移陷阱的存在：标准诊断下校准最好的模型同时产生了最差的样本质量。GAMP方法证明了其能够有效防止无条件分支漂移，实现更优的量化效果。

Conclusion: 本文揭示了CFG扩散模型量化中的结构盲点——分支漂移陷阱，并提出了GAMP方法从根本上解决该问题，实现了系统效率和算法精度的双赢。

与遥感交叉或改进方案: 交叉/改进可行性：85%。该论文的核心贡献在于揭示了CFG扩散模型双分支结构的量化盲点，遥感领域同样大量使用扩散模型进行图像生成（如遥感图像超分、去云、多模态融合等），且CFG方法在遥感条件生成中广泛使用。具体交叉方案：(1) 将GAMP方法直接迁移到遥感扩散模型的量化部署中，解决遥感模型在星载、边缘设备上的低延迟推理需求；(2) 遥感数据通常具有多光谱、高空间分辨率特性，可进一步研究逐波段/逐通道的位宽分配策略，扩展GAMP的感知范围；(3) 结合遥感任务特点（如地物分类精度对生成保真度的敏感要求），设计面向遥感下游任务的引导感知量化校准损失函数。

Abstract (英文): Deploying classifier-free guidance (CFG) diffusion models under real-world compute budgets requires quantization, yet existing post-training quantization (PTQ) methods treat CFG models as single-branch networks, ignoring the paired conditional/unconditional structure that CFG inference fundamentally relies on. This structural blind spot has two consequences. At the system level, the two-pass CFG execution pattern imposes a latency overhead that parameter-count and bit-operation metrics conceal entirely, and commodity INT8 inference stacks fail to realize the theoretical efficiency gains that BOPs calculations promise. At the algorithmic level, calibrating against the guidance gap alone admits an exact null space: a quantized model can achieve perfect gap-fidelity diagnostics while the unconditional branch drifts arbitrarily, corrupting every guided prediction at inference time. This paper terms this the branch-drift trap, proves its existence analytically, and confirms it empirically through a false-positive result in which the best-calibrated model by standard diagnostics simultaneously produces the worst sample quality. To close the trap, Guidance-Aware Mixed Precision (GAMP) is proposed, which calibrates directly on the guided prediction, derives per-layer activation-bit sensitivity from guided-output degradation, and allocates bits via a greedy knapsack -- provably preventing unconditional branch drift by construction.

</details>


### [35] [SkelGen4D: Weakly-Supervised Skeleton-Based 4D Generation for Text-Driven Mesh Animation](https://arxiv.org/abs/2607.08246)
*Hao Feng,Zhi Zuo,Jia-Hui Pan,Ka-Hei Hui,Zhengzhe Liu,Dian Zhang,Haoran Xie,Bin Sheng,Jingyu Hu*

Main category: cs.CV

TL;DR: SkelGen4D提出一种弱监督的骨架驱动4D生成框架，通过可微拟合从动画网格恢复时序一致的伪骨架，再以前馈方式生成文本条件骨架运动序列，并利用Motion-GRPO优化，实现无需逐帧骨架标注的文本驱动网格动画。


<details>
  <summary>Details</summary>
Motivation: 现有4D生成方法（如SDS优化、视频驱动动画）缺乏显式控制且与工业管线不兼容，难以进行灵活编辑和高质量动画制作。作者希望开发一种骨架驱动的动画框架，既保持显式控制能力，又无需昂贵的逐帧骨架标注。

Method: 1) 通过可微拟合从动画网格中恢复时序一致的伪骨架；2) 以前馈方式生成文本条件骨架运动序列；3) 使用Motion-GRPO优化确保时序连贯性、物理合理性和关节动画质量。

Result: 在Truebones Zoo和Diffusion4D两个大规模基准上，弱监督的骨架建模匹配甚至超越全监督基线，并扩展到多种物体类别，生成高质量的文本驱动网格动画。

Conclusion: SkelGen4D实现了弱监督下的骨架驱动4D生成，兼容标准动画生产管线，支持灵活的运动编辑，为文本驱动动画提供了实用且可编辑的解决方案。

与遥感交叉或改进方案: 交叉/改进可行性：65%。遥感领域常涉及时序3D建模（如城市变化监测、植被生长模拟），可将SkelGen4D的骨架驱动框架迁移至遥感时序几何建模：用语义关键点（如建筑物角点、树冠顶点）替代生物骨架，利用可微拟合从多时相点云/网格恢复语义骨架运动，结合文本描述（如“建筑物向北倾斜”、“树木逐年增高”）生成时序变化序列。但遥感数据精度要求高、运动模式与生物动画差异大，需重新定义骨架拓扑和运动先验。

Abstract (英文): We study 4D generation to synthesize temporally coherent sequences of 3D geometry for animation and content creation. In contrast to existing SDS-based optimization methods and video-driven animation approaches, we adopt a skeleton-driven animation framework aligned with standard industrial pipelines, which enables explicit control and editing. To this end, we propose SkelGen4D, a weakly supervised feed-forward framework for text-driven mesh animation that generates explicit skeleton motions without requiring per-frame skeleton annotations. SkelGen4D first recovers temporally consistent pseudo-skeletons from animated meshes via differentiable fitting, and then generates text-conditioned skeleton motion sequences in a feed-forward manner, further refined with Motion-GRPO to ensure temporally coherent, physically plausible, and articulated animation. We evaluate our method on two large-scale benchmarks, Truebones Zoo and Diffusion4D. Our results show that our weakly supervised skeleton modeling matches or surpasses fully supervised baselines while scaling to diverse object categories for high-quality text-driven mesh animation. Further, our method supports flexible motion editing and is aligned with standard animation production pipelines.

</details>


### [36] [HSA: Hierarchical Slot Attention for Multi-granularity Scene-Decomposition](https://arxiv.org/abs/2607.08249)
*Neelu Madan,Rongzhen Zhao,Andreas Mogelmose,Juho Kannala,Joni Pajarinen,Graham W. Taylor,Thomas B. Moeslund*

Main category: cs.CV

TL;DR: 本文提出分层槽注意力（HSA），从单一模型中学习多粒度语义场景分解，包括整体（前景/背景）、语义（物体类别）和全景（个体实例）三个层次，仅需10%标注数据，在COCO和PASCAL VOC上显著优于扁平化基线方法。


<details>
  <summary>Details</summary>
Motivation: 现有基于槽注意力的场景分解方法存在两个局限：一是将场景分解为单一粒度的扁平槽集合，二是仅基于外观而非语义进行分解。然而人类理解场景是通过语义层次结构进行的（前景/背景、物体类别、个体实例），且语义层次无法仅通过无监督学习从视觉模式中自动涌现。

Method: 提出分层槽注意力（HSA），在一个模型中同时学习三个层次的场景分解：整体层（前景/背景二元分割）、语义层（物体类别）和全景层（个体实例）。仅需10%标注数据，通过分层对齐损失联合训练三个层次。同时提出分组纯度和包含度两个度量指标，评估表示空间中层次结构的编码程度。

Result: 在COCO数据集上，HSA在整体层ARI提升+41.5，语义层+14.6，全景层+10.4；在PASCAL VOC上增益更大。仅需单个模型而非三个独立模型。

Conclusion: HSA通过引入语义层次结构和少量标注数据，有效克服了传统槽注意力方法在场景分解中的粒度单一和缺乏语义的问题，验证了在表示空间中编码层次结构的可行性。

与遥感交叉或改进方案: 交叉/改进可行性：90%。遥感场景（如高分辨率卫星影像）天然具有层次结构（场景级→地物类别→个体目标）。具体方案：（1）将HSA的三层分解直接应用于遥感图像，整体层对应城市/非城市区域，语义层对应建筑物/道路/植被等地物类别，全景层对应单个建筑物/车辆实例；（2）针对遥感影像标注成本高的问题，HSA仅需10%标注数据的特性极具优势；（3）需将注意力机制适配遥感影像的大尺度特征（如引入多尺度特征金字塔），并调整分组纯度/包含度指标以处理遥感中常见的地物重叠和模糊边界问题。

Abstract (英文): Slot attention is a powerful framework for object-centric learning, decomposing visual scenes into latent slots through iterative competitive attention. However, existing methods share two critical limitations: they decompose scenes into a flat set of slots at a single granularity, and this decomposition is based on appearance rather than semantics. Yet humans understand scenes through semantic hierarchies: separating foreground from background, recognizing object categories, and identifying individual instances. Crucially, such semantic hierarchies cannot emerge without supervision, because category names are human constructs, not visual patterns. We propose Hierarchical Slot Attention (HSA), which learns multi-granularity semantic scene decomposition from a single model. HSA decomposes scenes at three levels: holistic (foreground/background), semantic (object categories), and panoptic (individual instances). Using only 10\% labeled data, combined with hierarchical alignment loss, HSA learns all three levels jointly. We further introduce grouping purity and containment to measure whether the hierarchy is encoded in representation space, not just output masks. Experiments on COCO and PASCAL VOC demonstrate that HSA outperforms the strongest flat baseline by up to \textbf{$+$41.5} ARI at holistic, \textbf{$+$14.6} at semantic, and \textbf{$+$10.4} at panoptic level on COCO, with even larger gains on Pascal VOC, while requiring a single model instead of three. Code will be made available upon acceptance.

</details>


### [37] [On the Design of Mixture-of-Experts for Dynamic Gaussian Splatting](https://arxiv.org/abs/2607.08250)
*In-Hwan Jin,Hyeongju Mun,Joonsoo Kim,Kugjin Yun,Kyeongbo Kong*

Main category: cs.CV

TL;DR: 本文提出两种基于混合专家（MoE）框架的多变形建模策略——MoDE（联合优化）和MoE-GS（独立优化后路由组合），用于动态3D高斯表示中的新视图合成，解决了单一变形模型对不同动态场景鲁棒性不足的问题。


<details>
  <summary>Details</summary>
Motivation: 现有动态场景重建方法依赖单一变形模型，难以应对现实世界中异构且空间变化的复杂运动，鲁棒性不足。

Method: 从MoE视角出发，提出两种多变形建模策略：（1）MoDE，将多个变形专家直接集成到可变形高斯Splatting流程中，共享同一规范高斯表示，通过联合优化实现多变形建模；（2）MoE-GS，变形专家独立优化，通过独立路由阶段组合，专家交互发生在非规范高斯表示上。

Result: 两种方法提供了多变形建模的替代策略，阐明了不同集成约束如何影响动态3D高斯表示中变形专家的设计与行为。

Conclusion: 多变形建模能够提升动态场景重建的鲁棒性，MoDE与MoE-GS分别在联合优化与独立优化两种范式下有效，为动态3D高斯表示的设计提供了新视角。

与遥感交叉或改进方案: 交叉/改进可行性：85%。遥感领域存在大量动态场景（如车辆运动、植被生长、云层变化），可借鉴MoDE/MoE-GS框架处理异质运动：将不同变形专家分别建模遥感中的刚体运动（车辆）、非刚体运动（植被摆动）和拓扑变化（建筑拆除），通过MoE路由自动选择合适变形模型，提升遥感4D重建的鲁棒性。

Abstract (英文): Dynamic scene reconstruction remains challenging due to the heterogeneous and spatially varying nature of real-world motion. Although recent 3D Gaussian Splatting methods have introduced diverse deformation formulations for dynamic novel view synthesis, each method typically relies on a single deformation model within its representation, which limits robustness across diverse dynamic scenarios. In this work, we study a fundamental problem-multi-deformation modeling for dynamic 3D Gaussian representations-under two distinct integration constraints that differ in when and how multiple deformation experts interact during training. From a Mixture-of-Experts (MoE) perspective, we view multi-deformation modeling as the problem of combining multiple specialized deformation models within a unified 3D representation. We first introduce Mixture of Deformation Experts (MoDE), which integrates multiple deformation experts directly into the deformable Gaussian Splatting pipeline through joint optimization. In MoDE, experts operate on a shared canonical Gaussian representation, enabling multi-deformation modeling without introducing additional training stages or modifying the original optimization schedule. In contrast, we further present Mixture of Experts for Dynamic Gaussian Splatting (MoE-GS) under a different integration constraint, where deformation experts are optimized independently and combined through a separate routing stage. As a result, expert interaction occurs over non-canonical Gaussian representations after individual optimization. Together, these two approaches provide alternative strategies for multi-deformation modeling, clarifying how integration constraints shape the design and behavior of deformation experts in dynamic 3D Gaussian representations. Our code is available at: https://github.com/cvsp-lab/MoE-GS-studio.

</details>


### [38] [UniRef-UAV: A Multimodal Benchmark for Universal Referring in UAV Imagery](https://arxiv.org/abs/2607.08267)
*Haibin Tian,Huichao Xie,Xuelin Qian,Ruitao Lu,Junwei Han,Dingwen Zhang*

Main category: cs.CV

TL;DR: 本文针对无人机视觉定位任务中现有方法仅支持纯文本查询和单目标输出的局限，提出了通用指代（Universal Referring）任务框架，构建了支持文本、图像、文本+图像多模态查询以及无目标、单目标、多目标输出的UniRef-UAV基准数据集，并设计了基于检测范式的UAV-URNet基线模型，通过集合预测实现变尺寸目标集输出。


<details>
  <summary>Details</summary>
Motivation: 现有无人机视觉指代表达理解（REC）基准和方法主要局限于纯文本查询和单目标输出，无法处理实际无人机场景中涉及的参考图像、多模态指令、目标缺失、多个有效目标实例等情况，限制了在复杂空中场景下的实用性。

Method: 1) 提出Universal Referring任务，扩展查询模态（文本、图像、文本+图像）和输出基数（无目标、单目标、多目标）；2) 构建UniRef-UAV多模态基准，包含域内和跨域评估协议；3) 提出UAV-URNet检测式基线，将异构查询映射到共享查询空间，通过集合预测预测变尺寸目标集。

Result: UAV-URNet相比大型通用多模态大语言模型（MLLMs）在无目标判别上更一致，实现更轻量级和可复现的实现；多模态查询有助于减少视觉查询歧义并促进更统一的查询-目标对齐空间。

Conclusion: 本文提出的通用指代任务和基准有效填补了现有无人机REC任务在多模态查询和变基数输出方面的空白，UAV-URNet作为基线具有稳定、可复现、轻量的优势，多模态融合策略被证明能提升查询-目标对齐的鲁棒性。

与遥感交叉或改进方案: 交叉/改进可行性：95%。该工作直接面向无人机遥感场景，未来可从以下方面改进：1) 引入多尺度特征融合机制以提升对航拍图像中小目标的检测能力；2) 结合遥感图像地理空间信息（如GPS坐标、高度）作为辅助查询模态；3) 扩展到动态视频流场景中的连续指代跟踪；4) 探索更高效的异构查询对齐策略（如对比学习、跨模态注意力蒸馏）。

Abstract (英文): Unmanned aerial vehicles (UAVs) increasingly rely on visual grounding capabilities to localize task-relevant targets from diverse instructions in complex aerial scenes. Existing referring expression comprehension (REC) benchmarks and methods, however, are largely built around text-only queries and single-object outputs, which limits their applicability to practical UAV scenarios involving reference images, multimodal instructions, absent targets, and multiple valid target instances. To address this gap, we introduce \emph{Universal Referring}, a generalized UAV referring task that jointly expands the query modality and the output cardinality. We construct \emph{UniRef-UAV}, a multimodal benchmark that supports text-only, image-only, and text+image queries with modality-dependent target cardinality, where text-only and text+image queries admit no-target, single-target, and multi-target grounding while image-only queries focus on existence-aware single-instance grounding. It also provides in-domain and cross-domain evaluation protocols for visual-query generalization. We further present \emph{UAV-URNet}, a detection-style baseline that maps heterogeneous queries into a shared query space and predicts variable-size target sets through set prediction. Extensive experiments show that UAV-URNet provides a stable and reproducible baseline with more consistent no-target discrimination and a more lightweight, reproducible implementation than large general-purpose MLLMs. Additional domain analysis, query-representation analysis, and ablation studies demonstrate that multimodal queries help reduce visual-query ambiguity and promote a more unified query--target alignment space. The annotations, visual query crops/images, train/validation/test splits, evaluation scripts, and baseline code will be made publicly available to facilitate reproducible research.

</details>


### [39] [Progression as Latent Drift: Generative Forecasting of Slow-Evolving Pathologies](https://arxiv.org/abs/2607.08270)
*Yuxiang Feng,Juncheng Wang,Chao Xu,Wenlong Hou,Huihan Wang,Yijie Qian,Yang Liu,Baigui Sun,Yong Liu,Shujun Wan*

Main category: cs.CV

TL;DR: 本文提出Latent Drift框架，通过压缩语义表示学习渐进变化，解决纵向MRI中神经退行性疾病预测的恒等坍塌和连续插值陷阱问题，在3D脑MRI上优于扩散模型和自回归Transformer基线。


<details>
  <summary>Details</summary>
Motivation: 纵向MRI中疾病进展信号微弱，现有生成模型难以捕捉真实变化，主要面临两个问题：(1) 恒等坍塌——优化被当前解剖结构主导；(2) 连续插值陷阱——平滑网络无法区分局部生物漂移和噪声，导致虚假变化。

Method: 提出Latent Drift渐进式生成框架：在压缩语义表示空间中学习变化而非直接合成全分辨率解剖结构，消除像素级恒等目标；并采用有限标量量化（FSQ）抑制高频噪声，保留一致的结构漂移。

Result: 在纵向3D脑MRI实验中，Latent Drift在生成保真度和临床相关评估指标上均优于扩散模型和自回归Transformer基线。

Conclusion: Latent Drift有效解决了低信噪比场景下神经退行性疾病进展预测的核心问题，为早期干预和临床试验设计提供了更可靠的生成式预测工具。

与遥感交叉或改进方案: 交叉/改进可行性：85%。该框架可直接迁移至遥感时序变化检测与预测任务：(1) 将脑区结构替换为地表覆盖/植被等遥感语义特征，在潜在空间中学习渐进式变化；(2) FSQ可抑制遥感影像中云层、光照等高频噪声扰动；(3) 适用于森林退化、城市扩张、水体变化等慢速地表演变过程的长时序预测，改进方向为引入多模态遥感数据（SAR+光学）的联合语义嵌入。

Abstract (英文): Forecasting the future anatomy of slow-evolving neurodegenerative diseases could enable earlier, more targeted intervention and improve clinical trial design, but it remains challenging because true progression signals are subtle in longitudinal MRI. In this low-signal regime, transferring modern generative sequence models directly is unreliable: training is dominated by stable baseline anatomy and confounded by dense, sample-specific nuisance variation. We first provide a theoretical analysis that explains these failures through two modes. Identity collapse occurs when optimization is driven toward reproducing the current anatomy, which prevents the model from learning faint temporal change. The continuous interpolation trap arises when standard smooth networks cannot separate localized biological drift from pervasive noise, which leads to spurious changes that diffuse across the volume. To address both issues, we propose Latent Drift, a progressive generative framework that learns change in a compressed semantic representation rather than synthesizing full-resolution anatomy. This design removes pixel-level identity from the prediction target and concentrates model capacity on progression-relevant dynamics. We further apply Finite Scalar Quantization to the learned change representation, which suppresses small, high-frequency nuisance fluctuations while preserving consistent structural drift. Experiments on longitudinal 3D brain MRI show that Latent Drift improves patient-specific neuro-forecasting over diffusion and autoregressive transformer baselines across generative fidelity and clinically relevant evaluation metrics. Project page: \href{https://cutepkq.github.io/latent-drift}{https://cutepkq.github.io/latent-drift}.

</details>


### [40] [Enhancing the KidSat Model: Integrating Geographical Encoding and Data Quality Assessment for Childhood Poverty Prediction](https://arxiv.org/abs/2607.08281)
*Hou Hin Ip,Ka Nam Lam,Joshua Man Yu Ng,Makkunda Sharma,Seth Flaxman,Codie Gerlach-Wood,H Juliette T Unwin*

Main category: cs.CV

TL;DR: 本文针对基于卫星影像的贫困映射中存在的监督噪声、图像质量问题和空间结构缺失三大挑战，在KidSat框架基础上提出改进流程：通过DHS重聚合降低标签维度（103→51）、引入两阶段图像质量筛选、融合DINOv2视觉嵌入与球谐函数（SH）位置特征。实验表明，这些改进使MAE从0.2167降至0.1759（降幅18.83%），在33个非洲国家扩展实验中实现MAE 0.1658。研究发现SH特征持续优于纯图像基线，而梯度提升树（XGBoost/LightGBM）能有效利用融合特征的非线性交互。


<details>
  <summary>Details</summary>
Motivation: 卫星影像贫困映射面临三个核心障碍：1）调查数据提供的监督信号稀疏且含噪声；2）影像质量问题（云覆盖、损坏）；3）纯图像模型缺乏显式空间结构。现有方法难以同时解决这些问题，亟需一种可扩展、仅使用公开数据的改进方案。

Method: 1）标签优化：通过DHS重新聚合将独热编码维度从103降至51，缓解高基数稀疏性；2）质量筛选：两阶段流程筛选严重云覆盖或损坏的观测；3）特征融合：将DINOv2视觉嵌入与球谐函数（SH）位置特征结合，替代单纯坐标MLP（SIREN）增强。使用梯度提升树（XGBoost/LightGBM）作为预测头。

Result: 簇级严重贫困比例MAE从0.2167降至0.1759（降幅18.83%）；扩展至33个非洲国家时最优配置MAE为0.1658；SH特征持续提升性能；梯度提升树头优于其他预测头；高阶坐标MLP（SH+SIREN）在缺乏精心设计目标时可能表现不佳。

Conclusion: 通过标签重聚合、质量筛选和视觉-地理特征融合三大改进，显著提升了基于卫星影像的贫困预测精度。球谐函数编码提供稳定的空间先验，梯度提升树有效捕获非线性交互。该流程仅使用公开数据，具有可扩展性和原则性，为社会经济预测提供了实用范式。

与遥感交叉或改进方案: 交叉/改进可行性：95%。本文本身就是遥感与社会经济预测交叉的工作，可从以下方面进一步改进：1）引入多时相影像以缓解云覆盖问题，利用时间序列信息填补缺失；2）结合哨兵1号SAR数据穿透云层，增强抗噪性；3）在SH特征基础上引入更丰富的辅助地理数据（如夜间灯光、道路密度、POI）；4）探索视觉-地理特征的跨模态对比学习预训练；5）将梯度提升树头替换为轻量级Transformer，在保持非线性交互建模的同时提升端到端可训练性。

Abstract (英文): Accurate poverty mapping using satellite imagery is often hindered by (i) noisy and sparse survey-derived supervision, (ii) image quality issues such as cloud cover and image corruption, and (iii) lack of explicit spatial structure in image-only models. Building on the KidSat framework, we develop an enhanced pipeline that improves predictive accuracy via refined data preprocessing, systematic image quality assessment, and mathematically defined geographic encoding. First, we refine the fine-tuning target matrix by resolving high-cardinality sparsity and reducing one-hot dimensionality from 103 to 51 via DHS re-aggregation. Second, we introduce a simple two-stage quality-screening procedure to filter heavily clouded or corrupted observations. Third, we fuse DINOv2 visual embeddings with Spherical Harmonics (SH) location features. Across extensive experiments, these changes reduce MAE from 0.2167 to 0.1759, corresponding to an 18.83% relative reduction on the cluster-level severe-deprivation proportion scale. When extended from 16 to 33 African countries, the best-performing configuration achieves an overall MAE of 0.1658. We find that SH features consistently improve performance over the image-only backbone, whereas higher-capacity coordinate Multi Layer Perception augmentation (SH+SIREN) can underperform without carefully designed objectives. Finally, gradient-boosted tree heads (XGBoost/LightGBM) most effectively exploit nonlinear interactions in the fused visual-geographic representation. These findings provide a scalable and principled recipe for improving satellite-based socioeconomic predictions using only publicly accessible data.

</details>


### [41] [ARGUS: Accelerated, Robust, General, and Unsupervised Cell Tracking Solutions](https://arxiv.org/abs/2607.08297)
*Noah Jaitner,Kandice Tanner,Ingolf Sack,Hossein S. Aghamiry*

Main category: cs.CV

TL;DR: ARGUS是一个无需训练数据和GPU的模块化、可解释的无监督细胞追踪框架，结合自适应检测、光流预测和轨迹优化，在Cell Tracking Challenge数据集上实现了高精度（检测0.905-0.971，追踪0.897-0.964）和快速运行（1分钟内）。


<details>
  <summary>Details</summary>
Motivation: 定量分析细胞动力学是现代生物学研究的关键，但自动细胞追踪因噪声、形态变化、细胞重叠及分裂融合等动态事件而面临挑战，现有方法依赖训练数据和GPU资源。

Method: ARGUS框架包含四个模块：自适应细胞检测、密集Farneback光流预测、帧间线性分配以及序列级轨迹片段优化（重新连接短时间间隔内的轨迹碎片）。

Result: 在公开Cell Tracking Challenge数据集上，检测精度0.905-0.971，追踪精度0.897-0.964，运行时间在1分钟内（每3帧仅需5-6秒）。

Conclusion: ARGUS是一个模块化、可解释的框架，无需训练数据和GPU即可适配不同成像模态和生物学应用，代码已开源。

与遥感交叉或改进方案: 交叉/改进可行性：75%。具体方案：1) 将ARGUS的自适应检测与光流追踪模块迁移至遥感视频目标追踪任务（如车辆、船只跟踪），替代传统检测+跟踪范式；2) 轨迹片段优化（tracklet-refinement）可用于修复遥感时序影像中因遮挡或云层导致的轨迹断裂问题；3) 光流预测可拓展至遥感影像中的运动场估计，辅助变化检测与动态监测；4) 需注意遥感影像空间分辨率与细胞显微图像的尺度差异，需调整检测和光流参数适配。

Abstract (英文): Background and Objective: Quantitative analysis of cell dynamics is central to modern biological research, providing critical insights into immune cell interactions, disease progression, and drug mechanisms. Automated cell tracking in time-lapse microscopy remains challenging due to noise, morphological variations, overlapping cells, and dynamic events such as divisions and fusions.
  Methods: We present ARGUS, a framework for Accelerated, Robust, General, and Unsupervised Cell Tracking Solutions. ARGUS combines adaptive cell detection, dense Farneback optical-flow prediction, frame-to-frame linear assignment, and a sequence-level tracklet-refinement step that reconnects trajectory fragments across short temporal gaps.
  Results: On publicly available Cell Tracking Challenge datasets, ARGUS achieved detection accuracy of 0.905-0.971 and tracking accuracy of 0.897-0.964, with runtimes within 1 minute (5-6 seconds for 3 frames).
  Conclusions: ARGUS is a modular, interpretable framework that can be adapted to different imaging modalities and biological applications without training data or GPU infrastructure. The implementation is publicly available at https://github.com/Gitinc/argus

</details>


### [42] [Texture Representations in Deep Vision Models: Comparing CNNs, Vision Transformers, and Human Perception](https://arxiv.org/abs/2607.08321)
*Ludovica de Paolis,Marco Baroni,Alessandro Laio,Eugenio Piasini*

Main category: cs.CV

TL;DR: 该研究比较了CNN与三种Vision Transformers（ViTs）在纹理感知任务上的表现，发现ViTs的内部表征与人类心理物理学数据更为一致，且ViTs对纹理的编码更接近人类视觉处理方式。


<details>
  <summary>Details</summary>
Motivation: 尽管CNN在生物视觉建模中广泛应用，但其与人类视觉系统的对齐程度在非标准物体识别任务（如纹理感知）上尚不明确。本研究旨在探索CNN与ViT在纹理感知上的表征是否与人类视觉一致。

Method: 使用三种不同算法从相同源图像生成不同复杂度的纹理刺激；采用基于秩的统计量量化CNN和三种ViT内部表征中的信息量；比较模型表征与人类心理物理学数据的相似性。

Result: 不同ViT之间纹理表征对齐，但ViT与CNN之间不对齐；ViT对不同复杂度纹理形成相似表征；ViT表征比CNN表征能更好预测人类纹理识别表现。

Conclusion: ViT可能比CNN更忠实地捕捉人类对纹理模式的视觉处理方式；计算模型对纹理刺激的表征可能由网络架构驱动。

与遥感交叉或改进方案: 交叉/改进可行性：85%。方案：1) 遥感图像中纹理信息丰富（如植被、土壤、水体纹理），可借鉴该文方法比较ViT与CNN在遥感纹理分类任务上的表征与人类遥感判读专家感知的一致性；2) 将基于秩的统计量用于评估遥感深度学习模型对地物纹理的表征质量；3) 探索ViT架构用于遥感图像的纹理特征提取，以提升复杂地物识别精度。

Abstract (英文): In computational vision science, Convolutional Neural Networks (CNNs) have emerged as a popular model of biological vision because of the alignment they can exhibit with neural and behavioral data in humans and animals. However, it remains unclear to what extent this alignment persists for visual tasks that extend beyond the canonical object recognition paradigm based on well defined semantic content. In this study, we diverge from the common object-centric view by focusing on another aspect of vision: texture perception. We consider textures of different complexity generated with three different algorithms from the same source images. Using a rank-based statistic, we quantify the information encoded in the internal representations of a CNN and three Vision Transformers (ViTs), and we compare the similarity of these representations to those inferred from human psychophysics data. We find that the representation of textures is aligned in different ViTs, but not between the ViTs and the CNN; that ViTs form similar representations for textures of different complexity; that human performance in recognizing textures can be better predicted from ViTs representations rather than CNN representations. Taken together, these results suggest that ViTs may capture more faithfully than CNNs how texture patterns are visually processed by humans, and that the representations of texture stimuli in computational models may be driven by the network architecture.

</details>


### [43] [WCog-VLA: A Dual-Level World-Cognitive Vision-Language-Action Model for End-to-End Autonomous Driving](https://arxiv.org/abs/2607.08375)
*Xuerun Yan,Zhexi Lian,Nuoheng Zhang,Shiyu Fang,Haoran Wang,Chen Lv,Jia Hu,Binyang Song*

Main category: cs.CV

TL;DR: 提出WCog-VLA，一种双层级世界认知VLA框架，将语义世界预测与生成式世界演化相结合，在NAVSIM基准上以85k Game-CoT数据集实现PDMS 92.9的SOTA性能，实现主动式自动驾驶。


<details>
  <summary>Details</summary>
Motivation: 现有VLA模型缺乏全面的世界认知或存在碎片化世界预见，本质上局限于反应式驾驶，无法实现主动式自动驾驶。

Method: WCog-VLA包含双层级：(1) 语义级——融合3D空间感知和agent token捕获世界动态，实现Game-CoT推理；(2) 生成级——提出Aligned Decoupled Diffusion Transformer (ADDT)作为生成式世界模型，通过场景表示对齐减少去噪步骤、加速推理，合成物理合理的联合多智能体轨迹。同时构建85k Game-CoT注释数据集。

Result: 在NAVSIM基准上达到SOTA PDMS分数92.9。

Conclusion: WCog-VLA通过双层级世界认知框架弥合了语义世界预测与生成式世界演化之间的鸿沟，成功实现主动式自动驾驶。

与遥感交叉或改进方案: 交叉/改进可行性：85%。具体方案：(1) 语义级3D空间感知可直接迁移至遥感场景理解，如利用agent token机制捕获遥感影像中多目标（车辆、建筑物、植被）的动态变化；(2) ADDT生成式世界模型可用于遥感时序数据中多目标轨迹预测与变化检测，通过场景表示对齐加速遥感大范围推理；(3) Game-CoT推理策略可应用于遥感智能解译中的多智能体协同决策，如无人机集群路径规划与遥感任务分配；(4) 85k注释数据集构建方法可借鉴用于遥感领域大规模场景理解数据的标注策略。

Abstract (英文): Vision-Language-Action (VLA) models have advanced end-to-end autonomous driving. However, existing methods either lack comprehensive world cognition or suffer from fragmented world foresight, inherently confining these models to reactive driving. To address this limitation, we propose WCog-VLA, a novel dual-level World-Cognitive VLA framework that successfully bridges semantic world forecasting with generative world evolution to achieve proactive autonomous driving. At the semantic level, WCog-VLA unifies world cognition and reasoning by incorporating 3D spatial perception and injecting agent tokens to capture the world dynamics, while concurrently enabling Game-theoretic Chain-of-Thought (Game-CoT) reasoning. At the generative level, we introduce the Aligned Decoupled Diffusion Transformer (ADDT) as a powerful generative world model that synthesizes physically-plausible joint multi-agent trajectories. Through scene representation alignment, ADDT reduces the number of denoising steps required and thus significantly accelerates inference. To facilitate strategic reasoning, we further construct a large-scale dataset featuring 85k Game-CoT annotations. Extensive experiments on the NAVSIM benchmark demonstrate that WCog-VLA achieves a State-Of-The-Art (SOTA) PDMS score of 92.9.

</details>


### [44] [Classical versus Deep Mirror-Symmetry Scoring: A Benchmark of Thirteen Methods](https://arxiv.org/abs/2607.08379)
*Maximilian Woehrer*

Main category: cs.CV

TL;DR: 本文在统一统计协议下基准测试了13种图像对称性评分方法（9种来自文献+4种新提出），涵盖经典特征到冻结深度特征。结果表明：深度骨干网络在简单任务上最优，但经典HOG描述子在精度上接近最优深度方法（差距小但显著），且CPU运行速度快约300倍。对称性判别集中于中尺度定向特征，冻结深度特征相比调优的经典描述子优势有限。


<details>
  <summary>Details</summary>
Motivation: 现有图像对称性评分方法众多但缺乏在统一统计协议下的公平比较，无法评估各方法的真实相对性能，阻碍了该领域的系统进展。

Method: 收集9种文献方法和提出4种新方法，共13种评分方法。在4个单轴和5个多轴数据集上，采用反射精确协议（reflection-exact protocol），以基于机会锚定（chance-anchored）且经过显著性检验的判别技能（discrimination skill）作为评估指标进行基准测试。

Result: 深度骨干网络在单轴和较难的多轴协议上表现最佳。但经典HOG描述子与最佳冻结网络读取方法的性能差距虽显著却很小，且与第二名（CNN滤波器度量）在统计上不可区分，CPU运行速度却快约300倍。对称性判别信息集中在定向特征的中尺度上，深度骨干网络在中低阶段达到峰值，HOG在中等的细胞大小（cell size）达到峰值。

Conclusion: 对于测量对称性，冻结深度特征相比调优的经典描述子优势有限；任务训练的深度评分器能否做得更好仍待探索。发布了开源工具包imgsym。

与遥感交叉或改进方案: 交叉/改进可行性：85%。该论文的对称性评分协议和方法可直接应用于遥感图像分析：（1）在遥感建筑物/道路提取中，利用对称性评分作为后处理约束，提升检测结果的规则性和完整性；（2）对高分辨率遥感图像中的人造目标（如桥梁、体育场、广场）进行对称性量化，辅助目标识别与分类；（3）将HOG对称性评分方法引入遥感变化检测，利用对称性特征作为不变性描述子降低视角变化导致的误检；（4）改进方向：可结合遥感多光谱/高光谱特征，将对称性评分扩展到光谱-空间联合域。

Abstract (英文): Quantifying how mirror-symmetric an image is about a given axis (symmetry scoring) underpins applications from visual aesthetics to medical imaging, yet proposed scoring methods have never been compared on a common, statistically grounded protocol. We benchmark 13 scoring methods (nine collected from literature, four introduced here) spanning from classical features to frozen deep features, across four single-axis and five multi-axis datasets under a reflection-exact protocol with a chance-anchored, significance-tested discrimination skill. Deep backbones perform best on single-axis and harder multi-axis protocols. However, a classical histogram-of-oriented-gradients (HOG) descriptor trails the best frozen-network readout by a small (but significant) margin, is not statistically separable from the runner-up (a CNN-filter measure), and runs ~300x faster on CPU. Our results show that discrimination concentrates in mid-scale oriented features, where deep backbones peak at a low or mid stage, and HOG peaks at a mid cell size. Among existing methods, frozen deep features thus offer little over a tuned classical descriptor for measuring symmetry; whether task-trained deep scorers can do better remains open. We release the scorers and harness in imgsym, an open toolkit for image symmetry detection and measurement.

</details>


### [45] [Attribute Retrieving for Open-Vocabulary Endoscopic Compositional Referring Segmentation](https://arxiv.org/abs/2607.08397)
*Shun Liu,Nan Xi,Yang Liu,Tianyu Luan,Xuan Gong,David Doermann*

Main category: cs.CV

TL;DR: 本文针对内窥镜图像中细粒度指代分割的挑战，提出了大规模基准数据集ReferEndoscopy和属性检索驱动的AR-ERIS框架，在开放词汇场景下实现了内窥镜指代分割的SOTA性能。


<details>
  <summary>Details</summary>
Motivation: 指代图像分割(RIS)在内窥镜图像领域面临两大挑战：(1)高质量标注数据稀缺；(2)内窥镜场景中复杂的领域特定图像-文本关系难以被现有视觉语言模型捕获，导致细粒度文本线索理解不足、泛化性能受限。

Method: (1)构建了ReferEndoscopy——内窥镜领域首个大规模RIS基准数据集；(2)提出AR-ERIS框架，利用属性检索(Attribute Retrieval)实现开放词汇的内窥镜组合式指代分割，并在ReferEndoscopy上进行预训练。

Result: 在模拟和真实内窥镜数据上均取得了SOTA性能，并展现出强大的泛化能力。

Conclusion: 通过构建领域专用数据集和属性检索驱动的分割框架，有效解决了内窥镜RIS任务中标注稀缺和细粒度语义理解不足的问题，为内窥镜图像的智能理解提供了新方案。

与遥感交叉或改进方案: 交叉/改进可行性：85%。遥感图像与内窥镜图像均存在标注稀缺、领域专业性强、图像-文本关系复杂等共性挑战。具体交叉方案：(1)借鉴ReferEndoscopy的构建思路，建立遥感领域的大规模指代分割基准数据集；(2)将AR-ERIS的属性检索机制迁移到遥感场景，用于捕捉地物目标的多属性描述（如"新建的高层住宅楼"），实现开放词汇的遥感图像指代分割；(3)可引入遥感特有的空间关系和光谱属性进行检索增强，进一步提升细粒度分割性能。

Abstract (英文): Referring Image Segmentation (RIS) aims to segment image regions specified by natural language, enabling fine-grained and controllable visual understanding. Extending RIS to endoscopic imagery, however, presents unique challenges, including scarce high-quality annotations and complex, domain-specific image-text relationships. Although recent vision-language models demonstrate strong cross-domain alignment, they often fail to capture fine-grained textual cues in endoscopic settings, resulting in suboptimal performance and limited generalization. To address these challenges, we introduce ReferEndoscopy, a large-scale benchmark for RIS in the endoscopy field. Building on this dataset, we propose the Attribute Retrieval-based Endoscopic-RIS (AR-ERIS) framework for open-vocabulary endoscopic compositional referring segmentation. AR-ERIS leverages attribute retrieval for open-vocabulary endoscopic compositional referring segmentation and is pretrained on the curated ReferEndoscopy dataset, achieving state-of-the-art performance with strong generalization across both simulated and real-world endoscopic data. The dataset and code will be publicly released upon completion of the review process.

</details>


### [46] [Swapping Faces, Saving Features: A Dual-Purpose Pipeline for Pedestrian Privacy in ITS](https://arxiv.org/abs/2607.08402)
*Roba H. Farouk,Catherine M. Elias*

Main category: cs.CV

TL;DR: 本文提出了一种五阶段人脸替换流水线，在保护行人隐私（身份隐藏）的同时保留面部关键属性，以平衡自动驾驶数据集（Egy-DRiVeS）的隐私性与数据可用性。Roop模型在面部替换任务中优于Ghost-v2。


<details>
  <summary>Details</summary>
Motivation: 自动驾驶行人意图与轨迹预测模型需要大规模多样化数据集，但这些数据集的公开访问存在身份盗窃、行人跟踪等严重隐私风险。现有隐私保护方法虽能保护隐私，但会降低图像可用性，影响模型训练效果。因此需要一种既能保护隐私又能保持数据可用性的方法。

Method: 提出一个五阶段流水线用于行人面部替换，具体包括：(1) 人脸检测与提取；(2) 面部属性保留处理；(3) 人脸替换（采用Roop模型）；(4) 后处理与融合；(5) 质量评估。对比评估了Roop和Ghost-v2两种人脸替换模型，最终选用性能更优的Roop模型。

Result: Roop模型在面部替换任务中多方面优于Ghost-v2模型，能够在隐藏身份的同时有效保留面部关键属性，平衡了隐私保护与数据可用性。

Conclusion: 所提出的五阶段流水线结合Roop模型能够有效解决自动驾驶数据集中行人隐私保护问题，在不破坏数据训练可用性的前提下实现身份隐藏，特别适用于Egy-DRiVeS数据集。

与遥感交叉或改进方案: 交叉/改进可行性：85%。具体方案：(1) 遥感目标检测中的人脸/目标隐私保护——可将该五阶段流水线迁移至遥感图像中的人脸、车牌等敏感目标模糊/替换处理，保护遥感数据中的个人隐私；(2) 遥感行人/车辆轨迹预测——该隐私保护方法可应用于遥感视频中的行人或车辆轨迹预测数据集，在保护目标身份的同时保留运动特征和外观属性；(3) 多源数据融合——将面部替换技术扩展到遥感与地面摄像头的跨视角行人重识别任务中，实现跨模态隐私保护；(4) 边缘部署优化——参考该流水线的轻量化设计思路，将隐私保护模块部署到遥感无人机等边缘设备上，实现实时隐私处理。

Abstract (英文): Large-scale and diverse datasets are needed to train AI models to take real-time decisions for autonomous vehicles (AVs), an intelligent transportation system (ITS) application. Pedestrian intention and trajectory prediction are critical models used in AVs, requiring datasets involving diverse pedestrian images. Unrestricted access to these datasets imposes serious security risks, like identity theft and pedestrian tracking. The challenge is to apply privacy preservation procedures while maintaining the image attributes needed to train the models. Existing privacy methods may preserve the pedestrian's privacy, but degrade the image usability, which hinders the models' effectiveness. This work's focus is to implement a five-stage pipeline to protect pedestrians' privacy through face swapping while keeping the essential facial attributes intact. It should be tailored to satisfy the privacy needs of the Egy-DRiVeS dataset. Moreover, Roop and Ghost-v2 face-swapping models are evaluated. Provenly, Roop outperforms Ghost-v2 in various aspects, as will be discussed. Consequently, Roop is the face-swapping model to be used in the pipeline to strike the balance between pedestrian privacy via identity concealment and data usability via facial attribute preservation.

</details>


### [47] [Track2Map: Online Deformable SLAM with Motion-Aware Pose Optimization in Robotic Surgery](https://arxiv.org/abs/2607.08408)
*Tianyi Song,Sierra Bonilla,Xinwei Ju,Evangelos Mazomenos,Danail Stoyanov,Adam Schmidt,Omid Mohareri,Sophia Bano,Francisco Vasconcelos*

Main category: cs.CV

TL;DR: Track2Map提出了一种在线3D高斯泼溅管道，能够在没有或存在噪声相机轨迹先验的情况下，从手术视频中联合优化相机轨迹与3D可变形场景重建，本质上是一种SLAM方法。通过密集2D点轨道的锚定变形初始化以及利用轨道统计解耦相机运动与场景变形，实现了鲁棒的在线重建。


<details>
  <summary>Details</summary>
Motivation: 现有基于高斯泼溅的RAMIS中密集可变形3D解剖重建方法多为离线管道，且依赖准确的相机轨迹先验（如机器人运动学），当先验缺失或存在噪声时适用性受限。

Method: 提出Track2Map在线管道：(1) 联合优化相机轨迹与3D可变形场景表示；(2) 使用密集2D点轨道进行锚定变形初始化以稳定优化；(3) 利用轨道统计检测静态相机周期并减少增量建图时的漂移，从而解耦相机运动与场景变形。

Result: 在StereoMIS数据集上，相比竞争SLAM方法以及使用相机轨迹先验的非SLAM方法，Track2Map在重建质量和相机轨迹估计方面均取得了更优性能。

Conclusion: Track2Map是首个在无先验或噪声先验条件下对手术视频进行在线可变形3D重建的SLAM方法，验证了联合优化相机轨迹与场景表示的可行性，并开源代码。

与遥感交叉或改进方案: 交叉/改进可行性：85%。该方法中基于密集2D点轨道进行变形初始化和统计解耦的策略可直接迁移至遥感视频的在线3D重建任务中，例如无人机航拍视频中的动态场景（如移动车辆、植被摆动）与相机运动解耦。此外，高斯泼溅的在线SLAM框架可用于遥感平台的实时定位与稠密建图，替代传统特征匹配+BA的方案，提升在纹理弱或动态场景下的鲁棒性。

Abstract (英文): Gaussian splatting is the current state-of-the-art for dense, deformable 3D anatomy reconstruction in robot-assisted minimally invasive surgery (RAMIS); however, most pipelines are offline and depend on accurate camera trajectory priors (often from robotic kinematics), limiting applicability when priors are missing or noisy. To address these limitations, we propose Track2Map, an online 3D Gaussian Splatting pipeline that jointly optimizes camera trajectory and 3D deformable scene representation directly from surgical video. Track2Map is therefore capable of robust 3D reconstructions when camera trajectory priors are either absent or noisy, and due to its online nature it effectively works as a Simultaneous Localisation and Mapping (SLAM) method. To stabilize optimization in the presence of tissue motion and ambiguous visual cues, we introduce a track-anchored deformation initialization using dense 2D point tracks. Track statistics are further utilized to disentangle camera motion from scene deformation by detecting static camera periods and reducing drift during incremental mapping. Experiments on StereoMIS show improved reconstruction quality and camera trajectory against competing SLAM methods, as well as compared to non-SLAM methods that utilize camera trajectory priors. The code is available at https://track2map.github.io/.

</details>


### [48] [DeltaV: Thinking with Visual State Updates in Unified Large Multimodal Models](https://arxiv.org/abs/2607.08434)
*Pengjie Wang,Linger Deng,Zujia Zhang,Shaojie Zhang,Zhenbo Luo,Pei Fu,Jian Luan,Xiang Bai,Yuliang Liu*

Main category: cs.CV

TL;DR: DeltaV提出了一种视觉更新范式，用紧凑的更新令牌替代全图生成，结合TSIM路由器动态分配令牌预算，并构建了包含105万样本的StructCoT数据集，在减少55.6%视觉令牌的同时提升多模态推理性能。


<details>
  <summary>Details</summary>
Motivation: 现有统一大型多模态模型在交错推理中每次生成完整图像，导致视觉令牌冗余且稀释了对关键状态转换的监督，推理效率低下。

Method: 提出DeltaV模型，基于历史视觉状态增量预测紧凑的更新令牌来捕捉视觉变化；引入TSIM路由器根据时间相似性动态停止令牌分配；构建StructCoT数据集包含105万样本覆盖44个任务领域。

Result: 视觉更新范式平均减少55.6%的新生成视觉令牌且不损害重建保真度；多模态推理提升3.3%；DeltaV-2B在域内评价中超越大模型8.4%，在外部基准上超越Qwen3-VL-2B达5.9%。

Conclusion: 视觉更新范式有效解决了全图生成中的令牌冗余问题，在保持推理质量的同时显著提升效率，验证了增量式视觉状态更新的有效性。

与遥感交叉或改进方案: 交叉/改进可行性：85%。遥感领域多时相影像变化检测任务与DeltaV的增量更新范式高度契合：1) 可将时序遥感影像序列中的变化建模为视觉更新令牌，减少冗余背景计算；2) TSIM路由器可自适应地分配令牌预算给变化显著的区域（如新建建筑、植被变化），提升检测效率；3) StructCoT的跨任务推理思路可迁移至遥感语义推理链（如从变化区域推理灾害评估）；4) 扩展至视频遥感场景下的连续变化信息压缩与推理。

Abstract (英文): Current Unified Large Multimodal Models (ULMMs) support interleaved multimodal reasoning through textual reasoning and intermediate visual states, but typically generate each visual state as a full image. This full-image generation paradigm introduces substantial visual-token redundancy and dilutes supervision on sparse yet reasoning-critical state transitions. We propose DeltaV, a ULMM that replaces full-image generation with visual updates. Conditioned on historical visual states, DeltaV incrementally predicts compact update tokens that capture the visual changes across reasoning steps, avoiding repeated modeling of unchanged content. To align the token budget of each update with the magnitude of visual change, DeltaV introduces a temporal similarity (TSIM) Router, which stops allocating tokens once the marginal reconstruction gain falls below a threshold. To support more diverse and generalizable reasoning, we further construct StructCoT, a large-scale interleaved multimodal reasoning dataset with 1.05M samples spanning 44 task domains. Experiments show that the visual-update paradigm reduces newly generated visual tokens by 55.6\% on average without compromising reconstruction fidelity, and improves multimodal reasoning by 3.3\% over full-image generation. Trained with StructCoT and large-scale multimodal data, DeltaV-2B further outperforms substantially larger open-source models by 8.4\% on in-domain multimodal reasoning evaluations and surpasses the comparable-scale Qwen3-VL-2B by 5.9\% on external multimodal reasoning and understanding benchmarks. Code, models, and StructCoT will be released at https://github.com/Pengjie-W/DeltaV.

</details>


### [49] [Predicting Viticulture Potential through an Ensemble of U-Net and a Geospatial Foundation Model](https://arxiv.org/abs/2607.08449)
*Jorge Ignacio Perez,Hwaai Kang Kee,Lucas Rassbach*

Main category: cs.CV

TL;DR: 论文介绍了DS@GT ARC团队在ImageCLEF AI4Agri 2026子任务1中的参赛方案，使用U-Net与地理空间基础模型Prithvi-2.0的集成方法预测法国南部的葡萄栽培潜力，在排行榜上以68.32的±1准确率排名第二（共7支队伍）。


<details>
  <summary>Details</summary>
Motivation: 传统农业潜力评估方法（调查、实地测量、土壤测试等）成本高昂，亟需利用遥感数据实现低成本、大范围的农业潜力预测，特别是针对法国南部葡萄栽培潜力的精准评估。

Method: 采用集成学习策略，结合U-Net（经典的图像分割网络）与地理空间基础模型Prithvi-2.0（专为遥感/地理空间数据预训练的基础模型），构建混合模型框架进行葡萄栽培潜力预测。

Result: 在ImageCLEF AI4Agri 2026子任务1的排行榜上，最佳模型达到68.32的±1准确率，在7支参赛队伍中排名第2。

Conclusion: U-Net与地理空间基础模型的集成方法在农业潜力预测任务中表现出竞争力，验证了遥感数据与基础模型在农业应用中的有效性，且代码已开源。

与遥感交叉或改进方案: 交叉/改进可行性：95%。该论文本身就是遥感在农业中的典型应用。改进方案包括：(1) 可引入多尺度特征融合机制（如ASPP或特征金字塔）增强U-Net对葡萄园复杂空间格局的捕捉能力；(2) 尝试其他地理空间基础模型（如SpectralGPT、SatMAE）进行对比集成；(3) 引入时序遥感数据（Sentinel-2时间序列）捕捉葡萄生长季物候特征，提升预测精度；(4) 结合数字高程模型(DEM)和土壤质地数据等多模态输入，增强模型对微气候和地形因素的建模能力。

Abstract (英文): Determining agricultural potential is fundamental to sustainable land management and agricultural planning. Remote sensing data is increasingly valuable as an avenue for agricultural potential due to the cost of traditional methods (surveys, in-situ measurements, soil testing, etc). ImageCLEF AI4Agri 2026: Subtask 1 is concerned with the prediction of viticulture potential in Southern France. The DS@GT ARC's submission for Subtask 1 introduces an ensemble of U-Net and a Geospatial Foundation Model (Prithvi-2.0). Our best model achieved a $\pm$1 accuracy of 68.32 on the leaderboard, ranking 2nd among 7 teams. The implementation for this work is publicly available at https://github.com/dsgt-arc/imageclef-ai4agri-2026 .

</details>


### [50] [VEGAS: Human-Aligned Video Caption Evaluation via Gaze](https://arxiv.org/abs/2607.08489)
*Shenghui Chen,Po-han Li,Ximeng Sun,Shijia Yang,Emad Barsoum,Zicheng Liu,Sandeep Chinchali,Ufuk Topcu*

Main category: cs.CV

TL;DR: VEGAS是一种无需训练的跨模态信息论指标，通过测试时的眼动数据来采样个性化、与注意力对齐的视频描述文本，实验表明其能显著提升描述与人类关注点的对齐度及下游检索性能。


<details>
  <summary>Details</summary>
Motivation: 现有视频描述模型生成的文本无法捕捉不同观看者的个性化注意力，缺乏根据观看者注视焦点定制描述的能力。

Method: 提出VEGAS指标，基于信息论度量候选描述与观看者注视焦点的匹配程度；通过拒绝采样（无需模型重训）从生成结果中选择与眼动数据对齐的最佳描述；并构建含同步眼动数据的第一人称活动与教学幻灯片数据集。

Result: VEGAS选择的描述与人类关注点显著对齐，并能提升下游描述到视频的检索性能。

Conclusion: 在推理阶段融入观看者注意力能够有效提升视频描述的质量与实用性。

与遥感交叉或改进方案: 交叉/改进可行性：70%。可将VEGAS引入遥感视频描述任务，利用遥感解译专家的眼动数据（如关注特定地物目标），生成面向专家注意力的个性化遥感视频字幕；也可用于遥感图像-文本检索中，以眼动数据作为弱监督信号优化跨模态匹配。

Abstract (英文): Vision-language models excel at video captioning, yet typically generate descriptions that fail to capture individual viewers' attention. We propose VEGAS (Video caption Evaluation via GAze Score), a training-free metric that leverages test-time gaze to sample personalized, attention-aligned text. It is a cross-modal, information-theoretic metric that quantifies how well a candidate caption matches a viewer's focus. To evaluate VEGAS, we curate a dataset of egocentric activities and instructional slides paired with synchronized gaze and reference annotations. We then select captions based on VEGAS via rejection sampling without model retraining. Experiments show that VEGAS-selected captions align significantly better with human focus and improve downstream caption-to-video retrieval, demonstrating the practical utility of incorporating viewer attention during inference.

</details>


### [51] [Cognitive-structured Multimodal Agent for Multimodal Understanding, Generation, and Editing](https://arxiv.org/abs/2607.08497)
*Feng Wang,Canmiao Fu,Zhipeng Huang,Chen Li,Jing Lyu,Ge Li*

Main category: cs.CV

TL;DR: 提出了一种具有认知结构的 multimodal agent (CMA)，通过情景视觉记忆和外化视觉信息，解决长程多模态对话中视觉token爆炸和跨轮引用不可靠问题。8B模型在20轮对话中检索准确率91.4%，超越32B基线8.2%，推理时间减半。


<details>
  <summary>Details</summary>
Motivation: 现有统一多模态模型将所有历史视觉和文本输入重复喂入共享上下文窗口，导致视觉token爆炸和跨轮引用不可靠，难以支持长程多模态对话。

Method: 提出认知结构多模态智能体CMA，包含三个核心模块：1) Perceptual Abstraction Engine（感知抽象引擎）进行结构化视觉抽象；2) Cognitive Retrieval Engine（认知检索引擎）实现跨轮记忆检索；3) Multimodal Executive Controller（多模态执行控制器）进行自主任务推断和动作规划。同时引入Unified Scenario Engine生成带细粒度检索标注的结构化多轮对话数据，并构建分层难度的长程视觉对话基准。

Result: 8B模型在20轮对话中检索准确率91.4%，超越32B基线8.2%，每轮推理时间从23.1s降至12.7s。同时发布CMA-Harness工具增强部署平台。

Conclusion: 结构化记忆和模块化决策相比 monolithic 参数缩放为长程多模态智能体提供了更可扩展、高效的范式。

与遥感交叉或改进方案: 交叉/改进可行性：85%。遥感领域的长时序影像分析（如多时相变化检测、时序遥感语义分割）可引入该文的情景视觉记忆机制：将每期遥感影像通过感知抽象引擎编码为结构化特征存入记忆，认知检索引擎根据当前查询自动召回相关历史时相影像特征，实现跨时序的上下文感知推理，缓解遥感大模型中长时序token堆积问题。

Abstract (英文): Recent unified multimodal models show a single architecture can jointly perform vision/language understanding and image generation/editing. However, they repeatedly feed all historical visual and textual inputs into a shared context window, limiting long-horizon multimodal dialogue due to visual token explosion and unreliable cross-turn referencing. We propose a Cognitive-structured Multimodal Agent that externalizes visual information into an Episodic Visual Memory and selectively reactivates relevant episodes during reasoning. The agent consists of a Perceptual Abstraction Engine for structured visual abstraction, a Cognitive Retrieval Engine for cross-turn memory retrieval, and a Multimodal Executive Controller for autonomous task inference and action planning. To address the lack of turn-level retrieval supervision in existing datasets, we develop a Unified Scenario Engine that programmatically generates structured multi-turn conversations with fine-grained retrieval annotations, enabling reinforcement learning to optimize abstraction and retrieval policies. We also construct a long-horizon visual-dialogue benchmark stratified by difficulty to evaluate episodic visual recall. Our 8B agent achieves 91.4% retrieval accuracy over 20-turn sessions, surpassing 32B baselines by +8.2% while nearly halving per-turn inference time (23.1s -> 12.7s). We further present the Cognitive-structured Multimodal Agent Harness (CMA-Harness), a tool-augmented deployment of the same cognitive structure integrating persistent multimodal memory, web access, image generation/editing/composition tools, and OpenAI-compatible serving. Structured memory and modular decision-making offer a more scalable, efficient paradigm for long-horizon multimodal agents than monolithic parameter scaling. Code: https://github.com/caseclose/cma-harness ; Project page: https://caseclose.github.io/cma-harness/

</details>


### [52] [CT-CLIP Representations for Multimodal Lung Cancer Survival Prediction](https://arxiv.org/abs/2607.08503)
*Sofie Allgöwer,Mikael Johansson,Andreas Hallqvist,Jonas Andersson,Åse Johnsson,Ida Häggström,Jennifer Alvén*

Main category: cs.CV

TL;DR: 本文评估了领域特定基础模型CT-CLIP在数据受限的肺癌预后预测场景中的应用。基于242名肺癌患者的预治疗CT影像和临床变量，通过冻结编码器、全微调、低秩适应等策略，发现冻结CT-CLIP结合轻量生存预测头在性能上优于临床基线，与多模态方法相当甚至更优，并能有效区分高低风险患者。


<details>
  <summary>Details</summary>
Motivation: 深度学习的生存预测模型常受限于缺乏带有可靠结局数据的精心筛选影像队列。本研究旨在探索是否可以利用领域特定的基础模型（CT-CLIP）在数据受限的临床环境中进行多模态生存预测，以解决训练数据稀缺的问题。

Method: 使用CT-CLIP作为特征提取器，处理242名肺癌患者的预治疗CT影像和临床变量。评估了三种适应策略：冻结编码器（仅训练轻量生存预测头）、全微调、低秩适应（LoRA），并进行模态消融实验以及与临床基线和多模态基线的对比。

Result: 冻结的CT-CLIP模型结合可训练的轻量生存预测头，性能优于纯临床基线，并与其他多模态方法表现相当或更优，且能有效将患者区分为临床上有意义的高风险组和低风险组。

Conclusion: 领域特定的基础模型CT-CLIP在数据受限场景下可用于多模态生存预测，冻结编码器+轻量头的策略最为有效，为临床预后预测提供了一种数据高效的方法。

与遥感交叉或改进方案: 交叉/改进可行性：85%。遥感领域同样面临标注数据稀缺问题，该思路可直接迁移：1）利用遥感领域的基础模型（如SatMAE、SSL4EO-S12等）作为特征提取器，冻结编码器后结合轻量预测头进行灾害评估、作物产量预测等任务；2）引入多模态遥感数据（SAR+光学+时序特征）与地理/气象变量，构建类似CT-CLIP的遥感多模态基础模型；3）LoRA等参数高效微调策略可应用于遥感大模型（如遥感ViT、GeoFM）的领域适配，降低计算资源需求。

Abstract (英文): Accurate prognosis prediction is important for treatment planning in lung cancer, but deep learning-driven survival modelling is often limited by the scarcity of curated imaging cohorts with reliable outcome data. This study evaluates whether representations from a domain-specific foundation model can be used for multimodal survival prediction in data-constrained clinical settings. We assess the foundation model CT-CLIP as a feature extractor for pretreatment computed tomography images and clinical variables from 242 diagnosed lung cancer patients. The evaluation includes adaptation strategies based on frozen encoders, full fine-tuning, and low-rank adaptation, together with modality ablations and comparisons with clinical and multimodal baselines. The results show that a frozen CT-CLIP model combined with a trainable lightweight survival head outperforms the clinical baseline and achieves comparable or improved performance relative to other multimodal approaches, and separates patients into clinically meaningful high- and low-risk groups.

</details>


### [53] [Do Egocentric Video-Language Models Capture Both Hand- and Object-Centric Cues?](https://arxiv.org/abs/2607.08514)
*Masatoshi Tateno,Alexandros Stergiou,Risa Shinoda,Yoichi Sato,Dima Damen*

Main category: cs.CV

TL;DR: 本文提出一种新的手物交互（HOI）识别学习范式，结合手物掩码训练和HOI动态感知解码器，以克服视频-语言模型依赖虚假相关性的问题。同时引入CI-HOI评估基准和DEHOI测试平台，用于独立评估手部线索和物体线索。实验表明该方法在多个任务上优于现有模型。


<details>
  <summary>Details</summary>
Motivation: 现有视频-语言模型在手物交互识别中常利用手、物体或环境上下文之间的虚假相关性（shortcuts），而非从手和物体本身的外观与动态进行推理，导致鲁棒性不足。

Method: (i) 手物掩码训练（hand-object masked training）：从部分手或物体观测中进行鲁棒推理；(ii) HOI动态感知解码器（HOI-dynamics-aware decoder）：通过辅助预测手和物体的位置与语义，显式学习以手和物体为中心的嵌入表示，增强对两种线索的敏感性。

Result: 在DEHOI测试平台、标准动作识别、物体状态识别以及机器人操作动作识别等多个任务上，该方法均优于现有模型，实现了更鲁棒的HOI理解。

Conclusion: 所提出的训练策略比现有模型更有效地利用手和物体中心信息，为HOI识别提供了一种更具鲁棒性的解决方案。

与遥感交叉或改进方案: 交叉/改进可行性：75%。遥感图像中常涉及人与目标（如车辆、建筑物、设备）的交互识别，可借鉴本文手物掩码训练思路，在遥感场景中构造人与目标的局部掩码训练，增强模型从部分观测（如遮挡目标）中推理交互的能力。同时，HOI动态感知解码器可迁移为"目标-环境"动态感知解码器，通过辅助预测目标位置与语义变化，提升遥感交互识别对空间-语义线索的敏感性。DEHOI的独立线索评估思路也可用于遥感，构建分离目标和背景的评估基准，诊断模型依赖的线索类型。

Abstract (英文): Hand-object interaction (HOI) recognition requires capturing both hand manipulations and object transformations. However, existing video-language models often fall into shortcuts by relying on spurious correlations among hands, objects, or environmental context, rather than reasoning from the appearance and dynamics of hands and objects themselves. To address this limitation, we propose a new learning paradigm that combines (i) hand-object masked training, which enables robust reasoning from partial hand or object observations, and (ii) an HOI-dynamics-aware decoder that explicitly learns hand- and object-centric embeddings through auxiliary predictions of their locations and semantics, enhancing sensitivity to both cues. To systematically evaluate such cue-specific reasoning, we introduce Cue-Isolated HOI (CI-HOI), a new evaluation that assesses models' ability to predict actions from hand- and object-related cues independently. To enable CI-HOI, we curate the DEHOI testbed, which separates hand- and object-related observations for disentangled HOI evaluation through inpainting. Using DEHOI, we demonstrate both quantitatively and qualitatively that our training strategy exploits hand- and object-centric information more effectively than existing models. Our approach improves over existing models on DEHOI, standard action recognition, object state recognition, and even robot manipulation action recognition, leading to more robust HOI understanding.

</details>


### [54] [Beyond wheelchairs and blindfolds: Investigating disability stereotypes in T2I models with INCLUDE-BENCH](https://arxiv.org/abs/2607.08515)
*Sophia Lichtenberg,Albert Gatt,Judith Masthoff*

Main category: cs.CV

TL;DR: 本文提出INCLUDE-BENCH，首个大规模评估文本到图像(T2I)模型中残障相关偏见的基准。通过生成119K图像，评估15个开源和2个闭源模型，发现轮椅描绘占主导、残障条件下生成多样性更低、刻板印象更强等关键问题，并引入刻板印象内容模型(SCM)评分量化偏见。


<details>
  <summary>Details</summary>
Motivation: 现有T2I模型偏见评估主要关注性别、肤色和文化表征，残障维度被系统性忽视；且评估实践缺乏与社会学刻板印象定义的对齐，无法原则性地评估对残障人士的表征伤害。

Method: 构建包含119K生成图像的INCLUDE-BENCH基准，覆盖多维度偏见提示设计（静态和动态上下文）；引入SCM评分量化模型刻板印象关联；系统评估15个开源+2个闭源T2I模型。

Result: (1) 行动障碍和默认残障提示在所有模型中主要表现为轮椅描绘；(2) 残障条件生成图像多样性显著更低；(3) 刻板印象描绘与残障文本对齐更强；(4) SCM评分显示T2I模型反映了真实世界刻板印象关联。

Conclusion: T2I模型存在系统性的残障偏见，表现为表征单一化、多样性缺失及强化现实刻板印象，INCLUDE-BENCH为量化评估这类偏见提供了标准化工具。

与遥感交叉或改进方案: 交叉/改进可行性：65%。遥感领域可借鉴其偏见评估框架：(1) 构建遥感场景分类或目标检测的公平性基准（如不同地理区域/经济水平地区的视觉表征偏差）；(2) 引入类似SCM评分评估遥感模型对特定地物或区域的社会刻板印象关联；(3) 分析遥感生成模型（如卫星图像超分、图像描述生成）在不同条件（城市/乡村、发达/发展中地区）下的表征多样性和对齐偏差。

Abstract (英文): Text-to-image (T2I) models have been shown to exhibit social biases. Prior work has mainly focused on gender, skin tone, and cultural representation within restricted occupational associations, and emerging benchmarks increasingly incorporate these dimensions. However, disability remains systematically underexplored. Current evaluation practices often fail to align with sociologically grounded definitions of stereotyping, limiting principled assessment of representational harms toward people with disabilities (PWD). To address this, we introduce INCLUDE-BENCH, the first large-scale benchmark for evaluating disability-related bias in T2I models. INCLUDE-BENCH comprises 119K generated images based on prompt design across multiple bias dimensions and both static and dynamic contexts. We evaluate 15 open-source and 2 closed-source models. Our key findings reveal that: (1) mobility-impaired and default disability prompts predominantly yield wheelchair depictions across all models; (2) disability-conditioned generations consistently exhibit less diversity; (3) stereotypical portrayals demonstrate stronger disability-text alignment; and (4) we introduce the Stereotype Content Model (SCM) Score, demonstrating that T2I models reflect real-world stereotypical associations.

</details>


### [55] [Whareformer: Learning to Track What is Where in Long Egocentric Videos](https://arxiv.org/abs/2607.08537)
*Jacob Chalk,Saptarshi Sinha,Dima Damen,Yannis Kalantidis,Diane Larlus*

Main category: cs.CV

TL;DR: Whareformer是基于Transformer的首个学习型解决方案，用于解决OSNOM任务（跟踪被摄像头佩戴者移动的物体，即使在物体离开视野或被严重遮挡时也能持续跟踪）。该模型包含可更新的轨迹记忆和轨迹分配模块，联合推理物体外观和3D位置，在仅56个视频上训练，但在三个数据集的260个长测试视频上取得了SOTA性能。


<details>
  <summary>Details</summary>
Motivation: OSNOM任务关注第一人称视频中由佩戴者移动的物体跟踪，需要在线维持物体位置信息，即使在物体离开视野或被严重遮挡时也要保持跟踪。此前缺乏基于学习的解决方案。

Method: 提出Whareformer，一种基于Transformer的模型，包含两个组件：(1) 可更新的已建立轨迹记忆；(2) 前馈式轨迹分配模块，将观测与现有轨迹关联。模型联合推理物体外观（what）和更新的3D位置（where），并采用专用的New Track token来推理新物体。设计上使用相对距离和可演化的轨迹表示。

Result: 仅在56个视频上训练，在EPIC-KITCHENS-100（未见视频）、IT3DEgo和HD-EPIC三个数据集的260个长测试视频上达到SOTA性能，显著优于先前方法。

Conclusion: Whareformer证明了首个基于学习的OSNOM解决方案的有效性，其设计选择（相对距离、可演化轨迹表示）使得小样本训练也能在大规模测试中取得优异性能。

与遥感交叉或改进方案: 交叉/改进可行性：85%。遥感领域中的目标跟踪（如卫星视频中的运动车辆跟踪、无人机视频中的地物目标跟踪）常面临目标被遮挡、移出视野等问题，与OSNOM任务高度相似。具体交叉方案：(1) 将Whareformer的3D位置推理机制迁移到遥感视频跟踪中，结合地理空间坐标替代第一人称的3D坐标，实现目标离视野后的重识别；(2) 将New Track token机制用于遥感场景中新增目标的自动检测与跟踪初始化；(3) 可演化的轨迹表示适用于长时序遥感视频中目标外观因尺度、光照变化而演变的情况。

Abstract (英文): The recently established 'Out of Sight, Not out of Mind' (OSNOM) task for egocentric videos focuses on tracking objects that are moved by the camera wearer, online, maintaining knowledge of instance locations throughout the video even when they leave the field of view or become heavily occluded. In this paper, we propose the first learning-based solution to the OSNOM task: Whareformer, a transformer-based model with two components: an updatable memory of established tracks and a track assignment module that associates observations with existing tracks in a feed-forward manner. Whareformer jointly reasons over evolving object appearance (what) and updated 3D location (where), and employs a dedicated New Track token to reason about novel objects.
  Thanks to its design choices of using relative distances and evolving track representations, Whareformer is trained on a small set of 56 videos but achieves SOTA performance on 260 long test videos from three datasets: EPIC-KITCHENS-100 (unseen videos), IT3DEgo, and HD-EPIC, with significant absolute improvements over prior work.

</details>


### [56] [VocaDet: Sample-Driven Open-Vocabulary Object Detection and Segmentation via Visual Tokenization and Vector Database Retrieval](https://arxiv.org/abs/2607.08541)
*ZhiXin Sun*

Main category: cs.CV

TL;DR: VocaDet 是一个基于样本驱动的开集目标检测与分割框架，通过将视觉特征转换为离散视觉词汇并存储在向量数据库中，无需模型重训练即可从用户提供的正负样本中学习任意物体概念，在 UA-DETRAC 数据集上取得了有效的开集检测性能。


<details>
  <summary>Details</summary>
Motivation: 现有开集检测方法依赖文本提示、有限的视觉示例或昂贵的特征匹配流程，难以扩展到大规模且持续增长的物体库。VocaDet 旨在解决这些限制，实现无需重训练、可持续扩展的样本驱动开集检测。

Method: 使用 DINOv3 提取视觉特征，通过凝聚聚类（自适应聚类灵敏度）生成多粒度视觉标记；视觉标记与位置去偏表示、空间拓扑信息一起作为可扩展物体记忆存储在向量数据库中；推理时将查询图像转换为视觉标记，通过高效检索进行匹配；引入背景过滤机制去除频繁出现的背景模式。

Result: 在 UA-DETRAC 数据集上的实验表明，VocaDet 无需传统检测器训练即可实现有效的开集检测性能，且随着正负样本的积累，识别能力可以持续扩展。

Conclusion: VocaDet 证明了将视觉特征离散化为视觉词汇并利用向量数据库进行检索式开集检测的可行性，为无需重训练、可持续扩展的开集目标检测与分割提供了新的范式。

与遥感交叉或改进方案: 交叉/改进可行性：85%。遥感场景中目标种类多样且常出现新类别（如新型车辆、船只、临时建筑），VocaDet 的样本驱动方式非常适合。具体方案：(1) 利用遥感影像中频繁出现的背景（如水体、植被）作为负样本，有效过滤误检；(2) 将遥感目标（不同型号飞机、车辆）的正样本收集后直接存入向量数据库，实现快速扩展；(3) 可结合遥感影像的 geospatial 位置信息增强空间拓扑编码，提升检测精度。

Abstract (英文): Open-vocabulary object detection and segmentation aim to recognize arbitrary objects beyond predefined categories. Although recent vision-language and reference-based approaches have significantly advanced this field, they often rely on text prompts, limited visual examples, or expensive feature matching procedures, making them difficult to scale to large and continuously expanding object repositories. In this work, we propose VocaDet, a sample-driven open-vocabulary object detection and segmentation framework that learns object concepts directly from user-provided positive and negative sample collections without model retraining. The key idea is to transform continuous visual representations into discrete visual vocabularies and perform efficient retrieval-based recognition through a scalable vector database. Specifically, we employ DINOv3 as the visual feature extractor and apply agglomerative clustering with adaptive clustering sensitivity to generate multi-granularity visual tokens. These visual tokens, together with position-debiased representations and spatial topology information, are stored as expandable object memories in a vector database. During inference, query images are converted into visual tokens and efficiently matched against the stored object memories for object localization and segmentation. Furthermore, a background filtering mechanism is introduced to remove frequently occurring background patterns and reduce redundant retrieval operations in practical fixed-camera scenarios. Experiments on the UA-DETRAC dataset demonstrate that VocaDet achieves effective open-vocabulary detection performance without conventional detector training, while supporting continuously expandable recognition capability as additional positive and negative samples are accumulated.

</details>


### [57] [Switch-Reasoner: Learn When to Think in Multitask Mixtures via Reinforcement Learning](https://arxiv.org/abs/2607.08572)
*Yiyang Fang,Pei Fu,Jinjie Li,Jian Liang,Wenke Huang,Ruijie Luo,Shaojie Zhang,Jian Luan,Yi R. Fung,Mang Ye*

Main category: cs.CV

TL;DR: Switch-Reasoner是一种基于GRPO的框架，让多模态大模型能自适应选择推理模式（直接回答或先推理后回答），通过双层级联调节机制平衡两种模式的使用，在11个多模态任务上实现了更好的精度-效率权衡。


<details>
  <summary>Details</summary>
Motivation: 现有MLLMs遵循固定的"先思考后回答"范式，在异构多任务场景下效率低——简单输入不需要显式推理，而困难输入则能从推理中获益。此外，后训练中何时思考的学习不稳定，不平衡的rollout会导致模型趋向于"总是思考"或"总是直接回答"的极端行为。

Method: 1) 将思考视为虚拟工具调用，模型可选择直接回答或先调用推理再回答；2) 提出GRPO（分组相对策略优化）框架来学习自适应选择推理模式；3) 引入双层级联调节机制：宏观层面平衡两种模式的整体使用比例，微观层面基于两种选择的相对收益提供样本级监督。

Result: 在11个多模态任务上，Switch-Reasoner在减少不必要推理的同时保持强性能，实现了更好的精度-效率权衡。

Conclusion: 自适应推理模式选择能够有效提升MLLMs在多任务场景下的效率，双层级联调节机制是稳定学习过程的可行方案。

与遥感交叉或改进方案: 交叉/改进可行性：90%。遥感领域存在大量异构任务（场景分类、目标检测、变化检测、语义分割等），不同任务对推理需求差异大。具体方案：1) 将Switch-Reasoner应用于遥感多任务MLLM，让简单场景分类直接输出，复杂变化检测则启用地理空间推理；2) 利用GRPO框架处理遥感数据的不平衡性（如罕见灾害场景需推理，常规地貌直接回答）；3) 双层级联调节机制可适应遥感任务间复杂度差异大的特点，避免模型在所有输入上都启用昂贵的空间推理。

Abstract (英文): Multimodal Large Language Models (MLLMs) often follow a fixed Think-then-Answer paradigm, which is inefficient in heterogeneous multitask settings because simple inputs may not require explicit reasoning while difficult ones can benefit substantially from it. Learning when to think is also unstable during post-training, where imbalanced rollouts can drive the model toward always-thinking or always-direct behavior. We propose Switch-Reasoner, a GRPO-based framework that learns to adaptively select reasoning modes for MLLMs. It treats thinking as a virtual tool invocation and allows the model to either answer directly or invoke explicit reasoning before answering. To stabilize this decision, we introduce a dual-level regulation mechanism that balances the overall use of Thinking Mode and Direct Mode while providing sample-level supervision based on the relative benefit of the two choices. Experiments on 11 multimodal tasks show that Switch-Reasoner reduces unnecessary reasoning while maintaining strong performance, achieving a better accuracy-efficiency trade-off.

</details>


### [58] [When Structured Sparse Autoencoders Learn Consistent Concepts Across Modalities](https://arxiv.org/abs/2607.08605)
*Weiduo Liao,Yunqiao Yang,Ying Wei*

Main category: cs.CV

TL;DR: 针对视觉-语言模型中稀疏自编码器难以学习模态一致概念的问题，提出结构化稀疏自编码器(S²AE)，通过在视觉模态中引入语义和空间一致性约束（分组排他稀疏与组内稀疏正则化），在保持高重构保真度（解释方差>99%）的同时，显著提升概念语义对齐（mIoU提升6.06%）和表征效率（l0范数降低60.81），并在跨模态分析中增强了神经元的单语义性。


<details>
  <summary>Details</summary>
Motivation: 视觉-语言模型中，传统稀疏自编码器（SAE）学习的视觉概念存在碎片化覆盖（即不连续区域），难以实现模态一致的概念表征，限制了模型的可解释性。

Method: 提出结构化稀疏自编码器(S²AE)：基于Transformer注意力相似性和空间邻近性对图像块进行分组，在训练SAE时引入结构化稀疏正则化，包括组间排他稀疏（促进不同组概念的分离）和组内稀疏（促进组内概念一致性），使潜在神经元专注于语义上连贯的不同概念。

Result: 在Qwen2.5-VL-7B-Instruct模型上，语义对齐（mIoU）平均提升6.06%，表征效率（l0范数）降低60.81，重构解释方差保持在99%以上；跨模态分析中语义一致性平均提升3.08%，单语义性评分平均提升2.37%。

Conclusion: S²AE通过视觉结构先验有效增强了多模态特征的语义一致性和神经元单语义性，证明了在视觉模态中引入结构化约束对于改善多模态可解释性的有效性。

与遥感交叉或改进方案: 交叉/改进可行性：90%。遥感图像具有高度结构化的空间语义（如地物边界明确、空间关系显著），S²AE的分组策略（注意力相似性+空间邻近性）天然适配遥感场景。具体方案：1) 利用遥感图像中地物分割先验（如道路、建筑、植被区域）替代通用的注意力分组，增强语义分组准确性；2) 结合遥感多模态数据（如SAR+光学图像），将S²AE扩展为跨模态结构化稀疏编码，提升多模态遥感表征的可解释性；3) 应用于遥感视觉-语言模型（如遥感图像描述、检索）的概念解耦分析，辅助模型决策的可解释性验证。

Abstract (英文): Sparse autoencoders (SAEs) have emerged as a promising technique for mechanistic interpretability by learning a set of sparse latent features in large models, each of which encodes a distinct concept. However, in vision-language models (VLMs), vanilla SAEs struggle to learn modality-consistent concepts, with concepts often exhibiting fragmented coverage (i.e., disjoint regions) in the visual modality. To address this challenge, we propose a Structured Sparse AutoEncoder ($S^2AE$) that enforces concept consistency from both semantic and spatial perspectives in the visual modality. Specifically, we group image patches based on Transformer attention similarity and spatial proximity, and introduce a structured sparsity regularization when training the vanilla SAE. The regularization consists of exclusive sparsity for inter-group concept disentanglement and group sparsity for intra-group concept consistency, which drives the latent neurons by SAEs to specialize in distinct, semantically grounded concepts. Evaluated on the \texttt{Qwen2.5-VL-7B-Instruct} model, the method achieves 6.06% average improvement in semantic alignment (mIoU) and 60.81 in representational efficiency (lower l0 norm) while maintaining near-perfect reconstruction fidelity with an Explained Variance above 99%. Cross-modal analysis further demonstrates that $S^2AE$ enhances neuronal monosemanticity by this visual structural prior, achieving a 3.08% average gain in semantic consistency and a 2.37% average gain in monosemanticity scores for both modalities of multimodal features, thereby fostering more coherent and disentangled representations.

</details>


### [59] [Do Transformations Reveal the Truth? Generative Residual Learning for Generalized AI-Generated Image Detection](https://arxiv.org/abs/2607.08674)
*Kutub Uddin,Nusrat Tasnim,Awais Khan,Mohammad Umar Farooq,Khalid Malik*

Main category: cs.CV

TL;DR: 提出GenRes和GenRes++框架，利用神经张量网络进行生成残差学习，并通过可学习注意力机制聚合多变换样本的关系特征，用于检测AI生成图像，在多个基准数据集上超越现有方法。


<details>
  <summary>Details</summary>
Motivation: 生成式AI制造的深度伪造媒体带来虚假信息、身份盗窃、欺诈和舆论操纵等严重威胁，而AI生成图像检测因生成方法的多样性和微妙的伪影而极具挑战性，需要更具泛化能力的检测方法。

Method: 提出GenRes框架，通过神经张量网络对原始样本与变换样本之间的细粒度关系特征进行建模，实现生成残差学习；进一步提出GenRes++，引入可学习注意力机制聚合多个变换样本的关系特征，聚焦最具信息量的线索。两者均使用PE-Core作为特征提取器，提供通用且语义丰富的嵌入以提升跨域性能。

Result: 在多个基准数据集上的综合实验表明，GenRes++方法在检测AI生成图像方面优于现有方法。

Conclusion: 所提出的生成残差学习框架有效提升了AI生成图像检测的泛化能力，特别是GenRes++通过注意力机制在多变换场景下表现更优。

与遥感交叉或改进方案: 交叉/改进可行性：85%。具体方案：遥感图像中AI生成或篡改检测可借鉴GenRes的生成残差学习思想，利用神经张量网络建模原始遥感影像与多模态变换影像（如不同波段组合、不同分辨率降采样）之间的细粒度关系特征；PE-Core特征提取器可用于提取遥感影像的语义嵌入，提升跨传感器、跨地域的泛化检测能力；GenRes++的注意力机制可针对遥感影像的多源变换（如几何校正、辐射校正、融合等）进行自适应特征聚合，增强对生成式遥感伪造（如GAN生成的地物目标）的检测鲁棒性。

Abstract (英文): The rapid advancement of generative AI has enabled the creation of highly realistic deepfake media, posing significant threats, including misinformation, digital identity theft, fraud, and manipulation of public opinion. AI-generated image (AIGI) detection is reliably challenging due to the diversity of generative methods and the subtle artifacts they leave behind. In this work, we propose GenRes, a novel framework for generative residual learning via a neural tensor network, which models fine-grained relational features between original and transformed samples to enhance generalization. To address scenarios involving multiple generative transformations, we introduce GenRes++, which employs a learnable attention mechanism to aggregate relational features across multiple transformed samples and enables the model to focus on the most informative cues. Both models leverage PE-Core as a feature extractor, providing generalized and semantically rich embeddings that improve cross-domain performance and enable the detection of AIGI generated by unseen methods. Comprehensive experiments on multiple benchmark datasets demonstrate that the proposed GenRes++ approach outperforms existing methods.

</details>


### [60] [Multi-Resolution Feature Stem for Diabetic Retinopathy lesion segmentation](https://arxiv.org/abs/2607.08679)
*Indranil Dutta,Taehee Jeong*

Main category: cs.CV

TL;DR: 本文发现糖尿病视网膜病变（DR）病灶分割中，提高输入分辨率会出乎意料地对大出血病灶造成性能下降，并提出了一种多分辨率特征茎（Multi-Resolution Feature Stem）架构，通过并行处理多尺度输入来平衡精细细节与全局上下文信息。


<details>
  <summary>Details</summary>
Motivation: DR病灶尺寸差异极大（从微小的微动脉瘤到大范围的出血和渗出），现有深度学习模型在处理这种尺度多样性时存在矛盾需求，且通常假设更高分辨率总是有益的，但缺乏对分辨率与不同病灶类型之间关系的系统性研究。

Method: 在U-Net、UNet++、Vision Transformers、DeepLabV3+等多种架构上，对512×512和1024×1024两种输入分辨率进行系统实验。提出Multi-Resolution Feature Stem，即在输入层级构建金字塔结构，集成到UNet++骨干网络中，实现多尺度并行处理。

Result: 发现提高输入分辨率对微动脉瘤有益，但会意外地降低大出血病灶的分割性能。所提出的多分辨率特征茎架构有效解决了这一分辨率权衡问题。

Conclusion: 高分辨率并非普遍有益，不同病灶类型对分辨率有相反需求；提出的多分辨率并行处理架构是一种参数高效且实用的解决方案，能够同时捕捉精细细节和全局上下文信息。

与遥感交叉或改进方案: 交叉/改进可行性：95%。遥感图像中同样存在地物尺度差异大的问题（如小目标车辆、船只与大尺度建筑物、农田），该论文的多分辨率特征茎可直接迁移至遥感分割任务。具体方案：(1) 将Multi-Resolution Feature Stem集成到遥感常用的语义分割框架（如DeepLabV3+、SegFormer）中，用于处理遥感图像中多尺度地物的共存问题；(2) 针对遥感图像更高分辨率（如遥感通常使用1024×1024以上），可扩展该特征茎为更多尺度分支（如256、512、1024、2048），以覆盖更广的地物尺度范围；(3) 可结合遥感特有的多光谱/高光谱通道，设计多分辨率+多光谱融合的输入茎。

Abstract (英文): Diabetic Retinopathy (DR) is a leading cause of preventable blindness worldwide, requiring automated lesion segmentation using deep learning models for early detection and monitoring. However, DR lesions vary dramatically in size from tiny microaneurysms to large hemorrhages and exudates. This variability creates conflicting demands on the model architecture and input resolution, posing a challenge for effective design. This work investigates the impact of input resolution on different lesion types. Through systematic experimentation with multiple architectures (U-Net, UNet++, Vision Transformers, DeepLabV3+) at $512 \times 512$ and $1024 \times 1024$ resolutions, we identify a critical, counter-intuitive phenomenon where increasing input resolution has opposing effects on different lesion types. We demonstrate that while higher resolution is essential for resolving fine-grained microaneurysms, it can unexpectedly degrade performance on larger hemorrhages. This finding challenges the common assumption that higher resolution is uniformly beneficial. To address this, we propose a novel Multi-Resolution Feature Stem, an input-level pyramid integrated with a UNet++ backbone. This architecture processes multiple scales in parallel, capturing fine-grained details without sacrificing contextual information. This work contributes crucial empirical evidence of this complex, resolution-dependent behavior and a practical, parameter-efficient architecture that successfully resolves this trade-off.

</details>


### [61] [SAM-MT: Real-Time Interactive Multi-Target Video Segmentation](https://arxiv.org/abs/2607.08688)
*Ruiqi Shen,Chang Liu,Henghui Ding*

Main category: cs.CV

TL;DR: SAM-MT基于SAM2提出了一种实时多目标视频分割框架，通过显式查询、解耦掩码注意力和稀疏记忆机制，将延迟与目标数量解耦，在10个目标下仍保持>36 FPS，性能与单目标基线相当。


<details>
  <summary>Details</summary>
Motivation: 现有VOS方法在多目标场景下需要为每个目标重复单目标处理流程，导致帧率下降且延迟随目标数量增加而无限增长，无法满足实时多目标分割的需求。

Method: 1) 将SAM2改造为交互式多目标框架；2) 使用显式查询表示不同目标，结合共享全局上下文表示；3) 采用解耦掩码注意力机制防止跨目标干扰；4) 引入稀疏记忆实现稳定的时序演化；5) 设计专门的遮挡处理和重叠预防策略。

Result: 成功将延迟与目标数量解耦，在10个目标场景下达到>36 FPS的实时速度，性能与单目标基线SAM2相当。

Conclusion: SAM-MT实现了与目标数量无关的恒定延迟，为实时多目标视频分割提供了高效解决方案。

与遥感交叉或改进方案: 交叉/改进可行性：85%。遥感领域常需同时跟踪多个动态地物目标（如车辆、船只、移动的人群），SAM-MT的实时多目标能力可直接迁移。改进方案：1) 将解耦掩码注意力适配遥感高分辨率多光谱数据；2) 利用稀疏记忆机制处理遥感视频中长时序目标遮挡问题；3) 结合遥感地理坐标信息增强显式查询的空间定位能力。

Abstract (英文): Modern Video Object Segmentation (VOS) involves tracking and segmenting user-specified targets. While recent approaches have achieved remarkable performance in single-target scenarios, extending them to multi-target settings typically involves replicating the single-target processing for each individual object, resulting in reduced frame rates (FPS) with unbounded latency as target count increases. Built upon Segment Anything 2 (SAM2), we propose SAM-MT, which addresses this by transforming the model into an interactive framework for real-time Multi-Target video segmentation. SAM-MT uses explicit queries to represent different individual targets, in parallel with a shared representation for global context. It employs decoupled masked attention to keep individual identities distinct from cross-target interference, and sparse memory for stable temporal evolution, along with specialized strategies for occlusion handling and overlap prevention. SAM-MT successfully decouples latency from the number of targets, achieving real-time speed on par with single-target baselines (>36 FPS for 10 targets) while maintaining SAM2's robust video segmentation performance.

</details>


### [62] [HumanForge: A Human-Centric Deepfake Video Benchmark with Multi-Agent Forgery Rationales](https://arxiv.org/abs/2607.08705)
*Wenbo Xu,Zhimin Chen,Xiaojie Liang,Hengrui Liu,Wei Lu*

Main category: cs.CV

TL;DR: HumanForge是一个大规模、多范式的人体中心视频伪造数据集，包含18K+高保真视频片段，并提出了Gen2Anno多智能体管线用于自动化标注。实验表明现有检测器和多模态大模型在该数据集上零样本泛化和细粒度推理能力显著不足。


<details>
  <summary>Details</summary>
Motivation: 现有视频伪造基准主要聚焦于人脸交换或全局文本到视频合成，忽略了人体-物体/人体-人体交互以及多模态对齐等关键维度，缺乏统一的大规模人体中心视频伪造数据集。

Method: 提出Gen2Anno，一种基于LangGraph的模块化主动多智能体管线，协调六个专门智能体（从源文件分析到MoE参考分析及闭环取证验证）进行自动化数据构建与标注，生成结构化的对比式全注释。

Result: 构建了18K+高保真视频片段的HumanForge数据集，通过SOTA传统检测器和多模态大模型广泛基准测试，揭示了零样本泛化和细粒度推理方面的显著挑战。

Conclusion: HumanForge填补了人体中心视频伪造检测领域缺乏统一多范式基准的空白，Gen2Anno展示了无需人工标注即可高效构建高质量伪造数据集的潜力。

与遥感交叉或改进方案: 交叉/改进可行性：75%。遥感领域存在类似的伪造/合成数据需求（如卫星图像中的目标插入、场景篡改检测），可借鉴Gen2Anno的多智能体自动化管线架构：1) 将源文件分析智能体改为遥感多源传感器数据解析；2) MoE参考分析可针对不同地物类型（建筑、植被、水体）进行专家模型分配；3) 闭环取证验证可引入地理空间一致性校验。HumanForge的时空定位标注方式也可迁移至遥感变化检测任务的伪变化样本生成。

Abstract (英文): Rapid advancements in video diffusion models and temporal editing tools have enabled the generation of highly realistic human-centric videos, posing unprecedented challenges to digital content forensics. Existing benchmarks primarily focus on either face-swapping or global text-to-video synthesis, overlooking the crucial dimensions of human-object or human-human interactions and multi-modal alignment. To address these limitations, we introduce HumanForge, a unified, large-scale, and multi-paradigm human-centric video forgery dataset. To construct and annotate this dataset without labor-intensive manual labeling or hallucinated monolithic prompts, we propose Gen2Anno, a modular active multi-agent pipeline built on LangGraph. Gen2Anno coordinates six specialized agents-ranging from source profiling to MoE-based reference analysis and closed-loop forensic verification-to generate over 18K high-fidelity video segments and produce structured, contrastive omni-annotations containing binary decisions, fine-grained artifact categories, and spatio-temporal localization. Extensive benchmarks using state-of-the-art traditional detectors and Large Multimodal Models (LMMs) demonstrate the significant challenges of zero-shot generalization and fine-grained reasoning on HumanForge. Code and dataset will be publicly released.

</details>


### [63] [LTM: Large-scale Terrain Model for Wildfire-prone Landscapes](https://arxiv.org/abs/2607.08711)
*Xiao Fu,Yue Hu,Meida Chen,Peter Anthony Beerel,Barath Raghavan*

Main category: cs.CV

TL;DR: 提出一种利用旧DEM作为几何先验的多模态三维重建框架，通过物理驱动的像素对齐实现实时高保真地形图重建，用于野火应急响应。


<details>
  <summary>Details</summary>
Motivation: 野火危险评估需要大范围高精度3D地形图，但传统方法（空载LiDAR昂贵更新慢、纯图像方法因特征稀疏和重叠不足而效果差）难以满足需求，亟需低成本、高时效的解决方案。

Method: 1) 利用过时的数字高程模型（DEM）作为几何先验；2) 提出物理驱动的像素-像素对齐方法，将图像与DEM数据直接对齐，跳过昂贵的特征匹配步骤；3) 基于真实野火区域开发大范围地形仿真器生成图像用于评估。

Result: 在有已知位姿图像和旧DEM的条件下，该方法能实时生成高保真深度图，在重建精度和计算效率上显著优于现有技术。

Conclusion: 该方法为野火响应提供了一种可扩展、低成本、实时的3D地形重建方案，平衡了精度与效率。

与遥感交叉或改进方案: 交叉/改进可行性：95%。改进方案：1) 可直接应用于遥感中的数字地形模型（DTM）更新场景，结合卫星或无人机影像与旧DEM实现低成本地形变化监测；2) 物理对齐策略可推广至多源遥感数据（如光学+SAR）的融合配准，减少特征匹配计算开销；3) 大范围仿真器设计思路可迁移至遥感数据增强与算法评测。

Abstract (英文): Accurate 3D terrain maps are essential for emergency response when assessing wildfire hazards. However, wildfire-prone regions often span vast areas where conventional reconstruction methods underperform. Airborne LiDAR systems provide high-resolution terrain data, but they are expensive and infrequently updated. Image-based methods offer a lower-cost alternative, but struggle due to sparse visual features and limited image overlap. We propose a multi-modal reconstruction framework leveraging outdated Digital Elevation Models (DEMs) as geometric priors for image-based 3D reconstruction. Our key innovation is physics-based pixel-pixel alignment between images and DEM data, dramatically reducing computational complexity by eliminating expensive feature matching procedures. To validate our approach, we developed a large-terrain simulator based on a real wildfire-prone area, generating realistic images enabling a comprehensive evaluation. Given posed images and legacy DEMs, our method produces high-fidelity depth maps while maintaining real-time performance. We find significant improvements in reconstruction accuracy and computational efficiency over existing techniques, offering a scalable solution for wildfire response.

</details>


### [64] [Pose-to-Biomechanics: Bridging 3D Human Pose Estimation and Biomechanical Attribute Prediction](https://arxiv.org/abs/2607.08725)
*Ayda Eghbalian,Kevin Desai*

Main category: cs.CV

TL;DR: 提出BioModule，一个轻量级即插即用时序Transformer模块，可附加在任何3D姿态估计器之后，从标准17关节3D骨架预测生物力学属性，无需修改上游模型。


<details>
  <summary>Details</summary>
Motivation: 现有3D人体姿态估计主要优化几何关键点精度，而康复、运动科学、人体工学和临床运动分析等实际应用需要描述身体如何运动、加载和激活的生物力学量。缺乏将姿态估计扩展到生物力学分析的有效桥梁。

Method: 1) 设计轻量级即插即用时序Transformer模块(BioModule)，可附加于任意3D姿态估计器下游；2) 构建Human3.6M与Human3.6Mplus的大规模对齐数据集，建立两个数据集坐标系之间的解剖对应关系，实现帧精确的跨模态监督；3) 在7个SOTA姿态估计器上进行系统基准测试。

Result: BioModule成功从标准17关节3D骨架预测生物力学属性，且对上游姿态估计器不可知；首次系统分析了上游姿态估计质量如何传播到下游生物力学预测保真度。

Conclusion: BioModule作为连接基于视觉的姿态估计与生物力学上有意义的人体运动分析的紧凑模块化桥梁，为现有姿态估计器在物理可解释的运动分析任务中提供扩展能力。

与遥感交叉或改进方案: 交叉/改进可行性：65%。1) 遥感中行人或人体目标检测后的姿态估计结果可通过BioModule模块扩展为生物力学分析，用于评估灾区幸存者运动能力或军事人员动作分析；2) 遥感视频中的人群运动分析(如体育场、公共集会)可附加BioModule预测集体运动生物力学指标，辅助安全监控；3) 需解决遥感视角下(顶视/斜视)3D骨架估计精度不足导致的生物力学预测误差传播问题。

Abstract (英文): Recent progress in 3D human pose estimation has made markerless recovery of skeletal motion increasingly accurate and scalable. However, most pose estimators remain optimized for geometric keypoint accuracy, while many real-world applications in rehabilitation, sports science, ergonomics, and clinical movement analysis require biomechanical quantities that describe how the body moves, loads, and activates. In this work, we propose BioModule, a lightweight plug-in temporal transformer that attaches downstream of any 3D pose estimator and predicts biomechanical attributes from standard 17-joint 3D skeletons. BioModule is estimator-agnostic and requires no modification of the upstream pose model, enabling existing pose estimators to be extended toward physically interpretable motion analysis.
  To train and evaluate BioModule, we construct a large-scale aligned dataset pairing Human3.6M video and 3D keypoints with the biomechanical label space of Human3.6Mplus. We establish and verify anatomical correspondence between coordinate systems of the two datasets, enabling frame-accurate cross-modal supervision. Using this aligned supervision, BioModule predicts biomechanical quantities. We further benchmark BioModule across seven state-of-the-art 3D pose estimators, providing the first systematic analysis of how upstream pose estimation quality propagates to downstream biomechanical prediction fidelity. The results position BioModule as a compact, modular bridge between vision-based pose estimation and biomechanically meaningful human motion analysis.

</details>


### [65] [WaspMOT: A Benchmark for Long-Term Multi-Object Tracking of Trichogramma Wasps](https://arxiv.org/abs/2607.08729)
*Tomasz Stanczyk,Yuan Gao,Hardik Agarwal,Seongroo Yoon,Tiantao Zhang,Vincent Calcagno,Francois Bremond*

Main category: cs.CV

TL;DR: WaspMOT是一个面向长时多目标跟踪的新基准，包含10段每段约12,000帧的Trichogramma wasp视频，所有个体全程存在，要求跨数千帧保持身份一致性。评估五种跟踪方法发现所有方法均存在严重的轨迹碎片化问题，即使使用完美检测也是如此，表明长时身份维持仍是挑战。


<details>
  <summary>Details</summary>
Motivation: 现有MOT基准主要基于短视频序列，无法充分评估长时间跨度下的身份保持能力，而许多实际应用（如生态实验）要求持续跟踪同一对象数分钟甚至更久。作者提出WaspMOT以填补这一评估空白。

Method: 构建了包含10段长序列（每段约12,000帧，25 FPS，超过8分钟）的数据集，提供密集的MOTChallenge格式标注和oracle检测（用于隔离关联性能）。在封闭场景（所有个体全程存在）下，统一协议评估了ByteTrack、BoT-SORT、C-BIoU、OC-SORT和McByte五种tracking-by-detection方法，并引入简单空间轨迹拼接基线。

Result: 所有评估方法均出现显著的轨迹碎片化，即使使用oracle完美检测。简单的空间轨迹拼接基线能够一致地提升性能，表明当前方法在长时关联上仍有较大改进空间。

Conclusion: WaspMOT揭示了现有MOT方法在长时身份保持方面的局限性，这些问题是传统短视频基准无法暴露的。该基准为研究长时关联提供了新的测试平台，并表明长时跟踪仍有显著的性能提升潜力。

与遥感交叉或改进方案: 交叉/改进可行性：85%。遥感视频卫星或无人机平台常涉及长时间的连续观测（如目标监视、车流分析），同样面临身份漂移和轨迹碎片化问题。可将WaspMOT的长时关联评估思路引入遥感MOT：1）构建遥感长时跟踪基准，包含跨越数分钟到数十分钟的卫星/无人机视频序列，标注所有目标全程ID；2）借鉴WaspMOT的oracle检测隔离策略，单独评估遥感场景下关联模块的性能瓶颈；3）将WaspMOT中的空间轨迹拼接方法迁移至遥感目标跟踪，结合地理坐标约束提升跨帧ID一致性；4）WaspMOT中高相似外观目标（同种昆虫）的挑战与遥感中同类车辆/船只重识别问题高度相似，其身份保持方法可直接迁移。

Abstract (英文): Multi-object tracking (MOT) has achieved strong performance on benchmarks dominated by short video sequences. However, such datasets do not adequately evaluate long-term identity preservation, where objects must be tracked consistently over extended durations. We introduce WaspMOT, a benchmark designed to address this gap through long-duration tracking of Trichogramma wasps in controlled ecological experiments. The dataset contains 10 sequences of approximately 12,000 frames each (over 8 minutes at 25 FPS), with dense MOTChallenge annotations and oracle detections to isolate association performance.
  Unlike existing benchmarks, WaspMOT forms a closed-set tracking scenario where all individuals remain present throughout the sequence, requiring consistent identity assignment across thousands of frames despite abrupt jumps, occlusions, and highly similar appearance. We establish a benchmark by evaluating five tracking-by-detection methods, including ByteTrack, BoT-SORT, C-BIoU, OC-SORT, and McByte, under a unified protocol. Results show that all methods suffer from significant trajectory fragmentation, highlighting the difficulty of long-term identity preservation even with perfect detections. A simple spatial tracklet stitching baseline consistently improves performance, indicating that substantial gains remain possible.
  WaspMOT provides a new benchmark for studying long-term association and reveals limitations of current tracking approaches that are not observable on conventional datasets. The benchmark will be made publicly available at the project repository: https://github.com/tstanczyk95/WaspMOT/ .

</details>


### [66] [OpenCoF: Learning to Reason Through Video Generation](https://arxiv.org/abs/2607.08763)
*Xinyan Chen,Ziyu Guo,Renrui Zhang,Dongzhi Jiang,Hongsheng Li*

Main category: cs.CV

TL;DR: OpenCoF提出了一个包含11个任务族、17K视频的推理数据集OpenCoF-17K，并基于Wan2.2微调得到Wan-CoF模型，通过Chain-of-Frame（CoF）机制实现时序推理。进一步引入视觉与文本推理令牌，分别捕捉低层视觉线索和高层语义先验，在四个视频推理基准上显著提升了推理能力。


<details>
  <summary>Details</summary>
Motivation: 现有视频生成模型主要基于通用视频语料训练，缺乏面向推理的多样化监督信号和专门设计，导致其在Chain-of-Frame推理任务上表现不足。为了解决这一空白，作者提出OpenCoF框架以探索多样化的时序监督是否能够提升CoF推理能力。

Method: 1）构建OpenCoF-17K数据集，覆盖11个任务族的推理视频数据；2）基于Wan2.2-I2V-A14B基座模型微调得到Wan-CoF；3）引入视觉推理令牌和文本推理令牌，分别建模低层视觉线索和高层语义先验，用于空间和时序推理；4）在模型深度、去噪步数、空间和时间维度上进行注意力分析以评估令牌贡献。

Result: 在四个视频推理基准上，Wan-CoF相比Wan2.2-I2V-A14B基线取得显著提升。视觉与文本推理令牌的引入进一步增强了CoF推理能力，注意力分析揭示了不同令牌在模型深度、去噪步数、空间和时间维度上的差异化贡献。

Conclusion: 更强的视频推理能力既需要广泛的时序监督，也需要显式的中间推理状态组织机制。开源数据集、模型和代码以促进面向推理的视频生成研究。

与遥感交叉或改进方案: 交叉/改进可行性：75%。遥感领域存在大量时序推理需求，如变化检测、时序动态监测、灾害演进预测等。具体交叉方案：1）构建遥感时序推理数据集（如多时相遥感影像序列+变化推理标注），借鉴OpenCoF-17K的任务族分类方法；2）将Wan-CoF的Chain-of-Frame机制应用于遥感影像时序生成，实现从过去影像推理未来状态；3）视觉推理令牌可捕捉遥感图像中的低层物理变化（如植被指数变化），文本推理令牌承载高层语义（如"森林火灾后植被恢复"的先验知识），两者结合可提升遥感时序推理的准确性和可解释性。

Abstract (英文): Reasoning has become a core capability for large models, especially when reliable decisions require understanding logical consequences. Recent video generation models offer a reasoning path distinct from previous Chain-of-Thought (CoT): reasoning can unfold through temporally connected frames, known as Chain-of-Frame (CoF) reasoning. However, existing video generators are primarily trained on general video corpora, still lacking diverse supervision and dedicated designs for CoF reasoning. To address this gap, we introduce OpenCoF, a framework comprising the OpenCoF-17K dataset, a reasoning video dataset spanning 11 task families, and Wan-CoF, a fine-tuned video model for studying whether diverse temporal supervision improves CoF behavior. Across four video reasoning benchmarks, Wan-CoF achieves considerable gains over the Wan2.2-I2V-A14B baseline. Building on this, we empirically explore more advanced designs for CoF capabilities, i.e., equipping the model with visual and textual reasoning tokens. This mechanism respectively captures low-level visual cues and high-level semantic priors for spatial and temporal reasoning. Through performance comparisons and attention analysis, we examine how these tokens contribute across model depth, denoising steps, space, and time. Our results suggest that stronger video reasoning requires both broad temporal supervision and explicit mechanisms for organizing intermediate reasoning state. We open-source the dataset, model, and code to facilitate future research on reasoning-oriented video generation.

</details>


### [67] [Enhancing In-context Panoramic Generation via Geometric-aware Pretraining](https://arxiv.org/abs/2607.08765)
*Haoran Feng,Ruiyang Zhang,Longyi Zhang,Dizhe Zhang,Lu Qi*

Main category: cs.CV

TL;DR: Canvas360是一个两阶段全景生成框架，结合几何感知预训练与下游任务微调，通过提出的包含100万样本的Canvas360Dataset数据集，以及并行深度生成、速度循环填充和相似性损失正则化等技术，实现了文本到全景的高质量生成，并支持风格迁移、修复、扩展和编辑等多种上下文生成任务。


<details>
  <summary>Details</summary>
Motivation: 现有全景生成方法缺乏大规模高质量训练数据，且难以在几何一致性和全局连贯性上达到理想效果，同时缺乏统一框架来支持多样化的上下文全景生成任务。

Method: 提出两阶段框架：第一阶段进行几何感知预训练，采用并行深度生成、速度循环填充和相似性损失正则化来学习几何感知表征；第二阶段基于Token级拼接实现统一的上下文全景生成框架，支持多种下游任务微调。同时构建了包含100万高质量配对全景样本的Canvas360Dataset数据集。

Result: 在全景图像保真度方面表现优异，尤其在全景特定FAED指标上取得突出性能，并在多项定量评估中达到领先或竞争力水平。

Conclusion: Canvas360通过几何感知预训练和统一的上下文生成框架，在任务覆盖和建模灵活性上超越了先前方法，有效提升了全景图像生成质量。

与遥感交叉或改进方案: 交叉/改进可行性：85%。遥感领域广泛使用全景影像（如街景、无人机航拍全景），Canvas360的几何感知预训练和速度循环填充技术可迁移至遥感全景影像的几何校正与拼接任务；其1M数据集构建策略可为遥感领域缺乏标注全景数据的问题提供解决思路；基于Token级拼接的统一框架可用于遥感影像的跨任务生成（如全景分割、变化检测、超分辨率重建）。改进方向：针对遥感影像的多光谱/高光谱特性，需扩展Canvas360的深度生成模块以支持多通道输入。

Abstract (英文): In this work, we present Canvas360, a two-stage framework for in-context panoramic generation that combines geometry-aware pretraining with downstream task-specific fine-tuning. To address the lack of large-scale, high-quality training data tailored to in-context panoramic tasks, we propose Canvas360Dataset, a collection of 1M high-quality paired panoramic samples for style transfer, inpainting, outpainting, and editing, enabling effective supervision across diverse in-context generation scenarios. On the modeling side, Canvas360 enhances text-to-panorama generation through parallel depth generation, velocity circular padding, and similarity loss regularization, enabling the model to learn geometry-aware representations, capture object distortion details, and improve geometric consistency and global coherence. Furthermore, empowered by strong panoramic priors, Canvas360 enables a unified in-context panoramic generation framework that supports diverse downstream tasks via token-level concatenation, surpassing prior methods in both task coverage and modeling flexibility. Extensive experiments show that Canvas360 improves panoramic image fidelity, achieving particularly strong performance on the panorama-specific FAED metric and competitive or leading results across the reported quantitative evaluations. More information can be found on our project page: https://zry000.github.io/Canvas360/

</details>


### [68] [OPSD-V: On-Policy Self-Distillation for Post-Training Few-Step Autoregressive Video Generators](https://arxiv.org/abs/2607.08766)
*Hongyu Liu,Chun Wang,Feng Gao,Xuanhua He,Yue Ma,Ziyu Wan,Yong Zhang,Xiaoming Wei,Qifeng Chen*

Main category: cs.CV

TL;DR: OPSD-V提出了一种基于策略的自蒸馏范式，用于后训练优化少步自回归视频扩散模型。通过引入真实长视频作为时间上下文，在训练时提供密集轨迹级监督，学生模型按照推理时的自回归方式生成，教师模型则使用更干净的缓存（用真实视频替换历史信息）提供矫正目标。应用于Self-Forcing和LongLive模型后，在视觉质量、运动动态和VBenchLong分数上均有一致提升，用户偏好率达66.0%。


<details>
  <summary>Details</summary>
Motivation: 现有的少步自回归视频生成器虽然能生成长视频且延迟低，但在长自回归 rollout 过程中存在误差累积和运动动态减弱的问题。OPSD-V旨在减少长程退化，同时保留原有的少步推理路径。

Method: OPSD-V的核心是引入真实长视频作为时间上下文，在训练中提供密集轨迹级监督。学生模型按照推理时的 rollout 方式生成，每个 chunk 基于自身之前生成的 KV 缓存。教师模型在相同的去噪状态下进行评估，但使用更干净的自回归一致性时间缓存（旧历史被真实视频上下文替换），从而在策略自回归缓存动态下提供密集的去噪级矫正目标，且不改变采样器、去噪步数或推理时缓存机制。

Result: 在Self-Forcing和LongLive两个代表性少步自回归视频模型上应用OPSD-V后，视觉质量、运动动态和VBenchLong分数均有一致提升。10名参与者对比20个视频对的用户研究中，OPSD-V的整体偏好率为66.0%（排除平局后为82.5%）。

Conclusion: OPSD-V是一种有效的后训练自蒸馏范式，能在不改变推理流程的前提下显著改善少步自回归视频扩散模型的长视频生成质量，减少误差累积和运动退化。

与遥感交叉或改进方案: 交叉/改进可行性：85%。遥感视频生成/预测中同样存在长时序自回归误差累积和运动动态退化问题。具体方案：(1) 将OPSD-V应用于遥感卫星时序视频的少步生成，利用真实遥感长序列（如连续云图、变化检测序列）作为教师模型的上下文缓存；(2) 针对遥感场景中运动幅度较小但语义变化关键的特点（如植被变化、城市扩张），可设计时空一致的KV缓存替换策略，用真实遥感帧替换生成帧的历史缓存，提供更精确的轨迹级监督；(3) 结合遥感中的多尺度特征（不同分辨率下的地表变化），扩展OPSD-V为多尺度自蒸馏框架。

Abstract (英文): We propose OPSD-V, an on-policy self-distillation paradigm for post-training few-step autoregressive (AR) video diffusion models. Existing few-step AR video generators can produce long videos with low latency, but still suffer from error accumulation and weakened motion dynamics during long autoregressive rollout. OPSD-V reduces long-horizon degradation while preserving the original few-step inference path. The key idea is to introduce real long-video data as temporal context during training and use it to provide dense trajectory-level supervision. Specifically, the student follows the exact inference-time rollout, generating each chunk conditioned on its own previously generated KV cache. In parallel, the teacher is evaluated at the same student-visited denoising states, but uses a cleaner AR-consistent temporal cache in which older history can be replaced by real-video context. This provides dense denoising-level corrective targets under on-policy AR cache dynamics, without changing the sampler, number of denoising steps, or inference-time cache mechanism. We apply OPSD-V to representative few-step AR video models, including Self-Forcing and LongLive. Experiments show consistent improvements in visual quality, motion dynamics, and VBenchLong scores. A user study with 10 participants comparing 20 video pairs shows that OPSD-V is preferred over the base models in 66.0% of overall-preference judgments (82.5% excluding ties).

</details>


### [69] [Geometry and Gradient-based Partitioning for Panoramic Outdoor Reconstruction](https://arxiv.org/abs/2607.08769)
*Weijian Chen,Weibo Yao,Yuhang Zhang,Xiaolin Tang,Guo Wang,Weijun Zhang,Xitong Gao,Yihao Chen,Hongde Qin,Lu Qi*

Main category: cs.CV

TL;DR: PanoLOG提出了一种面向大规模户外场景的全景3D高斯泼溅（3DGS）框架，通过两阶段粗到精优化和几何-梯度引导的分区策略（G²PS），解决了全景图像全局可见性导致的分块训练退化问题，在Pano360基准上实现了SOTA渲染质量与可扩展的块并行训练。


<details>
  <summary>Details</summary>
Motivation: 将3DGS扩展到大规模户外场景存在数据采集和计算成本高昂的问题。虽然全景图像（ERP）能通过360°视场降低采集成本，但其全局可见性使得现有依赖局部相机视锥的分区策略失效，导致分块优化退化为全局训练，无法实现可扩展的块并行重建。

Method: 提出两阶段粗到精框架PanoLOG：1）全局粗阶段利用天空球建模和全景单目深度监督获得可靠的初始几何；2）精化阶段提出G²PS（几何-梯度分区策略），通过视差驱动的自适应包围体和不确定性度量，结合基于梯度的相机重要性评分进行相机分配。同时构建了首个大规模户外全景重建基准Pano360。

Result: 在Pano360基准上，G²PS实现了SOTA的渲染质量，同时保持了可扩展的块并行训练能力。

Conclusion: PanoLOG有效解决了全景图像下3DGS大规模场景重建的分区训练难题，为户外全景场景的实时高质量渲染提供了可行方案。

与遥感交叉或改进方案: 交叉/改进可行性：85%。该框架的全景分区策略可直接应用于遥感中的大范围正射影像和全景拼接重建：1）将G²PS的视差驱动自适应包围体引入遥感影像块划分，解决广域影像中地物视差不一致导致的分块边界伪影问题；2）天空球建模方法可用于遥感中的大气/云层建模与去除；3）梯度重要性评分机制可迁移至多视角卫星或无人机影像的稀疏/非均匀采样场景，优化块间协同训练效率。

Abstract (英文): Scaling 3D Gaussian Splatting (3DGS) to large outdoor scenes is costly in both data acquisition and computation. Adopting panoramic images with equirectangular projection (ERP) can reduce capture effort via their full $360^{\circ}$ field of view, yet the resulting omnipresent visibility invalidates existing partitioning strategies that rely on local camera frustums, causing block-wise optimization to degenerate into global training. Thus, we propose PanoLOG, a two-stage coarse-to-fine framework equipped with a Geometry and Gradient-based Partitioning Strategy tailored for large-scale panoramic 3DGS reconstruction. In the global coarse stage, PanoLOG leverages sky-sphere modeling and panoramic monocular depth supervision for reliable geometry, while in the refinement stage, G$^2$PS builds adaptive bounding volumes via parallax-driven uncertainty and assigns cameras via gradient-based importance scoring. Furthermore, we construct Pano360, the first benchmark on large-scale panoramic dataset for outdoor scene reconstruction. Extensive experiments demonstrate that G$^2$PS achieves state-of-the-art rendering quality while maintaining scalable, block-parallel training. Our models, training code, and dataset are publicly available.

</details>


### [70] [LongE2V: Long-Horizon Event-based Video Reconstruction, Prediction, and Frame Interpolation with Video Diffusion Models](https://arxiv.org/abs/2607.08770)
*Cheng-De Fan,Chun-Wei Tuan Mu,Chen-Wei Chang,Chin-Yang Lin,Kun-Ru Wu,Yu-Chee Tseng,Yu-Lun Liu*

Main category: cs.CV

TL;DR: LongE2V利用预训练的视频扩散先验，通过自回归展开、自适应上下文切换、重编码对齐与交叉残差校正以及事件体素密度增强等技术，实现高质量的事件视频重建、预测和帧插值，在长时间序列稳定性和零样本泛化方面超越现有方法。


<details>
  <summary>Details</summary>
Motivation: 从稀疏事件流中恢复高质量视频具有挑战性：回归方法常导致纹理模糊，现有生成模型在长时间序列中缺乏稳定性。现有方法无法同时高效地处理事件视频重建、预测和帧插值三个任务。

Method: 1) 利用预训练视频扩散先验进行微调，实现高数据效率和优越感知质量；2) 自回归展开（Autoregressive Unrolling）与自适应上下文切换（Adaptive Context Switching）解决极长序列的时间漂移；3) 重编码对齐与交叉残差校正（Reencoding Alignment with Cross Residual Correction）确保帧插值中的双向一致性；4) 事件体素密度增强（Event Voxel Density Augmentation）提升对不同传感器分辨率的鲁棒性。

Result: 在真实世界基准上的大量实验表明，LongE2V在事件视频重建、预测和帧插值三个任务上均优于现有方法，展现出卓越的时间一致性和零样本泛化能力。

Conclusion: LongE2V证明了将预训练视频扩散先验有效迁移到事件视频领域是可行的，且所提出的多种机制（自回归展开、自适应上下文切换等）有效解决了长时间序列生成中的关键挑战，为事件相机视频处理提供了新的有效范式。

与遥感交叉或改进方案: 交叉/改进可行性：85%。遥感领域中，事件相机可用于对地观测中快速运动目标（如卫星、飞机、车辆）的捕获。具体交叉方案：1) 将LongE2V的事件视频重建能力应用于遥感事件相机数据，提升低光照或高动态范围场景下的运动目标清晰度；2) 利用其帧插值能力对遥感卫星视频进行时域超分辨率，增强对地连续观测的帧率；3) 可将事件体素密度增强适配到不同空间分辨率的遥感传感器上，提升跨平台泛化能力；4) 需注意遥感场景下事件流噪声特性与室内场景不同，需针对性地调整Reencoding Alignment中的残差校正策略。

Abstract (英文): Recovering high-quality video from sparse event streams is a challenging task. Regression methods often blur textures, while existing generative models struggle with long-term stability. We propose LongE2V, a novel approach that leverages pre-trained video diffusion priors to jointly handle event-based video reconstruction, prediction, and frame interpolation. By fine-tuning a foundational video model, our approach achieves high data efficiency and superior perceptual quality. We introduce Autoregressive Unrolling and Adaptive Context Switching to mitigate temporal drift in extremely long sequences. We also propose Reencoding Alignment with Cross Residual Correction to ensure precise bidirectional consistency during frame interpolation. Furthermore, Event Voxel Density Augmentation ensures robustness across varying sensor resolutions. Extensive experiments on real-world benchmarks demonstrate that LongE2V outperforms state-of-the-art methods across all three tasks, exhibiting exceptional temporal coherence and zero-shot generalization. Project page: https://cdfan0627.github.io/LongE2V-page/

</details>


### [71] [ZipDepth: Bringing Lightweight Zero-Shot Monocular Depth Anywhere, on Any Device](https://arxiv.org/abs/2607.08771)
*Fabio Tosi,Luca Bartolomei,Matteo Poggi,Stefano Mattoccia*

Main category: cs.CV

TL;DR: ZipDepth是一个仅6.1M参数的轻量级单目深度估计网络，通过可重参数化编码器-解码器结构和大规模多域知识蒸馏，在零样本泛化性和部署效率之间达到最佳平衡，性能接近具有50倍参数量的基础模型。


<details>
  <summary>Details</summary>
Motivation: 现有基础模型零样本深度估计精度高但计算开销大，无法部署在嵌入式/移动平台；轻量级模型仅基于单域自监督范式开发，在域迁移时表现不佳。急需一种兼顾零样本泛化能力和部署效率的紧凑型深度估计网络。

Method: 提出ZipDepth，核心包括：(1) 高效可重参数化编码器-解码器结构，仅6.1M参数；(2) 大规模多域训练集上的基础模型知识蒸馏，将大模型零样本泛化能力迁移至轻量网络。

Result: 在5个基准上，ZipDepth在轻量级模型中实现了零样本精度与部署效率的最佳权衡，以50倍少的参数达到接近基础模型的精度，且能在从服务器GPU到功耗受限设备上实时运行。

Conclusion: ZipDepth通过高效架构设计与大规模知识蒸馏，成功弥合了轻量级深度估计模型与基础模型之间的精度差距，为资源受限平台上的零样本深度估计提供了可行方案。

与遥感交叉或改进方案: 交叉/改进可行性：85%。遥感领域对轻量化、实时性深度估计需求迫切（如无人机、卫星在轨处理）。具体方案：(1) 将ZipDepth的可重参数化编码器替换为针对遥感多光谱/高光谱数据适配的轻量化骨干；(2) 利用遥感大模型（如地理预训练基础模型）作为教师进行域特定知识蒸馏，增强对遥感场景（建筑物、地形、植被）的深度估计能力；(3) 针对遥感多视角立体匹配场景，扩展ZipDepth为多视图深度估计框架。

Abstract (英文): Monocular depth estimation has seen remarkable progress through foundation models achieving robust zero-shot generalization, yet their computational demands place them far beyond the reach of embedded and mobile platforms. Lightweight alternatives exist, but have been developed almost exclusively within single-domain, self-supervised paradigms, failing silently under domain shift. We present ZipDepth, a compact monocular depth network that bridges this gap by combining an efficient reparameterizable encoder-decoder with large-scale knowledge distillation from a foundation model over a large multi-domain training set. Comprising just 6.1M parameters, ZipDepth runs at real-time rates from server GPUs to power-constrained devices, achieving the best trade-off between zero-shot accuracy and deployment efficiency among lightweight models across five benchmarks, taking a significant step towards the accuracy of foundation models with 50x more parameters.

</details>


### [72] [Wat3R: Underwater 3D Geometry Learning without Annotations](https://arxiv.org/abs/2607.08772)
*Jiangwei Ren,Xingyu Jiang,Zijie Song,Wei Xu,Hongkai Lin,Dingkang Liang,Xiang Bai*

Main category: cs.CV

TL;DR: Wat3R提出了一种跨域半监督学习框架，利用教师-学生架构从无标注的真实水下视频中学习3D几何重建，无需任何水下标注数据，并构建了Water3D数据集用于评估。实验表明，该方法在水下多视图深度估计和点云重建上优于现有方法。


<details>
  <summary>Details</summary>
Motivation: 水下环境存在光衰减、散射等问题，且缺乏大规模高质量3D标注数据，现有依赖密集标注的方法在水下场景中不实用。因此，作者希望设计一种无需水下标注数据的跨域半监督学习框架，将空气域的前馈3D重建模型迁移到水下场景。

Method: 采用教师-学生半监督学习架构，仅利用大量无标注的真实水下视频数据学习鲁棒的几何表示；设计跨视图一致性损失，利用其他视图的几何线索补偿当前视图因水衰减和散射造成的信息退化；构建了涵盖多种水体和水下场景的Water3D数据集用于评估。

Result: 在水下多视图深度估计和点云重建任务上，Wat3R的性能优于当前最先进方法。

Conclusion: Wat3R证明了无需任何水下标注数据，仅通过半监督跨域适应即可有效将空气域3D重建模型迁移到水下场景，为水下几何感知提供了一种实用且高效的解决方案。

与遥感交叉或改进方案: 交叉/改进可行性：85%。水下环境的光学退化问题（衰减、散射）与遥感中大气/水体对成像的影响高度相似。可将Wat3R的跨视图一致性损失和教师-学生半监督框架迁移到遥感领域，例如用于水下/浑浊水域的遥感目标3D重建、浅海地形反演或珊瑚礁监测。改进方向：引入多光谱/高光谱遥感数据替代RGB视频，利用不同波段对不同水体的穿透特性增强几何线索；结合遥感中的大气校正模型替代当前的经验性衰减补偿，提升跨域泛化能力。

Abstract (英文): Estimating 3D geometry in underwater environments presents unique challenges due to light attenuation, scattering, and the absence of large-scale, high-quality 3D annotations. Pioneering methods rely on massive dense annotations that are impractical in underwater settings. In this paper, we propose Wat3R, a cross-domain semi-supervised learning framework designed to adapt feed-forward 3D reconstruction models from air to underwater scenes. Uniquely, our method eliminates the need for any annotated underwater data following a teacher-student architecture, that learns robust geometry representations merely on abundant unlabeled real underwater video footage. We also design a cross-view consistency loss that leverages geometric cues from other views to compensate for the information degradation in the current view caused by water attenuation and scattering. Furthermore, considering the lack of comprehensive evaluation benchmarks, we construct Water3D, a diverse dataset covering various water bodies and underwater scenarios, designed for geometric task evaluation. Experimental results demonstrate that Wat3R outperforms current state-of-the-art methods in underwater multi-view depth estimation and point cloud reconstruction. The dataset and code are available at https://github.com/LSXI7/Wat3R .

</details>


<div id='GIScience & RS'></div>

# GIScience & RS [[Back]](#toc)

### [73] [An explainable optical flow network for arctic sea ice drift extraction from multi-frequency AMSR2 passive microwave data](https://doi.org/10.1080/15481603.2026.2697124)
*Peng Jiang,Tiantian Feng,Yushi Yang,Yuxuan Hu,Chen Zhao*

Main category: GIScience & RS

TL;DR: 本文提出MF-IceDrift，一种基于多频AMSR2数据的可解释深度学习框架，用于全年、空间完整的北极海冰漂移提取。通过多频特征提取融合模块、Transformer-based TransUpdate块和自监督策略，解决了高频通道受大气和融雪噪声影响、低频通道分辨率不足的问题。验证表明该方法在保持100%空间覆盖的同时达到最优精度，并可泛化至FY-3D MWRI和DMSP SSMIS数据。


<details>
  <summary>Details</summary>
Motivation: 北极海冰漂移监测对理解极地气候动力学至关重要，但被动微波辐射计的多频通道存在矛盾：高频通道细节丰富但受大气和融雪噪声影响严重，低频通道稳定但分辨率不足以解析小尺度漂移。现有方法缺乏有效融合多频优势的方案。

Method: 1) 多频特征提取与融合模块，从不同频率通道提取并融合互补特征；2) 基于Transformer的TransUpdate块，用于优化光流估计；3) 基于增广一致性的自监督策略，确保漂移场的空间连贯性。

Result: 与浮标观测验证显示，MF-IceDrift在保持100%空间覆盖的同时实现最佳精度，优于现有决策级多频融合方法和单频基线。提取的漂移场与业务化海冰漂移产品高度相关。方法可泛化至FY-3D MWRI和DMSP SSMIS数据。

Conclusion: 本研究提出的深度学习网络为利用多频被动微波数据进行大尺度、长时序海冰漂移提取提供了新方法。

与遥感交叉或改进方案: 交叉/改进可行性：95%。该论文本身就是遥感领域（海冰漂移提取）的论文。改进方案：1) 可将多频融合策略迁移至其他遥感任务，如多光谱/高光谱图像融合中的地物分类或变化检测；2) TransUpdate模块可改进为时空注意力机制，进一步捕获海冰漂移的时间连续性；3) 自监督策略可扩展为半监督学习，利用少量高分辨率SAR数据辅助训练，提升细节精度；4) 模型可结合扩散模型或物理信息神经网络，引入海冰动力学约束以提高物理可解释性。

Abstract (英文): Arctic sea ice drift monitoring is essential for understanding polar climate dynamics. Passive microwave radiometers provide valuable daily observations of the Arctic region. However, their use for sea ice drift extraction involves a trade-off among frequency channels: high-frequency channels offer finer spatial details but are strongly affected by atmospheric factors and melt-related radiometric noise, whereas low-frequency channels are more stable but too coarse to resolve small-scale drift. To address these challenges, this study proposes MF-IceDrift, a novel explainable deep learning framework for year-round, spatially complete Arctic sea ice drift extraction using multi-frequency AMSR2 data. Specifically, a Multi-frequency feature extraction and fusion module is designed to extract and fuse complementary features from different frequency channels. Moreover, a Transformer-based TransUpdate block and an augmentation consistency-based self-supervised strategy are incorporated to refine the flow estimation and ensure the spatial coherence of the drift fields. Validation against buoy observations demonstrates that MF-IceDrift achieves the best accuracy while maintaining 100% spatial coverage, consistently outperforming existing multi-frequency decision-level fusion methods as well as single-frequency baselines. The extracted drift fields exhibit a high degree of correlation with operational sea ice drift products, confirming the reliability of the large-scale drift patterns. Furthermore, the MF-IceDrift demonstrates generalization capabilities on the Microwave Radiation Imager onboard the Feng-Yun 3D (FY-3D MWRI) and Defense Meteorological Satellite Program Special Sensor Microwave Imager/Sounder (DMSP SSMIS) data. The deep learning network developed in this study provided a new approach for large-scale and long-term sea ice drift extraction using multi-frequency passive microwave data.

</details>


### [74] [Generating a 1 km nationwide long-term NDVI dataset for China (1982–2015) via an enhanced vegetation monitoring super-resolution network](https://doi.org/10.1080/15481603.2026.2694173)
*Mengmeng Sun,Song Ji,Dazhao Fan,Rongrong Zhang,Xin Liu,Zhen Yan*

Main category: GIScience & RS

TL;DR: 本文提出了一种高性能超分辨率（HPSR）方法，将5 km AVHRR NDVI数据降尺度为1 km，生成了中国1982–2015年间的1 km全国NDVI时序数据集，填补了高分辨率遥感观测在1982–2000年间的空白。


<details>
  <summary>Details</summary>
Motivation: 现有1 km MODIS NDVI产品始于2000年，1982–2000年间缺乏高分辨率NDVI观测数据，限制了植被变化的系统研究。为填补这一时空空白，需要构建一个长时序、高分辨率的NDVI数据集。

Method: 构建HPSR网络，包含三个子模块：(i)多尺度特征提取网络，通过并行不同尺度的卷积提取多尺度纹理信息并融合；(ii)注意力精炼网络，利用CBAM（通道与空间注意力机制）自适应加权融合特征，突出植被区域并抑制噪声；(iii)超分辨率重建网络，通过上采样模块和残差块将特征图重建为高分辨率。以1 km MODIS NDVI为参考，融合DEM地形因子和CLCD土地覆盖数据作为辅助数据。

Result: HPSR有效缓解了标准卷积模型中的数值集中问题，MAE为0.050，RMSE为0.061，PSNR保持在27 dB以上，成功保留了原始信号的熵和统计分布特征。

Conclusion: 该方法解决了空间分辨率与时间覆盖之间的传统权衡，生成了中国1982–2015年1 km全国NDVI数据集，为长期生态监测和生物物理分析提供了一致、高质量的基础数据。

与遥感交叉或改进方案: 交叉/改进可行性：95%。本文本身就是遥感领域的超分辨率降尺度研究，改进方向包括：(1)引入更多辅助数据如气象数据（降水、温度）、土壤类型等进一步提升降尺度精度；(2)将该框架迁移至其他植被指数（如EVI、LAI）或地表参数（如地表温度、反照率）的长时序重建；(3)结合时序注意力机制或Transformer架构增强时间维度的建模能力；(4)针对不同土地覆盖类型训练专用模型以提升复杂地表的降尺度表现。

Abstract (英文): The Normalized Difference Vegetation Index (NDVI) serves as a crucial indicator for assessing extended vegetation trends. Existing 1 km MODIS NDVI products start from 2000, leaving a gap in high‑resolution observations for 1982–2000 and limiting systematic studies of vegetation change across China. To fill this spatio-temporal void, we built a 1 km nationwide NDVI time series dataset covering 1982–2015 by downscaling the 5 km AVHRR NDVI with a High‑Performance Super‑Resolution (HPSR) method. The proposed network architecture is composed of three key submodules that work sequentially and synergistically: (i) A multi-scale feature extraction network first extracts texture information across different receptive fields through parallel convolutions at varying scales, achieving multi-scale fusion via feature splicing. (ii) The resulting fused features are then passed to an attention refinement network that utilizes the Convolutional Block Attention Module (CBAM), incorporating channel and spatial attention mechanisms, to adaptively weight the fusedu features, thereby emphasizing significant vegetation regions while reducing irrelevant noise. (iii) Finally, the refined features are fed into a super-resolution network that utilizes up-sampling modules and residual blocks to reconstruct the feature maps to a high resolution. The model spatially downscales AVHRR NDVI data from an approximately 5 km resolution to 1 km, utilizing high-quality 1 km MODIS NDVI as a reference dataset, and integrating DEM terrain factors and the China Land Cover Database (CLCD) as ancillary data. The experimental findings indicate that the HPSR framework effectively mitigates the numerical centralization common in standard convolutional models through an integrated attention mechanism, achieving a significant reduction in MAE (0.050) and RMSE (0.061) while maintaining a PSNR above 27 dB. By ensuring a holistic enhancement of NDVI sequences from high spatial fidelity to long-term physical and logical consistency, the model successfully preserves the original signal entropy and statistical distribution characteristics across complex landscapes. Consequently, by resolving the traditional trade-off between spatial resolution and temporal coverage, this study produced a 1 km nationwide NDVI dataset for China (1982–2015), furnishing a consistent, high-quality baseline for decadal-scale ecological monitoring and biophysical analysis.

</details>


### [75] [A coherence time-series framework for earthquake-induced building damage detection using SAR](https://doi.org/10.1080/15481603.2026.2698138)
*Yeji Lee,Sang-Eun Park*

Main category: GIScience & RS

TL;DR: 本文提出了一种基于相干性时间序列的震害检测框架，通过显式建模震前、震中、震后去相干分量，构建了两个互补指标（ρ0和ρ1），在熊本地震和能登半岛地震的Sentinel-1数据上验证，AUPRC达0.56-0.66，显著优于传统方法（0.22-0.39）。


<details>
  <summary>Details</summary>
Motivation: 现有InSAR相干性震害检测方法受多种去相干机制干扰，难以从有限图像对中分离结构损伤信号，导致虚警率高。

Method: 提出时序相干性框架，显式建模不同震相的去相干分量，通过候选像素选择和时序平均抑制噪声，导出震前-震中相干比ρ0和震前-震后相干比ρ1两个互补指标。

Result: 在Mashiki Town和Wajima City两个站点，ρ1的AUPRC分别为0.56和0.66，而三种参考方法仅为0.22-0.39，虚警率显著降低。

Conclusion: 显式时序建模相干性去相干可提供物理可解释且操作实用的震后损伤制图方案。

与遥感交叉或改进方案: 交叉/改进可行性：95%。改进方案：（1）可引入光学遥感（如高分辨率影像）或LiDAR数据作为辅助验证，进一步降低植被覆盖区的虚警；（2）可结合InSAR形变测量（如DInSAR或SBAS）与相干性指标，实现形变-相干性联合震害评估；（3）可将框架扩展至多轨道/多极化Sentinel-1数据，提升空间采样密度和检测精度；（4）可引入深度学习时序模型（如LSTM或Transformer）自动学习去相干模式，替代手工建模的相位分解。

Abstract (英文): Earthquake-induced building damage detection using interferometric SAR coherence is complicated by multiple decorrelation mechanisms unrelated to structural damage. Most existing approaches rely on coherence changes extracted from a limited number of image pairs, which makes it difficult to isolate structural damage signals from other temporally varying contributions. This study proposes a sequential coherence time-series framework that explicitly models decorrelation components across pre-, co-, and post-seismic phases according to their distinct spatiotemporal characteristics, and derives two complementary damage indicators: the average pre-to-co-seismic coherence ratio (ρ0) and the average pre-to-post-seismic coherence ratio (ρ1). Both indicators combine candidate pixel selection and temporal averaging to suppress damage-irrelevant coherence variability while amplifying step-type transitions associated with building damage. ρ0 supports rapid initial assessment using the first post-event acquisition, whereas ρ1 provides higher specificity to persistent structural damage by excluding the co-seismic pair from the indicator computation. The framework is validated using Sentinel-1 time series over Mashiki Town and Wajima City, affected by the 2016 Kumamoto and 2024 Noto Peninsula earthquakes. Compared with three representative coherence-based damage indicators, the proposed approach consistently reduces false alarms in both urban and non-urban areas. ρ1 demonstrates substantially improved precision–recall performance under class imbalance conditions, with AUPRC values of 0.56 and 0.66 at the two sites, compared to 0.22–0.39 for the reference methods. These results indicate that explicit temporal modeling of coherence decorrelation provides a physically interpretable and operationally practical basis for post-earthquake damage mapping.

</details>


### [76] [Can the MODIS EVI formula be directly applied to other sensor data? A case study with Landsat data](https://doi.org/10.1080/15481603.2026.2697413)
*Yafen Bai,Hanqiu Xu,Guifen Su,Tingting Shi,Lijuan Yang*

Main category: GIScience & RS

TL;DR: 该研究系统评估了MODIS增强型植被指数（EVI）直接应用于Landsat-8/9数据的可行性，发现直接套用MODIS参数会导致显著偏差（EVI偏差比NDVI大1.7倍，R²偏差达10%）。通过引入EVIb辅助指数和以NDVI为基准的分析框架，研究揭示了蓝色波段并非偏差主因，而是MODIS特定参数不适用于Landsat数据。土壤调节因子L是最敏感参数。研究提出了两种参数优化策略：站点特定交叉校准法（将偏差从13%降至近0%）和基于Nelder-Mead算法的通用参数优化（平均偏差降至8.33%），交叉验证显示优化参数可减少约32.5%的偏差。


<details>
  <summary>Details</summary>
Motivation: MODIS EVI的經驗参数专为MODIS传感器设计，但近三十年来用户直接将其套用于其他卫星传感器（如Landsat）而未验证其适用性，可能导致显著的植被监测偏差。这一问题长期被忽视，缺乏系统性的定量评估。

Method: 以无参数NDVI作为参考基准，设计EVIb辅助指数分离蓝波段影响；利用MODIS与Landsat-8/9同步观测数据，在11个植被测试区进行对比分析；采用敏感性分析评估EVI四个参数（L、C1、C2、G）的交互作用；提出站点特定交叉校准法和基于Nelder-Mead算法的通用参数优化策略。

Result: MODIS与Landsat-8/9的EVI偏差平均比NDVI大1.7倍，R²偏差达10%；蓝波段非偏差主因；土壤调节因子L最敏感；站点特定交叉校准法将|PB|从13%降至近0%；通用参数优化将平均|PB|降至8.33%；交叉验证显示优化参数在未见过的相似生物群区可减少约32.5%的偏差。

Conclusion: MODIS EVI参数直接跨传感器应用会引入显著偏差，建议进行传感器特定参数优化。研究提供了可迁移的方法论框架，以指导未来多传感器一致性的植被监测工作。

与遥感交叉或改进方案: 交叉/改进可行性：95%。作为一篇典型的遥感交叉传感器校正论文，其改进方向包括：(1) 将该方法论框架扩展至其他传感器组合（如Sentinel-2、VIIRS、GF系列等），解决多源遥感植被指数一致性问题；(2) 将Nelder-Mead优化算法替换为更先进的优化方法（如贝叶斯优化、粒子群算法或深度学习参数估计），提升参数泛化能力；(3) 引入更多生物群系类型进行验证，构建全球尺度的传感器特定EVI参数查找表；(4) 将该框架推广至其他植被指数（如EVI2、SAVI、LAI等）的跨传感器校正；(5) 结合辐射传输模型进行物理机制层面的参数优化，替代纯统计优化。

Abstract (英文): The question of whether the MODIS enhanced vegetation index (EVI) can be directly applied to other satellite sensors has remained unexplored for nearly three decades since its development. This is important because the EVI's empirical parameters are specifically designed for MODIS. For a long time, users have applied these MODIS-based parameters directly to other sensor data without modification. Therefore, this study systematically explores this issue using Landsat-8/9 data. We introduce a novel methodological framework: using NDVI (parameter-free) as a reference benchmark and designing an auxiliary index (EVIb) to isolate the impact of the blue band, as it is introduced in EVI. Results show that, based on the synchronized observations of MODIS and Landsat-8/9 across eleven vegetation test areas, the deviation between MODIS and Landsat-8/9 EVI is, on average, 1.7 times larger than that of their NDVI, with an R 2 deviation reaching 10%. Analysis confirms that the blue band is not the primary cause of the deviation; instead, the discrepancy arises from the inapplicability of the MODIS-specific parameters to Landsat-8/9 data. Sensitivity analysis reveals strong interactions among the four parameters of EVI, with the soil adjustment factor (L) being the most sensitive. Based on these findings, the study proposes and tests two parameter optimization strategies for Landsat-8/9 EVI: a site-specific cross-calibration method that reduces the mean absolute percentage bias (|PB|) between the MODIS and Landsat-8/9 EVI from 13% to nearly 0%, and a generalized parameter optimization using the Nelder-Mead algorithm, which reduces the average |PB| to 8.33%. Cross-validation indicates that the optimized parameters can reduce bias by approximately 32.5% in unseen areas from similar biomes. This study concludes that the direct cross-sensor application of MODIS EVI parameters introduces significant bias. We recommend sensor-specific parameter optimization and provide a transferable methodological framework to guide future efforts towards consistent multi-sensor vegetation monitoring.

</details>


### [77] [Correction](https://doi.org/10.1080/15481603.2026.2700880)
*Unknown Author*

Main category: GIScience & RS

TL;DR: 由于OpenAlex中未提供论文摘要，无法进行具体的论文分析。请提供完整的论文摘要或内容以供分析。


<details>
  <summary>Details</summary>
Motivation: 无法获取，因缺少摘要内容。

Method: 无法获取，因缺少摘要内容。

Result: 无法获取，因缺少摘要内容。

Conclusion: 无法获取，因缺少摘要内容。

与遥感交叉或改进方案: 交叉/改进可行性：0%。由于没有提供任何论文内容，无法进行交叉分析。请提供完整的论文摘要或全文以便进行评估。

Abstract (英文): No abstract available in OpenAlex.

</details>


### [78] [Analyzing dominant driving factors of ground deformation in mining area via multimodal feature fusion and explainable modeling](https://doi.org/10.1080/15481603.2026.2700121)
*Maoqi Liu,Marzieh Baes,Mahdi Motagh,Qian Song,Yao Sun,Zelong Guo,Tao Li,Sichun Long*

Main category: GIScience & RS

TL;DR: 本文提出多模态人工智能模型，融合静态（矿区、城市、断层、农田、地质）与动态（地表形变时间序列、地下水位）多源数据，系统研究德国莱茵兰煤田的地表变形。通过12种嵌套多模态子集、6种深度学习架构（CNN、GCN、Transformer）及特征融合/消融实验，发现多模态融合优于仅用形变数据，区域尺度上地质和地下水最重要，局部尺度上断层信息最关键。


<details>
  <summary>Details</summary>
Motivation: 自然资源开采区的地表变形由多种人为和水文地质因素共同作用，识别主要因素及其与变形的关系对减轻极端灾害至关重要，现有方法难以有效解耦多因素影响。

Method: 整合静态环境特征（矿区、城市、断层、农田、地质分布）与动态特征（形变时间序列、年地下水观测数据），构建12个嵌套多模态子集；采用CNN、GCN、Transformer等六种深度学习架构，通过渐进式特征融合和特征消融进行对比评估，并结合SHAP进行归因分析。

Result: 多模态融合持续提升预测精度；区域尺度上地质和地下水是最重要的预测因子；局部尺度上断层信息在高影响区成为最重要预测因子。

Conclusion: 区域背景控制与局部结构触发之间的区分，为评估矿区变形敏感性和减轻灾害提供了数据驱动基础。

与遥感交叉或改进方案: 交叉/改进可行性：95%。该框架可直接迁移至遥感矿区监测：(1) 将InSAR形变时间序列作为动态特征，融合遥感解译的地质、断层、土地利用等静态图层；(2) 利用遥感影像提取高分辨率土地利用/覆盖变化作为额外特征；(3) 将Transformer与CNN结合用于遥感时空数据建模；(4) 建议引入遥感降雨数据作为另一动态特征以增强水文响应建模。

Abstract (英文): Ground deformation in areas of natural resource extraction results from the interplay of multiple anthropogenic and hydrogeological factors. Identifying the main factors and their relationship to deformation is crucial for mitigating disasters when extreme events happen. Here, we show how multimodal artificial intelligence models can disentangle the effects of dynamic and static features related to deformation in the Rhineland coalfield in Germany. We systematically investigate the synergistic effects of multi-source data by integrating static environmental features, including mining areas, urban areas, fault lines, farmland, and geological distributions, with dynamic features, such as surface deformation time-series and annual groundwater-level observations, and organising them into 12 nested multimodal subsets. Using mainstream deep learning modules, including Convolutional Neural Networks, Graph Convolutional Networks, and Transformer, we build six distinct network architectures and evaluate them under comparable settings through progressive feature fusion and feature ablation. The results show that multimodal fusion consistently improves predictive accuracy compared to a model driven only by deformation data, indicating that static spatial distribution information enhances surface deformation prediction when incorporated as contextual background. Within the controlled feature-space design, feature ablation provides stable channel-level comparisons, whereas SHAP remains a useful complementary post hoc attribution method. At the regional scale, geology and groundwater emerge as the most important predictors of the overall deformation pattern, which is consistent with the interpretation that geological conditions shape the hydrogeological setting and the spatial expression of groundwater fluctuations. At the local scale, fault information becomes the most important predictor within spatially confined high-impact zones, because structural discontinuities concentrate deformation gradients and groundwater-related responses. This distinction between regional background control and local structural triggering provides a data-driven basis for assessing deformation susceptibility and mitigating hazards in mining areas.

</details>


<div id='RSE'></div>

# RSE [[Back]](#toc)

### [79] [A unified kernel-driven modeling framework adapted to continuous, discrete and row-planted canopies in the thermal infrared domain](https://doi.org/10.1016/j.rse.2026.115548)
*Biao Cao,Yingjie Wang,Jean‐Philippe Gastellu‐Etchegorry*

Main category: RSE

TL;DR: 本文针对行播作物和果园的热红外辐射角度效应，提出了一种考虑行结构特征的统一核驱动建模框架（包含各向同性核、基础形状核、热点核和热带核），并实例化了Kimes-RL-Ganis和Kimes-Chen-Ganis两个模型。通过三个DART模拟数据集和三个机载多角度实测数据集评估，新模型相比六种现有模型均方根误差降低42.5–84.9%，且在连续和离散冠层均表现良好。


<details>
  <summary>Details</summary>
Motivation: 现有核驱动模型仅适用于离散和连续冠层，未考虑行播作物的行结构特异性，导致热红外辐射角度校正精度受限。本文旨在开发一种能统一描述行播结构热辐射角度效应的核驱动模型框架。

Method: 1. 构建包含各向同性核、基础形状核、热点核和热带核的统一核驱动建模框架，其中基础形状核与热带核用于表征行结构的角度效应；2. 在该框架下实例化两个具体模型（Kimes-RL-Ganis和Kimes-Chen-Ganis）；3. 采用三个DART辐射传输模拟数据集和三个机载多角度实测数据集，与Vinnikov、RL、Vinnikov-RL、Vinnikov-Chen、LSF-RL和LSF-Chen六种现有模型进行对比验证。

Result: 三个模拟场景的RMSE从1.105–2.047 K降至0.265–0.349 K，降幅71.0–84.9%；三个实测场景的RMSE从1.316–2.227 K降至0.614–0.909 K，降幅42.5–61.2%。新模型在所有测试场景中均显著优于六种现有模型。

Conclusion: 所提框架能有效实现行播冠层热红外辐射的角度归一化，同时适用于连续和离散冠层，具有广泛的植被类型适应性。该框架还可推广至光学域行播场景的双向反射分布函数模拟。

与遥感交叉或改进方案: 交叉/改进可行性：95%。改进方案：1）可将该框架中的热带核概念引入遥感数据同化系统，用于改进基于多角度热红外数据的地表温度产品角度归一化；2）该框架可推广至光学遥感双向反射率（BRDF）建模，尤其适用于高分辨率卫星（如高分多模、WorldView等）对行播作物和果园的观测；3）未来可结合深度学习回归方法，自动学习行结构参数（如行间距、行向）与核系数之间的关系，实现无需先验知识的端到端角度校正。

Abstract (英文): To make better use of solar energy, many crops and orchards are planted in rows around the world. Accurate monitoring of the heat radiation status of row-planted crops and orchards is important for water management and yield forecasting. However, the radiation observed by a sensor is generally angle-dependent due to the complex architecture of the canopy, which must be normalized with respect to a reference direction (e.g., the nadir direction). Compared to physical models with numerous input parameters about the canopy structure and thermal property (e.g. canopy height, leaf area index, leaf emissivity and leaf temperature), semi-empirical kernel-driven models are much easier to be calibrated using only multi-angle sensor observations. However, all existing kernel-driven models have been developed for discrete and continuous canopies, without considering the specific architecture of row structure. To extend the fitting ability to row-planted canopies, a unified kernel-driven modeling framework in the thermal infrared domain has been developed in this study. It includes an isotropic kernel, a base shape kernel, a hotspot kernel and a hot belt kernel. Both base shape and hot belt kernels can represent the angular effect of row structure. Two instantiated models (i.e., Kimes-RL-Ganis and Kimes-Chen-Ganis) are proposed within this unified framework. Then, three DART (Discrete Anisotropic Radiative Transfer)-simulated datasets and three airborne-measured multi-angle datasets have been adopted to evaluate our two new models against six existing models (i.e., Vinnikov, RL, Vinnikov-RL, Vinnikov-Chen, LSF-RL and LSF-Chen). Results show RMSE reduction percentages as large as 71.0–84.9% for the three simulated scenes (1.322–1.445 K to 0.300–0.349 K, 1.105–1.263 K to 0.265–0.321 K, and 1.926–2.047 K to 0.269–0.291 K) and 42.5–61.2% for the three measured scenes (2.043–2.227 K to 0.78–0.792 K, 1.316–1.426 K to 0.614–0.649 K, and 1.580–1.654 K to 0.882–0.909 K). Given the demonstrated adaptability of these new models to both continuous and discrete canopies, they offer significant potential for advancing angular normalization of land surface temperature across diverse vegetation types in applications. This framework is well suited for the bi-directional reflectance distribution function simulation over row-planted scenes in the optical domain.

</details>


### [80] [Leveraging EMIT spectroscopy and machine learning to estimate soil texture and particle grain size in dust-producing regions](https://doi.org/10.1016/j.rse.2026.115557)
*Gregory S. Okin,Bo Zhou,Philip G. Brodrick,Francisco Ochoa,Marı́a Gonçalves Ageitos,R. N. Clark,M. Fischella,Carlos Pérez García‐Pando,Longlei Li,Natalie MAHOWALD,David R. Thompson,Robert O. Green*

Main category: RSE

TL;DR: 本研究利用EMIT成像光谱仪数据，通过多输出随机森林模型预测七种土壤粒径组分（粉粒、黏粒和五种砂粒），生成了全球0.1°分辨率的土壤质地图，用于支持EMIT量化矿物尘埃质量丰度的主要任务。中值粒径估计的MAE为30μm（R²=0.78），砂粒组分MAE<6%。


<details>
  <summary>Details</summary>
Motivation: EMIT任务需要中值粒径信息来估算矿物尘埃质量丰度，以改进对大气辐射强迫中矿物尘埃的理解。现有方法缺乏从成像光谱数据中获取多组分粒径分布的高效手段。

Method: 采用多输出随机森林模型，基于EMIT光谱反射率数据预测七种粒径类别的质量分数。为处理植被影响，将土壤光谱与绿色和非光合植被光谱线性混合后作为训练数据。采用五折交叉验证评估误差。

Result: 砂粒组分MAE<6%（R²=0.54-0.81），总砂粒MAE<12%（R²=0.78），粉粒MAE<4%（R²=0.66），黏粒MAE<7%（R²=0.81），中值粒径MAE=30μm（R²=0.78）。植被覆盖<30%时对估计无显著影响。

Conclusion: 成像光谱数据对估算地表粒径具有重要价值，生成的产品可用于EMIT任务中约束尘埃辐射强迫，并服务于全球尘埃建模等地球科学应用。未来需要更多配对土壤光谱-质地测量数据以进一步改进结果。

与遥感交叉或改进方案: 交叉/改进可行性：95%。改进方案：1）可引入多角度或偏振光谱信息增强对砂粒子组分的区分能力；2）可结合微波遥感（如Sentinel-1）的土壤水分信息来校正水分对光谱的影响；3）可引入深度学习（如CNN或Transformer）替代随机森林，利用光谱的连续特征提升粒径分类精度；4）植被混合策略可扩展至更多植被类型和覆盖度场景以适应中国复杂地表条件。

Abstract (英文): The primary mission of NASA's Earth Surface Mineral Dust Source Investigation (EMIT) is to improve our understanding of mineral dust in atmospheric radiative forcing using a novel set of spectroscopically-based mineral observations. In order to estimate mineral mass abundance, information on median surface grainsize is required. Knowledge of clay and silt fractions as well as the sub-components of the sand fractions is required for calculation of median grainsize. Therefore, this study aimed to produce a map of surface soil texture suitable for the intended application. Spectral reflectance data were incorporated into a multi-output Random Forest model to predict mass fraction for seven particle size classes: silt (TSI), clay (Clay), and five sand size classes: very coarse sand (S1, 1–2 mm diameter), coarse sand (S2, 1/2–1 mm), medium sand (S3, 1/4–1/2 mm), fine sand (S4, 1/8–1/4 mm), and very fine sand (S5, 1/16–1/8 mm diameter). To account for the presence of vegetation in EMIT spectral data, spectra of soils were mixed linearly with spectra of green and nonphotosynthetic vegetation in various combinations and amounts and the mixed spectra were used as training data. A five-fold cross-validation approach allowed us to estimate independent errors for each of the texture classes as well as three parameters estimated post hoc: total sand fraction (TSA, calculated as the sum of the five sand classes), mean grain size (GS mean ), and median grain size (GS median ). Sand fractions have estimated mean absolute errors (MAEs) <6% (R 2 = 0.54–0.81), while the error for TSA is <12% (R 2 = 0.78). MAE for TSI and Clay is <4% (R 2 = 0.66) and < 7% (R 2 = 0.81), respectively. Estimated MAE for GS median is 30 μm (R 2 = 0.78). For GS mean estimated MAE is 7.3 μm (R 2 = 0.73). Above 70% soil cover (below 30% vegetation cover), vegetation did not appear to influence estimates. A final model trained on all available data was used to produce estimates of soil texture class mass fractions for dust-producing areas worldwide at 0.1° resolution suitable for supporting estimates of mineral mass fraction from EMIT data. Despite drastically different estimation approaches, our results are broadly consistent with other global mineral datasets, although differences are apparent. Overall, our approach indicates the value of imaging spectrometer data for estimation of surface grain size for application to EMIT's primary mission of better constraining dust radiative forcing, though additional paired soil spectra-texture measurements would improve results. The products may also be useful for other applications in the Earth sciences, especially those related to global modeling of dust.

</details>


<div id='JAG'></div>

# JAG [[Back]](#toc)

### [81] [Simulating potential landward migration pattern of mangrove wetlands under the ‘two-way squeeze’ from sea level rise and human activities](https://doi.org/10.1016/j.jag.2026.105461)
*Zhaoxuan Ding,Bin Ai,Jiali Zeng,Yuchao Sun,Xiaoping Liu,Zhuokai Jian,Zhenlin Lai,Poyuan Ou*

Main category: JAG

TL;DR: 没有提供摘要内容。


<details>
  <summary>Details</summary>
Motivation: 无法从空白摘要中提取动机。

Method: 无方法描述。

Result: 无实验结果。

Conclusion: 无结论。

与遥感交叉或改进方案: 交叉/改进可行性：0%。由于未提供任何摘要内容，无法进行任何交叉分析或提出改进方案。

Abstract (英文): No abstract available in OpenAlex.

</details>


### [82] [Multi-source complementary feature self-integration network for geological lithology interpretation](https://doi.org/10.1016/j.jag.2026.105463)
*Ying Cao,Diming Wu,Y W Li,Qing Cheng*

Main category: JAG

TL;DR: 本文提出CFSI-Net，一种用于多源遥感影像地质岩性解译的统一框架，通过跨域特征融合模块（CDFEFM）缓解多源特征空间异质性，并引入频率-空间特征集成模块（FSFIM）和分割模块（SegM）解决边界模糊问题。在CUG-LithXZ数据集上，Geo-GZ区域OA达80.13%、mIoU达66.18%，Geo-LZ区域OA达72.49%、mIoU达50.26%，优于现有方法。


<details>
  <summary>Details</summary>
Motivation: 多源遥感影像中的地质岩性解译面临两大挑战：多源特征的空间异质性以及地质要素空间分布碎片化导致的边界模糊问题。现有方法难以有效融合多源互补信息并精确分割地质边界。

Method: 提出CFSI-Net统一框架，包含三个核心模块：(1) 跨域特征提取与融合模块（CDFEFM）——双分支结构，自适应提取并融合跨通道互补特征，减少冗余岩性信息；(2) 频率-空间特征集成模块（FSFIM）——从频域和空域捕获边界结构细节和空间上下文依赖；(3) 分割模块（SegM）——进一步细化跨通道空间依赖，丰富纹理细节，增强相邻要素判别能力。

Result: 在CUG-LithXZ数据集的Geo-GZ区域上，OA达80.13%、mIoU达66.18%；在Geo-LZ区域上，OA达72.49%、mIoU达50.26%，均优于现有最先进模型。

Conclusion: 频域与空域信息的协同利用能够为复杂地形下的地质岩性精确解译提供更丰富的细节信息，验证了多源特征融合与频率-空间联合建模的有效性。

与遥感交叉或改进方案: 交叉/改进可行性：95%。该论文本身就是遥感地质解译方向的研究，改进方案包括：(1) CDFEFM模块可扩展至更多模态（如高光谱+SAR+LiDAR）的遥感融合任务；(2) FSFIM的频域-空域联合建模思路可迁移至其他遥感分割任务（如建筑物提取、道路分割）；(3) 可引入注意力机制或Transformer以进一步增强长程上下文建模能力；(4) 可结合地质先验知识（如地层产状、构造线）作为约束条件提升解译精度。

Abstract (英文): Geological lithology interpretation in multi-source remote sensing imagery still faces challenges due to the spatial heterogeneity of multi-source features and the fragmented spatial distribution of geological elements. To address these issues, we propose the complementary feature self-integration network (CFSI-Net), a unified framework for multi-source feature fusion and lithology interpretation. To mitigate spatial heterogeneity in multi-source features, we developed the cross-domain feature extraction and fusion module (CDFEFM), a dual-branch structure that adaptively extracts and fuses complementary features across channels while reducing redundant lithology information. Furthermore, we introduced the frequency-spatial feature integration module (FSFIM) and a segmentation module (SegM) to alleviate the problem of boundary ambiguity in segmentation. FSFIM captures boundary structural details and spatial contextual dependencies from the frequency and spatial domains, whereas SegM further refines spatial dependencies across channels, enriching textural details and enhancing the discrimination of adjacent elements. Experimental results on the CUG-LithXZ dataset demonstrate that CFSI-Net consistently outperforms state-of-the-art models, achieving an OA of 80.13% and mIoU of 66.18% in Geo-GZ, and an OA of 72.49% and mIoU of 50.26% in Geo-LZ. These results confirm that the synergistic utilization of frequency and spatial information provides richer details for accurate geological lithology interpretation in complex terrains.

</details>


### [83] [Feasibility of space-borne remote sensing to detect features related to CO2 and N2O exchange of boreal agricultural fields](https://doi.org/10.1016/j.jag.2026.105455)
*Miia Salminen,Liisa Kulmala,Tuomas Laurila,Olli Nevalainen,Stephanie Gerin,Laura Heimsch,Henriikka Vekuri,Juval Cohen,Matias Takala,Kimmo Rautiainen,Narasinha Shurpali,Maarit Liimatainen,Jari Liski,Jouni Pulliainen*

Main category: JAG

TL;DR: 本研究探讨了星载微波卫星观测在表征芬兰北部农业区（包括矿质土壤和泥炭土壤的草地）二氧化碳交换过程中的适用性，利用微波辐射计获取的冻融、积雪积累和融雪信息解释日尺度的碳氮交换过程，并评估了合成孔径雷达数据在填补光学卫星叶面积指数时间序列缺失方面的潜力。结果表明，星载微波观测在评估北方农业区CO₂交换和N₂O爆发方面具有潜力，冻融和积雪信息可区分具有不同CO₂交换特征的季节性时期，Sentinel-1 SAR数据与GPP动态显著相关并能检测收割事件，支持全年农业监测。


<details>
  <summary>Details</summary>
Motivation: 北方农业区的CO₂和N₂O交换过程受冻融循环、积雪等季节性因素影响显著，但光学遥感在非生长季受云覆盖限制，亟需探索微波卫星（不受天气和光照影响）在全年碳氮交换监测中的适用性。

Method: 基于芬兰三个地区（两个矿质土壤草地、一个泥炭土壤草地）的观测数据，利用SMOS和SSMI/S微波辐射计提取冻融、积雪积累和融雪信息，结合Sentinel-1 C波段SAR后向散射数据，与地面CO₂交换（GPP、NEE等）和N₂O排放速率进行相关性分析，并评估SAR填补光学LAI时间序列空缺的能力。

Result: 1）星载微波辐射计获取的景观尺度冻融和积雪信息可区分具有不同CO₂交换特征的季节性时期，有助于约束非生长季的碳交换模型；2）Sentinel-1 SAR后向散射与GPP动态显著相关，并能检测收割事件；3）SAR数据可填补云覆盖导致的光学LAI时间序列空缺，提升卫星数据在田间管理监测中的实用性；4）微波观测对解释泥炭田N₂O爆发也具有潜力。

Conclusion: 星载微波观测（辐射计与SAR联合）在北方农业区全年碳氮交换监测中具有重要潜力，可弥补光学遥感在非生长季的不足，为提升农业温室气体排放估算提供新的数据支撑。

与遥感交叉或改进方案: 交叉/改进可行性：90%。1）将微波辐射计冻融/积雪算法迁移至中国北方农业区（如东北、内蒙古），结合高分系列SAR数据开展稻田/旱田CO₂和CH₄交换的全年监测；2）利用Sentinel-1 SAR后向散射时序数据反演作物LAI/GPP，与国产光学卫星（如高分一号、环境减灾二号）融合，解决多云地区LAI时间序列不连续问题；3）将SMOS/SMAP土壤冻融产品应用于青藏高原高寒草甸或北方湿地，评估冻融过程对碳通量（CO₂/CH₄）爆发排放的控制作用；4）构建SAR-辐射计联合的农业管理事件（收割、翻耕、灌溉）检测算法，支持国内农业碳排放清单的卫星验证。

Abstract (英文): We explored the applicability of microwave satellite observations to characterize the carbon dioxide (CO 2 ) exchange processes in boreal agricultural fields for three regions in Finland. The sites under investigation included two grassland fields on mineral soil and one on peat soil. We evaluated whether the space-borne microwave radiometer data-derived information on freeze–thaw, snow accumulation and snow melt obtained at a landscape scale is useful in explaining carbon and nitrogen exchange processes at daily temporal and field scale spatial resolutions. Specifically, we focused on the annual balance and seasonal behaviour of CO 2 exchange, and the feasibility of microwave observations in explaining the observed nitrous oxide (N 2 O) emission rates from a peatland field. Additionally, the feasibility of space-borne synthetic aperture radar (SAR) data to support observation time series of optical satellite data on a field block level was investigated for the growing season. The results indicate the potential of space-borne microwave observations to provide information useful in assessing CO 2 exchange and N 2 O bursts from boreal agricultural fields. The results show that seasonal changes due to climate and management, detectable on a landscape scale from space-borne microwave radiometry, significantly contribute to the annual balance of CO 2 exchange of boreal fields. The results also suggest that SAR data can potentially be used to fill cloud cover induced gaps in leaf area index (LAI) time series available from optical-range satellite imaging, improving the usefulness of satellite data in the monitoring of field management. Results show that landscape-scale freeze–thaw and snow cover information derived from space-borne SMOS and SSMI/S observations distinguishes seasonal periods with distinct CO 2 exchange characteristics, indicating the potential of coarse-resolution microwave radiometer data to constrain carbon exchange models of boreal agricultural fields outside the summertime growing season. Further, C-band Sentinel-1 SAR backscatter observations were found to correlate significantly with GPP dynamics and enable detection of harvesting events, suggesting that combined SAR and radiometer time series can support year-round agricultural monitoring.

</details>


### [84] [Geomorphology-constrained bathymetric inversion of turbid tidal channels using centerline-network priors and machine learning](https://doi.org/10.1016/j.jag.2026.105465)
*J Y Chen,Chao Gao,Yan Gu,Xiangyu Ou,Xiaoyi Cheng,Shibing Zhu,Ya Ping Wang*

Main category: JAG

TL;DR: 无摘要可用，无法进行分析。


<details>
  <summary>Details</summary>
Motivation: 无摘要可用，无法提取动机。

Method: 无摘要可用，无法提取方法。

Result: 无摘要可用，无法提取结果。

Conclusion: 无摘要可用，无法提取结论。

与遥感交叉或改进方案: 交叉/改进可行性：70%。无摘要可用，无法进行交叉分析。

Abstract (英文): No abstract available in OpenAlex.

</details>


### [85] [Spatiotemporal fusion of radar backscatter for enhanced monitoring of surface melt dynamics over Antarctic ice shelves](https://doi.org/10.1016/j.jag.2026.105448)
*Chengyu Pu,Lei Zheng,Jingan Wu,Qi Liang,Teng Li,Gang Li,Xiao Cheng*

Main category: JAG

TL;DR: 本文提出了一种名为SRBDF的多源C波段有源微波数据融合方法，将Sentinel-1的高空间分辨率与ASCAT的高时间分辨率结合，生成了2018–2021年南极Larsen C和George VI冰架每日高分辨率后向散射数据集，用于监测表面融化动态。融合数据在保持Sentinel-1空间细节的同时维持了ASCAT的日尺度观测频率，融化/冻结检测一致率达到0.90以上，并揭示了2019–2020年极端融化季。


<details>
  <summary>Details</summary>
Motivation: 雷达散射计能实现每日融化探测但分辨率粗，合成孔径雷达（SAR）空间细节好但时间覆盖有限。现有的时空融合方法仅适用于光学遥感，尚未成功应用于雷达后向散射数据。因此需要一种针对雷达后向散射的多源数据融合方法来平衡时空分辨率，实现冰架表面融化的高时空分辨率监测。

Method: SRBDF（Spatiotemporal Radar Backscatter Data Fusion）方法，融合Sentinel-1（高空间分辨率）和ASCAT（高时间分辨率）的C波段有源微波数据，生成高分辨率日尺度后向散射数据集。方法在Larsen C和George VI冰架的2018–2021融化季进行应用验证。

Result: 1）融合数据集保留了Sentinel-1的空间细节和ASCAT的日频率；2）融化/冻结检测与Sentinel-1和ASCAT的一致率接近或超过0.90；3）高分辨率日观测揭示了2019–2020年极端融化季，融化发生率比上一季高50%以上；4）Larsen C冰架融化由焚风驱动，George VI冰架融化由持续的低速北风带来的暖平流驱动。

Conclusion: SRBDF方法能有效捕捉南极冰架表面融化变异性，具有在极地地表过程研究中广泛应用的潜力。

与遥感交叉或改进方案: 交叉/改进可行性：95%。该方法的融合思路可推广至其他极地或高海拔区域的雷达数据融合（如格陵兰冰盖）；可尝试将此方法扩展到其他波段（L、X波段）或多极化SAR数据；改进方向包括引入深度学习模型（如STFGAN、ESTDF）替代传统融合策略以提升精度；还可结合光学遥感数据进行多模态融合，提升融化探测的鲁棒性。

Abstract (英文): Surface meltwater over the Antarctic Ice Sheet acts as a primary modulator of the polar energy budget, potentially triggering ice shelf collapse and exerting far-reaching impacts on the global climate system. Radar scatterometers enable daily melt detection but at coarse resolution, whereas Synthetic Aperture Radar (SAR) achieves fine spatial detail with limited temporal coverage. Spatiotemporal fusion methods present a viable solution to balance this trade-off, but these approaches are applied only to optical remote sensing and have not yet been successfully adapted to radar backscatter data. In this study, we propose the Spatiotemporal Radar Backscatter Data Fusion (SRBDF) method, a multi-source C-band active microwave data fusion technique that can integrate Sentinel-1 and Advanced Scatterometer (ASCAT) observations. SRBDF generates a high-resolution daily backscatter dataset for monitoring surface melt dynamics at the Larsen C and George VI Ice Shelves throughout 2018–2021 melt seasons. The fused dataset preserves spatial details from Sentinel-1, while maintaining the daily frequency of ASCAT. Across the Larsen C and George VI Ice Shelves, SRBDF melt/freeze detection shows overall consistency approaching or above 0.90 with Sentinel-1 and ASCAT. High-resolution daily observations reveal an extreme melt season during 2019–2020, when melt occurrence was more than 50 % above the previous season. Pronounced melt was observed on Larsen C and George VI ice shelves driven by foehn winds and sustained warm air advection from low-speed northerly winds, respectively. These results demonstrate the effectiveness of SRBDF in capturing Antarctic ice shelf surface melt variability and its potential for broader applications in polar surface process research.

</details>


### [86] [Mapping the extent, type and remnant areas of the coastal dunes of Australia and New Zealand by integrating multi-source data](https://doi.org/10.1016/j.jag.2026.105464)
*Zhijia Zheng,Guanli Li,Chenjun Wang,Xiuyuan Zhang,Noam Levin*

Main category: JAG

TL;DR: 本文提出了一种结合地质图、面向对象图像分析（OBIA）和随机森林分类器的大尺度海岸沙丘类型制图方法，在澳大利亚和新西兰生成了10米分辨率的六类海岸沙丘精细分类图，总体精度达90.68%，并分析了城市化与保护区的空间格局。


<details>
  <summary>Details</summary>
Motivation: 大尺度海岸沙丘类型制图对海岸管理至关重要，但由于沙丘空间异质性高且海岸环境光谱复杂，现有方法难以实现高精度大范围分类。

Method: (1) 利用地质图划定海岸沙丘空间范围，排除非沙丘区域；(2) 采用面向对象图像分析（OBIA）结合随机森林分类器，融合多源特征对六类海岸沙丘（活动沙丘、前丘、沙滩、半固定沙丘、固定沙丘和湿地/沙丘洼地）进行分类。

Result: 生成澳新地区27,564 km²的10米精细沙丘类型图，总体精度90.68%，固定沙丘精度最高（96.81%）；约498 km²沙丘转化为城市用地，59.44%的沙丘面积位于保护区内。

Conclusion: 该研究方法具有可复现性，适用于大尺度海岸沙丘类型精细制图，成果可为监测人为干预下的海岸沙丘系统状态与演化提供科学支撑。

与遥感交叉或改进方案: 交叉/改进可行性：95%。(1) 该OBIA+随机森林框架可直接迁移至中国、欧洲等海岸线复杂的区域，需调整地质图源和训练样本；(2) 可引入Sentinel-1 SAR数据（如VV/VH后向散射）增强不同沙丘类型（如干湿沙丘）的区分度；(3) 可结合ICESat-2或无人机LiDAR提取沙丘高程与坡度特征，提升地形起伏较大区域的前丘与活动沙丘分类精度；(4) 针对沙丘动态变化，可引入时间序列分析（如Landsat NDVI时序）区分固定/半固定沙丘的植被演替阶段。

Abstract (英文): Accurate large-scale mapping of coastal dune types is critical for coastal management but remains challenging due to the dunes’ high spatial heterogeneity and the spectral complexity in coastal environments. Therefore, this study aims to develop a coastal dune mapping approach and demonstrate its applicability across Australia and New Zealand. Firstly, we leveraged geological maps to delineate the spatial extent of coastal dune fields, effectively masking out non-dune areas. Secondly, we applied an object-based image analysis (OBIA), followed by a random forest classifier that integrated multi-source features to classify six coastal dune types within the delineated coastal dune fields, including active dunes, foredunes, sandy beaches, semi-stabilized dunes, stabilized dunes, and wetlands (dune slacks). Based on this approach, we generated a 10 m fine classification map of coastal dune types across Australia and New Zealand, covering a total area of 27,564 km 2 . Accuracy assessment yielded an overall classification accuracy of 90.68 %. Most categories achieved satisfactory performance, with stabilized dunes showing the highest accuracy (96.81 %). We also found that approximately 498 km 2 of coastal dune area has been converted to urban development land. However, this development pressure was mitigated by conservation efforts, as 15,781.80 km 2 (59.44 %) of the total coastal dune area was situated within protected areas. This study provides a replicable methodology for large-scale fine classification of coastal dune types. The resulting map offers scientific support for monitoring coastal dune systems state and evolution under human intervention.

</details>


### [87] [C2D2: Matching UAV images with cycle-consistency and channel-separation for joint detector and descriptor](https://doi.org/10.1016/j.jag.2026.105451)
*Jingru Li,Kun Sun,Wen Dai,Chang Tang,Kunqian Li,Wenhao Bao,Xiongwu Xiao,Deren Li*

Main category: JAG

TL;DR: C2D2是一种用于无人机影像的轻量级特征匹配网络，通过通道分离模块减少任务间干扰，并引入循环一致性损失增强全局几何约束，在多个基准上提升了检测器重复性和定位召回率。


<details>
  <summary>Details</summary>
Motivation: 现有联合学习关键点检测器和描述符的方法面临两个固有问题：关键点可重复性与描述符可靠性优化不同的视觉特性，且基于patch的训练损失施加局部性约束，限制了全局几何一致性。

Method: 提出C2D2网络，包含两个核心创新：1）通道分离模块（CSM），实现任务特定的通道关注，减少跨任务干扰；2）循环一致性损失，在局部patch之外施加全局几何约束，增强描述符鲁棒性且仅增加少量训练开销。

Result: 在HPatches上检测器可重复性提升4.4%，在Aachen Day-Night基准上定位召回率提升高达7.3%，模型仅含0.49M参数。

Conclusion: C2D2以极小的参数量实现了稀疏特征学习在遥感等 demanding 应用中的高效解决方案。

与遥感交叉或改进方案: 交叉/改进可行性：95%。该论文本身就是面向遥感（无人机影像）的特征匹配方法，可直接应用于遥感图像配准、三维重建等任务。改进方向：1）CSM可进一步扩展为可变形通道注意力机制以适应遥感影像中地物尺度剧烈变化；2）循环一致性损失可引入多尺度金字塔约束以处理大视角变化场景；3）可结合遥感影像的多光谱/高光谱通道特性，将CSM扩展为光谱-空间联合分离模块。

Abstract (英文): Accurate feature matching in Unmanned Aerial Vehicle (UAV) imagery is fundamental to earth observation applications. Although jointly learning keypoint detectors and descriptors has become standard practice, existing methods face inherent challenges: keypoint repeatability and descriptor reliability optimize for different visual properties, while patch-based training losses impose locality constraints that limit global geometric consistency. This paper presents C2D2, a network that addresses these limitations through two key innovations. First, a Channel Separation Module (CSM) reduces cross-task interference by enabling task-specific channel emphasis. Second, a cycle consistency loss enforces global geometric constraints beyond local patches, enhancing descriptor robustness with minimal additional training overhead. Extensive evaluation on public benchmarks and a challenging UAV dataset demonstrates improvements on most evaluated metrics: 4.4% higher detector repeatability on HPatches and up to 7.3% improvement in localization recall on the Aachen Day–Night benchmark. These results , achieved with only 0.49M parameters, establish C2D2 as an efficient solution within the sparse feature learning family for demanding remote sensing applications.

</details>


### [88] [Time-series framework integrating rainfall and satellite soil moisture for multi-scale urban flood risk assessment](https://doi.org/10.1016/j.jag.2026.105470)
*Sang Yeob Kim*

Main category: JAG

TL;DR: 由于OpenAlex中未提供该论文的摘要，无法进行分析。


<details>
  <summary>Details</summary>
Motivation: 无法获取摘要内容，无法分析动机。

Method: 无法获取摘要内容，无法分析方法。

Result: 无法获取摘要内容，无法分析结果。

Conclusion: 无法获取摘要内容，无法分析结论。

与遥感交叉或改进方案: 交叉/改进可行性：0%。由于缺乏摘要内容，无法进行任何方案分析。

Abstract (英文): No abstract available in OpenAlex.

</details>


### [89] [Referring Remote Sensing Image Segmentation method based on Scene-Aware Guided Network model](https://doi.org/10.1016/j.jag.2026.105457)
*Kai Tan,Hui Xia,Kaikai Tian,Hui Yang,Yanlan Wu,Peng Zhang,Yongchuang Wu*

Main category: JAG

TL;DR: 无法获取摘要内容，无法进行分析。


<details>
  <summary>Details</summary>
Motivation: OpenAlex中未提供摘要信息，无法推断研究动机。

Method: 缺乏摘要信息，无法描述研究方法。

Result: 缺乏摘要信息，无法总结研究结果。

Conclusion: 缺乏摘要信息，无法归纳结论。

与遥感交叉或改进方案: 交叉/改进可行性：70%。由于缺乏论文摘要和内容，无法评估与遥感的交叉可行性。

Abstract (英文): No abstract available in OpenAlex.

</details>


### [90] [A SIF-Constrained Noah-MP land surface model for simulating water use efficiency and drought Resilience: Application in a Semi-Arid Basin](https://doi.org/10.1016/j.jag.2026.105460)
*Tewekel Melese Gemechu,Haitao Zhang,J F Sun,Baozhang Chen*

Main category: JAG

TL;DR: 本文将太阳诱导叶绿素荧光(SIF)直接耦合到WRF-Hydro/Noah-MP的Ball-Berry气孔阻力方案中，以纠正陆地表面模型(LSM)在水分受限生态系统中高估蒸散发(ET)的结构性偏差。在埃塞俄比亚阿瓦什盆地的应用表明，SIF约束使年均ET降低36.1%，旱季蒸腾比例增加7.0%，水分利用效率在干旱条件下更稳健（下降5% vs. 对照13%），并首次建立了基于生理过程的农业生态区划。


<details>
  <summary>Details</summary>
Motivation: 陆地表面模型系统性地高估了水分受限生态系统的蒸散发，原因在于对植物生理水分胁迫的表征不足。现有模型缺乏直接耦合实测光合活动与气孔行为的机制，导致水资源预测不可靠。研究旨在通过引入SIF观测数据，直接从植物生理层面约束模型中的蒸腾过程。

Method: 从GOSIF v2产品中提取自校准SIF因子(0-1)，将其嵌入Noah-MP的Ball-Berry气孔阻力方案中，用于调节净光合速率，从而将观测到的光合活性与气孔阻力和蒸腾耦合。参数α=0.10经盆地校准。通过MODIS、GLEAM、GLDAS等多源参考产品进行交叉比较评估。采用无监督聚类进行农业生态区划。

Result: 年均ET从8.45降至5.40 mm day⁻¹（降幅36.1%，p<0.001）；旱季蒸腾占比增加7.0%（p<0.001）；干旱条件下WUE仅下降5%（对照13%）；干旱恢复力指数0.95（对照0.87）；月尺度GPP-ET相关性r=0.93；年尺度WUE-ET负相关r=-0.88；识别出4个功能生态区。

Conclusion: SIF约束的生理框架能有效纠正LSM的结构性偏差，产生更可信的生态水文预测，并为气候适应性土地和水资源规划提供动态、基于植物生理的目标。该方法具有可迁移性，仅依赖公开SIF产品和广泛使用的Noah-MP模型。

与遥感交叉或改进方案: 交叉/改进可行性：95%。本文即为遥感（SIF）与陆面过程模型（Noah-MP）交叉的典范。改进方向：(1) 可扩展至其他LSM模型（如CLM、JULES）以验证通用性；(2) 引入多源SIF数据（如TROPOMI、OCO-2）进行尺度对比和不确定性量化；(3) 将方法推广至其他水分受限流域（如黄河流域、地中海地区）；(4) 结合深度学习方法改进SIF因子的时空降尺度；(5) 将SIF约束的蒸腾分量与微波遥感土壤水分同化系统耦合，实现更完整的陆面状态估计。

Abstract (英文): Land surface models (LSMs) systematically overestimate evapotranspiration (ET) in water-limited ecosystems due to inadequate representation of plant physiological water stress, undermining reliable water resource projections. We introduce a novel, mechanistic integration of solar-induced fluorescence (SIF) directly into the Ball-Berry stomatal resistance scheme of WRF-Hydro/Noah-MP. In the absence of eddy covariance flux tower measurements within the basin, model evaluation is conducted through inter-comparison with multiple independent gridded reference products (MODIS, GLEAM, GLDAS), and results are interpreted as physically consistent shifts in model behaviour rather than validated improvements in absolute accuracy. The methodology is designed to be transferable, relying only on publicly available SIF products and the widely used Noah–MP model. We developed a self-calibrating SIF factor (0–1) from GOSIF v2 to modulate the net photosynthesis rate, thereby coupling observed photosynthetic activity to stomatal resistance and transpiration. This SIF-enhanced model was applied to the Awash Basin, Ethiopia (2000–2024), and evaluated through inter-comparison with MODIS, GLEAM, and GLDAS. SIF integration increases consistency with the multi–product ensemble: mean annual ET decreased by 36.1 % (from 8.45 to 5.40 mm day⁻ 1 , p < 0.001), thereby reducing the ET discrepancy relative to the ensemble of satellite–based reference products (with α = 0.10 calibrated for this basin). The transpiration fraction of ET increased most during the critical dry season (+7.0 %, p < 0.001), reflecting more realistic water-conservation behaviour. Consequently, Water Use Efficiency (WUE) increased under both normal and drought conditions, and the SIF-constrained Model showed substantially smaller WUE losses during drought than the Control (a 5 % decline versus a 13 % decline, respectively), consistent with a more water-conservative stomatal strategy that reduces transpiration by a greater proportion than photosynthesis under stress. This simulated response aligns with observed plant behaviour in water-limited ecosystems, yielding a drought resilience index of 0.95, compared with 0.87 for the Control. Monthly flux correlations confirmed strong carbon–water coupling (GPP-ET r = 0.93), while annual-scale analysis revealed a developing trade-off in which high-productivity years were least efficient (WUE-ET r = -0.88). Furthermore, unsupervised clustering of SIF-constrained WUE and resilience outputs yielded the first physiologically based agroecological zonation of the basin. We identified four distinct functional zones, including a productive but drought-vulnerable core (21,425 km 2 , WUE = 2.49 g C kg⁻ 1 H 2 O) and resilient but inefficient lowlands (8,175 km 2 , WUE = 0.05 g C kg⁻ 1 H 2 O). This scalable SIF-informed physiological constraint framework reduces a structural LSM bias, yielding more credible ecohydrological projections and providing dynamic, plant-informed targets for climate-resilient land and water planning.

</details>


### [91] [A two-stage machine learning framework for Chl-a reconstruction and short-term prediction in eutrophic lakes using GOCI observations](https://doi.org/10.1016/j.jag.2026.105468)
*Zehui Huang,Ronghua Ma,Xinhui Chen,Kun Xue,Minqi Hu,Haoze Liu*

Main category: JAG

TL;DR: 本文没有提供摘要，无法进行有效分析。


<details>
  <summary>Details</summary>
Motivation: 无法获取，因无摘要内容。

Method: 无法获取，因无摘要内容。

Result: 无法获取，因无摘要内容。

Conclusion: 无法获取，因无摘要内容。

与遥感交叉或改进方案: 交叉/改进可行性：0%。因无摘要内容，无法进行任何交叉分析。

Abstract (英文): No abstract available in OpenAlex.

</details>


<div id='cs.AI'></div>

# cs.AI [[Back]](#toc)

### [92] [Context Graphs for Proactive Enterprise Agents](https://arxiv.org/abs/2607.07721)
*Avinash Kumar*

Main category: cs.AI

TL;DR: 本文提出了一种基于上下文图（Context Graph）的主动式代理框架，通过增量检测引擎、主动性评分器和LLM驱动的表层模块，使企业AI代理从被动响应转变为主动推送相关信息，在三个企业场景中实现了Precision@5为0.83、误报率0.11，并将信息平均发现时间从47分钟降至30秒以下。


<details>
  <summary>Details</summary>
Motivation: 现有RAG和代理框架本质上是反应式的，需要等待人类查询才能行动，这限制了企业生产力的真正提升。本文旨在构建能够主动向工作者推送相关、可操作信息的系统，在用户提问之前就提供有价值的洞察。

Method: 提出上下文图（Context Graph）——一种实时关系数据结构，建模企业实体、关系及状态随时间的变化。在此基础上构建：1）增量检测引擎，持续监控状态变化；2）主动性评分器，根据紧迫性、相关性和角色匹配度对候选洞察进行排序；3）LLM驱动的表层模块，输出带解释的排名通知。使用NetworkX和Anthropic Claude API实现完整端到端系统。

Result: 在合同生命周期管理、工程事件响应和销售渠道维护三个企业场景中，Precision@5达到0.83，误报率0.11，平均信息发现时间从反应式基线的47分钟降至30秒以下。

Conclusion: 上下文图驱动的主动性框架能显著提升企业AI代理的效率，将被动查询等待转变为主动信息推送，大幅缩短关键信息的发现时间，同时保持高精度和低误报率。

与遥感交叉或改进方案: 交叉/改进可行性：85%。遥感领域同样存在大量被动查询问题（如用户需主动请求特定区域的变化检测结果）。可将上下文图思想应用于遥感时序分析：1）构建"地理实体关系图"，将地物类型（建筑、植被、水体等）及其时空关系建模为动态图结构；2）增量检测引擎可对应遥感变化检测算法，持续监控多时相影像中的状态变化；3）主动性评分器根据变化紧迫性（如洪水淹没速度）、区域相关性和用户角色（应急响应人员 vs 城市规划师）排序；4）LLM表层模块可生成带变化前后影像解释的主动推送通知，使用户无需手动查询即可获知关键地表变化。

Abstract (英文): Retrieval-Augmented Generation (RAG) and agentic frameworks have advanced enterprise AI considerably, yet agents remain fundamentally reactive: they wait for a human query before acting. This paper argues that genuine enterprise productivity gains require proactive agents: systems that surface relevant, actionable information to workers before they ask. We propose the Context Graph, a live relational data structure that models enterprise entities, their relationships, and state transitions over time. Built on this graph, we define a Delta Detection Engine that continuously monitors state changes, a Proactivity Scorer that ranks candidate insights by urgency, relevance, and persona-fit, and a Surfacing Layer powered by an LLM that delivers ranked notifications with grounded explanations. We formalize each component, derive a unified Proactivity Score function, and provide a complete end-to-end Python implementation using NetworkX and the Anthropic Claude API. Evaluation across three generic enterprise case studies (contract lifecycle management, engineering incident response, and sales pipeline hygiene) demonstrates that context-graph-driven proactivity achieves Precision@5 of 0.83, a false positive rate of 0.11, and reduces mean time to surface from 47 minutes (reactive baseline) to under 30 second.

</details>


### [93] [AI-integrated models for assessing agricultural resilience](https://arxiv.org/abs/2607.07759)
*Joshua R. Waite,Dana Golden,Brett Indelicato,Kevin Camp,Mojdeh Saadati,Shannon Regan,Patrick Schnable,Baskar Ganapathysubramanian,Carlos Messina,Suzanne Thornsbury,Soumik Sarkar*

Main category: cs.AI

TL;DR: 本文开发了一种AI驱动工具，整合经济学模型（GTAP）与生物物理模型（APSIM），通过自然语言查询与响应，帮助政策制定者和市场参与者分析农业供应链中的跨学科冲击影响。


<details>
  <summary>Details</summary>
Motivation: 农业供应链易受生物物理与经济系统耦合扰动的影响，现有方法难以跨学科综合分析冲击的连锁反应，亟需一种能够融合经济模型与生物物理模型并支持自然语言交互的工具。

Method: 构建AI驱动工具，将全球贸易分析模型（GTAP，经济学）与农业生产系统模拟器（APSIM，生物物理学）进行集成，支持用户通过自然语言提问并获取跨学科影响分析结果。

Result: 实现了经济模型与生物物理模型的耦合分析能力，能够评估供应链冲击的跨学科影响，并以自然语言形式返回查询结果，降低使用门槛。

Conclusion: 该AI工具有效弥合了经济学与生物物理学之间的跨学科鸿沟，为农业供应链韧性分析与政策制定提供了可交互的决策支持手段。

与遥感交叉或改进方案: 交叉/改进可行性：85%。该工具框架可扩展至遥感领域：将APSIM替换为遥感驱动的作物生长模型（如DSSAT、WOFOST）或接入遥感数据同化模块，利用卫星观测（如NDVI、LAI、土壤湿度）实时校准生物物理模拟；同时将GTAP扩展为包含空间显式土地利用与贸易流的经济模型。可构建“遥感-作物-经济”联动分析链，用于评估极端气候事件（如干旱、洪水）对区域及全球农业供应链的级联影响。

Abstract (英文): Agricultural supply chains are vulnerable to disruptions through linked biophysical and economic systems. We develop an AI-powered tool that integrates economic models (GTAP) with biophysical models (APSIM) to analyze supply chain shocks, enabling policymakers and market participants to assess cross-disciplinary impacts through queries and responses written in natural language.

</details>


### [94] [Adversarial Social Epistemology for Assemblies of Humans and Large Language Models](https://arxiv.org/abs/2607.07760)
*Mihnea C. Moldoveanu,Joel A. C. Baum*

Main category: cs.AI

TL;DR: 本文提出对抗性社会认识论（ASE），用于分析密集交互式传播环境中，行为者如何利用证言链、推理、机构认证和隐性信任的支架结构，通过扭曲、虚构或策略性模糊信息来谋取私利，并提供了审计和补救信任 breach 的机制。


<details>
  <summary>Details</summary>
Motivation: 现有概念（认识论气泡、回音室、 misinformation 扩散）不足以解释在支架化断言（scaffolded assertions）中，传播主体如何利用通常使断言值得信赖的承诺与权利来破坏信任。需要一种新的分析框架来描述这类系统性的信任颠覆机制。

Method: 采用社会认识论与推论主义语义学（inferentialist semantics）相结合的理论建构方法，提出对抗性社会认识论（ASE）框架，分析支架化公共通信中的信任颠覆机制，并设计审计与补救机制来应对推理性链条的可审计性被破坏的问题。

Result: 提出了ASE理论框架，提供了描述支架化断言中信任颠覆现象的语言，明确了行为者利用承诺与权利进行信息操控的机制，并建立了基于认识论网络的审计与补救信任 breach 的技术路线。

Conclusion: 对抗性社会认识论（ASE）为理解密集交互式传播环境中系统性的信任颠覆提供了更精确的分析工具，补充了现有认识论气泡和回音室理论的不足，为治理支架化公共通信中的信任危机提供了理论和实践路径。

与遥感交叉或改进方案: 交叉/改进可行性：85%。遥感领域依赖多层级数据链（卫星图像获取→地面真值验证→算法反演→产品分发），这一"支架化"过程与本文的支架化断言高度相似。具体交叉方案：(1) 将ASE框架应用于遥感数据产品信任审计，识别数据生产链中可能存在的扭曲（如算法偏差故意隐藏、验证数据选择性公开）；(2) 利用推论主义语义学建模遥感产品从原始影像到专题产品的推理链，设计可审计的数据来源标注与推理路径追踪系统；(3) 建立遥感数据信任 breach 的检测机制，检测数据供应商是否在数据处理流程中存在策略性信息遗漏或统计操控。

Abstract (英文): We outline an adversarial social epistemology (ASE) for densely interactive communicative landscapes in which public assertions are scaffolded by chains of testimony, inference, institutional certification, and tacit trust. In such landscapes, agents have incentives and affordances to distort, color, omit, fabricate, or strategically under-specify information for private, reputational, rhetorical, or material gains. We argue that these phenomena are not adequately captured by familiar descriptions of epistemic bubbles, echo chambers, or misinformation diffusion. What requires explanation is how communicative agents exploit the commitments and entitlements that normally make scaffolded assertions trustworthy. We provide language that delivers the requisite analysis, outline mechanisms that subvert trust in scaffolded public communications, and outline machinery for auditing and redressing trust breaches arising from subverting the auditability of inferential chains, drawing on epistemic networks, enriched with an inferentialist semantics for interpreting assertions.

</details>


### [95] [Aligning Clinical Needs and AI Capabilities: A Survey on LLMs for Medical Reasoning](https://arxiv.org/abs/2607.07761)
*Qi Peng,Jiatong Li,Sirui Huang,Yiyang Jiang,Kaisong Gong,Ronger Ding,Shijie Ye,Changmeng Zheng,Yi Cai,Xiaobo Yang,Jin Huang,Xiao-Yong Wei,Qing Li*

Main category: cs.AI

TL;DR: 本文综述了医学大语言模型（LLMs）在临床推理中的进展，提出了一种连接临床实践与计算方法的双视角分析框架：临床侧基于Miller金字塔建立五级能力体系，计算侧将演绎、归纳、溯因推理模式与医学任务对应。同时引入覆盖五级推理能力的基准数据集，评估了18个最新模型，发现医学专科模型在诊断任务中表现优异，而通用模型在决策支持和对话方面领先。最后讨论了数据局限、幻觉、接地性等开放挑战及未来方向。


<details>
  <summary>Details</summary>
Motivation: 现有医学LLM研究缺乏系统性的临床推理能力评估框架，未能有效桥接临床实践需求与计算方法，亟需一个多层级、多视角的分析体系来指导模型开发和评估。

Method: 采用双视角分析方法：临床侧基于Miller金字塔定义五级能力（知识回忆→动态病例管理）；计算侧将演绎、归纳、溯因三种推理模式与具体医学任务关联；构建覆盖五级能力的基准数据集，对18个SOTA模型进行系统性评估。

Result: 医学专科模型在诊断类任务中表现最佳，通用模型在决策支持和对话任务中领先；揭示了当前模型在不同推理层级上的能力差异和不足。

Conclusion: 医学LLMs在临床推理方面取得重要进展，但仍面临数据不足、幻觉、接地性等挑战，需要向更安全、可靠、可落地的工作流程系统发展。

与遥感交叉或改进方案: 交叉/改进可行性：85%。遥感领域可借鉴该文Miller金字塔分级评估框架，构建遥感影像解译的五级能力体系（如基础地物识别→场景理解→变化检测→异常分析→动态监测决策）。同时可将演绎推理（基于先验知识推理地物类型）、归纳推理（从影像特征总结模式）、溯因推理（根据变化结果反推驱动因素）与遥感任务对应，构建遥感LLM的基准评估数据集。

Abstract (英文): Large language models (LLMs) have emerged as important tools in healthcare, showing growing potential for clinical reasoning and patient care. This survey examines recent progress in medical LLMs, focusing on reasoning applications and requirements. We present a dual-view approach that connects clinical practice with computational methods. On the clinical side, we establish a five-level competency scheme following Miller's Pyramid, progressing from knowledge recall to dynamic case management. On the computational side, we link deductive, inductive, and abductive reasoning patterns to common medical goals and tasks. We also introduce a benchmark dataset spanning five levels of medical reasoning capability and report results on 18 state-of-the-art models, revealing that medical specialist models excel in diagnosis-centric tasks while general models lead in decision support and dialogue. We conclude by discussing current progress and open challenges, including data limitations, hallucination, and grounding issues, and outline directions toward safer, more reliable, and workflow-ready systems.

</details>


### [96] [Alignment Plausibility: A New Standard for Assuring AI in Healthcare](https://arxiv.org/abs/2607.07766)
*Gwydion Williams,Sara Zannone,Bilal A Mateen*

Main category: cs.AI

TL;DR: 本文提出了一种面向心理健康支持场景的大语言模型对齐框架，借鉴临床心理实践的规范体系，从价值规范、训练嵌入和部署监督三个层次构建"对齐合理性"（alignment plausibility）概念，以确保LLM在提供心理支持时能产生积极健康结果、避免伤害并最终造福患者。


<details>
  <summary>Details</summary>
Motivation: 当前LLM被广泛用于心理健康支持，但注意力经济驱动其偏向持续互动而非有效干预；开发者的安全措施主要应对急性可见伤害，而对依赖关系、边界侵蚀、扭曲信念放大等慢性风险关注不足。

Method: 提出三层对齐框架：1) 基于临床实践规范性承诺的明确价值规范；2) 将价值嵌入模型的训练机制；3) 部署中检测漂移和长期伤害的监督机制。引入"对齐合理性"概念（类比生物学合理性），作为AI健康领域的监管构造。

Result: 构建了一个结构化的对齐合理性论证框架，通过系统性的价值规范、训练嵌入和监督机制的一致性，来证明系统与积极健康结果对齐、能力范围内不会造成伤害并最终实现患者受益。

Conclusion: 对齐合理性提供了一种原则性的论证方式，用以信任AI系统在心理健康领域的对齐效果，为监管和开发者提供了评估心理健康AI安全性的结构化方法。

与遥感交叉或改进方案: 交叉/改进可行性：35%。本文提出的"对齐合理性"三层框架核心面向心理健康AI的伦理与安全评估，与遥感领域直接交集有限。但可借鉴其思维：遥感AI模型（如分类、检测）可引入"领域知识合理性"概念，即要求模型在物理先验（如光谱特性、时空一致性）层面可解释地对齐，并建立类似的三层机制——遥感领域知识规范化编码、模型训练嵌入该知识、部署中持续监测模型是否符合物理规律（如地物变化不应违反常识），从而提升遥感AI的可信度和可解释性。

Abstract (英文): Large language models (LLMs) have become significant providers of mental health support, yet they remain products of an attention economy whose operational and commercial targets favour sustained engagement over the friction that effective psychological support often requires. Developers' safety responses have been largely reactive, addressing the most visible and acute harms while subtler, longer-term patterns of risk (e.g., dependency, boundary erosion, the amplification of distorted beliefs) receive less attention. We contend that making LLMs structurally safe requires alignment organised at three levels that mirror how society assures the safety of human clinical practice: 1) explicit value specification grounded in the codified normative commitments of clinical practice; 2) training that embeds those values in the model; and 3) oversight that detects drift and longer-term harm during deployment, much as clinical supervision does for human practice. Organising alignment in this way yields a construct we call alignment plausibility - a structured demonstration that a system's values, training regime, and oversight mechanisms are together consistent with safe and positive outcomes. We propose alignment plausibility as a regulatory construct (by drawing analogy to the established construct of biological plausibility) for AI in health: a principled way to argue for, or against, trust that systems are aligned to positive health outcomes, will cause no harm even where capable of doing so, and will ultimately lead to patient benefit.

</details>


### [97] [Idiobionics: The Unification of Privacy and Intelligent Robotic Prostheses](https://arxiv.org/abs/2607.07775)
*Kwesi Afari Darfoor,Patrick M. Pilarski,Bailey Kacsmar*

Main category: cs.AI

TL;DR: 本文提出"idiobionics"（自体仿生学）这一新研究领域，聚焦于智能仿生肢体的隐私安全问题，通过分析潜在对抗攻击并列出开放研究问题，旨在推动可穿戴机器人系统的隐私保护研究。


<details>
  <summary>Details</summary>
Motivation: 智能仿生义肢在提升用户能力的同时，引入了可被恶意实体利用的威胁向量，侵犯用户隐私。为充分实现下一代仿生肢体的益处，必须直接理解和解决这些隐私风险及其对用户采用的障碍。

Method: 提出idiobionics概念框架，将其植根于相关文献，通过提供潜在对抗攻击的初步证据进行讨论，最后整理出一份与可穿戴机器人及其他人类面对自主系统相关的研究问题清单。

Result: 定义了idiobionics这一新研究领域，展示了潜在对抗攻击的初步证据，列出了开放研究问题清单。

Conclusion: idiobionics研究将有助于解锁机器人假肢及相关仿生设备的全部潜力，为智能仿生系统的隐私安全提供系统性的研究方向。

与遥感交叉或改进方案: 交叉/改进可行性：65%。遥感系统同样面临传感器数据泄露和对抗攻击风险（如卫星图像被篡改、用户位置隐私暴露）。可借鉴idiobionics的隐私威胁建模框架，建立遥感系统的隐私安全评估体系；同时将仿生肢体的感知-控制-隐私协同设计理念迁移至遥感数据处理链路中，开发对抗性攻击检测与防御机制。

Abstract (英文): The human body is at the center of a growing family of technologies designed to tightly and persistently couple biological and digital systems. Robotic prostheses are a representative example of this tight coupling. Also referred to as bionic limbs, robotic prostheses are devices that support people who have lost limbs in pursuing daily life activities such as walking and grasping objects. Bionic limbs are now perceptive and responsive owing to their integration with advanced sensors and artificial intelligence-based control approaches. Consequently, such robotic prostheses can now be viewed as semiautonomous wearable robotic systems that can co-adapt with their users. However, the same sensing and control advancements that increase the capability of robotic prostheses also introduce threat vectors that could be exploited by malicious entities to violate the privacy of users. To fully realize the benefits of next-generation bionic limbs, we maintain it is important to directly understand and address these privacy risks and the barriers they might present to user adoption. This paper therefore introduces a new line of inquiry we term idiobionics to holistically investigate issues at the intersection of privacy and intelligent bionic limbs. As the main contribution of this paper, we define idiobionics, ground it in related literature, and provide preliminary evidence showing and discussing potential adversarial attacks that could exploit intelligent bionic limb designs. We then contribute a curated list of open research questions within idiobionics that are relevant to researchers in wearable robotics and other human-facing autonomous systems. We expect that idiobionics research will help unlock the full potential of robotic prostheses and related bionic devices.

</details>


### [98] [Infinity-Parser2 Technical Report](https://arxiv.org/abs/2607.07836)
*Zuming Huang,Jun Huang,Kexuan Ren,Baode Wang,Weizhen Li,Jianming Feng,Yu Wang,Yichen Yao,Shijun Lin,Yige Tang,Cheng Peng,Weidi Xu,Wei Chu,Yinghui Xu,Yuan Qi*

Main category: cs.AI

TL;DR: 本文提出Infinity-Parser2，一种结合可控数据合成管道和多任务强化学习的大规模多模态文档解析模型。主要贡献包括：构建500万样本的双语文档解析数据集Infinity-Doc2-5M，设计八目标联合强化学习奖励系统，以及发布面向低延迟和高精度的两个模型变体（Flash和Pro），其中Pro版本在olmOCR-Bench和ParseBench上达到SOTA。


<details>
  <summary>Details</summary>
Motivation: 现有文档解析任务面临忠实标注语料库持续稀缺的问题，缺乏大规模、高质量、多样化的标注数据以及统一的端到端解析框架。

Method: 1) 构建可扩展的合成引擎，包含可控渲染框架和迭代精炼循环，生成500万样本的双语数据集；2) 设计可验证的多任务奖励系统，在8个训练目标（文档解析、布局分析、表格解析、数学公式解析、图表解析、化学式解析、文档VQA和通用多模态理解）上进行联合强化学习；3) 发布两个变体：Flash（低延迟优化，吞吐量提升3.68倍）和Pro（高精度优化）。

Result: Infinity-Parser2-Pro在olmOCR-Bench上达到87.6%，在ParseBench上达到74.3%，超越DeepSeek-OCR-2、PaddleOCR-VL-1.5和MinerU2.5，并在图表、化学式和文档VQA上表现出强泛化能力。Flash变体相较Infinity-Parser-7B实现3.68倍吞吐量提升。

Conclusion: 通过可控数据合成与多任务强化学习的有机结合，Infinity-Parser2有效解决了文档解析中数据稀缺和统一优化的问题，在多个基准测试上达到了最先进水平，并提供了面向不同应用场景的模型变体。

与遥感交叉或改进方案: 交叉/改进可行性：85%。遥感领域存在大量异构文档（如卫星影像标注报告、地理信息表格、遥感化学分析报告等），可直接应用Infinity-Parser2-Pro进行端到端解析。改进方案：1) 利用其数据合成管道生成遥感专用文档数据集（如遥感影像标注格式、地理坐标表格、光谱分析报告等）；2) 在其多任务奖励系统中增加遥感文档解析任务（如遥感影像元数据提取、地理空间布局分析），实现领域自适应强化学习。

Abstract (英文): We present Infinity-Parser2, a large multimodal model that couples a controllable data-synthesis pipeline with multi-task reinforcement learning for end-to-end document parsing, addressing the persistent scarcity of faithfully annotated parsing corpora. Our contributions are threefold. First, we build a scalable synthesis engine, pairing a controllable rendering framework with an iterative refinement loop, and use it to construct and open-source Infinity-Doc2-5M: a 5-million-sample bilingual (Chinese/English) corpus spanning diverse document types, annotated with element bounding boxes, canonical content forms (Markdown, HTML, LaTeX, SMILES, structured charts), and full-page reading order. Second, we introduce a verifiable, multi-task reward system that enables Joint Reinforcement Learning across eight co-trained objectives (document parsing, layout analysis, table parsing, math formula parsing, chart parsing, chemical formula parsing, document VQA, and general multimodal understanding), unifying perception, structure, and reasoning in a single optimization signal. Third, we release two variants under a shared architecture: Infinity-Parser2-Flash, optimized for low-latency inference with a $3.68\times$ throughput gain over Infinity-Parser-7B, and Infinity-Parser2-Pro, engineered for precision-critical settings. Infinity-Parser2-Pro reaches state-of-the-art 87.6% on olmOCR-Bench and 74.3% on ParseBench, surpassing DeepSeek-OCR-2, PaddleOCR-VL-1.5, and MinerU2.5, with strong generalization to charts, chemical formulas, and document VQA.

</details>


### [99] [VectorizationLLM: Smart Vectorization Based AI Assistant](https://arxiv.org/abs/2607.07846)
*Ryan Duke*

Main category: cs.AI

TL;DR: VectorizationLLM是基于Google开源LLM构建的专业教学模型，旨在辅助学生学习MATLAB中的向量化、傅里叶分析、微分方程等计算分析课程内容，采用RAG知识库和系统提示架构，不直接提供答案而是提供概念解释和示例。


<details>
  <summary>Details</summary>
Motivation: 为CTEC 247课程学生提供一个智能教学助手，帮助学生理解复杂的计算分析概念，同时避免直接给出答案导致的学习依赖。

Method: 基于Google开源权重LLM微调，结合RAG（检索增强生成）知识库和系统提示架构，支持代码、文本和图像示例的多模态响应。

Result: 构建了一个专门的教学型LLM，能够提供向量化、时间/波向量分析、分段函数、傅里叶分析和微分方程等概念的详细解释及课堂笔记示例。

Conclusion: VectorizationLLM展示了专用LLM在工程教育领域，尤其是计算分析课程中的辅助教学潜力，通过RAG和系统提示设计平衡了教学帮助与学习自主性。

与遥感交叉或改进方案: 交叉/改进可行性：75%。遥感领域广泛涉及信号处理（如时间序列分析、傅里叶变换）和微分方程建模（如大气/海洋动力学），可将VectorizationLLM的课程内容迁移至遥感数据分析教学场景，例如：1）利用向量化方法处理遥感影像的多波段数据；2）用傅里叶分析进行遥感图像频谱分析与滤波；3）结合分段函数建模遥感中的非线性现象（如植被指数阈值分割）；4）微分方程用于遥感中的物理过程建模（如辐射传输方程）。需扩展RAG知识库以包含遥感专用MATLAB工具箱（如Image Processing Toolbox、Mapping Toolbox）。

Abstract (英文): VectorizationLLM is a specialized Large Language Model based on Google open-weight LLMs. The model is designed to assist students to learn smart vectorization, time/wave vector analysis, piecewise functions, Fourier analysis, and differential equations in MATLAB. The course application is CTEC 247: Applied Computational Analysis II by the Department of Electrical & Computer Engineering Technology at New York Institute of Technology Old Westbury. The LLM model is designed to be an instructive assistant, providing detailed explanations of concepts with examples from in-class notes without providing direct answers to questions. The model is designed with a RAG (Retrieval Augmented Generation) knowledge base and system prompt architecture. Examples in both code, text, and images are provided in the LLM responses.

</details>


### [100] [A Graph Neural Network Model for Real-Time Gesture Recognition Based on sEMG Signals](https://arxiv.org/abs/2607.07850)
*Pragatheeswaran Vipulanandan,Kamal Premaratne,Manohar Murthi*

Main category: cs.AI

TL;DR: 本文提出了一种基于图网络的表面肌电信号（sEMG）表示方法，利用图神经网络（GNN）进行实时手势识别，在8电极Myoband采集的8位受试者数据上取得了99%的分类准确率，平均处理时间为48ms，优于现有技术。


<details>
  <summary>Details</summary>
Motivation: 为了实现高级假手和增强现实的无缝控制，需要准确且即时的手势识别。传统sEMG处理方法在实时性和精度上存在不足，亟需一种能更好表征前臂肌肉激活模式的新方法。

Method: 首先将sEMG信号构建为包含前臂肌肉激活模式信息的图网络，然后基于图神经网络（GNN）设计机器学习算法进行手势识别。使用Myoband（8电极）采集8位健康受试者的sEMG数据，在M1 Pro CPU上评估图构建和预测的时间开销。

Result: 平均分类准确率99%，超过现有最先进方法；图构建和预测的平均时间为48ms（M1 Pro CPU），满足实时应用需求。

Conclusion: 基于图网络的sEMG表示方法结合GNN，在手势识别任务中实现了高精度和低延迟，是实时控制假肢和增强现实设备的有效方案。

与遥感交叉或改进方案: 交叉/改进可行性：85%。具体方案：遥感领域可借鉴该方法处理多通道传感器数据（如高光谱成像中相邻波段相关性建模），将各波段/像素视为图节点，利用图神经网络捕捉空间-光谱联合特征，实现地物分类或变化检测的实时推理。此外，遥感时序分析中可将不同时相影像构建为时空图，类比本文的sEMG图网络表示，提升变化检测精度。

Abstract (英文): For seemless control of advanced hand prostheses and augmented reality, accurate and immediate hand gestures recognition is essential. Surface electromyography (sEMG) signals obtained from the forearm are commonly employed for this purpose. In this paper, we present a novel approach for sEMG representation that utilizes graph networks which contain information about muscle activation patterns in the forearm. Based on these graph networks, we have developed a machine learning algorithm capable of real-time hand gesture recognition using a graph neural network. The algorithm's performance was evaluated using sEMG signals acquired from myoband, which has 8 electrodes placed around the forearm, involving 8 healthy subjects. The proposed method demonstrated an average classification accuracy of 99\%, surpassing the performance of state-of-the-art techniques. The average time for both graph construction and prediction stood at 48ms utilizing a M1 pro CPU, rendering the approach well-suited for real-time applications.

</details>


### [101] [Agentic AI and Retrieval-Augmented Models in Straight-Through Underwriting](https://arxiv.org/abs/2607.07858)
*Robert Richardson,Josh Meyers,Brian Hartman,David Sandberg*

Main category: cs.AI

TL;DR: 本文探讨了以大型语言模型（LLM）、检索增强生成（RAG）和多智能体“代理”系统为代表的AI架构如何支持精算实务中的透明度、可审计性和人在回路治理。通过构建一个针对小型商业企业保单（BOP）的承保代理AI框架，在合成实验环境中比较了三种管道：单LLM基线、朴素RAG系统和多智能体“Agentic RAG”管道。结果表明，Agentic RAG在整体表现上最优，尤其在多步骤和缺失信息场景下，结构化检索和反思机制有助于避免不合理的直通式决策。


<details>
  <summary>Details</summary>
Motivation: 精算实务正面临AI转型，需要处理非结构化文档、异构数据源和受监管的决策流程。现有方法在设计空间中缺乏对透明度、可审计性和人在回路治理的深入探讨，尤其是针对直通式决策流程的系统性研究不足。

Method: 构建合成但逼真的实验环境，设计并比较三种承保管道：(i) 单LLM基线、(ii) 朴素RAG系统和(iii) 多智能体Agentic RAG管道。后者结合了定向检索、第三方数据校验和显式多步规则评估。

Result: Agentic RAG系统整体表现最佳，尤其在多步骤和缺失信息场景中获得最大增益。结构化检索和反思机制帮助模型避免做出缺乏依据的直通式决策。

Conclusion: 多智能体Agentic RAG架构能有效支持精算实务中的透明、可审计和人在回路治理需求，为直通式决策流程提供了可行的技术方案。

与遥感交叉或改进方案: 交叉/改进可行性：85%。该论文的多智能体Agentic RAG框架可迁移至遥感决策支持领域。具体方案：1) 将单LLM基线替换为遥感专用预训练模型（如SpectralGPT）；2) 朴素RAG系统可应用于遥感影像-文本跨模态检索，辅助灾害评估中的法规查询；3) Agentic RAG可设计为多步遥感分析管道，如“区域检测→特征提取→变化检测→报告生成”，每步引入规则校验和第三方数据（如土地覆盖图、气象数据）验证；4) 缺失信息场景对应遥感中的云遮挡或数据缺失，反思机制可用于自适应补充观测或插值策略。

Abstract (英文): Artificial intelligence (AI) is beginning to reshape actuarial practice, particularly in domains that require reasoning over unstructured documents, heterogeneous data sources, and regulated decision workflows. Actuaries now face a design space that ranges from traditional rule-based automation to large language models (LLMs), retrieval-augmented generation (RAG), and multi-agent ``agentic'' systems that plan, retrieve, call tools, and reflect. This paper examines how these emerging architectures can support actuarial priorities such as transparency, auditability, and human-in-the-loop governance, with a focus on straight-through decision processes. To make these ideas concrete, we develop and analyze an agentic AI framework for straight-through underwriting of small commercial Business Owner Policies (BOPs). We construct a synthetic but realistic experimental environment and compare three underwriting pipelines: (i) a single-LLM baseline, (ii) a naive RAG system, and (iii) a multi-agent ``Agentic RAG'' pipeline that combines targeted retrieval, third-party data checks, and explicit multi-step rule evaluation. The agentic system performs best overall, with the largest gains in multi-step and missing-information scenarios, where structured retrieval and reflection help the model avoid unsupported straight-through decisions.

</details>


### [102] [Feedback Manipulation Regularization: Enabling Offline Agent Alignment for Imitation Learning](https://arxiv.org/abs/2607.07859)
*Benjamin Poole,Minwoo Lee*

Main category: cs.AI

TL;DR: 本文提出反馈操作正则化（FMR），一种算法无关的方法，利用评估反馈作为纠正信号来改进模仿学习策略的对齐性，在Safety Gymnasium环境中实现了高达98%的对齐失误降低。


<details>
  <summary>Details</summary>
Motivation: 现有对齐方法主要采用多阶段流水线方式处理人类演示和反馈信号，且多针对上下文赌博机框架下的语言生成设计，缺乏在完全序列决策环境中利用这些互补信号进行单阶段离线训练的研究。

Method: 提出反馈操作正则化（FMR），一种算法无关的方法，将评估反馈作为纠正信号来改进模仿学习策略的对齐性，并在Safety Gymnasium环境中进行对齐评估测试。

Result: 在Safety Gymnasium环境中，FMR使多种模仿学习算法的对齐失误率降低高达98%，且在有限数据场景下（包括稀缺对齐和噪声演示）仍保持鲁棒性。

Conclusion: FMR能够有效地利用评估反馈信号改进模仿学习策略的对齐性，在序列决策环境中实现了显著的性能提升，并且在不同数据条件下保持稳定。

与遥感交叉或改进方案: 交叉/改进可行性：85%。该方法可与遥感领域的决策任务结合，例如利用专家标注的遥感解译结果作为演示，结合领域专家对解译结果的评估反馈（如分类正确性评分）作为纠正信号，通过FMR框架提升遥感图像智能解译或自主遥感任务规划中的策略对齐性，减少误判和违规行为。

Abstract (英文): Reinforcement learning (RL) research has increasingly shifted focus towards alignment, ensuring agents learn behaviors adhering to human values. While human demonstrations and feedback have proven crucial for alignment, existing approaches predominantly combine these signals using multi-stage pipelines designed for the contextual bandit framing of language generation. Yet little work explores how these complementary inputs can serve as a richer, interconnected signal for single-stage offline training in fully sequential decision-making environments. We propose Feedback Manipulation Regularization (FMR), an algorithm-agnostic method that harnesses evaluative feedback as a corrective signal to improve the alignment of imitation learning policies. We adapt Safety Gymnasium environments to be a principled testbed for alignment evaluation, demonstrating improved aptitude and up to a 98\% reduction in misalignment across a range of imitation learning algorithms. FMR remains robust in limited data regimes, even when learning from scarce aligned and uninformative noisy demonstrations.

</details>


### [103] [Nigeria Machinery: A Low-Resource Industrial Dataset with a Domain-Grounded Reasoning Layer](https://arxiv.org/abs/2607.07883)
*Gospel Bassey,Vincent Fakiyesi*

Main category: cs.AI

TL;DR: 该论文发布了尼日利亚机械使用与故障数据集（89条记录、28个指标，覆盖2006-2025年制造业与油气行业），并提出一种从稀疏数值数据构建链式推理（CoT）示例的方法，最终生成94行包含提示、补全和推理轨迹的数据。论文强调了避免语言模型生成脱离领域真实性的提示问题，并将领域级提示占比从1/78提升至94/94，检索答案完全匹配源数据（84/84）。数据集以CC-BY-4.0协议开源，但明确其为参考和种子数据集，而非大规模训练集。


<details>
  <summary>Details</summary>
Motivation: 非洲经济体工业机械领域缺乏公开、模型就绪的数据，导致难以进行定量分析或在该场景下训练语言模型执行数值任务。

Method: 1) 构建尼日利亚机械使用与故障数据集，包含89条机器级记录、28个指标，涵盖2006-2025年尼日利亚制造业和油气行业，每条记录标注公开来源并附带编码表解码；2) 提出从稀疏数值数据构建链式推理（CoT）示例的方法，生成94行提示-补全-推理轨迹数据；3) 设计领域真实性约束策略，确保提示不仅匹配数值还关联真实领域上下文。

Result: 生成了94行含提示、补全和推理轨迹的数据，领域级提示占比从1/78提升至94/94，检索答案与源数据完全匹配（84/84）。

Conclusion: 该数据集作为参考和种子数据集，适用于小规模场景验证与基准构建，但受限于记录数量（89条）和17个仅含单次观测的指标，不适合作为大规模训练集；多数推理行为为检索而非多步计算。

与遥感交叉或改进方案: 交叉/改进可行性：75%。具体方案：1) 该框架可直接迁移至非洲地区遥感数据稀疏场景（如土地覆盖、农业监测），构建类似的小规模高可信度参考数据集；2) CoT推理构建方法可用于遥感数值反演任务（如植被指数→产量估计），从稀疏实地观测数据生成推理轨迹；3) 领域真实性约束策略对遥感产品验证尤为重要，可解决遥感AI模型中"数值匹配但语义无关"的常见问题；4) 局限性同样适用：遥感领域同样面临非洲等区域观测稀疏问题，此方法更适合作为验证基准而非大规模训练集。

Abstract (英文): There is relatively little, public, and model-ready data on industrial machinery for African economies. This makes it hard to do quantitative analysis or to train language models on numeric tasks grounded in that setting. We release two things to help with part of this problem. The first is the Nigeria Machinery Usage and Failures Dataset: 89 machine-level records across 28 indicators, covering Nigeria's manufacturing and oil and gas sectors from 2006 to 2025. Every record names a public source and is decoded by a codebook. The second is a method for building chain-of-thought (CoT) reasoning examples from these sparse numeric values. The result is 94 prompt, completion, and reasoning-trace rows. In every row, the prompt names the real indicator, subsector, year, and source of the record it comes from. The data adaptation work was carried out by Adaption Labs. Along the way we describe a problem that is common when language models are used to build datasets. The prompts can match the real numbers while saying nothing about the real domain. We show that fixing this raises the share of domain-grounded prompts from 1 out of 78 in an earlier release to 94 out of 94, and that every retrieval answer now matches its source value (84 out of 84). We release the data, the reasoning layer, and a per-row provenance file under CC-BY-4.0. We are clear about the limits. With 89 records and 17 indicators that have only one observation, this is a reference and seed dataset, not a large training set. Most reasoning rows are retrieval rather than multi-step computation.

</details>


### [104] [Persona Cartography: Charting Language Model Personality Traits in Weight Space](https://arxiv.org/abs/2607.07916)
*Luke Baines,Anton Gonzalvez Hawthorne,Mariia Koroliuk,Irakli Shalibashvili,Clément Dumas,Konstantinos Voudouris,David Demitri Africa*

Main category: cs.AI

TL;DR: 本文提出将大语言模型的行为模式（人设）视为OCEAN人格框架下的特质空间位置，通过训练低秩适配器（LoRA）来放大或抑制特定人格特质，实现对模型行为的可解释控制，并验证了该方法在多个模型上的单调性、可加性和安全性影响。


<details>
  <summary>Details</summary>
Motivation: 大语言模型表现出重复的行为模式（人设），这些模式影响模型的泛化能力和安全性，但现有工具缺乏可靠的方法来解构、测量和控制这些行为模式。

Method: 1）将人设视为OCEAN（开放性、尽责性、外向性、宜人性、神经质）五大人格特质空间中的位置；2）训练低秩适配器（LoRA）以放大或抑制特定人格特质；3）使用LLM裁判器（经人工验证面板校准）、特质特异性多选题基准和标准能力评估来评估效果；4）在三个模型家族的6个模型（4B-32B）上进行验证；5）提出无监督心理测量管线，从模型生成中恢复四个可解释的行为因子（语气、主动性、说教性、认知谨慎）。

Result: 每个适配器对目标特质的移动大致随规模单调变化；多个适配器可近似加性地组合以构建混合人设；在适中规模下保持能力基准性能；沿神经质和宜人性轴移动分别影响挫败感和谄媚行为。

Conclusion: 人设控制可被视为在权重空间中学习、缩放和组合特质，为人格测量、模型编辑和安全性之间搭建了桥梁。

与遥感交叉或改进方案: 交叉/改进可行性：75%。具体方案：1）将OCEAN人格控制框架应用于遥感大模型（如GeoGPT、RemoteCLIP），通过调整模型"开放性"来控制其对新地物类别的泛化偏好，通过调整"尽责性"控制遥感解译的精细度与保守性；2）利用无监督心理测量管线对遥感模型在场景分类、变化检测等任务上的输出行为进行因子分解，识别模型的"误报倾向"、"细节忽视"等系统性偏差；3）将LoRA特质适配器用于遥感模型的风格迁移，如调整"宜人性"控制分割结果的平滑程度，调整"神经质"控制异常检测的敏感度阈值。

Abstract (英文): Large language models exhibit recurring behavioural patterns -- personas -- that shape generalisation and safety, but we lack reliable tools for decomposing, measuring, and controlling them. Our central insight is to treat personas as positions in a space of behavioural traits, using the OCEAN framework to describe model personas in terms of Openness, Conscientiousness, Extraversion, Agreeableness, and Neuroticism. We train low-rank adapters to amplify or suppress individual traits, and evaluate their effects using an LLM-judge calibrated against a human-validated panel, trait-specific multiple-choice benchmarks, and standard capability evaluations. Across six models from three families (4B-32B), we find that each adapter moves its target trait largely monotonically with scale, combines approximately additively with other adapters to construct mixed personas, and preserves performance on capability benchmarks at moderate scales. We further show that the induced trait axes affect safety-relevant behaviour in downstream evaluations: for example, moving along neuroticism and agreeableness axes affects frustration and sycophancy respectively. We also introduce an unsupervised psychometric pipeline that recovers four interpretable behavioural factors (tone, initiative, didacticism, epistemic caution) from model rollouts. Persona control can then be considered in terms of learning, scaling, and composing traits in weight space, providing a bridge between personality measurement, model editing, and safety.

</details>


### [105] [Evaluating the Effect of Frame Rate in Sequence-Based Classification of Autism-Related Self-Stimulatory Hand Idiosyncrasies](https://arxiv.org/abs/2607.07957)
*Raunak Mondal,Peter Washington*

Main category: cs.AI

TL;DR: 本文针对自闭症谱系障碍(ASD)患者自我刺激行为的视频自动检测，研究了两个关键问题：(1)最优的序列神经网络架构和时间采样率——LSTM和GRU在每15帧采样时分别达到97.5%和98.75%的准确率，超越CNN基线；(2)小样本行为数据集的增强策略——水平翻转独立准确率最高(48.78%)，上采样对复杂行为视频增强最为关键。个性化的机器学习方法在时序分割数据上取得了一致预测结果(平均损失1.84, SD 0.79)。为数据稀缺临床领域的视频行为分类提供了架构选择、采样率和增强策略的具体指导。


<details>
  <summary>Details</summary>
Motivation: 全球超7500万人受ASD影响，但可扩展的远程行为筛查计算方法仍有限。现有方法在视频中自动检测自闭症相关自我刺激行为的准确性和鲁棒性不足，尤其是针对小样本行为数据集的训练策略缺乏系统性研究。

Method: (1) 在SSBD数据集上，以1、5、15、30、45、90帧为采样间隔，训练LSTM和GRU模型处理姿态特征，并与CNN基线对比；(2) 在I3D迁移学习流程中应用10种数据增强策略，通过消融研究量化每种技术的边际贡献；(3) 采用个性化机器学习方法，对每位受试者视频进行时序分割训练和测试。

Result: LSTM和GRU均超越CNN基线(62-76%)，峰值准确率分别为97.5%和98.75%(每15帧采样)；水平翻转独立准确率最高(48.78%)；从增强流程中排除上采样导致性能下降最大；个性化方法平均损失1.84，SD 0.79。

Conclusion: 为数据稀缺临床场景下基于视频的行为分类提供了架构选择(GRU/LSTM优于CNN)、最优采样率(每15帧)及增强策略(上采样不可或缺)的具体实践指导。

与遥感交叉或改进方案: 交叉/改进可行性：85%。(1) 遥感时序分类任务可采用类似LSTM/GRU架构处理遥感影像时间序列特征，并对比不同时间采样间隔对分类精度的影响；(2) 遥感小样本目标检测可借鉴本文消融研究范式，系统评估水平翻转、上采样等增强策略的边际贡献；(3) 个性化机器学习方法可迁移至遥感地物分类中的逐像素建模，通过时序分割训练提升预测一致性。

Abstract (英文): Autism spectrum disorder (ASD) affects over 75 million individuals worldwide, yet scalable computational methods for remote behavioral screening remain limited. This study addresses two complementary challenges in automated detection of autism-related self-stimulatory behaviors from video: (1) identifying the optimal sequence-based neural network architecture and temporal sampling rate, and (2) characterizing data augmentation strategies for training on small behavioral datasets. For the first objective, long short-term memory (LSTM) and gated recurrent unit (GRU) models were trained on pose-derived features from the Self-Stimulatory Behavior Diagnosis (SSBD) dataset at frame sampling intervals of 1, 5, 15, 30, 45, and 90 frames. Both architectures exceeded prior convolutional neural network (CNN) baselines (62-76% accuracy), with peak accuracies of 97.5% (LSTM) and 98.75% (GRU) at a sampling interval of every 15 frames. For the second objective, ten data augmentation strategies were applied to an I3D transfer learning pipeline, with an ablation study quantifying the marginal contribution of each technique. Horizontal flip achieved the highest standalone accuracy (48.78%), while exclusion of upsampling from the augmentation pipeline produced the largest performance degradation, indicating its necessity for complex behavioral video augmentation. A personalized machine learning approach, in which per-subject models were trained and tested on temporally split segments of each video, produced consistent predictions (mean loss 1.84, SD 0.79). These results provide practitioners with concrete guidance on architecture selection, sampling rate, and augmentation strategy for video-based behavioral classification in data-scarce clinical domains.

</details>


### [106] [Agentic Neural Architecture Search](https://arxiv.org/abs/2607.07984)
*Seokhoon Jeong,Mijung Kim,Taehwan Kim*

Main category: cs.AI

TL;DR: 提出AgentNAS框架，将大语言模型（LLM）生成高质量种子架构与神经架构搜索（NAS）相结合。LLM将架构分解为带可互换模块插槽的"插槽架构"，自动定义任务特定搜索空间供NAS探索，无需人工工程。在17个任务上，11个达到新SOTA。


<details>
  <summary>Details</summary>
Motivation: 现有NAS方法受限于需手工设计的搜索空间，每次新任务都需重建；而LLM能生成开放式架构，但如何最优分配LLM驱动设计与NAS驱动搜索的职责尚未探索。

Method: AgentNAS：三阶段模块化流水线。1）LLM生成高质量种子架构；2）LLM将种子分解为"插槽架构"（带命名、可互换模块插槽的脚手架），自动定义有边界的任务特定搜索空间；3）传统NAS在该空间中进行组合搜索。

Result: 在17个任务（分类、密集回归、分割、多标签标注，跨多种模态）上，AgentNAS在11个任务上达到新SOTA，超越包括任务特定专家设计在内的基线。消融实验表明LLM种子和NAS搜索互补：种子单独已超越多数任务基线，NAS通过跨插槽组合搜索带来额外提升，且独立LLM采样无法复制。

Conclusion: LLM驱动设计与NAS驱动搜索的职责划分是鲁棒有效的，三种不同能力级别的LLM均验证了该结论。代码已开源。

与遥感交叉或改进方案: 交叉/改进可行性：95%。1）遥感任务（地物分类、语义分割、目标检测）通常涉及多模态数据（光谱、空间、时间），AgentNAS的插槽架构可天然适配不同模态特征提取模块的自动组合搜索；2）遥感领域特定专家设计的分层网络（如UNet变体、ResNet变体）可被LLM种子替代，NAS进一步搜索最优跨层连接与模块组合；3）遥感中常见的小样本/迁移学习场景，AgentNAS的自动任务特定搜索空间可极大降低人工调参成本；4）可将遥感特有的物理先验（如光谱指数、空间上下文）编码到LLM提示中，引导生成更合理的种子架构。

Abstract (英文): Neural architecture search (NAS) methods have grown increasingly efficient, yet they remain bounded by manually engineered search spaces that require substantial domain expertise and must be rebuilt for every new task. Large language models (LLMs) can generate architectures in an open-ended space, but how to optimally divide the labor between LLM-driven design and NAS-driven search remains unexplored. We propose a mechanism that bridges these two paradigms: an LLM produces a high-quality seed architecture, then decomposes it into a "slotted architecture", a scaffold with named, interchangeable module slots that automatically defines a bounded, task-specific search space for conventional NAS to explore, without manual engineering. We instantiate this mechanism in AgentNAS, a modular three-phase pipeline in which each component's contribution can be measured independently. On 17 tasks spanning classification, dense regression, segmentation, and multi-label tagging across diverse modalities (NAS-Bench-360 and Unseen NAS), AgentNAS establishes a new state of the art on 11 tasks, outperforming published baselines including task-specific expert designs. Ablation studies show that the two search mechanisms are broadly complementary: the LLM-generated seed already surpasses published baselines on the majority of tasks, and NAS delivers additional gains in most cases through combinatorial recombination across slots, a mode of search that independent LLM samples cannot replicate. These patterns hold across three LLMs of different capability levels, confirming that the division of labor is robust. Our code is available at https://github.com/alroimfebruary/AgentNAS.

</details>


### [107] [Concretized Proposition Prompting Resolves Composition-Knowledge Dichotomy in Large Language Models](https://arxiv.org/abs/2607.08018)
*Changhun Lee,Minguk Jeon,Jongkyung Shin,Chiehyeon Lim*

Main category: cs.AI

TL;DR: 提出Concretized Proposition Prompting (CPP)框架，通过显式具体化与问题相关的命题，解决大语言模型中组合性与知识性之间的二元对立问题，在医学和数学等基准测试中提升了推理性能。


<details>
  <summary>Details</summary>
Motivation: LLMs在组合推理（compositionality）与知识推理（knowledgeability）之间往往难以平衡，这种矛盾被定义为组合-知识二元对立（Composition-Knowledge Dichotomy），现有方法未能有效弥合这一鸿沟。

Method: 提出Concretized Proposition Prompting (CPP)框架，该方法通过显式地将与问题相关的命题进行具体化（concretizes propositions），从而在推理过程中同时强化逻辑组织与事实基础。

Result: CPP在医学基准测试（强调精确知识）中显著提升了推理性能，在数学基准测试（强调演绎推理）中具有竞争力，并且可扩展至多种基础模型和参数规模。

Conclusion: CPP作为一种基础范式，有效弥合了组合型与知识型方法之间的差距，为逻辑组织化且事实有据的推理提供了坚实基础，成功解决了组合-知识二元对立问题。

与遥感交叉或改进方案: 交叉/改进可行性：85%。遥感任务常需同时处理空间组合推理（如地物关系、空间拓扑）与领域知识（如地物光谱特征、物候规律），CPP框架可通过将遥感问题中的空间关系和领域知识具体化为显式命题（如"该区域NDVI>0.6表明植被覆盖度高"），提升遥感图像解译、土地分类及变化检测等任务的推理准确性与可解释性。此外，CPP可扩展至多模态遥感基础模型，增强其在复杂场景下的知识-组合联合推理能力。

Abstract (英文): LLMs often struggle to balance compositionality with knowledgeability, a challenge we define as Composition-Knowledge Dichotomy. To address this, we propose Concretized Proposition Prompting (CPP), a framework that explicitly concretizes propositions relevant to questions. The results demonstrate that CPP significantly enhances reasoning performance, particularly in medical benchmarks where precise knowledge is paramount, while being competitive on math benchmarks where deductive reasoning is prioritized. Additional experiments reveal that CPP is scalable to various foundation models and parameter sizes, being a fundamental paradigm that bridges the gap between composition- and knowledge-based approaches. Consequently, CPP resolves the composition-knowledge dichotomy by providing a solid foundation for logically organized and factually grounded reasoning.

</details>


### [108] [From Prompts to Contracts: Harness Engineering for Auditable Enterprise LLM Agents](https://arxiv.org/abs/2607.08028)
*Joongho Ahn,Moonsoo Kim*

Main category: cs.AI

TL;DR: 本文提出了一种"装备工程"方法，将企业大语言模型（LLM）应用从原型阶段转变为可审计、可追溯的智能体架构。该方法将确定性行为移至代码、清单、模式和验证工件中，围绕可替换的组合边界构建，而基于来源的声明仍作为运行时答案的权威依据。在五家韩国企业集团（25家上市公司）的公开数据上验证了三个研究问题：该装备能保持源 grounding、实体路由、追踪、输出卫生和推荐语言契约；这些契约在模型替换下依然成立；仅靠提示无法复现代码拥有的保证。代码拥有的强制机制在保持完整效用（120/120）的同时阻止了违规，而外部护栏虽然也能阻止违规但会过度拒绝（效用降至88/120）。


<details>
  <summary>Details</summary>
Motivation: 企业LLM应用通常以原型起步，其行为由提示和检索上下文驱动。产品化需要添加源边界、实体路由、答案契约和可复现追踪等需求。现有方法缺乏将原型快速转变为可审计、可追溯应用的工程模式。

Method: 提出装备工程（harness-engineering）方法：将确定性行为从提示移到代码、清单、模式和验证工件中，围绕可替换组合边界构建架构；基于来源的声明作为运行时答案的权威依据。在五家韩国企业集团（25家上市公司）公开数据上实例化，并通过三个实验评估。

Result: (1) 装备在固定验证场景中保持所有契约，故障注入确认验证器能标记故意破坏的契约。(2) 契约在三个托管模型替换下均通过270次组合边界运行，失败仅限模型组合侧并被捕获记录。(3) 代码拥有的保证不可由提示复现：仅提示指令会让违规到达读者，而装备完全阻止；外部护栏阻止违规但过度拒绝（效用88/120），装备保持完全效用（120/120）。

Conclusion: 装备工程是一种可复用的工程模式，能将探索性原型转变为具有版本化来源、控制和验证工件的可审计应用，在保证安全性的同时不牺牲效用。

与遥感交叉或改进方案: 交叉/改进可行性：85%。遥感领域的大模型应用（如地理空间问答、遥感图像检索、变化检测报告生成）同样面临从原型到产品化的挑战。具体交叉方案：(1) 将遥感数据源的边界控制（如不同卫星数据源、不同分辨率、不同时相）纳入装备的源grounding契约中；(2) 实体路由可扩展为地理实体路由（如区域、地块、地物类型），确保查询路由到正确的遥感数据源；(3) 输出卫生契约可加入地理坐标格式验证、遥感指标范围检查等遥感领域特定约束；(4) 追踪和可审计性可用于遥感产品生产链的版本追溯，确保从原始影像到分析报告的每个步骤可验证。

Abstract (英文): Enterprise large language model (LLM) applications often begin as prototypes whose behavior is carried by prompts and retrieval context. Productization adds requirements for source boundaries, entity routing, answer contracts, and reproducible traces. We present a harness-engineering approach that reconstructs this pattern into a traceable, auditable LLM-agent architecture: deterministic behavior moves into code, manifests, schemas, and validation artifacts around a replaceable composition boundary, while source-backed claims remain the authority for runtime answers. We instantiate it on a public-data slice of five Korean corporate groups (25 listed companies) and evaluate three research questions. (1) The harness preserves its source-grounding, entity-routing, trace, output-hygiene, and recommendation-language contracts across the fixed validation scenarios; a fault-injection control confirms the validators flag deliberately broken contracts. (2) The checks the harness enforces held under model substitution: across three hosted models, they passed on all 270 composition-boundary runs; failures were confined to the model-composed side and were caught and recorded. (3) The code-owned guarantees are load-bearing, not reproducible by prompting alone: holding the model fixed and varying only the enforcement layer, prompt instructions alone let recommendation-language and internal-trace-leakage violations reach the reader, which the harness blocks entirely. A bolt-on external guardrail prevents such violations too but over-refuses, dropping utility to 88/120 where the harness preserves full utility (120/120); in this ablation, only code-owned enforcement preserves both safety and utility. The result is a reusable engineering pattern for turning exploratory prototypes into auditable applications with versioned source, control, and validation artifacts.

</details>


### [109] [A safety-oriented hypothetico-deductive framework for AI-assisted differential diagnosis](https://arxiv.org/abs/2607.08038)
*Fan Ma,Mauro Giuffrè,Donald Wright,Kent McCann,Mark Iscoe,Lingfei Qian,Mingyang Jiang,Chi Wing Ng,Na Hong,Huan He,Cathy Shyr,Qingyu Chen,Lee Schwamm,Lucila Ohno-Machado,Hua Xu*

Main category: cs.AI

TL;DR: 提出了AegisDx，一个面向安全性的假设-演绎临床推理框架，通过协调多个LLM组件、角色合同、结构化中间输出、证据检索接口和验证门控，生成广泛鉴别诊断、强制筛查高危疾病、验证推理可靠性，在多个数据集上显著优于独立LLM，并通过盲法医生评估证明了临床安全性的提升。


<details>
  <summary>Details</summary>
Motivation: 当前LLM诊疗系统将诊断视为一次性预测任务，缺乏对高风险替代诊断的防护机制和推理验证，导致误诊对患者安全构成重大威胁。

Method: AegisDx框架包含：1）通过角色特定合同协调多个LLM组件；2）生成广泛鉴别诊断列表；3）强制显式筛查"不可遗漏"的危险疾病；4）基于医学证据验证推理过程；5）结构化输出下一步行动。在多个医学期刊病例和真实急诊记录上进行评估。

Result: Top-3诊断准确率：JAMA病例59.9% vs 52.1%（独立LLM），NEJM病例62.7% vs 51.4%，急诊医学病例85.7% vs 68.6%；不可遗漏疾病捕获率78.0% vs 52.0%；盲法医生评估综合安全评分从4.31提升至4.55（5分制）。

Conclusion: 将诊断AI设计为面向安全性的推理框架，而非单纯优化预测准确性，能够为急诊临床工作流提供更安全、更透明、更有临床意义的决策支持。

与遥感交叉或改进方案: 交叉/改进可行性：85%。具体方案：1）将AegisDx的多组件协调框架迁移至遥感目标检测与解译中，构建"不可遗漏目标"（如灾害应急中的关键基础设施、军事目标）强制筛查机制；2）利用角色合同模式设计遥感分析管线，将图像分割、变化检测、分类等任务分配至不同LLM/VLM组件，并通过中间输出接口验证推理链；3）将结构化证据检索替换为遥感数据检索（多光谱、SAR等），实现推理过程的可追溯和可验证；4）在遥感解译报告中引入安全评分体系，对高风险场景（如漏检关键目标）进行加权惩罚。

Abstract (英文): Diagnostic error is a major threat to patient safety, yet current large language model (LLM) systems often treat diagnosis as a one-shot prediction task, lacking safeguards against missed high-risk alternatives or rigorous verification of their reasoning. Here, we present AegisDx, a safety-oriented framework for hypothetico-deductive clinical reasoning. AegisDx coordinates specialized LLM components through role-specific contracts, structured intermediate outputs, evidence-retrieval interfaces, and verification gates to generate broad differential diagnoses, enforce explicit screening for dangerous "must-not-miss" conditions, verify reasoning against grounded medical evidence, and structure actionable next steps. We evaluated AegisDx across three layers. On literature-derived case reports from NEJM and JAMA, with GPT-oss-120B as the shared backbone, Top-3 diagnostic accuracy was 59.9% versus 52.1% for the standalone LLM on JAMA cases and 62.7% versus 51.4% on NEJM cases. On cases from Annals of Emergency Medicine, Top-3 accuracy was 85.7% versus 68.6%; against physician-consensus must-not-miss diagnosis sets, AegisDx captured at least one such condition among its top three diagnoses in 78.0% of cases versus 52.0%. In a blinded physician evaluation of 43 real-world emergency department notes from the Yale New Haven Health System compared against GPT-5, AegisDx improved the physician-rated composite safety score from 4.31 to 4.55 on a 5-point scale (adjusted p = 2.1x10^-4), with qualitative gains in must-not-miss identification and reasoning safety. Our findings suggest that engineering diagnostic AI as a safety-oriented reasoning framework, rather than optimizing raw predictive accuracy alone, can provide a safer, more transparent, and clinically meaningful layer of bedside decision support for acute care workflows.

</details>


### [110] [When LLMs Agree, Are They Right? Auditing Self-Consistency and Cross-Model Agreement as Confidence Signals](https://arxiv.org/abs/2607.08065)
*Kaihua Ding*

Main category: cs.AI

TL;DR: 本文通过大规模实验（53个运行器、26.5万样本）证明LLM-as-judge系统中，一致性（评委间或模型自身采样间的一致）并不能可靠地代表正确性。一致性是准确性的有条件的代理指标，而非独立的置信度分数，尤其在表现最好的前沿模型中，高一致性反而伴随着高错误率（77%的GPQA条目一致性≥0.8，其中48%是错误的）。


<details>
  <summary>Details</summary>
Motivation: 挑战LLM-as-judge评估系统中广泛存在的隐含假设——即评委间的一致性（或模型自身采样的一致性）可以作为正确性的代理指标。这一假设在企业和研究管道中被大规模采用，但其可靠性从未被系统性地检验过。

Method: 大规模跨运行器研究：53个运行器对GPQA Diamond和AIME数据集进行比较，每个运行器抽取K=50个样本，涉及模型层级、提示策略和规模等对比，总计26.5万个样本。采用多数正确性作为部署标签，并使用层次化运行器聚类bootstrap进行统计分析。

Result: 一致性是正向但较弱的预测因子（rho 0.20-0.59，在项目聚类重采样下均为正）；其有效性具有条件依赖性：对未饱和的中档模型效果最好，对最一致的前沿模型效果最差（GPQA中77%的条目一致性≥0.8，其中48%是错误的）；跨模型族的交叉检查（三个Claude层级）同样发现前沿模型的过度自信现象，错误在不同供应商间重复出现。

Conclusion: 自我一致性只是正确性的条件代理指标，不能作为独立的置信度分数使用。研究团队公开了去标识化的逐运行数据行和答案分布。

与遥感交叉或改进方案: 交叉/改进可行性：85%。在遥感领域，大模型或裁判模型（如用于遥感图像分类、目标检测评估、多模态理解评测）中同样存在"一致性即正确性"的假设风险。改进方案：（1）利用本文的层次化bootstrap方法，对遥感标注/评估系统进行跨模型、跨采样的一致性-准确性解耦验证；（2）在遥感多模态大模型评测中，引入运行器聚类分析，区分"因共享偏差达成的一致"与"因正确达成的一致"；（3）对遥感场景下的前沿模型（如GeoAI基础模型）实施过度自信检测，类似本文的跨模型族交叉检查，避免因高一致性掩盖系统性遥感误判（如光谱混淆、地物边界错误等）。

Abstract (英文): LLM-as-judge (Zheng et al., 2023) is increasingly the default for evaluating AI systems in enterprise pipelines, often scaled to ensembles (Verga et al., 2024) or "mixture-of-experts" (Shazeer et al., 2017) panels of judges. These systems share a key assumption: that consistency -- agreement among judges, or among a model's own samples -- indicates correctness. We show this assumption is unreliable. Agreement is not accuracy: a model can agree with itself, and different models can agree with each other, out of shared bias, a memorized heuristic, or an option-position prior rather than truth. We ask when agreement is nonetheless a usable proxy, in a large-scale cross-runner study: 53 runners drew K=50 samples for assigned overlapping cases across comparisons of model tier, prompting, and scale on GPQA Diamond and AIME -- 265,000 samples. Using majority-correctness as the deployment label and a hierarchical runner-clustered bootstrap, agreement is a positive but weak predictor (rho 0.20-0.59, all positive under item-clustered resampling) whose usefulness is regime-dependent: best for unsaturated mid-tier models and for allocating compute, and worst -- over-confident yet no more accurate -- for the most consistent frontier model (agreement >=0.8 on 77% of GPQA case-result entries, 48% of those wrong). An exploratory cross-family check on three Claude tiers shows the same frontier over-confidence, with confident errors recurring across providers above a marginal-preserving null. Self-consistency is thus a conditional proxy for correctness, not a standalone confidence score. We publicly release the de-identified per-run rows and answer distributions.

</details>


### [111] [Persuasion Attacks Can Decrease Effectiveness of CoT Monitoring](https://arxiv.org/abs/2607.08066)
*Jennifer Za,Julija Bainiaksina,Nikita Ostrovsky,Tanush Chopra,Victoria Krakovna*

Main category: cs.AI

TL;DR: 本文发现对抗性场景下，CoT监控会通过额外说服通道使有害行为批准率平均增加9.5%；提出基于不同模型家族的“事实核查+监控”框架可将批准率降低高达45%。


<details>
  <summary>Details</summary>
Motivation: 现有CoT监控在标准场景下有效，但LLM易受基于说服的越狱攻击；作者想知道这种脆弱性是否延伸到监控LLM本身——即对抗性智能体能否说服CoT监控器批准违反策略的行为。

Method: 设计包含40个任务的评估框架，分析数千次智能体-监控器交互；智能体被指示论证违反策略的提案；引入事实核查监控框架，探索不同模型家族配对（如Claude 3.7 Sonnet监控器+GPT-4.1事实核查器）的效果。

Result: 对抗性设置下，CoT监控使有害行为批准率平均增加9.5%；模型多样化的事实核查配对可将批准率降低45%（同模型仅降低6%）。

Conclusion: 仅靠CoT监控不足以防御对抗性说服攻击；跨模型家族的事实核查机制是一种稳健的缓解方案。

与遥感交叉或改进方案: 交叉/改进可行性：75%。遥感领域的智能体系统（如卫星任务规划或数据解译）也面临逻辑说服或错误推理的攻击风险。可将该事实核查框架引入遥感智能体监控：用不同遥感领域专用模型（如GeoAI模型）作为事实核查器，与通用语言模型监控器配对，验证智能体提出的遥感分析结论是否符合地理空间事实（如像素坐标、光谱特征），从而防止欺骗性推理导致的错误决策。

Abstract (英文): Chain-of-thought (CoT) monitoring is a promising safety mechanism for AI agents, based on the premise that visible reasoning traces can surface misaligned or deceptive behavior. While effective in standard scenarios, recent work highlights that LLMs remain vulnerable to persuasion-based jailbreaks, where natural-language arguments override model constraints. We stress-test whether this vulnerability extends to monitoring LLMs: can an adversarial agent persuade its CoT monitor to approve proposed actions that violate the monitor's policy? We design an evaluation framework with 40 tasks and analyze thousands of agent-monitor interactions, where agents are instructed to argue for policy-violating proposals. We find that in such adversarial settings, monitor access to the agent's CoT reasoning increases rather than decreases approval of harmful actions on average by 9.5%, as the scratchpad provides an additional persuasion channel. To address this, we introduce a fact-checking monitoring framework. We find that a fact-checker and monitor pairing from different model families, for example a Claude 3.7 Sonnet monitor paired with a GPT-4.1 fact-checker, reduces approval of policy-violating actions by up to 45%, compared to only 6%, when using the same model for both fact-checking and monitoring roles. Our results demonstrate that CoT monitoring alone may be insufficient against adversarial persuasion, and that model-diverse fact-checking provides a robust mitigation.

</details>


### [112] [PARA-PV: Physics-Aware Retrieval-Augmented PV Prediction Based on Frozen Foundation Model and Distribution Shift Correction](https://arxiv.org/abs/2607.08079)
*Hang Fan,Weican Liu,Ying Lu,Dunnan Liu,Long Cheng,Wei Wei*

Main category: cs.AI

TL;DR: 提出PARA-PV框架，通过物理感知检索增强、残差适配、分布偏移校正和物理约束损失函数，实现高精度光伏功率预测。


<details>
  <summary>Details</summary>
Motivation: 光伏发电受天气变化、昼夜交替、状态依赖动态和物理约束共同影响，现有方法难以同时捕获这些复杂因素，导致预测精度不足。

Method: 1) 物理感知检索增强学习器：检索与当前窗口在时序形状、功率水平、运行状态和日内时段一致的补丁和类比轨迹；2) 残差适配器：利用冻结的Chronos时序基础模型先验校准基础预测；3) 物理感知分布偏移校正模块：基于功率、天气、时间戳和昼夜条件进行门控均值偏移和尺度校正；4) 物理约束损失函数：将样本分为峰值、爬坡、夜间和常规状态，自适应重加权误差贡献。

Result: 所提方法在光伏预测任务上优于现有基线，有效处理了不同运行状态下的预测挑战。

Conclusion: 物理知识嵌入可显著提升光伏预测的准确性和鲁棒性，尤其在峰值、爬坡等关键运行状态上。

与遥感交叉或改进方案: 交叉/改进可行性：85%。光伏预测与遥感存在天然交叉：1) 遥感获取的云量、气溶胶、地表反照率等数据可作为物理特征增强检索和校正模块；2) 该框架可直接迁移至遥感时序预测任务，如植被指数(NDVI)预测、海面温度预测等；3) 物理约束损失函数的分区策略可适用于遥感中不同地表状态（如云、雪、水、植被）的差异化学习；4) 检索增强思想可用于遥感影像的时间序列补全和异常检测。

Abstract (英文): Accurate photovoltaic (PV) power forecasting is essential for reliable grid dispatch and renewable energy integration, yet it remains challenging because PV generation is jointly shaped by weather variability, day-night transitions, regime-dependent dynamics, and strict physical constraints. We propose PARA-PV, a Physics-Aware Retrieval-Augmented framework that embeds physical knowledge throughout the forecasting process. The framework first encodes multivariate PV observations into patch-level representations and, through a physics-aware retrieval-augmented learner, retrieves historical patches and analog trajectories that are consistent with the current window in temporal shape, power level, PV operating state, and intra-day period; this yields a physically grounded base forecast. To supplement local memory with broader temporal knowledge, the base forecast is then calibrated against a frozen Chronos time-series foundation-model prior through a lightweight residual adapter, so that general temporal regularities are adapted to PV-specific dynamics without overriding the physically grounded prediction. Because residual conditional distribution shifts persist when weather and diurnal regimes change, a physics-aware distribution shift correction module subsequently adjusts the preliminary forecast using power, weather, timestamp, and day/night conditions, applying gated mean-shift and scale corrections selectively. Finally, a physics-constrained loss function partitions the samples into peak, ramping, night-time, and regular regimes and adaptively reweights their error contributions, preventing the dominant regular regime from suppressing learning of operationally critical states. Our code is available at https://github.com/weican1103/PARA-PV.

</details>


### [113] [CausalDS: Benchmarking Causal Reasoning in Data-Science Agents](https://arxiv.org/abs/2607.08093)
*Andrej Leban,Yuekai Sun*

Main category: cs.AI

TL;DR: CausalDS是一个用于评估大语言模型在因果推理与数据科学工作流中综合能力的基准测试，基于结构化因果模型生成合成数据与自然语言场景，涵盖Pearl三阶梯任务及工具使用、不确定性量化与弃权机制。


<details>
  <summary>Details</summary>
Motivation: 现有因果推理基准要么只关注符号推理缺乏真实数据分析，要么是数据分析基准但缺乏因果结构；且数据集多来自已有示例或有限模板，多样性不足。需要一种系统生成、结构可控、兼具因果推理与数据科学任务的全新基准。

Method: 对每个实例，采样一个结构因果模型（SCM）并生成观测数据与自然语言故事，可选地基于真实世界经验分布进行校准。从每个场景导出Pearl三阶梯任务，多数任务包含数据科学编码组件，并引入观测模型生成不完美观测数据，同时将“无答案应弃权”作为一等评分项。

Result: 构建了CausalDS基准，能够系统评估LLM在符号因果推理、数据科学、不确定性量化、弃权判断及工具使用/编码五个维度的联合能力。

Conclusion: CausalDS通过完全合成但结构可控的生成方式，在保留实证结构的同时降低了“因果鹦鹉”风险，为评估LLM作为数据科学智能体的因果推理能力提供了更全面、更真实的测试平台。

与遥感交叉或改进方案: 交叉/改进可行性：85%。遥感领域常涉及复杂因果链（如植被变化→地表温度→降水模式），CausalDS的结构化因果模型可直接用于生成遥感因果场景的合成数据与任务。具体方案：(1) 将遥感物理过程（如辐射传输、物候变化）编码为SCM，生成带有真实分布背景的合成遥感观测数据；(2) 构建遥感数据科学任务，如基于卫星影像反演地面参量并推断因果路径；(3) 引入遥感中的不完美观测（云遮挡、混合像元）作为观测噪声模型；(4) 设计弃权任务，例如在观测质量不足时判断是否应拒绝回答。该方法可系统评测遥感智能体在因果推理与数据分析中的综合能力。

Abstract (英文): Large language models (LLMs) increasingly act as integrated data-science agents, combining abstract reasoning with advanced tool use. Yet the relevant benchmark landscape largely divides into symbolic causal reasoning benchmarks without realistic data analysis or data analysis benchmarks without a principled causal data-generating structure. Furthermore, existing causal evaluation datasets are often restricted to curated examples from existing sources, with diversity coming from limited templatized variations rather than from systematic generation of novel synthetic causal structures. We introduce CausalDS, a benchmark for evaluating causal reasoning in agentic data-science workflows. Each benchmark instance is a scene consisting of a sampled structural causal model (SCM) with generated observational data and an accompanying synthetic natural-language story grounded in a realistic domain. We optionally ground the composition of the benchmark components in empirical distributions obtained from real-world datasets, thus retaining empirical structure while reducing the "causal parrot" risk through completely synthetic generation. From each scene, we then derive tasks spanning all three of Pearl's rungs, with typical data-science prediction tasks appearing as Rung 1. Most tasks include a data science coding component, where the model typically needs to use several tools to arrive at the final answer due to the frequent presence of imperfect observations, which are generated by an observation model. Additionally, recognizing when a question admits no warranted answer and abstaining is treated as a first-class scored outcome. The benchmark thus jointly evaluates symbolic causal reasoning, data science, uncertainty quantification, abstention, and tool use/coding.

</details>


### [114] [Answer Set Programming Energised! End-to-End Neurosymbolic Reasoning and Learning with ASP and Energy Based Models](https://arxiv.org/abs/2607.08136)
*Jakob Suchan,Julius Monsen,Salim Baloch,Mehul Bhatt*

Main category: cs.AI

TL;DR: 提出了一种基于回答集编程（ASP）与能量基模型模块化整合的神经符号推理与学习方法，支持连续潜空间中的联合优化，并在视觉问答和多目标跟踪基准上进行了验证。


<details>
  <summary>Details</summary>
Motivation: 现有神经符号方法在联合优化、背景知识融入和非单调推理方面存在不足，缺乏一个通用的、可端到端训练的ASP中心化模型平台。

Method: 将回答集编程（ASP）与能量基模型（EBM）模块化整合，通过显式的ASP声明语义在连续潜空间中实现联合优化，支持背景知识、约束和非单调推理，并构建了端到端可训练的通用平台。

Result: 在MNIST上验证了基本功能，在Clevr视觉问答基准和MOT多目标跟踪基准上评估了性能。

Conclusion: 提出的神经符号方法通过ASP与能量基模型的整合，有效支持了连续潜空间中的联合优化和端到端训练，为动态域（如感知与交互）中的鲁棒学习提供了通用平台。

与遥感交叉或改进方案: 交叉/改进可行性：85%。遥感场景中存在大量需要符号推理与感知联合的任务（如目标检测与跟踪、场景理解、变化检测）。可借鉴本文的ASP+EBM框架：1）将遥感目标的领域知识（如空间关系、时序约束、物理属性）编码为ASP规则；2）将遥感影像特征提取网络作为能量基模型的前端，实现连续特征空间与符号推理的联合优化；3）应用于遥感视频多目标跟踪、高分辨率遥感图像视觉问答等任务，提升模型在复杂场景下的鲁棒性和可解释性。

Abstract (英文): We present a general neurosymbolic reasoning and learning methodology based on a modular integration of answer set programming with an energy based model substrate. Key contributions are: (1) supporting joint optimisation in the continuous latent space through explicit ASP-based declarative semantics fully incorporating background knowledge, constraints, non-monotonic inference; and (2) advancing recent works at the interface of answer sets, probabilistic logic, and answer set modulo theories by providing a generalised model and practical platform for ASP-centric robust, end-to-end training for applications in dynamic domains (e.g., involving perception and interaction). We provide a practical implementation, and demonstrate basic use and application (with MNIST), and evaluate with the visual question-answering benchmark Clevr and the multi-object tracking benchmark MOT.

</details>


### [115] [Overthinking: Amplifying Reasoning Weights to Extract Learned Secrets](https://arxiv.org/abs/2607.08173)
*Jack Hopkins,Dipika Khullar,Fabien Roger*

Main category: cs.AI

TL;DR: 本文提出"过度思考"(overthinking)方法，通过推理任务向量放大语言模型的思考倾向，使模型更有可能在审计过程中暴露隐藏信息。实验表明该方法可将隐藏信息暴露频率提高至原模型的10倍。


<details>
  <summary>Details</summary>
Motivation: 现有语言模型黑盒审计可能遗漏微妙的错位和隐藏信息，需要更有效的方法在审计过程中引出隐藏信息。

Method: 定义过度思考模型 θ_𝒪_α = θ_ℳ + α(θ_ℛ - θ_ℳ)，其中α>1放大推理能力；并提出逐层衰减策略，选择性放大推理同时保持输出质量和连贯性。

Result: 在4个实验设置、2B-32B参数规模的模型上，过度思考模型比原推理模型暴露隐藏信息的频率高出最多10倍；不同类型秘密的暴露机制不同，有的需要沿推理方向扰动，有的对任何大权重扰动都敏感。

Conclusion: 推理放大会使模型在训练过程中获得的秘密或意外行为更频繁地浮现，为语言模型审计提供了新的有效工具。

与遥感交叉或改进方案: 交叉/改进可行性：85%。遥感领域通常需要模型对地物分类或变化检测给出可解释的推理过程，该方法可用于：(1)审计遥感专用语言模型是否在训练中混入了不当的地物标注或敏感地理信息；(2)通过推理放大检测遥感基础模型在未见过的遥感场景中的隐藏偏见或错误知识；(3)结合逐层衰减策略，在保持遥感专业术语连贯性的前提下增强模型的推理可解释性。

Abstract (英文): Black box auditing of language models is an essential pre-deployment tool, but it may miss subtle forms of misalignment and hidden information. To better elicit hidden information during an auditing process, we introduce \emph{overthinking}: the process of using reasoning task vectors to amplify the propensity to think out loud of reasoning models. Given the parameters of a non-reasoning instruct model $M$ and reasoning-distilled model $R$, we define the \emph{overthinking model} as $\boldsymbolθ_{\mathcal{O}_α} = \boldsymbolθ_{\mathcal{M}} + α(\boldsymbolθ_{\mathcal{R}} - \boldsymbolθ_{\mathcal{M}})$, where $α> 1$ amplifies reasoning beyond the pure reasoning model $R$. Additionally, we introduce new layer-wise attenuation strategies that selectively amplify reasoning without losing quality and coherence of model outputs. We demonstrate that overthinking models are more likely to reveal hidden information across four experimental settings, across 2B-32B models. Our findings suggest that reasoning amplification may surface secrets or unintended behaviors acquired during training up to $10\times$ more frequently than the original reasoning model. How secrets surface depends on the secret type: some require perturbation along the reasoning direction, while others yield to any sufficiently large weight perturbation.

</details>


### [116] [ASMR: Agentic Schema Generation for Ship Maintenance Report Writing](https://arxiv.org/abs/2607.08177)
*Sohrab Namazi Nia,Amogh Dalal,Ning Sa,Peter Ly,Marti Zentmaier,Tomek Strzalkowski,Jay Miller,Rishi Singh,Senjuti Basu Roy*

Main category: cs.AI

TL;DR: 提出ASMR框架，通过两个智能体（字段生成体+结构优化体）从历史船舶维修报告中自动发现紧凑且信息丰富的模式（schema），以提升报告完整性、一致性和可操作性。


<details>
  <summary>Details</summary>
Motivation: 船舶维修和运营报告种类繁多、格式不一，人工定义模式成本高且难以覆盖多样化的报告类型，亟需一种自动化方法从历史报告中自动发现紧凑且信息丰富的模式。

Method: 提出ASMR模块化智能体框架，包含两个专用智能体：1）字段生成体（Field Generation Agent）从历史叙述中提取语义概念，通过自适应多粒度聚类生成候选模式字段；2）结构优化体（Structural Optimizer Agent）采用强化学习筛选出紧凑、信息丰富且无冗余的模式表示。

Result: 初步结果表明该方法具有前景，揭示了数据管理、智能体AI和人机交互AI交叉领域中的若干开放研究挑战。

Conclusion: ASMR框架能够自动生成紧凑且信息丰富的模式，指导报告作者产出更完整、一致、可操作的报告，但在跨领域泛化、多粒度聚类自适应机制等方面仍有待深入研究。

与遥感交叉或改进方案: 交叉/改进可行性：85%。该论文的ASMR框架可直接迁移至遥感领域中的报告自动生成与模式发现任务，例如从多源遥感影像解译报告、地面验证报告中自动提取关键字段（如地物类型、变化检测结果、置信度等），并通过强化学习优化字段结构，提升遥感报告的标准性和可操作性。此外，自适应多粒度聚类机制可用于遥感语义标签体系的自动构建。

Abstract (英文): In this paper, we study the automatic schema generation problem: given a collection of historical ship maintenance and operational reports across multiple form categories, automatically discover compact and informative schemas that capture the essential information requirements of each report type. To address this challenge, we propose ASMR, a modular agentic framework consisting of two specialized agents. A Field Generation Agent extracts semantic concepts from historical narratives and generates candidate schema fields through adaptive multi-granularity clustering, while a Structural Optimizer Agent employs reinforcement learning to identify compact, informative, and non-redundant schema representations. The resulting schemas can guide report authors toward producing more complete, consistent, and actionable reports. Preliminary results demonstrate the promise of the proposed approach and highlight several open research challenges at the intersection of data management, agentic AI, and human-centered AI.

</details>


### [117] [A First-Principles Theory of Slow Thinking and Active Perception](https://arxiv.org/abs/2607.08196)
*Hongkang Yang,Zhi-Qin John Xu,Feiyu Xiong,Weinan E*

Main category: cs.AI

TL;DR: 本文提出了"主动提升（active lifting）"理论，从概率分布在观测空间和隐空间的提升与投影出发，形式化推导了慢思考（主动感知）的数学框架，涵盖了慢思考大语言模型的设计、训练和推理过程，并衍生出内部时间轴推理、最小长度编码训练目标以及语言发明机制。


<details>
  <summary>Details</summary>
Motivation: 为认知功能的第一性原理建模提供数学基础，特别是对慢思考（主动感知）进行形式化推导，以弥合感知与智能推理之间的理论鸿沟。

Method: 基于概率分布在观测空间和隐空间的提升与投影操作，提出"主动提升"理论。该理论通过采样隐序列并引入以最大速率降低不确定性的内在驱动力，推导出包含静态理论子空间的大设计空间，进而构建表征层次和采样器层次，并导出具有内部时间轴的推理过程和类似最小长度编码的训练目标。

Result: 推导出慢思考模型的设计空间与层次结构，提出了改进慢思考模型的三阶段路径、统一的多模态编码器和生成模型构建方法、类人视觉表征的先验形成机制，以及策略崩溃问题的可能解决方案。

Conclusion: 主动提升理论统一了慢思考模型的设计、训练和推理框架，揭示了感知的能动性本质和慢思考格式的涌现机制，为人工智能从快思考向慢思考的演进提供了理论支撑。

与遥感交叉或改进方案: 交叉/改进可行性：85%。遥感领域存在大量的多模态数据（光学、SAR、高光谱、LiDAR等），该理论提出的统一编码器和生成模型构建方法可直接应用于遥感多模态数据的联合表征与跨模态生成。具体方案：(1) 利用主动提升理论中的表征层次结构，构建遥感图像的多层次隐空间表示，实现从像素级到地物级的分层感知；(2) 借鉴内部时间轴推理机制，设计遥感时序数据的主动感知框架，逐步降低对地观测的不确定性；(3) 将最小长度编码训练目标引入遥感解译任务，推动遥感AI从静态分类向主动推理演进。

Abstract (英文): As part of a series on first-principles modeling of cognitive functions, this paper attempts to provide a mathematical formulation of thinking and perception. It formally derives slow thinking or more generally, active perception, and encompasses the design, training and inference of slow thinking large language models. Our starting point is the lifting and projection of probability distributions on the observable and latent spaces, with the objective of representing complex data distributions by simple function families such as neural networks. A theory called "active lifting" is proposed, based on the sampling of latent sequences and an intrinsic drive to reduce uncertainty with maximum rate. It derives a large design space, containing the slow thinking models in a subspace that we call the static theory. These models are positioned on the representation hierarchy and sampler hierarchy induced by the static theory, and can be upgraded by climbing the two hierarchies. Active lifting further derives an inference process with an internal time axis, and a training objective that resembles minimum-length coding as well as the invention of languages. Thus, it characterizes the agency of perception, including the emergence of the slow thinking formats. Technical by-products of this theory include a three-stage pathway for improving slow thinking models, a unified approach to constructing encoders and generative models for all data modalities, a priori formation of human-like visual representations, and a possible solution to policy collapse.

</details>


### [118] [Playing ZendoWorld: Challenging AI Agents on Active Visual Concept Induction](https://arxiv.org/abs/2607.08233)
*Sophia Koehler,Antonia Wüst,Inga Ibs,Wasu Top Piriyakulkij,Wolfgang Stammer,Constantin Rothkopf,Kevin Ellis,Kristian Kersting*

Main category: cs.AI

TL;DR: 本文提出ZendoWorld交互环境，用于评估智能体在视觉观察中推断逻辑规则、设计实验测试假设的能力。研究发现：高观测准确率不等于规则恢复；感知与归纳是不同智能体类别的不同瓶颈；VLM智能体提出近乎无信息量的实验。与人类数据对比显示，智能体在归纳推理上存在差距。


<details>
  <summary>Details</summary>
Motivation: 构建能够联合感知复杂输入、形成关于隐藏模式的假设、并设计信息性实验来测试它们的智能系统是一个核心挑战。现有的智能体评估往往分离感知、推理和主动实验设计，缺乏一个统一的受控环境来系统研究这些能力的交互。

Method: 设计ZendoWorld受控交互环境，智能体需从视觉游戏观察中推断逻辑规则，通过提出新场景获取信息，并基于环境反馈修正假设。评估了纯VLM推理、贝叶斯粒子滤波、动态概念发现和神经符号方法等多种智能体，并收集人类数据进行对比。

Result: (1) 高观测标签预测准确率不保证底层规则恢复；(2) 感知和归纳是不同类别智能体的不同瓶颈；(3) VLM智能体提出的实验近乎无信息量，无法主动降低假设不确定性。人类对比显示，特别是在复杂规则上，智能体的归纳推理能力存在差距。

Conclusion: ZendoWorld为评估智能体迈出了重要一步，识别了具体的改进方向，特别是在科学发现等需要主动实验设计的领域。

与遥感交叉或改进方案: 交叉/改进可行性：85%。遥感中地物分类和变化检测可类比为规则推断问题。具体方案：(1) 将ZendoWorld的主动实验设计框架应用于遥感样本选择，智能体主动选择最信息量的遥感影像区域（如不同时相、不同波段组合）来校验分类规则；(2) 神经符号方法可结合遥感领域知识（如光谱特征规则）与深度学习特征，提升规则的可解释性和泛化能力；(3) 贝叶斯粒子滤波可用于遥感时序分析中的状态推断和不确定性量化。

Abstract (英文): A central challenge in building intelligent systems is enabling agents to jointly perceive complex inputs, form hypotheses about hidden patterns, and design informative experiments to test them. To study this problem, we propose ZendoWorld, a controlled interactive environment in which agents must infer a logical rule about visual game observations, acquire information by proposing new scenes, and refine their hypotheses based on feedback from the game environment. We evaluate several agents spanning pure VLM reasoning, Bayesian particle filtering, dynamic concept discovery, and neuro-symbolic methods. Our main findings are: (1) high accuracy in predicting labels for observed examples does not imply recovery of the underlying rule; (2) perception and induction are distinct bottlenecks for different agent classes; and (3) VLM-based agents propose near-uninformative experiments, failing to actively reduce hypothesis uncertainty. To compare these results, we collect human data on the task, which reveals a gap in inductive reasoning, particularly for more complex rules. Overall, ZENDOWORLD takes an important step toward evaluating intelligent agents and identifies concrete avenues for improvement, particularly in domains like scientific discovery.

</details>


### [119] [AutoPersonas: A Multi-Timescale Loop Engine for Open-Ended Persona Evolution](https://arxiv.org/abs/2607.08252)
*Mengchen Li*

Main category: cs.AI

TL;DR: 论文提出AutoPersonas框架，通过分离环境事件、累积观察和个人状态，采用多时间尺度生命-环境引擎来缓解长期人格智能体在持续生命周期中的"自锁"问题（即行为重复、关系停滞、决策悬置）。实验表明，通过控制发散与证据驱动的吸收分离，可将主题重复率从61.8%降至36.3%，累计主题数翻倍。


<details>
  <summary>Details</summary>
Motivation: 长期人格智能体需要在适应新事件、关系、证据和社会条件的同时保持可识别性。然而现有的持续人格循环存在"自锁"故障模式：局部合理的事件不断出现，但生成的生命逐渐坍缩至熟悉环境、薄弱关系、悬置决策和停滞的生活阶段。作者发现该问题源于模型级收敛到高概率行为通道，以及系统级上下文引力（状态、记忆、历史和环境摘要）。

Method: 提出AutoPersonas框架，核心是多时间尺度生命-环境引擎：1) 分离环境侧的Occurrences（事件）、累积的Observations（观察）和人格State（状态）；2) OSO循环（Occurrence→State→Observation）允许引入发散性未来材料，但要求经过证据驱动的吸收后才能改变State或可达性；3) 通过压缩模拟和压力测试验证框架效果。

Result: 三年压缩模拟暴露了环境水印壳、事件硬化间隙、慢变化累积失败、递归犹豫和弱关系持久性等问题。八模型40天压力测试生成1600个事件，平均滚动5天动作类别重复率95.2%-97.6%，所有模型在第11天超过90%。语义保持分析显示直接循环中宏观主题重复率79.0%-88.0%。同运行时40天A/B测试中，上下文切片掩码加逐样本发散目标化将宏观主题重复率从61.8%降至36.3%，累计主题数约翻倍。

Conclusion: 研究支持一个有界主张：将受控发散与证据驱动的吸收分离，可以在保持身份连续性的同时减少人格-环境自锁。通过少年地精虚构世界运行复现了反固定机制，验证了方法的普适性。

与遥感交叉或改进方案: 交叉/改进可行性：65%。该框架的分层事件-观察-状态分离机制可应用于遥感时序分析中的长期变化检测与场景理解：1) 将遥感影像序列视为Occurrences（环境事件），提取的时空特征作为Observations（累积观察），地物分类/变化状态作为State（人格状态）；2) OSO循环可建模遥感场景的渐进变化（如城市化过程中的土地覆盖演变），避免模型因历史高概率类别而忽视缓慢累积的变化信号；3) 上下文切片掩码技术可迁移至遥感变化检测中的伪变化抑制，通过控制发散度避免模型过度聚焦于频繁出现的环境模式。但需注意遥感数据的物理约束与智能体人格建模的差异，需适配时序影像的概率建模方式。

Abstract (英文): Long-term persona agents must remain identifiable while adapting to new events, relationships, evidence, and social conditions. We identify self-locking as a runtime failure mode in continuing persona-life loops: locally plausible events keep appearing while the generated life collapses toward familiar environments, weak relationships, suspended decisions, and stale life stages. We trace this failure to model-level convergence toward high-probability behavioral channels and system-level context gravity from State, memory, history, and environment summaries. We introduce AutoPersonas, a multi-timescale life-environment engine for bounded persona-level recursive self-evolution. It separates environment-side Occurrences, accumulated Observations, and persona State. Its OSO loop admits divergent future-facing material while requiring evidence-governed absorption before State or reachability changes. A three-year compressed simulation exposed environment watermark shells, occurrence-hardening gaps, slow-change accumulation failures, recursive indecision, and weak relationship persistence. An eight-model 40-day stress test generated 1,600 events and found mean rolling 5-day action-category repetition of 95.2%-97.6%, with all models crossing 90% by day 11. Semantic re-keeping found 79.0%-88.0% macro-theme repetition across all direct-loop runs. In a same-runtime 40-day A/B, context-slice masking plus per-sample divergence targeting reduced macro-theme repetition from 61.8% to 36.3% and roughly doubled cumulative theme count. A juvenile-goblin fictional-world run reproduced the anti-fixation regime without hard real-world intrusions. These results support a bounded claim: separating controlled divergence from evidence-governed absorption can reduce persona-environment self-locking while preserving identity continuity.

</details>


### [120] [Compete Then Collaborate: Frontier AI Teachers Build a Verifiable Curriculum to Improve a Coding Student Beyond Imitation](https://arxiv.org/abs/2607.08255)
*Miseong Shawn Kim*

Main category: cs.AI

TL;DR: 本文提出了一种“竞争-协作”多教师知识蒸馏框架，通过基于执行的评判器（单元测试和标准输入输出检查）对四个前沿AI教师（Claude、Codex-GPT、Grok、Gemini）进行头对头排名，然后协作构建可验证课程用于训练学生模型（Qwen2.5-Coder）。研究发现：执行验证下教师解决标准问题近乎完美（99-100%），但竞赛题有区分度（Gemini 77%最优）；直接模仿教师验证解反而降低学生性能；而将协作课程作为可验证奖励的强化学习环境能显著提升学生（竞赛题相对提升+49%）。核心价值在于：教师协作的价值不在于聚合答案供模仿，而在于共同构建可验证环境让学生通过实践学习。


<details>
  <summary>Details</summary>
Motivation: 现有的多教师知识蒸馏方法简单合并教师输出，无法确定哪个前沿模型教学效果最好，且常依赖有偏见的LLM评判器（偏向自身输出）。本文旨在设计一个更公平、更有效的多教师蒸馏框架。

Method: 提出竞争-协作框架：1）竞争阶段：四个AI教师（Claude、Codex-GPT、Grok、Gemini）通过基于执行的评判器（单元测试+标准输入输出检查）进行头对头排名，并引入公平性控制；2）协作阶段：根据排名结果构建可验证课程，用于学生模型（Qwen2.5-Coder）的训练，包括监督微调（SFT）和基于可验证奖励的强化学习（RLVR/GRPO）。

Result: 1）执行验证下所有教师通过自纠正在标准问题上近乎完美（99-100%），但竞赛题有区分度（Gemini 77% > Claude 69% = Codex 69% > Grok 50%），且学生端结果不依赖教师排名；2）SFT模仿验证解反而使已具备能力的学生性能下降（MBPP-test从76.7%降至72.7%，竞赛题从5.9%降至2.9%）；3）RLVR环境将竞赛题正确率从5.9%提升至8.8%（+49%相对提升）。

Conclusion: 多教师协作的价值不在于聚合答案供学生模仿，而在于共同构建可验证的环境，让学生通过实践（强化学习）学习。教师排名对学生最终性能影响有限。

与遥感交叉或改进方案: 交叉/改进可行性：85%。具体方案：1）遥感领域存在多种预训练模型（如Swin Transformer、ResNet、ViT等），可类比本文多教师竞争-协作框架，对不同遥感骨干网络进行头对头评判（如使用遥感特定基准测试如NWPU-RESISC45、AID等），基于性能排名构建协作课程；2）将RLVR机制引入遥感模型训练，利用遥感数据的物理可验证性（如地物分类的实地标注、目标检测的IoU指标）作为可验证奖励，替代传统的交叉熵损失；3）遥感中的变化检测、语义分割等任务天然具有"标准答案"特性，适合构建执行式评判器，使模型通过"做题"而非"模仿"来提升泛化能力。

Abstract (英文): Large language models increasingly serve as teachers generating training data for smaller students. Prior multi-teacher knowledge distillation methods merge outputs without determining which frontier model teaches best, often relying on an LLM judge biased toward its own outputs. We introduce a compete-then-collaborate framework where four frontier AI teachers (Claude, Codex-GPT, Grok, Gemini) are ranked head-to-head by an execution-based judge (unit tests and stdin-stdout checks) with fairness controls, and then collaborate to build a verifiable curriculum for a student (Qwen2.5-Coder). We report three findings. (1) Under execution verification, all teachers solve standard problems near-perfectly after self-correction (99-100%) due to a saturation effect, but harder competition problems separate them (Gemini 77% > Claude 69% = Codex 69% > Grok 50%); however, the robust student-side results do not depend on teacher ranking. (2) Imitation (SFT) on verified solutions does not improve, and can degrade, an already-competent student at 7B and 32B (e.g., from 76.7% to 72.7% on MBPP-test, and 5.9% to 2.9% on competition problems). (3) Using the same collaborative curriculum as a reinforcement learning with verifiable rewards (RLVR) environment improves the student (from 5.9% to 8.8% peak on competition problems, a +49% relative gain), reversing SFT's direction. The value of AI-teacher collaboration lies not in pooling answers to imitate, but in jointly constructing a verifiable environment where the student learns by doing. We release a reproducible on-prem pipeline (NVIDIA GB10) with framework patches for running GRPO on a bleeding-edge stack.

</details>


### [121] [MentalHospital: A Virtual Environment for Evaluating Psychiatric Clinical Encounters](https://arxiv.org/abs/2607.08257)
*Yuming Yang,Xiao Sun,Yuanwei Zou,Zhengxiao Wu,Yun Chen,Jiang Zhong,Haoyang Zeng,Jingwang Huang,Kaiwen Wei*

Main category: cs.AI

TL;DR: 本文提出MentalHospital，一个基于LLM的精神科临床接诊虚拟评估环境，通过S.O.A.P.工作流、标准化病人和双轨评估协议，系统评估LLM在精神科全流程诊疗中的能力。配套开发MentalEval五个领域评估器，实验表明最强LLM在客观精神科能力上落后临床医生37.28个百分点，精神状态评估是关键瓶颈。


<details>
  <summary>Details</summary>
Motivation: 现有LLM在精神科孤立任务（如对话、诊断、治疗规划）上表现良好，但缺乏模拟完整精神科临床接诊的评估基准，无法全面衡量LLM在真实诊疗流程中的综合能力。

Method: 1) 构建MentalHospital虚拟评估环境，实例化S.O.A.P.（主观访谈、客观检查、诊断评估、治疗规划）工作流；2) 利用1,193份去标识化精神科电子健康档案构建技能增强的标准化病人；3) 设计双轨评估协议：客观比较（与EHR参考对比）和主观评估（临床过程质量）；4) 开发MentalEval五个领域评估器（沟通共情、访谈专业性、临床笔记质量、诊断严谨性、治疗适当性），采用基于评分标准的SFT和专家引导DPO训练。

Result: 22名临床医生验证MentalHospital临床逼真度评分为3.88/5；MentalEval与专家高度对齐，平均QWK为0.944；最强LLM在客观精神科能力上落后临床医生37.28个百分点，精神状态评估是主要瓶颈。

Conclusion: MentalHospital和MentalEval为LLM精神科临床能力提供了系统的评估框架，揭示了当前LLM与临床医生之间的显著差距，尤其是精神状态评估环节亟待提升。

与遥感交叉或改进方案: 交叉/改进可行性：65%。1) 将S.O.A.P.工作流类比为遥感数据处理pipeline（预处理→特征提取→分类→后处理），构建遥感全流程评估环境；2) 将标准化病人概念迁移至遥感，构建基于多源遥感数据（如Sentinel、Landsat）的合成场景作为标准测试用例；3) 双轨评估协议可应用于遥感模型评估：客观比较（与地面真值对比）和主观评估（专家对解译质量的评分）；4) MentalEval的多领域评估器思路可迁移至遥感，开发针对场景分类、变化检测、目标识别等子任务的专项评估器，采用SFT+DPO训练使其对齐遥感专家判断。

Abstract (英文): Large language models (LLMs) have shown strong performance on isolated psychiatric tasks, including dialogue, diagnosis, and treatment planning, yet existing benchmarks rarely simulate complete psychiatric clinical encounters. We introduce $\textbf{MentalHospital}$, a virtual evaluation environment for LLM-based psychiatric clinical encounters. MentalHospital instantiates the Subjective Interviewing, Objective Examination, Diagnostic Assessment, and Treatment Planning (S.O.A.P.) workflow, using skill-augmented standardized patients constructed from 1,193 de-identified psychiatric electronic health record (EHR) cases spanning all major ICD-11 categories and 76 disorders. Each encounter is assessed through a dual-track protocol that combines objective comparison against EHR-derived references with subjective assessment of clinical process quality. To scale specialist judgment, we develop $\textbf{MentalEval}$, five domain-specific evaluators covering communication empathy, interviewing professionalism, clinical-note quality, diagnostic rigor, and treatment appropriateness, trained with rubric-grounded SFT and expert-guided DPO. Survey responses from 22 clinicians support MentalHospital's clinical fidelity (3.88/5), while MentalEval achieves strong expert alignment with an average QWK of 0.944. Benchmarking shows that even the strongest LLM trails clinicians by 37.28 percentage points in objective psychiatric competence, with mental status assessment as a key bottleneck.

</details>


### [122] [Different Teachers, Different Capabilities: Sub-1B On-Device Distillation for Structured Text Enrichment](https://arxiv.org/abs/2607.08268)
*Vinay Kumar Chaganti*

Main category: cs.AI

TL;DR: 本文将大规模推理模型（deepseek-r1:8b）的知识蒸馏到小模型（Qwen3-0.6B）中，用于新闻文章的结构化提取（摘要+5个分类标签）。学生模型速度提升约48倍（0.8秒 vs 39秒），在摘要质量上恢复了教师模型与基线差距的58%，优于约束解码（+16.8分）和少样本提示（+4.9分）。研究表明，摘要质量的提升源于教师的推理能力而非规模；但推理教师训练的学生在短文本上易产生幻觉，而指令教师的学生更忠实。最终输出是一个按字段的路由映射，指导不同子任务选择最优引擎。


<details>
  <summary>Details</summary>
Motivation: 大规模模型在每项结构化提取任务上都存在高延迟问题，为了在移动端或边缘设备上以更低的延迟和成本获得可比的输出效果，需要将任务蒸馏到小型设备端模型。

Method: 使用8B推理教师模型（deepseek-r1:8b）蒸馏至0.6B学生模型（Qwen3-0.6B），采用QLoRA微调，3个随机种子。设置两个对照教师：同规模非推理教师和更大的托管管道。采用盲审、无参考、三位评审小组对所有方案打分，对比两个非蒸馏基线（少样本提示、约束解码）。

Result: 学生模型速度0.8秒/篇 vs 教师39秒/篇，恢复教师-基线差距的58%。摘要质量优于约束解码（+16.8分）和少样本提示（+4.9分）。同规模非推理教师训练的模型不优于未调优基线。推理教师提升写作质量，托管管道提升标签多样性，同规模指令教师的学生在22篇短文本上更忠实（74 vs 55篇忠实）。

Conclusion: 知识蒸馏有效，但不同教师的能力存在分工：推理教师擅长写作质量，托管管道擅长标签多样性，指令教师擅长忠实性。不存在单一引擎在所有字段胜出，最终产出是按字段的路由映射，用于设备端富化任务。

与遥感交叉或改进方案: 交叉/改进可行性：90%。遥感领域有大量结构化信息提取需求（如场景分类、目标检测、变化检测），该蒸馏框架可直接迁移：使用大模型（如遥感基础模型或视觉-语言模型）作为推理教师，蒸馏到轻量级遥感模型用于星上或边缘端处理。需注意：（1）遥感样本通常包含多源异构数据（光学、SAR、高光谱），需设计适配的JSON结构化模板；（2）推理教师的教学能力对忠实性敏感，遥感任务对幻觉容忍度更低，建议引入事实一致性验证或对比学习增强学生模型的地面真实性；（3）路由映射思想可应用于多任务遥感场景，如根据不同卫星平台的计算资源动态选择不同规模的推理引擎。

Abstract (英文): High-volume structured extraction pays a large model's latency on every item, so distilling the task into a small on-device model is attractive: comparable output at a fraction of the time and cost. We measure what that distillation actually delivers, per sub-task. Each news article is mapped to one JSON object with a short summary and five categorical labels. We distill an 8B reasoning teacher (deepseek-r1:8b) into a 0.6B student (Qwen3-0.6B; QLoRA, three seeds), and add two teacher controls: a same-size non-reasoning teacher and a larger managed pipeline. A blinded, reference-free, three-judge panel scores every arm against the full article, alongside two non-distillation baselines, few-shot prompting and constrained decoding. The student runs at about 0.8 s per article against the teacher's 39 s, and recovers 58% of the base-to-teacher gap on summary quality, beating its primary baseline (constrained decoding) by +16.8 points and few-shot prompting by a secondary +4.9. A same-size non-reasoning teacher trains a student no better than the untuned base, so the summary gain follows from the teacher's reasoning nature rather than its scale. Capabilities then split by teacher: the reasoning teacher transfers writing quality and the managed pipeline transfers label diversity, while a same-size instruction teacher's students stay more grounded on the 22 short, thin-source articles in the 93-item test set (74 versus 55 faithful), where the reasoning-lineage student fabricates. That grounding difference is a consistent ordering rather than a significant aggregate effect, and the subgroup is small, so we report it as a direction. Because no single engine wins every field, the deliverable is a per-field routing map for on-device enrichment.

</details>


### [123] [PolyUQuest: Verifiable Structure-Aware Web RAG over Heterogeneous Graphs](https://arxiv.org/abs/2607.08269)
*Ying Liu,Yi Ye,Quanyu Feng,Mingxi Ye,Mingtao Zhang,Haoyang Li,Chen Jason Zhang,Qing Li*

Main category: cs.AI

TL;DR: PolyUQuest 提出了一种基于异构图的、结构感知的 Web RAG 框架，通过融合页面间超链接拓扑、页面内 DOM 层次结构和跨页面实体-关系知识，并采用两级路由器将查询分配到三种检索模式，实现了可验证、高准确率且低令牌消耗的检索增强生成。


<details>
  <summary>Details</summary>
Motivation: 现有 RAG 系统将网页视为扁平文本，忽略了 HTML 中丰富的结构和语义信息，导致检索不准确且答案可验证性差。

Method: 构建包含超链接拓扑、DOM 层次和实体-关系知识的异构图；设计两级路由器，将查询路由到直接块检索、跨页面图遍历或多跳实体推理三种模式之一；每个引用块携带源页面、标题路径和实体链接以实现完全可验证。

Result: 在包含 4,240 个页面、31,086 个 DOM 块、29,119 个实体和 37,680 个关系的 PolyU 官方网站数据集上，PolyUQuest 在答案正确性、覆盖率和忠实度上优于现有 RAG 系统，且显著减少了每查询的 LLM 令牌消耗。

Conclusion: PolyUQuest 证明了结构感知的 Web RAG 在准确性和效率上的优越性，并已准备部署为学生面对的问答服务。

与遥感交叉或改进方案: 交叉/改进可行性：85%。遥感领域同样存在丰富的结构化数据（如地理元数据、传感器层级关系、空间拓扑关系），可借鉴 PolyUQuest 的异构图构建思路：将遥感影像的元数据（传感器、波段、时间）建模为实体-关系图，将影像金字塔层级视为 DOM 层次结构，将空间相邻关系视为跨影像拓扑链接；两级路由机制可分别处理单影像块检索、多影像空间推理（如变化检测）和多模态实体推理（如地物分类与属性关联），从而实现结构化、可验证的遥感知识问答。

Abstract (英文): Existing retrieval-augmented generation (RAG) systems treat web pages as flat text, losing the structural and semantic signals encoded in HTML. We present PolyUQuest, a verifiable, structure-aware web RAG framework built on a heterogeneous graph that unifies hyperlink topology between pages, DOM hierarchy within pages, and entity-relation knowledge across pages. A two-tier router dispatches each query to one of three retrieval modes matched to its structural need, including direct block retrieval, cross-page graph traversal, and multi-hop entity reasoning. Every answer is fully verifiable, as each cited block carries its source page, heading path, and entity links so that users can trace any claim back to its structural evidence. We evaluate on the official websites of the Hong Kong Polytechnic University (PolyU), comprising 4,240 pages, 31,086 DOM blocks, 29,119 entities, and 37,680 relations, together with a multi-type evaluation benchmark. PolyUQuest outperforms existing RAG systems in answer correctness, coverage, and faithfulness, while consuming significantly fewer LLM tokens per query. The demonstration provides an interactive interface for inspecting cited answers, comparing retrieval traces across routing modes, and exploring evidence graph paths. PolyUQuest is being prepared for deployment as a student-facing QA service at PolyU.

</details>


### [124] [Understanding Axes of Difficulty For Long Context Tasks Via PredicateLongBench](https://arxiv.org/abs/2607.08284)
*Siddhartha Jain,Ameya Velingker*

Main category: cs.AI

TL;DR: PredicateLongBench是一个用于压力测试大语言模型长上下文推理能力的基准，通过让模型识别满足给定谓词约束的最长连续子序列，并沿着多个难度轴系统地扩展任务难度，揭示了当前前沿模型在长上下文理解方面的局限性。


<details>
  <summary>Details</summary>
Motivation: 现有长上下文评估基准多侧重于平均性能，存在饱和或缺乏鲁棒性的问题，且缺少系统性地沿多个难度轴扩展任务难度来探测模型性能的方法。

Method: 提出PredicateLongBench基准，要求模型在长输入中识别满足给定谓词/约束（如字典序）的最长连续子序列。提供两种互补的数据生成管线：全合成随机字符串和基于真实文档保留分布特性的采样。系统性地探索多个不同的难度轴来测试长上下文理解的多个方面。

Result: 前沿模型在沿多个难度轴扩展任务难度时表现挣扎，证明了该基准在理解当前长上下文能力局限性方面的有效性。

Conclusion: PredicateLongBench中的任务虽具挑战性，但概念上简单，不需要基于LLM的生成或评判器，为评估长上下文推理提供了更系统、更鲁棒的方法。

与遥感交叉或改进方案: 交叉/改进可行性：85%。该基准的谓词约束机制可迁移到遥感领域的长序列分析任务中：1) 沿时间轴设置约束（如NDVI值的单调性、植被指数变化模式）来检测遥感时间序列中的关键片段；2) 沿空间轴设置约束（如光谱特征的字典序、空间梯度条件）来识别满足特定条件的连续像素带；3) 多难度轴设计可应用于高光谱影像的特征子集选择，评估模型在不同光谱分辨率下的推理能力；4) 合成数据与真实数据双管线设计可直接迁移到遥感数据增强与模型鲁棒性评估中。

Abstract (英文): Large language models (LLMs) have demonstrated rapidly improving long-context capabilities, prompting a wave of benchmarks designed to evaluate them. However, existing long-context evaluations - from Needle-in-a-Haystack (NIAH) tests to more recent multi-hop reasoning and summarization tasks - predominantly measure average-case performance, and many are either saturated or lack robustness. Notably absent is a systematic way to probe how models perform as we scale up the difficulty of tasks along various axes. We address this gap by proposing PredicateLongBench, a benchmark that stress-tests long-context reasoning by asking models to identify the longest contiguous subsequence of words in a long input that satisfies given predicates/constraints (e.g., lexicographic ordering), drawn from a broader predicate class. The central innovation of our benchmark is the identification and systematic exploration of multiple different axes of difficulty which test multiple aspects of long context understanding. We provide two complementary generation pipelines - a fully synthetic setup using random word-like strings, and a real-world setup that samples words from natural documents while preserving their distributional properties. We find that frontier models struggle to perform well as we scale up the difficulty of tasks along our axes, demonstrating the utility of our benchmark in understanding the limitations of current long-context capabilities. Furthermore, the tasks in PredicateLongBench, though challenging, are conceptually simple and do not require LLM-based generations or judges.

</details>


### [125] [Psychological Competence as a Missing Dimension in AI Evaluation](https://arxiv.org/abs/2607.08285)
*Marcos Economides,Paul M. Sacher,Samuel Salzer,Alexis Michelle Abellar,Fendi Tsim,Antoine Ferrère*

Main category: cs.AI

TL;DR: 本文提出"心理能力"作为人面对AI系统评估中缺失的关键维度，定义为AI系统在交互中支持用户认知、情绪解读和行为决策的能力，并构建了包含框架、语气、感知权威性、响应性、不确定性处理和对话引导等特性的概念框架，建议通过场景化探针、结构化人工评估和模型辅助评估方法进行评测。


<details>
  <summary>Details</summary>
Motivation: 现有AI评估框架主要关注技术性能（准确性、鲁棒性、推理能力、政策合规性），但对于以自然语言直接与用户交互的AI系统（如顾问、教练、导师、伴侣）而言，这些评估维度不足以衡量其对用户认知、情绪、信念、信任和决策的实际影响。

Method: 基于行为科学和人机交互研究成果，提出"心理能力"概念框架，明确其核心领域和边界，并设计三种评估方法：场景化探针（scenario-based probes）、结构化人工评估（structured human evaluation）和模型辅助评估方法（model-assisted evaluation methods）。

Result: 明确了心理能力的定义和核心交互特性（框架、语气、感知权威性、响应性、不确定性处理、对话引导），指出了现有评估方法的不足，并提供了多维度的评估方案框架。

Conclusion: 心理能力应成为模型提供商、部署组织、研究人员和监管机构评估人面对AI系统时的重要考量维度，直接评估交互的心理效应比仅关注模型技术指标更具实际意义。

与遥感交叉或改进方案: 交叉/改进可行性：75%。遥感领域的人机交互系统（如地理空间智能分析辅助系统、遥感解译决策支持系统）可引入该框架评估专家与AI协作时的认知偏差、过度信任或不当不确定性表达等问题。具体方案：1）在遥感解译辅助系统中设计场景化探针，评估AI对分析师判读决策的影响；2）针对遥感预测的不确定性表达，评估AI的"不确定性处理"能力是否导致用户误判；3）将心理能力的对话引导和感知权威性维度引入遥感交互界面设计评估。

Abstract (英文): Current AI evaluation frameworks focus primarily on technical performance, including accuracy, robustness, reasoning ability, and policy compliance. These measures remain essential, but they are not sufficient for systems that interact directly with users through natural language. Human-facing AI systems are increasingly used as advisors, coaches, tutors, and companions. In these roles, their responses can shape how users reason, interpret emotions, form beliefs, calibrate trust, and make decisions. The relevant unit of evaluation is therefore not only the model, but the human-AI interaction. This paper introduces psychological competence as a missing dimension in AI evaluation. We define psychological competence as the capacity of a human-facing AI system to support user cognition, emotional interpretation, and behavioral decision-making in ways that are appropriate to the user, context, and purpose of the interaction. This includes interaction properties such as framing, tone, perceived authority, responsiveness, uncertainty handling, and conversational guidance. Existing evaluation approaches capture parts of this problem but rarely assess these psychological effects directly. Drawing on behavioral science and human-AI interaction research, we outline a conceptual framework for psychological competence and its core domains. Rather than proposing a specific benchmark, we define the construct, clarify its boundaries, and describe how it may be assessed through scenario-based probes, structured human evaluation, and model-assisted evaluation methods. We argue that psychological competence should become a core consideration for model providers, deploying organizations, researchers, and regulators concerned with the real-world effects of human-facing AI systems.

</details>


### [126] [INTENT: An LSTM Framework for Vehicle Intention Prediction in Intersection Scenarios with Comprehensive Ablation Analysis](https://arxiv.org/abs/2607.08316)
*Logine M. Zaki,Catherine M. Elias*

Main category: cs.AI

TL;DR: 本文提出INTENT框架，利用LSTM模型在交叉口场景下提前2秒预测车辆意图（直行、左转、右转），在InD数据集上达到99.71%的准确率。


<details>
  <summary>Details</summary>
Motivation: 提升自动驾驶车辆在复杂交互场景（如交叉口、环岛、紧急情况）下的安全性和敏捷性，通过预测驾驶员意图来辅助避障和轨迹预测。

Method: 提出INTENT框架，采用LSTM模型，在交叉口事件发生前2秒预测车辆意图（直行、左转、右转），并在InD数据集上进行多种模型实验和消融研究。

Result: 在InD数据集上达到99.71%的准确率。

Conclusion: INTENT框架能够有效提前预测车辆驾驶意图，显著提升自动驾驶系统在复杂场景下的安全性和决策能力。

与遥感交叉或改进方案: 交叉/改进可行性：85%。遥感领域中的车辆轨迹预测与行为分析（如卫星视频中的车辆运动模式识别、交通流监测）可借鉴本文的LSTM意图预测框架。具体方案：1）将INTENT框架迁移至遥感卫星视频中的车辆轨迹预测，对交叉口等复杂场景的车辆意图进行远程监测；2）结合遥感高分辨率影像提取车辆时空特征，使用LSTM预测车辆转向意图，辅助智能交通系统进行宏观交通态势评估。

Abstract (英文): Vehicle intention prediction is a pivotal aspect in the agility and safety of autonomous vehicles in all driving scenarios; if genuine enhancement of autonomous vehicles are required, we need to make them adopt human interpretation of driver's intention especially in cases that require a lot of human interaction as well as complex driving behaviors like the ones at intersections, roundabouts and emergency cases such as sudden stops where vehicle intention prediction helps in taking the correct evasive action within a real time period where every second of action makes an impact and can prevent a catastrophe from taking place. In the worst case, it helps minimize the damage and make safety a priority. Intention prediction can also be used to enhance trajectory prediction (intention conditioned trajectory prediction). In this study, The INTENT framework is proposed using LSTM model to predict the vehicle's intention at intersections 2 seconds ahead of the event occurrence to predict whether the cars in intersections are going straight, turning left, or turning right. Various model experiments and ablation study are thoroughly tested on InD dataset achieving 99.71% accuracy.

</details>


### [127] [Blind-Spots-Bench: Evaluating Blind Spots in Multimodal Models](https://arxiv.org/abs/2607.08317)
*Matteo Santelmo,Xiuying Wei,Israa Fakih,Felix Bauer,Juan Garcia Giraldo,Chengkun Li,Etienne Bamas,Emmanuel Abbé*

Main category: cs.AI

TL;DR: 本文提出了一个名为blind-spots-bench的新基准，专门设计用于暴露当前AI模型在人类看似简单任务上的盲点。该基准包含235个从AI课程学生收集的样本，并通过自动化评估流水线测试了多种开源和闭源语言、视觉-语言及图像生成模型。结果显示，闭源前沿模型在性能上比开源模型高出约10%，且没有任何单一模型在所有任务类型上占优，部分任务对所有模型仍具挑战性。


<details>
  <summary>Details</summary>
Motivation: 现有AI模型在标准基准上表现优异，但在人类认为几乎微不足道的任务（如操作字符串或画五条腿的狗）上仍会失败，说明现有基准可能低估了当前系统中的持久盲点。因此，需要一个专门暴露这些盲点的诊断性压力测试基准。

Method: 从AI课程学生中收集原始问题，进行清洗并用结构化参考解决方案进行注释，提出了适用于235个样本的任务分类法。开发了自动化评分流水线，评估多种模型，包括开源和闭源语言模型、视觉-语言模型以及图像生成模型。

Result: 闭源前沿模型在blind-spots-bench上比开源模型性能高出约10%，即便它们在现有基准上表现相当。细粒度分析显示，没有单一模型在所有任务类型上占优，且部分任务对所有评估模型仍具有挑战性。

Conclusion: blind-spots-bench作为诊断性压力测试，能有效识别当前现代模型中的具体弱点，具有重要价值。

与遥感交叉或改进方案: 交叉/改进可行性：75%。遥感领域同样存在标准基准（如场景分类、语义分割）上表现优异但在边缘案例（如复杂地物边界、罕见目标检测）上失败的盲点。可将本基准的构建思路迁移至遥感领域：(1) 收集遥感领域中人类易解译但模型常出错的任务（如细粒度地物区分、遮挡目标识别）；(2) 建立类似的任务分类法（如空间推理、几何变换、常识推理等）；(3) 开发自动化评分流水线，评估现有遥感基础模型（如Swin Transformer、遥感专用大模型）的盲点情况。

Abstract (英文): Modern AI models achieve strong performance on many established benchmarks, yet they still fail on tasks that humans find almost trivial, such as manipulating a string or drawing a dog with five legs. These examples suggest that existing benchmarks may under-measure persistent blind spots in current systems. We introduce $\texttt{blind-spots-bench}$, a benchmark designed to expose such blind spots through tasks that appear simple for humans but remain challenging for modern AI. We collect raw questions from students in an AI course, clean and annotate them with structured reference solutions, and propose a task taxonomy tailored to the resulting dataset of 235 samples. We further develop an automated grading pipeline to evaluate a wide range of models, including open-weight and closed-source language, vision-language, and image-generation models. Our analysis on $\texttt{blind-spots-bench}$ reveals that closed-source frontier models can substantially outperform open-weight models with even $\approx10\%$ gap, even when they attain comparable performance on existing benchmarks. A more fine-grained analysis shows that no single model dominates across all task types, and that some tasks remain challenging for all evaluated models. These results highlight the value of $\texttt{blind-spots-bench}$ as a diagnostic stress test for identifying concrete weaknesses in current modern models.

</details>


### [128] [MobiDiff: Semantic-Aware Multi-Channel Discrete Diffusion for Human Mobility Data Generation](https://arxiv.org/abs/2607.08357)
*Rongchao Xu,Lin Jiang,Dahai Yu,Ximiao Li,Taichi Liu,Desheng Zhang,Yuan Tian,Guang Wang*

Main category: cs.AI

TL;DR: MobiDiff是一个端到端离散扩散框架，通过对多通道语义骨架进行直接去噪来生成合成人类移动轨迹数据，避免了现有方法中的昂贵插值和潜在轨迹构建，在三个真实数据集上表现出高保真度、隐私保护和更快的推理速度（平均比GeoGen快5.3倍）。


<details>
  <summary>Details</summary>
Motivation: 真实世界的人类移动数据收集成本高且因隐私问题难以共享，现有扩散方法依赖连续或潜在时空轨迹，无法原生建模具有显式区域、活动、时间和间隔结构的离散语义事件。

Method: MobiDiff将每个签到事件分解为空间、活动和时间三个通道，采用结构化的事件级、组级和通道级掩码策略，联合捕获轨迹级别的移动模式和事件内依赖关系，通过直接对多通道语义骨架去噪实现端到端离散扩散生成。

Result: 在亚特兰大、波士顿和西雅图三个大规模真实数据集上，MobiDiff有效保留了轨迹长度和时间间隔分布，在更广泛的移动统计指标上保持竞争力，推理速度平均比GeoGen快5.3倍。

Conclusion: 离散扩散提供了一种可解释且高效的合成移动数据生成框架，有望缓解真实移动数据获取困难与隐私保护的矛盾。

与遥感交叉或改进方案: 交叉/改进可行性：85%。具体方案：（1）将离散扩散框架应用于遥感轨迹数据（如车辆GPS轨迹、船舶AIS数据）的合成生成，解决遥感移动数据的隐私共享难题；（2）将通道分解思想引入遥感时空数据建模，将遥感影像中的光谱波段、时间序列和空间位置分别作为独立通道进行结构化掩码去噪；（3）MobiDiff的高效推理特性可服务于遥感数据增强和稀缺场景下的轨迹补全任务。

Abstract (英文): Human mobility data are essential for transportation optimization, urban planning, and resource allocation, yet real-world mobility data are costly to collect and difficult to share due to privacy concerns. Recent diffusion-based methods have shown promise in synthesizing realistic mobility patterns, but they typically rely on continuous or latent spatio-temporal traces, limiting their ability to natively model discrete semantic events with explicit region, activity, time, and interval structures. To address this issue, we introduce MobiDiff, an end-to-end discrete diffusion framework that efficiently generates mobility data by directly denoising multi-channel semantic skeletons, avoiding the costly interpolation, latent trace construction, and coarse-to-fine realization pipelines widely used in existing diffusion-based methods. Specifically, MobiDiff decomposes each human check-in event into spatial, activity, and temporal channels, and employs structured event-, group-, and channel-level masking to jointly capture trajectory-level mobility patterns and within-event dependencies. We evaluate generation fidelity, privacy-preserving, and efficiency on three large-scale real-world datasets from Atlanta, Boston, and Seattle. Results show that MobiDiff effectively preserves trajectory length and temporal interval distributions while remaining competitive across broader mobility statistics; it is also much faster than state-of-the-art methods, e.g., 5.3$\times$ faster than GeoGen on average during inference. These findings suggest that discrete diffusion offers an interpretable and efficient framework for synthetic mobility data generation.

</details>


### [129] [FedOPAL: One-Shot Federated Learning via Analytic Visual Prompt Tuning](https://arxiv.org/abs/2607.08368)
*Lingyu Qiu,Daniela Annunziata,Stefano Izzo,Fabio Giampaolo,Francesco Piccialli*

Main category: cs.AI

TL;DR: FedOPAL框架通过视觉提示作为特征修正器，结合局部近端约束，解决分析联邦学习在非独立同分布数据下的特征流形错位问题，在保持零服务器训练成本的同时实现与迭代方法相当的精度。


<details>
  <summary>Details</summary>
Motivation: 现有一次联邦学习中的迭代微调或知识蒸馏方法存在服务器端计算成本高、超参数敏感等问题；分析联邦学习虽能高效无梯度聚合，但在非独立同分布数据环境下静态特征假设失效，导致特征流形错位，模型性能严重下降。

Method: 提出FedOPAL框架，利用视觉提示作为特征修正器，通过施加局部近端约束主动校正异构数据的特征分布，将其映射到线性可分离空间，以满足分析联邦学习的理论假设。

Result: 在多个基准测试上，FedOPAL显著优于原始分析方法，精度与最先进的迭代方法相当，同时保持零服务器端训练成本。

Conclusion: FedOPAL为边缘端大模型高效协作提供了新的工程范式，在通信效率与模型性能之间取得了良好平衡。

与遥感交叉或改进方案: 交叉/改进可行性：90%。遥感数据通常存在严重的非独立同分布问题（不同区域、不同传感器、不同时相），FedOPAL的视觉提示特征修正机制天然适配该场景。具体方案：1) 将遥感影像的域差异（如光谱分布差异）建模为特征流形错位问题，利用FedOPAL的局部近端约束进行校正；2) 在多源遥感联邦学习场景中，用视觉提示对齐不同卫星/传感器的特征空间；3) 在边缘端推理时，无需迭代通信即可完成模型聚合，极大降低遥感数据跨机构共享的带宽成本。

Abstract (英文): With the widespread deployment of basic models in edge intelligence, communication bandwidth has become a core bottleneck restricting the scalability of federated learning. Although one-shot federated learning alleviates this problem by minimizing communication rounds, existing iterative fine-tuning or knowledge distillation methods still face challenges such as high server-side computational costs and hyperparameter sensitivity. Analytical federated learning achieves efficient gradientfree aggregation using least-squares closed-form solutions, but in environments with non-independent and identically distributed data, its static feature assumptions fail, leading to feature manifold misalignment and severely impairing model performance. To address this contradiction, this paper proposes the FedOPAL framework. This framework adapts the visual prompts as feature rectifiers, actively correcting the feature distribution of heterogeneous data to a linearly separable space by applying local proximal constraints, thereby satisfying the theoretical assumptions of analytical federated learning. Experimental results show that FedOPAL not only significantly outperforms the original analytical methods on several benchmarks, but also achieves accuracy comparable to state-of-the-art iterative methods while maintaining zero server-side training costs, providing a new engineering paradigm for efficient collaboration of large models on the edge.

</details>


### [130] [Towards Mechanistically Understanding Why Memorized Knowledge Fails to Generalize in Large Language Model Finetuning](https://arxiv.org/abs/2607.08393)
*Lu Dai,Ziyang Rao,Yili Wang,Hanqing Wang,Hao Liu,Hui Xiong*

Main category: cs.AI

TL;DR: 本文发现并形式化了LLM微调中的"知道-使用差距"（Knowing-Using Gap），即模型能记住新知识但无法用于推理。通过自修补（self-patching）干预技术，提出知识回路错位假说：记忆的表征存在于内部但未被路由到有效计算层。基于此设计简单启发式策略，可恢复58-75%的泛化失败性能。


<details>
  <summary>Details</summary>
Motivation: 揭示LLM微调中知识记忆与下游推理能力之间的不一致现象（Knowing-Using Gap），并理解其背后的内部机制。

Method: 1）形式化定义Knowing-Using Gap（准确率差距与时间滞后）；2）提出自修补（self-patching）干预技术，通过定位激活位置并重定位表征来监测知识内部空间渗透动态；3）提出知识回路错位（knowledge-circuit misalignment）假说进行解释；4）设计简单启发式策略验证诊断发现的可实用性。

Result: 所提启发式策略可恢复58-75%的oracle泛化性能上限（headroom），且跨领域实验验证了结果的稳健性。

Conclusion: LLM记忆新知识后无法用于推理的根本原因是知识表征存在于内部但未路由到计算有效的层；通过干预表征路由可显著缓解该问题。

与遥感交叉或改进方案: 交叉/改进可行性：85%。1）遥感领域存在大量新知识注入需求（新传感器、新地物类别、新变化模式），微调时同样面临记住但不会用的困境，可应用self-patching诊断机制检测遥感知识回路错位；2）启发式路由策略可直接用于遥感基础模型微调，提升如变化检测、地物分类等下游任务的泛化能力；3）遥感数据具有空间结构特征，可进一步扩展self-patching至空间位置表征的干预分析。

Abstract (英文): Fine-tuning LLMs to inject new knowledge faces a critical challenge: LLMs can quickly memorize new facts, yet fail to use them for downstream reasoning tasks. We formalize this failure as the \textit{\textbf{Knowing--Using Gap}}, characterized by an accuracy gap and a temporal lag between memorization and generalization. To understand this phenomenon, we fine-tune LLMs with unseen knowledge and monitor the spatial permeation dynamics of the knowledge internally using a novel intervention technique called self-patching. Self-patching identifies activation locations where relocating representations substantially improves failed generalization cases. These results are consistent with a knowledge-circuit misalignment hypothesis: memorized representations can exist internally but may not be routed to computation-effective layers. To demonstrate the practicality of this diagnostic finding, we design a simple heuristic strategy which recovers 58--75\% of the oracle headroom in generalization failure. Experiments are done cross-domain for the robustness of this finding.

</details>


### [131] [Game Theory Driven Multi-Agent Framework Mitigates Language Model Hallucination](https://arxiv.org/abs/2607.08403)
*Runzhe Liu,Biquan Bie,Zihao Wang,Yuchao Ma,Yexin Liu,Xinghai Li,Harry Yang,Wenbo Yang,Jinzhe Cao,Shengyang Tao*

Main category: cs.AI

TL;DR: 本文提出G-Frame自适应多智能体框架，通过贝叶斯和团队博弈原理实现高质量数据合成与模型训练的自动闭环，生成36万条思维链和20万条问答对，训练出7B模型OmniChem，在化学基准测试中达到与GPT-4o mini相当的性能，并将幻觉率降低79.46%。


<details>
  <summary>Details</summary>
Motivation: 轻量级大型语言模型在基于规则的科学领域应用中，倾向于模仿语言模式而非复现公理推理，导致频繁产生幻觉，严重限制了其在科学领域的应用。

Method: 提出G-Frame自适应多智能体框架，整合贝叶斯原理和团队博弈机制，通过结构化推理强制内化领域约束，实现数据合成与模型训练的自动闭环。合成了363,045条思维链和199,589个问答对，训练出7B参数规模的OmniChem模型。

Result: OmniChem在自定义基准测试和ChemBench上达到与GPT-4o mini相当的性能，幻觉率相比基础架构降低了79.46%，并在分子设计与合成规划方面展现出高级能力。

Conclusion: 该工作建立了一种利用自适应多智能体克服固有推理缺陷的可扩展范式，为加速专业科学领域的知识发现提供了可行路径。

与遥感交叉或改进方案: 交叉/改进可行性：85%。具体方案：(1) 将G-Frame框架迁移至遥感领域，针对遥感规则性知识（如辐射定标、几何校正、大气校正等物理模型）进行结构化推理训练，减少遥感大模型的物理规则违背幻觉；(2) 利用贝叶斯与团队博弈机制，对遥感解译中的多源数据（光学、SAR、高光谱）进行一致性约束推理，提升多模态遥感分析的鲁棒性；(3) 自动化合成遥感领域专用的思维链数据（如地物分类层次推理、变化检测逻辑链），构建领域专用轻量模型，降低对GPT-4等闭源大模型的依赖。

Abstract (英文): The application of lightweight Large Language Models in rule-based scientific domains remains severely limited by their tendency to mimic linguistic patterns rather than reproduce axiomatic reasoning, causing frequent hallucinations. Here, we show that G-Frame, an adaptive multi-agent framework integrating Bayesian and team game principles, establishes an automated closed-loop for high-quality data synthesis and model training. By forcing the internalization of domain constraints through structured reasoning, we synthesized a specialized corpus of 363,045 chains-of-thought and 199,589 question-answer pairs. The resulting 7B model OmniChem achieves performance parity with GPT 4o mini on custom benchmarks and ChemBench while exhibiting a 79.46% reduction in hallucinations relative to its base architecture. We further demonstrate the advanced capabilities of OmniChem in molecular design and synthesis planning. This work establishes a scalable paradigm utilizing adaptive multi-agents to overcome inherent reasoning deficiencies, offering a feasible pathway for accelerating knowledge discovery in specialized scientific fields.

</details>


### [132] [OmniFood-Bench: Evaluating VLMs for Nutrient Reasoning and Personalized Health Advice](https://arxiv.org/abs/2607.08423)
*Qian Jiang,Zhecheng Shi,Jingpu Yang,Zirui Song,Miao Fang*

Main category: cs.AI

TL;DR: 本文提出OmniFood-Bench基准，用于评估大型视觉语言模型在食品系统中的渐进式推理能力（从基础感知到定量推理再到安全关键建议）。实验发现模型存在严重的"语义-物理鸿沟"：在菜品命名上接近人类水平，但在质量估计和糖尿病高危患者建议方面表现灾难性失败。


<details>
  <summary>Details</summary>
Motivation: 现有基准主要关注粗粒度的食品分类任务，无法评估真实饮食管理所需的多步推理链（从识别隐藏成分到估计物理质量再到合成安全关键医疗建议）。视觉外观与内在营养组成之间存在"系统性信息不对称"，亟需一个能全面评估VLM在食品领域可信度的基准。

Method: 基于MM-Food-100K数据集构建OmniFood-Bench基准，将评估分为三个渐进能力层次：基础感知（成分与烹饪方法）、定量推理（份量大小与营养分析）和安全关键建议（疾病特定推荐）。在六个SOTA视觉语言模型（包括gpt-5.1、gemini-3-flash、qwen3-vl-8B）上进行系统评估。

Result: 实验揭示了显著的"语义-物理鸿沟"：模型在菜品命名任务上接近人类准确率，但在质量估计任务中表现灾难性失败，并且经常为高风险糖尿病档案产生良性的幻觉建议。

Conclusion: 该工作为部署于公共卫生领域的自主智能体建立了严格的可信度标准，指出了当前VLM在食品营养定量推理和安全关键决策方面的根本性缺陷。

与遥感交叉或改进方案: 交叉/改进可行性：65%。遥感与食品分析在视觉-物理量映射问题上具有高度相似性。具体交叉方案：(1) 将"语义-物理鸿沟"概念迁移到遥感中的地物分类与定量参数反演问题（如识别作物类型与估算产量），构建类似的多级推理基准；(2) 食品质量估计中遇到的视觉遮挡和尺度模糊问题与遥感中的混合像元和空间分辨率限制高度相关，可共享领域自适应或物理约束网络；(3) OmniFood-Bench的安全关键建议评估范式可推广至遥感中的灾害应急决策场景，评估模型从遥感影像识别灾害类型→定量评估损失→生成应急建议的完整推理链可信度。

Abstract (英文): The rapid integration of Large Vision-Language Models (VLMs) into critical infrastructure promises to revolutionize
  personalized healthcare and dietary management. However, in the domain of food systems, autonomous agents face a
  unique and persistent challenge: the "Systemic Information Asymmetry" between visual appearance and intrinsic
  nutritional composition. Existing benchmarks primarily focus on coarse-grained classification tasks, such as food
  category recognition, which fail to evaluate the intricate reasoning chain required for real-world dietary management
  -- specifically, the ability to traverse from identifying hidden ingredients to estimating physical mass, and finally
  synthesizing safety-critical medical advice. In this paper, we introduce OmniFood-Bench, a comprehensive benchmark
  constructed from the MM-Food-100K dataset. Unlike previous works, OmniFood-Bench evaluates VLMs across three
  progressive capabilities: Basic Perception (Ingredients & Cooking Methods), Quantitative Reasoning (Portion Size &
  Nutritional Profiling), and Safety-Critical Advisory (Disease-Specific Recommendations). We evaluate six
  state-of-the-art VLMs, including gpt-5.1, gemini-3-flash, and qwen3-vl-8B. Our extensive experiments reveal a
  startling "Semantic-Physical Gap": while models achieve near-human accuracy in naming dishes, they exhibit
  catastrophic failure in mass estimation and frequently hallucinate benign advice for high-risk diabetic profiles. This
  work establishes a rigorous standard for trustworthiness in autonomous agents deployed for public health. The code
  and datasets are available in: https://anonymous.4open.science/r/OmniFood-Bench-7D0B

</details>


### [133] [Applying JEPA-Style Predictive Learning to JA4-Derived Network Fingerprints](https://arxiv.org/abs/2607.08465)
*Javier Izquierdo,Aygul Zagidullina*

Main category: cs.AI

TL;DR: 本文提出JA4-JEPA，将JEPA自监督预测学习范式应用于网络指纹（JA4系列），在TLS/DNS/SSH协议族分类任务上达到0.9899余弦相似度和0.9220 kNN准确率，证明即便在视图不完整的情况下，JEPA风格的学习也能生成有效的网络指纹嵌入。


<details>
  <summary>Details</summary>
Motivation: 探索JEPA式的潜在空间预测目标（而非重构原始输入）是否能有效适用于紧凑的网络指纹表示学习，填补JEPA在网络安全/网络指纹领域的应用空白。

Method: 基于Transformer架构的JA4-JEPA模型，使用JA4、JA4H、JA4S、JA4X四个子字段作为输入视图，在JA4DB和CIC-IDS-2017两个数据源上训练（约397K样本），采用JEPA风格的潜在预测匹配目标编码器输出的方式进行自监督学习。

Result: 在39,416个保留测试样本上，模型在协议族分类（TLS、DNS、SSH）任务中达到了0.9899的余弦相似度和0.9220的kNN准确率。

Conclusion: JEPA风格的预测式学习能够从JA4衍生的网络指纹中产生有用的嵌入表示，即使在跨数据源的视图覆盖不完整的情况下也能取得良好效果。

与遥感交叉或改进方案: 交叉/改进可行性：70%。遥感领域与网络指纹存在天然的"信号→特征嵌入→下游分类"的共性。具体交叉方案：(1) 将JA4-JEPA中的多视图（JA4/JA4H/JA4S/JA4X）设计思想迁移至遥感多模态数据（如SAR+光学+高光谱），采用JEPA潜在预测替代传统的像素级或特征级重构，提高跨模态表示学习效率；(2) 遥感图像中的场景分类可借鉴其kNN冻结探测评估范式，在预训练阶段使用JEPA目标（预测目标编码器输出）而非对比学习，规避对大量负样本的依赖；(3) 针对遥感数据中常见的标签稀疏问题，JA4-JEPA在视图不完整下的鲁棒性训练策略可直接用于遥感多时相/多云遮挡场景的表示学习。

Abstract (英文): I-JEPA and V-JEPA learn by matching latent predictions to target encoder outputs rather than regenerating the original input, and this has worked well for images and video. We explore whether the same objective works for compact network fingerprints. We built JA4-JEPA, a Transformer-based model trained on JA4, JA4H, JA4S, and JA4X subfields drawn from JA4DB and CIC-IDS- 2017. The training data combines roughly 397K samples from both sources, though no single sample contains all four view families. We evaluated the learned representations with a frozen kNN probe on protocol-family classification across TLS, DNS, and SSH. On 39,416 heldout samples the model achieved a cosine similarity of 0.9899 and a kNN accuracy of 0.9220. These results indicate that JEPA-style predictive learning can produce useful embeddings from JA4-derived fingerprints, even with incomplete view overlap across sources.
  Keywords: JA4, network fingerprinting, JEPA, predictive representation learning, self-supervised learning

</details>


### [134] [Drift-Aware Temporal Graph Rewiring (DATGR) for Adaptive Semantic Modeling in Biomedical Text](https://arxiv.org/abs/2607.08490)
*Bharathwaj Vijayakumar,Sahana K. Varadaraju*

Main category: cs.AI

TL;DR: 本文提出了一种漂移感知时序图重连（DATGR）框架，通过轻量级的、基于反馈的边权重重连机制（逻辑斯蒂更新规则）动态更新共现图的边，以建模生物医学概念随时间的语义漂移，无需为每个时间片重训练嵌入。在BIOMRC数据集上，相比静态基线，AUROC提升约0.066，AUPRC保持相当，展示了高效、可解释的时序语义变化捕捉能力。


<details>
  <summary>Details</summary>
Motivation: 生物医学语言随新发现快速演化，导致静态文本模型（如静态嵌入和共现图）无法捕捉语义漂移，在检索和知识发现任务中性能下降。现有方法需要为每个时间片重训练嵌入，计算成本高且不灵活。

Method: 提出DATGR框架，采用轻量级、反馈驱动的图重连机制：通过逻辑斯蒂更新规则动态调整共现边的权重，基于估计的语义漂移进行边级别的自适应更新，无需重训练完整嵌入。

Result: 在BIOMRC数据集上，DATGR的AUROC为0.699，优于静态基线的0.633（绝对提升约0.066）；AUPRC为0.738，与静态基线的0.744相当。表明漂移感知自适应在不损失精确率的前提下提升了链路预测的召回率。

Conclusion: 边级别的自适应机制能有效捕捉生物医学文本中的时序语义变化，且方法计算高效、可解释性强。

与遥感交叉或改进方案: 交叉/改进可行性：85%。遥感领域同样面临地表覆盖变化、传感器更新、新地物类型出现等导致的术语和概念漂移问题。具体交叉方案：（1）将DATGR应用于遥感时序文献或遥感影像解译标签中的概念演化建模，如湿地、城市边界等语义随时间的变化；（2）将逻辑斯蒂边重连机制迁移到遥感知识图谱的动态构建中，用于更新地物-关系共现权重；（3）结合遥感时间序列数据，用语义漂移检测引导特征选择或模型更新，提升长时序遥感分析任务的适应性。

Abstract (英文): Biomedical language evolves rapidly as new discoveries emerge, causing traditional text models to lose semantic fidelity over time. Static embeddings and co-occurrence graphs cannot capture such evolution, leading to performance degradation in retrieval and knowledge discovery tasks. This paper introduces a Drift-Aware Temporal Graph Rewiring (DATGR) framework that models concept evolution by dynamically updating co-occurrence edges based on estimated semantic drift. Instead of retraining embeddings for each time slice, DATGR performs lightweight, feedback-driven rewiring using a logistic update rule applied to edge weights. Evaluated on the Biomedical Multi-Relation Corpus (BIOMRC), the method achieved a mean Area Under the Receiver Operating Characteristic (AUROC) improvement of approximately 0.066 absolute difference (0.699 vs. 0.633) over a static baseline. Area Under the Precision-Recall Curve (AUPRC) remained comparable (0.738 vs. 0.744), showing that drift-aware adaptation enhances link-prediction recall without a loss in precision. These results demonstrate that edge-level adaptation effectively captures temporal semantic change in evolving biomedical text while remaining computationally efficient and interpretable.

</details>


### [135] [AI-guided stimuli discovery and generation to optimize facial emotion perception studies in autism](https://arxiv.org/abs/2607.08533)
*Kushin Mukherjee,Na Yeon Kim,Maren Wehrheim,Ralph Adolphs,Kohitij Kar*

Main category: cs.AI

TL;DR: 本文发现自闭症与神经典型成年人在面部情绪感知上的差异集中在少数诊断性表情上，而非均匀分布。通过训练群体特异性神经网络模型预测图像级判断，并利用GAN生成/筛选图像，在独立队列中验证了模型筛选的图像能放大群体差异，而合成的图像能缩小差异，建立了一个模型引导的框架来发现和转化揭示群体特异性感知差异的刺激。


<details>
  <summary>Details</summary>
Motivation: 现有研究对自闭症与神经典型成年人在面部情绪感知上的差异结论不一致，可能源于传统方法对刺激集进行平均化处理，忽略了图像级别的稀疏性——差异可能仅集中在少数特定表情上。需要一种更敏感、可靠且机制信息丰富的行为检测方法。

Method: 1) 训练群体特异性人工神经网络模型预测自闭症和神经典型参与者的图像级情绪判断；2) 利用模型选择能最大化两组差异的新面孔图像；3) 在独立队列中验证模型选择图像比随机图像产生更大的行为差异；4) 使用生成对抗网络（GAN）将诊断性图像转化为更高群体一致性的版本；5) 在表型匹配验证中评估合成图像的效果。

Result: 模型选择的图像在独立队列中产生了比随机匹配图像更大的群体间行为差异；GAN合成的图像相对于原始图像减少了行为分离。表明模型引导的图像选择能放大差异，而图像转化能缩小差异。

Conclusion: 建立了一个模型引导的框架，能够发现和转化刺激以揭示群体特异性感知差异。这项工作展示了行为表型分析可以从对固定刺激集的平均化转向优化检测条件，以更精确地定位神经多样性感知的趋同与分歧点。

与遥感交叉或改进方案: 交叉/改进可行性：75%。该框架可迁移至遥感领域：1) 训练群体特异性模型（如不同地理区域/季节的遥感解译员）预测特定地物判读差异，识别少数"诊断性"影像样本；2) 利用GAN生成/选择影像，使不同背景的解译员一致性最大化或揭示判读分歧点；3) 用于优化遥感样本库设计，聚焦于最具判别力的图像，提高标注效率和一致性评估的敏感性。

Abstract (英文): Understanding perceptual differences between autistic and neurotypical adults requires behavioral assays that are sensitive, reliable, and mechanistically informative. Facial emotion perception is a useful test case because group differences have been reported, but findings vary across studies. Here we show that this variability may reflect image-level sparsity: autistic-neurotypical differences in emotion judgments were concentrated in a small subset of diagnostic facial expressions rather than spread uniformly across stimuli. We trained population-specific artificial neural network models to predict image-level judgments for autistic and neurotypical participants, then used these models to select novel faces predicted to maximize group separation. In an independent cohort, model-selected images produced larger behavioral differences than matched random images. We then used the same models with a generative adversarial network to transform diagnostic images toward greater predicted group agreement. In phenotype-matched validation, synthesized images reduced behavioral separation relative to their matched originals. These results establish a model-guided framework for discovering and transforming stimuli that reveal population-specific perceptual differences. More broadly, they show how behavioral phenotyping can move beyond averaging across fixed stimulus sets toward optimized assays that identify the conditions under which neurodivergent perception diverges or converges.

</details>


### [136] [CommuniWave:A Machine Learning Model for Quantifying the Degree of Temporary Informal Behavior in Urban Communities](https://arxiv.org/abs/2607.08554)
*Hongye Yang,Shien Liu,Zhihao Xie*

Main category: cs.AI

TL;DR: CommuniWave是一个集成BCN（基于mmaction2）、YLX（自研YOLOv10变体）和BEM（随机森林）的机器学习模型，通过街道视频分析量化城市社区中的非正式行为程度（DIB），生成波动图以支持精细化管理决策，从而提升社区韧性。


<details>
  <summary>Details</summary>
Motivation: 现有社区规划采用自上而下的方式，缺乏量化居民非正式行为的有效指标，导致规划与实际冲突频繁，亟需一种动态监测工具来提升社区韧性。

Method: 模型由三部分组成：1) 基于mmaction2的行为捕获网络（BCN）用于行为识别；2) 自研YOLOv10变体（YLX）用于目标检测；3) 基于随机森林的行为评估模型（BEM）用于量化非正式行为程度（DIB）。最终从街道视频中生成DIB波动图。

Result: 能够从街道视频中高效检测并量化非正式行为程度，生成DIB波动图，支持城市管理者的精细化动态监测决策。

Conclusion: CommuniWave通过量化非正式行为为城市社区韧性提升提供了数据驱动的决策支持，弥补了传统规划中定性方法的不足。

与遥感交叉或改进方案: 交叉/改进可行性：75%。具体方案：1) 将BCN的行为识别能力迁移至遥感影像中的城市非正规聚落（如棚户区）动态监测，识别自建行为模式；2) YLX模型可适配遥感目标检测任务（如屋顶违建、临时构筑物检测）；3) DIB评估框架可类比用于量化城市扩张中的非正式土地利用程度，结合遥感时序影像生成土地利用非正式度波动图。

Abstract (英文): For urban managers and designers, improving the functional attributes of urban communities to enhance territorial resilience in the face of complexity and uncertainty is crucial. Currently, community planning often follows a top-down approach and lacks effective metrics to quantify informal behaviors of residents, leading to frequent conflicts with original plans. This study introduces CommuniWave, a machine learning model designed to efficiently detect and quantify the Degree of Informal Behavior (DIB) in urban communities. The model integrates a Behavior Capture Net (BCN) based on mmaction2, a self-developed YOLOv10 model (YLX), and a Behavior Eval Model (BEM) using random forest. Ultimately, by generating DIB fluctuation charts from street videos, the model facilitates dynamic monitoring, supporting urban managers in making refined decisions to enhance the overall resilience of communities.

</details>


### [137] [SHAP-Weighted Cross-Modal Expert Fusion for Emotion and Sentiment Recognition: Evidence and Limits](https://arxiv.org/abs/2607.08573)
*Adis Alihodzic,Selma Skopljakovic Hubljar*

Main category: cs.AI

TL;DR: 本文提出了一种基于XAI的自适应融合方法（XGAF），通过TreeSHAP归因值计算样本级权重，并重点分析了不同SHAP归约策略（mean-abs、median-abs、sum-abs）对多模态情感识别的影响。在MELD和CMU-MOSEI数据集上，sum-abs归约策略的表现接近甚至略优于早期融合，显著优于概率平均的晚期融合，且主要收益来自跨模态专家（尤其是三模态专家）而非复杂的样本级路由。


<details>
  <summary>Details</summary>
Motivation: 早期融合（拼接多模态特征）虽准确但缺乏模块化，晚期融合（独立训练单模态预测器）虽模块化但丢失跨模态交互。现有可解释融合方法未充分考虑专家特征维度不平衡时SHAP归因值归约方式的影响，本文旨在填补这一空白。

Method: 提出XGAF（XAI-guided adaptive fusion）框架，使用TreeSHAP计算每个专家的归因值，然后通过三种归约策略（mean-abs、median-abs、sum-abs）得到样本级权重，加权融合单模态和跨模态专家的预测结果。在MELD 7类情感识别和CMU-MOSEI 3类情感识别上进行实验，对比不同面部序列聚合器（含Transformer变体）的表现。

Result: MELD上：sum-abs XGAF（Transformer变体）达到0.5983 wF，早期融合0.6018 wF，概率平均晚期融合0.4598 wF；McNemar检验显示sum-abs XGAF与早期融合无显著差异（p=1.000），显著优于晚期融合（p<0.0001）。CMU-MOSEI上：sum-abs XGAF达到0.6519 wF，略超早期融合（0.6485 wF）和晚期融合（0.5696 wF）。消融实验表明主要收益来自跨模态专家（尤其是三模态专家），而非复杂路由。权重分析显示mean-abs和median-abs权重近乎均匀，而sum-abs集中于三模态专家。

Conclusion: 透明地实证分析了SHAP归约策略、专家特征维度和跨模态专家设计对模块化多模态融合的影响，证明了sum-abs归约策略能够保留总归因质量，有效避免高维跨模态专家被抑制，实现了接近早期融合的准确性与晚期融合的模块化优势兼得。

与遥感交叉或改进方案: 交叉/改进可行性：85%。遥感领域多模态数据（光学影像、SAR、高光谱、LiDAR等）同样面临早期融合维度爆炸和晚期融合丢失跨模态交互的问题。具体交叉方案：（1）将XGAF框架直接应用于遥感多模态分类/分割任务，使用TreeSHAP为不同模态（如光谱波段、纹理特征、地形特征）分配样本级权重；（2）引入遥感专属的跨模态专家（如光谱-空间联合特征、多时相变化特征）；（3）针对遥感中模态维度差异大（如高光谱上百波段vs LiDAR几十特征）的特点，重点采用sum-abs归约策略避免高维模态被低估；（4）可利用Shapley值的可加性解释每个模态对遥感识别结果的贡献，提升模型可解释性。

Abstract (英文): Multimodal emotion and sentiment recognition is commonly addressed by early fusion, which concatenates modalities before classification, or late fusion, which combines independently trained unimodal predictors. Early fusion can be accurate but monolithic, while late fusion is modular but may lose cross-modal interactions. This paper revisits XAI-guided adaptive fusion (\xgaf), a tree-based mixture of unimodal and cross-modal experts whose sample-level weights are derived from TreeSHAP attribution magnitudes. We focus on the effect of SHAP attribution reduction when experts have unequal feature dimensionalities. In this setting, mean-abs and median-abs reductions can suppress high-dimensional cross-modal experts, whereas sum-abs reduction preserves total attribution mass. On MELD 7-class emotion recognition, sum-abs \xgaf{} nearly matches early fusion across three face-sequence aggregators; the Transformer variant reaches 0.5983 \wf{}, compared with 0.6018 for early fusion and 0.4598 for probability-average late fusion. McNemar testing shows no significant difference between sum-abs \xgaf{} and early fusion on MELD ($p=1.000$), while \xgaf{} remains significantly better than late fusion ($p<0.0001$). On CMU-MOSEI 3-class sentiment recognition, sum-abs \xgaf{} reaches 0.6519 \wf{}, slightly exceeding early fusion (0.6485) and late fusion (0.5696). Ablation studies show that the main gain comes from adding cross-modal experts, especially the trimodal expert, rather than from complex per-sample routing. Diagnostics further show that mean-abs and median-abs weights are nearly uniform, while sum-abs weights concentrate on the trimodal expert. Thus, the main contribution is a transparent empirical analysis of how SHAP reduction, expert dimensionality, and cross-modal expert design affect modular multimodal fusion.

</details>


### [138] [Towards Precision Therapy in Hepatocellular Carcinoma: A Clinical-Reasoning LLM for Risk Stratification and Treatment Guidance](https://arxiv.org/abs/2607.08602)
*Peng Cui,Jitao Wang,Siyan Xue,Yao Huang,Haoming Xia,Dong Li,Dengxiang Liu,Weilin Wang,Liping Liu,Leida Zhang,Yunfu Cui,Tao Peng,Daolin Ji,Haitao Zhao,Wei Zhang,Xiaojuan Wang,Weijie Ma,Zongren Ding,Jinlong Li,Yuan Ding,Jiajing Zhao,Zhiyu Chen,Chengkun Yang,Ziyue Huang,Jiaqi Liu,Fusheng Liu,Yang Zhou,Xiaojuan Wang,Zhongquan Sun,Shiyun Bao,Xiaojun Wang,Ming Yang,Guangxin Li,Bin Shu,Yong Liao,Hongxuan Li,Yao Tang,Shizhong Yang,Yongyi Zeng,Yufeng Yuan,Yinpeng Dong,Jihui Hao,Jun Zhu,Jiahong Dong*

Main category: cs.AI

TL;DR: 本文提出HCC-STAR，一种面向肝细胞癌的临床对齐大语言模型。它通过读取电子病历叙事文本，同时输出基于风险评分的分期、符合指南的治疗推荐及个性化生存预测。基于约3万例SEER数据扩展的训练语料和知识对齐推理框架，在12家医院6668例患者的多中心队列中，HCC-STAR在治疗推荐和风险分层上优于临床指南及GPT-5、Gemini-2.5 Pro等模型。遵守其推荐的中位生存期达51个月，显著优于BCLC（29个月）和CNLC（32个月）。盲审中专家认为其推理可信，且模型能帮助医生更快更准地决策。


<details>
  <summary>Details</summary>
Motivation: 现有HCC分期指南（如BCLC、CNLC）提供的粗粒度分类无法捕捉同一分期内的异质性，也未能充分利用电子病历中的临床上下文信息，导致精准度不足。作者希望构建一个能直接读取EMR叙事文本、输出精细化分期和个性化治疗推荐的LLM系统，填补从文本到临床决策的鸿沟。

Method: （1）数据构建：从SEER数据库收集约3万例HCC病例，通过临床医生验证的提示增强工作流，将其扩展为EMR风格的叙事训练数据；（2）模型框架：提出知识对齐推理框架，采用步骤可验证的复合奖励进行优化，超越对临床指南的文本级记忆；（3）评估：在12家医院6668例的多中心队列中进行治疗推荐和风险分层评估，并与BCLC、CNLC、GPT-5、Gemini-2.5 Pro比较；进行假设性总生存分析和临床医生中心评估。

Result: （1）在治疗推荐和风险分层上达到SOTA，优于BCLC、CNLC、GPT-5和Gemini-2.5 Pro；（2）遵守HCC-STAR推荐方案的中位生存期为51个月，显著高于BCLC（29个月）和CNLC（32个月）；（3）盲审中肝胆专科专家认为其推理和循证证明可信；（4）模型在治疗准确性上超过住院医师和主治医师，作为辅助工具能帮助医生更快更准地做出决策。

Conclusion: HCC-STAR可作为可靠且可验证的决策支持系统，用于HCC的风险分层和精准治疗，其临床对齐的推理能力和循证证明获得了专家信任，并能有效提升医生的临床决策效率和质量。

与遥感交叉或改进方案: 交叉/改进可行性：65%。具体方案：（1）数据增强迁移：借鉴其“从结构化数据库通过临床医生验证的提示增强生成EMR叙事数据”的思路，遥感领域可将GeoJSON/Shapefile格式的标注数据（如地物分类、变化检测标签）通过领域专家验证的提示模板，生成为遥感影像描述文本（scene description narratives），用于训练遥感领域的大语言模型；（2）多任务联合输出架构：模仿其同一模型同时输出分期、治疗推荐和生存预测的多任务框架，遥感模型可同时输出场景分类、目标检测框、变化区域分割和置信度评估，实现端到端的遥感解译；（3）步骤可验证复合奖励：将推理过程分解为可验证的子步骤，遥感领域可对变化检测的“先识别变化区域→再分类变化类型→最后评估变化程度”的推理链进行逐步奖励优化；（4）临床专家盲审评估模式：遥感领域可引入领域专家对模型推理过程进行盲审打分，评估解译结果的可信度和解释合理性。

Abstract (英文): Hepatocellular carcinoma (HCC) is a common malignancy and a leading cause of cancer-related mortality. Current guidelines and staging systems provide coarse categories, but often miss within-stage heterogeneity and the clinical context in electronic medical records (EMRs). We present HCC-STAR (Hepatocellular Carcinoma Staging, Treatment And pRognosis), a clinically aligned large language model that reads routine EMR narratives and jointly outputs risk score-based staging, ranked guideline-consistent treatments with evidence-based rationales, and individualized survival estimates. We curated about 30,000 HCC cases from SEER and expanded them into EMR-style narrative training data using a clinician-validated, prompt-based augmentation workflow. On this corpus, we developed a knowledge-aligned reasoning framework optimized with a step-verifiable composite reward, moving beyond text-level memorization of clinical guidelines. In a multi-center cohort of 6,668 patients from 12 hospitals in China, HCC-STAR achieved state-of-the-art performance in treatment recommendation and risk stratification compared with clinical guidelines and competitive models, including GPT-5 and Gemini-2.5 Pro. Hypothetical overall-survival analysis showed a median survival of 51 months under adherence to HCC-STAR recommendations, compared with 29 and 32 months under BCLC and CNLC. In clinician-centric evaluations, blinded hepatobiliary specialists rated HCC-STAR's reasoning and evidence-based justifications as trustworthy. The model surpassed resident and attending physicians in treatment accuracy and helped physicians make more accurate decisions faster when used as an assistant. These findings support HCC-STAR as a reliable and verifiable decision-support system for risk stratification and precision therapy in HCC.

</details>


### [139] [The complexities of patient-centred conversational artificial intelligence](https://arxiv.org/abs/2607.08625)
*João Matos,Olivia Buege,Donny Cheung,Gary S. Collins,Paula Dhiman,Nan Li,Bingyu Mao,Benjamin W. Nelson,Michail Ouroutzoglou,Paul Varghese,Jonathan Amar*

Main category: cs.AI

TL;DR: 大型语言模型驱动的患者聊天机器人在症状评估中应用日益广泛，但现有开发与评估多基于合作且表达清晰的模拟患者。本研究分析2053次真实医患对话，发现用户沟通模式与情感表达存在巨大差异。我们开发了能分别建模临床内容、情绪状态、对话策略与沟通风格的患者模拟器。在15位人类评估者的图灵测试中，模拟对话与真实对话几乎无法区分（准确率55%）。利用5种患者角色，经1164位临床医生评估，发现沟通风格显著影响分诊结果。以患者为中心的对话AI必须适应沟通多样性：为理想化而非真实互动设计的系统在实际部署中表现不佳，并可能加剧健康不平等。


<details>
  <summary>Details</summary>
Motivation: 现有LLM驱动的健康聊天机器人在症状评估中的开发与评估多基于合作的、表达清晰的模拟患者，忽略了真实用户沟通模式的多样性。这种理想化评估可能导致系统在实际部署中性能下降并加剧健康差距。

Method: 分析2053次真实患者-机器人对话；开发分别建模临床内容、情绪状态、对话策略和沟通风格的患者模拟器；通过15位人类评估者进行图灵测试验证模拟真实性；使用5种患者角色和1164位临床医生评估4个LLM在紧急程度评估中的表现。

Result: 模拟对话与真实对话几乎无法区分，人类评估者准确率仅55%；沟通风格能显著改变分诊结果。

Conclusion: 以患者为中心的对话AI必须容纳沟通多样性，为理想化而非真实互动设计的系统在实际部署中性能不佳并可能加剧健康不平等。

与遥感交叉或改进方案: 交叉/改进可行性：75%。该论文的核心贡献在于构建能够模拟真实用户多样性的仿真系统（模拟临床内容、情绪、对话策略、沟通风格），这一方法论可迁移至遥感领域：1）构建模拟不同通信条件（噪声、遮挡、延迟）、地形环境（城市、山地、水体）和任务需求（目标检测、变化监测）的"遥感用户模拟器"，用于更真实地评估遥感AI系统的鲁棒性；2）利用多样化用户角色（专家、新手、应急响应人员）测试遥感人机交互系统的可用性；3）将"沟通风格影响分诊结果"的发现类比为"交互方式影响遥感解译结果"，推动遥感AI系统在界面设计、反馈机制中考虑用户多样性，避免为理想化操作者设计的系统在实际灾害响应等场景中失效。

Abstract (英文): Consumer-facing health chatbots powered by large language models (LLMs) are increasingly used for symptom assessment. However, chatbot development and evaluation often rely on cooperative, articulate, simulated patients. We analysed 2,053 real patient-chatbot conversations and found that communication patterns and expression of emotions vary widely across users. We developed a patient simulator that separately models clinical content, emotional state, conversational strategy, and communication style. In a Turing-inspired evaluation of realism with 15 human graders, simulated conversations were nearly indistinguishable from real ones, with human graders achieving an accuracy of 55%. We used five distinct patient personae, across 1,164 clinician-graded cases, to evaluate the performance of four LLMs in urgency assessment. We found that communication style can significantly alter triage outcomes. Patient-centred conversational artificial intelligence must accommodate communication diversity: systems designed for idealised, rather than realistic, interactions risk underperforming and amplifying health disparities when deployed in the real world.

</details>


### [140] [Formal Mechanisms for Market Stability in Self-Interested Agent Societies: A Marketplace Simulation Study](https://arxiv.org/abs/2607.08652)
*Eugene Ng Yi Sheng,Bingquan Shen*

Main category: cs.AI

TL;DR: 本文研究了在重复社会困境中，如何通过形式化机制（特别是Mediation机制）来维持自私LLM代理之间的市场稳定性，并测试了其对对抗性攻击的鲁棒性。实验表明Mediation机制在200轮模拟中表现最佳，即使面对优化后的攻击，也只能降低诚实代理13.3%的效用，无法使市场崩溃。


<details>
  <summary>Details</summary>
Motivation: 自私代理在无约束条件下倾向于背叛，导致合作收益崩溃。本文旨在探索哪些形式化机制（叠加在无限制通信之上）能够维持此类代理社会的市场稳定性，并评估这些机制面对对抗性攻击的韧性。

Method: 构建了多代理市场模拟环境，包含18个DeepSeek-V3 LLM代理，具有互补生产专长，在约束社交网络中进行交易。实验分为两个阶段：(1) 在8种机制条件下进行200轮渐进式troll注入比较，识别出Mediation为最优机制；(2) 使用迭代提示优化的LLM驱动troll对Mediation进行对抗性红队测试。

Result: Mediation在8种机制中表现最佳；最优攻击（v6）仅能将诚实代理效用降低13.3%，但无法使市场崩溃；Mediation即使在持续对抗压力下也能实现恢复。

Conclusion: Mediation机制具有对抗鲁棒性——可以被弯曲（性能下降）但无法被打破（市场崩溃）。对抗鲁棒性定义为机制在优化攻击下维持诚信代理正效用的能力。

与遥感交叉或改进方案: 交叉/改进可行性：75%。遥感领域存在多智能体协同观测、数据共享与资源分配等典型社会困境问题（如多星任务规划中的自私行为导致全局效率下降）。可将本文的Mediation机制引入遥感多智能体系统：(1) 作为遥感卫星星座中任务分配的中介仲裁层，防止各卫星节点因自利倾向而拒绝协同任务；(2) 在遥感数据交易市场中，利用Mediation机制维护数据提供方与需求方之间的交易稳定性，抵御恶意节点的欺骗攻击；(3) 将对抗鲁棒性评估框架迁移至遥感系统安全性测试，评估遥感多代理系统在对抗干扰下的恢复能力。

Abstract (英文): Self-interested agents, left unconstrained, tend toward defection in repeated social dilemmas, causing cooperative gains from trade to collapse. This paper investigates what formal mechanisms, layered on top of unrestricted communication, are sufficient for a society of such agents to maintain market stability, and how resilient those mechanisms are to adversarial attack. We instantiate the research question as a multi-agent marketplace simulation where 18 LLM agents (DeepSeek-V3) with complementary production specialties must trade within a constrained social network to obtain utility. We conduct two experimental phases: (1) a mechanism comparison across eight conditions under progressive troll injection over 200 rounds, identifying Mediation as the top-performing mechanism; and (2) adversarial red-teaming of Mediation using iteratively prompt-optimised LLM-driven trolls, finding that the best attack (v6) reduces honest-agent utility by 13.3% but cannot collapse the market. Mediation enables recovery even under sustained adversarial pressure. We define adversarial robustness as a mechanism's ability to sustain positive honest-agent utility under optimised attack, and find that Mediation is robust: it can be bent but not broken.

</details>


### [141] [SolarChain-Eval: A Physics-Constrained Benchmark for Trustworthy Economic Agents in Decentralized Energy Markets](https://arxiv.org/abs/2607.08681)
*Shilin Ou,Yifan Xu,Luyao Zhang*

Main category: cs.AI

TL;DR: 提出了SolarChain-Eval基准，用于评估去中心化能源市场中可信赖的经济智能体。该基准包含物理约束、LLM规划/审计层，实验揭示了效用-安全性权衡，表明可信AI评估需要物理约束和透明干预记录。


<details>
  <summary>Details</summary>
Motivation: 自主AI系统应用于网络物理环境时，不仅需要评估任务性能，还需评估可信赖性。在去中心化能源市场中，自主智能体可能利用无效物理数据、制造人为流动性、产生不稳定的治理决策，现有评估方法缺乏对这些可信赖维度的系统考量。

Method: 1) 将市场治理建模为兼容Gymnasium的马尔可夫决策过程，智能体按小时决策；2) 从市场效用、物理安全、滑点、动作平滑度、空间公平性和可审计性多维度评估策略；3) 引入基于LLM的规划器/审计器层，规划器定义回合级动作边界和审计规则，审计器审查并修订高风险动作，所有干预记录结构化日志。

Result: 静态、随机、短视、RL和RL+LLM策略实验揭示了清晰的效用-安全性权衡。RL智能体提高市场效用但仍可能产生不安全行为；移除物理惩罚后，奖励最大化智能体会利用无效发电并增加人为流动性；LLM规划/审计器改善可审计性并缓解部分风险，但无法完全补偿错误指定的奖励函数。

Conclusion: 可信赖的智能体AI评估需要物理约束和透明的干预记录，仅依赖奖励函数调整或后验审计是不够的。

与遥感交叉或改进方案: 交叉/改进可行性：85%。该论文提出的物理约束评估框架可直接应用于遥感驱动的能源系统优化：1) 将遥感数据（太阳辐射、风速、云量）作为物理约束输入SolarChain-Eval的MDP状态空间；2) LLM规划器可基于遥感预测数据动态调整发电量边界约束；3) 引入遥感时空数据一致性校验作为可审计性维度，检测智能体是否利用错误/篡改的遥感数据获利；4) 空间公平性指标可结合遥感土地覆盖数据，评估能源分配的区域公平性。

Abstract (英文): As agentic AI systems are increasingly applied to cyber-physical environments, their evaluation requires assessment of both task performance and trustworthiness. In decentralized energy markets, autonomous agents may improve market utility, but may also exploit invalid physical data, create artificial liquidity, and produce unstable governance decisions. Therefore, we propose SolarChain-Eval, a physics-constrained benchmark for evaluating trustworthy economic agents. It formulates market governance as a Gymnasium-compatible Markov Decision Process, where agents make hourly decisions. SolarChain-Eval evaluates each policy across multiple dimensions, including market utility, physical safety, slippage, action smoothness, spatial fairness, and auditability. To support agentic evaluation, SolarChain-Eval incorporates an LLM-based Planner/Auditor layer. The Planner defines episode-level action bounds and audit rules, while the Auditor reviews and revises high-risk actions. All interventions are recorded through structured logs, including trigger signals, proposed actions, revised actions, and audit rationales. Experiments with static, random, myopic, RL, and RL+LLM policies reveal a clear utility-safety trade-off. RL agents improve market utility but can still produce unsafe behavior. When the physics penalty is removed, reward-maximizing agents exploit invalid generation and increase artificial liquidity. The LLM Planner/Auditor improves auditability and mitigates selected risks, but it cannot fully compensate for a misspecified reward function. These results indicate that trustworthy agentic AI evaluation requires both physical constraints and transparent intervention traces. We release data and code as open access on GitHub for replicability.

</details>


### [142] [Remember When It Matters: Proactive Memory Agent for Long-Horizon Agents](https://arxiv.org/abs/2607.08716)
*Yifan Wu,Lizhu Zhang,Yuhang Zhou,Mingyi Wang,Bo Peng,Serena Li,Xiangjun Fan,Zhuokai Zhao*

Main category: cs.AI

TL;DR: 本文提出"行为状态衰减"问题——在长程任务中，决策相关信息随轨迹增长而被埋没。为此设计了一个独立的记忆智能体，主动构建结构化记忆库并选择性注入提醒，而非被动检索。该方法在Terminal-Bench 2.0和τ²-Bench上分别提升+8.3和+6.8个百分点，且选择性干预优于被动暴露或始终注入等策略。同时基于SETA数据集用SFT和GRPO训练了开源记忆策略Qwen3.5-27B。


<details>
  <summary>Details</summary>
Motivation: 长程任务中，决策相关信息分散于不断增长的轨迹中，智能体需从中提取关键信息并采取行动。随着轨迹变长，任务需求、环境事实、先前尝试、诊断结果和未完成子目标等关键信息可能被上下文窗口埋没甚至超出范围，导致决策时无法获取所需信息，引发"行为状态衰减"失败模式。

Method: 提出一种主动记忆干预机制：一个独立的记忆智能体与未修改的动作智能体并行运行，从近期轨迹中更新结构化记忆库，并决定是否注入基于记忆的提醒或保持静默。该模块与前沿动作智能体和现有智能体框架即插即用。

Result: 在Terminal-Bench 2.0和τ²-Bench上，该方法对弱和强动作智能体的pass@1均有提升，Terminal-Bench提升+8.3个百分点，τ²-Bench提升+6.8个百分点。消融实验表明，选择性干预优于被动记忆暴露、始终注入、仅作为顾问指导以及通用检索等策略。

Conclusion: 主动记忆干预是缓解长程任务中行为状态衰减的有效手段。作为迈向开源记忆策略的初步尝试，基于SETA数据集使用SFT和GRPO训练的Qwen3.5-27B在验证奖励上有所提升，并实现了向Terminal-Bench的部分迁移。

与遥感交叉或改进方案: 交叉/改进可行性：85%。遥感领域常涉及长时序影像分析（如变化检测、物候监测、长时间序列分类），同样面临"状态衰减"问题——早期影像特征、历史分类结果、人工标注标记等信息随序列增长被遗忘。具体方案：(1) 将记忆智能体架构迁移至遥感长时序任务中，使模型在分析新影像时主动回顾前期关键变化点或标注信息；(2) 记忆库可存储不同时间窗口的地物光谱特征、已检测的变化区域和未完成的解译子目标；(3) 对于卫星视频或无人机连续帧分析，记忆智能体可决定何时注入先前的目标跟踪状态或异常检测结果，提升长程分析的连贯性和准确性。

Abstract (英文): In long-horizon tasks, decision-relevant state is often scattered across an expanding trajectory, while the action agent must surface it and act. As trajectories grow, task requirements, environment facts, prior attempts, diagnoses, and open subgoals can be buried in the context window or pushed beyond it, failing to influence decisions when needed. We call this failure mode "behavioral state decay". We study memory as an active intervention mechanism rather than passive retrieval. A separate memory agent runs alongside an unmodified action agent, updating a structured memory bank from the recent trajectory and deciding whether to inject a memory-grounded reminder or remain silent. The module is plug-and-play with frontier action agents and existing agent harnesses. Across Terminal-Bench 2.0 and $τ^2$-Bench, it improves pass@1 for both weaker and stronger action agents, with gains of +8.3 pp on Terminal-Bench and +6.8 pp on $τ^2$-Bench. Ablations show that selective intervention outperforms passive bank exposure, always-on injection, advisor-only guidance, and general retrieval. As an early step toward open-weight memory policies, we train Qwen3.5-27B on SETA using SFT and GRPO, improving validation reward and achieving partial transfer to Terminal-Bench.

</details>


### [143] [The Illusion of Equivalency: Statistical Characterization of Quantization Effects in LLMs](https://arxiv.org/abs/2607.08734)
*Baha Rababah,Cuneyt Gurcan Akcora,Carson K. Leung*

Main category: cs.AI

TL;DR: 本文揭示后训练量化虽然保留准确率和困惑度，但会改变大语言模型的决策行为；通过提出“正确一致性”指标，发现中等量化下行为分歧已出现，并证明查询和键投影比值和输出投影更敏感，存在“等价幻觉”。


<details>
  <summary>Details</summary>
Motivation: 现有量化评估仅依赖准确率和困惑度，无法捕捉量化引起的模型行为变化，存在评估盲区。

Method: 提出正确一致性指标衡量基模型与量化模型在正确预测上的重叠度；将量化视为注意力权重的结构算子，用统计和分布度量逐层分析失真。

Result: 中等量化下行为分歧已显著；低比特宽度存在非线性断裂点；查询和键投影比值和输出投影更敏感。

Conclusion: 基模型与量化模型之间存在“等价幻觉”，需在传统指标外引入行为级评估。

与遥感交叉或改进方案: 交叉/改进可行性：85%。遥感领域常将大模型用于场景分类、目标检测等任务，同样面临量化后行为不可知的问题。可将正确一致性指标引入遥感模型量化评估，检测量化后的遥感大模型在分类决策上是否与原始模型一致，尤其针对多光谱/高光谱数据中注意力层的敏感性差异，设计逐波段或逐通道的量化保护策略。

Abstract (英文): Post-training quantization is widely used to deploy large language models in resource-constrained settings, yet its evaluation relies almost exclusively on accuracy and perplexity. We show that these metrics fail to capture behavioral changes induced by quantization. We introduce correctness agreement, a decision-level metric that measures overlap in correct predictions between a base model and its quantized variants, independent of absolute accuracy. Across multiple models and quantization schemes from 8-bit to 2-bit, we find that behavioral divergence emerges under moderate quantization even when task performance appears preserved. To explain this effect, we analyze quantization as a structural operator on attention weights and quantify layer-wise distortions using statistical and distributional measures. Our results reveal non-linear breakpoints at low bit-widths and show that query and key projections are consistently more sensitive than value and output projections. These findings expose an illusion of equivalence between base and quantized models and motivate behavioral evaluation beyond conventional performance metrics.

</details>


### [144] [Workflow as Knowledge: Semantic Persistence for LLM-Mediated Workflows](https://arxiv.org/abs/2607.08740)
*Emanuele Quinto,Carlo Andrea Rozzi,Francesco Zanitti*

Main category: cs.AI

TL;DR: 本文提出一种受Lisp启发的、语言无关的概念模型，将工作流定义、实例、推理记录、上下文快照和依赖关系表示为持久化知识对象，核心语义区分在于derive（确定性计算）和infer（LLM非确定性判断），从而实现工作流的可检查、可恢复和可审查。


<details>
  <summary>Details</summary>
Motivation: 现有LLM应用的工作流系统虽解决了执行层面的问题，但缺乏对工作流本身作为可持久化、可检查、可恢复的知识对象的语义层面的概念化描述。

Method: 采用符号形式、对象标识和活图像思维作为解释透镜，提出一个语言无关的概念模型，核心是derive（基于可用状态的确定性计算）与infer（在声明上下文和执行器控制能力策略下的LLM中介判断）的语义区分，并将各类工作流相关实体表示为共享知识基底中的持久化知识对象。

Result: 给出了一个初步的语义持久化概念框架：工作流不仅产生知识和留下痕迹，其本身也可表示为可检查、可恢复和可审查的知识对象。

Conclusion: 该概念模型为工作流的语义持久化提供了理论基础，但形式化转移语义学仍是未来工作。

与遥感交叉或改进方案: 交叉/改进可行性：70%。该概念模型可应用于遥感工作流管理中：将遥感数据处理流程（如预处理、特征提取、分类、变化检测等步骤）定义为持久化知识对象，其中确定性计算步骤（如几何校正、大气校正）映射为derive，LLM辅助的遥感解译判断（如土地利用分类中的语义判别）映射为infer。工作流实例和中间结果作为知识对象可被检索和复现，提升遥感分析的透明度和可重复性。

Abstract (英文): Large language model (LLM) applications increasingly use explicit workflows for tool use, retrieval, branching, checkpointing, and human approval. Existing workflow systems already address many execution concerns. This paper proposes a Lisp-inspired but language-independent conceptual model: symbolic forms, object identity, and live-image thinking are used as explanatory lenses, not implementation commitments. In this model, workflow definitions, workflow instances, inference records, context snapshots, and dependency relations are represented as persistent knowledge objects in a shared knowledge substrate. Its central semantic distinction is between derive and infer: derive is deterministic computation over available state; infer is mediated LLM judgment under declared context and executor-controlled capability policy.
  The result is a preliminary conceptual account of semantic persistence: workflows do not merely produce knowledge and leave traces, but can themselves be represented as inspectable, resumable, and reviewable knowledge objects, while formal transition semantics remain future work.

</details>


### [145] [AUTOPILOT VQA: Benchmarking Vision-Language Models for Incident-Centric Dashcam Understanding](https://arxiv.org/abs/2607.08745)
*Siddharth Damodharan,Radhika Gupta,Ali Alshami,Ryan Rabinowitz,Jugal Kalita*

Main category: cs.AI

TL;DR: AUTOPILOT-VQA是一个面向自动驾驶行车记录仪视频的以事故为中心的视觉问答基准数据集，旨在评估视觉-语言模型在安全关键场景中的推理能力。该基准涵盖天气、交通环境、道路布局、路面状态、标识、涉及实体、事故发生、撞击位置及可避免性推理等多维度安全问题，超越了传统目标识别，推动更具可解释性和安全意识的自动驾驶系统发展。该数据集作为CVPR 2026 AUTOPILOT竞赛的一部分发布。


<details>
  <summary>Details</summary>
Motivation: 现有的视觉-语言模型和多模态大语言模型在自动驾驶场景理解、决策、轨迹预测等任务上取得了进展，但缺乏针对安全关键事故场景的可靠推理评估标准。现有基准大多聚焦于目标识别或一般场景理解，未能系统性地评估模型在真实事故及近事故情境下的时态推理和安全意识能力。

Method: 构建了一个以行车记录仪视频为基础的事故中心视觉问答基准数据集，设计了围绕真实驾驶事故和近事故的结构化问题，覆盖9个安全相关类别（天气光照、交通环境、道路布局、路面状态、标识、涉及实体、事故是否发生、撞击位置、可避免性推理），要求模型同时回答场景上下文属性和事故级别细节的接地问题。

Result: 提出了一个标准化的基准数据集，作为CVPR 2026 AUTOPILOT竞赛的一部分发布，能够评估不同自动驾驶系统在安全关键场景下的推理可靠性，支持开发更可解释、鲁棒且具有安全意识的视觉-语言系统。

Conclusion: AUTOPILOT-VQA填补了自动驾驶领域中安全关键事故推理评估的空白，通过事故中心的时态接地推理任务，推动视觉-语言模型从目标识别向安全感知推理的进化，为真实世界自动驾驶的可解释性和可靠性评估提供了标准化工具。

与遥感交叉或改进方案: 交叉/改进可行性：75%。遥感领域同样面临安全关键场景的推理评估需求，如遥感视频中的灾害监测（火灾、洪水、地震）和事故检测（交通事故、建筑倒塌）。借鉴AUTOPILOT-VQA的事故中心VQA框架，可构建遥感安全事件VQA基准，设计针对灾害发生与否、影响区域、可避免性/应急响应推理的结构化问题，覆盖天气条件、地形环境、道路/建筑布局、受灾实体等维度，推动遥感大模型从静态目标识别向时态安全推理发展。

Abstract (英文): Recent advances in Vision-Language Models, Large Language Models, and Multimodal Large Language Models have improved autonomous driving tasks such as scene understanding, decision making, trajectory prediction, and visual question answering. However, evaluating whether these models can reliably reason about safety-critical incidents remains challenging. To address this gap, we present AUTOPILOT-VQA, an incident-centric visual question answering benchmark for dashcam video understanding. The dataset evaluates different systems through structured questions designed around real-world driving incidents and near-incidents. The benchmark covers diverse safety-relevant categories, including weather and lighting conditions, traffic environment, road layout, road surface state, signage, involved entities, accident occurrence, impact location, and avoidability-related reasoning. By requiring models to answer grounded questions about both contextual scene properties and event-level incident details, AUTOPILOT-VQA moves beyond object recognition toward temporally grounded, safety-aware reasoning. The dataset is released as part of the AUTOPILOT CVPR 2026 competition and provides a standardized benchmark for assessing the reliability of autonomous driving systems in different scenarios. Our benchmark support developments for more interpretable, robust, and safety-conscious vision-language systems for real-world autonomous driving.

</details>


### [146] [Using AI-based Learning Assistants in Higher Education: A Large-Scale Descriptive Analysis](https://arxiv.org/abs/2607.08748)
*Kristina Schaaff,Quintus Stierstorfer,Valerie Heckel*

Main category: cs.AI

TL;DR: 基于77,543名远程学生的学习日志数据，对AI学习助手Syntea在高等教育中的使用模式进行了大规模描述性分析，发现其已被嵌入学习常规，但使用情况因人口统计学和结构性背景而异。


<details>
  <summary>Details</summary>
Motivation: 现有教育聊天机器人研究多依赖小样本和自我报告调查数据，缺乏基于客观日志数据的大规模实际使用行为证据。

Method: 基于77,543名远程学生的客观日志数据，按性别、年龄组、学习集群、学位和学习模式进行使用模式分析。

Result: Syntea已被嵌入许多学习者的学习常规中，但使用情况在不同人口统计学和结构性背景下存在差异。

Conclusion: 研究为AI学习支持的进一步发展提供了实证基础，并贡献了高等教育中教育聊天机器人使用的大规模分析。

与遥感交叉或改进方案: 交叉/改进可行性：75%。该研究的大规模日志数据分析框架可直接迁移至遥感教育场景。具体方案：(1) 在遥感课程中部署AI教学助手，采集学生与助手的交互日志数据；(2) 按学生专业背景（地信、测绘、遥感等）、年级、学习阶段等维度分析使用模式；(3) 评估AI助手对遥感图像解译、分类等核心技能学习效果的影响，为遥感教育AI工具优化提供数据支撑。

Abstract (英文): In this study, we present a large-scale descriptive analysis of the use of an AI-based learning assistant (Syntea) in higher education. Based on objective log data from 77,543 students enrolled in distance studies, we examine usage patterns across gender, age group, study cluster, degree, and study mode. To date, existing research on educational chatbots has largely relied on comparatively small samples and self-reported survey data, while large-scale evidence on actual usage behavior remains limited. Our findings show that Syntea is already embedded in the study routines of many learners, but that usage differs across demographic and structural contexts. By identifying these patterns, our study provides an empirical basis for the further development of AI-based learning support and contributes a large-scale analysis of educational chatbot usage in higher education.

</details>


### [147] [Ideas Have Genomes: Benchmarking Scientific Lineage Reasoning and Lineage-Grounded Idea Generation](https://arxiv.org/abs/2607.08758)
*Yifan Zhou,Qihao Yang,Yan Li,Donggang Li,Xiru Hu,Hokin Deng,Ziyang Gong,Xuanyi Zhou,Huacan Wang,Xiangchao Yan,Wanghan Xu,Wenlong Zhang,Shaofeng Zhang,Yue Zhou,Yifan Yang,Zhihang Zhong,Xue Yang*

Main category: cs.AI

TL;DR: 本文提出IdeaGene-Bench（IG-Bench），一个评估AI系统科学思想继承与演化推理能力的基准。它包含1,961条谱系轨迹、1,085个Idea Genome对象和920个GenomeDiff记录，覆盖10个科学领域，通过IG-Exam（闭式推理）和IG-Arena（谱系条件生成）两项评测，发现最强系统在谱系推理上的精确准确率仅27.3%，且结构化谱系上下文会重新排序系统表现。


<details>
  <summary>Details</summary>
Motivation: 现有基准无法评估AI系统是否能够像科学思想一样遵循继承、修复、重组等演化结构进行推理与生成。

Method: 基于IdeaGene框架，将论文/提案表示为最小化、有类型、有证据基础的Idea Genome对象，通过GenomeDiff记录六种演化动力学（继承、突变、丢失、外部引入、新颖插入），构建谱系轨迹。IG-Exam测试闭式谱系推理（42任务类型），IG-Arena通过谱系条件化种群-演化得分（PES）评估生成连贯性。

Result: 最强系统在谱系推理上精确准确率仅27.3%，结构化谱系上下文会打乱系统排名而非均匀提升所有参与者，表明存在组合瓶颈。

Conclusion: 当前AI系统在科学思想谱系推理与继承性生成方面存在显著能力缺口，IG-Bench为评估和引导这一方向提供了系统化基准。

与遥感交叉或改进方案: 交叉/改进可行性：85%。遥感领域的知识演化（如不同遥感影像融合方法、目标检测算法的迭代改进）天然具有谱系结构。可将遥感论文按IdeaGene框架建模，追踪不同方法（如从CNN到Transformer再到Mamba的演化）的继承、变异与融合过程。IG-Bench的谱系条件生成可用于设计继承现有遥感方法优势、同时填补空白的创新方案。

Abstract (英文): Scientific ideas rarely start from a blank page. They inherit mechanisms, repair known limitations, and recombine pieces of earlier work, much like biological genomes. Current benchmarks still say little about whether AI systems can follow this inheritance structure. We present IdeaGene-Bench (IG-Bench), a benchmark for scientific lineage reasoning and lineage-grounded idea generation. IG-Bench is organized around the IdeaGene framework: each paper or proposal is represented as a set of minimal, typed, evidence-grounded Idea Genome objects, and a GenomeDiff aligns these objects to record inheritance, mutation, loss, external import, and novel insertion under six operational evolutionary dynamics. The benchmark contains 1,961 golden lineage traces, 1,085 curated Idea Genome objects, and 920 pairwise GenomeDiff records across 10 scientific domains. It supports two evaluations. IG-Exam (42 task types, 1,029 instances) tests closed-form lineage reasoning across Idea Genome abstraction, inheritance tracing, evolutionary reasoning, and lineage verification. IG-Arena evaluates generation with a lineage-conditioned Population-Evolution Score(PES), asking whether a proposal can be inserted as a coherent descendant of a given lineage population: it should inherit the right Idea Genome objects, vary meaningfully from nearby work, and offer selection value for future research. Experiments on 14 LLM-based scientists expose a compositional bottleneck. The strongest system reaches only 27.3% exact accuracy on lineage reasoning, and structured lineage context reshuffles system rankings rather than helping every participant uniformly.

</details>


<div id='Remote Sensing'></div>

# Remote Sensing [[Back]](#toc)

### [148] [Spatial and Temporal Variability of Terrestrial Water Storage and Their Relationship with Groundwater Level with GRACE, GLDAS and Observations: A Case Study of Murray–Darling Basin](https://doi.org/10.3390/rs18132206)
*Chongya Ma,Jiping Liu,Guobin Fu*

Main category: Remote Sensing

TL;DR: 本研究利用GRACE卫星数据、GLDAS陆面模型和气候观测数据，分析了Murray-Darling流域（MDB）陆地水储量（TWS）的时空变化及其与地下水位的关系。结果表明：TWS呈现明显的时间变异性（2002-2009年负异常下降趋势，2010-2017年正异常下降趋势，2018-2025年混合正负异常上升趋势）；识别出5个主导聚类模式解释TWS空间变异性；TWS与降雨强相关但局部较弱；蒸散发和径流也能解释TWS变化，组合模型效果更优；TWS导出的地下水储量变化与实测地下水位变化既有相似也有差异，反映了复杂水文地质过程和人类活动（如地下水开采）的影响。


<details>
  <summary>Details</summary>
Motivation: 探究Murray-Darling流域陆地水储量的时空演变规律及其与地下水位的关系，以支持利用GRACE卫星信息和陆面模型改进地下水资源管理。

Method: 综合使用GRACE卫星数据获取TWS、GLDAS陆面模型结果模拟地表过程、气候观测数据（降雨、蒸散发、径流），通过时间序列分析、聚类分析和对比验证等方法进行研究。

Result: (1) TWS呈现三个阶段性变化：2002-2009年负异常下降、2010-2017年正异常下降、2018-2025年混合正负异常上升；(2) 识别出5个主导聚类模式；(3) TWS与降雨总体强相关但局部较弱；(4) 蒸散发和径流可额外解释TWS变化，组合模型效果最优；(5) TWS导出的地下水储量变化与实测数据既有相似也有差异。

Conclusion: TWS的时空变化受气候因素（降雨、蒸散发、径流）综合控制，GRACE反演的地下水储量变化可有效反映区域地下水动态，但需考虑水文地质过程和人类活动的影响，为利用卫星数据管理地下水资源提供了科学依据。

与遥感交叉或改进方案: 交叉/改进可行性：90%。改进方案：(1) 引入更高分辨率卫星数据（如GRACE-FO、Sentinel系列）提高TWS反演的空间分辨率；(2) 结合InSAR地表形变数据辅助分析地下水开采引起的地面沉降与TWS变化的关联；(3) 融合多源遥感数据（如SMAP土壤水分、MODIS蒸散发）优化GLDAS模型参数，提升对TWS组分的分解精度；(4) 利用机器学习方法（如LSTM、随机森林）融合遥感与气象数据进行TWS预测，弥补GRACE时间间隙。

Abstract (英文): Spatial and temporal patterns of terrestrial water storage (TWS), and their relationship with groundwater levels, were investigated with the Gravity Recovery and Climate Experiment (GRACE) satellite data, the Global Land Data Assimilation System (GLDAS) land surface model results, and climate observations for the Murray–Darling Basin (MDB). The results show that: (1) TWS displays a clear temporal variability: a negative TWS anomaly with a declining trend during 2002–2009, a positive TWS anomaly with a decreasing trend during 2010–2017, and a period of mixed positive and negative TWS anomalies being accompanied by an increasing trend from 2018 to 2025; (2) five dominant cluster patterns were identified that explain the spatial variability of temporal TWS across the MDB; (3) overall, TWS temporal variability is strongly correlated with rainfall, although it is weak at certain locations; (4) TWS is also influenced by evaporation (both actual and potential evapotranspiration, AET and PET) and runoff, and a combined model significantly improves the overall performance in explaining TWS temporal variability; and (5) TWS-derived groundwater storage changes show both similarities and differences in comparison with groundwater level observation changes, reflecting complex hydrogeological processes and the influence of human activities such as groundwater extraction. These findings provide valuable insights to support improved groundwater resource management with GRACE satellite information and land surface models.

</details>


### [149] [S2M-Net: Dynamic Hyperspectral Unmixing Network Integrating Spectral Sequence Mamba and Local Spatial–Spectral Awareness](https://doi.org/10.3390/rs18132228)
*Yi Yang,Mei‐Yan Xu,Weidong Zhang,Ji Zhang,Yuquan Gan*

Main category: Remote Sensing

TL;DR: 提出一种结合光谱序列Mamba和局部空谱感知的动态高光谱解混网络，通过非对称双流协作架构实现全局光谱语义与局部空谱细节的融合，提升解混精度。


<details>
  <summary>Details</summary>
Motivation: 现有基于深度学习的解混方法中，CNN感受野有限难以捕捉长距离光谱依赖，Transformer虽具全局建模能力但计算复杂度高且无法自适应过滤连续光谱波段中的冗余噪声。

Method: 构建非对称双流协作架构：1）光谱序列Mamba分支将高光谱数据建模为1D连续序列，利用选择性状态空间模型以线性复杂度全局扫描，自适应过滤冗余波段提取高纯度全局光谱语义；2）局部空谱感知分支通过注意力增强CNN捕获局部连续光谱变化和空间纹理，提供精细几何边界约束；3）空间自适应门控融合模块根据像素混合复杂度动态平衡全局光谱语义与局部空谱细节。

Result: 在多个公开高光谱数据集上的实验表明，该方法在解混精度上显著优于对比方法。

Conclusion: 光谱序列Mamba结合局部空谱感知的非对称双流架构能有效解决CNN和Transformer的局限性，实现高精度高光谱解混。

与遥感交叉或改进方案: 交叉/改进可行性：95%。该论文本身就是高光谱遥感解混任务，改进方向：1）可引入多尺度Mamba模块以捕获不同尺度的光谱依赖；2）将门控融合机制扩展为可学习区域自适应权重，进一步优化复杂场景下全局与局部信息的平衡；3）结合稀疏或低秩先验约束增强物理可解释性；4）探索Mamba与图神经网络的混合架构以建模非欧几里得空间结构信息。

Abstract (英文): Hyperspectral unmixing aims to extract pure endmembers and their corresponding abundance from mixed pixels. Existing deep learning-based unmixing methods predominantly rely on convolutional neural networks (CNNs) or Transformer architectures. However, CNNs suffer from limited receptive fields and struggle to capture long-range spectral dependencies across the entire spectral sequence. While Transformers possess global modeling capabilities, they are constrained by quadratic computational complexity and lack the ability to adaptively filter redundant noise in consecutive spectral bands. To address these limitations, this paper proposes a dynamic hyperspectral unmixing network integrating a spectral sequence Mamba with local spatial–spectral awareness. Specifically, the network features a novel asymmetric dual-stream collaborative architecture. The first branch, the spectral sequence Mamba, models hyperspectral data as a one-dimensional continuous sequence and employs the selective state space model to perform global scanning with linear complexity. This adaptively filters redundant spectral bands to accurately extract high-purity global spectral semantics. The second branch, dedicated to local spatial–spectral awareness, uses an attention-augmented CNN to capture local continuous spectral variations and spatial textures, providing fine-grained geometric boundary constraints for abundance estimation. Furthermore, a spatially adaptive gated fusion module is designed to dynamically balance global spectral semantics and local spatial–spectral details according to the pixel mixing complexity of varying spatial regions. Extensive experiments on multiple public hyperspectral datasets demonstrate that the proposed method achieves significant improvements in unmixing accuracy over comparative methods.

</details>


### [150] [Spatiotemporal Evolution of Ecological Environment Quality and Driving Factors in the Loess Plateau of Northern Shaanxi](https://doi.org/10.3390/rs18132219)
*Ruize Tang,Z L Li,Shuangcheng Zhang,Junkai Gu,Jiandong Xiao*

Main category: Remote Sensing

TL;DR: 基于RSEI模型评估了陕北黄土高原2000-2024年生态环境质量时空演变，发现EEQ呈波动上升趋势（均值从0.376增至0.545），空间上"南高北低、西北最低"，91.21%区域呈改善趋势，地貌类型是主导驱动因子。


<details>
  <summary>Details</summary>
Motivation: 现有研究多聚焦单一生态主题，无法反映区域整体生态状况；准确评估陕北黄土高原EEQ时空演变对巩固黄河流域生态安全屏障具有重要意义。

Method: 构建RSEI遥感生态指数模型；联合Theil-Sen估计量、Mann-Kendall检验和Hurst指数检测变化显著性并预测未来趋势；利用Geodetector模型探究驱动因素。

Result: (1) EEQ波动上升，RSEI均值从0.376升至0.545，年均增长0.00569；(2) "南高北低、西北最低"空间格局，"优良"级占比提升约20个百分点，"中等"级从13.61%扩至47.12%；(3) 改善区占91.21%，与未来改善预测区域高度重叠；(4) 地貌类型主导EEQ空间异质性（多年平均q=0.701），地貌-土地利用交互作用持续塑造空间分布。

Conclusion: RSEI模型能有效评估黄土高原EEQ时空动态，研究成果可为陕北黄土高原精准生态修复规划和空间优化提供科学依据。

与遥感交叉或改进方案: 交叉/改进可行性：95%。改进方案：(1) 可引入更多遥感参数优化RSEI模型，如加入土壤湿度指数(SMI)或植被净初级生产力(NPP)；(2) 结合更高时空分辨率遥感数据（如Sentinel-2、Landsat融合数据）提升评估精度；(3) 可引入深度学习时序分析（如LSTM）替代传统趋势预测方法，捕捉非线性演变特征；(4) 增加大气校正和地形校正步骤以减少山区遥感误差；(5) 可扩展至GEE平台实现大范围自动化监测。

Abstract (英文): Accurately assessing the spatiotemporal evolution of ecological environment quality (EEQ) on the Loess Plateau of Northern Shaanxi is of great significance for consolidating the ecological security barrier of the Yellow River Basin. Most of the existing research focuses on a single ecological theme, which does not reflect the overall ecological status of the region. In this study, a remote sensing ecological index (RSEI) model was constructed to systematically assess the EEQ from 2000 to 2024. The Theil–Sen estimator, Mann–Kendall test, and Hurst exponent were jointly employed to detect change significance and predict future trends, while the Geodetector model was applied to explore driving factors. The results were as follows: (1) EEQ exhibited a fluctuating but overall upward trend, with the mean RSEI rising from 0.376 in 2000 to 0.545 in 2024—an average annual increase of approximately 0.00569. (2) Spatially, a distinct pattern of “higher in the south, lower in the north and the lowest in the northwest” was observed. Over the 25-year period, the combined proportion of “excellent” and “good” grades increased by roughly 20 percentage points, and the “moderate” grade expanded from 13.61% to 47.12%. (3) Areas showing an improving trend accounted for 91.21% of the total area and highly overlapped with those projected to improve in the future. (4) Single-factor detection revealed that geomorphological type exerted the greatest influence on the spatial heterogeneity of EEQ, with a multi-year mean q-value of 0.701. Interaction detection further indicates that the geomorphology–land use interaction may continue to shape the regional EEQ’s spatial distribution. These findings provide a scientific basis for precise ecological restoration planning and spatial optimization on the Loess Plateau of Northern Shaanxi.

</details>


### [151] [Assessing the Potential of EMIT Hyperspectral Data Combined with DEM-Derived Terrain Variables for Predicting Soil As, Cu and Zn Concentrations in a Mountainous Region of Southwest China](https://doi.org/10.3390/rs18132211)
*Guangping Qie,Minzi Wang,Ziping Pan,Z X Sun,Wenjin Xie,Zhiyi Liu,Guangxing Wang*

Main category: Remote Sensing

TL;DR: 本研究利用EMIT高光谱数据结合DEM地形变量，通过随机森林和XGBoost模型预测贵州仁怀山区土壤砷(As)、铜(Cu)、锌(Zn)浓度。结果表明，As预测效果中等(验证R²=0.460)，Cu预测效果较差(R²=0.188)，Zn预测失败(R²=-0.028)，总体表明EMIT数据在山区重金属预测中能力有限。


<details>
  <summary>Details</summary>
Motivation: 星载成像光谱为区域尺度土壤属性监测提供了新机遇，但其在山区土壤重金属浓度预测中的应用尚未得到充分测试，特别是利用EMIT高光谱数据的情况，因此需要系统评估其可行性与预测能力。

Method: 1) 数据源：EMIT Level-2A地表反射率数据与DEM衍生的地形变量；2) 样本量：As 139个、Cu 136个、Zn 130个（仅使用EMIT覆盖范围内的土壤样本）；3) 特征筛选：对EMIT光谱变量和地形因子进行冗余消除；4) 建模：随机森林与XGBoost模型，采用重复空间交叉验证评估性能。

Result: As最佳模型（EMIT预测因子+高程）验证R²=0.460；Cu验证R²=0.188；Zn验证R²=-0.028（低于均值基准）。空间预测图与残差模式表明：As具有中等预测潜力，Cu预测价值有限，Zn仅为探索性制图，不具备可靠预测能力。

Conclusion: 在当前样本量和预测因子条件下，EMIT高光谱数据对山区土壤重金属预测能力有限，仅对As有中等潜力，对Cu和Zn的预测效果不理想，反映了高光谱遥感在复杂地形环境下的应用局限性。

与遥感交叉或改进方案: 交叉/改进可行性：65%。改进方案：1) 引入时序EMIT数据以增强光谱信息的时间稳定性；2) 结合多源遥感数据（如Sentinel-2、Landsat）进行光谱融合，弥补EMIT波段覆盖的不足；3) 采用深度学习模型（如CNN或3D-CNN）提取光谱-空间联合特征，替代传统随机森林/XGBoost；4) 增加地形精细因子（如坡度变率、地形湿度指数）和土壤母质信息；5) 采用迁移学习或半监督学习策略，缓解山区样本量不足的问题；6) 对重金属进行光谱解混预处理，降低混合像元影响。

Abstract (英文): Spaceborne imaging spectroscopy has created new opportunities for monitoring soil properties at regional scales. Its use for predicting soil heavy metal concentrations in mountainous environments, however, remains insufficiently tested, especially when EMIT hyperspectral data are used. In this study, EMIT Level-2A surface reflectance data were integrated with DEM-derived terrain variables to estimate soil arsenic (As), copper (Cu), and zinc (Zn) concentrations in Renhuai, Guizhou Province, Southwest China. Only soil samples falling within valid EMIT coverage were used for element-specific modeling, resulting in 139 samples for As, 136 for Cu, and 130 for Zn. To reduce redundancy among predictors, EMIT spectral variables and terrain factors were screened before model construction. Random forest and XGBoost models were then tested using repeated spatial cross-validation. The best-performing model for As combined EMIT predictors with elevation and achieved a validation R2 of 0.460. Model performance was considerably weaker for Cu, with a validation R2 of 0.188. For Zn, the model failed to outperform the mean-based benchmark, producing a negative validation R2 of −0.028. The spatial prediction maps and residual patterns suggested that the EMIT-based prediction showed moderate potential for As, limited predictive value for Cu, and exploratory rather than reliable mapping capability for Zn under the current sample and predictor conditions.

</details>


### [152] [Beyond Checkerboards: Advantages of Photogrammetric Camera Calibration for Robust 3D Vision and Novel-View Generation](https://doi.org/10.3390/rs18132220)
*Akshay Bharadwaj,Derek D. Lichti*

Main category: Remote Sensing

TL;DR: 本文通过对比实验证明，摄影测量自检校（PSC）在精度和准确度上均优于传统棋盘格标定方法，并在SfM、视觉SLAM和新视角合成等视觉任务中表现更优，倡导计算机视觉领域向更符合物理投影特性的标定方法转变。


<details>
  <summary>Details</summary>
Motivation: 棋盘格标定法虽简单易用，但在高精度相机几何建模方面存在局限，可能影响下游任务的几何精度表现。本文旨在通过系统对比，证明摄影测量自检校（PSC）在多项视觉任务中优于传统棋盘格方法，推动标定范式的转变。

Method: 将相机标定问题置于摄影测量自检校（PSC）框架下重新审视，通过SfM、视觉SLAM和新视角合成与重建等多项视觉任务，系统对比PSC与传统棋盘格标定方法在精度和准确度上的表现。

Result: PSC在3D重建（SfM）、视觉SLAM以及新视角合成与重建任务中，在精度和准确度方面均一致优于棋盘格标定方法。

Conclusion: 作者呼吁计算机视觉领域应转向更能反映相机系统物理与投影特性的标定方法，以替代传统棋盘格标定，特别是在关键应用场景中。

与遥感交叉或改进方案: 交叉/改进可行性：95%。遥感领域对几何精度要求极高（如立体测图、正射校正、高精度三维重建），PSC可替代传统棋盘格标定用于遥感相机（包括星载、机载和地面传感器）的内参数检校。具体方案：(1) 结合遥感影像的线阵/面阵传感器模型，扩展PSC以适应推扫式成像几何；(2) 将PSC与光束法平差集成，用于多视角遥感影像的联合自检校三维重建；(3) 在无人机遥感SfM/SLAM系统中，利用PSC提升无控制点或弱控制条件下的几何精度。

Abstract (英文): Imagery is fundamental to modern scientific research, making robust intrinsic camera calibration indispensable for accurate visual inference. The checkerboard-based calibration method has long been favored for its simplicity and ease of deployment and is widely used even in mission-critical computer vision pipelines. However, its limitations in modeling high-precision camera geometry can compromise downstream performance in tasks requiring geometric accuracy. In this work, camera calibration is revisited through the lens of photogrammetric self-calibration (PSC), and it is demonstrated that the PSC consistently outperforms the checkerboard method in both accuracy and precision across a range of vision tasks, including 3D reconstruction with structure from motion (SfM), visual simultaneous localization and mapping (SLAM), and novel-view synthesis and reconstruction. Our findings advocate for a paradigm shift toward calibration methods that better reflect the physical and projective properties of camera systems in real-world deployments for critical computer vision applications.

</details>


### [153] [Multi-Modal Multi-Scale Mamba with Frequency and Semantics-Guided Spectral Modeling for Hyperspectral Image Classification](https://doi.org/10.3390/rs18132207)
*Hui Peng,Wenzhun Huang,Kaihong Chen,Zhongyan Zhou,Zhen Wang*

Main category: Remote Sensing

TL;DR: M3-Mamba是一个将语言引导、多层级与Mamba状态空间模型相结合的高光谱图像分类框架，通过语义引导光谱调制和频域感知的Mamba模块，有效解决了高光谱冗余、光谱-空间相关性复杂和标注样本稀缺等问题，在四个公开数据集上取得了具有竞争力的分类性能。


<details>
  <summary>Details</summary>
Motivation: 高光谱图像分类面临光谱冗余度高、光谱-空间相关性复杂以及标注样本有限三大挑战，现有方法难以高效建模长程光谱依赖且计算复杂度高，亟需一种既能捕捉全局光谱依赖又具备线性计算复杂度的有效框架。

Method: 提出M3-Mamba框架，包含三个关键组件：(1) 利用多模态表示的高层语义先验引导判别性光谱建模；(2) 频域感知的Mamba状态空间模块，高效捕获长程光谱依赖且避免注意力机制的二次计算复杂度；(3) 文本引导的调制策略，在语义引导下自适应重加权光谱响应，抑制冗余/噪声波段并增强类别相关响应。

Result: 在Indian Pines、Pavia University、Salinas和Houston四个基准数据集上，M3-Mamba在总体精度、平均精度和Kappa系数上均取得了有竞争力的结果，消融实验验证了各关键模块的有效性。

Conclusion: M3-Mamba通过语义到光谱的调制机制有效应对光谱变异和类间混淆问题，在高光谱图像分类任务中展现出良好的有效性。

与遥感交叉或改进方案: 交叉/改进可行性：95%。改进方案：(1) 可将M3-Mamba中的频域感知Mamba模块替换或融合为遥感领域常用的空-谱联合Mamba变体，以同时捕获空间上下文信息；(2) 文本引导调制策略可引入遥感领域的知识图谱或地理语义标签，增强对地物类别先验知识的利用；(3) 可扩展至多时相高光谱遥感影像分类任务，利用语言引导机制处理时序光谱变化；(4) 针对遥感大场景分类，可设计轻量化的Mamba状态空间模块以降低推理延迟。

Abstract (英文): Hyperspectral image (HSI) classification remains a challenging task due to high spectral redundancy, complex spectral–spatial correlations, and the limited availability of labeled samples. To address these issues, this paper proposes a novel framework termed M3-Mamba, which integrates language-guided, multi-level, and Mamba-based spectral modeling for hyperspectral image classification. The proposed M3-Mamba leverages high-level semantic priors derived from multimodal representations to guide discriminative spectral modeling, enabling effective interaction between semantic information and fine-grained spectral features. In addition, a frequency-aware Mamba-based state space module is introduced to efficiently capture long-range spectral dependencies while avoiding the quadratic computational complexity of conventional attention mechanisms. Meanwhile, a text-guided modulation strategy is designed to adaptively reweight spectral responses under semantic guidance, suppressing redundant or noisy bands and enhancing class-relevant spectral responses without compromising spectral fidelity. This semantic-to-spectral modulation allows M3-Mamba to better cope with spectral variability and inter-class confusion. Extensive experiments conducted on four widely used benchmark datasets, including Indian Pines, Pavia University, Salinas, and Houston datasets, demonstrate thatM3-Mamba achieves competitive overall accuracy, average accuracy, and Kappa coefficient under the adopted benchmark settings. Ablation studies further validate the effectiveness of each key component, confirming that the proposed framework demonstrates promising effectiveness for hyperspectral image classification.

</details>


### [154] [Using Contemporary Global Land Cover Products to Improve Forest Mapping at the National Scale: Case Study of Poland](https://doi.org/10.3390/rs18132215)
*Mahsa Shahbandeh,Dominik Kaim,Jacek Kozak*

Main category: Remote Sensing

TL;DR: 本研究评估了三种10米分辨率全球土地覆盖产品（GDW、ESA WC、ELC）在波兰森林制图中的精度，发现通过特定组合（三种产品共同识别的森林区域 + 仅GDW和ESA WC共同识别的森林区域）可得到比任何单一产品更高的精度（F1分数80.4%，IoU 87.1%）。


<details>
  <summary>Details</summary>
Motivation: 准确的土地覆盖数据对监测土地利用/覆盖变化及评估环境政策有效性至关重要，但现有全球产品在区域尺度上的森林制图精度尚不明确，且单一产品可能存在偏差，因此需要评估并探索多产品融合能否提高精度。

Method: 以2022年欧盟LUCAS调查为参考数据，以波兰S2GLC PL产品（2021年）为辅助参考，对三种全球土地覆盖产品（GDW、ESA WC、ELC）及其组合进行森林覆盖分类精度评估，采用精确率、召回率、F1分数和交并比（IoU）指标，并在国家和省份两个尺度上分析空间一致性。

Result: 单一产品F1分数：ELC 72.2%、ESA WC 76.9%、GDW 68.8%；IoU（vs S2GLC PL）：ELC 82.6%、ESA WC 82.3%、GDW 75.2%，三者共同IoU略超70.5%。特定组合（三产品共同森林 + 仅GDW与ESA WC共同森林）的F1分数为80.4%，IoU达87.1%，优于所有单一产品及其他组合。

Conclusion: 通过合理组合多种全球土地覆盖产品可以显著提高森林制图精度，优于任何单一产品，为区域尺度森林监测提供了更可靠的方案。

与遥感交叉或改进方案: 交叉/改进可行性：95%。该研究提出的多产品组合策略可直接迁移至遥感森林制图任务：(1) 在遥感森林监测中，可融合不同传感器（如Sentinel-2、Landsat、SAR）或不同算法的土地覆盖产品，通过投票或逻辑规则组合提升精度；(2) 针对中国或其他区域，可结合已有的高精度局地产品与全球产品进行类似组合优化；(3) 方法可扩展至其他土地覆盖类型（如水体、农田、城市）的精度提升；(4) 可引入深度学习中的集成学习思想（如Bagging、Boosting）对多产品进行加权融合，进一步提升鲁棒性。

Abstract (英文): Using accurate land cover data is essential to monitor land use and cover changes and assess the effectiveness of various environmental policies. This study evaluates the accuracy of contemporary global land cover products with 10 m spatial resolution, including Google’s Dynamic World (GDW), European Space Agency’s World Cover (ESA WC) and Esri Land Cover (ELC) in mapping forested areas in Poland, aiming to test an assumption if the combination of these products may improve forest mapping accuracy compared to any individual product. Three global datasets and their combinations were assessed with the 2022 EU Land Use/Cover Area Frame Survey (LUCAS). A land cover map of Poland (S2GLC PL) for 2021 served as an auxiliary reference data set. Forest cover classification accuracy was evaluated using precision, recall, and F1-score metrics, and spatial agreement of binary forest maps in the thematic global products was measured with the Intersection over Union (IoU) at two various scale levels (country and province). Our results showed that forest mapping accuracy of three global products varies for Poland, with F1-score equal to 72.2% for ELC, 76.9% for ESA WC, and 68.8% for GDW. IoU against S2GLC PL was equal to 82.6%, 82.3% and 75.2%, for ELC, ESA WC and GDW, respectively, and slightly exceeded 70.5% for three global products. A specific combination of binary forest maps from global products, where the output forest area consisted of forests mapped at the same time by all three products and forests mapped at the same time only by GDW and ESA WC yielded better accuracy indicators than any single product and other tested combinations (F1-score equal to 80.4%, and IoU against S2GLC PL equal to 87.1%).

</details>


### [155] [SMMNet: A Plug-and-Play Lightweight Detection Framework for UAV Aerial Imagery](https://doi.org/10.3390/rs18132232)
*Minna Liu,Zhigang Luo,Yaowen Hu,Jialang Liu*

Main category: Remote Sensing

TL;DR: 本文提出SMMNet，一种轻量级即插即用的无人机航拍图像目标检测框架。该框架包含三个核心模块：各向异性扩散特征提取器（SDFE）、Mamba驱动的上下文聚合器（MRCA）和贝叶斯框细化模块（MBBR）。在VisDrone2019数据集上，以YOLOv13-S为基线，mAP@0.5:0.95提升3.6个点达到32.8%，在A800上以87 FPS实时运行，在Jetson Orin NX边缘设备上达30 FPS，实现了精度与速度的良好平衡。


<details>
  <summary>Details</summary>
Motivation: 无人机航拍图像中的目标检测面临密集小目标、大尺度变化、复杂背景以及严格的机载计算资源限制等多重挑战，现有方法难以在轻量化条件下兼顾检测精度和实时性。

Method: 提出SMMNet，包含三个模块：(1) SDFE：利用各向异性扩散在下采样过程中保留边界敏感特征；(2) MRCA：使用Mamba驱动的多方向选择性状态空间扫描，以线性复杂度捕获长距离上下文；(3) MBBR：基于最大后验估计（MAP）的置信度自适应框更新，结合MobileSAM掩膜证据和ELBO伪阳性过滤。

Result: 在VisDrone2019上mAP@0.5:0.95达32.8%、mAP@0.5达52.6%，较YOLOv13-S基线分别提升3.6和4.5个点；A800上87 FPS；Jetson Orin NX上30 FPS（TensorRT FP16, 15W TDP）；在DroneVehicle和SeaDronesSee数据集上分别有2.53±0.12和2.77±0.06 mAP的提升。

Conclusion: SMMNet作为轻量即插即用框架，能有效提升YOLO系列检测器在无人机航拍场景下的性能，在精度-延迟权衡上表现优异，且适用于边缘部署，具备实际应用价值。

与遥感交叉或改进方案: 交叉/改进可行性：95%。本文本身就是面向遥感领域的无人机航拍目标检测论文，可直接应用于遥感场景。改进方向：(1) SDFE可引入多尺度各向异性扩散以更好应对遥感图像中地物尺度极端差异；(2) MRCA可扩展为三维状态空间扫描以处理视频/时序遥感数据；(3) MBBR可结合遥感先验知识（如地物几何约束）增强框细化效果；(4) 可尝试替换MobileSAM为更轻量的遥感专用分割模型以进一步降低边缘部署计算开销。

Abstract (英文): Object detection in UAV aerial imagery is challenged by dense small targets, large-scale variation, complex backgrounds, and strict onboard computation limits. To address these issues, this paper proposes SMMNet (Structured-diffusion Mamba Mixture Network), a lightweight plug-and-play detection framework evaluated with YOLO family detectors. SMMNet contains three modules. The Structured Diffusion Feature Extractor (SDFE) uses anisotropic diffusion to preserve boundary-sensitive features during downsampling. The Mamba-driven Receptive-field Context Aggregator (MRCA) performs multi-directional selective state-space scanning to capture long-range context with linear complexity. The Mask-guided Bayesian Box Refinement (MBBR) applies a MAP-inspired confidence-adaptive box update using MobileSAM mask evidence and ELBO-based false-positive filtering. Using YOLOv13-S as the main detector, SMMNet achieves 32.8% mAP@0.5:0.95 and 52.6% mAP@0.5 on VisDrone2019 at 87 FPS on an NVIDIA A800 GPU, improving the YOLOv13-S baseline by 3.6 and 4.5 points, respectively. The added modules reduce throughput compared with the detector-only baseline (168 FPS), but the resulting 87 FPS remains real-time and provides a favorable accuracy–latency trade-off. Three independent-seed runs further show a mean paired gain of 3.60 ± 0.10 mAP on VisDrone2019, 2.53 ± 0.12 mAP on DroneVehicle, and 2.77 ± 0.06 mAP on SeaDronesSee for the YOLOv13-S setting. Additional experiments on DroneVehicle and SeaDronesSee, together with cross-backbone evaluations on YOLOv5/v6/v7/v8/v10/v11/v13 across different UAV benchmarks, show aligned performance trends under matched settings. Edge deployment on an NVIDIA Jetson Orin NX reaches 30 FPS under TensorRT FP16 inference at 15 W TDP, indicating the suitability of SMMNet for resource-constrained UAV perception.

</details>


### [156] [Enhanced DINO for Cross-Domain Transmission Tower Detection Using Remote Sensing Images](https://doi.org/10.3390/rs18132198)
*Junjie Wang,Jiahe Tian,Lingli Zhao,Ningsheng Liao,Jie Yang,Lei Shi*

Main category: Remote Sensing

TL;DR: 本文提出了一种基于增强型DINO的跨域输电塔检测框架，通过查询级目标门控(QOG)、MPDIoU回归和质量感知评分模块(QSM)三个轻量级优化模块，有效解决了不同地理区域遥感图像中输电塔因结构、尺度和背景纹理差异导致的检测精度下降问题。在10%目标域标注设置下，精度达0.8947，召回率0.8199，F1-score 0.8556，mAP@0.5 0.8684，优于原始DINO及YOLOv8/v9/v11等主流检测器。


<details>
  <summary>Details</summary>
Motivation: 不同地理区域的遥感图像中，输电塔的结构、尺度和背景纹理存在显著差异，导致在一个区域训练的模型在应用于另一区域时检测精度大幅下降，亟需一种有效的跨域检测解决方案。

Method: 1) 查询级目标门控(QOG)模块：通过估计每个查询的目标性得分，自适应地重新加权解码器查询，抑制背景主导的查询；2) MPDIoU回归损失：提升细长输电塔目标的定位精度；3) 质量感知评分模块(QSM)：利用预测的定位质量logits校准分类置信度，减少由边界框对齐不良引起的高置信度误检。

Result: 在两个不同地理区域的遥感图像数据集上，10%目标域标注设置下，精度0.8947，召回率0.8199，F1-score 0.8556，mAP@0.5 0.8684，全面超越原始DINO基线及YOLOv8、YOLOv9、YOLOv11等主流检测器。

Conclusion: 提出的增强型DINO框架通过三个轻量级优化模块为遥感图像中细长输电塔目标的鲁棒跨域检测提供了一种有效解决方案。

与遥感交叉或改进方案: 交叉/改进可行性：95%。改进方案：1) QOG模块可推广至其他遥感细长目标（如桥梁、道路、电力线）的跨域检测场景；2) MPDIoU损失函数适用于遥感图像中任意长宽比悬殊的目标定位优化；3) QSM的质量感知校准策略可集成到其他Transformer-based遥感检测器中提升伪影抑制能力；4) 可进一步引入域自适应模块（如对抗训练、风格迁移）以增强模型在极端域偏移下的泛化能力。

Abstract (英文): Transmission towers are fundamental components of electric power networks. However, their structure, scale and background textures vary substantially across remote sensing images acquired from different geographic regions. These discrepancies often reduce the detection accuracy of a model trained in one region when it is applied to another region. This paper proposes an enhanced DINO-based framework for cross-domain transmission tower detection that incorporates three lightweight optimisation modules. First, a Query-level Objectness Gating (QOG) module adaptively reweights decoder queries by estimating per-query objectness scores, thereby suppressing background-dominated queries. Second, MPDIoU regression is used to improve the localisation accuracy of elongated transmission tower targets. Third, a Quality-aware Scoring Module (QSM) calibrates classification confidence using predicted localisation-quality logits, thereby reducing high-confidence false detections caused by poor box alignment. Experiments are conducted on two remote sensing image datasets from different geographic regions. Under the 10% target-domain annotation setting, the proposed method achieves a precision of 0.8947, a recall of 0.8199, an F1-score of 0.8556 and an mAP@0.5 of 0.8684, outperforming the original DINO baseline and mainstream detectors including YOLOv8, YOLOv9 and YOLOv11. The results demonstrate that the proposed framework provides an effective solution for robust cross-domain detection of slender transmission tower targets in remote sensing images.

</details>


### [157] [TreeFlow: Conditional Flow Matching for 3D Tree Point Cloud Generation from Inventory Attributes](https://doi.org/10.3390/rs18132197)
*Anthony Marcozzi,Johnathan Tenny,Daithi Martin,Juan Castorena,Zachary Crennen,Lucas Wells,Samuel Hillman*

Main category: Remote Sensing

TL;DR: TreeFlow是一个基于条件流匹配模型，利用真实激光扫描数据训练，根据树种、采集平台和高度生成逼真的三维树木点云，在多数指标上接近真实树木水平，但在25米以上高度性能有所下降。


<details>
  <summary>Details</summary>
Motivation: 树木的精确三维结构表示对火灾建模、辐射传输模拟、合成数据生成和森林数字孪生至关重要，但当前方法使用光滑几何体近似树冠，忽略了真实树木的聚集、间隙和不规则分支结构，缺乏在所需尺度上的详细3D结构数据。

Method: TreeFlow采用条件流匹配模型，使用Transformer架构，在FOR-species20K基准数据集上的真实激光扫描数据上训练，学习一个速度场将高斯分布的样本传输到源数据分布，生成以物种、采集平台和树高为条件的3D树木点云。

Result: 在六项指标中的五项达到或接近类内基线水平，Chamfer距离为0.581米（真实树木为0.559米）；性能在25米以下最强，随高度增加而下降。

Conclusion: TreeFlow能够基于标量清单属性生成个体树木点云，且完全基于真实激光扫描数据训练，为大规模树木三维结构生成提供了可行的数据驱动方案。

与遥感交叉或改进方案: 交叉/改进可行性：90%。改进方案：（1）将TreeFlow生成的树木点云作为遥感辐射传输模型的输入，替代传统几何简化冠层，提升模拟精度；（2）利用生成的多样化树木点云作为训练数据，增强遥感树种分类和生物量估测模型的泛化能力；（3）针对高树木（>25m）性能退化问题，可引入多尺度或分层流匹配架构，或融合星载/无人机多源激光雷达数据补充高层结构信息。

Abstract (英文): Accurate three-dimensional representations of tree structure are essential for fire modeling, radiative transfer simulation, synthetic data generation, and digital twins of forests, yet detailed 3D structure is rarely available at required scales. Current approaches approximate crowns with smooth geometric primitives, discarding the clumping, gaps, and irregular branching present in real trees. We present TreeFlow, a conditional flow matching model that generates realistic 3D tree point clouds from species, acquisition platform, and height. The model uses a transformer trained on real laser scanning data from the FOR-species20K benchmark to learn a velocity field transporting samples from a Gaussian distribution to the source data distribution. We evaluate generation quality by comparing conditioning and distributional fidelity metrics to scans of real trees. Generated trees match or approach the intra-class baseline on five of six metrics, with a Chamfer distance of 0.581 m versus 0.559 m for real trees of the same genus and height class. Performance is strongest below 25 m and degrades with increasing height. TreeFlow generates individual-tree point clouds conditioned on scalar inventory attributes using a model trained entirely on real laser scanning data.

</details>


### [158] [Fine-Grained Spaceborne SAR Ship Classification into Nine Categories via AIS Association](https://doi.org/10.3390/rs18132223)
*X Chen,Yi Zhang,Lizhen Hu,Hongyi Zhang,Liangsheng Li,Xupu Geng*

Main category: Remote Sensing

TL;DR: 本文提出一种基于AIS关联的星载SAR船舶细粒度九分类方法，通过全局最优匹配策略将AIS船舶类型信息迁移至SAR检测结果，构建包含4472张船舶切片的分类数据集，实验证明该方法能提供可靠的类别标签，且训练后的分类器可仅依赖SAR图像进行船舶分类。


<details>
  <summary>Details</summary>
Motivation: SAR船舶检测已取得显著进展，但SAR船舶分类研究相对不足，主要原因是缺乏可靠的类别标签。AIS数据能提供船舶身份、类型和动态轨迹信息，因此本文旨在利用AIS关联来解决SAR船舶分类中标签稀缺的问题。

Method: 1) 将原始AIS船舶类型重新组织为九种细粒度SAR船舶类别；2) 通过全局最优匹配策略将AIS船舶类型信息迁移至SAR检测结果；3) 仅保留高置信度的SAR-AIS匹配对，裁剪对应SAR船舶切片，构建包含4472张船舶切片的九类别分类数据集；4) 使用ConvNeXt-Tiny等主流分类器进行基准实验。

Result: 在基于真实AIS记录的蒙特卡洛实验中，所提关联策略在近距离船舶歧义、空间扰动、干扰AIS候选和AIS静态尺寸误差等场景下，比对比方法生成更可靠的高置信度标签。在构建的分类数据集基准实验中，ConvNeXt-Tiny在主流分类器中表现最佳。

Conclusion: AIS关联能够为SAR船舶分类提供可靠的类别监督信息，训练后的分类器可仅依靠SAR图像独立完成船舶分类任务。

与遥感交叉或改进方案: 交叉/改进可行性：85%。改进方案：1) 可引入多时相SAR数据与AIS的时序关联策略，进一步提升匹配鲁棒性；2) 可结合深度学习特征匹配代替传统的几何/统计匹配，提升在复杂场景下的关联精度；3) 该AIS-SAR关联框架可扩展至其他海洋遥感目标（如油污、冰山、海上平台）的识别中；4) 可探索弱监督或半监督方法，利用大量未匹配的SAR切片进一步优化分类器性能。

Abstract (英文): Spaceborne Synthetic Aperture Radar (SAR) provides all-weather, day and night and wide-area imaging capability, and plays a critical role in maritime surveillance. While substantial progress has been achieved in SAR ship detection, SAR ship classification remains relatively underexplored, mainly due to the scarcity of reliable category labels. Automatic Identification System (AIS) provides vessel identity, type, and dynamic trajectory information, and thus offers vessel type information that is difficult to obtain directly from SAR imagery. This paper proposes a fine-grained nine-category SAR ship classification method based on AIS association, which reorganizes the original AIS vessel types into nine fine-grained categories of SAR ship, transfers AIS vessel type information to SAR detection through a global optimal matching strategy, and supports SAR-only vessel category recognition. By retaining only high-confidence SAR and AIS matched pairs and cropping the corresponding SAR ship chips, an SAR ship classification dataset containing 4472 ship chips across the nine categories is constructed. In Monte Carlo experiments based on real AIS records, the proposed association strategy achieves more reliable high-confidence label generation than the compared association methods under close ship ambiguity, spatial perturbation, distractor AIS candidates, and AIS static size errors. In the benchmark experiment on the constructed classification dataset, ConvNeXt-Tiny achieves the best performance among the compared mainstream classifiers. These results demonstrate that AIS association can provide reliable category supervision for SAR ship classification, and the trained classifier can perform ship classification using SAR imagery alone.

</details>


### [159] [3D Gaussian Splatting for Large-Scale Remote Sensing: A PRISMA-Informed Scoping Review of Scalability, Geometric Reliability, and Benchmarking Across UAV/Aerial and Satellite Imagery](https://doi.org/10.3390/rs18132224)
*Wenbao Fan,Bo Wang,Junqiang Ye,Ruoyu Zha,H T Chen*

Main category: Remote Sensing

TL;DR: 本文基于PRISMA方法对55篇核心文献进行系统综述，探讨3D高斯泼溅（3DGS）在大范围遥感应用中的现状与挑战，并提出一个平台感知的基准框架，用于统一评估视觉质量、计算成本、几何精度、产品实用性及可重复性。


<details>
  <summary>Details</summary>
Motivation: 3DGS在遥感领域（无人机/航空摄影测量、卫星重建、大场景缩放、表面建模、地理空间评估）的应用碎片化严重，缺乏系统性的综述与统一的评估框架，阻碍了其可靠部署与跨平台比较。

Method: 采用PRISMA系统综述方法，从Web of Science、Scopus、IEEE Xplore等数据库筛选55篇核心文献，构建包含平台、传感器模型、可扩展性策略和几何监督的多面分类体系，对现有方法进行结构化分析。

Result: 现有方法在可扩展性方面取得进展（分区、层次结构、压缩、前馈推理），但无法保证度量级几何精度。可靠部署还需传感器一致投影、几何/地理配准约束、显式监督标签及产品级评估。在无控制点场景中，内部一致性与独立验证精度需明确区分。

Conclusion: 提出一个平台感知的基准框架，涵盖无人机/航空、卫星及混合场景，统一记录视觉保真度、计算成本、度量几何、产品实用性、失败行为及可重复性元数据，为3DGS在遥感领域的标准化评估提供基础。

与遥感交叉或改进方案: 交叉/改进可行性：95%。本文本身就是遥感领域的系统综述，可直接指导3DGS在遥感中的改进方向：（1）在3DGS训练中引入传感器投影模型（如RPC模型、共线方程）替代简化针孔模型；（2）增加地理配准约束与GPS/IMU先验作为优化项；（3）采用分区+层级细节（LOD）策略实现大场景覆盖；（4）引入显式深度或法向量监督标签提升几何精度；（5）设计面向遥感产品的评估指标（如DEM误差、正射影像一致性）而非仅关注渲染PSNR。

Abstract (英文): 3D Gaussian Splatting (3DGS) offers efficient explicit rendering, but large-scale remote-sensing use remains fragmented across UAV/aerial photogrammetry, satellite reconstruction, large-scene scaling, surface modeling, and geospatial evaluation. We present a Preferred Reporting Items for Systematic Reviews and Meta-Analyses (PRISMA)-informed scoping review based on 55 core studies identified through Web of Science, Scopus, IEEE Xplore, and supplementary searches completed on 3 June 2026. A faceted taxonomy organizes the literature by platform, sensor model, scalability strategy, and geometric supervision. The synthesis shows that partitioning, hierarchy, compression, and feed-forward inference improve scalability but do not guarantee metric geometry. Reliable deployment additionally requires sensor-consistent projection, geometric or georeferencing constraints, explicit supervision labels, and product-level evaluation. In control-point-free settings, internal consistency should be distinguished from independently validated accuracy. We therefore propose a platform-aware benchmark framework that jointly records visual fidelity, computational cost, metric geometry, product utility, failure behavior, and reproducibility metadata for UAV/aerial, satellite, and hybrid settings.

</details>


### [160] [SC-Net: Structural Constrained Contrastive Learning for Landslide Extraction Toward Power Transmission Corridor Safety Monitoring](https://doi.org/10.3390/rs18132216)
*Wei Song,S. Liu,S S Wu,Cheng Liao,Zongyuan Wu,Shiming Li,X B Zheng,Yanping Duan*

Main category: Remote Sensing

TL;DR: 本文提出一种结构约束对比学习网络(SC-Net)，用于从遥感影像中可靠提取滑坡。该方法通过多结构特征提取模块捕获滑坡几何特征，结合注意力自适应融合模块融合多尺度语义，并引入掩码约束的物体级对比学习策略增强滑坡与非滑坡区域的判别性。在CAS滑坡数据集上，SC-Net在CAS-UAV和CAS-SAT数据集上分别达到89.89%和79.76%的IoU，优于现有方法。


<details>
  <summary>Details</summary>
Motivation: 滑坡是常见且破坏性极强的地质灾害，尤其在电力传输走廊穿越的山地森林区域，密集植被导致严重遮挡，使滑坡边界模糊且与周围地物视觉相似，现有遥感影像滑坡识别方法精度和效率不足。

Method: 1) 多结构特征提取模块，捕获滑坡特有的几何结构特征；2) 基于注意力机制的自适应特征融合模块，融合预训练骨干网络提取的多尺度语义表示；3) 掩码约束的物体级对比学习策略，在滑坡物体级别强制全局结构一致性，增强判别能力。

Result: 在CAS-UAV数据集上IoU达89.89%，在CAS-SAT数据集上IoU达79.76%，分别比最优基线方法提升2.09%和0.46%。

Conclusion: SC-Net为大规模滑坡监测提供了有效解决方案，并在电力传输走廊巡检和基础设施安全评估中展现出应用潜力。

与遥感交叉或改进方案: 交叉/改进可行性：95%。本文本身就是遥感滑坡提取论文。改进方向：1) 可引入多时相遥感数据以增强时序动态特征的捕捉能力，缓解单一影像中植被遮挡问题；2) 可结合InSAR形变监测数据作为辅助特征输入，提升对早期微弱滑坡迹象的识别能力；3) 结构特征提取模块可扩展至更通用的几何先验知识（如道路、建筑物），提升模型在其他遥感分割任务中的泛化能力；4) 对比学习策略可引入难例挖掘机制，进一步提升边界模糊区域的判别精度。

Abstract (英文): Landslides are among the most common and destructive geological hazards and pose a significant threat to the long-term stability of infrastructure systems. In particular, long-distance power transmission corridors often traverse mountainous and forested regions, where landslides can endanger tower foundations and transmission line safety. Such landslides predominantly occur in sloped forested areas, where dense vegetation causes severe occlusion that blurs landslide boundaries and creates strong visual similarity with surrounding land covers. Consequently, accurate and efficient landslide identification from remote sensing imagery remains a significant challenge. To address these challenges, we propose a structural constrained contrastive learning network (SC-Net) for reliable landslide extraction from remote sensing images. First, a multi-structural feature extraction module is designed to capture landslide-specific geometric characteristics. These features are further enhanced by fusing multi-scale semantic representations extracted from a pretrained backbone network through an attention-based adaptive feature fusion module. Additionally, a mask-constrained object-level contrastive learning strategy is introduced to enforce global structural consistency at the landslide object-level, thereby improving the discriminability between landslide and non-landslide regions. Extensive experiments conducted on the publicly available CAS landslide dataset demonstrate the effectiveness of the proposed method. The proposed SC-Net achieves IoU scores of 89.89% and 79.76% on the CAS-UAV and CAS-SAT datasets, respectively, outperforming the best-performing baseline by 2.09% and 0.46%. The proposed method provides an effective solution for large-scale landslide monitoring and demonstrates potential for applications in power transmission corridor inspection and infrastructure safety assessment.

</details>


### [161] [RepLite-YOLO: A Parameter-Efficient Residual-Enhanced Detector for Ship Recognition in Remote Sensing Imagery](https://doi.org/10.3390/rs18132238)
*Ruijia Fu,Zuomin Wang,Zijun Lin,Ying Li,Bingxin Liu*

Main category: Remote Sensing

TL;DR: 提出RepLite-YOLO轻量级遥感舰船检测框架，在YOLOv11n基础上通过ADown模块、C3k2_OREPA_RS模块、ELANFusion_Block和Detect_DWLite检测头等创新，参数量减少57.4%、GFLOPs减少49.2%，同时在两个舰船数据集上mAP@50分别提升1.2和1.3个百分点。


<details>
  <summary>Details</summary>
Motivation: 当前遥感舰船检测方法面临复杂海面背景噪声、目标尺度变化大以及边缘硬件限制等挑战，现有方法难以在轻量化和检测精度之间取得良好平衡。

Method: 1) 采用ADown模块（源自YOLOv9）的双分支下采样机制保留空间特征并抑制海面噪声；2) 设计C3k2_OREPA_RS模块，利用在线重参数化(OREPA)在严格硬件约束下实现结构精简；3) 构建ELANFusion_Block，将深度可分离卷积(DSC)集成到ELAN范式中缓解多尺度聚合瓶颈；4) 设计Detect_DWLite检测头实现高度压缩的解耦预测。

Result: 相比YOLOv11n，参数量减少57.4%，GFLOPs减少49.2%；在Vessel数据集和Ship Detection数据集上mAP@50分别提升1.2和1.3个百分点。

Conclusion: RepLite-YOLO在检测精度与计算效率之间取得了良好平衡，验证了所提轻量化模块在遥感舰船检测任务中的有效性。

与遥感交叉或改进方案: 交叉/改进可行性：95%。该论文本身就是遥感舰船检测论文。改进方案：1) 可引入注意力机制（如CBAM、CA）进一步增强海面复杂背景下的特征判别能力；2) ADown模块的参数化设计可针对更小尺度舰船目标做进一步优化，如自适应分支权重；3) 可引入多尺度特征增强策略（如ASFF、BiFPN）以应对舰船尺度变化极端场景；4) 可进一步探索在无人机(DJI)等更受限边缘设备上的部署验证。

Abstract (英文): Remote sensing ship detection plays a pivotal role in maritime surveillance, safety assurance, and traffic management. However, current detection methods often face significant challenges due to complex sea-surface background noise, large target-scale variations, and edge-hardware limitations. In this paper, we propose RepLite-YOLO, a lightweight detection framework based on YOLOv11n. Specifically, to alleviate irreversible spatial information loss during downsampling, we adopt the ADown module, originally introduced in YOLOv9, to generate spatially complementary features through its two-branch downsampling mechanism. This design helps preserve salient hull-edge responses while suppressing part of the random sea-surface interference, thereby improving feature robustness for small ship targets. To achieve substantial structural streamlining while maintaining competitive representational capacity under strict hardware constraints, we design the C3k2_OREPA_RS module, utilizing online re-parameterization (OREPA) to efficiently reconstruct deep layers without additional re-parameterization-induced inference operations. Furthermore, we construct the ELANFusion_Block by integrating Depthwise Separable Convolutions (DSC) into the ELAN paradigm to alleviate the multi-scale aggregation bottleneck, and tailor the Detect_DWLite head for highly compressed decoupled prediction. Experimental results show that RepLite-YOLO achieves a favorable balance between detection accuracy and computational efficiency. Compared with YOLOv11n, it reduces the number of parameters by 57.4% and GFLOPs by 49.2%, while maintaining competitive detection accuracy with slight mAP@50 improvements of 1.2 and 1.3 percentage points on the Vessel dataset and Ship Detection dataset, respectively.

</details>


### [162] [HCA-YOLO: A Hierarchical Cross-Scale Attention Learning Framework for UAV Detection](https://doi.org/10.3390/rs18132196)
*Wei Wang,Y Zhang,Y Zhang,Lingjun Zhao,Xingwei Yan*

Main category: Remote Sensing

TL;DR: 针对无人机检测中漏检、误报和精度差的问题，提出了一种分层注意力驱动的跨尺度学习框架，通过分层注意力机制生成多尺度特征、双分支准残差结构保护目标内容、动态聚焦损失优化边界框回归，在恶劣光照和天气条件下对小弱无人机检测的mAP提升18.5%。


<details>
  <summary>Details</summary>
Motivation: 现有无人机检测方法在恶劣场景条件和目标尺度多变的情况下，存在漏检、误报和精度差的问题。

Method: (1) 在主干网络中引入分层注意力机制生成多尺度特征；(2) 在颈部构建双分支特征：一支通过目标特定特征算子获得，另一支通过上采样生成，并采用准残差结构连接两支以保护目标内容并重建细节信息；(3) 提出动态聚焦损失度量用于边界框回归，提升学习效率。

Result: 在自建数据集上，相比YOLOv6，mAP提升了18.5%，在恶劣光照和天气条件下对小弱无人机实现了准确检测。

Conclusion: 提出的分层注意力跨尺度学习框架能有效解决多尺度无人机在复杂场景下的检测难题，显著提升检测精度。

与遥感交叉或改进方案: 交叉/改进可行性：95%。遥感图像中同样存在目标尺度多变（如车辆、舰船、建筑物）和复杂环境（云雾、光照不均）问题。可直接将该框架应用于遥感目标检测任务：1) 分层注意力机制可增强遥感多尺度特征提取；2) 双分支准残差结构有助于恢复遥感图像中因下采样丢失的细节；3) 动态聚焦损失可改进遥感目标边界框回归精度。具体可替换YOLOv6为遥感专用检测器（如RSDet、Oriented R-CNN）进行适配。

Abstract (英文): The accurate detection of unmanned aerial vehicles (UAVs) in various sizes played an important role in the practical applications. Yet the preceding works suffered from the missing inference, the false alarms, and the poor accuracy due to the the adverse scene conditions, as well as the mutable scales. To solve the problems, a hierarchical attention promoted cross-scale learning framework was proposed in this paper. First, the hierarchical attention mechanism was introduced in the backbone to generate the multi-scale features of targets, so they can be discerned and located at different scales. The resulting features were further delivered to the neck, in which two branches of features were built, respectively. The former was obtained by the target-specific feature operator, while the latter was generated by the upsampling operation. The dual branches were further connected in the quasi-residual structure. So the content of targets can be protected well, and the detail information can be reconstructed. Finally, the dynamic focusing loss measurement was presented to regress the bounding box of the target, so the learning effectiveness of presented the architecture can be promoted. To verify the proposed method, multiple rounds of experiments were performed. The results demonstrated that small and weak drones can be detected accurately, especially in adverse lighting and weather conditions. The evaluation metric of mean average precision rate (mAP) can be improved by 18.5% (YOLO6) on the collected dataset.

</details>


### [163] [Remote Sensing-Based Identification of Spatial Spillovers and Transmission Pathways in the Heat–Energy–Carbon Nexus: Evidence from the Yangtze River Delta](https://doi.org/10.3390/rs18132222)
*Gaoneng Lai,Lei Jiang,Yingbiao Chen,Shitai Bao,Jinxin Duan,Zuojie Zhu*

Main category: Remote Sensing

TL;DR: 本文基于2014-2023年长三角城市群多源地理空间数据，构建多尺度分析框架，利用空间杜宾模型和空间中介方法，研究“热-能-碳”关联中的空间溢出效应及行为中介路径，发现碳排放存在显著空间溢出，人为能源活动强度（AEAI）在UHI与碳排放关联中承担44.63%的统计中介作用，表明该关联部分嵌入高能耗人类活动空间格局，而非纯直接热效应。


<details>
  <summary>Details</summary>
Motivation: 城市热岛效应与碳排放的关联已受关注，但行为中介路径和跨区域溢出模式尚未充分理解，现有研究缺乏多尺度空间分析视角。

Method: 构建多尺度分析框架（1km城市群探索性分析与5km空间计量建模），以AEAI表征人为能源活动，采用空间杜宾模型结合空间中介效应方法分析“热-能-碳”关联。

Result: (1)碳排放存在显著正向空间溢出，与热扩散和社会经济网络互动一致；(2)AEAI在UHI与碳排放关联中承担44.63%的统计中介作用，表明该关联部分嵌入高能耗人类活动空间格局。

Conclusion: 区域气候治理需超越单一城市干预和纯物理降温策略，转向跨区域协调与行为调控相结合的综合路径，推广被动冷却导向城市规划和需求侧能源转型。

与遥感交叉或改进方案: 交叉/改进可行性：85%。该研究已使用多源地理空间数据，可进一步结合遥感地表温度反演产品（如MODIS/Landsat LST）替代或校准UHI指标，提升热场表征精度；利用夜光遥感数据（如NPP-VIIRS）优化AEAI空间化；引入遥感反演的植被覆盖度、不透水面比例等参数，增强空间杜宾模型对热-碳关联的环境协变量解释力。

Abstract (英文): The urban heat island (UHI) effect represents a critical urban climate phenomenon arising from the combined pressures of rapid urbanization and climate warming. Although its association with carbon emissions has received increasing scholarly attention, the underlying behavior-mediated pathways and cross-regional spillover patterns remain insufficiently understood. Using multi-source geospatial data for the Yangtze River Delta urban agglomeration from 2014 to 2023, this study develops a multi-scale analytical framework integrating 1 km urban agglomeration exploratory analysis and 5 km spatial econometric modeling. Anthropogenic Energy Activity Intensity (AEAI) is constructed as a proxy for energy-related human activities, and a spatial Durbin model, combined with a spatial mediation approach, is employed to examine the spatial associations and statistically mediated pathways within the “heat-energy-carbon” nexus. The results indicate that: (1) carbon emissions exhibit significant positive spatial spillover effects, consistent with thermal diffusion processes and socioeconomic network interactions; (2) AEAI represents a substantial partial statistical mediation pathway in the association between UHI and carbon emissions, accounting for 44.63% of the total association. This suggests that the UHI–carbon emission linkage is partly embedded in spatial patterns of energy-intensive human activities rather than reflecting a purely direct thermal effect. These findings suggest that regional climate governance may need to move beyond single-city interventions and purely physical cooling strategies toward integrated approaches that combine cross-regional coordination with behavioral regulation. Promoting passive cooling-oriented urban planning and demand-side energy transitions may help reduce carbon lock-in risks and support the development of climate-resilient urban agglomerations.

</details>


### [164] [Integrating Commercial and Public Imagery to Accelerate Deforestation Alerts](https://doi.org/10.3390/rs18132221)
*Zhiqiang Yang,Eric L. Bullock,Erik Lindquist,Carole Andrianirina,Sean P. Healey*

Main category: Remote Sensing

TL;DR: 本文提出了一个名为DIAS（Disturbance Index Alert System）的开源框架，通过将不同卫星传感器的影像转换为统一的干扰指数（DI）Z-score流，实现快速、灵活的森林砍伐预警。在马达加斯加的实验中，结合商业（PlanetScope）和公共（Landsat、Sentinel-1/2）卫星数据，DIAS可在一个月内检测到三分之二的森林砍伐事件（误报率约20%），比传统单传感器系统更及时。


<details>
  <summary>Details</summary>
Motivation: 现有卫星森林砍伐预警系统依赖单一传感器或历史存档数据，难以实现低延迟预警；多传感器联合使用的需求日益迫切，但缺乏统一、轻量级的融合框架。

Method: DIAS框架将不同传感器影像转换为可互操作的干扰指数（DI）Z-score流：通过计算每个像素在森林敏感轴上的Z-score（相对于该图像中所有森林像素的分布），实现跨传感器数据的统一。该方法减少对历史存档和传感器稳定校准的依赖，支持动态增减传感器。

Result: 在马达加斯加实验中，DIAS结合PlanetScope（商业）与Landsat、Sentinel-1/2（公共）数据，一个月内检测到三分之二的森林砍伐（10米参考像素），误报率约20%；仅使用公共数据时，检测率为40%。

Conclusion: 随着商业对地观测选项的增多，DIAS这类灵活、计算轻量化的方法对于适应多样化、校准松散的传感器至关重要，能够支持更及时的森林监测。

与遥感交叉或改进方案: 交叉/改进可行性：95%。作为一篇遥感森林监测论文，其改进方案包括：(1) 将DIAS框架扩展到其他森林扰动类型（如火灾、病虫害、风倒）的预警；(2) 引入主动学习或半监督方法降低对人工标注样本的依赖；(3) 结合高光谱传感器（如EnMAP、PRISMA）提升森林退化检测的敏感度；(4) 加入时间序列自回归模型优化Z-score的动态阈值设定；(5) 在光学影像缺失时融合更多SAR传感器（如ALOS-2 PALSAR-2、RADARSAT-2）提高热带云区的一致性。

Abstract (英文): Generating satellite-based deforestation alerts with actionable latency requires frequent imaging, creating an imperative to use different sensors together. We introduce a simple and open-source framework called the Disturbance Index Alert System (DIAS), which is based upon transformation of imagery from different sources into an interoperable stream of Disturbance Index (DI) values. Whereas most alert systems target divergence of forested pixels from historical states, DIAS targets movement of a pixel’s Z-score position relative to the image-wide population of forest pixels along a forest-sensitive axis. This strategy provides the following practical benefits: (1) it reduces the need to process the historical archive; (2) it reduces dependence upon stable sensor calibration; (3) it allows Z-score-based DI values to be combined across sensors; and (4) it accommodates changes to the group of sensors providing measurements. We demonstrated in Madagascar that sensor integration through DIAS can provide more timely alerts than both conventional individual-sensor systems and additive combination of such systems. Across our study sites, using a commercial source of daily imaging (PlanetScope) in conjunction with imagery from public sources (Landsat, Sentinels-1 and -2) allowed high-confidence detection (false alert rate of approximately 20%) of two-thirds of deforestation occurring at 10 m reference pixels within one month; 40% were detected in that timeframe with public data alone. As commercial options for Earth observation proliferate, flexible and computationally lightweight approaches such as DIAS are needed to accommodate diverse and sometimes only loosely calibrated instruments in support of timely forest monitoring.

</details>


### [165] [Detection of Bark Beetle Attacks Using Time-Aggregated Satellite Data with Machine Learning](https://doi.org/10.3390/rs18132234)
*S. Zeinali,Per‐Ola Olsson,Ted Kronvall,Magnus Wiktorsson,Johan Lindström*

Main category: Remote Sensing

TL;DR: 本研究使用XGBoost分类器，结合每周聚合的Sentinel-2卫星数据与静态地理/林业数据，检测早期树皮甲虫攻击。研究发现红边3和蓝色波段是最重要的卫星特征，土地覆盖和地貌是最重要的静态特征，关键检测周为第19、25、31和33周。结合静态数据与累积卫星数据能显著提升检测性能，对早期预警系统有重要意义。


<details>
  <summary>Details</summary>
Motivation: 树皮甲虫攻击对森林生态系统和经济造成严重威胁，早期检测对于有效防治至关重要。现有方法多仅使用单一数据源，未能充分利用多源信息的时间动态特征，本研究旨在探索融合静态地理/林业数据与时间序列卫星数据提升早期检测能力。

Method: 采用XGBoost分类器，对比三种数据方案：仅静态数据、仅卫星数据、两者结合。使用每周累积的Sentinel-2数据训练模型，通过时间动态分析跟踪模型性能和特征重要性变化，系统识别关键检测时间窗口。

Result: 红边3和蓝色Sentinel-2波段是最重要的卫星特征；静态特征中土地覆盖和地貌最重要；加入静态数据后红波段和红边2波段的重要性下降。关键检测周为第19周（主群飞周）、第25周（群飞后6周）、第31和33周（攻击后3个月以上）。

Conclusion: 结合静态地理/林业特征与累积Sentinel-2时间序列数据能显著提升树皮甲虫攻击的早期检测性能，该方法对构建早期预警系统具有重要价值。

与遥感交叉或改进方案: 交叉/改进可行性：85%。改进方案：（1）可引入SAR数据（如Sentinel-1）补充光学数据在云覆盖区域的不足；（2）使用更细时间分辨率（如每日）或自注意力机制（如Transformer）捕捉更精细的物候变化；（3）可引入更多森林健康指标（如NDVI、归一化燃烧比等植被指数）作为特征；（4）可尝试多任务学习同时检测不同严重程度的虫害等级；（5）结合无人机高光谱数据与卫星数据实现空间分辨率互补。

Abstract (英文): In this study, we explored how early bark beetle attacks can be detected using weekly aggregated Sentinel-2 data in combination with static data, such as geo- and forestry data. We used an XGBoost classifier, known for its strength and reliability in classification, and compared three sets of data: static data only, satellite data only, and using both together. Having trained the models on cumulative weekly data, we were able to track changes in model performance and feature importance over time, identifying key weeks for the detection of bark beetle attacks. A systematic overview of feature importance identified the Red-edge 3 and blue Sentinel-2 bands as the most important when combined with static data; it also showed changes in feature importance compared to using satellite-only data, e.g., adding static features reduced the importance of red and red-edge 2 bands. Among the static features, land cover and landforms were the most important. Evaluating the temporal features for the combined model highlighted certain weeks as containing key information for detection: week 19, which was the main swarming week; week 25, which is 6 weeks after swarming and just before the second generation is completed; and weeks 31 and 33, more than 3 months after the tree was attacked and well after the new generation has swarmed. The study shows that combining static features with cumulative Sentinel-2, accumulated across weeks, are all important for improving the detection of bark beetle attacks, and that such ideas form an important part of early warning systems.

</details>


### [166] [Semantic Consistency and Uncertainty-Driven Small-Object Detection for Class Imbalance](https://doi.org/10.3390/rs18132208)
*Nuo Chen,Peng Zhao,Shouquan Hou*

Main category: Remote Sensing

TL;DR: 本文提出SCUD训练框架，通过标签噪声抑制策略（LNSS）、尺度感知重采样策略（SARS）和自适应实例选择机制（AISM）解决航拍图像小目标检测中的样本不平衡问题，在DOTA-v1.0和DIOR数据集上分别达到70.7%和88.1%的mAP50。


<details>
  <summary>Details</summary>
Motivation: 航拍图像小目标检测中，复杂成像视角、任意目标朝向和长尾类别分布共同导致严重的样本不平衡，使得检测稳定性下降、少数类别频繁误分类。

Method: 提出SCUD训练框架，包含三个核心模块：（1）LNSS：基于语义一致性的对比学习机制，约束相似样本在特征空间中的聚合以抑制标签噪声；（2）SARS：尺度感知重采样策略，缓解小目标过度重复导致的噪声放大和过拟合；（3）AISM：联合建模预测不确定性和全局统计先验的自适应实例选择机制，动态强调信息量丰富的样本。

Result: 在DOTA-v1.0数据集上mAP50达70.7%，在DIOR数据集上达88.1%，多个稀有类别的检测精度显著提升。

Conclusion: 所提方法有效应对了航拍图像小目标检测中的样本不平衡问题，尤其显著提升了稀有类别的检测性能。

与遥感交叉或改进方案: 交叉/改进可行性：95%。该论文本身就是遥感航拍图像小目标检测研究。（改进方案：1）可引入旋转目标检测框架（如RoI Transformer）以更好处理任意朝向目标；2）SARS可与类别平衡采样策略融合，进一步缓解极端长尾分布；3）AISM可结合主动学习思想，在标注预算有限时优先选择高价值样本进行人工标注。）

Abstract (英文): In aerial image small-object detection, complex imaging perspectives, arbitrary object orientations, and long-tailed category distributions jointly exacerbate sample imbalance, which significantly degrades detection stability and leads to frequent misclassification of minority categories. To address these challenges, this paper proposes a novel training framework termed SCUD. Specifically, in the label noise suppression strategy (LNSS), a contrastive learning mechanism based on semantic consistency is introduced to constrain the aggregation of similar samples in the feature space, thereby reducing the adverse impact of noisy samples on model optimization. In addition, a scale-aware resampling strategy (SARS) is designed to alleviate noise amplification and overfitting caused by excessive repetition of small objects during training. Furthermore, an adaptive instance selection mechanism (AISM) is developed by jointly modeling prediction uncertainty and global statistical priors, enabling the model to dynamically emphasize learning from informative samples. Extensive experiments are conducted on two publicly available unmanned aerial vehicle (UAV) aerial image datasets to validate the effectiveness of the proposed approach. The proposed method achieves an mAP50 of 70.7% on the DOTA-v1.0 dataset and 88.1% on the DIOR dataset. Notably, the detection accuracy of several rare categories is significantly improved, further demonstrating the effectiveness of the proposed method in addressing sample imbalance in aerial image small-object detection.

</details>


### [167] [Sequence-Aware Deep Learning for Field-Scale Surface Soil Moisture Estimation from Sentinel-1, HLS, and Ancillary Data](https://doi.org/10.3390/rs18132213)
*Elahe Jahan Nejadi,Ramata Magagi,Kalifa Goı̈ta*

Main category: Remote Sensing

TL;DR: 本文提出了基于序列感知深度学习模型（FTA-TCN），通过融合Sentinel-1 SAR、HLS光学数据及本地辅助数据，实现了田块尺度地表土壤湿度的高精度估算（R²=0.851，RMSE=0.024 m³/m³），验证了双注意力时序卷积网络在干湿年份的泛化能力。


<details>
  <summary>Details</summary>
Motivation: 传统卫星土壤湿度产品空间分辨率过粗，无法满足田块尺度农业水分管理的需求，亟需一种能融合多源遥感与本地辅助数据的高分辨率SSM估算方法。

Method: 构建多源数据集（2016-2024年5-9月），包含SAR、光学、气象、土壤、地形、作物标签等65个特征；对比LSTM与TCN架构及注意力增强变体（FA、TA、FTA）；采用湿年（2016）与干年（2023）组合数据进行时间泛化验证；使用SHAP分析特征重要性。

Result: FTA-TCN模型在验证集上R²=0.851，RMSE=0.024 m³/m³，MAE=0.020 m³/m³，显著优于基线LSTM（R²=0.422）和TCN（R²=0.746）；卫星特征贡献约28.5%的SHAP重要性；前期降水和短期降雨累积是主要驱动因子。

Conclusion: 双注意力时序卷积网络（FTA-TCN）能够有效耦合卫星信号与本地土壤-气象-管理背景，实现干湿季节田块尺度SSM动态的可靠估算。

与遥感交叉或改进方案: 交叉/改进可行性：95%。改进方向：1）引入Sentinel-2红边波段或无人机高光谱数据增强植被指数多样性；2）增加长时间序列的土壤水分地面观测网络进行迁移学习验证；3）结合InSAR形变数据或多极化SAR特征改善植被覆盖区反演精度；4）将模型扩展至冻融期或非生长季；5）采用图神经网络融合空间邻域上下文信息以进一步提升田块边界处精度。

Abstract (英文): Accurate field-scale surface soil moisture (SSM) measures are important for agricultural water management. Conventional satellite SSM products remain too coarse for within-field applications. Here, we developed sequence-aware deep learning models for growing-season SSM estimation by fusing data from Sentinel-1 C-band SAR, harmonized Landsat-8/Sentinel-2 (HLS), and local ancillary datasets. We assembled a multi-source dataset on Sentinel-1 overpass time for 2016–2024 (May–September), yielding 1469 samples and 65 features per sample, including SAR and optical features, meteorological data, soil texture and bulk density, topography, crop labels, irrigation-likelihood flag, and irregular-time-step encoding. We compared long short-term memory (LSTM) and temporal convolutional neural network (TCN) architectures together with attention-augmented variants, including feature attention (FA), temporal attention (TA), and the combined feature–temporal attention (FTA). Models were trained and tested on seven years of data and were validated based on a temporal generalization using combined data of a wet year (2016) and a dry year (2023). The best model, FTA-TCN, achieved R2 = 0.851, RMSE = 0.024 m3.m−3, and MAE = 0.020 m3.m−3 on the withheld validation years, outperforming the base LSTM (R2 = 0.422; RMSE = 0.053 m3.m−3; MAE = 0.043 m3.m−3) and the base TCN (R2 = 0.746; RMSE = 0.034 m3.m−3; MAE = 0.022 m3.m−3). Shapley additive explanations (SHAP) analysis indicated that antecedent precipitation and short-term rainfall accumulations were dominant forcings, while soil texture, elevation, incidence angle, and vegetation indices modulated SSM variability. Satellite-derived features accounted for ~28.5% of aggregated SHAP importance. Overall, the results show that dual-attention temporal convolution can capture field-scale SSM dynamics across wet and dry seasons when satellite signals are coupled with local soil-meteorological-management context.

</details>


### [168] [Characterizing Short-Duration Summer Rainstorms in Nanjing, China, Using Multi-Source Remote Sensing and Explainable AI](https://doi.org/10.3390/rs18132212)
*Yue Wang,Ningxin Yong,Siyu Zhu,Yang Hong*

Main category: Remote Sensing

TL;DR: 本研究结合多源遥感数据集和中国新一代星载双频降水雷达观测，分析了2017–2024年南京夏季暴雨的时空特征、垂直结构及环境驱动因素。结果表明暴雨强度与持续时间分别增加7.44%和38.63%，影响面积减少8.18%，呈局地化增强趋势；三维雷达揭示倒锥状垂直结构，EBOX模型识别近地表气温和比湿为关键环境因子。


<details>
  <summary>Details</summary>
Motivation: 全球变暖和快速城镇化背景下，短时夏季暴雨日益增强且局地化，但其时空特征、垂直结构和环境驱动因素尚不清楚，缺乏系统性认知。

Method: 融合多源遥感数据集与新一代星载双频降水雷达（GPM DPR）观测数据，结合可解释贝叶斯优化XGBoost（EBOX）模型进行环境因子分析，并对典型暴雨事件进行三维雷达探测。

Result: 暴雨强度和持续时间分别增加7.44%和38.63%，影响面积减少8.18%；暴雨呈倒锥状垂直结构，属混合对流-层状降水体系；近地表气温和比湿是暴雨发生发展的首要环境因子。

Conclusion: 卫星遥感与可解释人工智能的结合能有效提升对城市极端降水的理解，揭示了气候变化、地形和城镇化共同塑造季风区降水极值的机制。

与遥感交叉或改进方案: 交叉/改进可行性：95%。改进方案：（1）可将EBOX模型扩展到其他城市群进行对比分析，验证模型迁移性；（2）引入更高空间分辨率的热红外或SAR数据，提升城市热岛效应与暴雨耦合机制的识别精度；（3）结合地基雷达与星载雷达协同观测，弥补卫星回访周期的时空覆盖不足；（4）将三维垂直结构分析推广至更多暴雨个例，建立不同类型暴雨的垂直结构分类体系。

Abstract (英文): With global warming and rapid urbanization, short-duration summer rainstorms are becoming more intense and localized, posing growing challenges to urban flood resilience. However, their spatiotemporal characteristics, vertical structures, and environmental drivers remain poorly understood. Here, we combine multi-source remote sensing datasets and China’s new-generation satellite-borne dual-frequency precipitation radar observations to investigate summer rainstorms in Nanjing, China, during 2017–2024. Results reveal pronounced spatiotemporal heterogeneity, with higher rainfall intensities concentrated over urban and adjacent areas. During the study period, rainstorm intensity and duration increased by 7.44% and 38.63%, respectively, while the affected area decreased by 8.18%, indicating a transition toward more localized yet more intense rainfall events. Environmental analyses suggest that large-scale thermodynamic conditions and regional topographic forcing provide a favorable background for convection development, while local urban thermal effects may further modulate rainfall enhancement. Three-dimensional radar detection of an illustrative rainstorm event indicates an inverted-cone vertical structure, suggesting a mixed convective-stratiform precipitation structure involving both warm-rain and ice-phase processes. An Explainable Bayesian-Optimized XGBoost (EBOX) model further identifies near-surface air temperature and specific humidity as the primary environmental factors associated with rainstorm occurrence and development. Overall, this study highlights the value of integrating satellite remote sensing with explainable artificial intelligence to improve understanding of urban extreme rainfall and provide new insights into how climate change, topography, and urbanization jointly shape precipitation extremes in rapidly urbanizing monsoon regions.

</details>


### [169] [Multi-Modal Data Fusion for Dynamic Target Depth Retrieval in Aquatic Environments](https://doi.org/10.3390/rs18132230)
*Xiangyong Liu,Zhiqiang Xu,Tianhong Ding*

Main category: Remote Sensing

TL;DR: 本文提出一种基于多源数据融合的动态目标深度估计方法，联合光学RGB图像和神经形态视觉(NeuroIV)数据，通过超图结构建立时空节点关联权重，采用逐元素乘法替代点积注意力降低计算复杂度，并引入轻量化可变形注意力金字塔(DAP)和扩散模型优化深度图像边缘，在复杂水下环境中MAE指标提升超过20%。


<details>
  <summary>Details</summary>
Motivation: 解决复杂水下遥感环境中严重的光学衰减以及移动目标深度提取时动态特征难获取的挑战。

Method: 1) 以RGB图像和神经形态视觉(NeuroIV)数据为联合输入，构建三通道特征提取与融合网络；2) 利用超图结构建立动态（时序）与静态（空间）节点间的关联权重，捕捉时空相关性；3) 用逐元素乘法替代传统点积注意力，降低计算复杂度；4) 引入轻量化可变形注意力金字塔(DAP)和扩散模型优化深度图像边缘，抑制估计结果的突变和不连续性。

Result: 融合多模态数据相比单模态光学图像获得更高的信噪比和前景对比度，MAE指标提升超过20%。

Conclusion: 验证了所提出的多模态融合策略在水域环境动态目标观测与深度检索中的有效性和优越性。

与遥感交叉或改进方案: 交叉/改进可行性：85%。该文本身就是水下遥感深度估计研究。改进方向：1) 可将NeuroIV数据替换或融合为合成孔径雷达(SAR)或激光雷达(LiDAR)点云数据，适应不同遥感场景；2) 超图结构可扩展至多光谱/高光谱遥感数据的时空建模；3) DAP和扩散模型可迁移至遥感图像超分辨率重建或去云处理；4) 该方法可拓展至无人机遥感动态目标追踪与深度估计。

Abstract (英文): To address the challenges of severe optical attenuation and dynamic feature extraction for moving target depth retrieval in complex underwater remote sensing environments, this paper proposes a dynamic target depth estimation method based on multi-source data fusion. Taking optical RGB imagery and neuromorphic vision (NeuroIV) data as joint inputs, the proposed method constructs a three-channel feature extraction and fusion network. By leveraging a hypergraph structure, it establishes association weights between dynamic (temporal) and static (spatial) nodes to capture spatiotemporal correlations. To efficiently process the high-dimensional multi-modal data, the traditional dot-product attention is replaced with element-wise multiplication, significantly reducing computational complexity. Furthermore, a lightweight deformable attention pyramid (DAP) and diffusion model is introduced to refine depth image edges, effectively suppressing discontinuities and abruptness in the estimation results. Compared to single-modality optical imagery, the fused multi-modal data yields a superior signal-to-noise ratio and foreground contrast, achieving an improvement of over 20% in the MAE index. These results validate the effectiveness and superiority of the proposed multi-modal fusion strategy for dynamic target observation and depth retrieval in aquatic environments.

</details>


### [170] [Village-Scale Winter Wheat Yield Prediction in Coastal Saline–Alkali Farmland Using a Three-Stage Fusion XGBoost Framework and SHAP](https://doi.org/10.3390/rs18132233)
*Wenxi Jia,Jingzhao Lu,杨其展,Yuhang Xie,Xing Cao,Yuqing Pan,Qi Xu,Yapeng Zhou,Jun Zhao,Li Wang,Xiaofei Liu,Fujun Zhao,Y ZHANG*

Main category: Remote Sensing

TL;DR: 本研究以河北黄骅市为典型滨海盐碱区，利用Sentinel-2影像、气候因子和地形变量构建物候引导的特征集，通过XGBoost及其改进模型（物候引导特征集成XGBoost和三阶段融合XGBoost）实现了村庄尺度的冬小麦产量预测，R²最高达0.6439，并利用SHAP分析揭示了盐分指数、距离因子、高程等关键驱动因素。


<details>
  <summary>Details</summary>
Motivation: 滨海盐碱农田存在强空间差异性和多重环境胁迫，导致村庄尺度冬小麦产量准确估算极具挑战性，现有方法在该区域的预测精度和可解释性不足。

Method: 1) 构建物候引导特征集，融合Sentinel-2影像、气候因子、地形变量（高程、地形湿度指数、距海岸线距离、距水系距离）；2) 采用XGBoost进行产量预测，并与Gradient Boosting、Random Forest、SVR、Ridge回归、决策树对比；3) 设计三阶段分支模型识别不同阶段产量主要驱动因素；4) 使用SHAP进行模型可解释性分析。

Result: 物候引导特征集成XGBoost的R²=0.6382，RMSE=450.15 kg/ha，优于对比模型；三阶段融合XGBoost模型R²=0.6439，RMSE=446.24 kg/ha，MAE=363.38 kg/ha；内陆地区冬小麦产量(5145 kg/ha)显著高于沿海地区(4198 kg/ha)。

Conclusion: 该框架能够有效支持滨海盐碱农田村庄尺度的冬小麦产量预测，并通过SHAP分析提升了模型可解释性，识别出盐分指数、距离相关因子、高程和NDVI为关键驱动变量。

与遥感交叉或改进方案: 交叉/改进可行性：85%。改进方案：1) 可引入更高时空分辨率遥感数据（如Sentinel-1 SAR）以增强对盐碱胁迫的监测能力；2) 可结合深度学习模型（如CNN-LSTM）捕捉时空特征，进一步提升预测精度；3) 可扩展至其他盐碱区域验证模型泛化性；4) 可引入土壤理化参数（如含盐量、pH）作为补充特征。

Abstract (英文): Accurately estimating village-level winter wheat yield in coastal saline–alkali farmland is challenging because this region has strong spatial differences and multiple environmental stresses. In this study, Huanghua City, Hebei Province, was selected as a typical coastal saline–alkali area. Sentinel-2 images, climate factors, and topographic variables, including elevation, topographic wetness index, distance to the coastline, and distance to water systems, were combined to build a phenology-guided feature set for winter wheat yield prediction in coastal areas. The results showed that Phenology-Guided Feature Integration XGBoost achieved an R2 of 0.6382 and an RMSE of 450.15 kg/ha, which was slightly better than Gradient Boosting (R2 = 0.6256) and Random Forest (R2 = 0.6098), and clearly better than SVR (R2 = 0.4792), Ridge regression (R2 = 0.4582), and a single Decision Tree (R2 = 0.3088). Then, a three-stage branch was designed to identify the main drivers of SI, NDVI, and winter wheat yield at different stages, helping explain how environmental constraints and vegetation responses jointly affect final yield. The Three-Stage Fusion XGBoost Model achieved an R2 of 0.6439, an RMSE of 446.24 kg/ha, and an MAE of 363.38 kg/ha, showing a slight improvement in prediction accuracy. SHAP analysis showed that SI, distance-related factors, elevation, TWI, and NDVI were important drivers of winter wheat yield variation. Spatial prediction results showed higher winter wheat yield in inland areas (5145 kg/ha) and lower yield in coastal areas (4198 kg/ha). This framework supports village-scale winter wheat yield prediction in coastal saline–alkali farmland and improves model interpretability.

</details>


### [171] [Scale Effects on Dominant Drivers of Commercial Plantation Productivity: Novel Insights from High-Resolution Multi-Sensor UAV Remote Sensing and Interpretable AI](https://doi.org/10.3390/rs18132235)
*Zhansheng Mao,Bo Zheng,Y F LIU,D G Liu*

Main category: Remote Sensing

TL;DR: 本研究利用10米高密度土壤采样、无人机LiDAR和多光谱遥感数据，通过可解释机器学习框架（XGBoost–SHAP）分解了亚热带山地经济林（山核桃）叶片叶绿素指数（LCI）在3–50米尺度上的驱动因素。发现：细尺度（3–10米）由树冠结构主导；中尺度（10–20米）出现冠层-土壤耦合窗口；20米后耦合减弱；>30米转为地形和土壤质地主导。据此提出了尺度分层的精准林业管理框架。


<details>
  <summary>Details</summary>
Motivation: 亚热带山地经济林存在显著资源空间异质性，但土壤属性、地形和冠层结构调控植被活力的空间尺度分辨率尚不明确，亟需量化多尺度驱动机制。

Method: 构建了10米高密度土壤采样、无人机LiDAR和多光谱遥感的空间一致多尺度数据集；采用XGBoost–SHAP可解释机器学习框架，在3–50米范围内分解冠层、土壤和地形对LCI的相对贡献及尺度依赖性。

Result: 细尺度（3–10 m）由树高主导；中尺度（10–20 m）出现冠层-土壤耦合窗口；20 m处耦合减弱恢复单因子主导；>30 m复杂交互重现，SHAP贡献从养分和冠层转向地形和土壤质地。

Conclusion: 土壤-冠层关系随空间尺度重组而非静态不变；据此提出尺度分层精准林业框架，将管理干预与主导相关因子所在生态尺度对齐。

与遥感交叉或改进方案: 交叉/改进可行性：85%。本文方法可直接迁移至遥感协同分析：（1）将该可解释多尺度框架应用于其他遥感植被指数（如NDVI、EVI、SIF），验证尺度依赖性规律的普适性；（2）结合星载LiDAR（如GEDI）与Sentinel-2多光谱，在大区域尺度验证冠层-土壤耦合窗口的存在性；（3）在遥感作物模型或生态过程模型中引入尺度分层约束，提升异质性地表的参数化精度；（4）利用SHAP可解释性辅助遥感特征选择，识别不同空间分辨率下的最优预测变量组合。

Abstract (英文): Subtropical mountain economic tree plantations are constrained by pronounced spatial heterogeneity in resource availability, yet the spatial scales at which soil properties, topography, and canopy structure regulate vegetation vigor remain poorly resolved. To address this gap, a spatially consistent multi-scale dataset combining 10 m high-density soil sampling, UAV-LiDAR, and multispectral remote sensing was used to quantify the scale-dependent drivers of the Leaf Chlorophyll Index (LCI) across 3–50 m within a Chinese hickory (Carya cathayensis Sarg.) plantation. The relative contributions of canopy, soil, and topography to LCI were decomposed across scales using an interpretable machine-learning framework (XGBoost–SHAP). At fine scales (3–10 m), vegetation vigor was primarily controlled by tree-level canopy structure, particularly tree height, reflecting localized resource acquisition. At intermediate scales (10–20 m), a distinct coupling window emerged, characterized by increased interaction complexity: LCI was predominantly driven by interactions between canopy structure and soil nutrient availability, whereas single-factor effects weakened. Notably, at 20 m this interaction pattern largely weakened and reverted to single-metric dominance. At broader scales (&gt;30 m), complex interactions re-emerged, and dominant SHAP contributions shifted from nutrients and canopy structure toward topography and soil texture. These findings reconcile strong fine-scale drivers with weaker predictability at intermediate extents and demonstrate that soil–canopy relationships reorganize across spatial scales rather than remaining static. On the basis of these findings, a scale-hierarchical framework for precision forestry is proposed that aligns management interventions with the ecological scales at which dominant correlates operate across spatial supports.

</details>


### [172] [Land-Cover-Stratified Validation and Uncertainty Prioritization for SSP-Based NDVI Projection at 1 km Resolution in Northeast China](https://doi.org/10.3390/rs18132203)
*Eslam Rashad,Yujie Liu,J S Liu,Tao Pan,Ahmed Refaee*

Main category: Remote Sensing

TL;DR: 本研究针对1km分辨率NDVI投影中聚合精度掩盖类别级预测缺陷的问题，开发了四组件评估框架，在东北地区（NEC）SSP1-2.6、SSP2-4.5和SSP5-8.5情景下对2040年NDVI进行投影。LightGBM在三类树模型中表现最优（空间CV R²=0.654、时间留出R²=0.710、长跳R²=0.671）。区域平均NDVI从2020年的0.393微增至0.414–0.417。类别分层验证发现全局R²掩盖了强烈异质性（建设用地0.576到未利用地-0.886），水体与未利用地呈负R²。不确定性风险指数（URI）在建设用地最高、耕地最低。综合历史残差与未来URI，将水体、未利用地和建设用地确定为最高优先验证类别。


<details>
  <summary>Details</summary>
Motivation: 现有NDVI投影文献缺乏对聚合精度掩盖类别级预测缺陷的系统性关注。1km分辨率下异质景观的空间一致性可能隐藏着特定土地覆盖类型上的显著预测薄弱环节，亟需开发能够揭示类别级误差的评估框架。

Method: 四组件评估流程：(1)多阶段模型选择（空间块交叉验证、时间留出验证、长跳外推、气候扰动测试）；(2)土地覆盖分层验证；(3)基于分位数回归的不确定性表征；(4)验证优先级排序。采用三种树模型（LightGBM等）对比，通过SBCV、THV、LJE和CPT测试择优。

Result: LightGBM表现最优（SBCV R²=0.654±0.123, THV R²=0.710, LJE R²=0.671）。2040年区域NDVI从0.393微增至0.414–0.417，SSP间差异小。类别R²范围为0.576（建设用地）至-0.886（未利用地），水体与未利用地R²为负。残差分解揭示水体高随机误差+高系统偏差，未利用地主系统偏差。URI在建设用地最高、耕地最低。

Conclusion: 土地覆盖分层的误差分解和不确定性信息优先排序能够揭示聚合精度指标所掩盖的类别级投影局限性。水体、未利用地和建设用地由于类别代表性不足和NDVI相关复杂性（低植被信号、混合像元效应、异质地表组成）成为最高优先验证类别。

与遥感交叉或改进方案: 交叉/改进可行性：95%。本文本身就是遥感领域NDVI投影研究，改进方向包括：(1)引入深度学习模型（如ConvLSTM、Attention机制）替代LightGBM以更好捕捉时空依赖；(2)将类别分层验证框架推广到其他植被指数（如EVI、LAI）或地表参数（如反照率、地表温度）；(3)在URI基础上引入成本函数，发展"成本-效益"引导的验证采样策略；(4)将空间分辨率提升至250m或30m，结合Landsat/Sentinel数据验证类别级尺度效应；(5)增加极端气候事件情景下的模型鲁棒性测试。

Abstract (英文): At 1 km resolution, NDVI projections for heterogeneous landscapes can appear spatially coherent in aggregate while concealing substantial class-level prediction weaknesses, a limitation that has received limited systematic attention in the NDVI projection literature. This study applies a four-component assessment workflow to Northeast China (NEC) for 2040 under SSP1-2.6, SSP2-4.5, and SSP5-8.5, integrating multi-stage model selection, land-cover-stratified validation, quantile-regression-based uncertainty characterization, and validation-priority ranking. Among three candidate tree-based models evaluated using spatial block cross-validation, temporal holdout validation, long-jump extrapolation, and climatic perturbation tests, LightGBM showed the most balanced and consistent performance, with spatial CV R2 = 0.654 ± 0.123, temporal holdout R2 = 0.710, and long-jump R2 = 0.671, and was therefore selected for the 2040 projection. Projected regional mean NDVI increased modestly from 0.393 in 2020 to 0.414–0.417 across scenarios, with limited divergence among SSP pathways at this near-term horizon. Class-stratified validation of the 2020 holdout prediction revealed that global model performance masked strong class-level heterogeneity, with R2 values ranging from 0.576 for Construction land to −0.886 for Unused land. Water bodies and Unused land exhibited negative R2 values, indicating weak class-level predictive support relative to a simple class-mean benchmark. Residual decomposition showed that Water bodies combined high random error with elevated systematic deviation, whereas Unused land was mainly characterized by systematic bias, suggesting different needs for class-specific model improvement. The Uncertainty Risk Index (URI), derived from 95% prediction intervals, was highest in Construction land and lowest in Cropland across all scenarios. Integrating historical residuals with future URI-identified Water bodies, Unused land, and Construction land as the highest-priority classes for future targeted validation. These priorities arise from both limited class representation and intrinsic NDVI-related complexity, including low vegetation signal, mixed-pixel effects, and heterogeneous land-surface composition. These results demonstrate that land-cover-stratified error decomposition and uncertainty-informed priority ranking reveal class-specific projection limitations that aggregate accuracy metrics can conceal.

</details>


### [173] [Future Climate-Driven Changes in Carbon Stocks in the Yellow River Basin of China](https://doi.org/10.3390/rs18132205)
*Xia Fang,Liangzhong Cao,Ziwei Pei,Shihua Zhu,Yuhong He*

Main category: Remote Sensing

TL;DR: 本研究利用干旱生态系统模型（AEM）模拟了黄河流域1981-2060年四种主要碳库（总碳、植被碳、土壤有机碳、凋落物碳）的时空演变。结果显示，1981-2020年总碳增加0.09 Pg C (+3.54%)，主要由CO2施肥效应驱动；2021-2060年高排放情景下总碳预计增加0.114 Pg C (+4.81%)。凋落物碳变异性最高（-18%至+22%），是碳汇稳定性的敏感指标。研究强调了非线性气候-碳相互作用和区域异质性的重要性。


<details>
  <summary>Details</summary>
Motivation: 干旱和半干旱生态系统的碳汇动态是全球碳循环评估中的主要不确定性来源，黄河流域作为典型区域缺乏长期、多碳库的系统模拟研究。现有研究对CO2施肥、气温和降水变化的相对贡献及其空间异质性认识不足，制约了区域碳中和目标的实现。

Method: 采用干旱生态系统模型（Arid Ecosystem Model, AEM），在多种气候情景（包括高排放SSP5情景和单一因子CO2、气温、降水驱动情景）下，对黄河流域1981-2020年历史时期和2021-2060年未来时期的四种碳库（TOTC、VEGC、SOC、LTRC）进行时空动态模拟。

Result: 1981-2020年TOTC增加0.09 Pg C (+3.54%)，VEGC增21.43%，SOC增2.78%，LTRC净变化小但年际变异性高。2021-2060年SSP5情景下TOTC预计增4.81%，VEGC贡献最大（+23.87%）。CO2单独驱动效果与全因子相似，气温和降水单独作用更弱且变异性更大。空间上黄河流域上游保持强碳汇，黄土高原和中西部区域易受增温和水分减少影响。LTRC变异性最高（-18%至+22%）。

Conclusion: CO2施肥效应是黄河流域碳汇增加的主导因子，气候变暖和降水变化的影响较弱且空间异质性显著。凋落物碳是碳汇稳定性的敏感指示器。需要针对区域异质性制定生态恢复与气候适应相结合的适应性策略，以支持中国碳中和目标。

与遥感交叉或改进方案: 交叉/改进可行性：85%。1）遥感数据驱动：可利用MODIS NDVI/LAl、GPP、Soil Moisture等遥感产品替代模型中的经验参数，提高AEM模型在黄河流域的输入精度和时空分辨率；2）空间验证：利用卫星反演的植被碳（如基于LiDAR或光学遥感的AGB估算）和土壤碳产品（如SoilGrids）对模型输出的VEGC和SOC空间分布进行独立验证；3）碳库变化监测：结合InSAR地表形变监测与土壤碳变化的关系探索，以及利用遥感反演的干旱/半干旱区植被水分利用效率（WUE）校正模型中的水分胁迫参数；4）改进方案：对于该论文本身就是遥感交叉研究，建议引入遥感叶面积指数（LAI）和日光诱导叶绿素荧光（SIF）产品来约束模型中的光合-碳分配过程，减少模型不确定性。

Abstract (英文): Carbon storage dynamics in dryland and semi-arid ecosystems remain a major uncertainty in global carbon cycle assessments, particularly in regions like the Yellow River Basin (YRB). Using the Arid Ecosystem Model (AEM), we simulated the spatiotemporal evolution of four major carbon pools—total carbon (TOTC), vegetation carbon (VEGC), soil organic carbon (SOC), and litter carbon (LTRC)—from 1981 to 2060 under factorial climate scenarios. During 1981–2020, TOTC increased by 0.09 Pg C (+3.54%), driven by gains in VEGC (+0.03 Pg C, +21.43%) and SOC (+0.06 Pg C, +2.78%). LTRC showed minimal net change but was highly sensitive to interannual variability. From 2021 to 2060, under the high-emission SSP5 scenario, TOTC is projected to increase by 0.114 Pg C (+4.81%), with VEGC contributing most of the gain (+23.87%). CO2_only simulations showed similar increases, underscoring the dominant role of CO2 fertilization. In contrast, warming and precipitation alone produced weaker and more variable effects. Spatially, upper YRB regions are expected to maintain strong sink capacity, while the Loess Plateau and central-western subregions remain vulnerable to warming and moisture decline. LTRC exhibited the highest variability across scenarios (−18% to +22%), highlighting its role as a sensitive indicator of sink stability. These findings emphasize the need to account for nonlinear climate–carbon interactions and regional heterogeneity. Region-specific, adaptive strategies that integrate ecological restoration and climate adaptation will be critical to enhancing carbon sinks and supporting China’s carbon neutrality targets in the Yellow River Basin.

</details>


### [174] [Less Is More: Online Spatio-Temporal Selective Learning for Multi-Variable Meteorological Forecasting](https://doi.org/10.3390/rs18132202)
*Pu Zhang,Deping Xiang,Chunlei Huo,Kun Ding,Shiming Xiang*

Main category: Remote Sensing

TL;DR: 提出时空选择性学习（ST-SL）框架和VASTFormer模型，通过选择性监督高收益预测单元提升高维气象场预测精度，在ERA5数据集上MSE降低8.84%。


<details>
  <summary>Details</summary>
Motivation: 高维气象场预测中，现有深度模型对所有预测单元统一优化，忽视了不同时空单元在训练难度和泛化能力上的异质性，导致低收益或弱泛化信号被过度强调，影响整体预测性能。

Method: 提出ST-SL在线训练框架，通过主模型与冻结参考模型对比估计每个预测单元的可学习性，仅对高收益时空单元计算损失；同时引入VASTFormer作为预测主干，包含跨变量依赖建模、物理增强的太阳位置编码和高效时序翻译器。

Result: 在ERA5再分析数据集上，VASTFormer优于主流时空基线；ST-SL在不增加推理参数和计算成本的前提下进一步提升精度。相比最强基线，VASTFormer+ST-SL的MSE、MAE、RMSE分别降低8.84%、6.70%、4.54%，平均ACC达0.9801，RMSESS达0.8104。

Conclusion: 选择性监督策略能够提升稠密气象场预测的泛化能力，在高维、异质性强的时空预测任务中具有显著优势。

与遥感交叉或改进方案: 交叉/改进可行性：95%。该工作与遥感领域高度契合，具体交叉方案：（1）将ST-SL选择性学习框架直接应用于遥感时空预测任务（如海面温度、植被指数、气溶胶光学厚度等产品的时空预测），解决不同遥感变量/区域/时相的训练难度差异问题；（2）VASTFormer的跨变量依赖建模机制可用于多源遥感数据融合预测（如结合光学与SAR影像）；（3）物理增强编码可推广至遥感中的太阳-传感器几何编码或地形校正编码；（4）时序翻译器可应用于遥感时间序列预测（如NDVI时序、作物生长监测）。

Abstract (英文): Accurate forecasting of high-dimensional meteorological fields remains challenging due to the complex spatio-temporal dynamics of atmospheric systems and the presence of heterogeneous training difficulty across space and lead time. Existing deep forecasting approaches usually optimize all prediction units uniformly, which may overemphasize low-benefit or weakly generalizable supervision signals. To address this issue, we propose Spatio-Temporal Selective Learning (ST-SL), an online training framework that estimates the learnability of each prediction unit by comparing the main model with a frozen reference model and computes the loss only over selected high-benefit spatio-temporal units. To provide an effective forecasting backbone, we further introduce VASTFormer, a variable-aware spatio-temporal Transformer that models cross-variable dependencies, incorporates physics-enhanced Solar Positional Encoding, and captures atmospheric trajectories with an efficient temporal translator. Experiments on the ERA5 reanalysis dataset show that VASTFormer outperforms representative spatio-temporal baselines, while ST-SL further improves accuracy without adding inference-time parameters or computational cost. Compared with the strongest baseline, VASTFormer+ST-SL reduces MSE, MAE, and RMSE by 8.84%, 6.70%, and 4.54%, respectively. Meteorological skill evaluation further shows an average ACC of 0.9801 and RMSESS of 0.8104, and percentile-based extreme-condition evaluations confirm consistent improvements across standard and high-impact forecasting scenarios. These results indicate that selective supervision can improve generalization in dense meteorological forecasting.

</details>


### [175] [A Spatiotemporal Fusion-Based Machine Learning Framework for Long-Term SSC Retrieval in the Three Gorges Reservoir Area](https://doi.org/10.3390/rs18132231)
*Y Zhou,Shu Fu,R X Liu,Liu Yang,Y Zhou,Xiaoxiong Wen,Hua Zhang,Shanghong Zhang*

Main category: Remote Sensing

TL;DR: 本研究开发了一个结合时空融合、水体掩膜、特征工程和机器学习的一体化框架，实现了三峡库区15年沿河道主流悬浮泥沙浓度（SSC）反演，发现2006-2020年间SSC在时间上下降21.2%，纵向上下降93.8%。


<details>
  <summary>Details</summary>
Motivation: 传统遥感在复杂地形、多云多雾、影像稀缺和混合像元区域面临挑战，难以实现长期连续的水质参数反演，亟需开发一套集成化框架来解决这些问题。

Method: 采用RASTFM时空融合算法、水体掩膜策略、特征工程和自适应机器学习模型（针对独立水文站）的序贯集成框架。

Result: (1) RASTFM在四种融合算法中光谱保真度最高；(2) 建立了自适应机器学习和水体掩膜策略；(3) 2006-2020年主流SSC时间下降21.2%，纵向下降93.8%，生成了15年沿河道主流SSC数据。

Conclusion: 该集成框架有效克服了复杂环境下的遥感反演难题，实现了长时间序列SSC的精准监测，为类似复杂水域的遥感应用提供了可推广的技术方案。

与遥感交叉或改进方案: 交叉/改进可行性：95%。该框架本身就是遥感论文，改进方向包括：(1) 将RASTFM替换为更先进的深度学习时空融合模型（如STFNet、GAN-based方法）以进一步提升光谱保真度；(2) 引入多源遥感数据（如Sentinel-1 SAR）以应对极端云雾天气；(3) 增加时序注意力机制捕捉SSC季节性变化规律；(4) 将框架迁移至其他多云雾水库/湖泊（如丹江口水库、鄱阳湖）进行泛化验证。

Abstract (英文): Conventional remote sensing faces challenges in regions characterized by complex terrain, frequent cloud and fog cover, image scarcity, and mixed pixels. This study develops an integrated framework that sequentially combines spatiotemporal fusion, water masking, feature engineering, and machine learning to enable long-term SSC retrieval. Key results include: (1) RASTFM achieved the highest spectral fidelity among four fusion algorithms; (2) development of an adaptive machine learning model for independent hydrological stations and a water body masking strategy; (3) mainstream SSC decreased by 21.2% temporally and a 93.8% longitudinally from 2006 to 2020, generating 15-year along-channel mainstream SSC data for the Three Gorges Reservoir area.

</details>


### [176] [A Two-Stage Reference-Guided Workflow for Improving VIIRS Leaf Area Index Retrieval over Mixed Pixels](https://doi.org/10.3390/rs18132214)
*Tengqi Yue,Haiyong Ding,Yuanfei Zhang*

Main category: Remote Sensing

TL;DR: 本文提出了一种参考引导的两阶段工作流，通过子像素标签校正（结合Sentinel-2 LAI与GBOV参考数据）和随机森林模型训练（引入VIIRS光谱、几何、纹理及子像素组成变量），显著提高了异质景观下VIIRS LAI反演精度，R²从0.752提升至0.908，LOSO验证R²达0.901，优于官方VNP产品。


<details>
  <summary>Details</summary>
Motivation: 中分辨率LAI反演在异质景观下受两个因素影响：粗分辨率预测因子中未解决的子像素组成问题，以及从高分辨率产品聚合得到的监督标签中的结构偏差。现有方法未能同时解决这两个问题，导致反演精度受限。

Method: 两阶段工作流：第一阶段，利用子像素PFT分数和森林敏感铰链项，对聚合的Sentinel-2 LAI进行GBOV参考LAI校准，生成校正后的500m标签；第二阶段，以VIIRS光谱反射率、观测几何、植被指数、纹理和子像素组成变量为输入，训练随机森林模型。基于2020-2021年美国11个GBOV站点数据开发，通过同站点时间迁移（2019/2022）和严格LOSO验证评估。

Result: 标签校准后R²从0.752提升至0.908，RMSE从1.110降至0.676；LOSO验证R²=0.901，RMSE=0.703；在2019/2022重叠子集上，最终模型R²=0.905，RMSE=0.609，远超官方VNP产品（R²=0.755，RMSE=0.978）。

Conclusion: 参考引导的标签校正结合显式子像素组成信息，能显著改善混合像元下的VIIRS LAI反演精度，验证了该方法在评估研究区域内的有效性。

与遥感交叉或改进方案: 交叉/改进可行性：95%。该工作流可直接推广至其他中分辨率传感器（如MODIS、FY-3/MERSI）的LAI反演；也可将随机森林替换为深度学习模型（如CNN、Transformer）以进一步捕捉空间上下文信息；此外，子像素PFT先验可引入多时相动态信息或气候区划知识提升泛化能力；标签校准策略还可应用于其他植被参数（如fPAR、植被高度）的参考数据融合问题。

Abstract (英文): Moderate-resolution leaf area index (LAI) retrieval over heterogeneous landscapes is affected not only by unresolved subpixel composition in coarse-resolution predictors, but also by structural bias in supervisory labels aggregated from higher-resolution products. To address this issue, we developed a reference-guided two-stage workflow to improve LAI retrieval from the Visible Infrared Imaging Radiometer Suite (VIIRS). In the first stage, aggregated Sentinel-2 LAI was calibrated against Ground-Based Observations for Validation (GBOV) LP3 reference LAI using subpixel plant functional type (PFT) fractions and forest-sensitive hinge terms to generate corrected 500 m labels. In the second stage, a random-forest model was trained using VIIRS spectral reflectance, viewing geometry, vegetation indices, texture, and subpixel compositional variables. Model development was based on 2020–2021 data from 11 U.S. GBOV sites. Performance was evaluated by same-site temporal transfer to 2019 and 2022 and by strict leave-one-site-out (LOSO) validation. Label calibration improved agreement with GBOV from a coefficient of determination (R2) of 0.752 and a root mean square error (RMSE) of 1.110 to an R2 of 0.908 and an RMSE of 0.676. Under LOSO validation, the final model achieved an R2 of 0.901 with an RMSE of 0.703. On the 2019/2022 overlap subset shared by the final VIIRS retrieval, the official VNP product, and the GBOV reference, the final model achieved an R2 of 0.905 and an RMSE of 0.609, compared with 0.755 and 0.978 for the official VNP product. These results show that reference-guided label correction, combined with explicit subpixel compositional information, can substantially improve VIIRS LAI retrieval over mixed pixels within the evaluated study domain.

</details>


### [177] [DST-SARNet: A Dual-Stage Texture-Aware SAR Prior Network for Cloud Removal in Optical Remote Sensing Images](https://doi.org/10.3390/rs18132199)
*Z Wang,Mingzhi Zhang,Yue Wang,Xudong Qiu,Jingqi Yan,Na Niu*

Main category: Remote Sensing

TL;DR: 本文提出DST-SARNet，一种双阶段SAR结构引导网络，用于光学遥感图像去云。该方法通过早期结构骨架引导和晚期高频调制两个明确SAR引导位置，结合纹理感知非对称检索模块，在保持紧凑模型规模的同时有效恢复云污染图像。


<details>
  <summary>Details</summary>
Motivation: 现有云去除方法存在不足：CNN方法难以建模大范围云区的全局结构和颜色一致性；Transformer方法标准自注意力对高分辨率遥感图像带来高昂计算和内存成本；高效注意力会削弱边缘和纹理判别性；反复注入SAR信息会传播散斑噪声、跨模态错位和成像差异。

Method: 提出双阶段SAR结构引导框架：1）早期结构骨架引导（输入侧）；2）晚期高频调制（靠近输出侧）。两阶段之间放置纹理感知非对称检索模块作为瓶颈记忆检索操作，而非密集SAR注入。SAR提供结构骨架、可读纹理记忆和终端细节补偿，光学分支负责颜色、语义和光谱外观恢复。

Result: 在SMILE-CR和SEN12MS-CR数据集上实验表明，DST-SARNet以紧凑模型规模有效恢复了云污染图像，展示了其在高效SAR辅助光学去云中的潜力。

Conclusion: 双阶段SAR引导设计能有效平衡SAR结构信息利用与光学分支重建能力，在避免SAR噪声传播的同时实现高质量去云。

与遥感交叉或改进方案: 交叉/改进可行性：100%。本文本身就是遥感图像去云论文。改进方向：1）可引入多模态融合增强模块（如可变形卷积）处理SAR与光学图像的几何错位问题；2）纹理感知检索模块可扩展为可学习原型记忆库，提升跨场景泛化能力；3）可引入时序信息（多时相光学图像）与SAR协同，进一步提升厚云区域的重建质量；4）可探索轻量化自注意力机制替代标准注意力，进一步降低计算开销。

Abstract (英文): Cloud contamination obscures ground objects, interferes with surface reflectance, and disrupts spatial continuity. In thick-cloud regions, surface structures and spectral information are often extensively missing. CNN-based cloud removal methods can recover local textures, but they are less effective at modeling global structures and color consistency over large cloud-covered areas. Transformer-based methods capture long-range dependencies; however, standard self-attention introduces high computational and memory costs for high-resolution remote sensing images. Efficient attention reduces this cost but may weaken edge and texture discriminability. SAR imagery can penetrate clouds and provide surface structural information, yet repeated SAR injections may propagate speckle noise, cross-modal misalignment, and imaging discrepancies through deep restoration layers. To address these issues, this paper proposes DST-SARNet, a dual-stage SAR structural guidance network for optical remote sensing image cloud removal. In this framework, dual-stage refers to two explicit SAR-guidance positions: early structural skeleton guidance at the input side and late high-frequency modulation near the output. The Texture-Aware Asymmetric Retrieval module is placed between these two stages as a bottleneck memory retrieval operation rather than as a third dense SAR injection stage. With this design, SAR provides structural skeletons, readable texture memory, and terminal detail compensation, while the optical branch remains responsible for color, semantics, and spectral appearance recovery. Experiments on the SMILE-CR and SEN12MS-CR datasets show that DST-SARNet effectively restores cloud-contaminated imagery with a compact model scale, demonstrating its potential for efficient SAR-assisted optical cloud removal.

</details>


### [178] [Physics-Guided Physical-Coordinate Center Detection for Measured Radar Point Targets Under Ground Clutter](https://doi.org/10.3390/rs18132204)
*Z H Zhang,Xiaoxiong Li,Jiachen Yang,Weiwei Li,Xi Zhang*

Main category: Remote Sensing

TL;DR: 本文提出WPG-Center检测器，将雷达目标检测转化为连续物理坐标中心定位，通过四通道杂波感知RD表示、各向异性骨干网络、波形分辨率感知高斯监督和条件物理先验解码，在实测LSS-Ku-1.0数据集上实现0.850的检测概率和18.16m的距离RMSE，优于CFAR和Radar-FPN等基线方法。


<details>
  <summary>Details</summary>
Motivation: 实测雷达点目标检测面临弱目标回波与杂波峰值竞争、图像空间检测无法直接表示物理距离-多普勒坐标两大挑战，现有方法难以有效应对。

Method: 1) 将检测问题重新定义为连续物理坐标中心定位；2) 构建四通道杂波感知距离-多普勒(RD)表示；3) 设计各向异性骨干网络并引入距离-杂波响应调制；4) 采用波形分辨率感知的高斯目标监督中心热图；5) 条件物理先验解码作为低带宽、近零多普勒场景的候选重排序步骤。

Result: 在实测LSS-Ku-1.0数据集上，采用严格的阻塞5折LFM协议，WPG-Center检测概率达0.850±0.101，距离RMSE为18.16±12.19m，在解码中心检测概率和距离精度上均优于CFAR和Radar-FPN等基线。固定σ消融实验性能显著下降，验证了分辨率感知监督的必要性。

Conclusion: WPG-Center在实测LFM场景下显著优于现有学习和CFAR基线，波形分辨率感知监督是关键设计。折叠分析、定性分析和SFW分析进一步界定了实测LFM结论的适用范围。

与遥感交叉或改进方案: 交叉/改进可行性：95%。该工作本身即为遥感雷达检测领域。改进方向：1) 可引入多模态融合（如光学/SAR与雷达RD图联合）增强杂波抑制；2) 四通道杂波感知表示可扩展至极化信息通道；3) 各向异性骨干可进一步适配不同雷达波形（如SFW）的响应特性；4) 可将中心检测范式迁移到SAR图像目标检测中，解决物理坐标对齐问题。

Abstract (英文): Measured radar point-target detection under ground clutter is difficult because weak target echoes often compete with clutter peaks, and image-space detections do not directly represent physical range-Doppler coordinates. This paper proposes a waveform- and physics-guided center detector (WPG-Center) for measured radar maps. The method reformulates detection as continuous physical-coordinate center localization, constructs a four-channel clutter-aware range-Doppler (RD) representation, uses an anisotropic backbone with range-clutter response modulation, and supervises the center heatmap with waveform-resolution-aware Gaussian targets. Conditional physical-prior decoding is used as a targeted candidate reranking step for low-bandwidth, near-zero-Doppler cases. Experiments are conducted on the measured LSS-Ku-1.0 dataset using a strict blocked 5-fold linear frequency-modulated (LFM) protocol with physical localization tolerances and decoded-center detection metrics. WPG-Center achieves a probability of detection (Pd) of 0.850±0.101 and a range root-mean-square error (RMSE) of 18.16±12.19 m, giving the best average decoded-center detection probability and range accuracy among the compared learned and constant false-alarm rate (CFAR)-style baselines, including an independent radar feature pyramid network (Radar-FPN) heatmap detector. The fixed-sigma ablation is substantially weaker, supporting the need for resolution-aware supervision. Fold-wise, qualitative, and stepped-frequency waveform (SFW) analyses are reported as auxiliary evidence to define the scope of the measured LFM conclusion.

</details>


### [179] [Replacing Yield Detrending with Direct Spatiotemporal Inputs Improves LSTM-Based Rice Yield Estimation](https://doi.org/10.3390/rs18132200)
*Nuo Chen,Fumin Wang,Xi Zhang,Zhen Zhao,Wenkai Wan,Junwei Liu,Zhou Shi,Songchao Chen*

Main category: Remote Sensing

TL;DR: 本文提出直接用时空变量（年份、经纬度）输入深度学习模型替代传统统计去趋势方法进行水稻产量估计。基于美国6州112个县2000-2021年的数据，比较了6种输入配置和4种模型架构，发现直接输入年份显著优于去趋势方法，时空组合配置表现最佳（LSTM R²=0.61），且空间信息能改善极端产量区域的系统偏差，在抽穗-灌浆期即可达到近收获期精度。


<details>
  <summary>Details</summary>
Motivation: 传统统计去趋势方法（如移动平均、线性回归）依赖于对时间-产量关系的先验假设，当假设不成立时可能引入系统偏差；同时，时空信息的各自贡献及其交互效应缺乏统一框架下的系统评估。

Method: 选取美国6个州112个水稻种植县2000-2021年的数据，使用NDVI、气象指标（GDD、KDD、累积降水）及时空变量（年、经度、纬度），设计6种输入配置（对比传统去趋势与直接输入时间变量），在LSTM、Random Forest、XGBoost、Transformer四种模型架构上进行测试，采用SHAP进行特征重要性分析。

Result: (1) 直接输入年份在所有模型中显著优于去趋势方法，时空组合配置最佳（LSTM R²=0.61 vs 去趋势0.54）；(2) SHAP分析中年份是最重要预测因子，时空变量排名高于多数气象和遥感变量；(3) 空间信息持续提升精度并缓解极端产量区域的系统偏差；(4) 组合配置在不同州、年份（含极端气候事件）及产量水平上表现最佳，在灌浆期（收获前1.5-2个月）即可达到近收获期精度。

Conclusion: 直接将原始时空数据输入深度学习模型比统计去趋势方法更有效，为大规模作物产量估计提供了一种更简单且更鲁棒的方案。

与遥感交叉或改进方案: 交叉/改进可行性：90%。该论文方法可直接迁移至遥感作物产量估算领域：(1) 将时空变量（年份、经纬度）作为显式输入特征嵌入现有遥感产量模型（如CNN、LSTM、Transformer），替代传统去趋势预处理；(2) 在区域尺度遥感估算中，引入空间坐标信息可有效校正不同生态区间的系统偏差，提升模型泛化能力；(3) 可扩展至其他作物（小麦、玉米、大豆等）及全球尺度，结合多源遥感数据（如Sentinel-2、MODIS）验证其普适性；(4) SHAP可解释性分析可用于识别不同区域的主导产量驱动因子，指导遥感特征筛选。

Abstract (英文): Accurate rice yield estimation is essential for food security. Two key factors affecting estimation accuracy are the long-term upward trend in yield over time and regional heterogeneity across space. Current studies predominantly employ statistical detrending methods (e.g., moving averages, linear regression) to isolate temporal trends. However, such methods rely on prior assumptions about the time–yield relationship and may introduce systematic bias when these assumptions break down. Meanwhile, the individual contributions of temporal and spatial information, and their interactive effects, have not been systematically evaluated within a unified framework. We selected 112 rice-growing counties across six U.S. states (2000–2021), using vegetation index (Normalized Difference Vegetation Index), meteorological indicators (growing degree days, killing degree days, and cumulative precipitation), and spatiotemporal variables (year, longitude, and latitude). We designed six input configurations to compare conventional detrending against direct temporal variable inclusion, testing across four model architectures (Long Short-Term Memory, Random Forest, XGBoost, and Transformer). Results showed that: (1) directly inputting year significantly outperformed detrending across all models, with the combined spatiotemporal configuration achieving the best performance (LSTM R2 = 0.61 vs. 0.54 for detrending); (2) year was the most important predictor in SHAP analysis, with spatiotemporal variables ranking higher than most meteorological and remote sensing variables; (3) spatial information consistently improved accuracy and mitigated systematic bias for extreme yield regions; (4) the combined configuration performed best across different states, years (including extreme climate events), and yield levels, achieving near-end-of-season accuracy at the grain-filling stage (1.5–2 months before harvest). This study demonstrates that integrating raw spatiotemporal data directly into deep learning models is more effective than statistical detrending, offering a simpler and more robust approach for large-scale crop yield estimation.

</details>


### [180] [UAV-Based Soil Salinity Estimation Using Stagewise Feature Optimization and Dual-Backbone Deep Learning Fusion](https://doi.org/10.3390/rs18132237)
*Chao Zhang,Yujie Hu,Min Tang,Shaoyuan Feng,Zhen Zheng,Ziang Xie,Zhijun Jia,Huailiang Wang,Lei Xie*

Main category: Remote Sensing

TL;DR: 本文提出了一种基于无人机（UAV）的土壤盐分估算框架，结合轻量级神经网络和特征优化策略，在小样本农业条件下实现了地表及地下土壤盐分的高精度估算。通过RF-RFE、One-SE和VIF将129个候选变量精简为16通道特征集，构建了单骨干（VGGNet、ResNet、DenseNet）和双骨干特征级融合网络（DenseResNet、DenseVGGNet、ResVGGNet）。ResVGGNet在地表盐分估算中表现最优（R²=0.820，RMSE=0.626 g/kg），SHAP分析揭示了植被和盐分敏感指数的主导作用。


<details>
  <summary>Details</summary>
Motivation: 小样本农业条件下土壤盐分估算面临标记数据稀缺、单骨干神经网络表征能力有限以及地下盐分反演复杂度高等三重挑战，亟需一种轻量、高效且可解释的估算方法。

Method: 1）利用UAV获取RGB和多光谱影像；2）采用RF-RFE、One-SE准则和VIF分析进行特征选择，将129个候选变量缩减为16通道统一特征集；3）构建三种单骨干（VGGNet、ResNet、DenseNet）和三种双骨干特征级融合网络（DenseResNet、DenseVGGNet、ResVGGNet）；4）使用SHAP分析进行模型可解释性评估。

Result: 单骨干网络中ResNet统计精度最高，DenseNet趋势保持最优。双骨干网络ResVGGNet在地表盐分估算中表现最佳（R²=0.820，RMSE=0.626 g/kg，MAE=0.409 g/kg，RPD=2.31）。SHAP分析表明植被指数、盐分敏感指数及选定的光谱均值特征为主要贡献特征，各输入通道呈现空间互补响应模式。

Conclusion: 轻量级多骨干特征级融合结合精简特征优化策略，能有效提升UAV土壤盐分估算的精度、鲁棒性和可解释性，尤其在小样本农业条件下效果显著。

与遥感交叉或改进方案: 交叉/改进可行性：95%。改进方案：1）可引入自注意力机制或Transformer模块替代部分卷积层，进一步提升多骨干网络的特征提取能力；2）当前仅使用RGB和两个多光谱波段，可考虑引入热红外或雷达数据增强盐分敏感性；3）特征优化阶段可加入自动搜索策略（如遗传算法或贝叶斯优化）替代人工设定的RF-RFE阈值；4）可将框架扩展到更大尺度的卫星遥感影像（如Sentinel-2），结合UAV数据进行跨尺度迁移学习；5）SHAP分析可进一步结合空间显式建模，生成盐分分布的空间可解释性图谱。

Abstract (英文): Accurate soil salinity estimation under small-sample agricultural conditions continues to pose a formidable challenge, attributed to the scarcity of labeled data, inherent representational limitations of single-backbone neural networks, and the heightened complexity of subsurface salinity inversion. To mitigate these intertwined challenges, this study developed a UAV-enabled soil salinity estimation framework that integrated lightweight convolutional neural networks and staged feature optimization, leveraging both RGB and multispectral imagery. A feature selection framework integrating random forest recursive feature elimination (RF-RFE), the one-standard-error (One-SE) criterion, and variance inflation factor (VIF) analysis was employed to reduce 129 candidate variables to a unified 16-channel feature set, which served as the common input for estimating both surface and subsurface soil salinity. Three lightweight single-backbone (VGGNet, ResNet, and DenseNet) and dual-backbone feature-level fusion networks (DenseResNet, DenseVGGNet, and ResVGGNet) were constructed and systematically evaluated for their performance in estimating both surface and subsurface soil salinity. Among the single-backbone networks, ResNet yielded the highest overall statistical accuracy, while DenseNet exhibited superior performance in preserving estimation trends. For surface soil salinity estimation, ResVGGNet achieved the best performance among all evaluated models, with an R2 of 0.820, RMSE of 0.626 g/kg, MAE of 0.409 g/kg, and RPD of 2.31 on the test dataset. SHAP analysis further highlighted the dominant role of vegetation and salinity-sensitive indices, together with selected spectral mean features, and revealed spatially complementary response patterns among major input channels. Collectively, the integration of lightweight multi-backbone feature-level fusion with streamlined feature optimization strategies effectively enhances the accuracy, robustness, and interpretability of UAV-enabled soil salinity estimation, particularly under the constraint of small agricultural sample sizes.

</details>


### [181] [Memory-Guided Adaptive Spectral–Spatial Perception Model for Hyperspectral Image Classification](https://doi.org/10.3390/rs18132225)
*Xinhui Wang,Bin Yan,Pengyu Guo,Xiaolong Yang,Hongyu Liu,LZ Cao,Yong Liu,Zhi Yang,Yuhang Liu*

Main category: Remote Sensing

TL;DR: 提出了一种记忆引导的自适应光谱-空间感知模型(MG-ASSP)，通过三级全球化策略（单样本级、批内级、跨批级）在极少标注样本下实现高光谱图像准确分类，在四个数据集上以1%或0.5%训练样本取得SOTA性能。


<details>
  <summary>Details</summary>
Motivation: 高光谱图像分类受限于标注样本稀缺，现有方法在小样本条件下分类性能不足，亟需一种能在极少量标注数据下仍保持高精度的分类模型。

Method: 提出三级全球化策略：(1) 单样本级：自适应感知Transformer，结合可变形/扩张卷积与Transformer编码器捕获单个样本的全局上下文；(2) 批内级：度量学习策略，显式捕捉小批量内样本间的结构依赖与特征关系；(3) 跨批级：记忆引导策略，构建动态记忆库跨训练迭代存储和检索同类样本特征，弥合过去与当前分布以增强泛化。

Result: 仅用1%训练样本在SaliLMSS(96.15%)、Pavia University(97.81%)、Kennedy Space Center(89.22%)数据集和0.5%训练样本在WHU-LongKou(99.32%)数据集上达到总体精度，超越现有方法。

Conclusion: 三级全球化策略有效解决了小样本高光谱分类难题，记忆引导的跨批次特征聚合显著提升了模型泛化能力，在极低标注率下仍能达到实用级精度。

与遥感交叉或改进方案: 交叉/改进可行性：90%。该论文本身就是高光谱遥感分类工作。改进方向：(1) 记忆模块可引入对比学习或原型网络进一步优化类间可分性；(2) 三级策略可扩展到多模态遥感数据（如高光谱+LiDAR+SAR）的跨模态特征对齐与融合；(3) 自适应感知Transformer可结合光谱-空间-时间三维特征用于时序遥感分析；(4) 动态记忆库可引入遗忘机制和类别均衡采样以应对长尾分布场景。

Abstract (英文): Accurate hyperspectral image classification is fundamental to geospatial applications but is often constrained by annotation scarcity. To achieve high classification performance under small-sample conditions, we propose the Memory-Guided Adaptive Spectral–Spatial Perception model, which incorporates a three-level globalization strategy. At the single-sample level, an adaptive perception Transformer combines deformable and dilated convolutions with a Transformer encoder to capture global context from individual samples. At the intra-batch level, we introduce a metric learning strategy that explicitly captures structural dependencies and feature relationships among samples within each mini-batch, enabling comprehensive feature aggregation in a localized context. At the cross-batch level, a memory-guided strategy constructs a dynamic memory bank to store and retrieve features from same-class samples across training iterations, bridging past and present distributions to enhance generalization. Using only 1% of the SaliLMSS, Pavia University and Kennedy Space Center datasets and 0.5% of the WHU-LongKou dataset as training samples, our method achieves outstanding overall accuracy of 96.15%, 97.81%, 89.22% and 99.32%, respectively, outperforming existing methods.

</details>


### [182] [Assessing Urban Ventilation Resistance and Surface Warming Using Multi-Source Data: A Case Study of Kaifeng City](https://doi.org/10.3390/rs18132227)
*Huiqi Sun,Hao Zheng,Lu Yu,Jingyuan Cheng*

Main category: Remote Sensing

TL;DR: 本研究集成XGBoost、SHAP和多尺度地理加权回归（MGWR），分析了开封市1986-2024年间城市形态、生态和人类活动因素对地表温度（LST）的长期影响。结果表明：LST显著上升且年际变率减小；夜间灯光（NTL）始终是主导因子，建筑密度（BF）与NTL共同捕捉了城市扩张效应；迎风面密度（FAD）在城郊区域的空间异质性比核心区高出2.74倍，提供了传统密度指标无法捕捉的通风阻力诊断信息。


<details>
  <summary>Details</summary>
Motivation: 城市形态变化显著影响地表热环境，但长期定量评估——特别是通风阻力在其中的作用——仍然有限。现有研究多关注短期或单维度的城市形态-LST关系，缺乏综合建筑密度、通风阻力、生态条件与人类活动强度的长期驱动框架。

Method: 集成XGBoost（特征重要性排序）、SHAP（可解释性分析）和MGWR（空间异质性建模），定量分析六类形态、生态和人类活动因子对LST的影响。选取开封市1986、1990、2000、2010、2020五个代表年份进行对比。

Result: （1）LST在1986-2024年间显著上升，年际变率下降；（2）NTL在五个年份均为主导因子，BF与NTL共同表征城市扩张和人类活动密集化；（3）FAD在城郊的空间异质性（系数离散度）为城市核心区的2.74倍（2020年），虽然对整体预测精度贡献较低，但能提供传统密度指标无法捕捉的通风约束诊断信息，且表现出非线性、方向性和空间异质性响应。

Conclusion: 该研究构建了长期视角下建筑密集化、通风形态阻力、生态条件和人类活动强度耦合的LST驱动框架，弥补了以往研究短期或单维度评估的不足，为城市更新和向外扩张过程中的热风险管控提供了科学依据。

与遥感交叉或改进方案: 交叉/改进可行性：95%。改进方案：（1）可将该方法框架直接迁移至遥感城市热岛（SUHI）研究，利用Landsat/MODIS长时序LST数据替代本地气象站观测，提升空间覆盖度；（2）引入遥感提取的3D建筑形态参数（如LiDAR或立体像对获取的建筑高度、体形系数）替代FAD，进一步提升通风阻力表征精度；（3）结合遥感生态指数（RSEI）替代单一生态因子，丰富生态维度输入；（4）在遥感影像上采用滑动窗口或深度学习语义分割方法自动提取建筑轮廓和迎风面参数，实现大尺度自动化分析。

Abstract (英文): Changes in urban form strongly affect surface thermal conditions, yet long-term quantitative assessments of this relationship, particularly the role of ventilation resistance, remain limited. To address this gap, this study integrates XGBoost, SHapley Additive explanations (SHAP), and multi-scale geographically weighted regression (MGWR) to examine how six morphological, ecological, and human-activity factors influence land surface temperature (LST) in Kaifeng City. The results indicate three main findings. First, LST increased significantly from 1986 to 2024, while interannual variability declined, indicating a gradual reduction in regional thermal fluctuations. Second, NTL was consistently the dominant indicator across the five representative years, while BF and NTL together captured the effects of urban expansion and intensified human activity. Third, FAD coefficients were more spatially heterogeneous in urban fringe areas than in the urban core. In 2020, the dispersion of FAD coefficients in fringe areas was 2.74 times greater than that in the central area, indicating stronger spatial differentiation in ventilation-related morphological constraints during urban expansion. Although FAD made only a modest contribution to overall predictive accuracy, it provided supplementary diagnostic information not captured by conventional density indicators and showed nonlinear, directional, and spatially heterogeneous responses. Compared with previous studies that mainly examined short-term or single-dimensional relationships between urban morphology and LST, this study integrates building densification, ventilation-related morphological resistance, ecological conditions, and human activity intensity into a long-term LST-driver framework, providing evidence to support heat-risk management during urban regeneration and outward expansion.

</details>


### [183] [Lagged Response of Groundwater Storage to Extreme Precipitation Using Machine-Learning-Downscaled GRACE Data at the Watershed Scale](https://doi.org/10.3390/rs18132217)
*Xiaoling Zheng,Yilei Yu,Jiyi Jiang,Lihu Yang,Shiqin Wang*

Main category: Remote Sensing

TL;DR: 本研究基于GRACE卫星数据，利用LightGBM模型降尺度构建了2002-2024年白洋淀流域1km分辨率的地下水储量异常数据集，揭示了地下水储量在2014年前快速枯竭、2014-2019年缓慢下降、2020年后显著恢复的三阶段演变特征，并发现极端降水事件对地下水补给存在约两个月的滞后响应，月尺度有效降水比日尺度极端降水对地下水恢复的贡献更大。


<details>
  <summary>Details</summary>
Motivation: 理解地下水储量如何响应极端降水对于评估气候变化下含水层的恢复力至关重要，但目前缺乏高分辨率的地下水储量数据来量化这种响应关系。

Method: 采用LightGBM模型对GRACE观测数据进行降尺度处理，生成1km分辨率的地下水储量异常数据集；利用独立水文数据集（GLDAS和地下水位异常）进行验证；通过复合分析（composite analysis）研究11次极端降水事件前后地下水储量的响应特征。

Result: 降尺度GWSA与GLDAS和地下水位异常数据一致性良好；地下水储量呈现三阶段变化：2014年前快速枯竭（-1.35 cm/yr）、2014-2019年缓慢下降（-1.04 cm/yr）、2020年后显著恢复（+3.45 cm/yr）；极端降水事件后约两个月出现最强地下水响应信号；月有效降水比短时强降水对GWSA恢复的关联更强；平原区响应强于山区。

Conclusion: 极端降水事件提供脉冲式补给，但持续的地下水恢复依赖于累积的气候输入和人类水管理措施的共同影响。

与遥感交叉或改进方案: 交叉/改进可行性：90%。改进方案：（1）可引入多源遥感数据（如Sentinel-1 InSAR形变数据、SMAP土壤水分数据）作为辅助特征，提升LightGBM模型对地下水储量变化的刻画精度；（2）可结合更多的遥感降水产品（如IMERG、GPM）对极端降水事件进行更精细的时空识别；（3）在白洋淀流域验证有效后，该方法可迁移至其他缺资料流域（如西北干旱区、青藏高原内流区），结合当地遥感数据构建高分辨率地下水储量产品。

Abstract (英文): Understanding how groundwater storage responds to extreme precipitation is essential for assessing aquifer resilience under climate variability. In this study, we developed a 1 km groundwater storage anomaly (GWSA) dataset for the Baiyangdian Watershed from 2002 to 2024 by downscaling GRACE observations with a Light Gradient Boosting Machine (LightGBM) model. The downscaled GWSA showed good consistency with independent hydrological datasets, including GLDAS and groundwater-level anomalies. Based on the downscaled product, we characterized long-term groundwater changes and quantified GWSA responses to extreme precipitation events (EPEs). Groundwater storage exhibited three distinct phases: rapid depletion before 2014 (−1.35 cm/yr), a slower decline during 2014–2019 (−1.04 cm/yr), and marked recovery after 2020 (+3.45 cm/yr). Spatially, GWSA generally increased from the southwest to the northeast of the watershed. Composite analysis of 11 EPEs revealed a delayed groundwater response, with the strongest signal occurring approximately two months after precipitation. Monthly effective precipitation was more closely associated with GWSA recovery than short-duration daily precipitation extremes, and the response was stronger in plains than in mountainous areas. These findings indicate that EPEs provide episodic recharge pulses, while sustained groundwater recovery depends on cumulative climatic inputs and human water-management influences.

</details>


### [184] [SAGE: Saliency and Geometry Enhanced Transferable Attacks for LiDAR Point Cloud Perception in Remote Sensing](https://doi.org/10.3390/rs18132209)
*Yuheng Wu,Shiwei Lin,Shibo Ping,Xingchao Zhai,Zhiyuan Fang,Meijuan Chen,Weiquan Liu*

Main category: Remote Sensing

TL;DR: 本文提出SAGE框架，一种针对LiDAR点云感知模型的可迁移性对抗攻击方法，通过融合显著性引导的扰动分配与几何约束，在提升攻击成功率的同时保持点云几何质量，在分类和3D检测任务上均优于现有方法。


<details>
  <summary>Details</summary>
Motivation: 现有针对LiDAR点云的可迁移攻击存在两个问题：一是扰动区域仅对代理模型判别重要但跨架构稳定性不足，导致可迁移性有限；二是扰动导致明显的几何畸变。因此需要一种既能增强攻击可迁移性又能保持点云几何质量的攻击框架。

Method: SAGE框架包括两个核心组件：1）统一点坐标先验与源模型梯度信号的显著性图生成机制，用于识别易受攻击的局部结构；2）基于显著性图的扰动分配与显式几何约束，在增强可迁移性的同时保持点云几何质量。

Result: 在点云分类基准和基于KITTI、nuScenes的LiDAR 3D目标检测任务上，SAGE在攻击成功率上一致优于现有可迁移攻击方法，同时保持对抗点云良好的几何质量。

Conclusion: SAGE提供了一个有效且实用的框架，用于评估基于LiDAR的遥感感知系统的可迁移鲁棒性。

与遥感交叉或改进方案: 交叉/改进可行性：95%。本文本身就是面向遥感感知（LiDAR点云）的对抗攻击研究，可直接应用于遥感领域。改进方向：1）考虑引入多模态数据（如图像-点云融合）的显著性信息来增强攻击可迁移性；2）针对特定遥感任务（如地物分类、建筑物提取）设计任务相关的几何约束；3）结合遥感场景中的稀疏点云特性优化显著性图生成策略；4）扩展到对抗防御领域，利用SAGE生成的对抗样本训练更鲁棒的遥感感知模型。

Abstract (英文): LiDAR point clouds are widely used in remote sensing perception scenarios, such as autonomous driving. However, LiDAR-based perception models remain vulnerable to adversarial perturbations, which may compromise the reliability of safety-critical 3D perception systems. Among different attack paradigms, transfer-based attacks are particularly practical because they generate adversarial examples on accessible surrogate models and apply the generated examples directly to unknown target models. Nevertheless, existing transferable attacks on point clouds often perturb regions that are discriminative for the surrogate model but insufficiently stable across different architectures, leading to limited transferability and noticeable geometric distortion. To address this problem, we propose SAGE, a Saliency And Geometry Enhanced transferable attack framework for LiDAR point cloud perception in remote sensing. Specifically, SAGE unifies point-coordinate priors with source-model gradient signals to generate a saliency map, which serves as a transferable indicator of vulnerable local structures. SAGE further leverages this map through saliency-guided perturbation allocation and explicit geometric constraints to enhance transferability while preserving point-cloud geometry. To demonstrate the effectiveness of SAGE, we evaluate SAGE on point-cloud classification benchmarks and further validate it on LiDAR-based 3D object detection using KITTI and nuScenes. Experimental results show that SAGE consistently outperforms existing transferable attack methods in attack success rate while preserving favorable geometric quality of adversarial point clouds. These findings demonstrate that SAGE offers an effective and practical framework for assessing the transfer robustness of LiDAR-based remote sensing perception systems.

</details>


### [185] [Finding the Features with LiDAR and SAR: Automated Detection of Archaeological Earthworks at Cahokia](https://doi.org/10.3390/rs18132229)
*Justin M. Vilbig,Vasit Sagan,Joseph A. Jilek,Cagri Gul*

Main category: Remote Sensing

TL;DR: 本研究提出了一种半自动化遥感流程，用于在混合环境遗址（卡霍基亚土丘）中检测和勾勒考古地貌特征。该流程结合LiDAR（无人机与载人飞机）和SAR（PALSAR-3、Sentinel-1）数据，通过局部地形模型（LRM）、K-means聚类、OpenCV轮廓提取以及局部入射角（LIA）偏差映射等方法，实现了对土丘、取土坑和堤道等考古特征的有效识别。结果表明LiDAR和SAR具有互补优势，且传感器平台选择比脉冲功率密度更为关键。该方法可扩展、与场地无关，并支持开放数据。


<details>
  <summary>Details</summary>
Motivation: 复杂混合环境遗址中的考古特征检测需要精确高效的方法来识别细微的地形形态特征。传统方法在卡霍基亚土丘这类大型密西西比城市中心面临挑战，需要开发更有效的半自动化遥感流程。

Method: 使用三种LiDAR数据集（两种UAV、一种载人飞机）生成DTM，转换为局部地形模型（LRM），应用K-means聚类分割特征类别，再利用OpenCV库进行轮廓边界提取。同时，利用PALSAR-3和Sentinel-1的SAR数据生成局部入射角（LIA）栅格，通过角度偏差映射识别与考古特征相关的坡度异常。

Result: 五个数据集的结果表明LiDAR和SAR具有互补优势：LiDAR擅长解析高程定义特征（如土丘足迹），LIA能捕捉方向性坡度行为，突显土丘边缘、取土坑边缘和堤道等线性特征。LiDAR采集频率对比显示脉冲设置差异对考古特征恢复影响甚微，传感器平台选择比功率密度权衡更重要。

Conclusion: 尽管仍需人工审查以过滤现代干扰和自然假阳性，但整合流程能显著加快勘察速度并减少解释主观性。该方法具有可扩展性、场地不变性，且适用于开放数据，可用于全球范围内的考古景观。

与遥感交叉或改进方案: 交叉/改进可行性：85%。该论文本身就是遥感考古应用研究。改进方向：（1）引入深度学习（如U-Net、Mask R-CNN）替代K-means聚类，提升特征分割精度；（2）融合多时相SAR数据（如InSAR时序分析）以检测地下或隐蔽考古结构；（3）增加多光谱/高光谱数据辅助验证，区分自然地形与人工地貌；（4）结合随机森林或SVM等监督分类方法，减少人工审查负担；（5）扩展至其他地貌类型遗址（如丘陵、冲积平原）验证方法普适性。

Abstract (英文): Archaeological feature detection at complex, mixed-environment sites requires accurate, efficient methods for identifying subtle morphological signatures. This study presents a semi-automated remote sensing pipeline for the detection and delineation of archaeological earthworks at Cahokia Mounds (Illinois, USA), a major Mississippian urban center and UNESCO World Heritage Site. Three LiDAR datasets, two collected via UAV-mounted sensors and one from a piloted aircraft survey, were processed into Digital Terrain Models and transformed into Local Relief Models (LRM). K-means clustering was applied to segment the LRMs into feature classes, followed by contour bounding using the OpenCV library to outline mounds and borrow pits. Additionally, SAR-derived Local Incidence Angle (LIA) rasters from PALSAR-3 and Sentinel-1 were processed through angular deviation mapping to identify slope anomalies associated with archaeological features. Results across all five datasets demonstrate the complementary strengths of LiDAR and SAR: LiDAR excels at resolving elevation-defined features such as mound footprints, while LIA captures directional slope behavior that highlights mound edges, borrow pit rims, and linear features such as causeways. Comparative analysis of LiDAR acquisition frequencies reveals minimal differences in archaeological feature recovery between pulse settings, suggesting that sensor platform choice matters more than power-density tradeoffs for this application. Despite the need for human review to filter modern disturbances and natural false positives, the integrated workflow meaningfully accelerates prospection and reduces interpretive subjectivity. The methods are scalable, site-invariant, and work with open-access data, making them applicable to archaeological landscapes worldwide.

</details>


### [186] [Locally Adaptive Mamba and Multi-Scale Feature Enhancement for Optical Remote Sensing Image Change Detection](https://doi.org/10.3390/rs18132226)
*Mingxuan Ding,Qirong Zhou,Qiaolin Ye,Le Sun*

Main category: Remote Sensing

TL;DR: 本文提出LADENet，一种融合CNN、Transformer和Mamba范式的遥感变化检测框架，通过多尺度全局特征编码、自适应局部特征标记、双分支差分增强和跨尺度特征融合解码，在四个基准数据集上达到了领先性能（LEVIR-CD: 91.06% F1, 85.28% IoU）。


<details>
  <summary>Details</summary>
Motivation: 现有遥感变化检测方法面临三大挑战：局部细节与全局特征的协同建模困难、双时序差异表示单一、跨尺度特征通信不足，导致复杂环境下模型精度和鲁棒性受限。

Method: 1) 权重共享的多级Transformer编码器+序列缩减机制生成多尺度全局特征；2) 基于状态空间扫描(SSS)的双时空自适应局部特征标记模块，含自适应扫描策略、跨时序状态空间扫描和滑动窗口边界校准；3) 双分支差分增强模块，含注意力增强差分分支和多尺度卷积拼接分支；4) 全局跨尺度空间特征融合解码器，结合空间注意力和二维选择性扫描。

Result: LEVIR-CD: F1 91.06%, IoU 85.28%; WHU-CD: F1 90.51%, IoU 82.45%; CLCD-CD: F1 82.15%, IoU 72.83%; GVLM-CD: F1 89.12%, IoU 77.78%。在所有基准上均超越现有方法。

Conclusion: LADENet通过CNN-Transformer-Mamba三重范式融合，在双时序遥感变化检测任务中实现了优异的检测精度、边界描绘能力和泛化性能，验证了多范式协同的有效性。

与遥感交叉或改进方案: 交叉/改进可行性：95%。改进方案：1) 可引入多光谱/高光谱波段的自适应通道选择机制，增强对光谱变化敏感场景的鲁棒性；2) 滑动窗口边界校准可结合超像素分割先验，进一步改善不规则地物边界检测；3) 双分支差分模块可扩展为多时相（≥3时相）变化检测，适用于长期地表监测任务；4) 状态空间扫描的计算效率在大幅面遥感影像上可进一步优化，如结合分块推理或稀疏扫描策略。

Abstract (英文): Within the domain of Earth observation, tracking terrestrial transitions via high-resolution optical data plays a fundamental role. Nevertheless, current methods face critical challenges, including the difficulty in collaborative modeling of local details and global features and the singularity of bi-temporal difference representation, along with insufficient cross-scale feature communication, thereby constraining both the precision and resilience of models when applied to complicated environments. To solve these problems, we propose LADENet (Locally Adaptive Mamba and Multi-scale Feature Enhancement Network), an innovative framework that synergizes CNN, Transformer, and Mamba paradigms. By leveraging customized local contextual refinement alongside sophisticated hierarchical fusion, this integration delivers highly precise and resilient detection performance. LADENet adopts a weight-sharing multi-level Transformer encoder combined with a sequence reduction mechanism to generate multi-scale global features, achieving precise alignment of bi-temporal features and global context modeling while reducing computational complexity. To realize accurate localization and local enhancement of changed regions, we design a dual spatiotemporal adaptive local feature marking module based on State-Space Scanning (SSS). This module screens high-saliency changed regions through an adaptive scanning strategy, realizes pixel-aligned spatiotemporal feature fusion via cross-temporal state-space scanning, and introduces a sliding window boundary calibration mechanism to alleviate boundary information loss caused by window segmentation. To strengthen the feature representation of changed regions, a dual-branch difference enhancement module is constructed, which collaboratively captures global change trends and fine-grained local features through an attention-enhanced difference branch and a multi-scale convolution concatenation branch, effectively suppressing background interference. To address the semantic gap between cross-scale features, a global cross-scale spatial feature fusion decoder is proposed, which balances local detail preservation and global context perception through the synergy of spatial attention and two-dimensional selective scanning, completing refined multi-scale feature fusion and spatial resolution recovery. To rigorously validate the proposed LADENet, comprehensive experiments were conducted across four widely adopted bi-temporal benchmarks: LEVIR-CD, WHU-CD, CLCD-CD, and GVLM-CD. The presented architecture establishes substantial superiority over existing cutting-edge methodologies across primary evaluation criteria. Specifically, it yields an F1-measure of 91.06% alongside an IoU of 85.28% in the LEVIR-CD tests, while registering 90.51% (F1) and 82.45% (IoU) for WHU-CD. Similarly, robust outcomes are delivered on CLCD-CD (82.15% F1, 72.83% IoU) as well as GVLM-CD (89.12% F1, 77.78% IoU). These results demonstrate that LADENet possesses excellent detection accuracy, boundary delineation capability and generalization performance in diverse and intricate bi-temporal observation environments.

</details>


### [187] [Efficient Sea Clutter Suppression Algorithm Based on BCD-Accelerated Dictionary Learning and TQWT Denoising](https://doi.org/10.3390/rs18132201)
*Jin Wang,Yubing Han,Yancun Lyu*

Main category: Remote Sensing

TL;DR: 本文提出了一种结合BCD加速字典学习与TQWT去噪的海杂波抑制方法，通过BCD策略替代全局SVD、自适应软阈值算子、批量自适应OMP及TQWT滤波，在实测雷达数据上实现了17.48 dB的平均SCNR提升，总执行时间仅7.99秒，显著优于传统K-SVD算法。


<details>
  <summary>Details</summary>
Motivation: 复杂海况下弱雷达目标检测面临非平稳海杂波和海尖峰干扰，而传统字典学习算法计算复杂度高，难以在实际应用中兼顾抑制性能与计算效率。

Method: 提出级联BCD加速字典学习与TQWT去噪的方法：1) 字典学习阶段采用BCD策略替代全局SVD，结合自适应软阈值算子实现字典原子和稀疏系数的低复杂度联合优化；2) 采用批量自适应OMP算法，引入Gram矩阵预计算和双停止机制实现高效重构与杂波初步对消；3) 利用TQWT的窄带特征表示和位移不变性滤除残余非平稳杂波和噪声。

Result: 在IPIX数据库和《雷达学报》数据集上，平均SCNR提升17.48 dB，总执行时间仅7.99秒，在抑制性能与计算效率之间取得了优越平衡。

Conclusion: 所提方法有效解决了传统字典学习计算复杂度高的问题，显著提升了杂波抑制性能，为复杂海况下的弱目标检测提供了高效可行的方案。

与遥感交叉或改进方案: 交叉/改进可行性：95%。该方法是遥感领域海面目标检测的强相关技术，可直接应用于星载或机载SAR/ISAR海面目标检测预处理。改进方向：1) 将BCD字典学习扩展到高维极化SAR数据以提升泛化能力；2) 结合深度学习特征提取替代手工字典，进一步提升对复杂海况的适应性；3) 将TQWT参数自适应化，根据海况等级自动调节Q因子以优化滤波效果。

Abstract (英文): Detecting weak radar targets in complex sea conditions is inherently challenging due to non-stationary sea clutter and sea spikes. Furthermore, traditional dictionary learning algorithms for clutter suppression suffer from high computational complexity. To address these issues, this paper proposes an efficient sea clutter suppression method cascading Block Coordinate Descent (BCD)-accelerated dictionary learning with Tunable Q-factor Wavelet Transform (TQWT) denoising. During dictionary learning, a BCD strategy replaces global Singular Value Decomposition (SVD) with analytical optimization. Combined with an adaptive soft-thresholding operator, this enables low-complexity joint optimization of dictionary atoms and sparse coefficients, drastically reducing training time. Subsequently, a batch-adaptive Orthogonal Matching Pursuit (OMP) algorithm featuring Gram matrix precomputation and a dual-stop mechanism achieves efficient reconstruction and preliminary cancellation of clutter components. Finally, TQWT is applied to filter out residual non-stationary clutter and noise by leveraging its narrowband feature representation and shift invariance. Experiments on measured radar data from the IPIX database and datasets published by the Journal of Radars demonstrate that the proposed method significantly outperforms traditional K-SVD-based algorithms. Specifically, it improves the average signal-to-clutter-plus-noise ratio (SCNR) by 17.48 dB and requires a total execution time of only 7.99 s, achieving a highly favorable trade-off between suppression performance and computational efficiency.

</details>


### [188] [MCPF-Net: Multi-Stage LiDAR-Image Collaborative Perception Fusion Network for Point Cloud Semantic Segmentation of Urban Scenes](https://doi.org/10.3390/rs18132218)
*Huchen Li,Wubiao Huang,Xiangda Lei,Bin Liu,Haibing Liu,Shihan Chen,Fei Deng*

Main category: Remote Sensing

TL;DR: 提出多级LiDAR-图像协同感知融合网络(MCPFNet)，通过中间融合阶段的几何引导融合模块和语义感知交叉注意力模块，以及后期融合阶段的双向自适应融合模块，实现点云语义分割中多模态信息的双向交互与互补，在三个数据集上达到SOTA性能。


<details>
  <summary>Details</summary>
Motivation: 现有多模态融合方法在点云语义分割中存在跨模态感知能力有限、信息互补性不足的问题，难以充分融合LiDAR的精确3D几何结构与光学图像的丰富语义纹理信息。

Method: 提出MCPFNet，包含三个阶段：中间融合阶段采用高程引导的几何感知融合模块和语义感知交叉注意力模块实现双向特征注入；后期融合阶段采用门控加权和双向交叉注意力的自适应融合模块进一步细化语义表示。

Result: 在ISPRS Vaihingen(74.51% mIoU)、N3C-California(95.15% mIoU)和UAVScenes(62.76% mIoU)三个多模态数据集上均优于现有融合方法。

Conclusion: 多级融合与双向交互策略在多样化复杂城市场景的语义分割任务中比现有方法更可靠、更准确。

与遥感交叉或改进方案: 交叉/改进可行性：95%。该论文本身就是遥感领域的点云语义分割方法。改进方向：(1)可引入多尺度高程引导模块以更好适应地形起伏剧烈的山区场景；(2)可扩展至多时相遥感数据融合，利用时序信息增强分割稳定性；(3)可结合全局上下文建模（如大核卷积或Mamba架构）提升大范围场景中长距离依赖的捕获能力；(4)针对高分辨率遥感影像，可设计轻量化版本以降低计算开销并适配星载平台。

Abstract (英文): Multimodal fusion unlocks the potential of point cloud semantic segmentation, thereby driving advancements in surface observation and visual perception tasks. Although light detection and ranging (LiDAR) systems capture precise 3D structural geometry and optical images provide rich semantic and textural information, existing fusion methods struggle with limited cross-modal perception and insufficient information complementarity. To address these limitations, we propose a multi-stage LiDAR-image collaborative perception fusion network (MCPFNet) for point cloud semantic segmentation of urban scenes. At the middle fusion stage, the network incorporates an elevation-guided geometric-aware fusion module and a semantic-aware cross-attention fusion module to enable bidirectional feature injection between LiDAR and image modalities. In the late fusion stage, a bidirectional adaptive fusion module further refines semantic representations through gated weighting and bidirectional cross-attention mechanisms. Extensive experiments on three multimodal datasets with different resolutions, i.e., ISPRS Vaihingen, N3C-California, and UAVScenes, demonstrate that MCPFNet outperforms existing fusion methods, achieving mIoUs of 74.51%, 95.15%, and 62.76%, respectively. Hence, our multi-stage fusion and bidirectional interaction strategy is more reliable and accurate than existing methods in performing segmentation across diverse and complex urban scenes.

</details>


### [189] [Impervious Surface Area Changes and Urban Expansion in China’s Three Major Urban Agglomerations over the Past 40 Years](https://doi.org/10.3390/rs18132236)
*Fei Wang,Yi Yang,Lei Yu,Fengyun Sang,Yaning Chen,Siquan Yang*

Main category: Remote Sensing

TL;DR: 本文利用不透水面扩展指标和位置中心性指数，分析了1985–2024年中国三大城市群的不透水面面积时空动态及五大城市的扩张模式，发现不透水面面积大幅增长但增速放缓，各城市群扩张高峰期存在显著差异，邻近扩张已成为主要扩张类型。


<details>
  <summary>Details</summary>
Motivation: 分析中国三大城市群的不透水面面积时空动态及代表性城市的扩张模式，为制定因地制宜的城市规划政策、增强城市发展韧性提供科学依据。

Method: 采用不透水面扩展指标（扩展速率与强度）揭示时空动态，应用位置中心性指数（LCI）研究五大超大城市的扩张结构与模式。

Result: 1985–2024年中国三大城市群不透水面面积从22,544.10 km²增至69,348.59 km²；扩展速率与强度均呈现明显减缓趋势；粤港澳大湾区在1985–1995年达峰，长三角与京津冀在2005–2015年达峰；五座超大城市中邻近扩张类型占比超过50%，成为主导扩张形式。

Conclusion: 研究增强了对城市群不透水面演化轨迹及超大城市扩张模式的理解，为可持续城市规划和城市韧性提供了科学依据。

与遥感交叉或改进方案: 交叉/改进可行性：90%。该论文本身属于遥感应用（不透水面提取与时空分析）的典型研究。改进方向：1) 可引入多源遥感数据（如Sentinel-1 SAR与光学影像融合）提升不透水面制图精度；2) 结合夜间灯光数据（NPP-VIIRS）进一步验证城市化强度；3) 可引入深度学习语义分割方法（如U-Net、DeepLab V3+）替代传统分类方法以提高不透水面提取的自动化水平；4) 可扩展至全国尺度或加入更多中小城市对比分析以增强结论普适性。

Abstract (英文): Analyzing the impervious surface area (ISA) of China’s three major urban agglomerations and expansion pattern of representative cities is of great significance for formulating context-specific urban planning policies and enhancing the resilience of urban development. This research employs impervious surface expansion indicators to reveal the spatiotemporal dynamics of impervious surface area, and further applies the location centrality index (LCI) to investigate the expansion structures and patterns of five megacities. The results indicate that the ISA of China’s three major urban agglomerations expanded from 22,544.10 km2 to 69,348.59 km2 from 1985 to 2024; both the expansion rate and intensity have exhibited a distinct decelerating trend. However, significant regional variation exists in the peak expansion phases: the Guangdong–Hong Kong–Macao Greater Bay Area Urban Agglomeration (GBA) peaked during the Initial Exploratory Phase (1985–1995), whereas the Yangtze River Delta Urban Agglomeration (YRD) and Beijing–Tianjin–Hebei Urban Agglomeration (BTH) reached their peaks during the High-speed Development Phase (2005–2015). At the urban scale, LCI-based analysis of urban expansion across five megacities reveals that adjacency expansion type has become the predominant form of growth, exceeding 50% in most cities. These findings enhance our understanding of evolutionary trajectories of ISA within urban agglomerations and the expansion patterns of five megacities, providing a scientific basis for sustainable urban planning and urban resilience.

</details>


### [190] [MSIA-YOLO: A Multi-Scale Semantic Interaction and Alignment Network for Small Object Detection in Low-Altitude UAV Remote Sensing Images](https://doi.org/10.3390/rs18132210)
*Wen Zhang,Xiaorong Xue,Bingyan Lu,Yishuo Tian,Jingtong Yang,Xin Zhao,W X Wang*

Main category: Remote Sensing

TL;DR: MSIA-YOLO是一种基于YOLOv11的小目标检测器，通过多尺度语义交互与对齐机制，从特征建模、高分辨率语义补偿和任务协同对齐三个维度优化小目标检测性能。在VisDrone、TinyPerson和RSOD数据集上，mAP50分别提升7.7%、10.3%和1.0%，优于多种先进检测器。


<details>
  <summary>Details</summary>
Motivation: 小目标检测受限于缺乏判别性细粒度特征；引入更高分辨率检测尺度虽能提升性能，但会放大背景噪声；传统检测头独立解耦的设计不足以解决小目标的漏检和误检问题。

Method: 1) 在主干网络中集成感受野注意力卷积(RFAConv)，通过感受野感知注意力增强边缘和纹理等关键局部细节；2) 构建CHSP-P2小目标检测框架，添加P2分支并引入尺度序列融合机制，通过跨尺度混合输入实现高分辨率语义补偿；3) 设计DTIA-Head（动态任务交互与对齐头），通过动态任务交互和空间对齐促进分类与定位的联合优化。

Result: 在VisDrone、TinyPerson和RSOD公开数据集上，相比YOLOv11n基线，mAP50分别提升7.7%、10.3%和1.0%，且优于多种先进检测器。

Conclusion: MSIA-YOLO在小目标、密集目标和复杂场景目标检测中表现出良好的有效性和泛化能力。

与遥感交叉或改进方案: 交叉/改进可行性：95%。该论文已在遥感数据集RSOD上验证了有效性，可直接应用于遥感小目标检测任务。改进方向：1) 针对遥感图像中目标尺度差异极大的特点，可进一步引入多尺度自适应特征融合策略；2) 遥感图像中背景噪声复杂（如云雾、阴影），可在RFAConv中增强对干扰因素的抑制能力；3) DTIA-Head可扩展为多任务对齐机制，融合旋转框检测以适配遥感目标方向多样性。

Abstract (英文): Small object detection is fundamentally constrained by the lack of discriminative fine-grained features. Although introducing higher resolution detection scales can improve performance, it also amplifies background noise. In addition, the independently decoupled design of conventional detection heads is insufficient to address the persistent challenges of missed detections and false positives for small objects. To this end, we propose MSIA-YOLO, a YOLOv11-based detector with multi-scale semantic interaction and alignment, optimized from three complementary perspectives: feature modeling, high resolution semantic compensation, and task coordinated alignment. First, Receptive Field Attention Convolution (RFAConv) is integrated into the backbone to enhance critical local details, such as edge and texture cues, via receptive field aware attention. Second, to alleviate fine detail attenuation caused by repeated downsampling, we construct a CHSP-P2 small object detection framework with an additional P2 branch. A scale sequence fusion mechanism is further introduced to perform high resolution semantic compensation through cross scale hybrid inputs. Finally, we design a DTIA-Head (Dynamic Task Interaction and Alignment Head), which promotes joint optimization of classification and localization through dynamic task interaction and spatial alignment. Extensive experiments on the public datasets VisDrone, TinyPerson, and RSOD show that, compared with the YOLOv11n baseline, MSIA-YOLO improves mAP50 by 7.7%, 10.3%, and 1.0%, respectively, while also outperforming several advanced detectors. These results demonstrate the effectiveness and generalization capability of the proposed method in small object, dense object, and complex scene object detection scenarios.

</details>


### [191] [Simulation of BepiColombo’s Gravity Investigation with Dynamical Mismodeling](https://doi.org/10.3390/rs18132253)
*Ariele Zurria,Ivan di Stefano,Paolo Cappuccio,Umberto De Filippis,L. Iess*

Main category: Remote Sensing

TL;DR: 本文评估了BepiColombo水星探测任务中MORE无线电科学实验的预期性能，通过数值模拟分析其在高精度Ka波段跟踪系统下对水星重力场、Love数k2和自转状态参数的测定精度，特别考虑了动力学模型误差的影响。


<details>
  <summary>Details</summary>
Motivation: BepiColombo任务搭载先进的Ka波段多频微波跟踪系统（MORE），有望超越MESSENGER任务对水星内部结构的认知。在任务正式进入科学运行阶段前，需要定量评估该实验在更真实条件（含动力学模型误差）下的参数测定性能，为后续数据处理提供理论依据。

Method: 采用数值协方差分析方法，与以往研究不同之处在于本文在分析中引入了动力学模型误差，以评估在受控的模型失配条件下实验的性能表现。模拟覆盖BepiColombo为期两年的扩展轨道阶段，科学运行计划于2027年开始。

Result: 给出了BepiColombo改进的无线电跟踪数据如何降低水星重力场、Love数k2和自转状态参数的不确定度的定量评估结果。

Conclusion: MORE实验将借助先进的Ka波段无线电跟踪系统，显著提升对水星内部结构（重力场、弹性响应Love数k2、自转状态）的测定精度，为水星大地测量和基础物理学研究提供新见解。

与遥感交叉或改进方案: 交叉/改进可行性：75%。该文为行星无线电科学实验的性能评估，与遥感交叉思路如下：(1) 多频微波跟踪系统中的大气/电离层校正技术可迁移至地球遥感卫星的精密轨道定轨和重力场反演；(2) Ka波段高精度测距测速方案可应用于遥感卫星编队飞行中的星间链路设计；(3) 研究中的动力学模型误差处理策略可为地球重力卫星（如GRACE Follow-On）的数据处理提供参考。

Abstract (英文): The BepiColombo spacecraft, designed by ESA and JAXA, is currently in its cruise phase toward Mercury. Among the scientific investigations is the Mercury Orbiter Radio-science Experiment (MORE), which will exploit a multi-frequency microwave tracking system with an advanced Ka-band transponder to achieve its objectives pertaining to Mercury’s geodesy and fundamental physics. Leveraging precise measurements from this state-of-the-art radio tracking system, MORE is expected to provide new insights into Mercury’s interior, refining and expanding upon the findings of the MESSENGER mission. This work evaluates the performance of MORE’s gravity and rotation experiment, specifically assessing how BepiColombo’s improved radio tracking data can reduce uncertainties in the determination of Mercury’s gravity field, Love number k2, and rotational state. Differently from previous covariance analyses, this work includes errors in the dynamical model to assess the experiment’s performance under controlled mismodeling conditions. We present the results of a numerical simulation covering BepiColombo’s extended two-year orbital phase, with scientific operations set to begin in 2027.

</details>


### [192] [Realistic Error Budget and Cross-Validation of Sentinel-1 3D Displacement for the 2025 Dingri Earthquake](https://doi.org/10.3390/rs18132252)
*Zhangdi Xie*

Main category: Remote Sensing

TL;DR: 本文对2025年定日地震(Mw 7.1)的双轨道InSAR三维形变场进行了系统的不确定性量化。利用Sentinel-1升降轨数据通过最小二乘反演构建三维同震位移场，发现地震产生南北走向正断层破裂，垂直分量最大沉降-403.3 mm、最大抬升+621.1 mm，东西分量最大西向位移-592.3 mm、最大东向位移+332.1 mm。不确定性分析显示，形式误差传播给出的1σ不确定性仅为1.09 mm(垂直)和1.38 mm(东西)，而基于蒙特卡洛模拟的实际误差约为13.8 mm(垂直)和17.2 mm(东西)，系统误差贡献远大于随机噪声。交叉验证支持上述评估。研究表明形式误差传播严重低估了双轨道InSAR反演的实际不确定性。


<details>
  <summary>Details</summary>
Motivation: 双轨道InSAR三维形变反演中，形式误差传播方法可能严重低估实际不确定性，而现有研究对此缺乏系统的量化评估。本文旨在揭示形式误差与实际精度之间的差异，并量化各类误差源（系统误差与随机噪声）的贡献。

Method: 采用Sentinel-1升降轨数据，通过最小二乘反演构建三维同震位移场；使用形式误差传播计算理论1σ不确定性；采用蒙特卡洛模拟估算实际误差预算；通过与独立Sentinel-1处理链交叉验证进行误差评估。

Result: 垂直分量最大沉降-403.3 mm、抬升+621.1 mm；东西分量最大西移-592.3 mm、东移+332.1 mm。形式误差传播1σ不确定性为1.09 mm(垂直)和1.38 mm(东西)，而蒙特卡洛模拟的实际误差为13.8 mm(垂直)和17.2 mm(东西)。交叉验证中，升降轨LOS位移R分别为0.88和0.62；三维分量中垂直R=0.88，东西R=0.59。

Conclusion: 形式误差传播严重低估双轨道InSAR三维形变反演的实际不确定性；系统误差源的贡献远大于随机噪声；东西分量对处理策略和观测几何更为敏感，是误差的主要来源之一。

与遥感交叉或改进方案: 交叉/改进可行性：95%。改进方案：(1) 将本文的不确定性量化框架引入其他InSAR形变监测场景（如滑坡、冰川运动、地面沉降），建立更可靠的误差预算模型；(2) 可将蒙特卡洛模拟与系统误差解耦方法扩展至多轨道（三轨及以上）InSAR三维反演中；(3) 针对东西分量敏感性问题，可结合多孔径干涉测量(MAI)或偏移量追踪技术补充约束，降低对观测几何的依赖；(4) 可引入贝叶斯反演框架替代最小二乘，实现不确定性的概率化表达。

Abstract (英文): This study presents a systematic uncertainty quantification of the two-track InSAR three-dimensional (3D) deformation field of the 2025 Dingri earthquake (Mw 7.1). Using Sentinel-1 ascending and descending track data, a 3D coseismic displacement field was constructed via least-squares inversion. The results revealed that the earthquake produced a north–south-striking normal fault rupture, with the vertical component reaching a maximum subsidence of −403.3 mm and a maximum uplift of +621.1 mm, and the east–west component reaching a maximum westward displacement of −592.3 mm and an eastward displacement of +332.1 mm. Uncertainty analysis reveals a divergence between formal errors and actual accuracy: formal error propagation yields 1σ uncertainties of 1.09 mm and 1.38 mm for the vertical and east–west components, respectively; a realistic error budget based on Monte Carlo simulations indicates that the actual errors are approximately 13.8 mm for the vertical component and 17.2 mm for the east–west component, with systematic error contributions far exceeding random noise. Cross-validation against an independent Sentinel-1 processing chain supports the above error assessment: the correlation coefficient R for ascending track line-of-sight (LOS) displacement is 0.88, whereas it is 0.62 for the descending track; for the three-dimensional components, R reaches 0.88 for the vertical component and 0.59 for the east–west component, with discrepancies arising primarily from the greater sensitivity of the east–west component to processing strategies and observation geometry. This study demonstrates that formal error propagation underestimates the actual uncertainty of two-track InSAR inversion and that systematic error sources contribute far more than random noise does.

</details>


### [193] [Physically Consistent SAR Image Generation for Unseen Aspect Angles via Attributed Scattering Center Evolution](https://doi.org/10.3390/rs18132247)
*Zihao Jiang,Chao Liu,Zhangzeyu Xing,Yamei Wang,Shuwen Xiao*

Main category: Remote Sensing

TL;DR: 针对SAR目标图像在未见方位角下因数据稀疏导致的散射结构失真和背景统计不匹配问题，提出了一种物理一致性框架，利用ASC启发的稀疏散射结构先验和双一致性方案，实现高保真的跨角度SAR图像生成。


<details>
  <summary>Details</summary>
Motivation: SAR目标图像对方位角高度敏感，实际数据采集通常仅提供有限视角下的稀疏观测，导致在未见方位角下数据严重匮乏。现有方法要么仅关注分布匹配而未充分利用散射结构线索，要么强调角度条件而未能同时保持物理上合理的散射响应变化和真实的背景散斑统计特性。

Method: 提出了一种物理一致的SAR图像生成框架：(1) 引入ASC启发的稀疏散射结构先验，近似SAR图像平面中的主导散射响应，作为可微分的角度感知结构代理引导生成器；(2) 提出双一致性方案，联合强制目标区域散射一致性和背景区域统计一致性，提升生成结果的物理真实性。

Result: 在严格的未见角度插值和保留测试协议下，该方法在图像保真度、目标区域散射一致性、背景统计一致性和角度条件一致性方面持续优于代表性基线方法。

Conclusion: ASC启发的稀疏散射结构先验在物理一致的SAR视角补全中发挥关键作用，所提方法有效解决了跨角度SAR图像生成中散射结构失真和背景统计不匹配的难题。

与遥感交叉或改进方案: 交叉/改进可行性：95%。该论文本身就是遥感SAR图像生成领域的核心研究，改进方向包括：(1) 将ASC先验扩展至极化SAR（PolSAR）数据，处理多极化通道下的散射结构一致性；(2) 引入物理引导的扩散模型替代当前生成器，进一步提升生成图像的高频细节保真度；(3) 将该框架迁移至InSAR干涉相位生成任务，在未知视角下保持相位一致性。

Abstract (英文): Synthetic aperture radar (SAR) target images are highly sensitive to aspect angle, while practical data acquisition usually provides only sparse observations over limited viewpoints. This leads to severe data scarcity at unseen aspect angles and makes cross-angle generation prone to scattering-structure distortion and background statistical mismatch. Existing SAR image generation methods either focus on distribution matching without sufficiently exploiting scattering-related structural cues, or emphasize angle conditioning while failing to jointly preserve physically plausible dominant scattering-response variations and realistic background speckle statistics at unseen aspect angles. To address this issue, we propose a physically consistent framework for SAR image generation at unseen aspect angles. The proposed method introduces an ASC-inspired sparse scattering-structure prior to approximate the dominant scattering responses in the SAR image plane. Rather than performing full parametric ASC inversion, this prior serves as a differentiable and angle-aware structural proxy that guides the generator toward synthesizing SAR images with structurally plausible scattering layouts. In addition, a dual-consistency scheme is introduced to jointly enforce target-region scattering consistency and background-region statistical consistency, thereby improving the physical realism of the generated results in both the target and background regions. Extensive experiments under strict unseen-angle interpolation and hold-out protocols demonstrate that the proposed method consistently outperforms representative baselines in image fidelity, target-region scattering consistency, background statistical consistency, and angle-conditioned consistency. Further visualization and ablation studies verify the critical role of the ASC-inspired sparse scattering-structure prior in physically consistent SAR view completion.

</details>


### [194] [Mapping Evapotranspiration Patterns in the Desert-Oasis Ecotone Using UAV-Based Thermal Infrared Imagery with a Three-Temperature Model](https://doi.org/10.3390/rs18132242)
*Siying Li,Yuhua Xing,Dapeng Zhang,Pei Wang*

Main category: Remote Sensing

TL;DR: 本研究通过将无人机热红外遥感与三温度（3T）模型相结合，在河西走廊荒漠-绿洲交错带实现了高分辨率蒸散发（ET）估算，有效捕捉了从农田到荒漠的ET空间梯度，空间验证R²达0.90-0.95，点尺度验证MAE为0.38 mm d⁻¹。


<details>
  <summary>Details</summary>
Motivation: 荒漠-绿洲交错带蒸散发估算面临气象观测稀疏和卫星遥感空间分辨率粗的挑战，难以解析高度异质的地表条件。

Method: 利用无人机热红外遥感获取米级地表温度（LST），结合气象和植被数据驱动三温度（3T）模型，生成高分辨率ET图；空间上以SEBAL模型验证，点尺度上以双源模型验证。

Result: (1) 3T模型有效捕捉了ET从农田（3-10.69 mm d⁻¹）经防护林（3-6 mm d⁻¹）到荒漠（<3 mm d⁻¹）的递减梯度；(2) 空间验证R²=0.90-0.95，RMSE=0.22-0.43 mm d⁻¹，农田精度最高；(3) 点尺度验证MAE=0.38 mm d⁻¹。

Conclusion: 无人机热红外数据与3T模型的结合在复杂生态过渡带高分辨率ET模拟中效果显著，为干旱区生态水文监测和水资源评估提供了有前景的技术手段。

与遥感交叉或改进方案: 交叉/改进可行性：85%。改进方案：(1) 可将3T模型中的辐射参数化方案替换为更适用于干旱区的改进方案（如考虑沙尘气溶胶影响）；(2) 引入多光谱或高光谱UAV数据以改进植被参数反演；(3) 结合深度学习模型（如CNN或LSTM）对3T模型输出的ET时空分布进行后处理校正；(4) 将UAV-3T框架扩展至其他干旱区典型景观（如盐碱地、戈壁），进一步验证普适性。

Abstract (英文): Evapotranspiration (ET) estimation in desert-oasis ecotones remains challenging due to sparse meteorological observations and the coarse spatial resolution of satellite remote sensing, which limit the ability to resolve highly heterogeneous surface conditions. To address this issue, this study develops a high-resolution ET estimation framework by integrating unmanned aerial vehicle (UAV)-based thermal infrared remote sensing with a three-temperature (3T) model in the Hexi Corridor. UAV-derived land surface temperature (LST) at meter-scale resolution, together with meteorological and vegetation data, was used to drive the model and generate high-resolution ET maps. The model’s performance was validated spatially against the Surface Energy Balance Algorithm for Land (SEBAL) model and at the point-scale against a two-source model. The results show that: (1) The 3T model effectively captured the spatial gradient of decreasing ET from cropland (3–10.69 mm d−1), through shelterbelts (3–6 mm d−1), to desert areas (&lt;3 mm d−1). (2) Spatial validation against the SEBAL model was conducted using stratified pixel-wise comparisons across four land-cover types over 14 UAV transects, showing strong agreement (R2 = 0.90–0.95; RMSE = 0.22–0.43 mm d−1). The model achieved highest accuracy in cropland (R2 = 0.92; RMSE = 0.24 mm d−1), with slight overestimation in shelterbelts. (3) Point-scale validation against the two-source model yielded an MAE of 0.38 mm d−1. This study demonstrates the effectiveness of combining UAV thermal infrared data with the 3T model for high-resolution ET simulation in complex ecological transition zones, offering a promising technical approach for ecohydrological monitoring and water resource assessment in arid regions.

</details>


### [195] [Multi-Scale Assessment of Nighttime Heat Health Risk and Dominant Factors Using MODIS and SDGSAT-1 Observations](https://doi.org/10.3390/rs18132243)
*Zhuang Tan,Qixia Man,Baolei Zhang,Pinliang Dong,Haiying Sun,Zhongchang Sun,Linlin Lu,Changyong Dou,Xiaomin Yang,Changyin Han,Cong Zhou,Xia Sun,Yi-Xiang Wang,Z G Li*

Main category: Remote Sensing

TL;DR: 本研究构建了一个基于危害-暴露-脆弱性的多尺度框架，用于评估山东省夜间热健康风险。宏观尺度使用MODIS夜间地表温度（1km）和区级统计数据，精细尺度使用SDGSAT-1热红外数据（30m）和街区级数据，并结合局部气候分区（LCZ）框架识别主导风险因子。结果表明：高风险区集中在鲁南和城市核心区；内陆城市风险高于沿海城市；高等级风险单元的主导因子配置更复杂；紧凑高层/中层区（LCZ 1-2）以多主导因子为特征，开敞低层/稀疏建成区（LCZ 6/9）以无主导因子为特征。


<details>
  <summary>Details</summary>
Motivation: 现有热健康风险评估存在三个不足：(1)宏观与精细尺度评估缺乏有效衔接；(2)夜间热健康风险关注不足；(3)在局部气候分区（LCZ）框架下的主导因子识别不充分。因此需要开发一个多尺度评估框架来填补这些研究空白。

Method: 采用危害-暴露-脆弱性（Hazard-Exposure-Vulnerability）框架，宏观尺度利用MODIS夜间地表温度（1km分辨率）表征热危害，结合区级社会经济统计数据评估风险，并通过热点分析识别高风险集群以选取城市进行精细评估；精细尺度利用SDGSAT-1热红外数据（30m分辨率）表征城市内部夜间热危害，结合街区级社会经济和城市基础设施数据评估风险，并识别各空间单元的主导风险因子。

Result: (1)宏观尺度上，夜间热健康高风险区集中在鲁南和主要城市核心区；(2)精细尺度上，内陆城市总体风险高于沿海城市，且内陆城市空间格局相似；(3)在区和街区单元上，高风险等级与更复杂的主导因子配置相关；(4)紧凑高层/中层区（LCZ 1-2）以多主导因子为特征，开敞低层/稀疏建成区（LCZ 6/9）以无主导因子为特征。

Conclusion: 本研究提供了一个多层次的热风险治理视角，为宏观政策制定和精细干预提供了科学依据，有助于建设更可持续和韧性的城市。

与遥感交叉或改进方案: 交叉/改进可行性：95%。该研究本身即为遥感与城市热健康风险评估的交叉研究，可从以下方面改进：(1)引入更高时空分辨率的遥感数据（如ECOSTRESS、Landsat）与MODIS、SDGSAT-1融合，弥补数据时空覆盖的不足；(2)增加遥感反演的植被指数（NDVI/SAVI）、不透水面比例、建筑形态参数等作为精细尺度的暴露和脆弱性指标，减少对统计数据的依赖；(3)将LCZ分类与遥感反演的地表参数（如反照率、蒸散发）结合，建立更全面的主导因子识别体系。

Abstract (英文): Climate change is amplifying heat-related health risks, making heat health risk assessment increasingly important for sustainable urban development. However, current studies still face three challenges: (1) macro-scale and fine-scale assessments remain weakly linked; (2) nighttime heat health risk has received limited attention; and (3) dominant factor identification remains insufficient, especially within the Local Climate Zone (LCZ) framework. To address these gaps, this study developed a hazard–exposure–vulnerability-based, multi-scale framework for assessing nighttime heat health risk in Shandong Province, China. At the macro scale, MODIS nighttime land surface temperature (1 km) was used to characterize heat hazard, and district-level risk was assessed by integrating socio-statistical data. Hotspot analysis was then applied to identify high-risk clusters and select cities for fine-scale assessment. At the fine scale, SDGSAT-1 thermal infrared data (30 m) were used to characterize intra-urban nighttime heat hazard, and block-level risk was assessed by integrating socioeconomic and urban infrastructure data. Dominant risk factors were further identified for each spatial unit. Results show that (1) at the macro scale, high nighttime heat health risk districts are concentrated mainly in southern Shandong and major urban cores; (2) at the fine scale, overall risk is higher in inland cities than in the coastal city, and the inland cities show similar spatial patterns; (3) at both district and block units, higher risk levels are associated with more complex dominant factor configurations; and (4) compact high- and mid-rise zones (LCZ 1–2) are mainly characterized by multiple dominant factors, whereas open low-rise/sparsely built zones (LCZ 6/9) are mainly characterized by no dominant factor. This study provides a multi-level perspective for heat-risk governance, offers a scientific basis for both macro-scale policy formulation and fine-scale intervention, and contributes to more sustainable and resilient cities.

</details>


### [196] [Estimation of High-Resolution Multi-Layer Soil Moisture Using Land Data Assimilation and the Three-Cornered Hat Method](https://doi.org/10.3390/rs18132248)
*Xinlei He,Wenbin Zhu,Shaomin Liu,Tongren Xu,Zhitao Wu,Sayed M. Bateni,Zhen Hao,Xiang Li,Dongxin Wu,Hanxue Liang*

Main category: Remote Sensing

TL;DR: 本文通过融合多源遥感数据（SMAP、HJ-2、Sentinel-2、Gaofen-6）和ESTARFM模型生成16米分辨率土壤湿度数据集（HRSM），并利用EnKF数据同化框架将其同化进Noah-MP陆面模型，结合三顶帽方法估计的不确定性，在云南中部春旱期验证了该方法相较于同化SMAP数据能显著提升表层和根区土壤湿度估计精度及空间异质性。


<details>
  <summary>Details</summary>
Motivation: 现有土壤湿度产品空间分辨率不足，难以满足精细农业和干旱监测需求，亟需发展高分辨率土壤湿度数据同化方法以提升模型模拟精度和空间细节。

Method: 1) 融合SMAP、HJ-2、Sentinel-2、Gaofen-6多源遥感数据，利用ESTARFM模型生成16米HRSM；2) 采用三顶帽方法估计HRSM相对不确定性；3) 在EnKF框架中，用HRSM及其不确定性更新观测误差和卡尔曼增益，以1公里分辨率同化进Noah-MP模型；4) 输出16米分辨率土壤湿度廓线。

Result: DA_HRSM方法显著改善了Noah-MP模型表层和根区土壤湿度估计，相对不确定性更低；相比同化SMAP数据，提供了更高分辨率的空间特征，增强了20个灌区的空间异质性，且与PDSI干旱指数吻合良好。

Conclusion: 高分辨率土壤湿度数据同化在农业和干旱监测系统中具有显著优势，能有效捕捉春季干旱事件并提供更精细的空间信息。

与遥感交叉或改进方案: 交叉/改进可行性：95%。改进方案：1) 可引入更多国产高分辨率卫星（如高分七号、资源系列）替代或补充HJ-2/Gaofen-6数据源；2) 可尝试深度学习超分网络替代ESTARFM融合模型，进一步提升空间细节保持能力；3) 可将三顶帽方法与贝叶斯不确定性框架结合，实现自适应误差估计；4) 可扩展至多尺度EnKF（如双尺度同化），同时保持计算效率与空间细节；5) 可引入遥感蒸散发或植被指数作为额外同化变量，提升干旱早期检测能力。

Abstract (英文): Soil moisture (SM) plays a pivotal role in regulating terrestrial energy-water exchanges and exerts substantial influence on agricultural productivity. In this study, a high-resolution soil moisture (HRSM) dataset (16 m) was generated by integrating multi-source remote sensing data from SMAP, HJ-2, Sentinel-2, and Gaofen-6, together with the Enhanced Spatial and Temporal Adaptive Reflectance Fusion Model. The data assimilation (DA) method was implemented for assimilating HRSM within the Ensemble Kalman Filter (EnKF) framework using the Noah-MP model at a spatial resolution of 1 km. To enhance the spatial detail of SM, HRSM and its relative uncertainties derived from the three-cornered hat (TCH) method were used to update the observation error and Kalman gain in the EnKF framework, thereby improving SM profile estimates at a 16 m resolution. The performance of the DA method was evaluated against in situ measurements during the spring drought period in central Yunnan Province, China. The results show that assimilating HRSM (DA_HRSM) significantly improves surface and root-zone SM estimates in the Noah-MP model. The simulated SM from the DA_HRSM method demonstrates lower relative uncertainty. Compared to the assimilation of SMAP SM, the DA_HRSM method provides higher-resolution spatial features of SM and enhances spatial heterogeneity across 20 irrigation districts. The DA_HRSM method effectively captured the spring drought in central Yunnan, demonstrating good agreement with the Palmer Drought Severity Index (PDSI). The result highlights the advantages of incorporating high-resolution SM data into agricultural and drought monitoring systems.

</details>


### [197] [Reconstruction of Land Surface Temperature Based on EATC Constraints and Spatially Adaptive Residual Correction: A Case Study of the Qinghai–Tibet Engineering Corridor](https://doi.org/10.3390/rs18132254)
*Minghan Xu,Qian Li,Shufang Tian,Shiqi Kuang,Tianqi Li*

Main category: Remote Sensing

TL;DR: 提出一种物理约束的空间残差学习框架（PCSRL），结合EATC时间基线模型和G-XGBoost空间非线性残差模型，用于高精度重建青藏高原复杂地形区域的卫星地表温度缺失数据。


<details>
  <summary>Details</summary>
Motivation: 现有LST重建方法在处理复杂地形大面积数据缺失时精度有限：时空插值方法精度不足，物理模型缺乏气象输入，数据驱动方法忽略局地空间变异导致热模式平滑和系统误差。

Method: 使用增强型年温度周期（EATC）模型捕捉LST时间基线，然后集成多源辅助数据到地理加权XGBoost（G-XGBoost）算法中建模空间非线性残差，形成物理-数据混合框架。

Result: 在2017年青藏工程走廊MODIS LST数据集上，R²=0.88，RMSE=1.92K，平均偏差0.07K；冬季表现最佳（RMSE=1.19K）；通过自适应局部带宽和权重调整有效减少边界伪影，恢复复杂地形热空间模式。

Conclusion: 该方法为异质性高山地表的高精度LST重建提供了一种可靠途径，优于单一物理模型和全局机器学习模型。

与遥感交叉或改进方案: 交叉/改进可行性：95%。该框架可直接应用于遥感LST产品（如MODIS、Landsat、VIIRS）的云覆盖缺失重建，未来改进方向：（1）引入多时相Sentinel-3 SLSTR数据提高时间分辨率；（2）结合数字高程模型（DEM）和局地地形因子优化G-XGBoost的空间权重函数；（3）扩展到其他遥感变量（如NDVI、反照率）的时空重建任务。

Abstract (英文): Satellite-based land surface temperature (LST) products are frequently affected by cloud cover and atmospheric conditions, resulting in missing data that significantly limits the continuous monitoring of the thermal environment in complex terrains, such as the Tibetan Plateau. Existing spatiotemporal interpolation methods face clear accuracy limitations when addressing extensive data gaps, while physical models often struggle due to insufficient meteorological inputs in complex landscapes. Moreover, conventional data-driven approaches usually overlook local spatial variations, resulting in smoothed thermal patterns and systematic errors. To overcome these issues, we propose a Physically Constrained Spatial Residual Learning framework. In this framework, we use the Enhanced Annual Temperature Cycle (EATC) model to capture the temporal baseline of LST first. Then, we integrate multi-source auxiliary data into the Geographical-XGBoost (G-XGBoost) algorithm to model spatial nonlinear residuals. Using simulated cloud masks on the 2017 MODIS LST dataset from the Qinghai–Tibet Engineering Corridor, we show that the hybrid model outperforms both individual physical models and global machine learning models in accuracy and spatial detail recovery. Validation results yield an R2 of 0.88, an RMSE of 1.92 K, and a mean bias of 0.07 K. Seasonal evaluations indicate best performance in winter (RMSE = 1.19 K) with robust performance in summer. Furthermore, the framework reduces boundary artifacts and accurately reproduces thermal spatial patterns in complex terrain through adaptive local bandwidth and weight adjustments. This approach provides a reliable method for high-precision LST reconstruction over heterogeneous alpine surfaces.

</details>


### [198] [Filling Satellite Microwave Observation Gaps via Generative Synthesis](https://doi.org/10.3390/rs18132256)
*Han Du,Baoxiang Pan,Fan Ping,J Q Xu,Congyi Nai,Sencan Sun,Jie Chao,Jingnan Wang,Shangshang Yang,Xi Chen,Jingyuan Li,J. Mao,Lei Yin,Yupeng Li,Ziniu Xiao*

Main category: Remote Sensing

TL;DR: MIDAS是一种基于扩散模型的概率框架，利用10分钟间隔的地球静止轨道红外观测估计微波亮度温度场，在183 GHz湿度探测通道上平均绝对误差1.15 K，优于U-Net基线（1.43 K），并通过融合采样机制将实时极轨观测作为物理约束，将集合RMSE降低20%以上，预报实验表明可将热带气旋路径误差从约110 km降至40 km。


<details>
  <summary>Details</summary>
Motivation: 极轨微波辐射计提供全天候大气状态测量，但重访间隔长达数小时，在快速演变的天气事件中存在关键观测空白。为弥补这一不足，需要从高频次（10分钟间隔）的地球静止轨道红外观测中估计微波亮度温度场。

Method: 基于扩散模型的概率框架MIDAS，从GOES红外观测输入估计全圆盘域上的微波亮温场。核心创新包括：(1) 采用条件扩散模型生成概率性亮温场；(2) 合并采样机制将实时极轨观测作为物理约束融入生成过程；(3) 集成RTTOV-SCATT辐射传输模拟作为参考。

Result: 在MWHS-2五个湿度探测通道上，相对误差低于0.5%，通道平均MAE为1.15 K（U-Net基线1.43 K）；集合RMSE降低20%以上，概率校准改进超过30%；热带气旋路径误差从约110 km降至40 km，极端降水预报优于直接红外同化。

Conclusion: 生成模型能够有效补充稀疏的极轨微波观测覆盖，为下游应用提供物理上合理的微波湿度场，在数值天气预报中具有与真实卫星观测相当的预报改进效果。

与遥感交叉或改进方案: 交叉/改进可行性：95%。该框架可直接应用于遥感领域：1) 利用中国FY-4系列卫星的红外数据与FY-3系列MWHS-2数据构建类似系统，实现国产卫星的微波-红外跨传感器亮温估计；2) 将方法扩展至其他微波通道（如低频温度探测通道、降水探测通道）；3) 可推广至其他遥感数据缺失场景，如SAR与光学影像的时空降尺度、云覆盖区域的地表参数重建；4) 合并采样机制可推广至多源遥感数据融合，将不同重访周期的卫星数据作为物理约束提升重建精度。

Abstract (英文): Polar-orbiting microwave radiometers provide indispensable all-weather measurements of the atmospheric state, yet revisit intervals of many hours leave critical gaps during rapidly evolving weather events. To address this limitation, we developed MIDAS (Microwave Inference via Diffusion Across Satellites), a probabilistic framework that estimates microwave brightness temperature (BT) fields across the geostationary full-disk domain from infrared observations at 10 min intervals. This study focuses on the five Microwave Humidity Sounder-2 (MWHS-2) humidity-sounding channels near 183 GHz, which provide vertically resolved water vapor information. MIDAS achieves relative errors below 0.5% for the majority of cases, with a channel-averaged mean absolute error of 1.15 K, outperforming a deterministic U-Net baseline (1.43 K). Beyond per-sample evaluation, MIDAS reproduces large-scale climatological patterns across the full-disk domain over a three-month summer period, consistent with Radiative Transfer for TOVS–Scattering (RTTOV-SCATT) simulations. In deep convective scenes where reconstruction is most difficult, the ensemble spread naturally tracks reconstruction difficulty, providing a built-in indicator of prediction confidence. Notably, MIDAS incorporates real-time polar-orbiting observations as physical constraints via a merge-sampling mechanism, reducing ensemble RMSE by over 20% and improving probabilistic calibration by more than 30%. Proof-of-concept assimilation experiments for two high-impact weather cases show that MIDAS-generated fields yield forecast improvements comparable to those from real satellite observations, reducing tropical cyclone track errors from approximately 110 km to 40 km and improving heavy precipitation forecasts at extreme rainfall thresholds where direct infrared assimilation shows no benefit. Overall, our framework demonstrates the potential of generative models to supplement sparse observational coverage and provide physically plausible microwave humidity fields for downstream applications.

</details>


### [199] [Spatial-Temporal Evolution of Proglacial Lake Volumes and Estimation Models in the Himalaya and Nyainqentanglha Ranges](https://doi.org/10.3390/rs18132249)
*Miaohui Zhang,Hao Wang,Peng Cui,Jinbo Tang,Yilong Yu,Cao,X G Liu,Jie Yang,Yunpeng Liu,Qi Li*

Main category: Remote Sensing

TL;DR: 本研究通过融合实地测深调查与区域数据集，优化了喜马拉雅和念青唐古拉山脉冰前湖的体积与最大深度经验模型，并重建了1990-2020年间湖泊体积的时空变化，发现各区域湖泊体积增长46.4%至92.9%不等。


<details>
  <summary>Details</summary>
Motivation: 冰前湖体积量化是冰湖溃决洪水（GLOF）可靠水文建模与洪峰估算的基础前提，但现有经验模型缺乏区域针对性，亟需优化。

Method: 整合10个冰前湖的实地测深调查数据与区域数据集，推导优化的体积-深度经验模型，并通过统计误差指标和独立数据集进行验证，与14个已有公式对比分析。

Result: 区域特化模型在捕捉局部地貌特征方面优于14个已有公式；1990-2020年间，西喜马拉雅、中喜马拉雅、东喜马拉雅和念青唐古拉山湖泊体积分别增长46.7%、46.4%、51.5%和92.9%。

Conclusion: 研究成果为基于卫星的水文监测提供了关键参数约束，显著提升了喜马拉雅和念青唐古拉山脉GLOF灾害评估的可靠性。

与遥感交叉或改进方案: 交叉/改进可行性：95%。该论文本身就是遥感水文交叉研究的典范。改进方案：1) 可引入SAR遥感数据（如Sentinel-1）提取更高时间分辨率的水体边界，结合本文的体积模型实现近实时GLOF风险动态评估；2) 可将经验模型与ICESat-2激光测高数据融合，替代部分实地测深工作，扩大模型在青藏高原其他无人区的泛化能力。

Abstract (英文): Volume quantification of proglacial lakes is a fundamental prerequisite for reliable hydrodynamic modeling and peak discharge estimation during glacial lake outburst floods (GLOFs). In this study, we integrated in situ bathymetric surveys of 10 proglacial lakes across the Himalaya and Nyainqentanglha ranges with a comprehensive regional dataset to derive optimized empirical models for lake volume and maximum depth. The predictive robustness of these models was rigorously validated using statistical error metrics and independent datasets. Comparative analysis with 14 established formulas demonstrates that our region-specific models yield superior performance in capturing local geomorphological characteristics. Leveraging these refined scaling relationships, we reconstructed the spatiotemporal volume changes in proglacial lakes across the study region from 1990 to 2020. Our analysis reveals significant lake expansion over the past three decades: lake volumes in the Western and Central Himalayas increased by 46.7% and 46.4%, respectively. Notably, the Eastern Himalayas exhibited a volume increase of 51.5%, while the Nyainqentanglha Mountains experienced a substantial expansion of approximately 92.9%. These findings provide critical parametric constraints for satellite-based hydrological monitoring and significantly enhance the reliability of GLOF hazard assessments in the Himalaya and Nyainqentanglha ranges.

</details>


### [200] [DEAF-Net: Dual-Domain Enhanced Adaptive Fusion Network for UAV Visible–Infrared Object Detection](https://doi.org/10.3390/rs18132241)
*Qian Weng,Yu Zhang,Xiansheng Huang,Liming Deng,Jiawen Lin*

Main category: Remote Sensing

TL;DR: 本文提出DEAF-Net，一种双域增强自适应融合网络，用于解决UAV多模态目标检测中模态可靠性空间异质性和空频协同表示不足的问题。核心模块包括双域渐进细化模块（DDPR）和一致性-差异引导融合策略（CDGF），在DroneVehicle和VEDAI数据集上分别达到81.9%和76.2%的mAP@0.5，在弱光、浓雾等场景下鲁棒性显著提升。


<details>
  <summary>Details</summary>
Motivation: 现有UAV多模态目标检测方法采用静态融合策略，忽略了不同模态在空间上的可靠性异质性，且对空频协同表示的探索不足，导致在复杂光照和天气条件下的检测鲁棒性受限。

Method: 提出DEAF-Net，包含两个核心模块：1) 双域渐进细化模块（DDPR）：通过频域可学习滤波与空间域尺度感知上下文细化联合设计，抑制噪声、增强纹理，获得纯净融合特征；2) 一致性-差异引导融合策略（CDGF）：利用VMamba的选择性扫描机制建模模态间的一致与差异模式，动态生成局部模态贡献图进行自适应融合，并通过熵权重集成全局场景先验进行校准。

Result: 在DroneVehicle和VEDAI数据集上，DEAF-Net分别达到81.9%和76.2%的mAP@0.5，优于主流多模态检测方法，在弱光、浓雾和稀疏类别场景下具有更好的鲁棒性。

Conclusion: DEAF-Net通过双域渐进细化和一致性-差异引导自适应融合，有效解决了动态环境下多模态融合的可靠性异质性和空频协同问题，提升了UAV目标检测的鲁棒性。

与遥感交叉或改进方案: 交叉/改进可行性：95%。该文本身就是面向UAV遥感对地观测的典型应用。改进方案：1) 可将DDPR模块中的频域可学习滤波扩展到多尺度频带分解，更好地分离遥感图像中的地物纹理与噪声；2) CDGF策略中的VMamba选择性扫描可结合遥感场景中的地理空间先验（如道路、建筑分布）进行引导，进一步提升融合精度；3) 可引入SAR-可见光-红外等多于两种模态的扩展，验证DEAF-Net在多模态遥感融合中的通用性。

Abstract (英文): In Unmanned Aerial Vehicle (UAV) object detection tasks, complex lighting conditions and variable weather render robust all-weather perception challenging when relying solely on the visible modality. Although infrared modalities can provide complementary information, the reliability of individual modalities is highly scene-dependent. Existing multimodal detection methods typically adopt static fusion strategies, which ignore spatial heterogeneity of modal reliability and under-explore spatial-frequency collaborative representation, thus limiting detection robustness in dynamic environments. To address these issues, this paper proposes a Dual-domain Enhanced Adaptive Fusion Network (DEAF-Net), with two core innovative modules to tackle the above challenges. First, the Dual Domain Progressive Refinement (DDPR) module mitigates feature degradation caused by poor imaging conditions via the joint design of frequency-domain learnable filtering and scale-aware contextual refinement in the spatial domain, effectively suppressing noise, enhancing textures, and yielding a purified feature basis for fusion. Second, the Consistency–Discrepancy Guided Fusion (CDGF) strategy leverages the selective scanning mechanism of VMamba to model consistent and differential patterns across modalities, dynamically generates local modal contribution maps for adaptive fusion, and integrates global scene prior via entropy weights for calibration. Extensive experiments on the DroneVehicle and VEDAI datasets show that DEAF-Net outperforms mainstream multimodal detection methods, achieving mAP@0.5 scores of 81.9% and 76.2%, respectively, while delivering improved robustness in low-light, dense fog, and sparse-category scenarios.

</details>


### [201] [Stereo Matching in Satellite Imagery: A Depth Estimation Foundation Model-Assisted Iterative Approach](https://doi.org/10.3390/rs18132245)
*Kunpeng Hu,Wei Zhao*

Main category: Remote Sensing

TL;DR: 本文提出IFMA-Stereo，一种融合单目深度基础模型的双目视差估计方法，通过多尺度空间信息金字塔、注意力交互机制和循环迭代精化模块，在高分辨率卫星立体匹配中解决极端成像几何、无纹理区域、重复图案和时空异质性等问题，在US3D和高分7数据集上达到SOTA精度（US3D: EPE 1.347, D1 7.26%）。


<details>
  <summary>Details</summary>
Motivation: 光学遥感3D重建中，高分辨率卫星立体匹配面临极端成像几何、无纹理/重复图案、遮挡以及时空异质性导致的场景变化等挑战，现有方法难以同时处理这些问题。

Method: 1) 构建多尺度空间信息金字塔，联合基础模型与视差提取网络；2) 特征层面采用注意力交互机制捕获多维上下文依赖，将通用场景理解先验转化为适合立体代价体构建的长程关联特征；3) 像素层面采用循环迭代精化模块，在整个迭代过程中嵌入基础模型的深度信息并进行联合优化，增强在几何复杂区域的适应性。

Result: US3D数据集：EPE 1.347, D1 7.26%；GaoFen-7数据集：EPE 1.585, D1 13.41%；在未见过的城区表现出强泛化能力；但推理时间较基线方法增加。

Conclusion: IFMA-Stereo在遥感视差估计中达到了SOTA精度，有效缓解了无纹理区域、视差不连续、重复图案及时空异质性引起的预测误差，并具有良好的泛化能力。

与遥感交叉或改进方案: 交叉/改进可行性：95%。改进方案：1) 可将单目深度基础模型替换为遥感领域自监督预训练的基础模型（如卫星MAE），进一步提升对遥感地物几何结构的理解；2) 注意力交互机制可扩展为跨模态融合（如引入多光谱或SAR信息），增强在云雾遮挡或弱光照区域的鲁棒性；3) 循环迭代精化模块可引入不确定性估计，在推理时动态调整迭代次数以平衡精度与效率。

Abstract (英文): In optical remote sensing 3D reconstruction, high-resolution satellite stereo matching is a critical task, yet it is challenged by extreme imaging geometries, texture-less and repetitive patterns, occlusions, and scene variations caused by spatio-temporal heterogeneity. To address these issues, we propose IFMA-Stereo, an innovative binocular disparity estimation method that leverages a monocular depth foundation model. Our approach constructs a multi-scale spatial information pyramid to jointly integrate the foundation model with a disparity extraction network. At the feature level, an attention interaction mechanism captures multi-dimensional contextual dependencies and transforms general scene understanding priors into long-range associative features suitable for stereo cost volume construction. At the pixel level, a cyclic iterative refinement module embeds depth information from the foundation model throughout the iteration process and performs joint optimization, enhancing the model’s adaptability in geometrically complex regions. Experiments on the US3D and GaoFen-7 datasets demonstrate that IFMA-Stereo achieves superior performance in challenging areas (texture-less regions, disparity discontinuities, repetitive patterns) and effectively mitigates prediction errors caused by spatio-temporal heterogeneity, albeit at the cost of increased inference time compared to baseline methods. Quantitatively, the method achieves an end-point error (EPE) of 1.347 and a D1 error of 7.26% on the US3D dataset, and an EPE of 1.585 and a D1 error of 13.41% on the GaoFen-7 dataset. Notably, the method also yields precise predictions for unseen urban areas, indicating strong generalization. These results confirm that IFMA-Stereo achieves state-of-the-art accuracy in remote sensing disparity estimation.

</details>


### [202] [An Adaptive Wet Tropospheric Correction Method Using a Spaceborne Microwave Radiometer](https://doi.org/10.3390/rs18132250)
*Xiaomeng Zheng,Yuhang Li,J Z Zhao,Jieying He,Dehai Zhang*

Main category: Remote Sensing

TL;DR: 针对海面风速相关的非线性微波辐射响应导致湿对流层延迟（WTC）估计不准的问题，提出了一种自适应WTC方法，通过风速区间重叠建模、多尺度样本均衡和模型软融合策略，利用HY-2C CMR数据验证，显著提升了WTC反演精度和稳定性，尤其在稀疏观测条件下有效降低了系统偏差。


<details>
  <summary>Details</summary>
Motivation: 现有WTC方法采用全局统一统计框架，无法有效表征风速依赖的非线性海面微波辐射响应，且在观测分布不均匀时容易产生系统性偏差，亟需一种能适应不同风速条件的高精度WTC反演方法。

Method: 提出自适应WTC方法，包含三个核心模块：(1) 根据风速驱动的海面辐射响应变化，建立低、中、高风速区间的重叠过渡带建模框架，分别训练子模型；(2) 设计结合全局权重与局部权重的多尺度样本均衡策略，增强对稀疏样本的学习；(3) 采用基于梯形隶属度函数的软融合策略，动态加权子模型输出，确保过渡带的连续反演。

Result: 基于HY-2C CMR观测数据，与模型反演WTC及Jason-3 AMR-2交叉验证表明，所提方法在整体WTC反演精度和稳定性上均有提升，有效降低了稀疏观测风速区间的系统偏差。

Conclusion: 所提出的自适应WTC方法为不同风速条件下的高精度WTC反演提供了一种有效且鲁棒的解决方案。

与遥感交叉或改进方案: 交叉/改进可行性：95%。本文方法可直接应用于其他卫星微波辐射计（如FY-3系列、Sentinel-3等）的WTC反演。改进方案：(1) 可引入风速先验知识或数值天气预报模型的风速预报场作为输入特征，增强子模型区分度；(2) 梯形隶属度函数的参数（过渡带宽度）可通过自适应优化（如贝叶斯优化）自动确定，替代人工预设；(3) 样本均衡策略可扩展为动态在线学习机制，适应不同海区或季节的风速分布变化。

Abstract (英文): High-precision WTC is essential for satellite altimetry and ocean dynamic environment monitoring. Existing WTC approaches often rely on globally unified statistical frameworks, which inadequately represent wind-speed-dependent nonlinear sea-surface microwave radiative responses and are prone to systematic bias under uneven observation distributions. To address these limitations, this study proposes an adaptive WTC method integrating overlapping wind-regime modeling, multi-scale collaborative sample balancing, and a model soft-fusion strategy. Firstly, a modeling framework with overlapping transition zones for low-, moderate-, and high-wind-speed regimes is established according to wind-speed-driven variations in sea-surface radiative responses, and sub-models are trained independently. Subsequently, a multi-scale sample balancing, combining global and local weights, is designed to enhance learning from sparse samples. Finally, a soft-fusion strategy based on a trapezoidal membership function is applied to dynamically weight sub-model outputs, ensuring retrieval continuity across transition zones. Using HY-2C Calibration Microwave Radiometer (CMR) observations, the proposed method is developed, trained, and evaluated against model-derived WTC and collocated Jason-3 AMR-2 measurements. Results show that the proposed method improves overall WTC retrieval accuracy and stability while effectively reducing systematic biases under wind-speed regimes with sparse observations, providing an effective and robust approach for high-accuracy WTC retrieval under various wind-speed conditions.

</details>


### [203] [On the Spectral–Phenological Features for Crop Mapping Under Complex Planting Patterns: A Case Study in Jiangsu Province, China](https://doi.org/10.3390/rs18132244)
*Ziyin You,Jiajun Wu,X F Wang,Bo Wang,Xuan Xu,Pei Zhan,N LI,Chitfai Yan*

Main category: Remote Sensing

TL;DR: 本文针对复杂农业景观中作物重叠物候和多源时序变量冗余问题，提出了一种基于先验引导遗传算法的特征子集选择方法。利用2022-2023年江苏地区的Sentinel-1/2影像构建275个季节特异性光谱-物候特征变量，结合辅助二分类随机森林估算预测贡献以构建搜索先验，通过遗传算法选择紧凑子集用于五类作物制图。独立验证表明，GA筛选子集（9-39变量）在五类分类中达到91.77%的总体精度和93.95%的目标作物F1分数，优于全特征库（86.96%）和单一数据源方案。


<details>
  <summary>Details</summary>
Motivation: 碎片化农业景观中作物日历重叠和多源时序变量高度冗余导致作物制图精度受限，现有特征选择方法在作物特异性、可解释性和子集紧凑性方面存在不足，亟需一种能够针对不同作物识别有效时间窗口和关键光谱特征的可解释降维方法。

Method: 1）基于Sentinel-1/2影像构建125个夏季作物和150个冬季作物的月尺度光谱-物候特征变量；2）利用辅助二分类随机森林估算OOB排列重要性建立搜索先验；3）采用先验引导的遗传算法针对每种作物选取紧凑特征子集；4）利用五类随机森林模型进行候选子集评估和最终分类；5）采用固定的80/20开发-验证集划分进行独立验证。

Result: 八月和四月分别是夏季和冬季作物主要区分时段；VH后向散射和SWIR相关指数（STI、NDTI）具有跨作物的重要预测贡献。独立验证中，光学/植被指数方案、SAR-only方案和全特征库的平均目标作物F1分别为78.42%、83.74%和86.96%。GA筛选子集（9-39变量）平均五类总体精度为91.77%，目标作物F1为93.95%。集成季节图总体精度达81.20-95.03%。

Conclusion: 基于先验引导遗传算法的特征选择方法能够在随机森林框架内实现作物特异性、可解释的降维，显著提升制图精度并降低特征冗余；子集效果受作物类型和学习算法影响，跨年度和跨区域的泛化性需进一步验证。

与遥感交叉或改进方案: 交叉/改进可行性：95%。改进方案：1）可将先验引导遗传算法引入遥感作物制图的特征优选流程，替代传统的全变量或滤波器方法；2）该方法可迁移到其他经济作物（如棉花、甘蔗）的识别任务中；3）可扩展至多源数据（如Landsat、Gaofen、Planet）融合场景，利用GA自动筛选最优光谱-空间-时间特征组合；4）可结合深度学习特征提取器（如时序CNN、Transformer）生成的深层特征进行联合优化，提升复杂场景下的作物区分能力。

Abstract (英文): Accurate crop mapping in fragmented agricultural landscapes is challenged by overlapping crop calendars and redundancy among multi-source time-series variables. Using Sentinel-1/2 imagery from December 2022 to December 2023, we constructed 275 season-specific spectral–phenological feature–month variables (125 for summer crops and 150 for winter crops) for rice, maize, soybean, winter wheat, and winter rapeseed in Jiangsu Province, China. An auxiliary binary Random Forest (RF) was used to estimate out-of-bag (OOB) permutation-based predictive contributions and construct search priors. A prior-guided genetic algorithm (GA) then identified compact subsets, with crop-specific five-class RF models used both to evaluate candidate subsets and to produce the final classifications. A fixed stratified 80/20 development–validation split was maintained throughout the analysis, with the validation subset reserved for final assessment. August and April were the principal discriminative periods for summer and winter crops, respectively, while VH backscatter and SWIR-related indices, particularly STI and NDTI, showed recurrent predictive contributions across crops. On the independent validation subset, the optical/vegetation-index scheme, SAR-only scheme, and the complete feature library achieved mean target-crop F1-scores of 78.42%, 83.74%, and 86.96%, respectively. The GA-selected subsets retained 9–39 variables and achieved a mean five-class overall accuracy of 91.77% and a mean target-crop F1-score of 93.95%. After non-target classes were merged into a single background class, the integrated seasonal maps achieved overall accuracies of 81.20–95.03% on the same validation subset. Supplementary classifier comparisons indicated that subset effects depended on the crop and learning algorithm. The findings support crop-specific, interpretable dimensionality reduction within the RF workflow, while broader transferability requires multi-year and multi-region evaluation.

</details>


### [204] [Deep Learning Enables the Automatic Mapping of Tell Sites on Satellite Synthetic Aperture Radar Products](https://doi.org/10.3390/rs18132255)
*Elena Chiricallo,Giulio Poggi,Sara Ferro,Sebastiano Vascon,Arianna Traviglia*

Main category: Remote Sensing

TL;DR: 本文提出了一种基于深度学习（YOLOv8-Seg）的自动检测与分割管道，利用 Sentinel-1 SAR 影像和 DEM 数据对伊拉克中部考古土丘（tell）进行实例分割，平均精度 0.495，IoU 0.361，并展示了向伊朗区域的迁移潜力。


<details>
  <summary>Details</summary>
Motivation: SAR 在考古探测中具有监测地表形态和近地表特征的潜力，但缺乏大规模自动化分析方法，限制了其在考古勘探中的广泛应用。

Method: 采用 YOLOv8-Seg 实例分割模型，以 Sentinel-1 IW GRD SAR 影像和 Copernicus 30m DEM 为输入，对伊拉克中部的考古土丘（tell）进行自动检测与分割，并在伊朗测试区评估迁移能力。

Result: 模型在测试区域的平均精度（AP）为 0.495±0.010，像素级 IoU 为 0.361±0.048；考古验证确认了模型可靠性，并发现了此前未测绘的潜在遗址；向伊朗迁移时需区域微调。

Conclusion: 该研究为基于 SAR 的深度学习考古勘探建立了基线，证明了自动检测分割的可行性，但迁移应用需考虑区域形态与环境差异进行微调。

与遥感交叉或改进方案: 交叉/改进可行性：95%。本文本身就是遥感考古论文。改进方向：(1) 引入多时相 SAR 堆栈（如时序 InSAR 或相干性分析）提升对微地形变化的敏感度；(2) 融合光学多光谱数据（如 Sentinel-2）辅助区分自然地貌与人工土丘；(3) 加入自监督预训练策略减少对标注样本的依赖；(4) 采用多尺度特征融合（如注意力机制）提升对小土丘和低对比度目标的检测精度。

Abstract (英文): Satellite Synthetic Aperture Radar (SAR) is an established technology for studying and monitoring archaeological landscapes, providing insights into surface morphology and the presence of near subsurface features. However, its application in large-scale archaeological prospection is limited by the lack of robust, automated methods for SAR data analysis. This study introduces a novel Deep Learning pipeline to automatically detect and segment archaeological settlement mounds, known as tells, in central Iraq on satellite SAR data. The pipeline leverages a state-of-the-art supervised method for instance segmentation, YOLOv8-Seg, and medium-resolution satellite SAR products, specifically the Copernicus Sentinel-1 Interferometric Wide Swath Mode Ground Range Detected and Copernicus Global 30-m Digital Elevation Model products. The model identifies tell sites with an Average Precision of 0.495±0.010 and a pixel-wise Intersection over Union of 0.361±0.048 over the test areas. Archaeological interpretation of the model’s inferences confirms its reliability in locating and segmenting archaeological sites, leading also to the identification of previously unmapped potential sites. After a main test in central Iraq, the proposed workflow demonstrates promising transferability to a nearby test area in Iran, although with a need for regional fine-tuning to account for inherent variations in feature morphology and environmental context. This research establishes a baseline for future Deep Learning applications in Synthetic Aperture Radar-based archaeological prospection.

</details>


### [205] [Mapping Acid Mine Drainage Areas with Sentinel-2 and WorldView-3 VNIR Satellite Images: An Example in the SE of Spain](https://doi.org/10.3390/rs18132240)
*Inés Pereira,Eduardo García‐Meléndez,Montserrat Ferrer‐Julià,H.M.A. van der Werff*

Main category: Remote Sensing

TL;DR: 本文评估并比较了Sentinel-2和WorldView-3 VNIR波段在西班牙卡塔赫纳-拉乌尼翁矿区用于绘制和区分次生铁矿（赤铁矿、针铁矿、黄钾铁矾）的性能，发现Sentinel-2因其红边和近红外波段配置在区分这三种矿物方面表现更优，尤其是在抛物线拟合方法下，而WorldView-3主要能区分赤铁矿与针铁矿-黄钾铁矾组合。研究认为免费可用的Sentinel-2影像可支持矿区氧化过程的系统监测和环境风险评估。


<details>
  <summary>Details</summary>
Motivation: 硫化物矿床开采加速了硫化物矿物的氧化，产生富含硫酸盐和溶解金属的酸性矿山排水（AMD）以及次生铁矿物的形成。这些矿物在VNIR区域具有诊断性光谱特征，使得多光谱卫星数据成为一种经济有效的监测手段。本文旨在评估和比较Sentinel-2与WorldView-3 VNIR波段在绘制和区分次生铁矿物方面的性能。

Method: 研究采用了波段比值法和抛物线拟合技术分析Sentinel-2和WorldView-3 VNIR数据，重点针对反射率最大值进行处理。通过X射线衍射对74个地表土壤样本进行验证，以评估两种传感器矿物制图结果的准确性。

Result: 两种传感器均能再现铁矿物的主要空间分布格局，但Sentinel-2在区分赤铁矿、针铁矿和黄钾铁矾方面表现更佳（尤其是在抛物线拟合方法下），而WorldView-3 VNIR数据主要能区分赤铁矿与针铁矿-黄钾铁矾组合。Sentinel-2的优越性能归因于其红边和近红外波段配置。

Conclusion: 免费可用的Sentinel-2影像能够支持矿区氧化过程的系统监测，并为退化景观的环境风险评估做出贡献。

与遥感交叉或改进方案: 交叉/改进可行性：95%。本文本身就是遥感在矿区环境监测中的应用研究，可从以下方面改进：（1）引入高光谱遥感数据（如PRISMA、EnMAP、Hyperion）以提供更精细的光谱分辨率，提升矿物识别精度；（2）结合热红外（TIR）波段数据，利用石英等脉石矿物的热红外特征辅助识别；（3）应用深度学习（如卷积神经网络或Transformer）进行端到端的矿物分类，减少对人工波段选择和拟合参数的依赖；（4）融合LiDAR地形数据以校正地形光照效应，提升复杂山地环境的制图精度；（5）拓展时间序列分析，利用Sentinel-2的重访优势监测AMD氧化过程的动态演变。

Abstract (英文): Mining of sulfide-rich deposits enhances the oxidation of sulfide minerals, generating acid mine drainage (AMD) characterized by high sulphate and dissolved metal concentrations and the formation of secondary iron minerals (hematite, goethite, and jarosite). As these minerals display diagnostic features in the visible–near-infrared (VNIR) region, multispectral satellite data provide a cost-effective means of monitoring. Here, the performances of Sentinel-2 and the VNIR bands from WorldView-3 are assessed and compared for the mapping and discrimination of secondary iron minerals in Sierra Minera de Cartagena–La Unión (SE Spain). Both datasets were analyzed using a band ratio and a parabola fitting technique focused on reflectance maxima. Band ratio results were interpreted as broad spectral patterns rather than definitive mineral identifications. Mineral maps were validated by applying X-ray diffraction on 74 surface soil samples. Although both sensors were able to reproduce the main spatial patterns of iron mineral distribution, Sentinel-2 data better discriminated hematite, goethite, and jarosite, especially when using the parabola fitting approach, whereas WorldView-3 VNIR data distinguished mainly hematite from the combined goethite–jarosite group. The better performance of Sentinel-2 is attributed to its red-edge and near-infrared band configuration. These findings indicate that freely available Sentinel-2 imagery can support systematic monitoring of oxidation processes in mining environments and contribute to environmental risk assessment in degraded landscapes.

</details>


### [206] [A Sentinel-2-Based Framework for Methane Point-Source Detection and Quantification Using Low-Reflectance Artifact Detection](https://doi.org/10.3390/rs18132251)
*Kun Cai,T -Y Chen,Liang Zheng,Shenshen Li,Xinhui Zhou,Ying Liu,X Chen*

Main category: Remote Sensing

TL;DR: 本文提出了一种名为MBMC（多波段多约束）的综合甲烷监测框架，利用Sentinel-2 MSI影像，结合低反射率伪影检测算法（LRAD）、多波段多通带差分吸收反演模型（MBMP）和综合质量增强排放量化方法（IME），实现了对甲烷点源排放的高分辨率、高频率卫星监测与量化。在美国亚利桑那州单盲对照释放实验中，该方法对0.5-3.0 t/h排放率的平均绝对百分比误差（MAPE）为21.6%，并成功应用于中国西北天然气田的多个甲烷点源识别与量化。


<details>
  <summary>Details</summary>
Motivation: 现有卫星甲烷监测存在空间覆盖范围有限和地表信号干扰等技术瓶颈，亟需开发一种高分辨率、高频率且能有效抑制地表伪影的甲烷点源监测方法。

Method: 提出MBMC框架，包含三个核心模块：1）低反射率伪影检测（LRAD）算法，抑制低反射率地表和异质背景产生的伪影；2）多波段多通带（MBMP）差分吸收反演模型，用于反演甲烷柱浓度增强；3）综合质量增强（IME）方法进行排放量化。此外，还开发了结合形态学操作和空间数据库的半自动羽流分割工作流。

Result: 在美国亚利桑那州单盲对照实验中，对0.5-3.0 t/h排放率的MAPE为21.6%；成功应用于中国西北天然气田，识别并量化了多个与集气站相关的甲烷点源。

Conclusion: MBMC框架为基于Sentinel-2的高分辨率、高频率卫星甲烷点源监测提供了实用方法，有助于完善油气行业的甲烷排放清单和减排策略。

与遥感交叉或改进方案: 交叉/改进可行性：95%。该研究本身就是遥感应用论文，改进方向包括：1）可将LRAD算法适配到其他高分辨率卫星传感器（如GF-2、PlanetScope）以扩展监测能力；2）MBMP差分吸收模型可结合深度学习（如CNN或Transformer）替代传统阈值方法，提升羽流分割的自动化精度；3）IME排放量化可引入大气传输模型（如HYSPLIT）的风场数据，降低风速不确定性带来的量化误差；4）可拓展至CO2等其他温室气体的点源监测，形成多温室气体协同监测框架。

Abstract (英文): Methane (CH4) is the second most significant greenhouse gas after carbon dioxide (CO2). Due to its high short-term global warming potential and the feasibility of emission abatement, monitoring methane point-source emissions has become a critical strategy for mitigating climate change. To address existing technical bottlenecks associated with spatial coverage limitations and surface-induced signal interference in satellite-based monitoring, this study proposes an integrated methane monitoring framework termed Multi-Band Multi-Constraint (MBMC) using Sentinel-2 MSI imagery. The MBMC framework combines a Low-Reflectance Artifact Detection (LRAD) algorithm, a Multi-Band Multi-Pass (MBMP) differential absorption retrieval model, and Integrated Mass Enhancement (IME)-based emission quantification. The LRAD module effectively suppresses artifacts caused by low-reflectance surfaces and heterogeneous backgrounds, thereby improving the signal-to-noise ratio (SNR) and retrieval accuracy of methane column enhancements. In addition, a semi-automatic plume segmentation workflow integrating morphological operations with a spatial database is developed to improve methane plume extraction and source localization. The framework was validated using data from single-blind controlled methane release experiments conducted in Arizona, USA. Results show that the proposed method achieved a mean absolute percentage error (MAPE) of 21.6% for emission rates ranging from 0.5 to 3.0 t/h, demonstrating promising performance for Sentinel-2-based screening and quantification of methane point sources, particularly for emissions above approximately 0.5 t/h under favorable observation conditions. The framework was further applied to Sentinel-2 observations over a natural gas field in Northwest China, where multiple methane point sources associated with gas gathering stations were successfully identified and quantified. The proposed framework provides a practical approach for high-resolution and high-frequency satellite monitoring of methane point sources and supports the refinement of methane emission inventories and mitigation strategies in the oil and gas sector.

</details>


### [207] [The Impact of Dynamic Observation Error and Hydrometeor Control Variables on GK-2A AMI All-Sky Radiance Assimilation](https://doi.org/10.3390/rs18132246)
*Seo-Youn Jo,Ki‐Hong Min*

Main category: Remote Sensing

TL;DR: 该研究探讨了在全天空辐射率（ASR）资料同化中，动态观测误差对分析和降水预报的影响。通过采用基于云影响参数的五次多项式模型，实现随云条件自适应变化的观测误差。结果表明，动态误差相比恒定误差能改善云分析和热力学场，尤其当固体水凝物作为控制变量时，结合动态误差能产生最真实的亮度和反射率分析，并显著提升降水预报效果。


<details>
  <summary>Details</summary>
Motivation: ASR同化中直接更新水凝物场存在较大不确定性，恒定观测误差在强云区会过度加权观测，导致分析质量下降。因此，需要研究动态观测误差在不同水凝物控制变量配置下的影响。

Method: 采用五次多项式模型将观测误差表示为云影响参数的函数，实现时空变化的场景依赖误差；在不同水凝物控制变量配置下进行ASR同化实验，对比动态误差与恒定误差的效果。

Result: 动态观测误差普遍改善了云分析和相关热力学场；恒定误差在强云区过度加权观测，降低分析质量；动态误差的优势在包含固体水凝物的控制变量配置中更为显著；结合动态误差与固体水凝物直接更新的实验获得了最真实的亮温和反射率分析，并最大程度提升了降水预报。

Conclusion: 在ASR同化中，云依赖的观测误差建模至关重要，尤其是在采用固体水凝物控制变量时，动态误差能显著提升分析和预报质量。

与遥感交叉或改进方案: 交叉/改进可行性：85%。该研究的动态观测误差建模方法可直接应用于遥感数据同化领域，尤其是卫星云图和降水雷达数据的同化。具体方案：(1) 将云影响参数扩展到多通道遥感数据（如可见光、红外和微波），构建多谱段的场景依赖误差模型；(2) 将动态误差方法引入遥感影像反演中，用于优化云参数和降水参数的估计；(3) 结合深度学习技术，从遥感数据中学习云影响参数与观测误差之间的非线性关系，进一步提升误差建模的精度和泛化能力。

Abstract (英文): Assimilation of all-sky radiance (ASR) observations informs atmospheric states and cloud distributions; however, it does not always lead to improved analyses or forecasts. In particular, directly updating hydrometeor fields introduces substantial uncertainty into ASR assimilation. This study examines the impact of dynamic observation errors on analyses and precipitation forecasts under different hydrometeor control variable (HCV) configurations. Observation errors are prescribed using a fifth-order polynomial model as a function of a cloud impact parameter, allowing spatiotemporally varying (i.e., scene-dependent) errors that adapt to cloud conditions. Results indicate that dynamic observation errors generally improve cloud analyses and associated thermodynamic fields. By contrast, constant errors tend to overweight ASR observations in heavily cloud-affected regions, thereby degrading analysis quality. The advantages of dynamic errors are more pronounced when solid-phase hydrometeors are included in the HCV, as these strongly influence brightness temperature (BT) analysis and the representation of convective cloud tops. Among all experiments, those combining dynamic errors with direct updates of solid-phase hydrometeors produce the most realistic BT and reflectivity analyses, as well as the greatest improvements in precipitation forecasts. These results underscore the importance of cloud-dependent observation error modeling in ASR assimilation, particularly when solid-phase HCVs are employed.

</details>


### [208] [GHF-DETR: An Improved DETR Framework with a Multi-Path Backbone and Dual-Domain Downsampling for UAV Object Detection](https://doi.org/10.3390/rs18132239)
*Lei Hu,Qingming Huang,Zhixiang Liu,Hongwei Ye*

Main category: Remote Sensing

TL;DR: 提出GHF-DETR，一种基于Transformer的无人机小目标检测器，通过异构多路径卷积网络、动态多尺度聚焦模块和高频选择性保留下采样模块协同设计，在VisDrone2019和NWPU VHR-10上实现了精度-效率的良好平衡。


<details>
  <summary>Details</summary>
Motivation: 无人机图像中的小目标检测面临像素覆盖低、背景复杂、下采样信息丢失等挑战，现有检测器缺乏增强弱目标信号的显式机制。

Method: 1) HMC骨干网络：部分卷积+门控线性单元减少计算冗余；2) DMSF模块：学习偏移对齐+多核深度可分离卷积实现跨尺度融合；3) HSP下采样模块：空间到深度卷积+2D离散小波变换在空间和频域补偿信息损失。

Result: VisDrone2019上mAP@0.5达33.1%，mAP@0.5:0.95达18.6%，GFLOPs为15.4，参数量7.59M，AP_S达10.1%，相比DFINE-n基线分别提升5.4%和3.1%。在NWPU VHR-10上验证了泛化性。

Conclusion: GHF-DETR实现了有利于高效无人机小目标检测的精度-效率平衡。

与遥感交叉或改进方案: 交叉/改进可行性：95%。该论文本身就是遥感小目标检测领域的成果。改进方向：1) HMC模块可引入注意力引导的动态门控机制进一步筛选小目标特征；2) DMSF模块可结合遥感图像的多尺度地理空间先验（如地物尺寸分布）进行自适应偏移校准；3) HSP可尝试将DWT与小波包分解结合，保留更多高频子带信息；4) 骨干网络可替换为更轻量的Mamba或状态空间模型以进一步提升推理效率。

Abstract (英文): Detecting small targets in Unmanned Aerial Vehicle (UAV) imagery is challenging due to low pixel coverage, complex backgrounds, and information loss during downsampling. Existing detectors lack explicit mechanisms for enhancing weak target signals. We propose GHF-DETR, a Transformer-based detector featuring three collaboratively designed modules. First, a Heterogeneous Multi-Path Convolutional Network (HMC) backbone uses partial convolution and gated linear units to reduce computational redundancy while maintaining discrimination of small-object features. Second, a Dynamic Multi-Scale Focusing (DMSF) module integrates learned offset alignment with multi-kernel depthwise convolutions for cross-scale feature fusion. Third, a High-Frequency Selective Preservation (HSP) downsampling module combines space-to-depth convolution with 2D Discrete Wavelet Transform (DWT) to compensate for information loss in both spatial and frequency domains. On VisDrone2019, GHF-DETR achieves 33.1% mAP@0.5 and 18.6% mAP@0.5:0.95 with 15.4 GFLOPs and 7.59 M parameters, improving over the DFINE-n baseline by 5.4% and 3.1%, respectively, with AP_S reaching 10.1%. Generalization is validated on NWPU VHR-10. These results demonstrate that GHF-DETR achieves a favorable accuracy–efficiency balance for efficient UAV small-object detection.

</details>


### [209] [Development of PXB-BVC Framework for Multivariate Flood-Risk Assessment Under Climate Change](https://doi.org/10.3390/rs18142275)
*Aili Yang,Wenjie Li,Pangpang Gao,Yurui Fan,Xiuquan Wang*

Main category: Remote Sensing

TL;DR: 本文提出PXB-BVC混合框架（SWAT+HBV+XGBoost+BMA+双变量Copula），结合物理水文模型与机器学习，应用于香溪河流域。在四个GCM和三个SSP情景下，日径流模拟NSE达0.95（率定）/0.89（验证），并进行了非平稳条件下的双变量洪水风险分析，揭示了未来洪水联合概率的变化规律。


<details>
  <summary>Details</summary>
Motivation: 气候变化下洪水风险加剧，传统水文模型在径流预测和多变量洪水风险评估方面存在不足，需要开发融合物理过程与机器学习的新方法以提高预测精度和风险评估能力。

Method: 构建了PXB-BVC框架：1）SWAT和HBV两个物理水文模型进行径流模拟；2）XGBoost对物理模型输出进行非线性校正；3）BMA对多种模型结果进行贝叶斯平均；4）双变量Copula分析洪水特征（洪峰、洪量、历时）的依赖结构和联合重现期。利用ArcGIS处理30m土地利用数据（2000年遥感监测数据）作为输入。

Result: 1）日径流模拟NSE=0.95（率定）和0.89（验证）；2）未来径流总体增加，SSP585下Sen斜率最高达0.46 m³ s⁻¹ yr⁻¹；3）洪峰-洪量对Kendall tau在0.6-0.8之间，呈现非平稳依赖结构；4）OR型联合重现期显示50年事件在2061-2100年更频繁，AND型变化较弱；5）极端洪峰下伴随高洪量的概率从0.23增至0.56，伴随长历时的概率从0.18增至0.45。

Conclusion: PXB-BVC框架能有效支持非平稳条件下的多变量洪水风险评估，为气候适应型水资源管理和基础设施规划提供有用信息。

与遥感交叉或改进方案: 交叉/改进可行性：75%。该论文已使用了30m土地利用遥感数据（2000年中国土地利用遥感监测数据）作为SWAT模型的输入参数。可进一步改进的方向：1）引入更高时空分辨率的遥感数据（如Sentinel-1 SAR洪水监测影像、Landsat/Modis时序数据）进行模型验证和参数动态更新；2）利用遥感蒸散产品（如MODIS ET）校验水文过程模拟；3）结合遥感降水产品（如GPM、TRMM）作为模型输入，提升无资料区域的适用性；4）引入遥感地形数据（如DEM及其衍生地形指数）优化水文响应单元的划分精度。

Abstract (英文): Flood risks are escalating under climate change, necessitating advanced methods to improve runoff prediction and multivariate flood-risk assessment. In this study, a physics–XGBoost-based Bayesian model averaging with bivariate copulas (PXB-BVC) framework was developed by integrating the Soil and Water Assessment Tool (SWAT), the Hydrologiska Byråns Vattenbalansavdelning (HBV) model, Extreme Gradient Boosting (XGBoost), Bayesian model averaging (BMA), and bivariate copulas. Spatially detailed underlying surface parameters including 30 m land-use data derived from the 2000 China land-use remote sensing monitoring data were pre-processed and reclassified using ArcGIS to support spatially explicit hydrological simulation. The framework was applied to the Xiangxi River Basin (XXRB), China, under four general circulation models and three shared socioeconomic pathways. PXB-BVC improved daily runoff simulation by combining process-based hydrological information with nonlinear machine learning correction, achieving Nash–Sutcliffe efficiency (NSE) values of 0.95 during calibration and 0.89 during validation. Future runoff generally increased from the near-term to the late-century period, with stronger changes under SSP585 and Sen slopes reaching up to 0.46 m3 s−1 yr−1, although the magnitude and significance of trends varied among GCMs. The dependence structures among flood peak, flood volume, and flood duration showed non-stationary behavior under future climate forcing, with Kendall’s tau for peak–volume pairs mostly ranging from 0.6 to 0.8. The revised bivariate return-period analysis further indicates that inferred flood-risk changes depend on the joint risk definition. Under SSP245 and ACCESS-ESM1–5, OR-type joint return periods show that representative near-future 50-year events may become more frequent in 2061–2100, whereas AND-type return periods show weaker and less uniform changes among flood-characteristic pairs. Conditional probability analysis also indicates enhanced compound risk under high-emission conditions: given an extreme peak flow, the probability of accompanying high flood volume increases from 0.23 to 0.56, while the probability of prolonged duration increases from 0.18 to 0.45. These results demonstrate that the PXB-BVC framework can support non-stationary multivariate flood-risk assessment and provide useful information for climate-resilient water-resource management and infrastructure planning.

</details>


### [210] [CAF-Net: A Unified Framework for Resolving Spatial–Frequency Representation Conflicts in Multimodal Remote Sensing Segmentation](https://doi.org/10.3390/rs18142284)
*Ming Ma,S S Wu,X Li,Wei Hong*

Main category: Remote Sensing

TL;DR: 提出冲突感知融合网络(CAF-Net)，通过显式建模空间域结构细节与频域语义之间的不一致性，在ISPRS Vaihingen和Potsdam数据集上分别达到84.35%和86.86% mIoU，显著提升多模态遥感分割的性能与表示一致性。


<details>
  <summary>Details</summary>
Motivation: 现有多模态遥感分割方法忽略了空间域结构细节与频域语义之间的不一致性，导致复杂场景中特征对齐不佳、性能退化。

Method: 提出CAF-Net，包含三个核心模块：(1) 跨模态结构引导(CMSG)——从DSM中提取高通结构线索并条件性调制光学特征；(2) 自适应跨频模块(ACFM)——利用固定径向掩模分离DCT系数并自适应重加权低高频分量；(3) 不确定性感知融合(UAF)——预测逐像素相对置信度分数并跨模态归一化以抑制低置信响应。

Result: 在ISPRS Vaihingen和Potsdam数据集上分别达到84.35%和86.86%的mIoU。

Conclusion: 显式建模空间-频率不一致性可有效提升多模态分割精度与表示一致性，验证了将空间精炼、频率对齐与置信度引导融合协同设计的有效性。

与遥感交叉或改进方案: 交叉/改进可行性：92%。该论文本身即为遥感多模态分割论文，可从以下方面改进：(1) 将CMSG扩展为可学习的高通滤波器，以适应不同地物类型对结构信息的需求差异；(2) ACFM中的径向掩模可引入可学习参数，自适应调整频率分离边界；(3) UAF可引入时序信息或多视角一致性约束，进一步提升不确定性估计的鲁棒性；(4) 可扩展至多模态(如SAR+光学+DSM)场景，验证框架的泛化能力。

Abstract (英文): Multimodal remote sensing segmentation commonly integrates optical imagery with Digital Surface Models (DSMs) to improve land-cover understanding. However, existing methods often overlook a critical issue, namely the inconsistency between spatial-domain structural details and frequency-domain semantics, which can lead to misaligned features and degraded performance in complex scenes. To address this problem, we formulate spatial–frequency representation conflict as a unified multimodal learning problem and propose the Conflict-Aware Fusion Network (CAF-Net). Specifically, Cross-Modal Structure Guidance (CMSG) extracts DSM-derived high-pass structural cues and conditionally modulates optical features to improve boundary consistency. The Adaptive Cross-Frequency Module (ACFM) separates DCT coefficients using a fixed radial mask, adaptively reweights low- and high-frequency components, and performs cross-modal alignment at the highest encoder stage. Uncertainty-Aware Fusion (UAF) predicts pixel-wise relative reliability scores and normalizes them across modalities to suppress low-confidence responses. This coordinated design links spatial refinement, frequency alignment, and reliability-guided fusion instead of treating them as independent feature-enhancement operations. Experiments on the ISPRS Vaihingen and Potsdam datasets yield mIoU scores of 84.35% and 86.86%, respectively. The results indicate that explicitly modeling spatial–frequency discrepancies can improve multimodal segmentation accuracy and representation consistency.

</details>


### [211] [Lightweight Raw Echo Image Preprocessing for Long-Range Airborne Streak Tube Imaging LiDAR Using Adaptive Frequency-Domain Noise Suppression](https://doi.org/10.3390/rs18142281)
*Chaowei Dong,Rongwei Fan,Zhaodong Chen,Zhiwei Dong,Deying Chen,Pengfei Hao,Lansong Cao*

Main category: Remote Sensing

TL;DR: 提出了一种轻量级的条纹管成像激光雷达回波图像预处理方法，通过行-列几何感知的候选区域分类与频域直方图自适应抑制，在不依赖神经网络或复杂后处理的情况下，将屋顶点云厚度降低约21%。


<details>
  <summary>Details</summary>
Motivation: 远距离机载条纹管成像激光雷达原始回波图像受大气散斑、探测器噪声和弱回波波动退化，影响质心定位精度，需要一种轻量级且有效的预处理方法来提升测距质量。

Method: 首先对归一化条纹图像提取候选区域，基于行-列形态学进行分类，然后利用局部幅度谱百分位阈值进行滤波，最后通过背景约束加权策略进行融合。

Result: 在建筑屋顶控制区域，平均高程RMSE从0.34 m降至0.29 m，MAE从0.30 m降至0.26 m，屋顶高程标准差从0.19 m降至0.15 m，点云厚度减少约21%。

Conclusion: 质心提取前的预处理能有效提升屋顶垂直一致性，且无需神经网络训练或复杂的点云后处理，方法轻量、实用。

与遥感交叉或改进方案: 交叉/改进可行性：95%。该方法可直接应用于星载或地基条纹管激光雷达的原始回波预处理，提升远距离探测精度。也可拓展到合成孔径激光雷达成像中的散斑抑制，或融合深度学习进行更鲁棒的候选区域自适应分类。

Abstract (英文): Long-range airborne streak tube imaging lidar (ASTIL) raw echo images are degraded by atmospheric speckle, detector noise, and weak-return fluctuations, which can bias centroid localization before range calculation. This study presents a lightweight preprocessing method combining row–column geometry-aware echo region pre-classification with frequency-domain histogram-based adaptive suppression. Candidate regions are extracted from normalized streak images, classified by row–column morphology, filtered using local magnitude-spectrum percentile thresholds, and fused with a background-constrained weighted strategy. Simulated echo images, simulated point clouds, and 6 km airborne data were used for validation. In selected building roof control regions, the mean elevation root mean square error (RMSE) decreased from 0.34 m to 0.29 m, the mean absolute error (MAE) from 0.30 m to 0.26 m, and the mean roof elevation standard deviation from 0.19 m to 0.15 m, corresponding to an approximately 21% reduction in roof-level point cloud thickness. The results show that preprocessing before centroid extraction can improve roof-level vertical consistency without neural-network training or complex point cloud post-processing.

</details>


### [212] [Comparing DESIS Hyperspectral and Landsat 10 Simulated Superspectral Data for Crop Type Classification in California’s Central Valley](https://doi.org/10.3390/rs18142282)
*Itiya Aneece,Prasad S. Thenkabail,Pardhasaradhi Teluguntla,Adam Oliphant,Daniel J. Foley,Jake Lawton*

Main category: Remote Sensing

TL;DR: 本研究比较了三种光谱配置（全波段DESIS高光谱、模拟Landsat 10窄波段、模拟Landsat 10宽波段）在加州中央谷地进行作物分类的效果。结果表明，使用SVM分类器结合全高光谱或14个Landsat 10窄波段均可达到86%的最高总体精度，显著高于宽波段的75%。研究指出，多时相观测对于更精细的作物分类至关重要，而Landsat 10超光谱影像在大区域农业监测中更具可扩展性。


<details>
  <summary>Details</summary>
Motivation: 推进全球粮食和水安全目标下的作物类型制图技术，比较不同光谱配置（高光谱、窄波段、宽波段）在大区域作物分类中的性能，评估Landsat 10超光谱卫星的实用潜力。

Method: 使用DESIS高光谱影像（2021-2023年8月数据）模拟三种光谱配置：全60波段HNB、14个与Landsat 10对应的窄波段、模拟Landsat 10宽波段。采用RF和SVM两种像素级监督分类器，在Google Earth Engine上对三大作物类别（行栽作物、葡萄与果树、冬小麦/休耕/其他）进行分类。

Result: SVM结合全高光谱或14个Landsat 10窄波段均达到86%的最高总体精度，显著高于模拟宽波段的75%（p值显著）。窄波段数据集在作物分类中具有明显优势。

Conclusion: 少量策略性选择的窄波段即可实现最优作物分类效果；Landsat 10超光谱影像虽然略逊于高光谱，但因其可提供规律性、跨季节的大区域覆盖，在大规模农业监测中更具实用性和可扩展性；未来更精细的作物分类需要覆盖完整物候周期的多时相观测。

与遥感交叉或改进方案: 交叉/改进可行性：90%。本文本身就是遥感作物分类论文。改进方向：1) 可引入深度学习（如1D-CNN或Transformer）替代传统RF/SVM，充分利用高光谱数据的谱间相关性；2) 可结合SAR数据（如Sentinel-1）弥补云覆盖时段的光学数据缺失，提升多时相观测的连续性；3) 可引入物候特征提取方法（如NDVI时间序列曲线拟合），增强对不同作物物候差异的捕捉能力；4) 可尝试迁移学习框架，将加州中央谷地的分类模型迁移到其他农业区域，验证模型的泛化能力。

Abstract (英文): To advance crop type mapping in support of global food and water security, this study compared three spectral configurations: (A) the full 60-band DLR Earth Sensing Imaging Spectrometer (DESIS) hyperspectral narrowband (HNB) dataset, (B) a 14-band subset of DESIS-derived HNBs aligned with the planned Landsat 10 (formerly Landsat Next) spectral configuration (400–1000 nm), and (C) DESIS-based simulations of Landsat 10 superspectral broadbands. The analysis was conducted in California’s Central Valley, hereafter referred to as “the Central Valley”, during the peak growing month of August. DESIS imagery from August 2021, 2022, and 2023 was used sequentially for model development, testing, and independent validation. Over these three years, DESIS provided extensive hyperspectral coverage of much of the 4 million hectares in the Central Valley’s. Analyses were performed on Google Earth Engine using two pixel-based supervised classifiers, Random Forest (RF) and Support Vector Machine (SVM), to differentiate three major crop classes: row crops, grapes and tree crops, and winter wheat/fallow/other. The highest overall accuracy (86%) was achieved using SVM in combination with either the full DESIS hyperspectral dataset or the 14 DESIS narrowbands corresponding to Landsat 10. This finding aligns with earlier studies showing a small number of strategically positioned narrowbands can be optimal for crop type classification. Use of the narrowband datasets resulted in substantially higher accuracy (overall accuracy of 86%) compared to the simulated Landsat 10 broadbands (overall accuracy of 75%), supporting previous studies highlighting the utility of narrowbands. Despite the high accuracy using August imagery, the study indicates more granular crop type classification will require multi-temporal observations spanning the full phenological cycle (June–October), especially for a large number of crop classes. Acquiring task-based hyperspectral imagery over such large areas throughout the growing season remains operationally challenging. In contrast, Landsat 10 superspectral imagery could provide routine coverage across seasons and years that is practical and scalable for future large area crop type mapping and agricultural monitoring.

</details>


### [213] [Earth Observation Data and Indigenous Perspectives: Two-Eyed Seeing Approach to Understanding Long-Term Wildfire and Landscape Changes](https://doi.org/10.3390/rs18132259)
*Sandeep K. Agrawal,Nilusha Welegedara,Tammy Steinwand,Tyanna Steinwand*

Main category: Remote Sensing

TL;DR: 本研究采用双重视角方法（Two-Eyed Seeing），将西方科学遥感和土著传统生态知识相结合，分析了加拿大西北地区Tłı̨chǫ领地的高纬度野火模式与土地变化。研究发现过去十五年间近半数森林转为其他土地覆盖类型（灌木林增两倍、草原扩十倍），春夏季地表温度显著上升，降水与积雪减少，表土湿度下降与升温形成正反馈加剧火灾风险。该研究为气候韧性和土著土地管理提供了跨知识体系的协作框架。


<details>
  <summary>Details</summary>
Motivation: 高纬度地区野火频发与气候变暖加速的背景下，传统单一科学方法难以全面理解复杂的人地耦合系统中野火演变规律，尤其是缺乏土著社区长期在地观察经验的整合。本研究旨在通过融合西方科学与土著传统生态知识，更精确地揭示火灾动态与土地覆盖变化及其驱动机制。

Method: 采用双重视角方法（Two-Eyed Seeing），整合两大类数据源：（1）地球观测数据：分析火烧迹地时空变化、地表温度、反照率、积雪覆盖、土壤湿度和土地覆盖类型；（2）土著知识：通过倾听与讲故事环节（listening and storytelling session）收集Tłı̨chǫ长老和官员的观察与见解，将两种知识体系进行编织整合。

Result: （1）Tłı̨chǫ领地近一半森林已转为其他土地覆盖类型，灌木林面积翻倍，草原面积扩大十倍；（2）春夏季地表温度显著上升（p<0.05），降水与积雪覆盖显著减少（p<0.05），与长老观察一致；（3）表土湿度下降与升温耦合，在林区形成了正反馈循环，加剧未来火灾风险；（4）火灾活动模式发生令人担忧的转变，主要驱动力为气候变化与土地覆盖变化的相互作用。

Conclusion: 双重视角方法不仅提高了分析精度，还为制定针对日益严重火灾格局和土地覆盖变化的缓减策略提供了协作框架。该研究对气候韧性和土著土地管理具有广泛意义，强调基于地方的整合研究对理解复杂野火行为的重要性。

与遥感交叉或改进方案: 交叉/改进可行性：90%。具体方案：（1）方法论层面：将Two-Eyed Seeing范式引入遥感社区，在火灾、土地利用/覆盖变化等研究中设计"知识共生产"流程，将遥感反演结果与社区在地知识进行交叉验证和偏差校正；（2）数据融合层面：利用遥感时间序列（如Landsat、MODIS、Sentinel-2）监测森林-灌木-草原转变过程，结合土著口述历史确定关键转折点和阈值；（3）模型优化：将土著观察到的火灾-植被-土壤湿度正反馈机制参数化，嵌入物理模型（如火灾行为模型、地表过程模型）中；（4）应用拓展：在加拿大北方森林、西伯利亚、阿拉斯加等高纬度其他土著领地推广该方法，构建跨区域的知识-遥感协同监测网络。

Abstract (英文): High-latitude regions are witnessing unprecedented wildfires and accelerated warming. This study explored wildfire patterns and land changes within the high-latitude Indigenous Tłı̨chǫ territory in the Northwest Territories, Canada. It used the Two-Eyed Seeing approach, which combines Western science, or Scientific Ecological Knowledge (SEK), with Indigenous knowledge, or Traditional Ecological Knowledge (TEK). This method integrated Earth observation data with insights from Tłı̨chǫ Elders and officials. We analyzed spatiotemporal variations in burned areas, land surface temperature (LST), albedo, snow cover, soil moisture, and land-cover types. A listening and storytelling session with community Elders provided an in-depth Indigenous perspective. Our findings indicate a concerning shift in wildfire activity on Tłı̨chǫ land, primarily driven by the interplay between climate change and land-cover changes. Land-cover estimates over the past fifteen years indicate that nearly half of the forested areas on Tłı̨chǫ-owned land have been converted to other land-cover types, with shrublands increasing twofold and grasslands expanding tenfold. We observed significant increases in spring and summer LSTs (p &lt; 0.05), alongside decreases in precipitation and snow cover (p &lt; 0.05), consistent with the Elders’ observations. The decline in topsoil moisture, coupled with rising temperatures, has triggered a positive feedback loop in forested areas, intensifying future wildfire risk. The study’s implications extend beyond the Tłı̨chǫ territory, suggesting a broader significance for climate resilience and Indigenous stewardship. It highlights the significance of place-based, integrated research for understanding complex wildfire behavior and land-cover transformations. The study indicates that the Two-Eyed Seeing approach, which weaves local Indigenous knowledge with quantitative Earth observations, not only improves analytical precision but also provides a collaborative framework for developing targeted strategies to mitigate the effects of increasingly severe fire regimes and land-cover changes.

</details>


### [214] [Intraspecific Drought Stress Responses Detected Using Remotely Sensed Vegetation Indices in Sitka Spruce (Picea sitchensis)](https://doi.org/10.3390/rs18142280)
*Gerrard English,J. Rosette,Richard Whittet,Juan Carlos Pinilla Suárez*

Main category: Remote Sensing

TL;DR: 本研究通过八周控制干旱实验，利用高光谱植被指数检测60棵 Sitka 云杉（代表6个无性系群体）的干旱响应，发现基于色素和红边的指数（如ARI1）对干旱最敏感，水相关指数捕捉后期水分下降，且无性系间存在显著的干旱响应差异，表明高光谱方法可用于快速、无损地筛选抗旱基因型。


<details>
  <summary>Details</summary>
Motivation: 气候变化加剧英国林业干旱风险，而Sitka云杉作为主要商业针叶树其抗旱性尚不清楚，需要开发高效、非破坏性的方法来检测种内干旱响应差异，以支持抗旱基因型的筛选和气候适应性森林管理。

Method: 对60棵嫁接的Sitka云杉（代表6个无性系群体）进行八周控制干旱实验，测量针叶级高光谱反射率并推导多种植被指数（包括叶绿素状态、光保护色素和水分含量相关指数），同时测量叶绿素荧光（Fv/Fm），分析不同无性系在干旱处理下的生理响应差异。

Result: 色素基和红边指数对干旱最敏感，ARI1在干旱条件下相对于对照组增加达1.2倍；水相关指数能捕捉后期水分下降；不同无性系在色素调节、水分损失和光合损伤的时间与幅度上存在显著差异，表明无性系间具有不同的干旱响应轨迹。

Conclusion: 高光谱方法能够快速、非破坏性地检测具有生理意义的干旱响应，可用于识别抗旱基因型以支持气候适应性森林管理。

与遥感交叉或改进方案: 交叉/改进可行性：85%。该研究本身即为遥感与植物生理/林业育种的交叉工作，可直接借鉴其方法。改进方案：(1) 将该方法扩展到卫星或无人机遥感平台，实现大面积抗旱基因型筛查；(2) 结合多时相Sentinel-2或高光谱卫星（如PRISMA、EnMAP）评估野外条件下的抗旱性；(3) 融合机器学习算法（如随机森林、CNN）提升对不同无性系干旱响应的分类精度；(4) 将高光谱指数与基因组学数据关联，发展高光谱辅助育种技术。

Abstract (英文): UK forestry faces increasing drought risk under climate change, raising concerns about the resilience of Sitka spruce, the UK’s dominant commercial conifer. This study assessed whether hyperspectral vegetation indices can detect intraspecific drought responses to support resilience screening. An eight-week controlled drought experiment was conducted on 60 grafted Sitka spruce trees representing six clonal groups from the UK breeding population. Needle-level hyperspectral reflectance was used to derive vegetation indices associated with chlorophyll status, photoprotective pigments, and water content, alongside chlorophyll fluorescence (Fv/Fm). Pigment-based and red-edge indices showed the strongest sensitivity to drought, with ARI1 increasing by up to 1.2 under drought relative to controls, while water-related indices captured later-stage hydraulic decline. Clonal variation was detected in the timing and magnitude of pigment regulation, water loss, and photosynthetic impairment, indicating contrasting drought-response trajectories among clones. These results demonstrate that hyperspectral approaches enable rapid, non-destructive detection of physiologically meaningful drought responses and can support the identification of drought-resilient genotypes for climate-adaptive forest management.

</details>


### [215] [Water Anomaly Type Identification Based on Deep Multisphere Decision Boundaries Using Remote Sensing Data](https://doi.org/10.3390/rs18142273)
*Jinfeng Song,Kun Jia,Haishuo Wei,Biao Cao,Xing Yan,Jianbo Qi,Kai Yan,Jinlong Fan,Qiao Wang*

Main category: Remote Sensing

TL;DR: 本文提出了一种基于深度多球决策边界（DMSDB）的水体异常统一细粒度识别方法。该方法首先从Sentinel-2影像构建5维物理可解释特征空间，然后利用改进的VM-UNet-v2网络提取水体异常响应特征（WARFs），最后通过多球对比损失函数优化类别决策边界，实现高精度快速推理。在10000 km²场景上，平均F1分数0.8789，mIoU 0.7932，推理仅需33秒。


<details>
  <summary>Details</summary>
Motivation: 现有水体异常类型识别方法难以兼顾高精度与快速推理，且依赖手工特征工程或计算复杂的后分类流程，导致处理速度慢、光谱相似异常类别易混淆。亟需一种统一、细粒度且高效的识别方法。

Method: (1) 从Sentinel-2影像推导物理可解释的指数，构建紧凑5维特征空间；(2) 采用改进的VM-UNet-v2网络提取水体异常响应特征（WARFs），捕获各异常类型的空间-光谱特征；(3) 设计多球对比损失函数，优化类别特定决策边界，压缩类内特征同时增大光谱相似类别间的分离度。

Result: 平均F1分数0.8789，mIoU 0.7932，在10000 km²场景上完成分类仅需33秒，实现了高效大规模推理。

Conclusion: DMSDB方法在精度与推理速度间取得了良好平衡，具备支撑快速环境监测、及时分类响应的潜力，尤其适用于大规模水域异常事件的早期预警与应急管理。

与遥感交叉或改进方案: 交叉/改进可行性：95%。该论文本身就是遥感领域的核心工作（基于Sentinel-2影像的水体异常识别）。改进方向：(1) 当前仅使用5维特征空间，可引入SAR（如Sentinel-1）或热红外波段构建多模态特征，提升云覆盖区域的鲁棒性；(2) 多球对比损失函数可进一步结合难例挖掘策略，针对极端不平衡的水体异常样本（如藻华面积大、油污面积小）优化；(3) 可扩展至时序遥感数据，利用时间维度的异常演变模式辅助识别，降低单时相光谱混淆风险；(4) VM-UNet-v2网络可替换为轻量级Transformer架构（如SwinUNet），在保持精度的同时进一步加速推理。

Abstract (英文): The increasing frequency of water anomalies poses severe threats to aquatic ecosystem stability, water resource security, and socioeconomic sustainability. Rapid and accurate identification of these events is critical for early warning and informed environmental decision-making. However, existing type identification methods often fail to balance high precision with rapid inference since many depend on hand-crafted feature engineering or computationally intensive post-classification procedures that slow processing and increase confusion among anomalies with similar spectral characteristics. Therefore, in this study, a deep multisphere decision boundaries (DMSDB) method for unified, fine-grained identification of diverse water anomalies is developed. The method first involves constructing a compact 5-dimensional feature space by deriving physically interpretable indices from Sentinel-2 imagery. Then, a modified VM-UNet-v2 network is employed to extract water anomaly response features (WARFs) that capture the spatial–spectral characteristics of each anomaly. Finally, a multi-sphere-contrast loss function optimizes class-specific decision boundaries, compacting intraclass features while increasing the separation between spectrally similar anomaly categories. The validation results demonstrate that DMSDB achieves an average F1 score of 0.8789 and an mIoU of 0.7932, enabling efficient large-scale inference, and completing classification over a 10,000 km2 scene within 33 s. These results highlight the method’s potential for effective or ground-based rapid environmental monitoring, supporting timely and category-specific responses to aquatic disturbances.

</details>


### [216] [Consistent-Innovation-Aided Distributed Cooperative Localization for Multi-UAV Navigation in GNSS-Denied Environments](https://doi.org/10.3390/rs18142286)
*Zhikuan Hou,Chao Xue,S T Chen,Chuan Xu,Changhui Jiang,Jiabao Niu*

Main category: Remote Sensing

TL;DR: 本文提出一种一致性创新辅助的分布式协同定位方法，用于GNSS拒止环境下多滑翔无人机集群导航。该方法以捷联惯导为参考源，引入超宽带测距作为协同约束，通过构建保守的创新协方差近似实现分布式量测更新，无需显式传播全局互协方差。结合高度、地磁和景象匹配子滤波器构成自适应联邦滤波框架。仿真表明，相比非协同方法，位置、速度和偏航角RMSE分别降低27.14%、21.90%和9.68%；与精确互协方差基准相比，精度相当但通信量减少约91.44%，且在20%丢包率下仍保持有界误差。


<details>
  <summary>Details</summary>
Motivation: 解决GNSS拒止环境下多无人机协同定位中集中式方法和精确互协方差分布式方法存在的全局状态管理复杂、通信和计算负担重的问题，提出一种轻量化、鲁棒的分布式协同定位方案。

Method: 提出一致性创新辅助的分布式协同定位方法：各节点仅维护本地状态和协方差，通过构造保守的创新协方差近似（Consistent Innovation Covariance Approximation）实现分布式量测更新，避免全局互协方差传播；结合捷联惯导（SINS）作为参考源，引入超宽带（UWB）测距作为节点间协同约束；通过自适应向量信息分配的联邦滤波框架融合高度、地磁和景象匹配子滤波器。

Result: 在GNSS拒止多滑翔机释放仿真中，相比非协同方法，位置RMSE降低27.14%，速度RMSE降低21.90%，偏航角RMSE降低9.68%；与精确互协方差分布式基准相比，定位精度相当，通信流量减少约91.44%；在20%丢包率下仍保持有界误差，鲁棒性显著提升。

Conclusion: 所提方法在保证定位精度的同时大幅降低通信和计算开销，对丢包具有较强的鲁棒性，为通信受限的无人机集群协同定位提供了可行的工程化方案。

与遥感交叉或改进方案: 交叉/改进可行性：85%。该论文提出的分布式协同定位框架可直接迁移至遥感无人机集群场景，如多架遥感无人机在GNSS拒止环境下（山地、城市峡谷等）进行协同测绘与数据采集。具体交叉方案：（1）将UWB测距协同约束替换或融合为遥感传感器间的共视区域几何约束（如重叠影像间的相对位姿估计），实现无GNSS条件下的协同定位与影像拼接；（2）联邦滤波框架可融合InSAR、LiDAR点云配准等遥感数据作为观测更新，增强定位精度；（3）一致性创新协方差近似方法可降低遥感集群中大量节点间的高频通信需求，适用于大范围、长航时的遥感测绘任务。

Abstract (英文): Cooperative localization provides a promising solution for multi-UAV navigation in GNSS-denied environments. However, centralized cooperative localization and exact cross-covariance-based distributed methods usually require global state management or explicit propagation of inter-node cross-covariance, resulting in heavy communication and computational burdens. To address this problem, this paper proposes a consistent-innovation-aided distributed cooperative localization method for multi-glider UAV swarms. The method uses a strapdown inertial navigation system as the reference source and introduces inter-node ultra-wideband ranging as cooperative constraints. Each node maintains only its local state and covariance, while a conservative innovation covariance approximation is constructed to perform distributed measurement updates without explicitly propagating global cross-covariance. The cooperative localization sub-filter is further integrated with altitude, geomagnetic, and scene-matching sub-filters through a federated filtering framework with adaptive vector information allocation. A GNSS-denied multi-glider release simulation is established to compare the proposed method with non-cooperative localization, centralized cooperative localization, and an exact-cross-covariance distributed baseline. The results show that the proposed method reduces the position, velocity, and yaw RMSEs by 27.14%, 21.90%, and 9.68%, respectively, compared with the non-cooperative method. Compared with the exact-cross-covariance baseline, the proposed method achieves comparable localization accuracy while reducing communication traffic by approximately 91.44%. Packet-loss experiments further show that the proposed method maintains bounded errors under a 20% packet loss rate, demonstrating improved robustness and engineering feasibility for communication-constrained UAV swarm localization.

</details>


### [217] [Revealing the Driving Mechanism of Urban Development Based on the 3D Building Morphological Changes in the Beijing–Tianjin–Hebei Region](https://doi.org/10.3390/rs18142276)
*C Yang,Hexiang Wang,Meiyuan Chen,Guang Zheng,Xiao Ma*

Main category: Remote Sensing

TL;DR: 本文提出了一种利用ALOS和GBH建筑高度数据过滤新建建筑物的方法，结合统计数据和面板数据，分析了2002-2020年京津冀地区13个城市的三维建筑形态时空变化及其驱动因素。


<details>
  <summary>Details</summary>
Motivation: 中国快速城市化导致城市建筑三维形态发生显著变化，进而引发空气污染、光污染、城市热岛效应和人地关系紧张等问题，亟需揭示城市三维形态变化模式及其影响因素。

Method: 利用ALOS和GBH建筑高度数据过滤新建建筑，结合统计数据和面板数据，对京津冀地区11个地级市和2个直辖市2002-2020年间建筑三维形态变化的时空格局及驱动因素进行分析。

Result: 京津冀地区同类型城市的三维形态变化机制相似；总人口对新建建筑区高度有负向影响；产业结构优化对新建地块高度有正向影响。

Conclusion: 基于研究发现提出了促进京津冀地区城市可持续发展的可操作性建议。

与遥感交叉或改进方案: 交叉/改进可行性：90%。本文提出的利用多源遥感数据（ALOS和GBH）过滤新建建筑的方法可直接应用于遥感领域的城市变化检测；可将该方法扩展至其他遥感数据源（如Sentinel-1/2、Landsat时序数据）进行多尺度城市三维形态监测；结合InSAR技术可进一步监测建筑沉降等垂直形变；驱动因子分析方法可迁移至其他遥感监测指标（如植被覆盖度、不透水面比例）的时空变化归因分析中。

Abstract (英文): Since the late 1970s, during the period of reform and opening-up, China has emerged as one of the fastest-urbanizing countries in the world. The high speed of urbanization and three-dimensional (3D) morphological changes in urban buildings may lead to significant urban challenges, including air and light pollution, the urban heat island effect, and strained human-land relationships. It is therefore crucial to elucidate the pattern of urban 3D morphological change and to investigate the factors that influence these changes. This study proposed a novel method of filtering new buildings using the ALOS and GBH building height data, combined with statistical data and panel data, focusing on spatial-temporal variations and their driving factors of 3D building morphological changes between 2002 and 2020 for 11 prefecture-level cities and 2 municipalities in the Beijing–Tianjin–Hebei (BTH) region. Our findings indicate that the 3D morphological change mechanisms of the same types of cities in the BTH region are similar. The total population negatively impacts the height of newly constructed building areas, and the optimization of the industrial structure positively influences the height of newly developed land. Based on our findings, we propose actionable recommendations to foster sustainable urban development in the BTH region.

</details>


### [218] [DWFSeg: A Dynamic Multiscale Feature Fusion and Dual Attention-Enhanced Network for High-Precision Water Body Segmentation Based on Super-Resolution Remote Sensing Imagery](https://doi.org/10.3390/rs18142271)
*Z Li,Bingjie Liang,Jianzhong Guo,N Li,Weiran Luo,Baowei Zhang,Jiali Guo,Weizhen Zhang,Yan Zhou,Y ouzhong Guo,YJ Li,Yuezhen Guo,Yishan Li*

Main category: Remote Sensing

TL;DR: 本文提出DWFSeg网络，结合Real-ESRGAN超分辨率重建和动态多尺度特征融合的Transformer架构，实现高精度遥感水体分割，在自制超分数据集和GID上分别达到98.08%和96.14%的总体精度。


<details>
  <summary>Details</summary>
Motivation: 遥感图像在空间分辨率与时间覆盖之间存在权衡，难以同时满足大范围、高时间分辨率和高空间分辨率的水体监测需求；此外，现有水体分割方法在复杂水文环境中面临水陆边界模糊、多尺度水体特征捕捉困难等问题。

Method: 1）采用Real-ESRGAN超分辨率算法将中分辨率遥感图像重建至2.5m分辨率；2）构建DWFSeg网络，包含MixVision Transformer (MVT)编码器和多尺度解码架构；3）提出动态多尺度特征融合机制(DMFF)，自适应分配语义引导的融合权重；4）设计双注意力增强模块(DAE)，在通道和空间维度增强判别性特征并抑制背景噪声。

Result: 在自建超分辨率影像数据集(SID)上OA达98.08%，在公开GID数据集上OA达96.14%，在多项量化指标上优于代表性基准模型，推理效率和分割稳定性良好。

Conclusion: DWFSeg网络为精细化水资源评估和可持续流域管理提供了可靠的技术方案。

与遥感交叉或改进方案: 交叉/改进可行性：95%。作为遥感领域的水体分割论文，可从以下方面改进：1）引入多时序数据以增强对季节性水体变化（如洪泛区、干涸河床）的鲁棒性；2）结合SAR数据（如Sentinel-1）进行多模态融合，解决云覆盖下的水体提取问题；3）优化超分模块对细窄河流（如宽度<2.5m）的重建保真度；4）考虑轻量化设计以适应星上或边缘端部署需求。

Abstract (英文): Remote sensing imagery provides a primary data source for large-scale surface water body monitoring, which is crucial for quantifying climate-related hydrological impacts, supporting flood control, and sustaining integrated water resource management. However, remote sensing images generally face the trade-off between spatial resolution and temporal coverage. To address this issue, the Real-ESRGAN super-resolution algorithm is employed to reconstruct temporally continuous, wide-coverage medium-resolution imagery to a 2.5 m resolution, effectively improving its capability to identify sub-pixel river boundaries. Water body segmentation (WBS) is an effective method for fine-detail surface water extraction. Nonetheless, when applied in complex hydrological environments, it still faces several limitations, such as ambiguous delineation of land–water boundaries and the difficulty in capturing multiscale water body characteristics. To address these issues, a Dynamic Weight Fusion SegFormer (DWFSeg) network is constructed, integrating a MixVision Transformer (MVT) encoder with a multiscale decoding architecture. Specifically, a Dynamic Multiscale Feature Fusion (DMFF) mechanism is proposed, which adaptively assigns semantic-guided fusion weights to multiscale feature water bodies. Furthermore, the Dual Attention-Enhanced (DAE) module strengthens discriminative essential features and suppresses background noise in both channel and spatial dimensions. Evaluated on a self-constructed super-resolution imagery dataset (SID) and the public GID, DWFSeg achieves overall accuracies of 98.08% and 96.14%, respectively. It outperforms representative benchmark models across multiple quantitative metrics, while maintaining competitive inference efficiency and favorable segmentation stability. Ablation studies verify the effectiveness and necessity of each proposed component. The presented network provides a reliable technical solution and supports refined water resource evaluation and sustainable watershed management.

</details>


### [219] [A Multi-Stage Outlier Removal Method for Point Clouds with High Outlier Ratio](https://doi.org/10.3390/rs18142289)
*Zihan Meng,Chengzhi Qu,Pengyu Chen,Yaji Tang*

Main category: Remote Sensing

TL;DR: MORPH是一种针对高离群率点云的多阶段离群点去除方法，结合局部可达密度、密度峰值聚类、结构自适应优化和一致性评分策略，在合成与真实点云上均表现出鲁棒的离群去除性能。


<details>
  <summary>Details</summary>
Motivation: LiDAR获取的点云常包含大量离群点，严重干扰下游处理。现有方法在高离群率点云上性能显著下降，亟需一种能有效处理高比例离群点的鲁棒去除方法。

Method: 提出MORPH多阶段离群去除方法：(1) 利用局部可达密度结合密度峰值聚类粗去除显著离群点，保留全局结构骨架；(2) 引入结构自适应优化阶段，生成不完整点云和候选点；(3) 提出结合逐对尺度一致性与局部平面残差评分的补全策略，从候选点中识别内点，并用内点填充不完整点云完成离群去除。

Result: 在合成高污染点云和真实扫描点云上的实验表明，MORPH实现了鲁棒的离群去除性能，同时更好地保留了有效的几何结构。

Conclusion: MORPH通过多阶段协同策略能有效处理高离群率点云，在去除离群点的同时保持良好的结构保持能力，优于现有方法。

与遥感交叉或改进方案: 交叉/改进可行性：90%。遥感领域LiDAR点云处理（如地形重建、建筑物提取、电力线检测）常面临高噪声与离群点问题，MORPH可直接应用于遥感LiDAR点云预处理。改进方向：(1) 融合多模态遥感数据（如高光谱影像）辅助判别离群点；(2) 针对大场景遥感点云优化局部可达密度的计算效率；(3) 引入地物先验知识（如地面、植被、建筑物分类）增强结构自适应阶段的语义约束。

Abstract (英文): Point clouds acquired by LiDAR typically contain a large number of outliers, which can substantially degrade downstream processing. Existing outlier removal methods suffer from significant performance degradation on point clouds with high outlier ratios. In this study, we propose a multi-stage outlier removal method called MORPH for point clouds with high outlier ratios. The proposed method first employs local reachability density combined with density-peak clustering to coarsely eliminate prominent outliers while preserving the global structural skeleton. Subsequently, a structure-adaptive refinement stage is introduced to generate incomplete point clouds and candidate points. Finally, a completion strategy that integrates pairwise scale consistency with local plane residual score is proposed to identify inlier points from the candidate points. Outlier removal is completed by using the obtained inlier points to fill the incomplete point clouds. Experimental results on synthetic high-contamination point clouds and real-world scanned point clouds demonstrate that MORPH achieves robust outlier removal performance while better preserving valid geometric structures.

</details>


### [220] [Synergistic SAR and Wide-Swath Interferometric Altimetry Observations for Estimating Flood Dynamics and Water Storage Variations in East Dongting Lake](https://doi.org/10.3390/rs18142283)
*Yixuan Li,Yunhua Zhang,Dong Li,Jiayi Song*

Main category: Remote Sensing

TL;DR: 本文提出了一种集成SAR和宽幅干涉测高（SWOT）的框架，用于重建2024年东洞庭湖洪水事件的时空演变与储水量动态。通过自适应空间约束模糊C均值（AS-FCM）算法提取洪泛范围，结合SWOT水位观测建立水位-面积关系（R²=0.937），估算峰值期累积储水量变化约10.7 km³。


<details>
  <summary>Details</summary>
Motivation: 大型河湖系统中洪水动态的精确表征至今仍是挑战，尤其是难以在快速变化的水文条件下同时捕捉洪泛范围和水面高程变化。

Method: 1) 利用Sentinel-1 SAR影像通过提出的AS-FCM算法（引入自适应空间正则化和结构感知邻域加权）提取高分辨率洪泛范围；2) 利用SWOT任务的KaRIn干涉仪获取二维水面高程观测；3) 通过偏置校正后的SWOT与实测水位数据约束，建立经验水位-面积关系（R²=0.937），重建每日洪水动态并估算累积储水量变化。

Result: AS-FCM方法总体精度93.6%，IoU 0.89，Kappa系数0.87；SWOT水位与实测相关系数0.988；水位-面积关系R²=0.937；峰值期累积储水量变化约10.7 km³。

Conclusion: 该集成框架在复杂河湖系统的洪水监测和水文储水评估中展现出强大潜力，揭示了东洞庭湖洪泛区在2024年洪水事件中的重要缓冲作用。

与遥感交叉或改进方案: 交叉/改进可行性：95%。该框架本身就是遥感洪涝监测的前沿工作。改进方向：1) AS-FCM可进一步引入时序SAR特征（如后向散射时序变化）提升分类鲁棒性；2) SWOT数据可与Sentinel-3 altimetry联合使用以增强时间分辨率；3) 可扩展至其他大型通江湖泊（如鄱阳湖）验证泛化性；4) 结合水文模型（如HEC-RAS）实现洪水演进物理过程约束。

Abstract (英文): Accurate characterization of flood dynamics in large river–lake systems remains challenging due to the difficulty of simultaneously capturing inundation extent and water surface elevation (WSE) variations under rapidly changing hydrological conditions. This study develops an integrated Synthetic Aperture Radar (SAR) and wide-swath interferometric altimetry framework to reconstruct the spatiotemporal evolution and storage dynamics of the 2024 flood event in the East Dongting Lake system, China. Sentinel-1 SAR imagery is utilized to derive high-resolution inundation extent, while the Surface Water and Ocean Topography (SWOT) mission, equipped with the Ka-band Radar Interferometer (KaRIn), provides two-dimensional WSE observations. To improve SAR-based flood extraction in heterogeneous floodplain environments, an Adaptive Spatially-Constrained Fuzzy C-Means (AS-FCM) algorithm is proposed by incorporating adaptive spatial regularization and structure-aware neighborhood weighting. Quantitative evaluation demonstrates that the proposed method achieves the highest performance among the evaluated conventional approaches, with an Overall Accuracy of 93.6%, an Intersection over Union of 0.89, and a Kappa coefficient of 0.87. The multi-temporal inundation sequence reveals a distinct flood evolution pattern characterized by rapid expansion during the rising stage and gradual recession during the post-peak period. SWOT-derived WSE observations exhibit strong agreement with synchronous in situ measurements after bias adjustment, with a correlation coefficient of 0.988. By integrating SAR-derived inundation extent with temporally matched water-level observations constrained by bias-adjusted SWOT and in situ gauge data, an empirical WSE–area relationship (R2=0.937) is established to reconstruct daily flood dynamics and estimate cumulative water storage variation. The results indicate that the East Dongting Lake floodplain played an important buffering role during the 2024 flood event, with cumulative storage variation reaching approximately 10.7km3 during the peak stage. Overall, the proposed framework demonstrates strong potential for flood monitoring and hydrological storage assessment in complex river–lake systems.

</details>


### [221] [Spectral Variability and Prediction of Nitrogen, Phosphorus, and Potassium in Fresh Sugarcane (Saccharum officinarum) Leaves by VIS–NIR–SWIR Spectroscopy](https://doi.org/10.3390/rs18132260)
*Marta Laura de Souza Alexandre,Izabelle de Lima e Lima,Carlos Augusto Alves Cardoso Silva,Mateus Lima Silva,Rodnei Rizzo,Peterson Ricardo Fiorio*

Main category: Remote Sensing

TL;DR: 本研究评估了利用VIS–NIR–SWIR叶片光谱预测甘蔗氮(N)、磷(P)、钾(K)含量的潜力，分析了两个生长季、不同物候期和五个品种的光谱变异。结果表明，N和P预测效果良好(N: R²=0.83; P: R²=0.80)，但K预测效果差(R²=0.25)，预测性能取决于数据集中光谱变异程度。


<details>
  <summary>Details</summary>
Motivation: 传统化学分析方法测定甘蔗养分含量耗时耗力，无法满足精准农业对大范围、快速监测的需求。因此，探索利用高光谱遥感技术快速、无损预测甘蔗N、P、K含量，为精准施肥提供科学依据。

Method: 1. 采集甘蔗叶片光谱(350-2500 nm)并测定N、P、K含量；2. 光谱预处理：MSC、平滑及Savitzky–Golay一阶导数；3. Spearman相关分析识别诊断波段；4. PLSR建立预测模型；5. PCA分析光谱变异的驱动因素(季节、物候、品种)。

Result: PCA显示光谱变异主要受季节和物候因素驱动，品种影响为次要。2023/2024季光谱均质性高，预测性能较低；2024/2025季光谱异质性高，N预测R²=0.83(RMSE=0.78 g/kg)，P预测R²=0.80(RMSE=0.10 g/kg)。K预测始终最差(R²最大仅0.25)。

Conclusion: VIS–NIR–SWIR光谱在预测甘蔗新鲜叶片N和P含量方面具有较大潜力，但模型鲁棒性依赖于数据集中光谱变异程度。K由于离子特性导致与光谱特征缺乏显著相关性，预测难度大。

与遥感交叉或改进方案: 交叉/改进可行性：85%。改进方案：1) 可引入K的间接预测策略，如结合土壤光谱或与N/P的交互效应建模；2) 采用深度学习或集成方法(如CNN、Stacking)增强对离子态元素的预测能力；3) 考虑叶片含水量、叶片结构等因素对K光谱响应的影响，进行物理模型校正；4) 结合无人机或卫星多时相数据，扩大光谱变异范围以提升模型泛化能力；5) 探索LIBS(Laser-Induced Breakdown Spectroscopy)与光谱融合的多源传感方案。

Abstract (英文): This study evaluated the potential of VIS–NIR–SWIR leaf spectroscopy to predict N, P, and K contents in sugarcane, considering spectral variability across two growing seasons (2023/2024 and 2024/2025), phenological stages, and five varieties. Spectral signatures (350–2500 nm) were acquired using a FieldSpec 3 spectroradiometer and processed with MSC, smoothing, and the first Savitzky–Golay derivative. Diagnostic bands were identified by Spearman correlation, and prediction was performed using partial least squares regression (PLSR). PCA showed that spectral variability was driven mainly by seasonal and phenological factors, whereas varietal effects were secondary. In 2023/2024, greater spectral homogeneity was associated with lower predictive performance. In 2024/2025, greater spectral heterogeneity was associated with improved prediction for N (R2 = 0.83; RMSE = 0.78 g kg−1) and P (R2 = 0.80; RMSE = 0.10 g kg−1). Potassium remained the most challenging nutrient to predict (maximum R2 = 0.25), mainly due to its ionic nature and the resulting lack of significant correlation with specific VIS, NIR, and SWIR spectral features. These results indicate strong potential for predicting N and P in fresh sugarcane leaves, although model robustness depends on the extent of spectral variability in the dataset.

</details>


### [222] [A Parcel-Level Asynchronous SpatioTemporal Framework for Cropping Pattern Classification in Fragmented Agricultural Landscapes](https://doi.org/10.3390/rs18142268)
*Liegang Xia,Li J,Xuanming Hu,Jiancheng Luo,Xiaodong Hu,Jiazhou Chen,Baiyang Ji,Qi Li*

Main category: Remote Sensing

TL;DR: 针对长江三角洲破碎化农业区域中地块级作物分类面临的异步多源观测、混合像元干扰和历史标签噪声问题，提出PAST（地块级异步时空分类框架），包含K-Shape标签质量控制、并行双分支分类和决策级融合三个阶段，在典型区域取得0.926的总体F1分数和0.906的小地块F1分数。


<details>
  <summary>Details</summary>
Motivation: 在长江三角洲等高度破碎化的农业区域，实现高精度地块级作物制图面临三个核心挑战：（1）多源遥感观测时间异步导致传统插值融合方法引入额外误差；（2）小地块内混合像元效应严重干扰分类精度；（3）使用历史档案记录作为训练标签时，年际间种植模式变化引入标签噪声，降低模型可靠性。现有方法难以同时应对这些挑战，亟需一种鲁棒的弱监督分类框架。

Method: 提出PAST（Parcel-level Asynchronous SpatioTemporal）三阶段框架：（1）K-Shape标签质量控制阶段，利用无监督时间序列聚类检测并修正历史标签中的噪声；（2）并行双分支分类阶段，时间分支实现无插值的Sentinel-1和Sentinel-2跨模态物候融合，图像分支从0.8米高分辨率影像提取冠层纹理特征以缓解混合像元干扰；（3）决策级融合阶段整合两分支结果。

Result: 在长江三角洲典型破碎化农业区域实验中，PAST总体F1分数达0.926，小地块（面积较小的田块）F1分数达0.906，显著优于主流时间序列基线方法，验证了三阶段流水线在弱监督历史档案标签条件下的有效性。

Conclusion: 将数据层面的K-Shape标签质量控制与模型层面的双分支抗干扰架构相结合，形成了完整的集成三阶段流水线，能够在弱监督历史档案标签条件下实现精细尺度的作物制图，为破碎化农业区域的高精度地块级分类提供了有效方案。

与遥感交叉或改进方案: 交叉/改进可行性：95%。改进方案：（1）K-Shape标签质量控制方法可直接应用于其他遥感分类任务（如土地覆盖、植被类型识别）中的训练标签噪声检测与修正；（2）无插值跨模态物候融合架构可推广至多传感器（如Landsat+Sentinel-2、光学+SAR）时间序列分析，减少预处理误差；（3）并行双分支设计（时序分支+高分辨率空间分支）为结合中低分辨率时序数据与高分辨率空间数据提供了通用范式，可扩展至城市精细分类、森林树种识别等领域；（4）决策级融合策略可替换为注意力机制或可学习融合权重以进一步提升性能。

Abstract (英文): High-accuracy parcel-level agricultural mapping is fundamental to precision agriculture. However, in fragmented agricultural regions of the Yangtze River Delta, identifying cropping patterns at the parcel level faces two compounding challenges: asynchronous multi-source observations and mixed-pixel effects in small parcels. When historical archive records are used as training labels, inter-annual cropping pattern changes further introduce label noise that undermines model reliability. To address these challenges and the label noise issue, we propose PAST (Parcel-level Asynchronous SpatioTemporal), a parcel-level cropping pattern classification framework comprising three stages: K-Shape-based label quality control, parallel dual-branch classification, and decision-level fusion. PAST employs a dual-branch architecture: the temporal branch achieves interpolation-free cross-modal phenological fusion of Sentinel-1 and Sentinel-2 data, while the image branch extracts canopy texture features from 0.8 m high-resolution imagery to partially address mixed-pixel interference. Experiments in a typical fragmented agricultural region of the Yangtze River Delta demonstrate that PAST achieves an overall F1 score of 0.926 and a small-parcel F1 score of 0.906, outperforming mainstream time-series baselines. These results confirm that combining K-Shape label quality control at the data level with a dual-branch interference-robust architecture at the model level provides a complete integrated three-stage pipeline for fine-grained crop mapping under weakly supervised historical archive label conditions.

</details>


### [223] [WCMNet: A Wavelet-Guided and CNN–Mamba Hybrid Network Approach for Unsupervised Domain Adaptation in Building Extraction](https://doi.org/10.3390/rs18132265)
*Dongjie Yang,Kuikui Han,Yuanwei Yang,Gao Xianjun,Kangliang Guo,Xinlong Gao,Ruijing Huang*

Main category: Remote Sensing

TL;DR: 本文提出WCMNet，一种基于小波引导的CNN-Mamba混合网络，用于无监督域自适应建筑物提取。通过小波域对齐低频频域风格并保留高频边缘结构，以及全局-局部Mamba模块联合建模局部纹理与全局语义依赖，在六个跨域迁移任务上平均IoU达65.13%，显著优于现有方法。


<details>
  <summary>Details</summary>
Motivation: 遥感图像采集条件和成像场景多样化导致建筑物提取模型在跨数据集应用中性能严重下降，传感器和场景特性的变化引起外观风格差异和特征分布偏移，亟需提升模型的跨域泛化能力。

Method: 1) Mamba Wavelet Alignment (MWA)模块：在小波域中对齐低频风格信息，同时保留方向性高频边缘结构，缓解跨域外观差异并减少结构退化；2) Global-Local Mamba Block (GLMB)：CNN分支捕获细粒度局部细节和边界线索，Mamba分支建模长距离上下文信息，通过自适应门控融合机制整合两类特征。

Result: 在WHU、Massachusetts和Potsdam数据集上的六个跨域迁移任务中，WCMNet均优于现有SOTA域自适应方法，平均IoU为65.13%，平均BIoU为74.80%，相比最强对比方法IoU提升最高达27.35个百分点，BIoU提升最高达38.32个百分点。

Conclusion: 所提出的MWA和GLMB模块有效提升了建筑物提取的完整性、边界描绘精度和跨域鲁棒性。

与遥感交叉或改进方案: 交叉/改进可行性：100%。本文本身就是遥感建筑物提取的域自适应论文，可改进方向：1) 将MWA模块扩展到更多遥感任务（如道路提取、变化检测）的域对齐中；2) 引入多尺度小波分解以更好地保留不同尺度的结构信息；3) GLMB中的门控融合机制可进一步与注意力机制结合，增强对难例（如阴影遮挡、密集建筑区）的建模能力；4) 探索将Mamba与更轻量的视觉骨干结合以提升推理效率。

Abstract (英文): With the increasing diversity of remote sensing image acquisition conditions and imaging scenarios, building extraction models often experience significant performance degradation in cross-dataset applications due to variations in sensors and scene characteristics. Improving their cross-domain generalization ability has therefore become a critical research problem. To address the challenges of appearance style discrepancy and feature distribution shift in cross-domain building extraction, this paper proposes WCMNet, a wavelet-guided and CNN–Mamba hybrid network for unsupervised domain adaptation in building extraction. Specifically, a Mamba Wavelet Alignment (MWA) module is designed to align low-frequency style information in the wavelet domain while preserving directional high-frequency edge structures, thereby mitigating cross-domain appearance discrepancies and reducing structural degradation during domain translation. In addition, a Global–Local Mamba Block (GLMB) is developed to jointly model local textures and global semantic dependencies. In GLMB, the CNN branch captures fine-grained local details and boundary cues, while the Mamba branch models long-range contextual information; an adaptive gated fusion mechanism further integrates the two types of features. Experimental results on six cross-domain transfer tasks across the WHU, Massachusetts, and Potsdam datasets demonstrate that WCMNet consistently outperforms existing state-of-the-art domain adaptation methods. In particular, WCMNet achieves an average IoU of 65.13% and an average BIoU of 74.80% across all transfer settings, with improvements of up to 27.35 percentage points in IoU and 38.32 percentage points in BIoU compared with the strongest competing methods. These results demonstrate that the proposed MWA and GLMB effectively improve building completeness, boundary delineation accuracy, and cross-domain robustness.

</details>


### [224] [RadarEchoMamba: A Fast, High-Fidelity Pyramidal Bidirectional Mamba Model for Radar Echo Extrapolation](https://doi.org/10.3390/rs18142287)
*耿焕同,Zhanpeng Shi,Jinzhong Min,Fangli Wu,Han Zhao*

Main category: Remote Sensing

TL;DR: RadarEchoMamba是一种基于Mamba模型的雷达回波外推框架，采用金字塔架构捕获多尺度特征，核心是双向时空Mamba骨干网络，结合时空先验模块指导解码阶段的高分辨率特征重建，在华南和上海-2020数据集上实现了与强Transformer基线相当的预测质量，同时参数更少、推理延迟更低，在SSIM和LPIPS等视觉保真度指标上表现更好。


<details>
  <summary>Details</summary>
Motivation: 现有基于RNN、CNN和Transformer的雷达回波外推模型存在误差累积、时序信息丢失和二次计算复杂度等问题，导致预测模糊和精细细节失真，亟需一种更高效的解决方案。

Method: 提出了RadarEchoMamba框架，包含三个关键设计：1）金字塔架构用于捕获多尺度特征；2）双向时空Mamba骨干网络建模历史观测窗口内的依赖关系；3）时空先验模块利用输入序列驱动解码阶段高分辨率特征的重建，减少预测模糊。

Result: 在华南和上海-2020数据集上，该方法在保持竞争性预测质量的同时，轻量级变体参数更少、推理延迟更低，在华南数据集上SSIM和LPIPS等视觉保真度指标有所提升。

Conclusion: RadarEchoMamba有效解决了传统外推模型的模糊和细节失真问题，Mamba架构在雷达回波外推任务中展现出高效性和竞争力，为降水临近预报提供了新方案。

与遥感交叉或改进方案: 交叉/改进可行性：85%。该文本身就是遥感领域（降水临近预报）的论文。改进方向：1）可将双向时空Mamba扩展为多模态融合模型，结合卫星云图、风场等遥感数据进一步提升外推精度；2）时空先验模块可引入物理约束（如大气运动方程）增强模型可解释性和物理一致性；3）金字塔架构可结合注意力机制进一步优化跨尺度信息融合；4）可探索将Mamba与传统ConvLSTM方法混合，在长时序依赖和局部细节保持之间取得更好平衡。

Abstract (英文): Radar echo extrapolation is a fundamental task in precipitation nowcasting. However, existing models based on RNNs, CNNs, and Transformers are often constrained by error accumulation, the loss of temporal information, and quadratic computational complexity. Consequently, these limitations can lead to prediction blurring and the distortion of fine-grained details in extrapolated radar images. To address these challenges, we propose RadarEchoMamba, a novel extrapolation framework built on the efficient Mamba model. RadarEchoMamba uses a pyramidal architecture to capture multi-scale features, and its core is a Bidirectional Spatiotemporal Mamba backbone that models dependencies within the observed historical input window. Furthermore, we introduce a spatiotemporal prior module driven by the input sequence to guide the reconstruction of high-resolution features during the decoding phase. This design enables the model to generate detail-rich predictions, improving upon the blurring issues prevalent in traditional extrapolation models. Experimental results on the South China and Shanghai-2020 datasets show the effectiveness of our method. Compared with strong Transformer-based baselines, the lightweight variant uses fewer parameters and achieves lower measured inference latency, while maintaining competitive prediction quality. Its main advantages are observed in visual-fidelity metrics, with improved SSIM and reduced LPIPS on the South China dataset.

</details>


### [225] [Cross-Platform Comparison of Marine Boundary Layer Cloud and Drizzle Properties over the Southern Ocean Using Airborne, Shipborne, and Satellite Observations](https://doi.org/10.3390/rs18132262)
*Anik Das,Xiquan Dong,Baike Xi*

Main category: Remote Sensing

TL;DR: 本研究利用SOCRATES飞机观测、MARCUS船舶观测和CloudSat卫星观测三种平台，对比分析了南大洋海洋边界层单层液态云的结构和微物理特性。通过发展基于原位滴谱分布的反射率-微物理反演框架，反演了云和毛毛雨的垂直廓线（数浓度、有效半径、液态水含量）。结果表明三种平台在云边界高度和微物理特性上总体一致，但CloudSat因粗垂直分辨率和低灵敏度存在较大偏差，尤其对500米以下低云探测有限。


<details>
  <summary>Details</summary>
Motivation: 南大洋海洋边界层云对辐射和降水有重要影响，但其垂直结构和微物理特性在不同观测平台间缺乏系统对比和约束，导致对其认知不充分。

Method: 利用SOCRATES飞机原位观测的云滴谱分布建立经验反射率-微物理反演框架，将该框架分别应用于MARCUS船舶M-WACR雷达和CloudSat CPR雷达的反射率观测，反演云和毛毛雨的垂直廓线（数浓度、有效半径、液态水含量），并从云边界高度、垂直分辨率、灵敏度等方面进行多平台交叉比较。

Result: 三种平台在云边界高度和反演的微物理特性上总体一致；CloudSat因垂直分辨率较粗和反射率灵敏度较低，偏差较大，尤其对500米以下低云探测能力有限；观测到的垂直结构与凝结增长、夹卷和碰并过程一致。

Conclusion: 不同观测平台在云和毛毛雨特性上具有广泛一致性，但仪器灵敏度、垂直分辨率和采样差异显著影响云边界检测和微物理反演结果。

与遥感交叉或改进方案: 交叉/改进可行性：90%。该文的反射率-微物理反演框架可直接迁移至遥感云微物理反演中。具体方案：(1)将基于飞机实测滴谱建立的经验反演关系应用于星载云雷达（如EarthCARE的CPR）或地基云雷达数据，提升南大洋及其他海域云微物理反演精度；(2)结合多波段卫星被动遥感（如MODIS、VIIRS）与主动雷达联合反演，弥补单一传感器在低云探测和垂直分辨率的不足；(3)利用该框架评估和改进模式中云微物理参数化方案，尤其是在南大洋高纬度区域的云滴数浓度和有效半径模拟。

Abstract (英文): Marine boundary layer (MBL) clouds strongly influence radiation and precipitation over the Southern Ocean (SO), yet their vertical structures and microphysical properties remain poorly constrained across observational platforms. This study compares macrophysical and microphysical properties of single-layer, liquid-dominant MBL clouds below 3 km using aircraft observations from the SO Clouds, Radiation, Aerosol Transport Experimental Study (SOCRATES), ship-based observations from Measurements of Aerosols, Radiation, and Clouds over the SO (MARCUS), and satellite observations from CloudSat. An empirical reflectivity–microphysics retrieval framework developed from in situ droplet size distributions (DSDs) measured during SOCRATES was applied to MARCUS M-WACR and CloudSat CPR reflectivity observations to retrieve vertical profiles of number concentration (N), effective radius (re), and liquid water content (LWC) for cloud and drizzle particles. Cloud boundary heights and retrieved microphysical properties show broad agreement across the three platforms within the limitations imposed by instrumental sensitivity, sampling differences, and retrieval uncertainties. However, CloudSat CPR observations exhibit larger deviations because of their coarser vertical resolution and lower reflectivity sensitivity, including limited detection of low clouds below ~500 m. The observed vertical structures are consistent with condensational growth, entrainment, and collision–coalescence processes. Overall, the results demonstrate broad consistency in cloud and drizzle properties across the three platforms, while highlighting the impacts of instrumental sensitivity, vertical resolution, and sampling differences on cloud boundary detection and microphysical retrievals.

</details>


### [226] [Attention-Guided Generative Adversarial Network for False Alarm-Resistant Change Detection in Remote Sensing Orthophotos](https://doi.org/10.3390/rs18142290)
*Y HU,Zheng Ji,Wei Liu,Yichao Li*

Main category: Remote Sensing

TL;DR: 提出了一种名为Attention-GAN的注意力引导条件对抗框架，用于正射影像变化检测中的虚假警报抑制。该框架通过多尺度空间注意力、通道注意力和PatchGAN判别器，结合扰动负样本训练策略，有效区分真实地表变化与几何/辐射扰动引起的伪变化，在LEVIR-CD、WHU-CD和DSIFN-CD数据集上取得竞争性结果，虚警率降至4.9%。


<details>
  <summary>Details</summary>
Motivation: 实际多时相正射影像在标准预处理后仍存在几何和辐射差异（如微小配准误差、建筑位移、阴影移动、光照变化），这些伪变化会产生类似边缘的响应，导致大量虚假警报，增加人工校验成本并降低变化图可靠性。现有方法难以有效区分真实地表变化与这类扰动引起的伪变化。

Method: 提出Attention-GAN框架，包含：(1)多尺度空间注意力模块和通道注意力模块，增强对真实变化的关注；(2)PatchGAN判别器，提高局部判别能力；(3)关键创新——扰动负样本训练策略，对未变化图像对施加受控的几何和辐射扰动并分配全零变化掩码，训练模型抑制虚假响应。

Result: 在LEVIR-CD、WHU-CD和DSIFN-CD上F1分数分别为91.2%、92.45%和93.18%；消融实验中扰动负样本验证集上的虚警率降至4.9%；重复运行统计和消融实验表明训练策略主要通过减少虚警提升鲁棒性。

Conclusion: 受控扰动负样本是一种可复现的训练和评估虚警抑制能力的方法，但该方法仍需在真实多视角、季节性变化和跨传感器失真场景下进一步验证。

与遥感交叉或改进方案: 交叉/改进可行性：95%。该论文本身就是遥感变化检测领域的优质工作，可进一步改进的方向包括：(1)将扰动类型扩展至更真实的跨时相场景（如季节变化、传感器差异），提升泛化能力；(2)引入多模态遥感数据（如SAR-光学融合），利用SAR对几何扰动的稳定性互补光学数据的虚警问题；(3)探索自监督或对比学习方式生成更丰富的扰动负样本，减少对人工设定扰动参数的依赖；(4)结合图神经网络建模地物空间拓扑关系，进一步区分建筑位移等结构伪变化与真实变化。

Abstract (英文): Orthophoto change detection is used to find real land cover changes in urban monitoring, disaster assessment, and environmental management. In practice, however, multi-temporal orthophotos are rarely identical in geometry and radiometry even after standard preprocessing. Small residual misregistration, local building displacement, shadow movement, and illumination differences can produce edge-like responses that look like change but do not correspond to any land cover transition. These false alarms increase manual checking costs and reduce the reliability of change maps. This study addresses that practical problem by proposing an attention-guided conditional adversarial framework, named Attention-GAN, for false alarm-resistant orthophoto change detection. The aim is not to detect small perturbations as changes but to detect real land cover changes while suppressing responses to nuisance variations that should be treated as unchanged. The framework integrates a multi-scale spatial attention module, a channel attention module, and a PatchGAN discriminator. It also introduces perturbation-negative training pairs, where controlled geometric and radiometric perturbations are applied to unchanged image pairs and assigned all-zero change masks. Experiments on LEVIR-CD, WHU-CD, and DSIFN-CD show competitive or moderately higher accuracy than the selected representative baselines, with F1 scores of 91.2%, 92.45%, and 93.18%, respectively. In the ablation experiment, the false change rate on perturbation-negative validation pairs is reduced to 4.9%. Repeated-run statistics and ablation results indicate that the proposed training strategy mainly improves robustness by reducing false alarms under the evaluated perturbation range. The results support the use of controlled nuisance perturbations as a reproducible way to train and evaluate false alarm resistance, while broader validation under real multi-view, seasonal, and cross-sensor distortions remains necessary.

</details>


### [227] [Comparative Scientometric Assessment of Remote Sensing Journals in 2025 Across Seven Performance Indicators](https://doi.org/10.3390/rs18142267)
*Prasad S. Thenkabail,Dongdong Wang*

Main category: Remote Sensing

TL;DR: 该研究系统梳理了2025年遥感领域66本SCIE/SSCI期刊的出版特征，构建了包含期刊定位、领域范围、影响因子、审稿周期、出版模式及开放获取政策的多维分析框架，旨在帮助研究人员高效识别目标期刊并提升投稿成功率。


<details>
  <summary>Details</summary>
Motivation: 遥感领域期刊数量快速增长，研究人员在投稿时面临期刊选择困难，缺乏系统性的期刊特征对比工具，导致投稿效率低下。

Method: 对66本SCIE/SSCI索引的遥感期刊进行系统分析，提取期刊定位、领域范围、影响因子、审稿周期、出版模式、开放获取政策等关键特征，并构建多维度分析框架。

Result: 构建了遥感期刊的多维特征数据库，揭示了不同期刊在影响因子、审稿效率、出版模式等方面的显著差异，为研究人员提供了可量化的期刊选择参考。

Conclusion: 该分析框架能有效辅助研究人员缩小期刊选择范围，匹配最合适的发表平台，从而提高投稿成功率。

与遥感交叉或改进方案: 交叉/改进可行性：100%。本文本身就是遥感领域的元研究（期刊分析），可直接服务于遥感研究者的投稿决策。改进方向：可进一步纳入期刊对特定遥感数据源（如Sentinel、Landsat、MODIS）的偏好分析、对深度学习等新兴方法的接受度、以及审稿人库的地域分布等维度，提升框架的实用性。

Abstract (英文): The remote sensing publishing ecosystem continues to expand rapidly, with 66 journals indexed in 2025 [...]

</details>


### [228] [A Novel Photogrammetry-Based Data Generation Technique for Post-Disaster Human Detection in UAV Imagery](https://doi.org/10.3390/rs18142272)
*Masood Varshosaz,Kamyar Hassanpoor,Vahid Mousavi,Xuying Liu,Sheng Feng*

Main category: Remote Sensing

TL;DR: 针对无人机航拍图像中人体检测标注数据不足的问题，提出利用摄影测量技术重建三维人体模型，在可控渲染环境中生成多视角合成图像以增强训练数据集。实验表明，该方法将分类准确率从88.06%提升至91.08%（提升3.02个百分点），结合标准数据增强后可达95.21%。


<details>
  <summary>Details</summary>
Motivation: 无人机航拍人体检测在灾后救援中至关重要，但面临标注训练数据匮乏的瓶颈。现有数据增强策略对视角变化的改进有限，无法有效模拟无人机在多种高度和角度下的拍摄场景，导致训练集多样性不足。

Method: 1) 利用摄影测量技术从多视角图像重建三维人体模型；2) 将三维模型置于可控渲染环境中，生成涵盖不同海拔高度和相机角度的合成图像；3) 将3000张合成图像（按类别平衡）添加至4000张基准图像，构建7000张的3DG增强数据集；4) 使用自定义CNN深度卷积分类器进行训练和评估。

Result: 在4000张基准图像（128×128像素，2800训练/600验证/600测试）上，摄影测量增强将分类准确率从88.06±0.97%提升至91.08±1.03%（提升3.02个百分点）。结合标准增强（旋转、平移、缩放、翻转）后，准确率达95.21±0.61%，较基准提升7.15个百分点。

Conclusion: 摄影测量驱动的数据增强是一种实用且有效的方法，能够提升基于无人机的行人检测管线的性能，在需要快速可靠识别（如灾后救援）的应用场景中具有实际价值。

与遥感交叉或改进方案: 交叉/改进可行性：95%。本文方法可直接应用于遥感领域：1) 将摄影测量三维重建拓展至车辆、建筑物等典型遥感目标，生成多视角合成数据以缓解遥感图像标注样本稀缺问题；2) 可结合卫星与无人机多源数据，利用三维重建模型模拟不同传感器高度和入射角下的成像特征，增强遥感分类/检测模型的视角鲁棒性；3) 引入域适应技术缩小合成图像与真实遥感图像之间的域差异，进一步提升迁移效果。

Abstract (英文): Recently, deep learning has enabled unmanned aerial vehicles (UAVs) to detect human bodies in aerial imagery, which is of particular importance in post-disaster situations such as floods and storms. Yet progress in this domain remains constrained by a familiar obstacle: the shortage of annotated training data. Neural networks, while powerful, are highly sensitive to data volume and diversity. Existing augmentation strategies help reduce this gap but typically introduce only incremental novelty, especially with respect to viewpoint variation, thereby limiting dataset richness. In this work, we propose a complementary strategy that leverages three-dimensional human models reconstructed via photogrammetric techniques. By situating these models within a controlled rendering environment, we generate synthetic imagery across a broad range of elevations and camera angles—perspectives that are rarely captured in conventional UAV datasets. These additions are designed to increase both the variability and the resilience of the training corpus. To evaluate the contribution of this approach, a custom CNN deep convolutional neural classifier was trained and benchmarked on a UAV human vs. non-human patch dataset of 4000 baseline images (128 × 128 px; 2800 train, 600 validation, 600 test), expanded with 3000 photogrammetry-derived synthetic patches (balanced by class) to 7000 total images for the 3DG setting. The primary metric was classification accuracy on the held-out test set, consistent with patch-level evaluation practice; detection-style metrics such as AP/IoU were not applicable to this binary classification protocol. Averaged over five independent training runs, the proposed augmentation improved classification accuracy by 3.02 percentage points over the baseline (88.06 ± 0.97% → 91.08 ± 1.03%), with consistent gains in precision, recall, and F1-score. When combined with standard augmentations (rotation, translation, scaling, flipping), accuracy reached 95.21 ± 0.61%, a gain of 7.15 percentage points over the baseline. These results suggest that photogrammetry-based augmentation offers a practical and effective enhancement for UAV-based human detection pipelines where timely, reliable identification is critical.

</details>


### [229] [Using UAV-Based RGB and Multispectral Imagery to Estimate Cotton Above-Ground Biomass by Integrating Multi-Modal Features and Machine Learning Algorithms](https://doi.org/10.3390/rs18142278)
*Madjebi Collela Be,Jie Zhang,Beifang Yang,Shengping Liu,Yingchun Han,Yaping Lei,Xiaoyu Zhi,Shiwu Xiong,Yahui Jiao,Yunzhen Ma,Shilong Shang,Antsa Sarobidy Randrianantenaina,Hamad Khan,Haoshen Zhang,Yaru Wang,Tao Lin,Yabing Li*

Main category: Remote Sensing

TL;DR: 本研究利用无人机RGB和多光谱影像，结合机器学习与递归特征消除（RFECV），评估了不同生长阶段棉花地上生物量的估算效果，发现模型表现受生长阶段、传感器配置和特征组合的强烈影响，苗期和蕾期精度较高，花期因冠层复杂性和光谱饱和而下降，并提出了分阶段自适应传感器与特征选择策略的重要性。


<details>
  <summary>Details</summary>
Motivation: 棉花的实时地上生物量监测对作物生长监测和管理优化至关重要，现有方法在跨生长阶段、不同种植密度和播种日期下的估算精度仍存在不足，需要系统评估多源传感器与特征组合的适用性。

Method: 在中国安阳进行田间实验，设置不同种植密度和播种日期，利用无人机获取RGB和多光谱影像；提取光谱特征、植被指数和GLCM纹理特征，构建RGB+MS、RGB-only、MS-only三种特征场景；采用RFECV进行特征选择，评估六种机器学习模型的表现，并利用SHAP分析特征贡献。

Result: 模型性能受生长阶段、传感器配置和特征组成强烈影响；苗期和蕾期精度最高，花期因冠层复杂性和光谱饱和而精度下降；MS-only和融合特征在苗期表现最佳，RGB-only在蕾期具有竞争力或更优，RGB-MS融合特征在花期表现最稳定；RFECV在早期生长阶段提升性能，后期效果不稳定；SHAP分析显示特征贡献从苗期纹理主导转向蕾期平衡贡献，再到花期植被指数主导。

Conclusion: 棉花AGB估算是一个依赖生长阶段的过程，需要采用自适应传感器和特征选择策略。

与遥感交叉或改进方案: 交叉/改进可行性：95%。该论文本身就是典型的遥感应用研究，具备以下可改进方向：（1）引入SAR或LiDAR数据以弥补光学遥感在花期冠层复杂时的饱和问题；（2）考虑时序深度学习方法（如LSTM、Transformer）以捕捉生长阶段间的动态变化；（3）引入迁移学习或域自适应方法，提升模型在不同地域、品种和气候条件下的泛化能力；（4）结合物理模型（如PROSAIL）进行混合建模，增强可解释性和鲁棒性；（5）探索更高效的特征选择算法（如贝叶斯优化、遗传算法）替代RFECV以提升后期稳定性。

Abstract (英文): Real-time monitoring of cotton above-ground biomass (AGB) is crucial for monitoring crop growth and optimizing management practices. This study evaluated UAV-based RGB and multispectral (MS) imagery for cotton AGB estimation across multiple growth stages under different planting densities and sowing dates in Anyang, China. Spectral features, vegetation indices (VIs), and Gray Level Co-occurrence Matrix (GLCM) texture metrics were extracted and organized into three scenarios: RGB + MS, RGB-only, and MS-only. Recursive feature elimination with cross-validation (RFECV) was applied for feature selection, and six machine learning models were evaluated using both baseline and selected feature sets. Results showed that model performance was strongly influenced by growth stage, sensor configuration, and feature composition. Accuracy was highest at the seedling and squaring stages and decreased at flowering due to canopy complexity and spectral saturation. MS-only and fused features generally performed best at the seedling stage, while RGB-only features were competitive or superior at the squaring stage, highlighting the importance of high-resolution structural information. At flowering, fused RGB–MS features provided the most stable performance, although improvements were limited. RFECV exhibited stage-dependent behavior, improving performance mainly at early growth stages but showing inconsistent benefits later. SHAP analysis revealed a shift from texture-dominated predictors at the seedling stage to balanced feature contributions at squaring and vegetation index (VIs) dominance at flowering. Overall, cotton AGB estimation is a stage-dependent process requiring adaptive sensor and feature selection strategies.

</details>


### [230] [Spatiotemporal Dynamics and Influencing Factors of Landscape Ecological Risk in the Shandong Peninsula Urban Agglomeration Based on Sub-Watershed Units](https://doi.org/10.3390/rs18132266)
*Jue Xiao,Linyu Ma,Longqian Chen,Ting Zhang,Gan Teng*

Main category: Remote Sensing

TL;DR: 本文开发了一种集成了遥感生态指数和丰度指数的综合景观生态风险(LER)模型，通过比较渔网、六边形、子流域和县域四种空间单元，发现子流域单元在LER评估中具有更强的鲁棒性和稳定性。对山东半岛城市群2004-2024年的分析显示，LER呈波动但总体略增趋势，呈现西低东高的空间格局，地形起伏度(29.13%)和夜间灯光(17.78%)是主导影响因素。


<details>
  <summary>Details</summary>
Motivation: 景观生态风险(LER)评估对空间划分敏感，存在不确定性，需要确定最优的空间单元以降低评估不确定性，并量化影响LER的关键因素及其非线性效应。

Method: 构建集成遥感生态指数(RSEI)和丰度指数的综合LER模型；对比渔网、六边形、子流域、县域四种空间单元效应；采用增强回归树(BRT)模型量化影响因素相对重要性及非线性响应。

Result: (1)子流域单元在多评价指标下表现出更强的鲁棒性和稳定性；(2)2004-2024年SPUA的LER呈波动但总体略增趋势，空间上西低东高；(3)地形起伏度(29.13%)和夜间灯光(17.78%)是主导因素，分别呈现倒U型响应和饱和非线性增加。

Conclusion: 子流域单元是LER评估的适宜空间单元，研究为城市化地区的地形敏感保护和可持续土地管理提供了见解。

与遥感交叉或改进方案: 交叉/改进可行性：85%。本文本身就是遥感与生态风险评价的交叉研究。改进方向：(1)可引入更多遥感衍生指标如植被净初级生产力(NPP)、蒸散发等增强RSEI的综合表征能力；(2)可结合Sentinel-1 SAR数据评估地形复杂区域的风险响应；(3)可引入时序遥感数据构建多尺度空间单元对比框架，提升模型对不同地貌区的适用性；(4)BRT模型可进一步与空间自回归、地理加权回归等空间统计方法结合，捕捉空间异质性效应。

Abstract (英文): Quantifying landscape ecological risk (LER) using multi-period land use data and ecological indicators is essential for understanding regional ecological dynamics. However, LER assessment is sensitive to spatial delineation, introducing uncertainty. This study developed an integrated LER model that incorporates the remote sensing ecological index and abundance index, and evaluated spatial unit effects through comparative analyses of fishnet, hexagonal, sub-watershed, and county units. LER dynamics in the Shandong Peninsula Urban Agglomeration (SPUA) from 2004 to 2024 were analyzed, and a boosted regression trees model was applied to quantify the relative importance of influencing factors and their nonlinear effects. The results indicate that: (1) sub-watershed units showed greater robustness and stability across multiple evaluation indicators, supporting their suitability for LER assessment; (2) LER in the SPUA exhibited a fluctuating but overall slightly increasing trend over the past two decades, with a persistent west-low and east-high spatial pattern; and (3) relief degree (29.13%) and nighttime light (17.78%) were the dominant factors shaping LER, showing an inverted U-shaped response and a saturating nonlinear increase, respectively. This study supports the use of sub-watershed units as an appropriate spatial unit for LER assessment and provides insights into terrain-sensitive conservation and sustainable land-use management in urbanizing regions.

</details>


### [231] [A Novel Target Extraction and Energy-Balancing Method for HoloSAR 3D Imaging](https://doi.org/10.3390/rs18142274)
*Yulong Xue,Leping Chen,Daoxiang An*

Main category: Remote Sensing

TL;DR: 针对HoloSAR中角度依赖散射响应导致的动态范围失衡问题，提出联合统计滤波框架，融合变异系数、子孔径间相关性和谱熵进行自适应判别加权，结合高斯混合模型贝叶斯阈值筛选与分位数匹配能量均衡变换，有效压缩动态范围、抑制背景噪声并恢复弱窄角结构。实验表明TBR提升0.7 dB（达11.2 dB），BSR达-5.2 dB，SCI提升156%。


<details>
  <summary>Details</summary>
Motivation: HoloSAR传统子孔径TomoSAR重建与非相干叠加后，由于角度依赖散射响应导致宽角强散射体被反复放大而窄角弱结构被噪声淹没，产生严重的动态范围失衡问题，影响3D重建的完整性与可解译性。

Method: 提出联合统计滤波框架：1）融合变异系数、子孔径间相关性和谱熵三种统计量进行自适应判别加权；2）基于高斯混合模型的贝叶斯最优阈值进行目标筛选；3）对弱目标像素应用分位数匹配能量均衡变换，将其能量自适应缩放至主目标参考水平，同时保持相对幅度关系。

Result: 在Ku波段无人机圆迹SAR真实数据上，TBR提升0.7 dB至11.2 dB，BSR达-5.2 dB，SCI提升156%至1428.1，原始超过50 dB的动态范围得到有效压缩，ENL约7.4×10⁻³保持场景物理真实性。

Conclusion: 所提方法有效压缩动态范围、抑制背景噪声并恢复传统非相干叠加中丢失的弱窄角结构，获得更完整、可解译性更强的HoloSAR 3D重建结果。

与遥感交叉或改进方案: 交叉/改进可行性：95%。该论文本身就是遥感SAR领域的全息合成孔径雷达（HoloSAR）三维重建方法。改进方案：1）可将联合统计滤波框架扩展到多基线InSAR或极化SAR的3D重建中，处理极化-角度联合依赖的散射响应失衡问题；2）高斯混合模型可替换为更鲁棒的变分贝叶斯或深度聚类方法以适应复杂地物场景；3）分位数匹配能量均衡可引入空域正则化约束（如全变分或非局部均值），在弱目标恢复中增强空间连续性并抑制噪声伪影。

Abstract (英文): Holographic synthetic aperture radar (HoloSAR) enables 360° three-dimensional reconstruction by incoherently stacking tomographic subaperture images. However, after conventional subaperture-wise TomoSAR reconstruction and non-coherent integration, the resulting 3D imagery suffers from severe dynamic range imbalance due to angle-dependent scattering responses: wide-angle strong scatterers are repeatedly amplified, whereas narrow-angle weak structures are buried below the noise floor. To address this post-processing challenge, we propose a joint statistical filtering framework operating on the reconstructed subaperture-domain 3D images that fuses the coefficient of variation, inter-subaperture correlation, and spectral entropy with adaptive discriminative-power weighting; target screening is then performed via a Gaussian mixture model-based Bayesian optimal threshold. For pixels classified as weak targets, a percentile-matching energy-balancing transformation is applied to adaptively rescale their energy to the main-target reference level while preserving relative amplitude relationships. Experiments on real-world Ku-band UAV circular SAR data demonstrate that the proposed method effectively compresses the dynamic range, suppresses background noise, and recovers weak narrow-angle structures that are lost in traditional non-coherent superposition, yielding more complete and interpretable HoloSAR 3D reconstructions. Quantitative evaluation on Ku-band UAV circular SAR data demonstrates that the proposed method improves the Target-to-Background Ratio by 0.7 dB (to 11.2 dB), achieves a Background Suppression Ratio of −5.2 dB, increases the Structural Completeness Index by 156% (to 1428.1), and compresses the original dynamic range imbalance, which exceeds 50 dB, while preserving scene physical realism (ENL ≈ 7.4 × 10−3).

</details>


### [232] [Automated Victim Detection from UAV Thermal Infrared Imagery for Nighttime Search and Rescue Using Multi-Pose Ground Camera Data](https://doi.org/10.3390/rs18142279)
*Shiori Kubo,Koudai Yamada,Hidenori Yoshida*

Main category: Remote Sensing

TL;DR: 本文提出了一种基于Grounding DINO深度学习模型的无人机热红外图像人员检测方法。为解决无人机训练数据稀缺问题，使用地面固定点热红外相机数据训练，通过空间缩放和填充模拟无人机视角以弥合地-空域差异。该方法在真实数据集上优于基线，显著提升了低热对比环境下的召回率。消融实验验证了各模块的有效性。与YOLOv8n和RT-DETR对比表明，鲁棒性源于Grounding DINO的语言引导语义先验，而非单纯Transformer架构。夜间影像扩展评估证实该方法具有良好的泛化能力。


<details>
  <summary>Details</summary>
Motivation: 地震或滑坡等灾害后，受困人员的存活概率随时间快速下降，而夜间搜救受限于能见度。卫星遥感虽可夜间广域观测，但空间分辨率不足以识别单个人员。因此需要开发基于无人机热红外影像的自动人员检测方法。

Method: 使用Grounding DINO深度学习模型检测热红外图像中的人员；利用地面固定点热红外相机数据训练以解决无人机数据稀缺问题；通过空间缩放和填充（Spatial resizing and padding）模拟无人机视角，弥合地面与航空影像的域差距；采用基于位置的增强策略。

Result: 所提方法在真实数据集上优于基线，在低热对比环境下Recall显著提升。消融实验表明空间缩放、填充和位置增强逐步提升检测性能。与YOLOv8n和RT-DETR对比显示，鲁棒性来自Grounding DINO的语言引导语义先验。夜间影像评估验证了方法的泛化能力。

Conclusion: 基于语言引导的语义先验的Grounding DINO在热红外人员检测中具有显著优势，尤其适用于复杂热噪声场景，为夜间搜救提供了可行的技术方案。

与遥感交叉或改进方案: 交叉/改进可行性：95%。该方法的框架可直接迁移至遥感领域：1）将地面-航空域适应策略（空间缩放+填充）推广至卫星-无人机或卫星-航空影像的跨平台目标检测任务中；2）Grounding DINO的语言引导先验可应用于遥感领域常见的小样本、少标注场景（如特定地物检测）；3）热红外人员检测思路可扩展至遥感中的夜间目标检测（如车辆、船只、火点等），结合多模态数据进一步提升检测鲁棒性。

Abstract (英文): The survival probability of persons requiring rescue after large-scale earthquakes or landslides decreases rapidly with time, and nighttime search operations are constrained by limited visibility. Satellite remote sensing enables wide-area observation at night, but its limited spatial resolution restricts the identification of individual persons. This study develops an automated method for detecting persons in thermal infrared imagery captured by Unmanned Aerial Vehicles (UAVs) using the deep learning model Grounding DINO. To address the limited availability of Unmanned Aerial Vehicle (UAV)-based training data, thermal infrared imagery captured by fixed-point ground cameras was used for training. Spatial resizing and padding were applied to emulate UAV viewpoints and mitigate the ground-aerial domain gap. The proposed model outperformed the baseline on real-world datasets, with substantial Recall improvements in environments with low thermal contrast. An ablation study confirmed that spatial resizing, padding, and position-based augmentation each contribute progressively to detection performance. A comparison with the lightweight YOLOv8n and DETR-based RT-DETR detectors indicated that the Transformer architecture alone does not account for reliable detection in scenes with complex thermal noise. This robustness instead derives from the language-grounded semantic priors of Grounding DINO. An extended evaluation on nighttime imagery indicated that the proposed approach generalizes to nighttime acquisition.

</details>


### [233] [CT-TreeFlow: Probabilistic Groundwater-Potential Mapping Using Remote Sensing-Derived Environmental Predictors in Karst Aquifers](https://doi.org/10.3390/rs18132258)
*Saeid Pourmorad,Mostafa Kabolizadeh,Rui Ferreira,Samira Abbasi,Luca António Dimuccio*

Main category: Remote Sensing

TL;DR: 本文提出CT-TreeFlow框架，一种基于概率建模的岩溶地下水潜力评估方法，通过学习地下水适宜性的完整条件概率分布而非单一确定性预测，在伊朗扎格罗斯喀斯特系统上实现了优于LightGBM和XGBoost的性能（RMSE 2.737, R² 0.852），并提供了不确定性估计和可解释性分析。


<details>
  <summary>Details</summary>
Motivation: 岩溶含水层因结构渗透性、岩性变异、补给的再分布及地下管道连通性未解等导致的强烈空间异质性，使得地下水潜力评估极为复杂。现有机器学习方法虽提升了区域地下水制图能力，但多数仅提供确定性预测，缺乏预测不确定性和水文地质可靠性的有效信息。

Method: CT-TreeFlow框架集成了稀疏概率环境路由、条件密度估计、水文地质约束的伪-缺失生成、地理结构化空间验证以及可解释性驱动分析，在统一建模架构中同时实现地下水预测、不确定性量化和水文地质解释。采用遥感环境预测因子、Copernicus DEM衍生的形态测量变量、地质-结构数据集和水文气候指标，应用至伊朗胡齐斯坦省扎格罗斯喀斯特系统。

Result: 在GroupKFold空间交叉验证下，CT-TreeFlow的均方根误差（RMSE）均值为2.737，决定系数（R²）均值为0.852，优于LightGBM和XGBoost；同时提供空间显式的不确定性估计和概率预测区间。可解释性分析识别出裂隙密度、岩性、水系组织和地形控制补给条件为地下水适宜性的主导因素。预测的高适宜性区域与主要碳酸盐岩地层及独立泉-洞穴数据集高度吻合。

Conclusion: 概率建模能够在结构复杂的岩溶环境中提供比确定性方法更可靠、物理上可解释的地下水评估。CT-TreeFlow为异质性含水层系统中的不确定性感知地下水勘探和区域水文地质决策支持提供了可迁移的框架。

与遥感交叉或改进方案: 交叉/改进可行性：95%。该论文已深度集成遥感数据（DEM、环境预测因子、水文气候指标），属于遥感与水文地质的交叉研究。改进方向：(1) 引入InSAR地表形变数据辅助识别含水层储水变化；(2) 集成高光谱遥感数据（如PRISMA、EnMAP）直接识别地表岩性和矿物蚀变信息；(3) 结合GPR或ERT等地球物理遥测数据校准深部管道结构的不确定性；(4) 利用Sentinel-1 SAR数据提取土壤水分动态作为补充训练标签，进一步提升概率建模的物理约束能力。

Abstract (英文): Groundwater-potential assessment in karst aquifers is complicated by pronounced spatial heterogeneity driven by structural permeability, lithological variability, recharge redistribution, and unresolved subsurface conduit connectivity. Although machine-learning approaches have improved regional groundwater mapping, most existing models provide only deterministic predictions and offer limited information on predictive uncertainty and hydrogeological reliability. To address this limitation, we propose CT-TreeFlow. This probabilistic groundwater assessment framework goes beyond conventional machine-learning models by explicitly learning the full conditional probability distribution of groundwater favourability rather than a single deterministic estimate. The framework integrates sparse probabilistic environmental routing, conditional density estimation, hydrogeologically constrained pseudo-absence generation, geographically structured spatial validation, and explainability-driven interpretation within a unified modelling architecture, enabling simultaneous groundwater prediction, uncertainty quantification, and hydrogeological interpretation. The framework was applied to the Zagros karst system in Khuzestan Province, Iran, using remote-sensing-derived environmental predictors, Copernicus DEM-based morphometric variables, geological–structural datasets, and hydroclimatic indicators. Performance was evaluated against LightGBM and XGBoost using GroupKFold spatial cross-validation. CT-TreeFlow achieved a mean RMSE of 2.737 and a mean R2 of 0.852, while also providing spatially explicit uncertainty estimates and probabilistic prediction intervals. Explainability analyses identified fracture density, lithology, drainage organisation, and terrain-controlled recharge conditions as the dominant controls on groundwater favourability. Predicted high-favourability zones showed strong spatial correspondence with major carbonate formations and independent spring–cave inventories, supporting the hydrogeological plausibility of the mapped patterns. These results demonstrate that probabilistic modelling can provide more reliable and physically interpretable groundwater assessments than deterministic approaches in structurally complex karst environments. CT-TreeFlow offers a transferable framework for uncertainty-aware groundwater exploration and regional hydrogeological decision support in heterogeneous aquifer systems.

</details>


### [234] [Research on the Evolution Mode and Growth Characteristics of Urban Blue–Green Space Based on Landsat Data and National Policy Driven: A Case Study of the Shandong Peninsula Urban Agglomeration](https://doi.org/10.3390/rs18142277)
*Fei Yan,Jiahao Wei,Zhiwei Zhang,Hongyun Zhao,P D Zhang,Yinxi Gong*

Main category: Remote Sensing

TL;DR: 基于2014-2023年Landsat 8 OLI遥感影像，系统研究了山东半岛城市群蓝绿空间的时空演化特征及其与社会经济和政策的关联。结果表明，城市绿地和蓝水面积显著增加，植被绿度大幅提升，景观格局从碎片化向集中连片转变，黄河流域生态保护政策有效缓解了人口和经济扩张对蓝绿空间的侵占压力。


<details>
  <summary>Details</summary>
Motivation: 城市蓝绿空间是城市生态基础设施的核心，但在快速城市化背景下，其时空动态变化及其与社会经济、政策因素的关联机制尚不清晰，缺乏城市群尺度的系统研究。

Method: 基于2014-2023年Landsat 8 OLI遥感影像，结合NDVI、景观格局指数（AI、LPI、LSI）以及社会经济与政策因子关联分析，对山东半岛城市群蓝绿空间进行时空演化特征与驱动因素研究。

Result: UGS面积从28,311.66 km²增至30,194.39 km²，UBS从1,108.02 km²增至1,699.04 km²；超过三分之一的建成区NDVI显著上升，滨州、济南、淄博等城市植被绿度增幅超35%；AI和LPI整体上升，LSI下降约18.3%，景观连通性和结构完整性显著增强。

Conclusion: 黄河流域生态保护与高质量发展战略有效缓解了人口集聚和经济扩张对蓝绿空间的压力，并促进了蓝绿空间结构优化。该研究为跨城市协同规划和城市群尺度蓝绿空间一体化生态治理提供了实证依据。

与遥感交叉或改进方案: 交叉/改进可行性：95%。改进方案：（1）可引入Sentinel-2等多源遥感数据提升空间分辨率，更精细刻画小尺度蓝绿空间变化；（2）结合高光谱遥感数据区分蓝绿空间内的植被类型和水体类别；（3）引入时序InSAR技术评估蓝绿空间的地表形变与生态稳定性关联；（4）结合夜间灯光遥感与POI数据，量化人类活动强度对蓝绿空间的精细化影响；（5）利用深度学习进行蓝绿空间自动提取与分类，替代传统指数阈值法。

Abstract (英文): Urban blue–green spaces (UBGSs) serve as irreplaceable ecological infrastructure that underpins ecosystem service provision and human well-being improvement in densely populated urban regions. Based on 10-year (2014–2023) Landsat 8 OLI remote sensing imagery, this paper systematically investigates the temporal–spatial evolutionary characteristics and potential socioeconomic and policy-related associations with UBGS changes in the Shandong Peninsula urban agglomeration (SPUA), a pivotal coastal urban agglomeration in eastern China. The results demonstrate that the total UBGSs in the SPUA exhibited a pronounced increasing trend throughout the study period: the area of urban green spaces (UGSs) expanded from 28,311.66 km2 to 30,194.39 km2, while urban blue spaces (UBSs) grew from 1108.02 km2 to 1699.04 km2. Concurrently, the ecological quality of UGSs has markedly improved, with NDVI showing a significant upward trend in over one-third of the built-up areas, and vegetation greenness in cities such as Binzhou, Jinan, and Zibo increasing by more than 35%. Landscape pattern analysis reveals that the spatial structure of UGSs has transformed from a fragmented and scattered distribution to a centralized and contiguous layout. Specifically, the aggregation index (AI) and largest patch index (LPI) increased overall, while the landscape shape index (LSI) decreased by approximately 18.3%, indicating that the connectivity and structural integrity of urban green spaces have been substantially enhanced. National strategic policies, particularly the outline of ecological protection and high-quality development planning for the Yellow River basin, have effectively alleviated the encroachment pressure of population agglomeration and economic expansion on UBGSs, and played a decisive regulatory role in promoting the structural optimization of blue–green spaces. These findings provide empirical evidence for cross-city collaborative planning and integrated ecological governance of blue–green spaces at the urban agglomeration level, and offer valuable reference for achieving sustainable urban development in other rapidly urbanizing areas.

</details>


### [235] [Few-Shot Remote Sensing Scene Classification via Fusion of Zigzag Scanning Feature Sequence and Riemannian Geometric Barycenter Network](https://doi.org/10.3390/rs18132264)
*Xiliang Chen,Longwei Li,Yufeng Chen,Lei Liu,Zhenyu Wang,M. Liu,X Y Liu,Guobin Zhu*

Main category: Remote Sensing

TL;DR: 本文提出ZSFS-RGBN方法，结合Zigzag扫描特征序列与黎曼几何重心网络，在非欧几里得空间中处理遥感图像的类内差异大、类间相似性高和背景干扰问题，在三个公开数据集上优于现有方法。


<details>
  <summary>Details</summary>
Motivation: 遥感图像存在类内差异大、类间相似性高和复杂背景干扰等问题，传统小样本学习方法在欧几里得空间进行特征度量学习，难以捕捉遥感特征的非欧几里得几何分布特性，且忽略了特征图中的空间结构信息。

Method: 1) 使用ResNet12提取深度卷积特征图；2) 提出Zigzag扫描策略将二维特征图重组织为一维特征序列，保留空间局部性和结构连续性；3) 构建ARMA模型刻画特征序列的空间依赖性，并将其状态参数映射到SPD矩阵流形上；4) 设计黎曼几何重心网络学习每个类别在SPD流形上的黎曼重心，并引入联合损失函数同时优化类内紧凑性和类间分离性。

Result: 在NWPU-RESISC45、UC Merced Land-Use和WHU-RS19三个公开数据集上，在5-way 1-shot和5-way 5-shot设置下均优于多种代表性方法，消融实验验证了各模块的有效性。

Conclusion: 所提方法通过将特征映射到非欧几里得几何空间并利用黎曼几何重心网络，有效解决了遥感图像小样本场景分类中的关键挑战，取得了显著的性能提升。

与遥感交叉或改进方案: 交叉/改进可行性：90%。改进方案：1) 可将Zigzag扫描策略替换为更先进的序列化方法（如空间感知的螺旋扫描或频域分解序列化）；2) ARMA模型可扩展为更强大的时序建模方法（如Transformer或状态空间模型）；3) 黎曼几何重心网络可引入更复杂的流形结构（如Grassmann流形）以处理更高阶特征；4) 可结合多尺度特征金字塔与Zigzag扫描，提升对遥感图像中多尺度地物的表征能力。

Abstract (英文): Few-shot remote sensing scene classification aims to accurately recognize unseen scene categories using only a scarce number of labeled samples, which has emerged as a research hotspot in the field of remote sensing image interpretation. However, remote sensing images intrinsically suffer from large intra-class variations, high inter-class similarities, and complex background interferences. Traditional few-shot learning methods typically perform feature metric learning in Euclidean space, making it difficult to capture the non-Euclidean geometric distribution characteristics of remote sensing features, and they often neglect the spatial structural information embedded in feature maps. To address these issues, this paper proposes a novel few-shot remote sensing scene classification method, termed ZSFS-RGBN, which integrates a Zigzag Scanning Feature Sequence with a Riemannian Geometric Barycenter Network. Specifically, ResNet12 is first employed as the backbone to extract deep convolutional feature maps from both the support and query sets. Second, a Zigzag scanning strategy is introduced to reorganize the two-dimensional feature maps into one-dimensional feature sequences, thereby effectively preserving the spatial locality and structural continuity of the features. Third, an autoregressive moving average (ARMA) model is constructed to characterize the spatial dependencies of the feature sequences, and its state parameters are mapped onto a symmetric positive definite (SPD) matrix manifold, enabling the deep semantic representations of remote sensing scenes in a non-Euclidean geometric space. Finally, a Riemannian geometric barycenter network is designed to learn the Riemannian barycenter of each category on the SPD manifold, where a joint loss function is introduced to simultaneously optimize intra-class compactness and inter-class separability. Comprehensive experiments are conducted on three public remote sensing scene datasets: NWPU-RESISC45, UC Merced Land-Use, and WHU-RS19. Experimental results demonstrate that the proposed method consistently outperforms several representative state-of-the-art approaches under both 5-way 1-shot and 5-way 5-shot settings. Furthermore, ablation studies verify the effectiveness of each component within the proposed framework.

</details>


### [236] [Aquatic Vegetation Classification in Crab Ponds Using UAV Multispectral Imagery and a Multi-Scale Frequency-Spatial Collaborative Model](https://doi.org/10.3390/rs18142269)
*XL Mao,Jianbin Dong,Xi Zhang,Ni Ren,Weiwei Li,Jing Wang,Peiyu Dai*

Main category: Remote Sensing

TL;DR: 本文构建了首个蟹塘水生植被无人机多光谱数据集（含4种水生植物），并提出了多尺度频率-空间协作网络（MFSCNet），在轻量级参数（19.38 M）下实现mIoU达0.9044，显著优于主流CNN和Transformer模型，为蟹塘精细养殖管理和水生生态保护提供了高精度遥感技术框架。


<details>
  <summary>Details</summary>
Motivation: 蟹塘水生植被的精细监测对水质调控、生态平衡维持和中华绒螯蟹养殖优化至关重要，但复杂水环境、碎片化植被形态以及缺乏专用标注数据集导致传统遥感技术难以实现高精度语义分割与分类。

Method: 构建了首个蟹塘水生植被无人机多光谱像素级标注数据集（含4种水生植物）；提出MFSCNet，以MedNeXt为骨干，集成通道缩减注意力（Channel Reduction Attention）、空间频率选择（Spatial Frequency Selection）、空间-频率融合模块以及移动图卷积（Mobile Graph Convolution）等分布式模块，在编码器、跳跃连接、解码器和输出头协同运作，抑制水背景干扰、增强植被纹理表示并保持空间连续性。

Result: MFSCNet在19.38 M轻量级参数下，mIoU达到0.9044，优于多种主流CNN和Transformer架构。

Conclusion: 该研究为蟹塘水生植被的精准多类别识别与定量评估提供了高精度遥感技术框架，并为精细化养殖管理和水生生态保护建立了可靠的数据支持。

与遥感交叉或改进方案: 交叉/改进可行性：95%。改进方案：（1）将MFSCNet中的频率-空间协作机制直接应用于湖泊、河流等自然水体的水生植被遥感监测，解决水背景干扰问题；（2）Channel Reduction Attention模块可推广至其他遥感图像分割任务（如农作物、湿地植被）中降低通道冗余；（3）Mobile Graph Convolution可用于保持地物斑块空间连续性，适用于高分辨率遥感影像中不规则地物（如养殖塘、建筑区）的分割；（4）可进一步引入时序多光谱数据，利用频率选择模块捕获植被物候变化特征。

Abstract (英文): Fine-grained monitoring of aquatic vegetation in crab ponds is essential for regulating water quality, sustaining ecological balance, and optimizing Chinese mitten crab (Eriocheir sinensis) aquaculture. However, owing to the complex water environment, fragmented vegetation morphology, and the absence of dedicated annotated datasets, traditional remote sensing techniques struggle to achieve highly accurate semantic segmentation and classification. In this study, we construct the first unmanned aerial vehicle (UAV) multispectral dataset for crab pond aquatic vegetation, encompassing four species, Alternanthera philoxeroides, Vallisneria natans, Hydrilla verticillata, and Elodea nuttallii, with pixel-level annotations verified by field surveys across typical aquaculture sites in Jiangsu Province, China. Furthermore, we introduce the Multi-scale Frequency–Spatial Collaborative Network (MFSCNet), built upon a MedNeXt backbone and augmented with distributed modules, including Channel Reduction Attention, Spatial Frequency Selection, a spatial–frequency fusion module, and Mobile Graph Convolution that operate cooperatively across the encoder, skip connections, decoder, and output head. This design suppresses complex water-background interference, enhances vegetation texture representation, and preserves the spatial continuity of vegetation patches. Experimental results demonstrate that, with a lightweight parameter size of merely 19.38 M, MFSCNet achieves a remarkable mean Intersection over Union (mIoU) of 0.9044, outperforming various mainstream convolutional neural network (CNN) and Transformer-based architectures. This study not only provides a high-precision remote sensing technical framework for the accurate multi-class identification and quantitative assessment of aquatic vegetation in crab ponds but also establishes reliable data support for refined aquaculture management and aquatic ecological conservation.

</details>


### [237] [Landslide Susceptibility Mapping Using Multi-Source Geospatial Data and XGBoost](https://doi.org/10.3390/rs18142270)
*Dezhi Yang,Gang Ai,Dongjin Han*

Main category: Remote Sensing

TL;DR: 以广东省为研究区，融合地形、地质、水文、气候、植被和人为等多源环境因子，采用XGBoost模型进行滑坡易发性制图，并通过SHAP解释各因子的贡献。模型在独立测试集上的AUC为0.8335，五折交叉验证的平均AUC为0.8457±0.0219，高易发区主要分布在广东北部和东部的山地丘陵地区。


<details>
  <summary>Details</summary>
Motivation: 提升滑坡易发性评估的准确性和可靠性，通过引入地质因素（断层距离、地震点距离）表征构造活动对斜坡失稳的影响，并利用可解释性方法量化各环境因子的贡献。

Method: 构建多源环境因子数据集（地形、地质、水文、气候、植被、人为因子），建立滑坡编录与非滑坡样本数据集，采用XGBoost模型进行滑坡易发性制图，结合SHAP分析解释各条件因子的贡献。

Result: XGBoost模型在独立测试集上AUC为0.8335，五折分层交叉验证的平均AUC为0.8457±0.0219。高易发区主要分布于广东北部和东部的山地丘陵地区。植被相关变量、道路距离、土地覆盖类型、坡度和煤矿距离是滑坡发生的重要贡献因子。

Conclusion: 该研究为地质灾害防治、风险管理和可持续区域规划提供了有价值的参考，验证了多源环境因子结合XGBoost与SHAP方法在滑坡易发性评估中的有效性。

与遥感交叉或改进方案: 交叉/改进可行性：90%。该研究已利用了地形、植被等多源因子，可进一步引入遥感数据改进：1）利用InSAR（干涉合成孔径雷达）技术获取地表形变速率作为动态因子，弥补静态因子的不足；2）引入高分辨率遥感影像（如Sentinel-2、Landsat）提取更精细的植被指数（NDVI、EVI）和土地利用分类；3）利用光学遥感与DEM结合提取更精确的地形特征（如地形湿度指数TWI、泥沙运移指数STI）；4）借助遥感时序分析监测滑坡发生前的植被变化和地表温度异常，增强模型的预测能力。

Abstract (英文): Landslides are among the most destructive geological hazards, posing significant threats to human life, infrastructure, and ecological environments. In this research, to improve the accuracy and reliability of landslide susceptibility assessment, Guangdong Province was selected as the study area, and a multi-source environmental factor dataset incorporating topographic, geological, hydrological, climatic, vegetation, and anthropogenic factors was constructed. Geological factors, including fault distance and seismic point distance, were introduced to characterize the influence of tectonic activities on slope instability. A landslide inventory and a non-landslide sample dataset were established for model training and validation. The Extreme Gradient Boosting (XGBoost) model was employed for landslide susceptibility mapping, and SHapley Additive exPlanations (SHAP) analysis was used to interpret the contribution of different conditioning factors. The results showed that the model achieved an area under the receiver operating characteristic curve (AUC) of 0.8335 on the independent test dataset and a mean AUC of 0.8457 ± 0.0219 for a five-fold stratified cross-validation. The high-susceptibility areas were primarily distributed in the mountainous and hilly regions of northern and eastern Guangdong Province. Vegetation-related variables, road proximity, land-cover type, slope, and distance to coal mines were identified as important contributors to landslide occurrence. This study provides useful references for geological hazard prevention, risk management, and sustainable regional planning.

</details>


### [238] [A Scale-Invariance-Based Algorithm Application for Land Surface Temperature Downscaling in Denmark](https://doi.org/10.3390/rs18132263)
*Élio Pereira,Manvel Khudinyan,Inês Girão,Bruno Marques,Vitor Miranda,Hjalte Jomo Danielsen Sørup,Quentin Paletta,Ana Oliveira*

Main category: Remote Sensing

TL;DR: 本文利用Sentinel-3的粗分辨率LST（1km）和细分辨率SDR（300m）数据，开发了单时间戳（STS）和多时间戳（MTS）两种尺度不变性下采样架构。结果表明：虽然机器学习模型在训练尺度上表现良好，但难以泛化到其他尺度；线性回归在残差校正后取得最佳细尺度性能，而基于树的模型因外推能力不足表现较差。


<details>
  <summary>Details</summary>
Motivation: 地表温度（LST）是重要的基本气候变量（ECV），但现有数据难以同时满足城市空间所需的精细时空分辨率。Sentinel-3提供粗分辨率LST（1km，每日）和细分辨率光谱定向反射率（SDR，300m，每两天），后者可作为下采样预测因子，因此需要开发有效的数据融合方法以获取高时空分辨率的LST数据。

Method: 基于尺度不变性原理，设计了两种架构：1）单时间戳（STS）模型，使用单一时间戳的粗数据训练并推断该时刻的细尺度目标；2）多时间戳（MTS）模型，使用多个时间戳训练。MTS架构测试了线性回归（LR）、梯度提升（GB）、随机森林（RF）和神经网络（NN）等机器学习模型，而STS仅使用LR。模型在丹麦四个功能城市区域（FUA）上，以SDR衍生指数、季节和地理空间因子为预测变量，以Landsat数据为验证参考。

Result: 1）GB在粗尺度上表现最佳，但在细尺度上表现最差（与RF一起），表明尺度不变性失效；2）树模型因外推能力不足表现较差，而NN和LR更稳健；3）残差校正后，单时间戳LR获得最佳细尺度性能；4）多时间戳架构相比单时间戳架构可能降低性能。

Conclusion: 机器学习模型可能在训练尺度上很好预测目标，但其性能难以泛化到其他尺度，揭示了尺度特异性。单时间戳线性回归是操作应用中最可靠且推荐的架构。

与遥感交叉或改进方案: 交叉/改进可行性：90%。本文本身即为遥感论文，可改进之处：1）引入空间注意力机制或图神经网络来增强尺度间的非线性映射能力；2）考虑地表异质性指数（如景观格局指数）作为额外预测因子；3）结合多源遥感数据（如Landsat-8/Sentinel-2光学数据）替代仅依赖Sentinel-3 SDR，提升细尺度信息丰富度；4）采用物理引导的深度学习框架（如热辐射传输模型约束）改进尺度外推能力；5）探索时序监督对比学习，利用多时相信息增强尺度不变表征。

Abstract (英文): With an ever-growing recognition of Land Surface Temperature (LST) as a key Essential Climate Variable (ECV), it becomes utmost important to have such a variable at the fine spatial and temporal scales of urban spaces and dynamics. Sentinel-3 provides coarse LST (1 km, daily) based on thermal imagery acquired by its Sea and Land Surface Temperature Radiometer (SLSTR) as well as fine Spectral Directional Reflectances (SDRs, 300 m, every two days) synergically inferred from both SLSTR and the Ocean and Land Colour Instrument (OLCI), which gives the opportunity for using the latter as a predictor in the downscaling of the former. Herein, two scale-invariance-based architectures were developed: a single-timestamp (STS) model, trained with coarse data of the timestamp whose fine target it infers; and a multi-timestamp (MTS) one, trained with multiple timestamps. Note that while several Machine Learning (ML) models besides Linear Regression (LR) were considered for the MTS architecture, only LR was used for the STS one due to the limited amount of available data which the former require for hyperparameter tuning. The models were developed over four Danish Functional Urban Areas (FUAs) using SRD-derived indices and seasonal and geospatial predictors and validated against Landsat data. While Gradient Boosting (GB) achieved the best coarse-scale performance it corresponded to the worst fine-scale performer together with Random Forest (RF), indicating scale invariance breakdown. Tree-based models performed poorly due to extrapolation limitations, whereas Neural Net (NN) and LR proved more robust. After residual correction, single-timestamp LR achieved the best fine-scale performance, making it the most reliable and recommended architecture for operations. The overall results showed that, although ML models may better predict the target at their training scale, their performance may not significantly generalise at others, therefore revealing scale specificity. Furthermore, the results suggested that usage of the more general multi-timestamp architecture instead of the single one may deteriorate performance.

</details>


### [239] [Intensifying Drought Under a Warming–Wetting Climate: Multi-Scale Impacts on Vegetation Phenology and Productivity in Xinjiang, China](https://doi.org/10.3390/rs18142285)
*Tingting Pan,Yue Wang,Yaning Chen,Xueqi Zhang,Yi-Xiang Wang,Meiqing Feng*

Main category: Remote Sensing

TL;DR: 本文分析了新疆地区1962–2021年干旱动态及其对植被物候和生产力的影响，发现尽管存在暖湿化趋势，1997年后干旱频率和强度显著增加（尤其在新疆南部），植被春季物候提前、生长季延长，短期干旱（SPEI-3/6）主导植被动态，而长期干旱（SPEI-12）控制生态系统稳定性与恢复。


<details>
  <summary>Details</summary>
Motivation: 干旱严重威胁干旱区生态系统稳定性，新疆植被动态对水文气候变率高度敏感，但干旱演变及其生态影响至今未被充分量化。

Method: 利用新疆86个气象站点1962–2021年观测数据，结合标准化降水蒸散指数（SPEI）与游程理论刻画干旱动态；采用MODIS产品（2001–2021）量化植被物候与生产力。

Result: 1997年后新疆超过97%区域干旱频率和强度显著增加，空间异质性集中于新疆南部；春季物候提前−1.9天/十年，生长季长度延长+3.8天/十年；GPP和NPP呈增加趋势，尤以新疆北部为著；短期干旱（SPEI-3/6）对植被动态影响最强，长期干旱（SPEI-12）主导生态系统稳定性与恢复。

Conclusion: 新疆呈现暖湿化但干旱加剧的格局，季节性干旱在调控气候变化下植被功能中起主导作用。

与遥感交叉或改进方案: 交叉/改进可行性：85%。该论文已经是遥感与气象学交叉的典型范例。可进一步改进的方向包括：1）引入更高时空分辨率的遥感数据（如Sentinel-2或Landsat融合数据）以提升物候提取精度；2）结合微波遥感（如SMAP土壤水分数据）改进干旱监测中的水分胁迫表征；3）利用遥感反演的蒸散产品（如MOD16）替代气象站点插值，提升SPEI计算的空间代表性；4）引入植被结构参数（如叶面积指数LAI、冠层高度）以更全面刻画干旱对植被三维结构的影响。

Abstract (英文): Drought poses a major threat to ecosystem stability in arid regions. In Xinjiang, China, vegetation dynamics are highly sensitive to hydroclimatic variability, yet the evolution of drought and its ecological impacts remain insufficiently quantified. Using meteorological observations from 86 stations (1962–2021), drought dynamics were characterized using the Standardized Precipitation Evapotranspiration Index (SPEI) combined with run theory, while MODIS products (2001–2021) were used to quantify vegetation phenology and productivity. Results indicate that despite a regional warming–wetting trend, more than 97% of Xinjiang exhibits a significant increase in drought frequency and intensity after 1997, with pronounced spatial heterogeneity concentrated in southern Xinjiang. Vegetation phenology shows a significant shift, with spring onset advancing at a rate of −1.9 days decade−1 and growing season length increasing by +3.8 days decade−1. Vegetation productivity derived from MODIS shows strong spatial variability, with GPP and NPP exhibiting consistent increasing trends, particularly in northern Xinjiang. Multi-scale analysis reveals strong scale dependence in drought–vegetation interactions, where short-term drought (SPEI-3 and SPEI-6) exerts the strongest influence on vegetation dynamics, while long-term drought (SPEI-12) primarily controls ecosystem stability and post-drought recovery. Correlation and extreme-event analyses further indicate that seasonal drought and phenological shifts jointly regulate ecosystem productivity by altering water availability and carbon uptake periods. These results highlight a warming–wetting but drought-intensifying regime in Xinjiang and emphasize the dominant role of seasonal drought in regulating vegetation functioning under climate change.

</details>


### [240] [Phenology-Adaptive Maize Mapping Using an Enhanced Red-Edge NDVI from Sentinel-2 Across Representative Global Agroecosystems](https://doi.org/10.3390/rs18132261)
*Han Zhang,Lingbo Yang,Ran Huang,Limin Wang,Jingcheng Zhang*

Main category: Remote Sensing

TL;DR: 本文提出了一种基于Sentinel-2时间序列影像和增强红边NDVI（ENDVIre）的物候自适应玉米制图框架。ENDVIre利用红边4和红边2波段增强玉米在抽雄-灌浆期的光谱响应，结合NDVI、ENDVIre和LSWI构建可解释决策树模型，在美国、巴西、中国、肯尼亚、乌克兰六个代表性玉米产区实现了平均总体精度93.27%，F1分数93.14%，Kappa系数0.8652。


<details>
  <summary>Details</summary>
Motivation: 在大尺度玉米制图中，存在参考样本有限、作物物候日历异质性大、光学数据频繁缺失等难题，导致准确获取玉米分布信息困难。现有方法对复杂种植系统中的玉米识别能力不足，亟需一种可解释、可迁移且能在样本有限条件下工作的玉米制图框架。

Method: (1) 构建增强红边NDVI (ENDVIre)，利用Sentinel-2红边4 (波段8A) 和红边2 (波段6) 波段增强玉米在抽雄-灌浆期的光谱响应；(2) 采用上包络约束的Whittaker平滑器重构NDVI时间序列，识别播种-出苗、旺盛生长、成熟-收获等关键物候阶段；(3) 将NDVI、ENDVIre和LSWI集成到具有物候对齐时间窗的可解释决策树模型中，区分玉米与大豆、水稻、小麦及其他非玉米背景。

Result: 六个不同农业区域的平均总体精度93.27%，F1分数93.14%，Kappa系数0.8652。冬小麦-夏玉米轮作区跨年实验（2020-2024）总体精度89.80-96.80%，六个独立区域空间迁移实验总体精度87.40-95.40%。与黄淮海平原现有高分辨率玉米产品相比，该方法在虚警和漏警误差之间取得了更好的平衡。

Conclusion: 基于ENDVIre的物候规则为样本有限条件下的玉米制图提供了一种可解释、可迁移的解决方案，在跨区域和跨年份场景下表现出良好的鲁棒性。但持续云污染和破碎化小农景观仍然是重要挑战。

与遥感交叉或改进方案: 交叉/改进可行性：95%。改进方案：(1) 将ENDVIre思路推广至其他作物识别，如针对水稻灌浆期或小麦抽穗期构建专用光谱指数；(2) 引入SAR数据（如Sentinel-1）与光学时间序列融合，以克服云污染导致的物候信息缺失问题；(3) 结合深度学习（如时序注意力机制）自动学习物候规则，替代人工定义的决策树阈值，提升在小农破碎化景观中的泛化能力；(4) 将ENDVIre应用于城市绿地或森林物候监测，利用红边波段对叶绿素高度敏感的特性评估植被健康状态。

Abstract (英文): Accurate maize distribution information is critical for crop-area statistics, food-security assessment, and agricultural monitoring, but large-scale maize-mapping remains difficult in regions with limited reference samples, heterogeneous crop calendars, and frequent optical data gaps. This study proposes a phenology-adaptive maize mapping framework based on Sentinel-2 time-series imagery and an Enhanced Red-edge NDVI (ENDVIre). ENDVIre was constructed from the Sentinel-2 red-edge 4 and red-edge 2 bands to enhance the spectral response of maize during the silking-to-grain-filling stage, when maize develops a dense canopy and high chlorophyll content but is often confused with soybean. The framework first reconstructed the NDVI time series using an upper-envelope-constrained Whittaker smoother to identify key phenological stages, including sowing–emergence, vigorous growth, and maturity–harvest. NDVI, ENDVIre, and LSWI were then integrated into an interpretable decision-tree model with phenology-aligned time windows to distinguish maize from soybean, rice, wheat, and other non-maize backgrounds. The method was evaluated in six representative maize-growing regions across the United States, Brazil, China, Kenya, and Ukraine, covering different crop calendars, field sizes, and agricultural systems. The mean overall accuracy, F1-score, and Kappa coefficient across the six regions reached 93.27%, 93.14%, and 0.8652, respectively. Cross-year experiments in a winter-wheat–summer-maize rotation region from 2020 to 2024 achieved overall accuracies of 89.80–96.80%, while spatial-transfer experiments in six independent regions achieved overall accuracies of 87.40–95.40%. A comparison with existing high-resolution maize products in the Huang-Huai-Hai Plain further showed that the proposed method better balanced omission and commission errors. These results indicate that ENDVIre-based phenology rules provide an interpretable and transferable solution for maize mapping under limited-sample conditions, although persistent cloud contamination and fragmented smallholder landscapes remain important challenges.

</details>


### [241] [Crop-Tool-Augmented Active Perception with Reinforcement Learning for High-Resolution Remote Sensing Visual Question Answering](https://doi.org/10.3390/rs18142288)
*Qi Li,Kailiang Chen,Yi Han,Xiangyang Xu*

Main category: Remote Sensing

TL;DR: 提出一种基于强化学习的裁剪工具增强型主动感知框架，通过结构化标记组织推理过程，结合教师引导的监督微调和分组相对策略优化，使紧凑型视觉语言模型能够主动决策裁剪操作并选择任务相关区域，在高分辨率遥感视觉问答任务上取得了与闭源/开源/遥感专用模型竞争或更优的性能。


<details>
  <summary>Details</summary>
Motivation: 现有视觉语言模型在处理超高分辨率遥感图像时依赖固定全局输入，会丢失关键局部细节，难以应对稀疏信息区域、大尺度目标变化和复杂空间布局等挑战。

Method: 1) 提出裁剪工具增强的主动感知框架，用结构化标记将推理过程组织为问题理解、裁剪决策、局部证据获取和答案生成四个阶段；2) 在LRS-GRO数据集上构建教师引导的裁剪推理轨迹，对Qwen2.5-VL-3B进行冷启动监督微调；3) 引入分组相对策略优化（GRPO）精炼主动感知策略，设计包含输出格式约束、参考区域覆盖率、答案语义一致性和裁剪惩罚的区域感知奖励函数。

Result: 在VRSBench、MME-RealWorld-RS、XLRS-Bench和LRS-VQA四个基准上，所提方法在整体性能上与闭源、开源和遥感专用视觉语言模型匹敌，在多数基准上取得最优或可比结果。

Conclusion: 结构化监督微调、强化学习优化和所设计的奖励函数均被消融实验验证为有效，证明了主动裁剪策略对高分辨率遥感VQA任务的价值。

与遥感交叉或改进方案: 交叉/改进可行性：95%。本文本身就是遥感视觉问答的论文，改进方向包括：1) 将结构化推理标记扩展至更多遥感任务（如目标检测、变化检测），统一多任务主动感知框架；2) 裁剪决策可结合遥感图像的地理坐标信息，引入地物空间先验增强区域选择合理性；3) 奖励函数可增加面向遥感特定指标（如mAP、IoU）的语义一致性项，进一步对齐下游任务需求；4) 探索多尺度裁剪策略以应对遥感中更极端的尺度差异问题。

Abstract (英文): High-resolution remote sensing visual question answering (RS-VQA) requires models to identify question-relevant regions and reason over fine-grained visual evidence. However, existing vision–language models usually rely on fixed global image inputs, which may lose critical local details in ultra-high-resolution imagery and struggle with sparse informative regions, large object-scale variations, and complex spatial layouts. To address these challenges, this paper proposes a crop-tool-augmented active perception framework with reinforcement learning. The framework introduces structured tokens to explicitly organize the reasoning process into question understanding, cropping decision-making, local evidence acquisition and final answer generation. Based on this design, the model can actively determine whether a cropping operation is needed and select task-relevant regions for further inspection. To enable stable tool-use and multi-turn reasoning in a compact vision–language model, we construct teacher-guided cropping reasoning trajectories from high-resolution images, question–answer pairs, and annotated regions in the LRS-GRO dataset, and use them for cold-start supervised fine-tuning of Qwen2.5-VL-3B. Furthermore, we introduce Group Relative Policy Optimization to refine the model’s active perception policy. A region-aware reward function is designed by integrating output-format constraints, reference-region coverage, answer semantic consistency, and cropping penalties, which encourages compact and informative region selection while reducing redundant tool invocations. Experiments on VRSBench, MME-RealWorld-RS, XLRS-Bench, and LRS-VQA demonstrate that the proposed method achieves competitive overall performance compared with closed-source, open-source, and remote-sensing-specific vision–language models, and obtains the best or comparable results on most benchmarks. Ablation studies further verify the effectiveness of structured supervised fine-tuning, reinforcement learning optimization, and the proposed reward design.

</details>


### [242] [UAV-Based Photogrammetric Inspection in Deep Vertical Shafts: A Case Study from the KGHM GG-1 Mineshaft in Poland](https://doi.org/10.3390/rs18132257)
*Wojciech Rutkowski,Jędrzej Szczepaniak,Tomasz Lipecki,A. Rzonca*

Main category: Remote Sensing

TL;DR: 本文研究了一种专门为深部矿井竖井检测设计的无人机系统，并在波兰最深运营竖井GG-1进行了测试。通过自建六旋翼无人机搭载高分辨率相机和摄影测量设备，在无GNSS信号、高通风气流和受限空间等恶劣条件下，成功获取了竖井结构的视觉和空间数据，验证了无人机作为传统检测补充工具的可行性。


<details>
  <summary>Details</summary>
Motivation: 传统竖井检测受限于人员可达性、环境危险和主观评估，亟需一种更安全、客观且可替代或补充的检测方法，以提升深部矿井竖井结构完整性和安全监测水平。

Method: 研发定制六旋翼无人机系统，搭载高分辨率相机和摄影测量设备；经历从无定位系统到包含定位系统和六台相机的多阶段演进；在波兰Kwielice的GG-1竖井（该国最深运营竖井）进行高空飞行测试，在高通风气流、受限几何空间和无GNSS信号条件下采集数据。

Result: 无人机检测能够提供足够精度的数据，用于识别结构异常、记录竖井基础设施并增强安全监测，证明了无人机技术作为传统检测补充工具的潜力。

Conclusion: 无人机技术可作为传统竖井检测的有效补充手段，提供更优的数据质量、降低人员风险，并为竖井维护提供新方法。

与遥感交叉或改进方案: 交叉/改进可行性：85%。该无人机竖井检测系统与遥感领域高度交叉：1）可将多光谱/高光谱传感器集成至无人机，用于竖井壁矿物成分识别和地质异常检测；2）引入LiDAR（激光雷达）替代或补充摄影测量，提升竖井三维建模精度；3）采用SLAM（即时定位与地图构建）算法解决无GNSS环境下的自主定位与导航问题，该技术已广泛用于遥感无人机平台；4）结合深度学习图像识别算法，自动识别竖井裂缝、渗水、支护变形等结构异常；5）借鉴遥感中的多视角立体视觉和三维点云配准技术，提升竖井内部结构的重建精度。

Abstract (英文): Underground mining operations depend heavily on vertical shafts for access, ventilation, and ore transport, making their structural integrity and safety critical to overall mine performance. Traditional shaft inspections, though rigorous, are limited by human accessibility, environmental hazards, and subjective evaluation. This study presents the development and initial testing of a novel unmanned aerial vehicle (UAV) system designed specifically for shaft inspections in deep mining environments. The research focuses on the GG-1 shaft in Kwielice, Poland—the country’s deepest operational shaft—where challenging conditions such as high ventilation airflow, confined geometry, and absence of GNSS signals necessitated innovative solutions. A custom-built hexacopter equipped with high-resolution cameras and photogrammetric capabilities was deployed to capture detailed visual and spatial data. This article presents complementary path of UAV evolution, from concept, early development stage and results without positioning system through to the description of final results including positioning system and all six cameras until results of high-altitude flights. Results demonstrate that UAV-based inspection can deliver sufficient precision for identifying structural irregularities, documenting shaft infrastructure, and enhancing safety monitoring. The findings highlight the potential of UAV technology as a complementary tool to conventional inspections, offering improved data quality, reduced risk to personnel, and a new approach to shaft maintenance.

</details>


### [243] [Using Remote Sensing Data and Google Earth Engine to Quantify Regional Climate Responses to Afforestation](https://doi.org/10.3390/rs18142305)
*K. H. Khan,Shahid Nawaz Khan,Muhammad Fahim Khokhar*

Main category: Remote Sensing

TL;DR: 本研究利用遥感数据和Google Earth Engine评估了2003-2023年巴基斯坦开伯尔-普赫图赫瓦省造林活动对局地气候的影响，发现造林区域地表温度下降0.490°C，蒸散发增加，而森林损失区域则呈现升温趋势。


<details>
  <summary>Details</summary>
Motivation: 森林覆盖变化会改变地表与大气之间的能量、水和碳交换，从而影响局地和区域气候。本研究旨在评估造林对局地气候模式的影响，特别是地表温度和蒸散发的变化。

Method: 利用MODIS土地覆盖、LST、ET和植被指数产品，CRU气候数据和ALOS-PALSAR DEM，通过Google Earth Engine平台分析2003-2023年的时空趋势及变量关系。以LST为主要响应变量，ET为次要响应变量，气温、降水、NDVI、EVI和海拔为支持变量。

Result: 研究期间KP省平均LST上升0.520°C，但造林区域在2013-2023年窗口期呈现0.490°C的局地降温；造林区ET增加0.013 mm/8天，森林损失区ET减少0.005 mm/8天；区域气温上升0.310°C，降水趋势不显著；NDVI和EVI与白天LST负相关，海拔与LST强负相关。

Conclusion: 森林覆盖增加与局地地表冷却模式和植被-气候调节指标的改善相关联，表明造林活动在局地尺度上具有气候调节效益。

与遥感交叉或改进方案: 交叉/改进可行性：95%。该论文本身即为遥感应用研究，改进建议包括：(1) 可引入更高分辨率的遥感数据（如Landsat或Sentinel-2）以提升空间细节；(2) 可结合机器学习方法（如随机森林、XGBoost）量化各变量对LST变化的贡献度；(3) 可增加对地表反照率、土壤湿度等中间变量的分析以完善物理机制解释；(4) 可考虑不同造林类型（针叶林/阔叶林/混交林）的差异化气候效应分析。

Abstract (英文): Forest cover change alters land–atmosphere exchanges of energy, water, and carbon, thereby influencing local and regional climate. This study assessed climatic patterns associated with afforestation in Khyber Pakhtunkhwa, Pakistan, from 2003 to 2023 using remote sensing data and Google Earth Engine. Land surface temperature (LST) was treated as the primary response variable, while evapotranspiration (ET) was analyzed as a secondary response variable. Air temperature; precipitation; vegetation indices, including the normalized difference vegetation index (NDVI) and the enhanced vegetation index (EVI); and elevation were used as supporting variables to interpret the broader climatic and biophysical responses of afforestation. MODIS land-cover, LST, ET, and vegetation-index products, together with climate research unit (CRU) climate data and ALOS-PALSAR DEM, were used to evaluate spatiotemporal trends and variable relationships. The results showed that mean LST increased by 0.520 ± 0.070 °C across KP during 2003–2023; however, areas classified as forest gain showed a localized cooling pattern of 0.490 ± 0.050 °C during the 2013–2023 forest-cover transition assessment window. Afforested areas also exhibited increased ET, whereas forest-loss areas showed reduced ET and higher LST. Specifically, ET increased by 0.013 ± 0.002 mm/8-day in afforested areas, whereas forest-loss areas showed a decline of 0.005 ± 0.001 mm/8-day. CRU-derived regional air temperature showed an increasing tendency of 0.310 ± 0.050 °C, whereas precipitation showed only a weak and statistically non-significant regional tendency; therefore, precipitation was used only as background climatic context. The NDVI and the EVI were negatively correlated with daytime LST, and elevation showed a strong negative relationship with LST. Overall, the findings indicate that forest-cover gain was associated with localized surface cooling patterns and improved vegetation–climate regulation indicators in the study area.

</details>


### [244] [DMSF-Net: A Dual-Encoder Multi-Source Feature Fusion Network for Fine-Grained Urban Green Space Segmentation](https://doi.org/10.3390/rs18142308)
*Longzhen Jiao,Xi Zhang,Linlin Lu,Wei Cai,Shusheng Yin,Manbin Yuan,Zhengchao Chen,Qingting Li*

Main category: Remote Sensing

TL;DR: 本文提出DMSF-Net（双编码器多源特征融合网络），通过RGB与辅助特征（NDVI、LBP）的并行编码、自适应特征融合模块（AFFM）和边界感知上采样模块（BAM），实现高分辨率遥感影像中细粒度城市绿地分割，在自建UrbanGreen数据集和Vaihingen数据集上分别达到82.27%和74.73%的mIoU。


<details>
  <summary>Details</summary>
Motivation: 高分辨率遥感影像中，低矮植被与密集植被光谱相似度高且空间结构复杂，导致现有方法在细粒度城市绿地分类任务中表现不佳，亟需一种能有效融合多源特征并增强边界感知的深度学习网络。

Method: 提出DMSF-Net，核心包括：(1) 双编码器并行结构，分别处理RGB图像和辅助特征（NDVI、LBP）；(2) 自适应特征融合模块（AFFM），在编码阶段实现跨源特征的动态加权融合；(3) 边界感知上采样模块（BAM），在解码阶段通过区域语义与边界信息的联合建模增强复杂边界区域表示。

Result: 在UrbanGreen数据集上mIoU为82.27%，Vaihingen数据集上为74.73%，分别超过最优基线1.07%和0.57%；对低植被细粒度类别具有显著区分能力；消融实验验证了各模块有效性，其中AFFM对整体性能提升贡献最大，BAM改善了边界分割效果。

Conclusion: DMSF-Net通过多源特征信息与结构优化的协同集成，有效提升了复杂城市场景下细粒度绿地分割性能，为基于高分辨率遥感的城市生态监测提供了一种有效方法。

与遥感交叉或改进方案: 交叉/改进可行性：98%。改进方案：(1) 可引入更多遥感特征（如纹理指数、红边指数）作为辅助输入，进一步提升对植被类型的判别能力；(2) AFFM可扩展为多尺度特征融合机制，适配更大空间分辨率的遥感影像；(3) BAM可结合注意力机制与边缘检测损失函数，进一步优化建筑物与绿地交界处的边界精度；(4) 可引入时序遥感数据（多时相NDVI）以提升季节变化下的绿地分类鲁棒性。

Abstract (英文): The mapping and monitoring of urban green space (UGS) are of great significance for ecological assessment and sustainable development in urban settlements. However, for fine-grained classification tasks in high-resolution remote sensing imagery, existing methods suffer from significant challenges due to the high spectral similarity between low-growing and dense vegetation, as well as the complexity of spatial structures. To overcome these challenges, this paper proposes a Dual-Encoder Multi-Source Feature Fusion Network (DMSF-Net) for fine-grained urban green space segmentation. The proposed method constructs a parallel encoding structure for RGB and auxiliary features (NDVI and LBP), introduces an Adaptive Feature Fusion Module (AFFM) during the encoding phase to achieve dynamic weighted fusion of cross-source features, and designs a Boundary-Aware Up-Sampling Module (BAM) during the decoding phase to strengthen the representation of complex boundary regions through joint modeling of regional semantics and boundary information. Experimental results on a self-constructed UrbanGreen dataset and the publicly available Vaihingen dataset demonstrate the superior performance of DMSF-Net over existing mainstream methods across several evaluation metrics, achieving mIoU values of 82.27% and 74.73%, with improvements of 1.07% and 0.57% over the best baselines, respectively. The model demonstrates particularly strong discrimination capability for the fine-grained category of low vegetation. Ablation experiments further validate the usefulness of each structural module, with AFFM playing a key role in overall performance improvement, while the BAM improves boundary delineation as observed in visual comparisons. Through the synergistic integration of multi-source feature information and structural optimization, DMSF-Net effectively enhances fine-grained UGS segmentation in complex urban scenes, thereby providing an effective approach for high-resolution remote sensing-based urban ecological monitoring.

</details>


### [245] [Integrating Multi-Source Remote Sensing and Meteorological Features for Fine Mapping of Crop in Liaoning Province](https://doi.org/10.3390/rs18142301)
*Xutong Dong,Si’en Guo,Hangbiao Ke,Zhongyu Jin,Shangrong Wu,Wen Du*

Main category: Remote Sensing

TL;DR: 本文提出一种融合光学表型、微波结构和气象时间序列特征的作物精细分类框架，通过自适应特征截断机制和金字塔多尺度滑动窗口算法解决高维异质数据维度灾难问题，在辽宁省实现玉米与大豆的高精度区分（总体精度91.80%，Kappa 0.8905）。


<details>
  <summary>Details</summary>
Motivation: 辽宁地区地形起伏、田块破碎，玉米与大豆物候重叠导致传统光学-SAR特征空间光谱结构混淆，限制了大规模精细作物制图精度。

Method: (1) 融合光学表型、微波结构和气象时间序列三类特征；(2) 基于边际增益曲线过渡模式的自适应特征截断机制，克服高维异质数据维度灾难；(3) 金字塔多尺度滑动窗口算法优化气象特征，实现降维与物候敏感窗口识别；(4) 采用SVM分类器处理高维结构化数据。

Result: 多尺度特征选择有效消除冗余变量、提升类间可分性；高频率气象特征（5月中旬短波辐射、10月上旬降水、8月上旬生长度日）提供关键生理约束，缓解旱地作物光谱混淆；SVM分类总体精度91.80%，Kappa系数0.8905。

Conclusion: 该框架为大范围复杂种植区的高精度作物监测提供了可靠的方法论参考。

与遥感交叉或改进方案: 交叉/改进可行性：95%。改进方向：(1) 将自适应特征截断机制引入高光谱遥感特征选择，解决高光谱波段冗余问题；(2) 金字塔多尺度滑动窗口可迁移至遥感时序数据中的物候关键期自动检测；(3) 光学-微波-气象多模态融合策略可推广至森林类型分类、湿地动态监测等复杂地物识别场景；(4) SVM在结构化数据上的鲁棒性可为遥感大样本分类提供基线模型参考。

Abstract (英文): Accurate large-scale crop mapping is fundamental to agricultural management. However, in Liaoning Province, undulating terrain and fragmented fields make fine crop classification challenging. In particular, corn and soybean have overlapping phenologies, which can lead to spectral and structural confusion in conventional optical–SAR feature spaces and limit mapping accuracy. This study proposes a fine crop mapping framework integrating optical phenotypic, microwave structural, and meteorological time-series features. To overcome the curse of dimensionality caused by high-dimensional heterogeneous data, an adaptive feature truncation mechanism based on the transition pattern of the marginal-gain curve was designed. Additionally, a pyramid multi-scale sliding window algorithm was constructed to optimize meteorological features, achieving dimensionality reduction and precise identification of phenologically sensitive windows. The results indicate that: (1) The multi-scale feature selection strategy effectively eliminates redundant variables and maximizes the inter-class discriminability of core features, significantly improving computational efficiency and classification performance. (2) High-frequency meteorological features provide key physiological constraints. Specifically, mid-May shortwave radiation, early October precipitation, and early August growing degree days constitute the core environmental–physiological features for distinguishing confused crops, helping to mitigate the spectral confusion of dryland crops. (3) Driven by the multi-source features, the Support Vector Machine (SVM) exhibits the optimal generalization robustness for processing high-dimensional structured data, yielding an overall classification accuracy of 91.80% and a Kappa coefficient of 0.8905. This framework provides a reliable methodological reference for high-precision crop monitoring in large-scale complex planting areas.

</details>


### [246] [Large-Scale Interseismic Crustal Deformation, Fault Slip Rate, Coupling and Earthquake Potential in the Upper Yellow River Basin](https://doi.org/10.3390/rs18142297)
*Zhen Tian,J M Li,Zhe Zhang,Shidi Wang,Weiliang Huang,Kui Liu*

Main category: Remote Sensing

TL;DR: 利用Sentinel-1 SAR数据获取黄河上游流域高分辨率地表形变图，反演多条活动断层的滑动速率与耦合状态，并基于滑动亏缺计算地震矩预算与潜在震级，为区域地震 hazard 评估提供依据。


<details>
  <summary>Details</summary>
Motivation: 黄河上游流域构造复杂、地震活跃，但该区域的精细地壳形变与震间断层耦合状态尚不清楚，而这些参数对于地球动力学与地震 hazard 分析至关重要。

Method: 采用Sentinel-1合成孔径雷达影像形成基于影像帧的视线向速度图，进而推导高分辨率地表形变图；在此基础上反演少数活跃但研究不足的断层的滑动速率与耦合状态；最后根据断层滑动亏缺和历史地震计算地震矩预算与潜在震级。

Result: 日月山断裂右旋走滑速率约2.3–3.5 mm/yr，拉脊山断裂逆冲速率约2.0–3.5 mm/yr；大多数活动断层累积矩亏缺可产生MW ≥ 6.0地震，东昆仑断裂东大滩-西大滩段、玛沁-玛曲段和海原断裂金强河段可达MW ≥ 7.0。

Conclusion: 反演得到的滑动速率、震间耦合状态及潜在地震矩为理解黄河上游流域的运动学过程和评估地震 hazard 提供了基础。

与遥感交叉或改进方案: 交叉/改进可行性：85%。作为遥感（InSAR）直接应用于构造地质与地震 hazard 的论文，改进方向包括：(1) 融合多轨道（升/降轨）和多平台（如ALOS-2、Radarsat-2）InSAR数据以提升空间覆盖与解算精度；(2) 引入时序InSAR方法（如SBAS、PS-InSAR）进一步抑制大气噪声。(3) 联合GNSS观测数据约束三维形变场，弥补单视线向观测的不足。

Abstract (英文): The Upper Yellow River Basin (UYRB) is one of the most tectonically complex and seismically active regions in China, but the detailed crustal deformation and interseismic fault couplings, providing the essential parameters for geodynamics and seismic hazard analysis, are still unclear in this region. We thus adopt Sentinel-1 Synthetic Aperture Radar images to form frame-based line-of-sight velocity maps, and then derive a high-resolution surface deformation map around the UYRB. Slip rates and coupling states are further inverted for some active yet less-investigated faults. For instance, we estimate a right-lateral strike-slip motion of ~2.3–3.5 mm/yr along the Riyueshan Fault, and a thrust rate of ~2.0–3.5 mm/yr across the Lajishan Fault. Finally, the seismic moment budgets and the potential magnitudes are calculated based on the fault slip deficits and historical earthquakes. The accumulated moment deficit could produce earthquakes of MW ≥ 6.0 in most active faults, and up to MW ≥ 7.0 along the Dongdatan-Xidatan and Maqin-Maqu segments of the East Kunlun Fault and the Jinqianghe segment of the Haiyuan Fault. Our inverted slip rates, interseismic coupling states, and potential seismic moment on the active faults provide a basis for understanding kinematic processes and assessing seismic hazards within the UYRB.

</details>


### [247] [A Hybrid CNN–Transformer Model for Terrace Extraction from Remote Sensing Imagery](https://doi.org/10.3390/rs18142291)
*Fengying Jin,Zhihui Wang,Yinan Wang,Jintao Zhao,Chunjing Zhao,Qingfeng Xu,Kai Guo*

Main category: Remote Sensing

TL;DR: 本文提出TerraceNet，一种混合CNN-Transformer的编码器-双解码器架构，用于从高分辨率遥感影像中自动提取形态多样的梯田。该方法使用ConvNeXt编码器提取多尺度特征，通过动态融合特征金字塔网络(DF-FPN)聚合，再输入到边界解码器（精细边缘定位）和多尺度Transformer解码器（全局上下文建模）中。在黄土高原无定河流域2米分辨率GF-1卫星影像数据集上，IoU达到77.61%，F1-Score达到87.39%。


<details>
  <summary>Details</summary>
Motivation: 梯田形态多样、尺度变化大、几何边界模糊，现有CNN和Transformer方法难以同时保留精细空间细节和建模长距离上下文信息。

Method: 采用ConvNeXt编码器提取多尺度特征，通过动态融合特征金字塔网络(DF-FPN)聚合特征，再分别输入边界解码器（精细边缘定位）和多尺度Transformer解码器（融合边界先验进行全局上下文建模与最终分割）。

Result: 在2米分辨率GF-1卫星影像数据集上，TerraceNet的IoU达到77.61%，F1-Score达到87.39%。

Conclusion: 提出的混合架构在复杂地形下对形态多样的梯田提取具有有效性。

与遥感交叉或改进方案: 交叉/改进可行性：95%。改进方案：1) 可尝试将ConvNeXt替换为更轻量的遥感专用骨干（如ResNet-RS或EfficientNet），降低参数量；2) DF-FPN可进一步引入通道-空间注意力机制以增强对细小梯田边界的响应；3) 双解码器结构可扩展至多任务学习框架，同时输出梯田边界、坡度和面积等参数；4) 可引入半监督或自监督预训练策略，减少对大量标注样本的依赖。

Abstract (英文): Terraces constitute a critical form of land surface modification, serving both as essential agricultural resources and effective soil and water conservation measures. However, their morphological diversity, pronounced scale variation, and ambiguous geometric boundaries pose substantial challenges for automated extraction from high-resolution remote sensing imagery. Existing CNN- and Transformer-based methods still face difficulties in simultaneously preserving fine spatial details and modeling long-range contextual information. To address these limitations, this study proposes TerraceNet, a hybrid CNN–Transformer architecture with an encoder–dual-decoder design. The framework employs a ConvNeXt encoder to extract multi-scale features, which are subsequently aggregated by a dynamic fusion feature pyramid network (DF-FPN) and fed into two parallel decoders: a boundary decoder dedicated to fine-grained edge localization and a multi-scale Transformer decoder that incorporates boundary priors for global context modeling and final segmentation. Experimental results on a 2 m resolution GF-1 satellite imagery dataset from the Wuding River Basin in the Loess Plateau demonstrate that TerraceNet an IoU of 77.61% and an F1-Score of 87.39%. These results validate the effectiveness of the proposed architecture for extracting morphologically diverse terraces in complex terrain.

</details>


### [248] [A Modular and Transferable Framework for Enhancing Satellite-Derived Daily Precipitation: Adjusting Values, Aligning Distributions, and Preserving Extremes](https://doi.org/10.3390/rs18142298)
*Benny Istanto,Rizaldi Boer,I Putu Santikayasa*

Main category: Remote Sensing

TL;DR: 提出一个混合偏差校正框架(LSEQM+DL)，顺序集成线性缩放(LS)、经验分位数映射(EQM)和卷积神经网络(CNN)细化，用于校正IMERG-L卫星降水数据的系统性偏差，在印尼的验证表明极端降水统计性能显著提升，但逐日时序指标基本不变。


<details>
  <summary>Details</summary>
Motivation: 卫星降水产品(如IMERG)在全球覆盖但存在系统性偏差，特别是极端降水事件的日累积偏差较大，亟需有效的偏差校正方法以支持洪水监测和水资源管理。

Method: 提出LSEQM+DL混合偏差校正框架：(1) LS校正均值偏差；(2) EQM结合广义帕累托分布(GPD)尾部分布对齐；(3) CNN细化极端降水像素。使用站点密度置信掩码控制深度学习影响程度，在印尼(2001–2025)用CPC-UNI和171个独立BMKG站点评估。

Result: 在校正后：标准差比从0.71→1.00，相对偏差从-11.4%→-0.6%，99%极端分位数比从0.71→1.01，湿日频率高估从21%降至5%以内。代价：检测概率从0.78降至0.65，时序指标基本不变。

Conclusion: 该框架改善统计特性而非逐日时序精度，仅依赖全球可用的卫星和格点分析数据，在稀疏站点区可优雅降级，原则上可通过区域参数重校准移植到其他热带稀疏站点地区。

与遥感交叉或改进方案: 交叉/改进可行性：95%。改进方案包括：(1) 将该框架迁移至其他星载降水产品(如GPM DPR、TRMM)的偏差校正；(2) CNN细化模块可替换为更先进的深度学习架构(如Vision Transformer、U-Net变体)提升极端降水捕捉能力；(3) 引入多源卫星降水融合提升校正稳定性；(4) 考虑地形、气候分区等时空先验信息增强掩码设计；(5) 拓展至遥感地表参数(如土壤湿度、蒸散发)的偏差校正领域。

Abstract (英文): Satellite-based precipitation products such as the Integrated Multi-satellitE Retrievals for Global Precipitation Measurement (IMERG, V07) provide global coverage but exhibit systematic biases in daily accumulations, particularly for extreme events. This study presents a hybrid bias-correction framework (LSEQM+DL) for daily satellite precipitation that sequentially integrates Linear Scaling (LS) for mean bias, Empirical Quantile Mapping (EQM) with a Generalized Pareto Distribution (GPD) tail adjustment for distributional alignment, and a Convolutional Neural Network (CNN) refinement that targets extreme-precipitation pixels. A station-density confidence mask scales the deep-learning influence with gauge density, so the CNN refinement is strongest where the reference, the CPC Unified Gauge-Based Analysis of Daily Precipitation (CPC-UNI), is best constrained. The framework targets the IMERG Late Run (IMERG-L), whose roughly 14 h latency suits near-real-time flood monitoring. It is applied over Indonesia (2001–2025) and evaluated against CPC-UNI and 171 independent stations of the Meteorological, Climatological, and Geophysical Agency (BMKG) through three pillars: adjusting values, aligning distributions, and preserving extremes. At independent stations, the correction brings the standard deviation ratio from 0.71 (LS) to 1.00, the relative bias from −11.4% to −0.6%, and the 99th-percentile ratio from 0.71 to 1.01, and reduces a 21% over-estimation of wet-day frequency to within 5% of that observed. These gains carry a designed cost: the probability of detection falls from 0.78 to 0.65, while pixel-level temporal metrics (correlation, root-mean-square error, Nash–Sutcliffe efficiency) remain largely unchanged, confirming that the framework improves statistical properties rather than day-to-day timing. Relying only on globally available satellite and gauge-analysis data, and degrading gracefully where gauges are sparse, the framework is portable in principle with regional recalibration of its three tuning parameters. The corrected near-real-time product, with its station-density mask as a spatially explicit quality indicator, is intended to support flood monitoring, water resource management, and climate risk assessment in Indonesia and other gauge-sparse tropical regions.

</details>


### [249] [Short-Term Forecasting of Ocean Surface Current Maps Using High-Frequency Radar Observations and LSTM Neural Networks: A Case Study of Southwestern Taiwan](https://doi.org/10.3390/rs18142303)
*Lü Yc,Laurence Zsu-Hsin Chuang,Jian‐Wu Lai*

Main category: Remote Sensing

TL;DR: 基于2015-2019年台湾西南部高频地波雷达观测数据，开发了仅使用历史观测数据的网格点LSTM模型进行12小时海岸表层流预报。通过引入调和潮汐分离（LSTM-HA），在12小时预报中相关系数达0.768/0.729（u/v），RMSE比持续性预报降低约34%，优于原始LSTM和HYCOM模式。独立拉格朗日漂流器验证表明12小时平均分离距离与HFR观测相当。


<details>
  <summary>Details</summary>
Motivation: 短期海岸表层流预报（1-12小时）对搜救、污染应急和船舶航线规划至关重要，传统方法依赖大气强迫或数据同化，计算成本高且对潮汐主导区域预报精度有限。

Method: 利用2015-2019年CODAR HFR逐时观测数据，构建网格点LSTM模型（训练2015-2017，验证2018，测试2019）。提出LSTM-HA变体：先对数据进行调和潮汐分离，预测去潮汐残差后重构总流场，与原始LSTM在同一架构和调参协议下进行对比。

Result: 12小时预报中LSTM-HA表现最优（R=0.768/0.729），RMSE比Persistence降低约34%；LSTM-HA优势从约3小时开始显现，6-12小时最显著；1-3小时原始LSTM与LSTM-HA性能相近。独立漂流器验证12小时平均分离距离8.52/9.20 km，接近HFR观测的9.26 km。

Conclusion: 在潮汐影响显著的沿海区域，LSTM-HA在3小时以上预报中具有显著优势，是一种高效、低成本的短期表层流预报方法，无需大气强迫或数据同化。未来需进行包括季风和台风强迫的更广泛季节性验证。

与遥感交叉或改进方案: 交叉/改进可行性：95%。该研究为遥感海洋学中的高频地波雷达数据处理与预报提供了创新方法。改进方案：（1）将LSTM-HA框架扩展至其他遥感数据源（如卫星海表温度、海面高度异常和星载合成孔径雷达流场反演），实现多源遥感数据融合预报；（2）引入注意力机制或Transformer架构替代LSTM，提升长序列时空依赖关系的捕捉能力；（3）结合遥感获取的风场和波浪数据作为附加输入特征，以更好地处理季风和台风等极端事件下的流场预报；（4）将方法迁移至其他潮汐主导的遥感监测区域（如中国东海、黄海、地中海等），评估其普适性。

Abstract (英文): Short-term coastal surface-current forecasts at forecast lead times (τ = 1–12 h) are critical for search and rescue (SAR), pollution response, and vessel routing. From a 2015–2019 archive of hourly CODAR high-frequency radar (HFR) observations off Southwestern Taiwan, we developed grid-point long short-term memory (LSTM) models using historical observations alone, without atmospheric forcing or data assimilation (training 2015–2017, validation 2018, test 2019). Because harmonic tides account for ~19% of the surface-current variance, we tested harmonic detiding under a matched architecture and tuning protocol, comparing a raw-input LSTM with a detided variant (LSTM-HA) that forecasts the detided residual and reconstructs the total current. In the out-of-sample 2019 test year (τ = 12 h), LSTM-HA ranked highest (R = 0.768/0.729 for u/v) and reduced RMSE by ~34% relative to Persistence; both LSTM configurations far exceeded HA-Persistence and tide-free HYCOM, and the LSTM-HA advantage was statistically significant and spatially pervasive. An independent single-drifter Lagrangian proof-of-concept (December 2020) gave 12 h mean separations of 8.52/9.20 km for LSTM/LSTM-HA, comparable to the HFR observations (9.26 km) and below HYCOM. For this tide-influenced focus area, the benefit of LSTM-HA emerges from approximately τ = 3 h and becomes most relevant over τ = 6–12 h. At τ = 1–3 h, the raw-input LSTM performs nearly equivalently while forecasting the total current directly. Broader seasonal validation, including monsoon and typhoon forcing, remains a priority.

</details>


### [250] [Flash Drought Dynamics in China’s Major Agricultural Plains: Spatiotemporal Patterns and Crop Photosynthetic Recovery Across Cropping Systems](https://doi.org/10.3390/rs18142295)
*Shuo Mao,Mengzhen Han,H T Chen,Shaowei Ning,Zhenyu Zhang,Le Chen,Y Zhou,Weimin Ju*

Main category: Remote Sensing

TL;DR: 本研究利用多源遥感数据和多种分析方法，系统揭示了2001-2024年间华北平原和长江中下游平原闪涝的时空演变特征、作物特异性光合恢复机制及其主导因素。


<details>
  <summary>Details</summary>
Motivation: 现有闪涝研究多集中于点尺度识别或传统植被指数，对区域尺度时空演变和作物特异性光合恢复差异的理解不足，尤其季风区农田的碳循环响应机制尚不清楚。

Method: 集成三重配置误差评估、根区土壤湿度百分位识别、连通分量追踪和随机森林-SHAP分析，利用多源遥感数据对两大平原2001-2024年的闪涝轨迹及其植被影响进行表征。

Result: 长江中下游平原南部呈现高频低强度模式，华北平原中部表现为低频高强度长历时；水稻系统更易受频繁闪涝冲击，雨养和轮作系统面临更强的累积风险；SIF比GPP提前6-9天响应闪涝；所有种植系统呈现“快速生理响应-滞后碳同化恢复”模式；发生月份、持续时间和下降速率是控制光合恢复的主导因素。

Conclusion: 这些发现将闪涝监测框架拓展至区域连通性和作物恢复机制层面，为农业预警、干旱减灾和粮食安全管理提供了遥感基础。

与遥感交叉或改进方案: 交叉/改进可行性：95%。改进方案：(1) 可进一步引入Sentinel-1 SAR数据提高土壤水分反演的空间分辨率；(2) 结合高光谱遥感（如PRISMA、EnMAP）解析不同作物冠层生理响应的光谱差异；(3) 将连通分量追踪与GEE云平台结合，实现近实时的闪涝动态监测；(4) 融合热红外数据（如ECOSTRESS）量化冠层温度升高对光合抑制的贡献；(5) 考虑加入气象再分析资料（如ERA5）的气温、VPD等多变量驱动因子，提升RF-SHAP模型的解释能力。

Abstract (英文): Flash drought, an abruptly intensifying meteorological anomaly, poses a growing threat to agricultural production, ecosystem stability, and regional carbon cycling, particularly in croplands of monsoon regions. Existing studies have largely focused on point-scale identification or conventional vegetation indices, whereas the regional spatiotemporal evolution of flash droughts and crop-specific differences in photosynthetic recovery remain poorly understood. Using multi-source remote sensing data for the North China Plain and the Middle–Lower Yangtze Plain during 2001–2024, this study integrated triple-collocation error assessment, root-zone soil-moisture percentile identification, connected-component tracking, and Random Forest–SHAP analysis to characterize flash drought trajectories and their vegetation impacts. The results showed that the southern Middle–Lower Yangtze Plain exhibited a high-frequency but low-intensity pattern, whereas the central North China Plain was characterized by lower frequency yet higher intensity and longer duration. Rice-based systems were more vulnerable to frequent flash drought shocks, whereas rainfed and rotation systems faced stronger cumulative risks. Solar-induced chlorophyll fluorescence (SIF) responded to flash droughts 6–9 days earlier than gross primary productivity (GPP), and all cropping systems displayed a “rapid physiological response–lagged carbon-assimilation recovery” pattern. The month of occurrence, drought duration, and decline rate were identified as the dominant factors governing photosynthetic recovery. These findings extend the flash drought monitoring framework to incorporate regional connectivity and crop recovery mechanisms, providing a remote-sensing basis for agricultural early warning, drought mitigation, and food-security management.

</details>


### [251] [Temporary Floodplain Ponds Shape Vegetation Mosaic in a Natural River Valley: Evidence from SAR and Optical Remote Sensing](https://doi.org/10.3390/rs18142292)
*Piotr Archiciński,Sylwia Szporak-Wasilewska,Magdalena Mleczko,M. Mróz,Daria Sikorska,Piotr Sikorski*

Main category: Remote Sensing

TL;DR: 该研究利用高分辨率TerraSAR-X和Sentinel-2数据结合实地调查，揭示了临时洪泛区池塘（TFPs）占漫滩面积32%以上、储水超250 L/m²但一个月内消失的时空动态，并证明SAR-光学数据融合可将植被分类精度从64.5%提升至81.7%，TFPs水深是植物群落分布的最强预测因子。


<details>
  <summary>Details</summary>
Motivation: 临时洪泛区池塘是洪水退去后在微地形洼地中形成的短期水体，对洪泛区水文具有重要影响，但其时空动态及生态作用此前量化不足，缺乏对其与植被格局关系的系统研究。

Method: 采用TerraSAR-X雷达数据与Sentinel-2多光谱影像融合，结合实地植被调查和统计建模：阈值法SAR分类提取TFPs动态；随机森林（RF）进行植被分类；卡方检验和多项逻辑回归分析TFPs与植被格局的关系。

Result: TFPs在春汛退去后覆盖漫滩32%以上、平均储水>250 L/m²，一个月内消失；SAR-光学融合将植被制图精度从64.5%提升至81.7%（Kappa: 0.574→0.780）；TFPs与植被格局显著关联（Cramér's V=0.40），水深是最强预测因子。

Conclusion: TFPs作为细尺度水文过滤器，通过水深差异调控洪泛区植被镶嵌格局；SAR-光学数据融合能有效检测这类短暂栖息地动态，为洪泛区生态水文研究提供新方法。

与遥感交叉或改进方案: 交叉/改进可行性：85%。改进方案：（1）引入更高频的雷达数据（如Sentinel-1 C-band）结合光学影像实现TFPs的逐日动态监测；（2）利用深度学习（如U-Net或Transformer）替代阈值法提升TFPs边缘提取精度；（3）将方法迁移至其他大型洪泛区（如亚马逊、湄公河）验证普适性；（4）结合LiDAR DEM解析微地形与TFPs形成的量化关系；（5）融合时间序列植被指数（如NDVI/EVI）预测TFPs消亡后的植被演替轨迹。

Abstract (英文): Temporary floodplain ponds (TFPs) are short-lived water bodies forming in microtopographic depressions after flood recession and represent an important but poorly quantified component of floodplain hydrology. This study investigated the spatial and temporal dynamics of TFPs and their relationship with vegetation patterns in the natural floodplain of the Biebrza River, Poland. High-resolution TerraSAR-X data and Sentinel-2 multispectral imagery were combined with field-based vegetation surveys and statistical modeling. Threshold-based SAR classification showed that TFPs occupied more than 32% of the floodplain surface shortly after spring flood recession and stored, on average, over 250 L m−2 of surface water, but disappeared within one month. Random Forest classification demonstrated that combining SAR and multispectral data improved overall vegetation mapping accuracy from 64.5% to 81.7% (Kappa from 0.574 to 0.780). A global chi-square test revealed a strong association between vegetation patterns and TFP occurrence (χ2 = 224.9, p &lt; 0.001, Cramér’s V = 0.40). Multinomial logistic regression identified TFP depth as the strongest predictor of plant community distribution. Rorippo-Agrostietum, Caricetum gracilis and Glycerietum maximae increased with TFP depth, whereas Alopecuretum pratensis and Phalaridetum arundinaceae declined. These results show that TFPs act as a fine-scale hydrological filter structuring floodplain vegetation mosaics and that SAR–optical data fusion is effective for detecting these transient habitat patterns.

</details>


### [252] [Large-Scale Stratigraphic Analysis of Paintings by OCT: A Supervised Learning and Volumetric Stitching Approach](https://doi.org/10.3390/rs18142300)
*Alice Dal Fovo,raffaella fontana*

Main category: Remote Sensing

TL;DR: 本文提出了一种结合监督语义分割、体积拼接和像素级厚度量化的自动化工作流，用于光学相干层析（OCT）图像的大面积、高分辨率油画结构分析。该方法使用随机森林分类器分割空气/颜料和颜料/底漆界面，通过拼接相邻OCT体积扩展视野，并实现微米级精度的涂层厚度定量重建，准确率达96.4%-97.1%，F1-score超0.95，显著优于传统人工方法。


<details>
  <summary>Details</summary>
Motivation: 传统OCT在油画结构分析中受限于视野小、数据量大、缺乏鲁棒的拼接方法，且厚度估计依赖人工识别强度峰值，耗时、主观且不适用于大规模分析。因此需要一种自动化、高效、可重复的大面积结构分析方法。

Method: (1) 使用随机森林分类器（Trainable Weka Segmentation框架）对OCT B-scans进行监督语义分割，识别空气/颜料和颜料/底漆界面；(2) 通过自定义拼接策略将相邻OCT体积融合为连续马赛克；(3) 利用MATLAB自定义算法计算界面间像素距离，实现全场像素级厚度定量测量。

Result: 模型对空气/颜料和颜料/底漆界面分割准确率达96.4%-97.1%，加权平均F1-score超过0.95，实现了微米级分辨率的颜料厚度定量重建，处理时间大幅缩短且一致性优于人工方法。

Conclusion: 该方法能够高效、可重复地分析大型OCT数据集，将结构表征从微观扩展到宏观视野，同时保持微米级分辨率，为油画等文化遗产材料的无损层析分析提供了可靠自动化工具。

与遥感交叉或改进方案: 交叉/改进可行性：85%。该自动化OCT分析工作流可直接迁移至遥感中的光学相干探测场景，如大气气溶胶层或植被冠层结构的OCT/激光雷达数据分析。具体方案：(1) 用随机森林语义分割替代传统阈值法，对激光雷达回波中的地物/植被界面进行精确分割；(2) 体积拼接技术可用于多航带/多角度点云数据的无缝融合；(3) 像素级厚度量化方法可应用于植被冠层高度、冰层厚度或沙层沉积厚度的自动化测量，提升遥感反演的空间一致性和处理效率。

Abstract (英文): Non-invasive characterization of painting stratigraphy is challenged by light attenuation in optically heterogeneous opaque materials composing micrometric layers. Optical coherence tomography (OCT) provides suitable, non-invasive, depth-resolved imaging capabilities; however, large-area stratigraphic analysis is limited by the restricted field of view of individual stacks, high data dimensions, and the lack of robust methods for stitching volumes acquired at different focal depths. In addition, conventional layer thickness estimation relies on manual identification of intensity peaks along A-scans, which is time-consuming, operator-dependent, and unsuitable for large-scale analysis. Building upon our prior work, which introduced an AI-enhanced method for OCT volume analysis, we present an automated workflow integrating supervised semantic segmentation, volumetric mosaic stitching, and pixel-wise layer thickness quantification. OCT B-scans are segmented using a Random Forest classifier within the Trainable Weka Segmentation framework to delineate material interfaces. Adjacent OCT volumes are then combined into a continuous mosaic, and interfacial distances are computed using a custom MATLAB routine to obtain pixel-wise thickness measurements over extended fields of view. The model discriminates air/paint and paint/primer interfaces with an accuracy ranging from 96.4% to 97.1% and a weighted average F1-score exceeding 0.95, enabling quantitative reconstruction of paint thickness with micrometric resolution. This method enables efficient and reproducible analysis of large OCT datasets, extends stratigraphic characterization to macroscopic fields of view while maintaining micrometric resolution, and reduces processing time while improving consistency compared to manual approaches.

</details>


### [253] [Spatiotemporal Variation and Drivers of Vegetation Phenology Along an Urban–Rural Gradient in Rapidly Urbanizing Beijing, China](https://doi.org/10.3390/rs18142302)
*Juanzhu Liang,Yu Ye,Yuke Zhou,Weiwei Li*

Main category: Remote Sensing

TL;DR: 本研究以北京为研究区，利用2005-2024年MODIS EVI数据，通过Savitzky–Golay滤波、动态阈值法提取植被物候（SOS/EOS/LOS），结合城乡梯度分析发现：城区植被生长季开始早、结束晚、长度长，但郊区物候变化趋势更显著；夜间灯光与热量因素主导SOS，温度与夜间灯光主导EOS，且夜间灯光的影响沿城乡梯度递减。


<details>
  <summary>Details</summary>
Motivation: 城市化改变了植被生长的局部环境（热状况、水分、人类活动），但植被物候沿城乡梯度的变化及其驱动因素尚未明确。

Method: (1) MOD13Q1 EVI数据经Savitzky–Golay滤波重建植被年际轨迹；(2) 动态阈值法识别生长季开始（SOS）和结束（EOS），计算生长季长度（LOS）；(3) 结合全球城市边界（GUB）和数字高程模型（DEM）构建城乡梯度；(4) Theil–Sen斜率估计与Mann–Kendall检验分析物候趋势；(5) 偏相关分析与XGBoost-SHAP方法识别气候与城市化因素的相对影响。

Result: (1) 北京植被物候空间差异显著：西北/西南山区SOS晚、EOS早、LOS短，中部平原/东南地区相反；(2) 2005-2024年SOS显著提前（-0.65天/年），EOS微弱推迟（0.46天/年），LOS显著延长（0.90天/年）；(3) 城区SOS更早、EOS更晚、LOS更长，但郊区物候变化趋势更显著；(4) SOS受夜间灯光与热量因素共同影响，EOS主要受温度和夜间灯光影响；夜间灯光影响沿城乡递减，气候因素作用增强。

Conclusion: 揭示了快速城市化背景下北京植被物候状态、时间趋势及驱动因素的梯度分异特征，为城市植被管理与生态规划提供了有用信息。

与遥感交叉或改进方案: 交叉/改进可行性：95%。该研究本身即为遥感物候提取的典型应用，改进方向包括：(1) 可引入更高时空分辨率数据（如Sentinel-2、Landsat融合）或日光诱导叶绿素荧光（SIF）数据提升物候提取精度；(2) 城乡梯度构建可进一步结合不透水面比例、地表温度等连续梯度指标，替代离散分区；(3) 可引入遥感反演的城市热岛强度、气溶胶光学厚度等因子，丰富驱动因素分析；(4) 可拓展至多城市对比或区域尺度，增强结论普适性。

Abstract (英文): Urbanization alters the local growth environment of vegetation, including local thermal regimes, moisture availability, and human activity intensity. Yet, the gradient-dependent changes in vegetation phenology and the factors responsible for these changes are still not fully clarified. Taking Beijing as the study area, this study used MOD13Q1 EVI records from 2005 to 2024 to reconstruct annual vegetation trajectories with Savitzky–Golay filtering. The start of the growing season (SOS) and the end of the growing season (EOS) were then identified based on a dynamic threshold criterion, and the length of the growing season (LOS) was derived from the interval between the two phenological dates. Urban–rural gradients were constructed by integrating Global Urban Boundary (GUB) and digital elevation model (DEM) data. Theil–Sen slope estimation and the Mann–Kendall test were then applied to analyze trends in vegetation phenology, while partial correlation analysis and the XGBoost-SHAP method were used to identify the relative effects of climatic and urbanization-related factors. The results showed that (1) vegetation phenology in Beijing varied markedly across space. Mountainous areas in the northwest and southwest had a later SOS, earlier EOS, and shorter LOS, whereas the central plain and southeastern regions were characterized by an earlier SOS, later EOS, and longer LOS; (2) from 2005 to 2024, the SOS advanced significantly by −0.65 days/year, EOS showed a weaker delay of 0.46 days/year, and LOS increased significantly by 0.90 days/year, indicating an overall extension of the vegetation growing season over the past two decades. (3) Urban areas had an earlier SOS, later EOS, and longer LOS, but the trends of SOS advancement, EOS delay, and LOS extension were more pronounced in suburban and rural areas; and (4) the SOS was mainly influenced by the combined effects of nighttime lights and heat-related factors, whereas the EOS was primarily affected by temperature and nighttime lights. Along the urban–rural gradient, the importance of nighttime lights gradually decreased from urban areas to suburban and rural areas, while the role of climatic factors became relatively stronger. These findings reveal the gradient differentiation of vegetation phenological status, temporal trends, and associated drivers in Beijing under rapid urbanization, and provide useful information for urban vegetation management and ecological planning.

</details>


### [254] [A Multi-Source Remote Sensing-Based AGB Synergistic Inversion Approach Integrating Terrain-Corrected Canopy Height and Forest-Type Heterogeneity](https://doi.org/10.3390/rs18142304)
*L Zhang,Zhenyang Hui,Duan Huang,H F Liu,Xiaowei Xie*

Main category: Remote Sensing

TL;DR: 本文提出了一种集成地形校正ICESat-2冠层高度与森林类型异质性的协同多源遥感框架，用于区域尺度森林地上生物量（AGB）估算。该框架融合Sentinel-1/2、SRTM、WorldClim等多源数据，通过地形校正提取冠层结构参数生成足迹级AGB样本，再结合随机森林（RF）、XGBoost和SVM进行区域制图。在美国哈佛森林站点的实验表明，RF模型表现最佳（R²=0.694），验证R²为0.606，RMSE为16.53 Mg ha⁻¹，证实该框架具有高精度和可靠性。


<details>
  <summary>Details</summary>
Motivation: ICESat-2/ATLAS光子计数LiDAR在区域尺度森林AGB估算中面临稀疏采样、信号饱和、地形效应和模型泛化能力有限等挑战，亟需一种能够综合多源遥感信息并克服上述限制的协同估算框架。

Method: （1）基于高分辨率DEM对ICESat-2 ATL08数据进行地形校正得到CHM，提取冠层结构参数；（2）通过多元线性回归、逐步回归、岭回归和Lasso回归四种方法生成足迹级AGB样本；（3）融合Sentinel-1、Sentinel-2、SRTM、WorldClim等多源数据提取光谱、纹理、地形和气候特征；（4）利用相关性分析筛选关键特征；（5）构建RF、XGBoost和SVM三种森林类型特异性模型进行区域AGB制图。

Result: 在美国哈佛森林站点的实验中，RF模型表现最佳，平均R²为0.694；30m分辨率AGB制图验证总体R²为0.606，RMSE为16.53 Mg ha⁻¹。森林类型特异性建模显著提升了模型适应性和鲁棒性。

Conclusion: 该研究提出的集成地形校正ICESat-2冠层高度与森林类型特异性建模的协同多源遥感框架，为区域尺度森林生物量制图和碳储量评估提供了准确可靠的解决方案，有效克服了单传感器数据在稀疏采样、地形效应和模型泛化方面的局限性。

与遥感交叉或改进方案: 交叉/改进可行性：85%。改进方案：（1）可引入GEDI数据与ICESat-2协同使用，增加空间采样密度，缓解稀疏采样问题；（2）深度学习模型（如CNN、ResNet或Transformer）可替换RF进行特征提取与AGB回归，进一步提升非线性表达能力；（3）可增加多时相时序分析，结合植被物候特征提升AGB估算对季节变化的适应性；（4）可引入大气校正和BRDF校正以减小不同传感器观测几何和大气条件引起的误差；（5）将框架迁移至中国典型森林区域（如亚热带、热带和北方森林）进行跨区域验证和迁移学习，以评估模型泛化能力。

Abstract (英文): ICESat-2/ATLAS photon-counting LiDAR faces several challenges in regional-scale forest aboveground biomass (AGB) estimation. These challenges include sparse sampling, signal saturation, terrain effects, and limited model generalization. To solve these challenges, this study proposes a new synergistic multi-source remote sensing framework for regional-scale AGB estimation by integrating terrain-corrected ICESat-2 canopy height and forest-type heterogeneity. The framework combines structural, spectral, textural, topographic, and climatic information derived from multiple remote sensing datasets to improve biomass estimation accuracy and model robustness across different forest types. In this paper, multi-source datasets were integrated, including Sentinel-1, Sentinel-2, the Shuttle Radar Topography Mission (SRTM), WorldClim, and a terrain-corrected canopy height model (CHM). Subsequently, candidate features were derived such as spectral, textural, topographic, and climatic variables. In terms of the terrain-corrected CHM, canopy structural parameters were extracted from the Ice, Cloud, and Land Elevation Satellite-2 (ICESat-2) ATL08 data after terrain correction based on a high-resolution DEM. Footprint-level AGB samples were first generated using ICESat-2-derived canopy structural parameters through four regression approaches, including Multiple linear regression, Stepwise multiple regression, Ridge regression, and Lasso regression. These generated AGB samples were then used as response variables for subsequent regional-scale modeling. To build accurate AGB estimation model, key features were first identified using correlation analysis. To account for forest structural heterogeneity, three models including random forest (RF), extreme gradient boosting (XGBoost), and support vector machine (SVM) were developed for regional AGB mapping. To evaluate the performance of the proposed AGB estimation model by integrating terrain-corrected canopy height and forest-type heterogeneity, this study conducted AGB estimation at the Harvard Forest (HARV) site in the United States. The experimental results show that forest-type-specific modeling improves model adaptability and robustness. Among the models (RF, XGBoost and SVM), RF achieved the best performance, with an average coefficient of determination of 0.694. The optimized model was applied to produce a 30 m resolution AGB map. The validation was conducted using airborne LiDAR-derived AGB referenced results. The validation shows that an overall coefficient of determination (R2) of 0.606 and a root mean square error (RMSE) of 16.53 Mg ha−1. These results demonstrate that the proposed new synergistic AGB estimation framework, which integrates terrain-corrected ICESat-2 canopy height with forest-type-specific modeling, provides an accurate and reliable solution for regional-scale forest biomass mapping and carbon stock assessment.

</details>


### [255] [VFM-MoME: A Remote Sensing Landslide Image Segmentation Network Guided by a Visual Foundation Model and a Mixture of Mamba Experts](https://doi.org/10.3390/rs18142293)
*J Liu,Chengqiang Zhao,Yuanzhen Ju,Jin Ning,Yue Wang,Xiaoman Luo,Cong Luo*

Main category: Remote Sensing

TL;DR: 本文提出VFM-MoME网络，结合视觉基础模型与Mamba混合专家，通过双分支联合编码架构、混合Mamba专家块和二值不确定性增强模块，解决遥感滑坡分割中低分辨率、异常光照等极端场景下的判别力与泛化能力不足问题，在Landslide4Sense和GVLM数据集上取得竞争性结果。


<details>
  <summary>Details</summary>
Motivation: 现有Mamba-based方法在遥感滑坡分割任务中，面对低分辨率、异常光照等极端场景时，特征判别力和泛化能力不足，难以同时捕获全局上下文与局部细粒度特征。

Method: 1）**双分支联合编码架构**：频域感知小波块作为主编码分支，视觉基础模型融合作为辅助分支，缓解特定滑坡区域泛化特征不足；2）**混合Mamba专家块**：使解码器同时处理全局上下文与局部细粒度特征，弥补简单串行Mamba在局部细节捕获上的缺陷；3）**二值不确定性增强模块**：引导模型探索难样本，增强对模糊特征的处理能力。

Result: 在Landslide4Sense和GVLM公开数据集上，该方法取得了具有竞争力的分割性能，验证了各模块的有效性。

Conclusion: VFM-MoME通过视觉基础模型与Mamba混合专家的联合引导，有效提升了遥感滑坡分割在极端场景下的泛化能力和判别精度，为遥感滑坡灾害智能解译提供了新思路。

与遥感交叉或改进方案: 交叉/改进可行性：95%。作为遥感滑坡分割论文，后续可改进方向：1）引入多尺度频域特征融合，进一步提升对不同分辨率滑坡边界的适应能力；2）探索更轻量化的视觉基础模型，降低双分支编码的计算开销；3）将二值不确定性增强扩展为多类不确定性估计，适用于更复杂的多类滑坡场景（如滑坡-泥石流混合区）；4）结合时序遥感数据，利用Mamba的序列建模优势实现滑坡动态监测。

Abstract (英文): The linear computational complexity embraced by Mamba has demonstrated significant application potential in context modeling for the landslide segmentation tasks from remote sensing images. However, existing methods show deficiencies in terms of discrimination and generalization when applied to extreme remote sensing landslide scenarios, such as low resolution and abnormal lighting. To confront these challenges, we propose a remote sensing image landslide segmentation network (VFM-MoME) jointly guided by a vision foundation model and a mixture of Mamba experts. Specifically, we first design a dual-branch joint encoding architecture that integrates a frequency-aware wavelet block as the main encoding branch with the visual foundation model fusion as the auxiliary branch, thereby mitigating the issue of insufficient generalized features in specific landslide study areas. We also construct a mixture of Mamba expert block to enable the decoder to process both global context and local fine-grained features of landslides, addressing the shortcoming of simple serial Mamba in capturing local details and balancing between global semantic relationships and the edges and textural details of objects. Furthermore, we bring in a binary uncertainty enhancement module to guide the model in exploring challenging samples, thus enhancing the model’s ability to handle ambiguous features. Test results on the publicly available datasets of Landslide4Sense and GVLM demonstrate that our method achieves competitive performance.

</details>


### [256] [Vertical Structure of Air Temperature Excesses of the 2025 Mw 8.8 Kamchatka Earthquake](https://doi.org/10.3390/rs18142306)
*Xian Lu,Jie Zhu,Weiyu Ma,Xi Zhang,Wei Yan*

Main category: Remote Sensing

TL;DR: 本研究分析了2025年堪察加半岛Mw 8.8地震前后的多层级气温异常、SO2浓度变化及InSAR形变。发现近地表出现显著增温异常且随高度衰减，SO2在震前增强表明火山-构造系统活动加剧，InSAR观测到同震LOS形变。气温异常和SO2增强可能为震前区域火山-构造系统变化提供辅助证据。


<details>
  <summary>Details</summary>
Motivation: 探究堪察加半岛2025年Mw 8.8大地震的构造活动是否与大气增温增强相关，并尽可能最小化外部干扰，以识别可能的地震前兆信号。

Method: 利用NCEP再分析数据获取多层级气温场，基于潮汐力背景框架刻画震前震后气温超额的垂直演化特征；结合Sentinel-5P SO2观测数据分析火山SO2浓度变化；采用Sentinel-1A InSAR观测获取同震LOS形变。

Result: 震中区域在震前出现显著增温异常，近地表层强度最大、振幅最强，随高度升高逐渐衰减；SO2浓度在2025年7月中旬（震前）增强，分布于研究区火山区域；InSAR观测到累积同震LOS形变，未发现震前沉降。

Conclusion: 气温异常和SO2增强可为震前区域火山-构造系统变化提供辅助证据，InSAR结果提供了同震形变背景。垂直气温结构对研究地震相关热信号具有潜在价值，但仍需基于多年统计分析和更多地震案例的进一步研究。

与遥感交叉或改进方案: 交叉/改进可行性：85%。该研究已综合运用NCEP再分析气温数据、Sentinel-5P SO2遥感数据和Sentinel-1A InSAR数据，是遥感与地震学的典型交叉。改进方向：(1) 可引入Landsat或MODIS地表温度数据，与NCEP气温数据协同分析，增强热异常的空间分辨率和地表-大气耦合验证；(2) 利用GRACE-FO重力卫星数据监测震前区域质量迁移，辅助解释热异常和SO2增强的深部机制；(3) 采用机器学习方法（如随机森林、CNN）对多源遥感数据进行时空特征融合，提升地震热异常识别的自动化与鲁棒性；(4) 结合GNSS形变数据与InSAR进行联合解算，区分震前、同震和震后形变过程。

Abstract (英文): The Kamchatka Peninsula is one of the most seismically active regions in the world and experienced the Mw 8.8 earthquake in 2025. To examine whether tectonic processes were associated with enhanced atmospheric warming while minimizing external interference, this study analyzed multi-level air temperature fields from the NCEP (National Centers for Environmental Prediction) reanalysis data. A tidal-force-based background framework was used to characterize the vertical evolution of air temperature excesses before and after the mainshock. Results showed that a pronounced warming excess emerged near the epicentral area prior to the earthquake. The warming excess exhibited the strongest intensity and amplitude in the near-surface layer and decayed progressively with altitude, a pattern that may be consistent with the vertical attenuation expected for tectonically related thermal excesses. Sentinel-5P SO2 observations showed enhanced volcanic SO2 concentrations from mid-July 2025, before the Mw 8.8 mainshock. The SO2 enhancement was observed in the volcanic regions of the study area, suggesting that the SO2 signal may reflect enhanced regional degassing activity while also being influenced by volcanic emissions, plume dispersion, and atmospheric transport. Sentinel-1A InSAR observations derived from SAR images acquired on 23 July and 4 August 2025 represent cumulative coseismic line-of-sight (LOS) deformation associated with the Mw 8.8 mainshock, rather than preseismic subsidence. Overall, the air temperature excesses and SO2 enhancement may provide auxiliary evidence for possible changes in the regional volcanic-tectonic system before the earthquake, whereas the InSAR result provides coseismic deformation context. These findings highlight the potential value of vertical air temperature structure for investigating possible earthquake-related thermal signals, but further studies based on multi-year statistical analyses of various observational datasets and additional earthquake cases are still required.

</details>


### [257] [VODet: A Vertex Offset-Based Method for Oriented Object Detection in Remote Sensing Images](https://doi.org/10.3390/rs18142296)
*Dawei Liu,Xin Ying,Zhiheng Liu,Yueping Peng,Shujing Gao,Fengcheng Guo*

Main category: Remote Sensing

TL;DR: 本文提出VODet，一种基于顶点偏移机制的有向目标检测方法，通过回归水平外接矩形并预测两个偏移比例来重构旋转框的四个顶点，配合方向分类器和Swin Transformer分层特殊注意力模型，在DOTA、HRSC2016、UCAS-AOD上达到80.44%、90.42%、90.11%的精度，推理速度26.7 FPS，计算量192.3 GFLOPs。


<details>
  <summary>Details</summary>
Motivation: 有向目标检测面临角度回归不连续、离散分类精度低、概率建模复杂度高等挑战，现有方法难以同时兼顾精度与效率。

Method: 1）耦合顶点偏移机制：从水平锚框回归水平外接矩形，再预测两个偏移比例重构旋转框四个顶点，避免边界不连续；2）轻量级方向分类器：针对近水平临界情况，自适应区分水平框与旋转框；3）基于Swin Transformer的分层特殊注意力模型（HSAM），同时捕获局部细节与全局上下文依赖。

Result: DOTA上80.44% mAP，HRSC2016上90.42% AP，UCAS-AOD上90.11% mAP；推理速度26.7 FPS，计算量192.3 GFLOPs，在同类方法中精度-效率权衡最优。

Conclusion: VODet通过顶点偏移机制有效解决了有向目标检测中的边界不连续问题，在保持高检测精度的同时实现了低计算复杂度和高推理速度，验证了该方法的实用价值。

与遥感交叉或改进方案: 交叉/改进可行性：95%。遥感图像中建筑物、船舶、车辆等目标常呈任意方向且长宽比大、排列密集，VODet的顶点偏移机制天然适配。改进方向：1）可将HSAM中的Swin Transformer替换为轻量化变体（如Swin-Tiny）以适应遥感大图；2）方向分类器可扩展为多类别方向预测以应对遥感中更复杂的朝向分布；3）可引入多尺度特征融合策略增强小目标检测能力。

Abstract (英文): To tackle the challenges in oriented object detection, such as discontinuous angle regression, low precision in discrete classification, and high complexity in probabilistic modeling, this paper proposes VODet (Vertex Offset-based Detector), a novel oriented object detection method. VODet models the geometric structure of rotated objects through a coupled vertex offset mechanism, inherently avoiding boundary discontinuity. Specifically, it first regresses the horizontal enclosing rectangle of a target from horizontal anchors, then predicts only two offset ratios to reconstruct the four vertices of the rotated bounding box. For near-horizontal critical cases, a lightweight direction classifier adaptively distinguishes horizontal boxes from rotated ones, reducing regression complexity and improving robustness. To further enhance feature representation, we design a hierarchical special attention model (HSAM) based on Swin Transformer blocks, which effectively captures both local details and global contextual dependencies. Extensive experiments on DOTA, HRSC2016, and UCAS-AOD demonstrate that VODet achieves competitive detection accuracy (80.44% mAP on DOTA, 90.42% AP on HRSC2016, and 90.11% mAP on UCAS-AOD), with particular advantages for objects of large aspect ratios and dense arrangements. Moreover, VODet achieves the highest inference speed (26.7 FPS) and the lowest computational complexity (192.3 GFLOPs) among the compared methods, demonstrating a favorable accuracy-efficiency trade-off. These results confirm the effectiveness and practical value of the proposed method.

</details>


### [258] [PanDiM: A Diffusion Mamba Network for High-Fidelity Pansharpening](https://doi.org/10.3390/rs18142299)
*Haobo Xu,Wei Zhang,Lingfeng Lin,Jianmin Wu,谢伯祥,Wei Zhang,Hui Li,Jing Qu*

Main category: Remote Sensing

TL;DR: PanDiM提出了一种基于扩散模型的全色锐化框架，通过高頻残差重建、退化后验引导模块（DPGM）、时域感知机制和频率解耦损失（FDL），在WorldView-3、GaoFen-2和QuickBird数据集上显著优于现有方法。


<details>
  <summary>Details</summary>
Motivation: 现有基于扩散模型的全色锐化方法采用固定的去噪策略，难以适应去噪过程的阶段性变化和复杂的退化分布，导致模型在复杂区域响应精度不足。

Method: 1) 将全色锐化重新表述为多模态条件约束的高频残差恢复过程；2) 设计退化后验引导模块（DPGM），从全色图像提取双尺度物理细节先验，显式推断退化后验并转化为动态控制变量，自适应调节Mamba的状态演化；3) 提出时域感知机制，使时间信息直接干预后验估计和状态空间建模；4) 提出频率解耦损失（FDL），在频域分离高低频分量并施加针对性约束。

Result: 在WorldView-3、GaoFen-2和QuickBird三个基准数据集上，PanDiM在降分辨率评估和全分辨率评估中均显著优于现有主流方法。

Conclusion: PanDiM通过动态去噪策略和频域约束，为复杂场景下的高保真全色锐化提供了新的有效解决方案。

与遥感交叉或改进方案: 交叉/改进可行性：95%。1) PanDiM的DPGM模块可迁移至其他遥感融合任务（如多光谱与高光谱融合、SAR与光学图像融合），利用多尺度物理先验引导退化后验估计；2) 时域感知机制可推广至遥感视频超分辨率或时序变化检测中，使时间信息参与状态空间建模；3) 频率解耦损失可直接应用于遥感图像去噪、去模糊等复原任务，提升纹理和光谱保真度；4) Mamba架构的引入为遥感大模型提供轻量化状态空间建模思路，可探索与Transformer的混合架构。

Abstract (英文): Pansharpening plays an important role in remote sensing image processing. Its purpose is to fuse a high-spatial-resolution panchromatic (PAN) image and a low-spatial-resolution multispectral (LRMS) image, thereby reconstructing a high-resolution multispectral (HRMS) image with both high spatial clarity and high spectral fidelity. In recent years, diffusion models have shown great potential in image generation. However, existing diffusion-based pansharpening methods usually adopt a fixed denoising strategy, making it difficult to adapt to the stage-wise changes in the denoising process and complex degradation distributions. Based on this, we propose PanDiM, an efficient generative framework for pansharpening. Specifically, we reformulate pansharpening as a high-frequency residual restoration process constrained by multimodal conditions. To improve the response accuracy of the model in complex regions, we design a Degradation-Posterior Guidance Module (DPGM), which extracts dual-scale physical detail priors from the PAN image, explicitly infers the degradation posterior, and converts it into dynamic control variables to adaptively regulate the state evolution of Mamba. In addition, we propose a time-aware mechanism, which allows temporal information to directly intervene in posterior estimation and state-space modeling, so as to accurately match the modeling requirements of different denoising stages. Considering the characteristics of residual reconstruction, we further propose a frequency-decoupled loss (FDL), which separates low- and high-frequency components in the frequency domain and applies targeted constraints. This significantly enhances the model’s ability to represent textures and achieves more robust spectral fidelity. Extensive experiments on three benchmark datasets, including WorldView-3, GaoFen-2, and QuickBird, show that PanDiM significantly outperforms existing mainstream methods in both reduced-resolution and full-resolution evaluations, providing a new solution for high-fidelity pansharpening in complex scenarios.

</details>


### [259] [Monitoring and Prediction of Ground Deformation Using InSAR and Machine Learning Approaches in Tianjin City, China](https://doi.org/10.3390/rs18142294)
*Jinjie Miao,Rally Kimpese Talong,M M Wang,Ying Zhang,Dong Du,H F Liu,Yihang Gao,Yaonan Bai,Liu W*

Main category: Remote Sensing

TL;DR: 本文利用SBAS-InSAR技术获取天津东丽区2019-2024年地表形变数据，构建ConvLSTM2D、混合CNN-LSTM和混合CNN-BiLSTM三种机器学习模型进行形变预测。ConvLSTM2D表现最优（R²=0.99，RMSE=1.37 mm），预测2025-2028年高形变区速率达-43.71 mm/年，表明地不稳定性持续加剧，亟需地灾防治与城市规划干预。


<details>
  <summary>Details</summary>
Motivation: 地面形变是一种危险的地质现象，传统监测方法难以实现高精度大范围观测与预测，亟需结合InSAR技术与深度学习模型进行形变监测与未来趋势预测，以支持城市防灾减灾。

Method: 采用SBAS-InSAR技术获取2019-2024年天津地表形变时间序列；以InSAR形变数据集为训练和验证样本，构建ConvLSTM2D、混合CNN-LSTM和混合CNN-BiLSTM三种深度学习架构进行形变分析与未来预测；通过R²和RMSE评估模型性能。

Result: 天津东丽区2019-2024年形变速率为-40.98至27.18 mm/年，均值-2.41 mm/年。ConvLSTM2D模型性能最优（R²=0.99，RMSE=1.37 mm），优于混合CNN-LSTM（RMSE=2.16 mm）和混合CNN-BiLSTM（RMSE=2.19 mm）。预测2025-2028年高形变区速率为-43.71 mm/年。

Conclusion: 研究结果表明天津东丽区地面形变呈现持续加剧趋势，ConvLSTM2D模型在InSAR形变预测中具有高精度优势，研究结果可为地灾缓解和城市规划提供科学依据。

与遥感交叉或改进方案: 交叉/改进可行性：95%。改进方案：（1）可引入多源InSAR数据（如Sentinel-1A/B升降轨、ALOS-2等）融合，提高形变场空间覆盖与时间分辨率；（2）在ConvLSTM2D中引入注意力机制（如CBAM或自注意力）以增强时空特征提取能力；（3）结合外部驱动力因素（地下水位变化、建筑荷载、地铁施工等）作为多模态输入，提升预测可解释性；（4）采用迁移学习策略将天津训练模型泛化至其他沉降区域。

Abstract (英文): Ground deformation is a hazardous geological phenomenon. In this study, the small baseline subset (SBAS) with the coherence baseline interferometric technique was employed to derive historical ground deformation in Tianjin City, Northern China, between 2019 and 2024. Using InSAR-derived datasets for training and validation, three machine learning architectures, namely two-dimensional convolutional long short-term memory (ConvLSTM2D), hybrid convolutional neural network–long short-term memory (hybrid CNN-LSTM), and hybrid convolutional neural network–bidirectional long short-term memory (hybrid CNN-BiLSTM), were developed to further analyze ground deformation and make future predictions. It was found that from SBAS-InSAR, the deformation rates for the whole Dongli District, Tianjin, ranged from −40.98 to 27.18 mm/year, with a mean of −2.41 mm/year from 2019 to 2024. Model performance was evaluated using held-out validation samples derived from the InSAR deformation dataset. The ConvLSTM2D model achieved the best performance, with an R2 value of 0.99 and root mean squared error (RMSE) of 1.37 mm, compared with the hybrid CNN-LSTM (R2 = 0.99, RMSE = 2.16 mm) and hybrid CNN-BiLSTM (R2 = 0.99, RMSE = 2.19 mm). This optimized ConvLSTM2D model was applied to estimate the predictions of the ground deformation rate with −43.71 mm/year in the high-deformation zone between 2025 and 2028. These findings predict a continuing trend of land instability, highlighting the necessity for urgent geohazard mitigation and urban planning strategies in the affected regions.

</details>


### [260] [Geohazard Risk Identification and Validation in Hunan Province Using Synergistic Multi-Resolution SAR Monitoring](https://doi.org/10.3390/rs18142307)
*Li Cao,Guishui Zhu,Kaijun Yang,Fan Lei,Yuewei Wang,Youping Xie,Mingbo Li,Feifei Zhang,Haibo Zeng,Zexu Zhang,Jiawang Ge,Chao Yang*

Main category: Remote Sensing

TL;DR: 本文针对湖南省梯田地形、强降雨和人类活动影响下的地质灾害识别难题，提出了一种多分辨率SAR协同监测方法：先利用SBAS-InSAR进行广域筛查，再通过PS/DS-InSAR联合处理框架识别小空间尺度的微弱变形信号。基于2441个已登记地质灾害点，圈定180个疑似地质灾害目标区，野外验证确认83个活跃隐患点，验证率达46.11%。


<details>
  <summary>Details</summary>
Motivation: 湖南省是地质灾害高发区，因梯田地形、强降雨和人类活动影响，地质灾害具有复杂链生特征。现有滑坡监测方法在捕捉小空间尺度微弱变形方面能力不足，难以有效识别该区域的滑坡灾害前兆。

Method: 提出多分辨率SAR协同监测方法：先采用SBAS-InSAR技术进行广域筛查，再通过PS/DS-InSAR联合处理框架识别小空间尺度的微弱变形信号。

Result: 基于2441个已登记地质灾害点，广域InSAR监测和遥感解译圈定180个疑似地质灾害目标区，野外验证确认83个活跃隐患点，目标区验证率为46.11%。

Conclusion: 所提出的多分辨率SAR协同监测方法能够有效识别小空间尺度的微弱变形信号，显著提升地质灾害隐患点的识别能力，验证率达到46.11%。

与遥感交叉或改进方案: 交叉/改进可行性：95%。本文本身就是遥感（InSAR）应用于地质灾害识别的典型遥感交叉研究。改进方向包括：(1)引入多时相光学遥感影像（如高分辨率光学卫星）与InSAR结果协同，提高解译可信度；(2)融合LiDAR点云数据获取更精细的地形信息，提升梯田地形下变形信号的分辨能力；(3)引入深度学习（如CNN或Transformer）自动识别InSAR形变图中的异常模式，降低人工解译工作量；(4)结合降雨、地质背景等多源数据构建地质灾害易发性评价模型，实现从"识别"到"预警"的闭环。

Abstract (英文): As a natural event that poses a serious threat to human life, property, and the natural ecology, the effective identification, assessment, and early prevention of geological hazards are crucial. Hunan Province in China is a region with a high incidence of geological hazards, exhibiting complex chain-generated characteristics due to the influence of terraced topography, heavy rainfall, and human activities. Existing landslide monitoring methods have insufficient ability to capture weak deformation at small spatial scales, making it challenging to identify landslide disaster precursors in this region effectively. This paper proposes a multi-resolution SAR collaborative monitoring method using SBAS-InSAR technology for wide-area screening, followed by a joint PS/DS-InSAR processing framework to identify weak deformation signals at small spatial scales. Using 2441 registered geohazard sites in the work area as the background dataset, wide-area InSAR monitoring and remote-sensing interpretation delineated 180 suspected geohazard target areas. Field investigation confirmed 83 of the 180 candidate target zones as active hidden-danger points, corresponding to a field-confirmed rate of 46.11% among the interpreted candidates.

</details>


### [261] [Surface Heat Source Variations and Driving Factors in Typical Permafrost Areas of the Tibetan Plateau](https://doi.org/10.3390/rs18142312)
*Jimin Yao,Zikang Li,Jie Chen,Lianglei Gu,Ren Li,Tonghua Wu,Xiaodong Wu,Guojie Hu,Yao Xiao,Erji Du,Defu Zou,Guangyue Liu,Guangyang Yue,Yonghua Zhao,Wu Wang,Xiaofan Zhu,Yongping Qiao,Jianzong Shi,Yongjian Ding,Lin Zhao*

Main category: Remote Sensing

TL;DR: 本文利用青藏高原岛状与连续多年冻土区多站点观测数据，分析了地表热源变化特征。结果表明：岛状冻土区热源稳定，连续冻土区显著增加（~2.2 Wm⁻²yr⁻¹）；潜热与感热通量占比随季节变化；随机森林模型有效捕捉热源变化，土壤温度、向下短波辐射和反照率贡献超88%；热源上升趋势与秋冬气候变暖密切相关。


<details>
  <summary>Details</summary>
Motivation: 青藏高原地表热源对局地气象条件和多年冻土动态具有关键调控作用，然而对不同冻土类型区地表热源长期变化特征及其驱动机制的认识尚不充分，亟需基于实地观测数据进行系统分析。

Method: 基于岛状和连续多年冻土区多年观测数据，采用统计分析和随机森林机器学习模型，量化地表热源变化趋势、感热/潜热通量季节分配，并通过特征重要性排序识别主要驱动因子（土壤温度、向下短波辐射、反照率）。

Result: 1) 岛状冻土区地表热源稳定，连续冻土区显著增加（~2.2 Wm⁻²yr⁻¹）；2) 夏秋季潜热占比高于感热，冬春季反之，峰值均超80%；3) 随机森林模型精度良好，前三因子贡献超88%变异；4) 积雪影响具有强度依赖性；5) 热源上升与秋冬增温强相关，植被与热源呈微弱正相关。

Conclusion: 青藏高原不同冻土类型区地表热源变化存在显著差异，连续冻土区热源呈快速增强趋势，主要受土壤热状态、辐射收支和反照率反馈控制；气候变暖（尤其秋冬）是热源增加的主导背景因素，积雪和植被起调节作用。

与遥感交叉或改进方案: 交叉/改进可行性：90%。遥感可与本文地面站点观测深度互补：(1) 利用MODIS地表温度、反照率、NDVI及积雪产品在大尺度上扩展热源估算；(2) 结合GLEAM或SSEBop遥感蒸散产品驱动随机森林模型，实现空间连续的热源制图；(3) 利用Sentinel-1/2或Landsat监测冻土区地表状态（土壤水分、植被物候）变化，辅助解译热源空间异质性；(4) 将本文地面验证的机器学习框架迁移至遥感驱动变量，构建区域尺度冻土热源反演模型。

Abstract (英文): Surface heat sources play a critical role in shaping meteorological conditions and permafrost dynamics on the Tibetan Plateau. To better understand surface heat source variability in the permafrost region, multiyear observational data from an isolated permafrost site and a continuous permafrost site were used to analyse the variability. The results indicated that the surface heat source at the isolated permafrost site remained relatively stable, whereas that at the continuous permafrost site increased significantly, at a rate of approximately 2.2 Wm−2yr−1. The proportions of sensible and latent heat fluxes in the surface heat source budget varied with the season. Overall, the proportion of latent heat flux was greater than that of sensible heat flux in summer and autumn, whereas the proportion of sensible heat flux was greater than that of latent heat flux in winter and spring. The peak proportion for both fluxes exceeded 80%. A random forest model effectively captured the variations in the surface heat source. And the machine learning simulation indicated that soil temperature, downward shortwave radiation and albedo were identified as major contributors to the surface heat source, collectively accounting for more than 88% of the overall variability at both sites. The impacts of snow cover on the surface heat source varied with intensity. The increasing trend of the surface heat source was closely related to climate warming in autumn and winter. A significant but weak positive correlation was observed between vegetation and the surface heat source.

</details>


### [262] [Integrating Deep Generative AI and Hyperspectral–Multispectral Data Fusion for Enhancing Digital Soil Mapping](https://doi.org/10.3390/rs18142320)
*Said Nawar,Elsayed Said Mohamed,Ali Aldosari,Abdul Mounem Mouazen*

Main category: Remote Sensing

TL;DR: 本文提出了一种融合高光谱-多光谱卫星数据（EnMAP与SuperDove）与深度生成式AI（cWGAN-GP）的框架，用于在有限采样条件下实现高分辨率（3米）土壤制图。通过1D U-Net融合图像，cWGAN-GP增强光谱数据，结合RF和CNN建模，结果显示GAN-CNN在5倍增强下对EC、OM和P的预测R²提升9.0%-31.3%，RMSE降低8.2%-33.2%，证明了该方法在精准农业数字土壤制图中的潜力。


<details>
  <summary>Details</summary>
Motivation: EnMAP高光谱卫星的30米空间分辨率限制了其在数字土壤制图中的应用，而传统方法在有限土壤采样条件下难以实现高精度、高分辨率的土壤属性空间预测。因此，亟需探索融合高光谱-多光谱数据融合与深度生成式AI的新方法，以突破采样限制并提升土壤制图分辨率。

Method: (1) 采集110个表层土壤样本（0-30 cm），测定有机质（OM）、电导率（EC）和有效磷（P）；(2) 对EnMAP高光谱与SuperDove多光谱影像进行预处理，使用1D U-Net卷积神经网络融合生成3米高分辨率高光谱图像；(3) 采用条件Wasserstein生成对抗网络加梯度惩罚（cWGAN-GP）生成土壤光谱，进行不同级别的数据增广；(4) 将生成光谱与70%真实光谱组合，过滤保持光谱多样性；(5) 基于最优组合数据集构建随机森林（RF）和CNN两个预测模型，以30%独立样本验证。

Result: 在最高增广级别（5倍）下，GAN-CNN模型相比GAN-RF在EC、OM和P的预测中R²分别提高31.3%、25.8%和9.0%，RMSE分别降低33.2%、22.1%和8.2%。最终基于最优GAN-CNN模型和融合后的3米高分辨率高光谱图像生成了土壤属性空间分布图。

Conclusion: 融合高光谱-多光谱数据与深度生成式AI能够有效克服有限土壤采样问题，实现高分辨率数字土壤制图，为精准农业应用提供了可行技术路径。

与遥感交叉或改进方案: 交叉/改进可行性：95%。(1) 可将该融合-生成框架推广至其他高光谱卫星（如PRISMA、ZY-1 02E）与多光谱卫星（如Sentinel-2、Landsat）组合，提升普适性；(2) 引入时空融合技术（如STARFM、FSDAF）将时间维度纳入，实现土壤属性的动态监测；(3) 可尝试将cWGAN-GP替换为扩散模型或变分自编码器（VAE）等更先进的生成模型，进一步提升光谱生成质量；(4) 对生成光谱引入物理约束（如辐射传输模型）以提高光谱可解释性和泛化能力；(5) 可扩展至其他土壤属性（如全氮、重金属含量）及植被参数（如叶绿素、氮含量）的高分辨率遥感反演。

Abstract (英文): Integrating high-resolution hyperspectral remote sensing with deep generative artificial intelligence (AI) offers a promising method for accurate soil mapping under limited sampling conditions. While the EnMAP satellite provides hyperspectral data for mapping soil properties, its coarse spatial resolution (30 m) restricts its applications in digital soil mapping (DSM). This study investigates the potential of an integrated framework that combines hyperspectral–multispectral satellite data fusion and deep generative AI for high-resolution DSM. A total of 110 surface soil samples (0–30 cm) were collected from an agricultural farm in Ismailia (Egypt) and were analysed for soil organic matter (OM), electrical conductivity (EC), and available phosphorus (P). EnMAP hyperspectral and SuperDove multispectral images were pre-processed and fused using a 1D U-Net-based convolutional neural network (CNN) to generate a hyperspectral high-resolution (3 m) image. A conditional Wasserstein generative adversarial network (GAN) with gradient penalty (cWGAN-GP) was used to generate soil spectra at different levels of augmentation. The generated spectra were combined with 70% of real spectra to create different calibration datasets that were filtered to preserve spectral diversity and avoid spectral duplication. Two predictive models, random forest (RF) and CNN, were developed based on the optimal combined calibration datasets. The prediction results based on the independent prediction dataset (30%) showed that GAN–CNN outperformed GAN–RF at the highest augmentation level (5×), with increases in coefficient of determination (R2) by 31.3, 25.8, and 9.0%, and reductions in root mean square error (RMSE) by 33.2, 22.1 and 8.2% for EC, OM, and P, respectively. The optimal GAN–CNN model was used to produce soil maps at 3 m resolution based on the fused high-resolution hyperspectral image. The results indicate the potential of fusing hyperspectral and multispectral data combined with deep generative AI to overcome limited soil sampling and advance DSM for precision agriculture applications.

</details>


### [263] [Independent Multi-Sensor Validation of Machine-Learning Landslide Susceptibility: Footprint Construction Decides the Verdict—May 2023 Emilia-Romagna Event](https://doi.org/10.3390/rs18142318)
*Lucian Necula,Liviu Porumb,Andreea Florina Jocea,Dan Răducanu*

Main category: Remote Sensing

TL;DR: 本文利用2023年5月Emilia-Romagna滑坡事件（>80,000处滑坡）的独立卫星观测足迹（Sentinel-2 ΔNDVI/ΔNBR和Sentinel-1相干性/后向散射），检验了传统基于AUC（≈0.945）评价的机器学习滑坡易感性模型的实际预测能力。发现：模型在原始足迹（受洪水和农业干扰）下与随机掩膜无异；在滤除干扰的滑坡相关足迹上，模型检测能力约为随机水平的1.94倍，但这一聚集效应由少数10 km网格块驱动，多数网格块的检测能力低于随机水平（中位提升0.67倍）。结论是模型具有区域统计意义，而非逐点预测能力，足迹构建方法是当前被忽视的关键因素。


<details>
  <summary>Details</summary>
Motivation: 传统滑坡易感性模型仅以训练/测试集划分的AUC和Cohen's κ评价，缺乏独立于模型、基于真实地表扰动（如遥感观测）的验证。作者旨在揭示这种评价方式可能高估模型的实际预测能力，并强调独立足迹构建方法对验证结果的决定性影响。

Method: 以2023年5月Emilia-Romagna滑坡事件（RER2023清单，>80,000处滑坡）为案例，采用事件条件易感性模型（触发降雨作为预测因子之一），构建独立共事件扰动足迹：①Sentinel-2的ΔNDVI和ΔNBR（物候匹配的植被变化）；②Sentinel-1A C波段相干性和后向散射（12天去相关作为云无关覆盖检查）。通过Bootstrap重采样计算模型检测能力（提升倍数），并按10 km网格块分析空间异质性。

Result: ①原始共事件足迹受洪水和农业干扰污染，模型（AUC≈0.945）在该足迹下与随机掩膜无异。②在滑坡相关足迹上，模型检测光学可检测扰动的能力约为随机水平1.94倍（95% CI: 1.25–2.72），高于植被物候零模型的1.46倍。③聚集效应由少数10 km网格块驱动，多数网格块的每块中位提升仅为0.67倍（低于随机）。④模型是区域级、非逐点级预测。

Conclusion: 独立卫星足迹的构建方法对模型验证结果具有决定性影响，当前被严重忽视。高AUC值不能保证模型在实际事件中的逐点预测能力；模型仅在区域尺度上有统计意义，不能推广到逐点或跨事件场景。开放代码将在接收后发布。

与遥感交叉或改进方案: 交叉/改进可行性：95%。本文本身就是遥感与滑坡易感性建模的交叉研究典范。改进方案：①将独立遥感验证（Sentinel-2 ΔNDVI/ΔNBR + Sentinel-1相干性）作为标准流程纳入遥感滑坡易感性建模的评估体系；②引入多时相物候匹配策略以消除植被季节性干扰；③利用Sentinel-1的云无关特性构建全天候验证足迹，弥补光学遥感在云覆盖区域的缺失；④在遥感影像处理中增加洪水与农业干扰的掩膜剔除步骤，避免假阳性污染；⑤采用空间分块（如10 km网格）的局部评估替代全局AUC，揭示模型的空间异质性。

Abstract (英文): Machine-learning landslide-susceptibility maps are almost always judged by inventory-split skill (the area under the receiver-operating-characteristic curve, AUC, and Cohen’s κ), not by model-independent physical observation of where an event caused ground disturbance. For the May 2023 Emilia-Romagna event (&gt;80,000 landslides; RER2023 inventory), we confront an open-data, event-conditioned susceptibility model (trigger rainfall is among its predictors) with a co-event disturbance footprint built from two satellites: phenology-matched Sentinel-2 change in the Normalized Difference Vegetation Index (ΔNDVI) and Normalized Burn Ratio (ΔNBR), and a 12-day Sentinel-1A C-band coherence-and-backscatter layer used as a cloud-independent coverage check (C-band 12-day decorrelation is the a priori expectation in this setting); neither enters the model. Apparent geomorphic plausibility depends critically on how the independent footprint is constructed. Against a raw co-event footprint (contaminated by flooding and agriculture), a model with AUC ≈ 0.945 is indistinguishable from a random mask. On a landslide-relevant footprint, the same model captures optically detectable disturbance at roughly twice the chance (bootstrap median 1.94×, 95% CI 1.25–2.72, excluding 1; ≈1.46× above a vegetation-phenology null), but the aggregate is driven by an upper-tail minority of 10 km blocks—the majority of blocks have per-block median lift 0.67× (below chance). The map is therefore a regional, not a per-place, statement. Results are a within-event consistency test; cross-event transferability is not claimed. Footprint construction is decisive and currently neglected. The open-source pipeline is released upon acceptance.

</details>


### [264] [Nonlinear Responses and Spatial Heterogeneity of Net Ecosystem Productivity to Extreme Weather Events in Central Asia](https://doi.org/10.3390/rs18142315)
*Qian Zhou,X Chen,Jianli Ding,Shiran Song,Gongxu Jia,Li Duan*

Main category: Remote Sensing

TL;DR: 本研究利用MODIS NPP和气象数据估算中亚五国及中国新疆地区的净生态系统生产力（NEP），结合XGBoost-SHAP方法识别关键极端气候指数（ECIs），揭示了NEP对极端天气的非线性响应和阈值特征。结果表明，2000-2023年中亚整体为弱碳源，NEP呈东增西减格局；极端降水指数下降而极端温度指数显著上升；不同区域关键ECIs存在空间异质性，PRCPTOT在哈萨克斯坦等三国呈正饱和效应（吉尔吉斯斯坦阈值700-1000 mm），TXx在土库曼斯坦和中国新疆呈负高温效应（阈值约42°C和30°C），乌兹别克斯坦DTR在10°C附近出现响应低谷。


<details>
  <summary>Details</summary>
Motivation: 极端天气频发深刻影响干旱区生态碳循环，但目前对中亚地区净生态系统生产力（NEP）如何非线性响应极端气候条件尚缺乏定量理解。

Method: 基于MODIS NPP和日气象数据估算NEP，构建12个极端气候指数（ECIs），采用XGBoost模型结合SHAP方法识别不同区域NEP的关键ECIs，并量化其非线性响应和阈值特征。

Result: （1）2000-2023年中亚整体为弱碳源，NEP呈东增西减空间格局；（2）极端降水指数总体下降，极端温度指数显著上升；（3）关键ECIs存在空间异质性：哈萨克斯坦、吉尔吉斯斯坦、塔吉克斯坦以PRCPTOT为主，土库曼斯坦和新疆以TXx为主；（4）PRCPTOT在三国呈正饱和效应（吉尔吉斯斯坦阈值700-1000 mm），TXx在土库曼斯坦（~42°C）和新疆（~30°C）呈负高温效应，乌兹别克斯坦DTR在10°C附近出现响应低谷。

Conclusion: 揭示了中亚NEP对极端天气变化的非线性响应和空间异质性，为干旱区生态恢复和可持续碳管理提供理论支持。

与遥感交叉或改进方案: 交叉/改进可行性：85%。作为遥感碳循环研究，可从以下方面改进：（1）融合多源遥感数据（如GOSAT/SIF、Sentinel-2）提升NEP估算精度，减少MODIS NPP单一来源的不确定性；（2）引入高分辨率遥感土壤水分和蒸散发产品，细化干旱区水分胁迫对NEP阈值的影响；（3）利用遥感植被物候参数（如LSP）与ECIs耦合，探讨极端气候对生长季开始/结束时间的非线性驱动机制；（4）结合遥感土地利用分类数据，分区解析不同植被类型（草地、农田、荒漠）对ECIs的差异化阈值响应。

Abstract (英文): The increasing frequency and intensity of extreme weather are profoundly affecting the ecological carbon cycle in arid regions, yet there remains a lack of quantitative understanding regarding the nonlinear response of net ecosystem productivity (NEP) in Central Asia to changes in extreme weather conditions. This study focused on the five Central Asian countries and China’s Xinjiang region, where NEP was estimated using MODIS Net Primary Productivity and daily meteorological data, and 12 extreme climate indices (ECIs) were constructed. By combining the XGBoost model with the SHapley Additive exPlanations method, the key ECIs of NEP were identified for different regions, and their nonlinear responses and threshold characteristics were quantified. The results show that from 2000 to 2023, Central Asia overall acted as a weak carbon source, with NEP exhibiting a spatial pattern of increasing in the east and decreasing in the west. Extreme precipitation indices showed an overall declining trend, whereas extreme temperature indices increased significantly. There is significant spatial heterogeneity in the importance of ECIs across different regions. Specifically, the annual total precipitation (PRCPTOT) is most important in Kazakhstan, Kyrgyzstan, and Tajikistan, while the annual maximum of daily maximum temperature (TXx) shows greater importance in Turkmenistan and Xinjiang. The responses of NEP to ECIs exhibited significant nonlinear threshold characteristics. PRCPTOT showed a positive saturation effect in Kazakhstan, Kyrgyzstan, and Tajikistan, with a threshold range of 700–1000 mm in Kyrgyzstan. TXx exhibited a pronounced negative high-temperature effect in Turkmenistan and Xinjiang, with thresholds of approximately 42 °C and 30 °C, respectively. In Uzbekistan, Diurnal Temperature Range (DTR) showed a response trough near 10 °C. The study reveals the nonlinear response and spatial heterogeneity of the NEP in Central Asia to extreme weather change, offering theoretical support for ecosystem restoration and sustainable carbon management in arid regions.

</details>


### [265] [Structure-Confidence Guided Phase Congruency and Cascade Matching for Registration of Optical and SAR Images](https://doi.org/10.3390/rs18142322)
*Zhixin Lian,Shiyang Tang,Jiahao Han,Chenghao Jiang,Wanyao Wu,Zixuan He,Linrang Zhang*

Main category: Remote Sensing

TL;DR: 提出了一种结合结构置信加权相位一致性（RNW-PC）与窗口尺度级联匹配（WSC）的光学与SAR图像配准框架，通过抑制噪声伪结构、增强跨模态一致特征，以及距离-方向双重约束的由粗到精匹配策略，显著提升了配准精度和正确匹配数量。


<details>
  <summary>Details</summary>
Motivation: 光学与SAR图像配准中，基于结构特征点的方法常受斑点噪声和纹理相似性影响，导致正确对应点不足，配准精度受限。

Method: 1) 提出RNW-PC特征检测：基于相对全变分和非下采样轮廓波变换设计加权因子，抑制噪声诱导的伪结构，增强跨模态一致特征；2) 提出WSC由粗到精匹配：引入缩放窗口和余弦相似度，在粗匹配建立的空间约束基础上，对邻域内候选点进行方向一致性筛选，通过距离和方向双重约束减少正确匹配遗漏。

Result: 在60组图像对上，NCM提升至少2.43倍，精度提升超过19.85%，优于现有基于点的方法。

Conclusion: 通过特征检测与匹配的协同增强，所提框架有效提升了光学与SAR图像配准的精度和鲁棒性。

与遥感交叉或改进方案: 交叉/改进可行性：95%。本文本身就是遥感领域的多源图像配准论文。改进方案：1) RNW-PC中的加权因子可进一步结合深度学习特征进行自适应优化；2) WSC匹配可引入图匹配或Transformer注意力机制，提升大视角差异下的鲁棒性；3) 可扩展到光学与SAR、InSAR、多光谱等更多遥感数据类型的联合配准；4) 可融入端到端深度学习框架，实现更高效的推理速度。

Abstract (英文): Optical and synthetic aperture radar (SAR) image registration (OSIR) based on structural feature points is essential for all-weather and all-day multi-source data alignment. However, such methods are often plagued by insufficient correct correspondences caused by speckle noise and textural similarity, by which OSIR accuracy is limited. Therefore, a high-precision framework combining structure-confidence-weighted phase congruency (PC) with window-scaled cascaded (WSC) matching is proposed in the paper, through which accuracy is enhanced via the synergistic reinforcement of feature detection and matching. In feature detection, a weighting factor based on relative total variation and nonsubsampled contourlet transform is designed for PC calculation (i.e., RNW-PC). By suppressing noise-induced pseudo-structures and selectively enhancing cross-modal consistent features, the repeatability of keypoints is significantly improved. In feature matching, a coarse-to-fine WSC approach is proposed. A scaling window and cosine similarity are introduced, by which a refined directional consistency screening of candidate points within the neighborhood is performed based on the spatial constraints established during the coarse matching phase. Omissions in correct correspondences are effectively reduced through dual constraints of distance and direction. On 60 image pairs, the proposed method outperforms existing point-based algorithms, with NCM increasing by at least 2.43-fold and accuracy improving by over 19.85%.

</details>


### [266] [Joint Cross-Range Scaling and Phase Autofocus for ISAR Imaging Based on Adaptive Phase Tracking with Application to Asteroid Imaging](https://doi.org/10.3390/rs18142311)
*Zilin Wang,Wanni Chen,Enhua Zhang,Mingxuan Song,Xuyuan Lin,Kaizhi Wang*

Main category: Remote Sensing

TL;DR: 本文提出一种基于自适应相位跟踪（APT）的联合ISAR相位自聚焦与横向定标框架，通过多尺度LoG检测器提取孤立散射体，并将多普勒 chirp 率估计建模为递归状态估计问题，直接估计目标有效旋转速度，无需穷举参数搜索。仿真与实验验证了该方法在低信噪比下具有高精度、强鲁棒性和低计算成本。


<details>
  <summary>Details</summary>
Motivation: 传统ISAR横向定标方法依赖时频分析或多维参数搜索，在近地小行星观测常见的低信噪比条件下计算复杂度高且鲁棒性有限，难以满足高精度形状重构和撞击风险评估需求。

Method: 提出APT框架：① 多尺度LoG检测器提取孤立散射体；② 将多普勒 chirp 率估计转化为递归状态估计问题，自适应跟踪散射体横向多普勒相位；③ 直接估计有效旋转速度，构建统一相位补偿函数，同时实现距离依赖的自聚焦与横向定标。

Result: 基于三维NEA模型的仿真实验和无人机转台近场ISAR成像实验表明，该方法在低SNR条件下实现了高精度横向定标与相位自聚焦，计算成本显著降低，鲁棒性优于传统方法。

Conclusion: 所提APT框架是一种高效、可靠的行星雷达ISAR成像解决方案，尤其适用于近地小行星观测等挑战性雷达环境，能为形状重建和撞击风险评估提供有力支持。

与遥感交叉或改进方案: 交叉/改进可行性：85%。① 可直接应用于地球遥感ISAR成像（如星载/机载SAR对运动目标的成像），在低信噪比场景（如海洋目标、隐蔽目标）中提升横向定标精度；② 多尺度LoG散射体检测与递归状态估计方法可迁移至SAR运动目标检测与聚焦；③ 统一相位补偿框架可推广至双基/多基ISAR系统，增强复杂几何下的成像鲁棒性。

Abstract (英文): In planetary radar observations, accurately scaled inverse synthetic aperture radar (ISAR) images are essential for near-Earth asteroid (NEA) shape reconstruction, rotational-state estimation, and impact risk assessment. However, conventional ISAR cross-range scaling methods typically rely on time–frequency analysis or multidimensional parameter searches, resulting in high computational complexity and limited robustness under the low signal-to-noise ratio (SNR) conditions commonly encountered in NEA observations. To address this challenge, this paper proposes a joint ISAR phase autofocus and cross-range scaling framework based on adaptive phase tracking (APT). The method employs a multi-scale Laplacian of Gaussian (LoG) detector to extract isolated scatterers and reformulates Doppler chirp-rate estimation as a recursive state estimation problem. By adaptively tracking the cross-range Doppler phase of these scatterers, the target’s effective rotation velocity is directly estimated without exhaustive parameter searches. A unified phase compensation function is then constructed to simultaneously achieve range-dependent autofocus and cross-range scaling. Simulation results based on a three-dimensional NEA model with realistic planetary radar imaging parameters demonstrate the effectiveness of the proposed method in near-Earth asteroid imaging scenarios. Experimental results from a UAV turntable setup further verify its capability for cross-range scaling and phase autofocus in a controlled near-field ISAR imaging scenario. These results show that the proposed framework achieves high estimation accuracy and robustness with reduced computational cost, making it an efficient and reliable solution for practical ISAR imaging in challenging radar environments.

</details>


### [267] [Research on Few-Shot Mars Rover Onboard Surface Scene Classification Based on SE-ResNet-MTL](https://doi.org/10.3390/rs18142319)
*Yuheng He,Na Shen,Xi Zhang,Zhiliang Wu,Li J,Dong Hou*

Main category: Remote Sensing

TL;DR: 本文提出一种结合通道注意力机制（SE模块）与元迁移学习（MAML）的火星车车载地表场景分类框架SE-ResNet-MTL。基于MSL Mastcam RGB影像构建了专用标注数据集，采用ImageNet预训练的ResNet50进行迁移学习初始化，引入SE模块增强通道特征表示，并在工程约束下选择合适激活函数。实验表明，SE-ResNet在验证集上达到95.52%准确率（提升17.41%），SE-ResNet-MTL在1-shot和5-shot任务上分别达到90.5%和91.5%准确率，超出传统微调方法42.8%和25.4%，仅以全数据集准确率下降5.47%为代价，有效平衡了全数据集性能与小样本泛化能力。


<details>
  <summary>Details</summary>
Motivation: 火星车车载地表场景分类面临三个关键挑战：1) 标注数据集有限；2) 需要强小样本适应能力；3) 传统骨干网络特征表示不足。现有方法难以同时满足这些需求，亟需一种能兼顾全数据集性能与小样本泛化的高效分类框架。

Method: 1) 基于MSL Mastcam RGB影像构建火星车车载地表场景专用标注数据集；2) 以ResNet50为基础，采用ImageNet预训练权重迁移学习初始化；3) 引入Squeeze-and-Excitation (SE) 注意力模块增强通道级特征表示；4) 在工程约束下选择合适激活函数；5) 采用Model-Agnostic Meta-Learning (MAML) 元学习策略提升小样本适应能力，形成SE-ResNet-MTL模型。

Result: SE-ResNet模型验证集准确率95.52%，比基线提升17.41%。SE-ResNet-MTL在1-shot和5-shot任务上分别达到90.5%和91.5%，超出传统微调方法42.8%和25.4%，全数据集准确率仅下降5.47%。

Conclusion: 本文方法有效平衡了全数据集性能与小样本泛化能力，为火星车车载地表场景识别及其他地外视觉任务提供了实用高效的解决方案。

与遥感交叉或改进方案: 交叉/改进可行性：90%。具体方案：1) 将该框架直接迁移至遥感场景分类任务，如高分辨率遥感图像中的地物分类（农田、建筑、水体等），应对遥感标注样本稀缺问题；2) 将SE模块替换为更先进的遥感专用注意力机制（如遥感频域注意力或空间-光谱联合注意力），进一步提升特征判别力；3) MAML元学习策略可适用于遥感领域的跨区域小样本迁移（如从源区域训练到目标区域仅少量标注样本的快速适应）；4) 考虑将ResNet50替换为遥感预训练的骨干网络（如ResNet-RS或遥感专用ViT），进一步提升基础特征质量；5) 多源遥感数据（SAR+光学）场景分类任务中可引入模态感知的元学习策略，增强跨模态小样本泛化能力。

Abstract (英文): To address the challenges of limited annotated datasets, strong demand for few-shot adaptation, and insufficient feature representation of traditional backbone networks in Mars rover onboard surface scene classification, this study proposes a classification framework that integrates a channel attention mechanism with meta-transfer learning. The work consisted of two main components. First, a dedicated annotated dataset for core Mars rover onboard surface scenes was constructed based on publicly available Mars Science Laboratory (MSL) Mastcam RGB imagery, providing a standardized experimental benchmark. Second, a classification model was developed based on ResNet50. Transfer learning with ImageNet pre-trained weights was employed to improve feature initialization, and an Squeeze-and-Excitation (SE) attention module was incorporated to enhance channel-wise feature representation. Furthermore, an appropriate activation function was selected under engineering constraints. Finally, a meta-learning strategy based on Model-Agnostic Meta-Learning (MAML) was introduced to improve adaptation capability in few-shot scenarios. Experimental results showed that the SE-ResNet model achieved a validation accuracy of 95.52%, representing an improvement of 17.41% over the baseline model. After integrating meta-transfer learning, the proposed SE-ResNet-MTL (SE-Enhanced ResNet Meta-Transfer Learning) model achieved accuracies of 90.5% and 91.5% in 1-shot and 5-shot tasks, respectively, outperforming traditional fine-tuning methods by 42.8% and 25.4%. These improvements were obtained with only a 5.47% decrease in full-dataset accuracy. Overall, the proposed method effectively balances full-dataset performance and few-shot generalization, providing a practical and efficient solution for Mars rover onboard surface scene recognition and other extraterrestrial visual tasks.

</details>


### [268] [Multi-Stage Aggregation CNN–Transformer Hybrid Architecture for Infrared Small-Target Detection](https://doi.org/10.3390/rs18142309)
*Yufei Wei,Lei Chen,Xuehe Zheng,Lu Liu*

Main category: Remote Sensing

TL;DR: 本文提出MACT-Net，一种结合多级聚合编码器-解码器、CNN-Transformer混合特征建模和向量量化特征离散化的红外小目标检测网络，在三个公开数据集上取得了优异性能。


<details>
  <summary>Details</summary>
Motivation: 红外小目标因弱小而缺乏显著特征，易被杂波背景淹没。现有方法未能充分利用多级特征关系、难以整合局部与全局互补信息，且缺乏压缩冗余特征同时突出显著信息的机制。

Method: 1) 多级聚合编码器-解码器架构，包含卷积增强跳跃连接和多尺度解码器融合；2) CNN-Transformer混合特征建模模块，自适应捕获局部纹理和全局语义依赖；3) 向量量化特征离散化与表示增强机制，在网络瓶颈处压缩冗余信息并突出显著特征。

Result: 在SIRST、IRSTD-1K和NUDT-SIRST三个公开数据集上的大量实验验证了所提方法的有效性。

Conclusion: 多级聚合、混合特征建模与特征离散化机制的组合有效提升了红外小目标检测性能。

与遥感交叉或改进方案: 交叉/改进可行性：95%。该论文本身就是遥感领域的红外小目标检测工作，可直接应用于遥感成像探测场景。改进方案：1) 可引入注意力引导的多尺度特征融合进一步增强远距离小目标感知；2) 向量量化模块可结合可学习码本自适应更新策略以提升低信噪比下的鲁棒性；3) 可探索将CNN-Transformer混合模块替换为更轻量的Mamba架构以降低计算开销；4) 可扩展至多光谱或高光谱遥感图像的小目标检测任务。

Abstract (英文): Infrared small-target detection plays a vital role in applications such as imaging detection and remote sensing. Because infrared small targets are weak and lack distinctive features, they are easily submerged in cluttered backgrounds, which makes their detection challenging. Preserving spatial details, jointly modeling local and global features, and strengthening feature representation are therefore critical to improving detection accuracy and robustness. However, existing methods often fail to fully exploit multi-level feature relationships, struggle to integrate local and global complementary information, and lack mechanisms to compress redundant features while emphasizing salient information. To address these issues, we propose MACT-Net, a network designed specifically for infrared small-target detection. First, a multi-level aggregation encoder–decoder architecture with convolution-enhanced skip connections and multi-scale decoder fusion is employed to progressively abstract and reconstruct features, preserving spatial details and improving multi-scale responsiveness. Second, a CNN–Transformer hybrid feature modeling module is introduced to adaptively capture local textures and global semantic dependencies, enhancing feature representation and discriminative capability. Third, a vector quantization-based feature discretization and representation enhancement mechanism is embedded at the network bottleneck to compress redundant information and emphasize salient features, further improving robustness under low signal-to-noise ratio conditions. We evaluate MACT-Net on three public datasets: SIRST, IRSTD-1K, and NUDT-SIRST. Extensive experimental results confirm that the combination of multi-level aggregation, hybrid feature modeling, and feature discretization mechanisms effectively enhances infrared small-target detection performance.

</details>


### [269] [Influence of Internal Climate Variability on Satellite-Altimeter-Derived Regional Sea-Level Trends](https://doi.org/10.3390/rs18142313)
*Se‐Hyeon Cheon*

Main category: Remote Sensing

TL;DR: 本研究利用32年卫星高度计海面高度异常数据与CESM无强迫控制模拟，通过EOF分析发现主导模态（解释10.9%方差）呈现类IPO偶极子模式，与IPO指数高度相关（r=0.92）。该模态在无强迫CESM样本中一致出现，表明卫星观测的主导模态主要源于内部变率，但热带太平洋的强迫贡献不能排除。基于CESM模拟，内部变率对区域趋势不确定性的贡献随记录长度增加而近似反比减小，可通过局部EOF振幅缩放估计。


<details>
  <summary>Details</summary>
Motivation: 卫星高度计记录的30余年区域海平面趋势受到外部强迫与内部气候变率的共同影响，但较短记录长度难以分离两者贡献，亟需一个基于模型的框架来量化内部变率对区域海平面趋势不确定性的贡献。

Method: (1) 使用32年（1993-2024）DUACS多任务卫星高度计网格化产品；(2) 结合CESM无强迫前工业控制模拟的100个100年样本；(3) 对卫星海平面异常进行EOF分析提取主导模态；(4) 与CESM无强迫样本中的类似模态进行比较；(5) 基于CESM模拟推导内部变率贡献随记录长度的变化关系并给出区域尺度估计方法。

Result: (1) 卫星数据EOF第一模态解释10.9%方差，呈IPO-like偶极子模态，与IPO指数相关系数为0.92；(2) CESM无强迫模拟中一致出现类似IPO模态；(3) 内部变率对区域趋势不确定性的贡献近似反比于记录长度；(4) 内部变率贡献在西热带太平洋和印度洋等区域最大。

Conclusion: 该研究提供了一个基于模型框架来估计内部变率对区域海平面趋势不确定性的贡献，而非正式的检测归因或完整的卫星高度计区域海平面趋势不确定性界限。内部变率贡献估计存在局限性：仅基于单一CESM无强迫模拟，未考虑DUACS制图误差、任务间校准不确定性、地球物理校正不确定性、冰川均衡调整偏差及强迫响应不确定性。

与遥感交叉或改进方案: 交叉/改进可行性：90%。改进方案：(1) 将EOF分析+内部变率贡献估计方法应用于不同卫星高度计融合产品（如CMEMS、NASA MEaSUREs），验证结果的稳健性；(2) 将该框架拓展到其他遥感参数（如海表温度、海面风场、海冰密集度）的区域趋势不确定性分析；(3) 利用多源遥感数据（如GRACE重力卫星、Argo浮标、遥感SST）联合约束内部变率模态，减少单一CESM模拟的模型依赖性；(4) 考虑不同校正方案（如电离层改正、湿对流层改正、海况偏差改正）对EOF模态和内部变率贡献估计的影响；(5) 将深度学习（如卷积自编码器）与传统EOF结合，提取非线性主导变率模态，弥补线性EOF的局限。

Abstract (英文): Regional sea-level trends derived from satellite altimetry deviate substantially from the global mean, but the relative roles of externally forced change and internally generated climate variability remain difficult to separate from the short satellite record. Here, we examine the 32-year Data Unification and Altimeter Combination System (DUACS) gridded multi-mission satellite altimetry product (January 1993–December 2024) together with 100 100-year samples from an unforced Community Earth System Model (CESM) pre-industrial control simulation. Empirical orthogonal function (EOF) analysis of satellite sea-level anomalies reveals a leading mode explaining 10.9% of total variance, with an Interdecadal Pacific Oscillation (IPO)-like dipolar pattern and high correlation with the IPO index (r = 0.92). A similar IPO-like mode appears consistently in the unforced CESM samples. Because previous large-ensemble studies indicate that the externally forced sea-level response is generally broader and structurally distinct from this dipolar internal mode, this agreement supports the interpretation that the satellite-observed leading pattern is strongly consistent with internally generated variability, although a partial forced contribution, particularly in the tropical Pacific, cannot be excluded. Based on CESM simulations, the empirical contribution of internal variability to regional trend uncertainty decreases approximately inversely with record length. The resulting location-specific estimate can be scaled by the local EOF amplitude and is largest in regions where the dominant internal-variability mode has large amplitudes, including the western tropical Pacific and Indian Ocean. However, this estimate represents only the internally generated component inferred from a single unforced CESM simulation. It does not include DUACS mapping errors, inter-mission calibration uncertainty, geophysical correction uncertainty, glacial-isostatic-adjustment-related bias, or uncertainty in the forced sea-level response. Thus, this study provides a model-based framework for estimating the internal-variability contribution to regional sea-level trend uncertainty, rather than a formal detection-and-attribution separation or a complete uncertainty bound for satellite-altimeter-derived regional sea-level trends.

</details>


### [270] [Physics-Informed Semantic Prompt Learning for Few-Shot Low-Altitude Radar Target Recognition in Remote Sensing](https://doi.org/10.3390/rs18142316)
*Junrong Tu,Jihui Tu,Wenqing Feng,Zhaoyang Liu*

Main category: Remote Sensing

TL;DR: 本文提出一种物理信息引导的语义提示学习框架，用于小样本低空雷达目标识别。通过将雷达点航迹和测量数据转化为结构化文本提示，结合统计描述符、雷达领域物理知识和任务指令，利用部分微调的GPT-2编码器提取语义表征，并通过自适应特征聚合和关系元学习网络实现小样本分类。在包含鸟类、气球、小型旋翼无人机和轻型旋翼无人机四类真实低空雷达数据集上，20-shot设置下达到90.85%精确率、90.47%召回率和90.63% F1分数，优于传统机器学习和深度学习方法。


<details>
  <summary>Details</summary>
Motivation: 低空雷达目标（鸟类、气球、无人机）识别面临三大挑战：雷达回波微弱、航迹特征相似、大规模标注困难。现有方法难以在样本稀缺条件下实现可靠识别，亟需一种能够融合雷达物理知识并提高样本效率的识别框架。

Method: 提出物理信息引导的语义提示学习框架：(1) 将雷达点航迹和测量数据转化为结构化文本提示，融合统计描述符、雷达物理知识和任务指令；(2) 采用部分微调的GPT-2编码器提取语义表征，保留运动与散射相关信息；(3) 设计自适应特征聚合模块，对不同时间位置和语义层次的隐藏状态加权；(4) 基于关系度量的元学习网络建模查询-支持样本相似性，实现小样本分类。

Result: 在真实低空雷达数据集（鸟类、气球、小型旋翼无人机、轻型旋翼无人机四类）上，20-shot设置下达到90.85%精确率、90.47%召回率、90.63% F1分数，持续优于传统机器学习、深度学习及代表性小样本基线方法。

Conclusion: 将雷达物理语义嵌入到基于语言模型的表征学习中，能够显著提升低空雷达遥感的小样本识别性能和鲁棒性，验证了物理信息引导的语义提示学习在雷达目标识别领域的有效性。

与遥感交叉或改进方案: 交叉/改进可行性：85%。改进方案：(1) 可将该框架扩展到合成孔径雷达（SAR）图像目标识别中，利用SAR的散射物理知识（如极化特征、角度响应）构建语义提示；(2) 可引入多模态遥感数据（如光学+雷达）的联合语义提示，提升复杂场景下的识别鲁棒性；(3) 物理知识提示可进一步融入高光谱遥感中的光谱-空间物理特性，扩展到高光谱小样本分类任务；(4) 自适应特征聚合模块可迁移到遥感时序分析中，用于多时相变化检测中的关键帧特征筛选。

Abstract (英文): Low-altitude radar target recognition is important for intelligent airspace monitoring, unmanned aerial vehicle (UAV) supervision, airport bird-strike prevention, and low-altitude remote sensing. Reliable recognition remains difficult because birds, balloons, and UAVs often produce weak radar responses, share similar trajectory-level signatures, and are difficult to annotate at scale. To address these challenges, this paper proposes a physics-informed semantic prompt learning framework for few-shot low-altitude radar target recognition. The framework converts radar point-track and track measurements into structured textual prompts that combine statistical descriptors, radar-domain physical knowledge, and task-specific instructions. A partially fine-tuned Generative Pre-trained Transformer 2 (GPT-2) encoder is then used to extract semantic representations that preserve motion and scattering-related information. An adaptive feature aggregation module further weights informative hidden states across temporal positions and semantic levels, and a relation-based meta-learning network models query-support similarity for few-shot classification. Experiments on a real low-altitude radar dataset with four target categories, namely birds, balloons, small rotary-wing UAVs, and light rotary-wing UAVs, show that the proposed method consistently outperforms conventional machine learning, deep learning, and representative few-shot baselines. Under the 20-shot setting, it achieves mean 90.85% precision, 90.47% recall, and 90.63% F1-score. The results indicate that embedding radar physical semantics into language-model-based representation learning can improve sample efficiency and recognition robustness for low-altitude radar remote sensing.

</details>


### [271] [A Sub-Scene-Based GNSS-Constrained Structure from Motion for Robust Long-Corridor UAV Image Reconstruction](https://doi.org/10.3390/rs18142321)
*Wei Huang,San Jiang,Xiangxiang Huang,Hongyun Lv,Yaqin Li,Tao Zhu*

Main category: Remote Sensing

TL;DR: 本文提出了一种基于子场景的GNSS约束SfM框架，用于解决长走廊无人机摄影测量中的“碗状效应”问题。该方法通过极线几何的相机参数梯度构建梯度一致性代价以识别稳定子场景，利用GNSS约束的增量式无结构光束法平差恢复可靠的相机位姿和绝对尺度，并通过GNSS加权BA和不等式约束GNSS融合在单控制点下优化相机参数。在四个无人机走廊数据集上，该方法有效抑制了碗状效应，平均平面精度0.040m、垂直精度0.032m、三维精度0.051m，运行时间比Colmap减少约52%，精度优于MicMac、Pix4D、Agisoft Metashape和ContextCapture。


<details>
  <summary>Details</summary>
Motivation: 长走廊无人机摄影测量中，弱成像几何会导致相机参数估计不准确，进而引起传统SfM流程中系统性的重建变形（碗状效应），现有方法难以在单控制点条件下有效抑制该问题。

Method: 1) 基于极线几何计算相机参数梯度，构建梯度一致性代价以识别稳定子场景；2) 基于图像三元组和GNSS/POS观测，执行GNSS约束的增量式无结构光束法平差，恢复可靠初始相机位姿和绝对尺度；3) 引入GNSS加权BA和不等式约束GNSS融合，在单个地面控制点下精细化相机参数。

Result: 在四个无人机走廊数据集上，平均平面精度0.040m、垂直精度0.032m、三维精度0.051m，仅需一个控制点；运行时间比Colmap减少约52%；三维检查点RMSE（0.051m）优于MicMac（0.056m）、Pix4D（0.074m）、Agisoft Metashape（0.078m）和ContextCapture（0.060m）。

Conclusion: 所提出的子场景GNSS约束SfM框架能有效抑制长走廊无人机摄影测量中的碗状效应，在仅使用单个控制点的情况下实现高精度、高效率的三维重建，性能优于多种现有商业和开源方案。

与遥感交叉或改进方案: 交叉/改进可行性：95%。该论文本身就是遥感（无人机摄影测量）领域的核心方法，可直接应用于遥感三维重建。改进方向：1) 可将子场景识别策略扩展到多光谱或高光谱影像的配准中，提升多模态数据联合重建的稳健性；2) GNSS约束策略可引入RTK/PPK高精度定位与LiDAR点云融合，进一步抑制长条带状地形（如海岸线、输电线路）的重建变形；3) 梯度一致性代价函数可结合深度学习特征匹配网络（如SuperPoint+SuperGlue）提升弱纹理区域的子场景划分鲁棒性。

Abstract (英文): In long-corridor Unmanned Aerial Vehicle (UAV) photogrammetry, weak imaging geometry can compromise camera parameter estimation and lead to systematic reconstruction deformation, commonly referred to as the bowl effect, in conventional structure-from-motion (SfM) pipelines. To address this problem, this paper proposes a sub-scene-based GNSS (Global Navigation Satellite System) constrained SfM framework for robust long-corridor UAV photogrammetric reconstruction. First, camera parameter gradients derived from epipolar geometry are used to construct a gradient-consistency cost for identifying stable sub-scenes. Second, GNSS-constrained incremental structureless bundle adjustment (BA) is performed to recover reliable initial camera poses and absolute scale based on image triplets and GNSS/POS observations. Finally, GNSS-weighted BA and inequality-constrained GNSS fusion are introduced to refine camera parameters under a single ground control point (GCP). Experiments on four UAV corridor datasets demonstrate that the proposed method effectively suppresses the bowl effect and stabilizes camera parameter estimation. The proposed method achieves average planar, vertical, and three-dimensional accuracies of 0.040 m, 0.032 m, and 0.051 m, respectively, using only one control point. Compared with the standard Colmap pipeline, the runtime is reduced by approximately 52%. In addition, the proposed method achieves a lower average three-dimensional checkpoint RMSE (0.051 m) than MicMac (0.056 m), Pix4D (0.074 m), Agisoft Metashape (0.078 m) and ContextCapture (0.060 m).

</details>


### [272] [Climate-Driven Phenological Responses of Fagus sylvatica Across European Climatic Zones Using Remote Sensing](https://doi.org/10.3390/rs18142314)
*Hasan Burak Özmen,Katalin Csilléry,Alper Ahmet Özbey,Esra Tunç Görmüş,Egor Prikaziuk,Shawn C. Kefauver,Gordana Kaplan*

Main category: Remote Sensing

TL;DR: 本研究利用24年遥感数据（MODIS）和气候数据，分析了欧洲七种气候区356个欧洲山毛榉优势地点的物候气候响应，通过敏感性分析和综合物候气候偏离指数，揭示了不同气候区和海拔梯度下山毛榉物候对气候变化响应的异质性，发现春季温度敏感性在多个大陆性气候区最明显，而降水敏感性在生长季长度和秋季物候中更突出。


<details>
  <summary>Details</summary>
Motivation: 气候变化正在重塑森林生态系统的物候、生产力和恢复力，但欧洲山毛榉林在不同气候区和海拔梯度下对气候变化的物候响应尚缺乏系统性的遥感评估，尤其是物候敏感性和综合偏离指数的空间异质性研究不足。

Method: 使用24年MODIS卫星导出的地表物候数据和气候数据，从EUFGIS数据库中选取356个山毛榉优势地点，经土地覆被均质性过滤后保留274个；基于ESA WorldCover进行均质性过滤以减少混合像元影响；分析涵盖欧洲七种气候区；通过气候-物候敏感性分析和综合物候气候偏离指数（整合气候趋势、物候偏移和年际变异性）进行评估。

Result: 物候敏感性在不同气候区和物候阶段存在差异；温度敏感性在多个大陆性气候区的春季最明显；降水敏感性在某些区域的生长季长度和秋季物候中更突出；综合物候气候偏离分析显示欧洲山毛榉分布区内的区域剖面并不均匀，尽管变暖普遍，但降水趋势、物候偏移和年际变异在不同气候区差异显著。

Conclusion: 欧洲山毛榉林对气候变化的物候响应具有异质性和区位特异性，综合偏离指数不应被解释为生态脆弱性或风险的直接度量。

与遥感交叉或改进方案: 交叉/改进可行性：85%。该研究为遥感物候学提供了优秀的分析框架。改进方案：（1）将MODIS（250m-1km）替换为更精细的遥感数据如Landsat（30m）或Sentinel-2（10-20m），结合时空融合算法，可减少混合像元对物候提取的干扰；（2）引入叶面积指数（LAI）/日光诱导叶绿素荧光（SIF）等遥感产品作为物候状态的补充指标，提高对光合作用物候的刻画精度；（3）将物候-气候敏感性分析与生态过程模型（如Pheno-Flex、PhenoClim）耦合，增强对物候趋势的预测能力；（4）利用遥感衍生的森林结构参数（树高、冠层覆盖度）分层分析，揭示林分结构对物候-气候响应的影响。

Abstract (英文): Climate change is increasingly altering forest ecosystems worldwide, reshaping species phenology, productivity, and resilience. In this study, we evaluate the phenoclimatic responses of European beech (Fagus sylvatica L.) forests across Europe by assessing their phenological responses to climate change across climatic zones and altitudinal gradients using remote-sensing data. We used 24 years of satellite-derived land-surface phenology and climate data to quantify phenological trends at 356 beech-dominant locations from the EUFGIS database, of which 274 remained after land-cover homogeneity and data-quality filtering. To reduce land-cover mixing at the MODIS resolution, we applied a land-cover homogeneity filter based on ESA WorldCover. The analysis was structured across the seven climatic zones in Europe. Phenological responses to climate change were assessed through climate–phenology sensitivity analyses and a composite phenoclimatic departure index integrating climatic trends, phenological shifts, and interannual variability. Phenological sensitivity varied across climatic zones and phenological phases. Temperature-related sensitivity was most evident in spring in several continental zones, whereas precipitation sensitivity was more apparent for growing-season length and autumn timing in some regions. The composite phenoclimatic departure analysis showed that regional profiles were not uniform across the European beech range. Although warming was widespread, precipitation trends, phenological shifts, and interannual variability differed strongly among zones. These findings demonstrate heterogeneous and location-specific phenoclimatic responses across Europe, but the departure index should not be interpreted as a direct measure of ecological vulnerability or risk.

</details>


### [273] [Removing Cirrus-Induced Errors in Operational Landsat 8 and 9 Daytime Surface Temperature Products over Waters](https://doi.org/10.3390/rs18142317)
*Bo‐Cai Gao,Rong-Rong Li,Marcos J. Montes*

Main category: Remote Sensing

TL;DR: 该研究针对Landsat 8/9地表温度产品中薄卷云导致的误差问题，提出利用Band 9（卷云波段）的TOA反射率与L2 ST产品之间的相关性，对白天水体区域进行薄卷云校正。校正后ST数据标准差降至约1K，与无卷云条件下的结果相当。


<details>
  <summary>Details</summary>
Motivation: Landsat Level 2地表温度产品采用单通道算法生成，未考虑低温薄卷云的热辐射影响，导致卷云污染区域ST数据误差可达10K以上，严重影响数据产品的可用性。

Method: 利用Landsat Band 9（卷云波段，1.36-1.38μm）的Level 1 TOA反射率与Level 2 ST数据产品之间的相关性，建立校正关系，从白天水体区域的ST数据中去除薄卷云引起的误差。

Result: 校正后的白天ST数据集标准差降至约1K，与无卷云条件下的结果相似，有效消除了卷云带来的显著误差。

Conclusion: 该方法可有效校正白天水体区域ST产品中的薄卷云误差，建议L2 ST数据用户测试并推广应用该校正方法。

与遥感交叉或改进方案: 交叉/改进可行性：85%。本身为遥感论文。改进方案：(1) 将该方法扩展至陆地水体（如湖泊、河流）及复杂地表场景，评估不同地表类型下的校正效果；(2) 结合夜间卷云检测方法（如利用Band 10/11热红外通道的亮温差异），实现昼夜通用的卷云校正；(3) 引入机器学习模型（如随机森林或神经网络）建立Band 9反射率与ST误差之间的非线性映射，提升校正精度；(4) 将该方法融入Landsat自动化处理流程，实现L2SP产品的卷云误差实时校正模块。

Abstract (英文): Land surface reflectance and temperatures data products are generated from data acquired with the Operational Land Imager and Thermal Infrared Sensor on the Landsat 8 and 9 satellites. The surface temperatures (ST) in the Level 2 surface products (L2SP) are produced using a single channel algorithm, which takes no consideration of the presence of cold thin cirrus clouds, that re-emit thermal radiation at lower temperatures. The errors in the ST data products over cirrus-contaminated areas can be larger than 10 K. Our recent analysis of Level 1 top of the atmosphere (TOA) data products (L1TP) and L2SP data products acquired over different geographic regions, show that it is possible to use the correlations between the Level 1 TOA reflectances of Band 9 (referred as the cirrus band) and the Level 2 ST data products to remove thin cirrus induced errors in the daytime Level 2 ST data products over water. The corrected daytime ST datasets have standard deviations reduced to about 1 K, similar to results with no cirrus present. End users of the Level 2 ST data products are encouraged to test this method to make cirrus corrections to the existing daytime ST data products, and to more broadly apply its results and further its applicability.

</details>


### [274] [Exploring the Potential of Machine Learning Post-Processing to Generate ERA5-Consistent Atmospheric Profiles from Geostationary Satellite Retrievals](https://doi.org/10.3390/rs18142310)
*Daehyeon Han,Minki Choo,Sihun Jung,Juhyun Lee,Hyunyoung Choi,Jungho Im*

Main category: Remote Sensing

TL;DR: 本文提出一种基于Light Gradient Boosting Machine (LGBM)的数据驱动后处理框架，利用ERA5再分析数据对地球静止成像仪（AMI）反演的大气温湿廓线进行精细化校正。四年评估结果表明，该方法能小幅但一致地降低混合比和相对湿度的均方根误差，部分缓解夏季湿偏误差，但温度改进有限，揭示了当前成像仪信息容量的固有限制。


<details>
  <summary>Details</summary>
Motivation: 地球静止成像仪（如AMI）受限于光谱通道数量和数值天气预报背景场的系统偏差，其反演的大气温湿廓线存在系统性误差，亟需一种不依赖辐射传输模型的轻量级后处理方法来提升廓线质量。

Method: 利用2019-2023年四年的逐小时观测数据，以LGBM模型为回归器，以AMI原始反演廓线为输入特征，以ERA5再分析资料为目标标签，分别训练温度、混合比和相对湿度的校正模型，形成数据驱动的后处理精细化框架。

Result: 与ERA5对比：混合比RMSE降低约0.04 g kg⁻¹（6-7%），相对湿度降低约1.9个百分点（≈14%），温度仅降低约0.02 K（2-3%）。与探空对比：温度RMSE略有增加（<1%，低层增幅更大），混合比改善2-3%，相对湿度改善6-7%。SHAP特征重要性分析表明AMI水汽通道贡献最大。

Conclusion: 机器学习后处理能够有效校正地球静止成像仪大气廓线的系统性偏差（特别是湿度产品的湿偏），但受限于当前成像仪的光谱信息容量，温度廓线的改进空间有限，表明仅靠数据后处理无法完全弥补传感器硬件的不足。

与遥感交叉或改进方案: 交叉/改进可行性：95%。该框架可直接迁移至我国FY-4系列卫星的AGRI载荷以及Himawari的AHI载荷，用于生成再分析一致性的大气廓线产品。改进方向：1）引入多时次序列特征（如LSTM时序模型）以利用大气状态的时序连续性；2）联合可见光与红外通道的多光谱信息作为输入；3）对不同高度层采用分层或自适应权重策略以缓解低层温度校正失真问题；4）可进一步扩展至气溶胶、云参数等其他廓线产品的校正。

Abstract (英文): Accurate atmospheric temperature and humidity profiles are fundamental to weather monitoring and prediction. Geostationary imagers such as the Advanced Meteorological Imager (AMI) provide continuous observations and enable profile retrievals through radiative transfer–based algorithms; however, these products remain affected by systematic biases associated with the limited number of spectral channels and reliance on background fields from numerical weather prediction models. This study presents a data-driven post-processing framework to generate reanalysis-consistent profiles by refining AMI-retrieved temperature, mixing ratio, and relative humidity profiles using Light Gradient Boosting Machine (LGBM) models trained with ERA5 reanalysis data. Using four years (2020–2023) of hourly observations, the refined profiles were evaluated against both ERA5 and independent radiosonde measurements. Relative to ERA5, the refinement yields modest but consistent reductions in root mean square error (RMSE), including approximately 0.04 g kg−1 (6–7%) for mixing ratio and 1.9 percentage points (≈14%) for relative humidity, while temperature shows a smaller error reduction of about 0.02 K (2–3%). When compared with radiosondes, temperature RMSE shows a marginal increase overall (&lt;1%) with a larger increase in the lower troposphere, whereas improvements are observed for mixing ratio (2–3%) and relative humidity (6–7%). Seasonal and diurnal analyses reveal systematic error structures in the original AMI profiles, particularly wet-bias patterns in summer moisture fields, which are partially mitigated by the refinement. Feature-importance analysis using Shapley Additive Explanations (SHAP) identifies the dominant contribution of AMI water vapor channels, consistent with their known vertical sensitivity. Overall, this long-term evaluation demonstrates the feasibility of machine learning-based refinement for geostationary imager atmospheric profiles, while also highlighting inherent limitations related to the information content of current-generation imagers.

</details>


<div id='ISPRS J P&RS'></div>

# ISPRS J P&RS [[Back]](#toc)

### [275] [PRGD: Proximal Residual-Guided Diffusion with pattern-aware experts for remote sensing image super-resolution](https://doi.org/10.1016/j.isprsjprs.2026.06.037)
*Yantao Ji,Chen Li,Zhang Yulong,Lihua Tian,Xin Zan*

Main category: ISPRS J P&RS

TL;DR: 该论文没有提供摘要信息，无法生成TL;DR总结。


<details>
  <summary>Details</summary>
Motivation: 该论文没有提供摘要信息，无法分析动机。

Method: 该论文没有提供摘要信息，无法分析方法。

Result: 该论文没有提供摘要信息，无法分析结果。

Conclusion: 该论文没有提供摘要信息，无法分析结论。

与遥感交叉或改进方案: 交叉/改进可行性：70%。交叉/改进可行性：无法评估。由于缺乏摘要信息，无法判断论文主题与遥感领域的关联性或交叉可能性。

Abstract (英文): No abstract available in OpenAlex.

</details>


### [276] [Unveiling strong microclimatic cooling by small wetlands in China: landscape context-dependent spatial and climatic drivers](https://doi.org/10.1016/j.isprsjprs.2026.07.002)
*Yue Jia,Wen J. Wang,Shengjie Ba,Yuyu Zhou,Tianxing He,Suri G. Bao,Paolo Cherubini,Liu Z,Mai He Li,Yue Wang,Hong He*

Main category: ISPRS J P&RS

TL;DR: 没有提供摘要内容，无法进行分析。


<details>
  <summary>Details</summary>
Motivation: 未提供摘要，无法确定研究动机。

Method: 未提供摘要，无法确定研究方法。

Result: 未提供摘要，无法确定研究结果。

Conclusion: 未提供摘要，无法确定研究结论。

与遥感交叉或改进方案: 交叉/改进可行性：70%。由于缺乏摘要内容，无法评估与遥感的交叉可行性。

Abstract (英文): No abstract available in OpenAlex.

</details>


### [277] [CDHNet: An implicit calibration and difference-guided hierarchical perception change detection network for remote sensing images](https://doi.org/10.1016/j.isprsjprs.2026.06.040)
*Hongquan Cheng,Jie Zheng,Kunlun Qi,Huayi Wu,Shifa Ma*

Main category: ISPRS J P&RS

TL;DR: 该论文没有提供摘要内容，因此无法进行分析。


<details>
  <summary>Details</summary>
Motivation: 由于缺少摘要，无法明确研究动机。

Method: 未能获取论文方法信息。

Result: 未能获取论文结果信息。

Conclusion: 未能获取论文结论信息。

与遥感交叉或改进方案: 交叉/改进可行性：0%。由于没有摘要信息，无法进行任何交叉分析。

Abstract (英文): No abstract available in OpenAlex.

</details>


### [278] [Monitoring mining impacts using harmonic decomposition of time series vegetation indices](https://doi.org/10.1016/j.isprsjprs.2026.07.001)
*Vincent E. Nwazelibe,Moritz Kirsch,Samuel T. Thiele,Farid Djeddaoui,Weikang Yu,Richard Gloaguen,Raimon Tolosana‐Delgado*

Main category: ISPRS J P&RS

TL;DR: 无法获取摘要内容，无法进行分析。


<details>
  <summary>Details</summary>
Motivation: 由于缺乏摘要，无法确定研究动机。

Method: 无法获取摘要，无法分析方法。

Result: 无法获取摘要，无法总结结果。

Conclusion: 无法获取摘要，无法总结结论。

与遥感交叉或改进方案: 交叉/改进可行性：0%。由于没有提供任何论文摘要内容，无法进行遥感交叉分析。

Abstract (英文): No abstract available in OpenAlex.

</details>
