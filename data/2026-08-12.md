...
- [JAG](#JAG) [Total: 10]
- [Remote Sensing](#Remote Sensing) [Total: 102]
...
Abstract (英文): Most learned image compression systems rely on a single latent representation combined with a hyperprior, which limits their ability to efficiently capture image structure across spatial scales. In this work, we propose a hierarchical latent representation to improve the efficiency of the entropy model. By using multiple latents at different scales, each with its own entropy model, we better capture the spatial structure of the latent representation. Our experiments show that this approach achieves a 17.9% BD-rate reduction over VVC on Kodak, demonstrating the effectiveness of multi-scale latent representations. Furthermore, the approach is orthogonal to other advances in learned image compression, making it a versatile addition to existing methods.
...
Abstract (英文): AI-assisted facial phenotyping supports rare genetic disorder prioritization by retrieving visually similar diagnosed cases from facial image reference databases such as the GestaltMatcher Database (GMDB). Existing GestaltMatcher-based retrieval frameworks compare each test image with individual gallery images in a facial phenotype embedding space. However, this pointwise formulation does not fully exploit available evidence, because patients may have multiple images and disorders may be represented by multiple diagnosed gallery patients.
...
Abstract (英文): Reinforcement learning (RL) has shown promising performance in autonomous driving, yet ensuring the safety of online RL policies remains challenging due to insufficient exposure to safety-critical driving scenes. The long-tailed nature of real-world traffic situations makes dangerous and rare interactions difficult to encounter through conventional sampling, limiting the ability of RL policies to learn robust safety behaviors. Existing methods improve training diversity by synthesizing challenging scenes or adversarial situations. However, these approaches typically optimize scene generation objectives separately from the evolving policy, without explicitly modeling how generated perturbations relate to the current policy's weaknesses and learning needs. In this paper, we propose Threat-guided Policy-aware Scene Perturbation (TPSP) for safe autonomous driving with online RL. TPSP introduces a policy-aware scene encoder to capture the interaction between policy behaviors and surrounding environments, enabling scene perturbation aligned with the current policy. Based on this representation, TPSP selectively perturbs critical objects rather than applying uniform modifications across the scene. Furthermore, we develop a threat-guided optimization strategy that evaluates perturbed scenes through threat-level differences between policy rollouts on original and perturbed scenes, guiding the generation of safety-critical scenes with higher training value. Comprehensive experiments demonstrate that TPSP improves safety learning efficiency, achieving strong safety performance on NAVSIM v2 with approximately 4 million kilometers of simulated driving data. Ablation studies verify that policy-aware targeted perturbations provide more informative safety-critical experiences than random or policy-unaware strategies, enabling safer driving under limited interaction budgets.
...
Abstract (英文): The Traveling Salesperson Problem (TSP) is one of the best-known problems in computer science and arises in many engineering applications, such as smart vehicles and intelligent transportation systems. In the "Euclidean" case, each node is defined by its coordinates in the plane and distances are computed using the Euclidean metric. In the Constraint Programming (CP) literature, the Euclidean TSP is typically addressed by computing the full distance matrix and treating it as a general case; however this approach ignores the geometric information carried by the points' coordinates. In this work, we propose new filtering algorithms, implemented in Constraint Logic Programming (CLP), that exploit such geometric information to achieve stronger constraint propagation than existing approaches. Moreover, we show how this methodology can be extended to other Euclidean variants of the TSP, including the Euclidean Generalized Traveling Salesperson Problem (EGTSP), which is relevant in practical routing and logistics applications. Experimental results demonstrate the computational advantages of the proposed approach.
...
Abstract (英文): AI agents are increasingly used in mathematics research, but it is often unclear how to use them effectively. Towards this, we present an extensive case study of how AI was used to improve bounds on the Grothendieck constant $K_G$, which captures the hardness between combinatorial problems and their continuous relaxations. Specifically, while the precise value of $K_G$ is not known, we recently tightened the best known bounds to \[
  \frac{6π}{11}
  \;\le\;
  K_G
  \;\le\;
...
### [197] [V-BUILD: A graph Transformer-based decision framework for selective updating of vector building data](https://doi.org/10.1016/j.jag.2026.105507)
...
### [198] [Reducing grid artifacts in air temperature downscaling with a Volume-Preserving interpolation preprocessing step](https://doi.org/10.1016/j.jag.2026.105515)
...
### [199] [Global cloud phase monitoring from FY-3G multi-angle polarimetric observations using the new MAPLE-CP framework with independent multi-site lidar validation](https://doi.org/10.1016/j.jag.2026.105512)
...
### [200] [Linear-Fitting calibration algorithm for Aerosol-Cloud detection channels at 532 nm of the DQ-1/ACDL](https://doi.org/10.1016/j.jag.2026.105504)
...
### [201] [Early detection of rice bacterial leaf blight disease using canopy hyperspectral data and machine learning](https://doi.org/10.1016/j.jag.2026.105511)
...
### [202] [Global assessment of karst drought propagation from multisource Earth Observation: Epikarst-mediated rapid shallow coupling and delayed deep responses](https://doi.org/10.1016/j.jag.2026.105516)
...
### [203] [Brick kilns across India: A temporal analysis of brick kiln distribution and technology types](https://doi.org/10.1016/j.jag.2026.105496)
...
### [204] [Integrating spatiotemporal prior knowledge from co-segmented time-series into factor-driven modeling for forest insect damage warning](https://doi.org/10.1016/j.jag.2026.105518)
...
### [205] [Feature selection from Multi-Angular TECIS data for forest biomass estimation using a Model-driven sensitivity analysis approach](https://doi.org/10.1016/j.jag.2026.105519)
...
### [206] [Quantifying earthquake-induced building damage from VIIRS nighttime lights: A multi-scale framework with deep learning–based area validation](https://doi.org/10.1016/j.jag.2026.105513)
...
### [207] [Agreement–Disagreement Guided Knowledge Transfer for Cross-Scene Hyperspectral Imaging](https://doi.org/10.3390/rs18152601)
...
### [208] [Improving Vegetation Mapping from LiDAR Point Clouds Using a Transmissivity-Based Feature](https://doi.org/10.3390/rs18152602)
...
### [209] [Infrared–Visible Multi-Sensor Fusion for UAV Photovoltaic Defect Detection Under Real-World Weak Misalignment](https://doi.org/10.3390/rs18152607)
...
### [210] [SPFMamba: A Mamba-Based Network with Semantic Prompt and Frequency-Adaptive Fusion for Remote Sensing Image Semantic Segmentation](https://doi.org/10.3390/rs18152592)
...
### [211] [Empirical Optimization of the Stumpf Method Parameter for Satellite-Derived Bathymetry](https://doi.org/10.3390/rs18152613)
...
### [212] [Research on an Absolute Radiometric Calibration Method Based on a Point Source Array](https://doi.org/10.3390/rs18152590)
...
### [213] [Mapping Neighborhood Spatial Structure in Traditional Home Gardens Using UAV-Derived 3D Canopy Models](https://doi.org/10.3390/rs18152605)
...
### [214] [Temporal-Variation-Resistant Bidirectional Convolution-Transformer GAN for Remote Sensing Image Spatiotemporal Fusion](https://doi.org/10.3390/rs18152597)
...
### [215] [Assessing the Impact of Spatial Resolution and Aggregation Method on Sentinel-2 NDVI Time Series in Grasslands of Mainland Spain](https://doi.org/10.3390/rs18152611)
...
### [216] [PCFD-Net: A Parallel Collaborative Fusion-Detection Network for SAR and Optical Imagery](https://doi.org/10.3390/rs18152595)
...
### [217] [PSG-RTDETR: Towards Stable Cross-Scale Feature Fusion for Small Object Detection](https://doi.org/10.3390/rs18152609)
...
### [218] [Remote Sensing-Assisted Stockpile Landslide Monitoring Based on Change Detection Analysis and Identification of Topographical Failure Precursors](https://doi.org/10.3390/rs18152594)
...
### [219] [Shoreline Behavior at California Groin Fields from Satellite-Based Measurements](https://doi.org/10.3390/rs18152608)
...
### [220] [FDM-Net: A Multi-Level Feature Aggregation Network Based on Frequency-Decomposition for Hyperspectral Image Classification](https://doi.org/10.3390/rs18152603)
...
### [221] [UAV Hyperspectral Characterization of Spatial Color Heterogeneity in Alpine Karst Lakes](https://doi.org/10.3390/rs18152596)
...
### [222] [Scaling Foliar Phenolics from Airborne Imaging Spectroscopy to Sentinel-2 Across Diverse Vegetation Types](https://doi.org/10.3390/rs18152599)
...
### [223] [MLS-CAPS: Optimising Path Spacing for Mobile Laser Scanning of Vegetation Structure via Completeness Analysis](https://doi.org/10.3390/rs18152606)
...
### [224] [Can UAV-Derived Maize Canopy Traits Inform Soil Mineral Nitrogen Estimation? Linking Above- and Below-Ground Nitrogen Dynamics in Black Soils](https://doi.org/10.3390/rs18152588)
...
### [225] [Land Subsidence-Induced Horizontal Displacement Along the High-Speed Rail in Central Taiwan: An Integrated Multi-Temporal InSAR, GNSS, and Leveling Approach](https://doi.org/10.3390/rs18152612)
...
### [226] [Centroid-Preserving Dynamic Star Image Deblurring for Remote Sensing Satellite Attitude Measurement via Physics-Guided Bi-Level Optimization](https://doi.org/10.3390/rs18152610)
...
### [227] [Perspectives and Challenges of Machine Learning for Applications in Agriculture and Vegetation Using Remote Sensing](https://doi.org/10.3390/rs18152589)
...
### [228] [Geometry-Aware InSAR Feedback Purification Sampling for Negative Sample Selection in Landslide Susceptibility Assessment: A Case Study in the Shigatse Region](https://doi.org/10.3390/rs18152591)
...
### [229] [SLAM–UAV LiDAR Co-Registration for Individual-Tree Carbon Across a Tropical Dry-Forest Canopy Gradient](https://doi.org/10.3390/rs18152604)
...
### [230] [Sensitivity of Vegetation Greenness to Multi-Depth Soil Moisture on the Mongolian Plateau: Nonlinear Responses Revealed by RF–SHAP](https://doi.org/10.3390/rs18152600)
...
### [231] [Airborne Point Cloud Fusion with Local Plane Constraints for Advanced Semantic Consistency](https://doi.org/10.3390/rs18152598)
...
### [232] [Integrating Multi-Source Geoscientific Data via Geologically Constrained Feature Engineering for Gold Prospectivity Mapping: A Case Study of Jiaoxibei, China](https://doi.org/10.3390/rs18152593)
...
### [233] [Lightweight Bayesian SAR Image Object Detection and Recognition Method Based on Heavy-Tail Prior and Variational Inference](https://doi.org/10.3390/rs18152627)
...
### [234] [Fine-Scale Maturity Recognition of Eucalyptus Plantations Based on Deep Learning and Multimodal Feature Fusion](https://doi.org/10.3390/rs18152632)
...
### [235] [Landslide Susceptibility Evaluation Based on Deep Learning and Imbalanced Sampling at Multi-Scale](https://doi.org/10.3390/rs18152617)
...
### [236] [High-Squint Imaging Method for Spaceborne Bistatic SAR Considering Orbit Curvature Effect](https://doi.org/10.3390/rs18152640)
...
### [237] [A Hybrid Multicriteria Index for Assessing Documentary-Methodological Robustness in Flood Mapping: Integrating Documentary Evidence, Entropy-Based Weighting, Remote Sensing, DEMs, Hydrological Data, and Statistical Validation](https://doi.org/10.3390/rs18152641)
...
### [238] [Comprehensive Use of GNSS Vertical Deformation and GRACE/GFO Data to Invert the Joint Drought Index of Three Central China Provinces](https://doi.org/10.3390/rs18152633)
...
### [239] [Radiometric Sensitivity Requirements for Detecting Live Coral and Seagrass Cover Using Spaceborne Imaging Spectroscopy](https://doi.org/10.3390/rs18152643)
...
### [240] [LDM-PUNet: A Lightweight Network for Denoising and Phase Unwrapping SAR Interferograms in Mining Deformation Monitoring](https://doi.org/10.3390/rs18152616)
...
### [241] [MSF-Net: A Multimodal SAR–Optical Fusion Network for Agricultural Land Use Classification in Smallholder Landscapes of Northern Benin](https://doi.org/10.3390/rs18152622)
...
### [242] [A China-Specific Near-Real-Time GNSS Water Vapor Retrieval Model Based on LightGBM](https://doi.org/10.3390/rs18152637)
...
### [243] [A Cross-System Remote Sensing Framework for Diagnosing Event-Scale Soil Wetting, Vertical Propagation, and Pre-Cipitation Thresholds Across China’s Croplands](https://doi.org/10.3390/rs18152614)
...
### [244] [Robust Wheat Residue Cover Quantification Under Moisture Variability from ASD Spectroscopy Using Conditional Autoencoder Normalization and Linear Unmixing](https://doi.org/10.3390/rs18152636)
...
### [245] [A Data-Driven InSAR Failure-Risk Index for Early Warning of Mining Infrastructure Instability: The Çöpler Case Study, İliç, Türkiye](https://doi.org/10.3390/rs18152624)
*Mahmut Çavur*
...
### [246] [Assessing Individual-Building Vertical Light Exposure in Urban Environments with a Residual Cascade Framework](https://doi.org/10.3390/rs18152621)
...
### [247] [Spatial Heterogeneity and Driving Mechanisms of Carbon Storage on the Chinese Loess Plateau](https://doi.org/10.3390/rs18152630)
...
### [248] [Machine Learning-Based Atmospheric Radiation Calculation Incorporating Earth Curvature](https://doi.org/10.3390/rs18152635)
...
### [249] [HGRHDNet: Hierarchical Gated Residual Fusion and High-Frequency Guided Deformable Upsampler Network for Boundary-Enhanced Binary Urban Green Space Extraction](https://doi.org/10.3390/rs18152620)
...
### [250] [Characterizing Free-Atmospheric Wind Fields and Their Statistical Associations with Aerosol Variability in the Huaihe River Basin](https://doi.org/10.3390/rs18152626)
...
### [251] [Remote Sensing-Based Identification of Sensitive Environmental Intervals Controlling Drought Propagation Time Across China](https://doi.org/10.3390/rs18152615)
...
### [252] [FAD-Net: Frequency Alignment Dual-Branch Network for Hyperspectral Image Super-Resolution](https://doi.org/10.3390/rs18152631)
...
### [253] [An Interpretable Uncertainty-Aware Framework for Landslide Susceptibility Mapping Based on Weak Supervision and Probabilistic Inference](https://doi.org/10.3390/rs18152619)
...
### [254] [Joint Interpretation of Archaeological, Geological, Geophysical and Remotely Sensed Data for Fluvial Geomorphology: The Case of the Calore River Meander North of Benevento (Italy)](https://doi.org/10.3390/rs18152629)
...
### [255] [TRB-Net: Terrain-Residual and Boundary-Assisted Multimodal Martian Landslide Segmentation on a Local MMLSv2 Split](https://doi.org/10.3390/rs18152638)
...
### [256] [Removal of RLAN Interference from C-Band Weather Radar Data: Algorithm and Case Studies](https://doi.org/10.3390/rs18152625)
...
### [257] [Disentangling Spectrally Similar Urban Vegetation via Semantic Segmentation-Guided Object Analysis and Multi-Periodic Phenological Features](https://doi.org/10.3390/rs18152623)
...
### [258] [Trait-Specific Contributions of UAV Multispectral, RGB and Structural Features to Soybean SPAD and Plant Height Phenotyping](https://doi.org/10.3390/rs18152642)
...
### [259] [Multi-Agent Pipeline for Crop-Type Classification and Label Refinement Using Sentinel-1 SAR Time Series and Field-Level Temporal Features in the Nakasatsunai Region, Hokkaido](https://doi.org/10.3390/rs18152628)
...
### [260] [Spatial Pattern of Extreme Rainfall-Induced Forest Aboveground Biomass Loss and Its Influencing Factors in the Tableland-Gully Region of the Loess Plateau, China](https://doi.org/10.3390/rs18152639)
...
### [261] [A Persistent Scatterer Interferometry-Based Parametric Framework for Characterizing Pre-, Co-, and Post-Seismic Surface Deformation: Application to the 2025 Dingri Earthquake (Southern Tibet)](https://doi.org/10.3390/rs18152634)
...
### [262] [Surface-Water Fragmentation and Heterogeneous Responses of Dish-Shaped Sub-Lakes in Poyang Lake During the 2022 Extreme Drought](https://doi.org/10.3390/rs18152618)
...
### [263] [Shape-Prior Dynamic Refinement Network with Diffusion Wavelets for Fine-Grained Ship Detection in Remote Sensing Images](https://doi.org/10.3390/rs18162649)
...
### [264] [FFT-Based Multiscale Frequency Decomposition for Atmospheric Lidar Attenuated Backscatter Profile Forecasting](https://doi.org/10.3390/rs18162663)
...
### [265] [Improving Accuracy of Low-Altitude Three-Dimensional Wind Field Reconstruction via Multi-Task Learning](https://doi.org/10.3390/rs18162655)
...
### [266] [GSANet: Geometric Structure-Aware Siamese Network for 3D Change Detection](https://doi.org/10.3390/rs18162647)
...
### [267] [DBKNet: A Dual-Encoder KAN Segmentation Network for Joint Extraction of Photovoltaic Power Stations and Impervious Surfaces in Arid Regions](https://doi.org/10.3390/rs18162662)
...
### [268] [Detection and Tracking of Medicanes Through DeMeTrA Self-Supervised Vision Transformer](https://doi.org/10.3390/rs18162659)
...
### [269] [Daytime–Nighttime Contrasts in Morphology–LST Associations Across Urban Functional Zones Under Heatwave Conditions: Evidence from Beijing and Nanjing, China](https://doi.org/10.3390/rs18162666)
...
### [270] [Extracting Value from Fused Aerial and Terrestrial LiDAR Scans](https://doi.org/10.3390/rs18162644)
...
### [271] [Predicting Future Forest Plantation Establishment Outcomes from UAV-Derived Pre-Planting Environmental Conditions](https://doi.org/10.3390/rs18162665)
...
### [272] [Use of Multi-Source Remote Sensing Data to Understand Long-Term Wetland Dynamics and Water Environment Responses in the Chaohu Lake Basin](https://doi.org/10.3390/rs18162646)
...
### [273] [Maturity-Gated Anti-Forgetting Sampling for Remote-Sensing Object Detection Training](https://doi.org/10.3390/rs18162657)
...
### [274] [Task-Guided CycleGAN for Domain-Adapted Coastal Boundary Segmentation in Remote Sensing Images](https://doi.org/10.3390/rs18162658)
...
### [275] [Marine Oil Film Segmentation Based on GCN-IGJO Method](https://doi.org/10.3390/rs18162660)
...
### [276] [MPC-DETR: A Multi-Scale Patch Context Transformer for Small Object Detection in UAV Imagery](https://doi.org/10.3390/rs18162650)
...
### [277] [Influence of Dust Aerosol on Temperature Field and Boundary Layer During a Typical Dust Storm Event in China](https://doi.org/10.3390/rs18162656)
...
### [278] [TinyCapsViT: Ultra-Lightweight Hyperspectral and Multispectral Image Classification for UAV Edge Deployment Using Capsule Vision Transformers](https://doi.org/10.3390/rs18162661)
...
### [279] [Fine-Scale Eddies Revealed by the Surface Water and Ocean Topography (SWOT) Mission](https://doi.org/10.3390/rs18162654)
...
### [280] [Analyzing Variability and Trends in NDVI: A Remote-Sensing Approach for Long-Term Monitoring of Urban Green Spaces](https://doi.org/10.3390/rs18162652)
...
### [281] [Red-Edge Vegetation Index Optimization Within Phenological Windows for Discriminating Rice from Artificial Grassland: A Case Study in Jurong, China](https://doi.org/10.3390/rs18162653)
...
### [282] [Seasonally Optimized Interferometric Network Stacking-InSAR for Subsidence Detection of Mined-Out Zones in Alpine Terrain: A Case Study of the Songhu Iron Mine, Xinjiang, China](https://doi.org/10.3390/rs18162648)
...
### [283] [Remote Sensing-Based Detection and Statistical Characterization of Large Landslides in the Transylvanian Basin](https://doi.org/10.3390/rs18162645)
...
### [284] [Distinguishing High- and Low-Yielding Durum Wheat Genotypes Using UAV Spectral and Textural Data](https://doi.org/10.3390/rs18162664)
...
### [285] [Integrating Multi-Season Sentinel-1/2 and Topographic Features to Improve Tree Species Diversity Estimation Accuracy](https://doi.org/10.3390/rs18162651)
...
### [286] [Unsupervised Estimation of Post-Event Standing Urban Floodwater Depth Using Aerial Imagery and Digital Terrain Models](https://doi.org/10.3390/rs18162673)
...
### [287] [Freq-LoRA: Frequency-Domain Low-Rank Adaptation for Weather-Robust Aircraft Segmentation in EO Remote Sensing](https://doi.org/10.3390/rs18162674)
...
### [288] [Prototype-Driven Semantic Tree with Bimodal Embedding Space for Zero-Shot SAR Ship Recognition](https://doi.org/10.3390/rs18162671)
...
### [289] [Single-Tree Structural Parameter Estimation from SLAM–UAV LiDAR Data Using a Bi-Directional Cross-Attention Fusion Network](https://doi.org/10.3390/rs18162670)
...
### [290] [Mirrored Aperture Synthesis Radiometric Imaging Based on Spatial Bandpass Sampling and Quad-Beam Antennas: Design and Numerical Validation](https://doi.org/10.3390/rs18162672)
...
### [291] [Spatiotemporal Differentiation Evaluation of Flood Adaptability in Waterfront Cities Based on PSR Framework and Game Theory Combined Weighting](https://doi.org/10.3390/rs18162668)
...
### [292] [Brazilian Mangrove Under Watch: A Review of Remote Sensing Approaches for Land Cover Change Detection](https://doi.org/10.3390/rs18162667)
...
### [293] [From Multisensor Fusion to Intelligent Geospatial Monitoring: Emerging Architectures for Geotechnical Hazard Assessment](https://doi.org/10.3390/rs18162669)
...
### [294] [Advances in Multi-Scale Remote Sensing and Machine Learning for Canopy-to-Root Phenotyping of Drought Adaptation in Sorghum: A Systematic Review](https://doi.org/10.3390/rs18162676)
...
### [295] [Satellite-Based Assessment of Urban Expansion, Heating Demand, and Rooftop Photovoltaic Potential in Ulaanbaatar, Mongolia, from 2016 to 2025](https://doi.org/10.3390/rs18162675)
...
### [296] [Impact of Observation Density of Next-Generation GeoHIS on Global Numerical Model Performance: A KIM-OSSE Study](https://doi.org/10.3390/rs18162685)
...
### [297] [Automated Extraction of Long-Term Cyanobacteria Blooming Series from Landsat Imagery Using Deep Learning](https://doi.org/10.3390/rs18162682)
...
### [298] [Adaptive Spatial–Frequency Information Fusion for SAR Ship Detection](https://doi.org/10.3390/rs18162687)
...
### [299] [IceRWKV: A Novel River Ice Segmentation Network Based on Context Clustering RWKV](https://doi.org/10.3390/rs18162683)
...
### [300] [Monitoring the Concentration of Dissolved Inorganic Nitrogen and Phosphorus at the Sea Surface Using a Hyperspectral Image—A Case Study of Sheyang Estuary, Yellow Sea](https://doi.org/10.3390/rs18162686)
*Yong Xu,Dong Zhang*
...
### [301] [Long-Term Assessment of UHI and SUHI in Modena: Integrating Landsat Land Surface Temperature and Meteorological Observations](https://doi.org/10.3390/rs18162681)
...
### [302] [MRU-YOLO: Marginal-Utility-Guided Selective Local Re-Observation for Small-Object Detection in UAV Imagery](https://doi.org/10.3390/rs18162680)
*Jiajun Chen,Jinxin He,Yongzhi Wang,Peng Lu,Hengshuo Li*
...
### [303] [Perennial Crop Type Discrimination with AVIRIS-NG Hyperspectral Imagery and Machine Learning](https://doi.org/10.3390/rs18162677)
...
### [304] [RETRACTED: Biondi, F.; Malanga, C. Synthetic Aperture Radar Doppler Tomography Reveals Details of Undiscovered High-Resolution Internal Structure of the Great Pyramid of Giza. Remote Sens. 2022, 14, 5231](https://doi.org/10.3390/rs18162679)
...
### [305] [Evaluating Adaptive Classification Methods for Mangrove Mapping with Multi-Resolution Remote Sensing Imagery](https://doi.org/10.3390/rs18162684)
...
### [306] [VDCnet: Calibrated Domain Expansion and View Semantic Matching for Cross-Scene HSI Classification](https://doi.org/10.3390/rs18162688)
...
### [307] [Validation of Multi-Mission Significant Wave Height Product from CMEMS in China’s Coastal Waters](https://doi.org/10.3390/rs18162678)
...
### [308] [Airborne Streak Tube Imaging LiDAR-Based Effective Reconstruction of Urban Water Areas](https://doi.org/10.3390/rs18162689)
...
### [309] [Do maps still matter for machines: revisiting the role of choropleth maps in foundation model spatial understanding](https://doi.org/10.1080/15481603.2026.2711495)
...
### [310] [Wet‑year productivity pulses dominate asymmetric vegetation responses to precipitation variability on the Loess Plateau](https://doi.org/10.1080/15481603.2026.2713788)
...
### [311] [Comparative quality assessment of digital surface models derived from Pléiades Neo and WorldView-3 tri-stereo imagery for high-resolution 3D mapping of plastic greenhouse agricultural landscapes](https://doi.org/10.1080/15481603.2026.2706873)
...
### [312] [A thermal infrared remote sensing approach for groundwater discharge detection: a case study with SDGSAT-1 data in the Heihe River](https://doi.org/10.1080/15481603.2026.2711463)
...
### [313] [Data-efficient cross-domain transfer learning for cone yield prediction of Korean pine from ultrahigh-resolution UAV imagery](https://doi.org/10.1080/15481603.2026.2716411)
...
### [314] [TALineNet: A benchmark and network for temporally-aware continuous line segment detection in event streams](https://doi.org/10.1016/j.isprsjprs.2026.07.030)
...
### [315] [ADAPT-Tracker: Adaptive dynamic perception and trajectory-consistent network for aerial multi-object tracking](https://doi.org/10.1016/j.isprsjprs.2026.07.010)
...
### [316] [CoRE-UIR: Prior-guided common and residual experts for efficient all-in-one remote sensing image restoration](https://doi.org/10.1016/j.isprsjprs.2026.07.028)
...
### [317] [HCFNet: Hierarchical and complementary feature embedding for remote sensing change captioning](https://doi.org/10.1016/j.isprsjprs.2026.07.020)
...
### [318] [PVP-adapter: Dual-branch spatial-frequency adaptation for change detection of photovoltaic panels](https://doi.org/10.1016/j.isprsjprs.2026.07.032)
...
### [319] [A vertical FVC profile retrieval method (OLV-FVC) based on airborne LiDAR point cloud under the constraint of optical remote sensing for forest plots](https://doi.org/10.1016/j.isprsjprs.2026.07.031)
...
### [320] [Aerial Image Gravity Direction Estimation (AGE): Robust single-image estimation by fusing foundation model features with camera geometry](https://doi.org/10.1016/j.isprsjprs.2026.07.019)
...
### [321] [Drivers of spectro-temporal variability in multi-spectral soil reflectance measured from space: A continental-scale analysis](https://doi.org/10.1016/j.rse.2026.115599)
...
### [322] [A scheme to quantify the adjacency effect of shortwave radiation from highly reflective surfaces](https://doi.org/10.1016/j.rse.2026.115601)
...
### [323] [Where sky meets sea: Integrated aerosol and ocean color retrieval from PACE SPEXone multi-angle polarimetry](https://doi.org/10.1016/j.rse.2026.115603)
...
### [324] [Tracking global extreme afternoon photosynthesis depression: Frequency, duration, and drivers from satellite-based hourly GPP products](https://doi.org/10.1016/j.rse.2026.115597)
...
### [325] [A weakly supervised, wetness-based dynamic framework for wetland delineation across globally distributed sites](https://doi.org/10.1016/j.rse.2026.115604)
...
### [326] [A robust method for high-resolution C/S-band SAR image matching via pseudo-modality generation and coupled spectrum-spatial learning](https://doi.org/10.1016/j.rse.2026.115612)
...
### [327] [LAPAC: A self-adaptive atmospheric correction method for fine-resolution satellite imagery using pseudo-invariant features from Landsat archive](https://doi.org/10.1016/j.rse.2026.115602)
*Wenchang Li,Tao He*
...
### [328] [Maximizing the potential of Sentinel-2 satellite observations in mapping Sargassum seaweed in the Wider Caribbean Region](https://doi.org/10.1016/j.rse.2026.115608)
