...
- [Remote Sensing](#Remote Sensing) [Total: 37]
- [ISPRS J P&RS](#ISPRS J P&RS) [Total: 5]
- [JAG](#JAG) [Total: 14]
- [RSE](#RSE) [Total: 12]
...
Abstract (英文): High quality reference data remain a critical bottleneck for crop-type mapping at any spatial and temporal scale. Operational systems such as WorldCereal aggregate labels from heterogeneous sources such as parcel registers, national databases, field surveys, and map-derived products, each with their own biases, coverage gaps and unknown label noise. Simple global rules are inadequate, since crop phenology and observation conditions vary strongly across regions and seasons.
...
Abstract (英文): This paper presents a summary of the Competition on Adapting Foundation Models for Face Recognition Using Synthetic Training Data (AFMFR), held at the 2026 International Joint Conference on Biometrics (IJCB 2026). The competition received a total of eight valid submissions from four distinct teams across two complementary tracks: a Full Data Track, in which participants adapt the CLIP ViT-L/14 foundation model using large-scale synthetic identity data, and a Limited Data Track, designed to reflect more resource-constrained adaptation regimes. All training data was generated exclusively using IDPERTURB. Submitted solutions are ranked based on verification and identification performance across a diverse suite of benchmarks, including LFW, CFP-FP, AgeDB-30, CALFW, CPLFW, IJB-B, IJB-C, and TinyFace, using the Borda count method. Fairness evaluation is additionally conducted on the RFW dataset across four demographic groups. The results demonstrate that adaptation of the CLIP foundation model with synthetic training data substantially improves over the off-the-shelf model and, in several cases, surpasses the baseline. Notably, full fine-tuning with Sub-Center ArcFace (DMSTI-Neurotechnology) leads the Full Data Track, while rank-stabilized LoRA adaptation (Idiap-BSP) proves most effective under limited-data conditions.
...
Abstract (英文): Modern compute-intensive software must migrate across a changing ecosystem of accelerators, programming APIs, compiler stacks, and portability layers, including CUDA, OpenMP, OpenCL, and OpenMP target offload. Large language models and autonomous coding agents are increasingly proposed for such migration, but the field lacks reliable ways to measure whether they preserve the low-level parallel semantics that make translations behaviorally valid, including thread indexing, synchronization, memory management, host-device coordination, and API-specific execution structure.
...
Abstract (英文): Large language models (LLMs) have enabled increasingly capable conversational agents, but reliably controlling their behavior in real-time interactive environments remains a significant challenge. Existing approaches often rely on model fine-tuning or alignment procedures that are difficult to adapt to changing interaction requirements. This paper introduces layered scenario-driven LLM control, a framework that enables runtime behavior control through structured prompting. By combining persistent context with scenario-specific constraints, the approach allows agent behavior to be modified during interaction without changing the underlying model. The framework is implemented in ARDena, a real-time multimodal embodied agent that integrates speech interaction, visual perception, tool use, and avatar-based response generation. The proposed approach is evaluated with respect to control effectiveness, response latency, and operational stability. The results demonstrate that scenario definitions alone can produce substantially different interaction behaviors while maintaining stable real-time operation, highlighting the effectiveness of scenario-driven prompting for controlling LLM agents.
...
Abstract (英文): A formal model enables verifying reachability, localizing an outage, or anticipating the blast radius of a change. Yet, virtually no production network has one, since writing a model by hand demands rare expertise and is hard to keep current as the network changes frequently. At its core, network modeling is a typographical exercise: it translates network artifacts (e.g., configurations, topology, and routing state) into rules in formal logic. Translation of this kind is what large language models (LLMs) nowadays do well. Unlike free-form AI reasoning, such translation can be formally verified.
...
Abstract (英文): Long-lived AI agents increasingly evolve after deployment by retaining experience, acquiring skills and tools, revising workflows, delegating work, and moving across task phases. This improves adaptation but creates a distinct authorization problem. Tool-enabled agents can turn model errors and prompt injections into consequential external actions; when evolution occurs under a live grant, the subject exercising that authority or the context in which it acts may no longer match what the user evaluated. Evolution can change both the effects reachable under an old grant and the authority required by the task, which may rise, fall, or become incomparable. Existing tool policies constrain actions but do not determine when a grant survives this change.
...
### [373] [Spatial heterogeneity of land subsidence in the california central valley: a model-based attribution perspective](https://doi.org/10.1080/15481603.2026.2706911)
...
Abstract (英文): The California Central Valley (CCV), a globally important agricultural region, has long experienced severe land subsidence (LS) associated with groundwater overextraction, thereby threatening infrastructure safety and aquifer storage capacity. Although the spatial pattern of subsidence in this region is strongly heterogeneous, existing approaches still have limitations in fully quantifying the spatial variability of internal statistical associations and in systematically summarizing differentiated zones. To address this issue, this study develops the Data-Ensemble-Explanation-Cluster-Threshold (DEECT) framework, which integrates stacking-based prediction, SHAP-based explanation, clustering, and GAM-based analytical transition analysis within a single exploratory workflow. Under a conventional random split benchmark, the stacking ensemble achieved strong fit (R2 = 0.9756, RMSE = 5.3997); however, this result is interpreted only as an upper-bound reference for within-study-area exploratory analysis rather than as evidence of spatial generalizability. Under a conventional random-split setting, the stacking ensemble achieved strong fit (R² = 0.9756, RMSE = 5.3997). To assess model robustness under spatial separation, we also applied a 10 km spatial block cross-validation, under which the stacking model achieved R² = 0.9122 and RMSE = 10.1306. Compressible layer thickness, precipitation, and non-ponded agricultural pumping emerged as common high-ranking model-based attribution factors. The study area was further partitioned into four typical subsidence zones with distinct statistical association patterns, and cluster-specific nonlinear analytical transition points were identified for selected variables. Overall, the DEECT framework provides an exploratory basis for organizing within-region heterogeneity in subsidence associations and for informing subsequent zonal hydrogeological validation.

</details>


### [374] [Dominant associations of precipitation and soil water with global vegetation reversals under climate change](https://doi.org/10.1080/15481603.2026.2702240)
*Shouhai Shi,Shunping Ji,Zhaohui Luo*
...
Abstract (英文): Satellite observations have revealed significant changes in global terrestrial vegetation over the past four decades. However, the neglect of interactions among influencing factors has sparked intense debates regarding vegetation dynamics at regional and global scales. This study systematically evaluated the spatiotemporal evolution of global vegetation dynamics (1982–2020) and their complex underlying mechanisms by integrating five remote sensing-derived vegetation products with climate and socio-economic data. The results showed that approximately 51.98% of the global terrestrial vegetation area experienced change reversals between 1995 and 2005, with vegetation “greenness” predominantly transitioning from increasing to decreasing. This percentage differs between different climate zones, ranging from 46.41% in the cold zone to 54.99% in the tropical zone. This phenomenon is associated with a weakening of the interaction explanatory power (i.e. the explained spatial variance quantified by the q-statistic) of the interactive coupling among total precipitation (TP), soil water (SW), and other factors (e.g. temperature and vapor pressure deficit, VPD), decreasing from 70% ± 4% to 64% ± 4%, rather than with an independent dominance of VPD changes, as emphasized in previous studies. These findings underscore the importance of explicitly accounting for interactions among water-related factors when interpreting regional and global vegetation dynamics under a warming climate.

</details>


### [375] [SWOT-derived high-resolution lake surface topography for enhanced lake water level retrieval](https://doi.org/10.1080/15481603.2026.2707798)
*Xin Luo,Ziyue Dou,Yihan Xu,Zhiqiang Xie*
...
Abstract (英文): Lake water level is a fundamental variable for water-resources monitoring and hydrodynamic modeling. Although a lake surface is conventionally assumed to be flat and level, satellite altimetry measurements referenced to the ellipsoidal height system reveal an undulating surface, which renders them unsuitable for water-flux analysis and other hydrological applications. Robustly retrieving a uniform water level that conforms to a true equipotential surface therefore requires these apparent height fluctuations to be effectively removed. This study investigates the potential of the recently launched Surface Water and Ocean Topography (SWOT) satellite for lake water level retrieval. We first developed a framework integrating automatic SWOT outlier removal, geophysical correction, and multitemporal data stacking to reconstruct high-spatial-resolution lake surface topography (LST). Building on this framework, we proposed a rigorous LST correction method to improve water level retrieval from satellite altimetry. Validation against both wide-swath SWOT and nadir-looking ICESat-2 measurements demonstrates that the LST correction effectively removes surface undulations and substantially improves retrieval precision: it reduced the mean surface fluctuation magnitude of SWOT measurements from 5.04 cm to 1.50 cm and the mean standard deviation of ICESat-2-derived water levels from 4.36 cm to 2.40 cm. Relative to orthometric height, which is commonly adopted as the lake water level in standard applications, the LST-corrected water level more closely approximates a constant value. Quantitative analysis of their discrepancies shows that relying solely on orthometric height introduces an average spatial uncertainty of 6.3 cm, with local biases reaching up to 82.9 cm in extreme cases. These results confirm the critical importance of the LST correction for consistent and reliable hydrological monitoring.

</details>


### [376] [A singularity regression kriging for spatial prediction](https://doi.org/10.1080/15481603.2026.2690341)
...
Abstract (英文): Improving spatial prediction remains a key challenge in geographical and environmental modeling, particularly in regions with strong spatial heterogeneity. This study proposes a local-outlier-based context-aware model, named LOCA, for precipitation spatial prediction. The novelty of LOCA lies not in spatial stratification or local modeling alone, but in formalizing local outliers of explanatory variables as continuous spatial contextual predictors and integrating them with spatial stratification and stratum-specific machine-learning models. Unlike conventional approaches that mainly treat outliers as errors or noise, LOCA uses positive and negative departures from the surrounding spatial background to represent local anomaly information. The model was evaluated through a case study of precipitation prediction in Qinghai Province, China, using CRU TS precipitation data, ERA5 meteorological variables, and topographic data for 2022. Because the target precipitation product was derived from CRU TS at 0.5° resolution, this case study should be interpreted as a statistical downscaling and fine-resolution spatial prediction experiment rather than direct validation against independent 1 km precipitation observations. K-means, Gaussian mixture model, and spectral clustering were used for spatial stratification, and multiple machine-learning models were compared within different strata. Model performance was assessed using spatial block cross-validation and compared with global machine-learning models, stratified context-aware models without local outlier features, and additional spatial reference models. The best-performing configuration, GMM-based LOCA, achieved an R2 of 0.858 ± 0.125, RMSE of 77.67 ± 15.43, and MAE of 57.12 ± 14.44. The improvement was associated with two complementary mechanisms: spatial stratification enhanced regional adaptability, while local outlier-derived features provided supplementary information on local spatial contrasts. These findings suggest that explanatory-variable local outliers can provide useful contextual information for statistical downscaling and spatial prediction in heterogeneous environments.
...
### [378] [Enhancing Flood Susceptibility Mapping Through High-Resolution Earth Observation: A Data-Driven Comparative Analysis](https://doi.org/10.3390/rs18142418)
...
### [379] [Remote Sensing Reveals How Agricultural Restructuring Spatially Organizes Non-Cropland Ecological Restoration in the Mu Us Sandy Land](https://doi.org/10.3390/rs18142421)
...
### [380] [MSGMamba: A Multi-Scale Dynamic Graph State-Space Model for Satellite Telemetry Anomaly Detection](https://doi.org/10.3390/rs18142420)
...
### [381] [Gated Spiking Attention Transformer for Hyperspectral Image Classification](https://doi.org/10.3390/rs18142419)
...
### [382] [DPSF: A Dynamic Patch Shape Selection Framework for Cross-Model Transferable Adversarial Attacks](https://doi.org/10.3390/rs18142426)
...
### [383] [A Knowledge-Guided Factor Graph Neural Network for Event Detection in Simulated Non-Cooperative Space Targets](https://doi.org/10.3390/rs18142423)
...
### [384] [State-Space-Based Mamba and Convolutional Neural Network Integration for Anthropogenic Geomorphic Feature Extraction from Land Surface Parameters](https://doi.org/10.3390/rs18142415)
...
### [385] [Reliability Analysis of Agricultural Foundation Models Under Distribution Shift](https://doi.org/10.3390/rs18142416)
...
### [386] [DFELD-YOLO: A Decoupled Lightweight Detection Model for UAV-Based Wind Turbine Blade Damage Inspection](https://doi.org/10.3390/rs18142422)
...
### [387] [Scattering Center Prior-Guided Diffusion for Unknown-Azimuth SAR Image Generation](https://doi.org/10.3390/rs18142417)
...
### [388] [A Framework Integrating Slope-Unit Parameter Optimization and Ensemble Machine Learning for Landslide Susceptibility Mapping](https://doi.org/10.3390/rs18142424)
...
### [389] [A UAV–TLS Point-Cloud Fusion Framework for Three-Dimensional Characterization of Mining-Induced Surface Cracks](https://doi.org/10.3390/rs18142425)
...
### [390] [CLIFF: A Multi-Modal Remote Sensing Model for Geological Hazard Monitoring Based on Bitemporal UAV Images](https://doi.org/10.3390/rs18142432)
*Quanxi Zhou,Qianxiao Su,Xinran Wei,Wencan Mao,Yili Ren,Y Z Chen,Jianzhong Bi,Mingjun Zhao,Manabu Tsukada*
...
### [402] [TCM-CR: Multi-Temporal SAR–Optical Cloud Removal with a Reference Image and Gated Bounded Residual](https://doi.org/10.3390/rs18152443)
...
Abstract (英文): Infrared spectroscopy is based on the emission from a heated body, oftentimes dust particles. This body is generally assumed to have a spherical shape. We explore the temperature of blackbodies with non-spherical shapes. Five non-spherical shapes are considered: (1) a prolate cylinder and (2) a prolate spheroid, both with aspect ratio 2:1, (3) agglomerated debris particles, equidimensional in size, (4) ballistic particle–cluster aggregates (BPCAs), and (5) ballistic cluster–cluster aggregates (BCCAs). Such particles have been used to model the light-scattering response of particles in cometary comae and debris disks. The first three types of particles yield remarkably similar blackbody temperatures that are only ~9 K lower than a spherical blackbody. Blackbodies with BPCA and BCCA morphologies are much colder than the spherical blackbody due to the large surface area of the fractal aggregates.

</details>


### [404] [First-Derivative Analysis for Estimating Water Quality and Optical Metrics in Shallow Coastal Waters](https://doi.org/10.3390/rs18152441)
*Aminah Kaharuddin,Stefan Forster,Hendrik Schubert*
...
Abstract (英文): The study quantifies correlations between remote sensing reflectance (Rrs), first derivatives (FDs), and water quality and optical metrics: chlorophyll-a (Chl-a), phaeopigments (Pha), total suspended matter (TSM), coloured dissolved organic matter (CDOM), and the scalar ambient attenuation coefficient for photosynthetically active radiation, K0(PAR). Data from 14 stations (n = 23) along the German Baltic coast during summers of 2022 and 2023 established a summer-specific optical baseline. Rrs was pre-processed using 10-point moving average (10MA) smoothing or discrete wavelet transformation (DWT) before computing FD. Results show moderate to strong correlations for estimating variables using simple algorithms. Chl-a and Pha are best estimated using the FD peak at 685 nm (R2 of 0.67 and 0.69, respectively), though retrieval highly depends on localized nutrient enrichment. TSM estimation was optimized via a DWT baseline peak algorithm (670 to 750 nm), yielding a physically consistent positive correlation (R2 = 0.17) over stronger inverse models. CDOM and K0(PAR) are optimally retrieved at 689 nm and 629 nm (R2 = 0.51 and 0.48, respectively). Signal processing significantly affects results, and the 600–850 nm window is proved highly sensitive. This approach mitigates wind and wave effects, demonstrating the utility of high-resolution in situ spectra free from atmospheric interference.

</details>


### [405] [Delineating Roofing Materials in Urban Areas Using Transformed High-Resolution Satellite Imagery and Convolutional Neural Networks](https://doi.org/10.3390/rs18152440)
*Cibele Hummel do Amaral,Maxwell Cook,Johannes Uhl,Joseph McGlinchy,Stefan Leyk,Erick Verley,Jennifer K. Balch*
...
Abstract (英文): In complex infrared scenarios, where prominent edge contours and strong noise interference coexist, effectively suppressing these interfering factors has become the core challenge faced by algorithms for detecting weak and small targets. Current mainstream algorithms generally suffer from a high false alarm rate, mainly due to the interference effect of background clutter. To this end, this paper proposes a Weighted Nuclear Norm Minimization method based on low-rank clustering idea. This method first clusters the image patches adaptively by a similarity measure combining local statistical features and Euclidean distance, which enhances the low-rank property of the background matrix. On this basis, a differentiated weighted nuclear norm penalty is designed for image patches of different categories, which further improves the ability of the model to separate the background from the target. An energy shrinkage mechanism is also introduced to alleviate target energy dilution. Experimental results on six different scenes, compared with six mainstream detection methods and two deep learning methods, using BSF, SNR, and three AUC metrics, demonstrate that the proposed method achieves superior background suppression and detection performance. Although the computational cost is high (80 to 245 s per frame) and currently not suitable for real-time or near-real-time applications, its advantages in background suppression and false alarm control make it particularly suitable for offline or batch processing scenarios, such as post-mission analysis and surveillance data review, where accuracy takes precedence over real-time requirements. This innovative modeling approach provides a new technical solution for weak and small target detection in complex scenarios.

</details>


### [407] [CMGFDet: Cross-Modal Gated Fusion Network with Multi-Receptive Field Aggregation for RGB–Infrared Aerial Object Detection](https://doi.org/10.3390/rs18152439)
*Man Wu,Xiaozhang Liu,Xiulai Li,Wenbiao Gan*
...
### [415] [Urban sidewalk detection via a cross-view joint segmentation method using satellite and street-view imagery](https://doi.org/10.1016/j.isprsjprs.2026.07.003)
...
### [416] [Edge-preserving and uncertainty-guided domain adaptation for weakly supervised rooftop photovoltaic mapping](https://doi.org/10.1016/j.isprsjprs.2026.07.008)
*Siyuan Wang,Bowen Cai,Zhiwen Luo,Dongyang Hou,Xiaoyu Zheng,Jinglei Liu,Zhenfeng Shao*
...
### [417] [SARMelt: MGLANet-driven high-resolution surface melt mapping of circum-Antarctic ice shelves from Sentinel-1 imagery during 2015–2023](https://doi.org/10.1016/j.isprsjprs.2026.07.012)
*Lu Zhang,Qi Zhu,Dong Liang,Yiming Liu,Zherong Wu,Ji Ge,Yuqian Zhang,Huadong Guo*
...
### [418] [Tracking forest carbon and growth in large-scale passive restoration using multi-temporal airborne lidar](https://doi.org/10.1016/j.isprsjprs.2026.07.013)
*Felipe Nincao Begliomini,Fabian Jörg Fischer,Srinivasan Keshav,Pedro H. S. Brancalion,Paulo Guilherme Molin,Angelica M. Almeyda Zambrano,Angélica Faria de Resende,Cássio Augusto Patrocínio Toledo,Catherine Torres de Almeida,Daniel de Almeida Papa,Davi Ernesto Alvarenga César,Eben N. Broadbent,Juliano van Melis,Laura B. Vedovato,Paulo Ricardo da Silva Rodrigues,Ricardo Ribeiro Rodrigues,Vinícius Castro Souza,David Coomes*
...
### [419] [Editorial Board](https://doi.org/10.1016/s0924-2716(26)00365-5)
*Unknown Author*
...
### [420] [Pioneering Test-Time training for robust Fine-Grained chlorophyll-a retrieval from Sentinel-2: Unveiling China’s lake eutrophication dynamics (2018–2025)](https://doi.org/10.1016/j.jag.2026.105487)
...
### [421] [Self-supervised spectral unmixing using scene-aware deep priors](https://doi.org/10.1016/j.jag.2026.105483)
...
Abstract (英文): High spectral resolution of hyperspectral sensors facilitates the detailed identification of material, but the coarse spatial resolution affects accurate interpretation. In this regard, spectral unmixing techniques attempt to estimate the constituent materials (endmembers) and their fractional areas (abundances) within a pixel’s instantaneous field of view (IFOV). However, many existing methods optimize denoising, endmember detection, and abundance estimation separately, which can limit robustness under noise and spectral variability. The current study proposes a self-supervised deep learning framework for joint hyperspectral unmixing and denoising. A noise-aware probabilistic encoding strategy is adopted to model a structured latent space to implicitly detect the endmembers. Further, the proposed encoding scheme and regularizations ensure spatio-spectral fidelity to improve robustness to noise. Additionally, a contrastive learning mechanism is incorporated to improve latent separability and robustness to spectral variability. The framework also integrates available partial endmember priors when present and employs a policy-based adaptive refinement mechanism to update endmember estimates under complex noise and mixing conditions. In addition, a learnable loss-balancing strategy is introduced to reduce manual tuning and stabilize optimization. Experiments on synthetic and real hyperspectral datasets demonstrate strong and balanced performance across reconstruction accuracy, spectral fidelity, and clustering quality, indicating the effectiveness of the proposed framework for practical hyperspectral unmixing.

</details>


### [422] [Spatiotemporal dynamics of grasshopper habitats under future climate projections, vegetation growth, and land use change](https://doi.org/10.1016/j.jag.2026.105481)
*Jing Guo,Wenjiang Huang,Yingying Dong,Qiaoyun Xie,Guofei Fang,Fangzheng Yue,Kun Wang,Zhuoqing Hao,Y Zhang,Xiangzhe Cheng,Yue Shi,Bing Zhang*
...
### [423] [Quantifying methane emissions from large coal mine using Sentinel-5 Precursor satellite data and Gaussian plume model](https://doi.org/10.1016/j.jag.2026.105491)
*Yuanyuan Chen,Yi Li,Lixia Tan,Yufan Qian,Shuangxi Fang*
...
### [424] [Enabling maize mapping in double-cropping regions of Argentina and Brazil without ground reference data by leveraging multiple satellite platforms](https://doi.org/10.1016/j.jag.2026.105489)
...
Abstract (英文): Matter-loaded windrows are high-density submesoscale accumulation structures that form on the ocean surface and are often associated with extreme environmental events. This study presents a framework for windrow detection using a UNet deep-learning model applied to Planet SuperDove (PSD) 8-band satellite imagery (FLODAS). The model is trained on hand-annotated cases representing a variety of windrows and evaluated using complementary performance metrics, demonstrating consistent detection across independent scenes, while also revealing scene-specific patterns of false positives. Spectral analysis and classification based on the spectral angle distance metric is used to examine the potential for discriminating floating matter types using the PSD bands. Limited ground-truth data and the incomplete spectral characterisation of floating matter currently constrain classification performance. Application of the framework in the Aegean Sea enabled the identification of surface matter accumulations linked to storm-driven runoff and riverine discharge. The results demonstrate the potential of the framework to support in situ sampling and debris-removal operations, as well as short-term assessments of matter transport and windrow dynamics. Overall, the proposed framework represents a proof-of-concept step towards the development of operational detection systems for surface matter and marine debris accumulations.

</details>


### [426] [An integrated picture fuzzy entropy–MAIRCA–TOPSIS framework for sustainable soil suitability evaluation](https://doi.org/10.1016/j.jag.2026.105474)
*Karthik Subramani,Deva Nithyanandham,Felix Augustin,Saravanakumar Ramasamy,Ajeesh Puthusserry Paulose,Pandiyaraju V*
...
### [427] [A transferable framework for roof vegetation mapping across tropical cities based on open-source multimodal data and morphology-aware UDASS](https://doi.org/10.1016/j.jag.2026.105490)
*Jinhui Liu,Mi Zhang,Lina Tang,Guofan Shao,Huiwei Jiang,Xiangyun Hu,Yibing Xiong,Jiongwei Wang*
...
Abstract (英文): Green roofs play a vital role in urban sustainability, offering substantial environmental, ecological, and social benefits. However, automated roof vegetation extraction mostly relies on costly high-resolution data, with limited investigations in Global South cities. Existing methods insufficiently fuse multimodal priors and overlook domain adaptation, thereby constraining extraction accuracy and cross-regional generalization. This work presents a transferable open-source data-driven framework for cross-domain multimodal extraction and analysis of roof vegetation. Specifically, we introduce a multimodal transformer encoder to fuse Google imagery, Sentinel-2-derived products and OpenStreetMap building footprints at the feature level to improve extraction accuracy. We then propose an unsupervised domain-adaptive semantic segmentation (UDASS) model with morphology-aware pseudo-label correction to improve cross-domain generalization cost-effectively. Validated across six Southeast Asian cities, our framework achieves an F1-score of 81.71% for supervised source domain extraction and up to 70.85% for UDASS. Further validation on four additional cities spanning East Africa, South America, West Asia, and South Asia verifies its strong cross-regional generalization ability. Finally, approximately 297 ha of roof vegetation are detected across the six Southeast Asian cities, mainly on low-to-mid-rise buildings with distinct regional disparities and diverse vegetation intensity. This study delivers the first large-scale dataset on roof vegetation and green roofs for tropical Southeast Asia, establishes a cost-effective and scalable multimodal paradigm for cross-regional monitoring, and supports evidence-based green roof planning and analysis.

</details>


### [428] [MFRXG: A multi-feature stacking ensemble RF-XGBoost model for total nitrogen retrieval in the Yellow River from Landsat 8/9 data](https://doi.org/10.1016/j.jag.2026.105492)
...
Abstract (英文): Accurate global sea surface wind speed retrieval under high-wind conditions (> 15 m/s) remains a persistent bottleneck for spaceborne Global Navigation Satellite System Reflectometry (GNSS-R). Conventional data-driven models often experience severe performance degradation in these regimes due to inherent physical limitations, including long-tail data distributions, low signal-to-noise ratios (SNR), and scattering signal saturation. To transcend these limitations, this study proposes the Physics-Guided Local-Global Fusion Network (PLGF-NET), establishing a novel physics-data dual-driven paradigm. At the data level, a Comprehensive Difficulty Score (CDS) is introduced to quantify the intrinsic physical complexity of Delay-Doppler Maps (DDMs). This metric drives a difficulty-aware hierarchical sampling strategy that rectifies long-tail biases and dynamically mines the scarce, high-wind scattering signatures. Within the network, a Physics-Guided Spatial Attention (PGSA) mechanism translates specular reflection theory into a structural inductive bias. This enforces a rigorous feature purification process, locking the receptive field onto the effective scattering window while suppressing peripheral thermal noise. Furthermore, geophysical consistency is guaranteed through a selective regularization mechanism governed by Sample Prediction Difficulty (SPD). Rather than applying uniform constraints, this approach adaptively enforces Geophysical Model Function (GMF) constraints exclusively on high-uncertainty samples, providing a robust physical anchor against signal saturation. Comprehensive evaluations demonstrate the exceptional robustness of PLGF-NET. The framework achieves a state-of-the-art overall root mean square error (RMSE) of 1.38 m/s and drastically compresses the high-wind RMSE to 2.40 m/s, effectively correcting systematic negative biases. Ultimately, this scalable and physically interpretable architecture provides a highly reliable Earth observation tool for high-wind marine environment monitoring.

</details>


### [433] [Earth Observation–Based Machine learning for operational forecasting of harmful algal blooms in the Arabian Gulf and Sea of Oman](https://doi.org/10.1016/j.jag.2026.105501)
*Kaltham A. Ismail,Roqaya A. Ismail,Mohammed Salim*
...
Abstract (英文): Harmful algal blooms (HABs) pose increasing risks to coastal ecosystems, fisheries, and desalination infrastructure in the Arabian Gulf and Sea of Oman. Forecasting bloom occurrence remains challenging due to strong spatial variability in physical forcing and biological response. Here, we develop a physics-informed machine-learning framework to forecast HAB probability using satellite Earth observation data and reanalysis-derived environmental predictors over the period 2002–2024. Bloom events are defined using region-specific, seasonally varying chlorophyll thresholds derived from training data, and a Random Forest classifier is used to estimate bloom probability at 8-day resolution. Operational probability thresholds are calibrated using historical data (2002–2020) and applied unchanged to independent evaluation (2021–2022) and strictly forward (2023–2024) periods. The model demonstrated moderate classification skill during the independent evaluation period (F1 = 0.46–0.67) and strong forward operational performance, achieving F1 scores of 0.79–0.97 across the study regions. Permutation importance analysis showed that chlorophyll anomaly contributed most strongly to forecast performance across regions, indicating that deviations from seasonal biomass conditions are strongly associated with bloom detection skill, while physical variables provide complementary predictive information. These findings demonstrate that a biomass-informed, Earth observation–based framework enables robust and operationally stable detection of harmful algal blooms across hydrographically complex marginal seas, supporting its application as an early-warning system for HAB monitoring and management in the Arabian Gulf and Sea of Oman.
...
### [434] [Optical remote sensing of sea surface wind streaks: Retrievals of wind directions and wind waves](https://doi.org/10.1016/j.rse.2026.115569)
...
### [435] [Towards efficient multi-hazard detection from single satellite imagery via spectral-semantic graph reasoning](https://doi.org/10.1016/j.rse.2026.115566)
...
### [436] [Normalization of Sentinel 2 data over tropical forests to nadir BRDF adjusted reflectance](https://doi.org/10.1016/j.rse.2026.115583)
...
### [438] [Editorial Board](https://doi.org/10.1016/s0034-4257(26)00347-0)
*Unknown Author*
...
### [439] [Label-efficient 3D forest mapping: Self-supervised and transfer learning for instance segmentation, semantic segmentation, and species classification](https://doi.org/10.1016/j.rse.2026.115564)
...
### [440] [Enhanced land surface phenology detections in the western U.S. drylands by fusing harmonized Landsat and Sentinel-2 observations with near-surface PhenoCam time series](https://doi.org/10.1016/j.rse.2026.115588)
*Yuxia Liu,Xi Zhang,Yu Shen,Yongchang Ye,Shuai An*
...
### [441] [Substantial improvement of geostationary satellite-derived shortwave radiation product through correcting geolocation displacement](https://doi.org/10.1016/j.rse.2026.115589)
*Wangkang SHE,Changkun Shao,Wenjun Tang,Guopeng Zhu,Zengqiang Pang,Tongfei Liu,Shuibao Liang,Kun Yang*
...
### [442] [Breaking barriers in rice chlorophyll retrieval from satellite imagery: Mitigating row structure and saturation effects via RiceGORT and chlorophyll sensitivity enhancement](https://doi.org/10.1016/j.rse.2026.115581)
*Haoran Sun,Hifza Mariam,Junnan She,Ming Chen,Dong Li,Chongya Jiang,Xia Yao,Iftikhar Ali,Yan Zhu,Weixing Cao,Tao Cheng*
...
### [443] [Retrieval of sea surface salinity from SMAP L-band radiometry under rainfall conditions](https://doi.org/10.1016/j.rse.2026.115570)
*Xuchen Jin,Xianqiang He,Yan Bai,Yue Wang,Lizhang Zhou,Shujie Yu,Teng Li,Fang Gong,Delu Pan*
...
### [444] [A scattering model-informed deep transfer learning framework for surface soil moisture retrieval](https://doi.org/10.1016/j.rse.2026.115571)
*Zuo Wang,J M Chen,Jiangyuan Zeng,Chang Huang,Lisheng Song,Yingying Chen,Liujun Zhu*
...
### [445] [cscMTCI: A soil and structure corrected MERIS terrestrial chlorophyll index for improving leaf chlorophyll content estimation](https://doi.org/10.1016/j.rse.2026.115582)
*Runfei Zhang,Peiqi Yang*
...
Abstract (英文): Leaf chlorophyll content (LCC) is an important indicator of photosynthetic capacity. As a classical chlorophyll-sensitive vegetation index, the MERIS Terrestrial Chlorophyll Index (MTCI) at the leaf level (MTCI leaf ) has a strong linear relationship with LCC. However, our theoretical analysis reveals that the canopy-level MTCI (MTCI c ) is clearly influenced by soil and canopy structure, limiting its ability to accurately estimate LCC. To address this limitation, we developed a two-step approach that downscales MTCI c to the leaf level by mitigating the effects of soil and canopy structure. First, direct soil reflection is removed from canopy reflectance to derive the MTCI signal of the ‘pure’ vegetation component (MTCI v ). The MTCI v approximates the MTCI signal derived from the vegetation canopy reflectance over black soil. Still, MTCI v is not directly equivalent to MTCI leaf , and their relationship is strongly modulated by leaf area index (LAI). Second, given that MTCI v and MTCI leaf always maintain a strong linear relationship at any given LAI, we introduce a canopy structure coefficient (CSC) to approximate the MTCI v -MTCI leaf relationship as: MTCI v ≈ CSC × MTCI leaf , where CSC is a canopy structural factor accounting for the slope variation in the MTCI v -MTCI leaf space. Theoretical analysis and PROSAIL simulations demonstrate that the canopy soil-adjusted vegetation index (SAVI c ) serves as a proxy for CSC (i.e., SAVI c ∝ CSC). After correcting for soil and canopy structural effects, cscMTCI is proposed as MTCI v /SAVI c , which serves as an approximation of MTCI leaf . The performance of cscMTCI is evaluated using both PROSAIL simulations and experimental datasets and compared against MTCI c (derived from original canopy reflectance), MTCI v (derived from reflectance of the ‘pure’ vegetation component within the canopy), and MTCI n (i.e., MTCI c /SAVI c , normalized MTCI without soil correction). The experimental datasets include ground-based LCC measurements across seven vegetation types from the National Ecological Observatory Network (NEON), along with corresponding NEON airborne hyperspectral and Sentinel-2 satellite multispectral observations. The PROSAIL simulations indicate that cscMTCI consistently exhibits the strongest correlation with LCC, followed by MTCI n , MTCI v , and MTCI c across both individual leaf angle distribution (LAD) types and non-erectophile LAD combinations. The cscMTCI-LCC relationship remains nearly consistent over a wide range of LAI, soil backgrounds, and solar zenith angles. The observations at the NEON sites confirm the results of PROSAIL simulations. In leave-one-site-out cross-validation across all vegetation types, cscMTCI reduces RMSE by 12.39%, 30.35%, and 30.32% compared to MTCI n , MTCI v , and MTCI c for NEON airborne observations and by 9.44%, 21.03%, and 24.00% for Sentinel-2 satellite observations. For individual vegetation types, cscMTCI markedly outperforms MTCI c in homogeneous croplands, grasslands, and shrublands, with RMSE reductions of 78.52%, 52.76%, and 31.48% for NEON and 39.28%, 24.40%, and 35.12% for Sentinel-2. In more heterogeneous wetlands and forests, performance differences among the four MTCI variants narrow, but cscMTCI generally retains slight superiority. These results demonstrate the practical applicability of cscMTCI across different spatial scales and the potential for large-scale LCC mapping.
