...
- [JAG](#JAG) [Total: 22]
- [Remote Sensing](#Remote Sensing) [Total: 97]
- [ISPRS J P&RS](#ISPRS J P&RS) [Total: 18]
...
Abstract (英文): Hi-OPD addresses a failure mode left uncontrolled by flat open-prompt training: descendant retrieval need not persist under ancestor queries when multi-source remote sensing annotations exhibit inconsistent granularity and missing labels. A detector may localize \textit{car} and \textit{van} under atomic prompts yet miss the same instances under \textit{vehicle}; flat AP does not expose this cross-level inconsistency.
...
Abstract (英文): The annotation used to select a segmentation threshold is part of the evaluation protocol, yet its effect is easily conflated with model quality. We examine this choice for retinal vessel segmentation using all 28 CHASE DB1 images and both human annotations. A fixed seven-fold protocol keeps both eyes of each of the 14 subjects together. Random forests and Extra Trees are fitted against observer 1 with three random seeds, yielding 42 fits. Five threshold policies share identical score maps: fixed 0.50, observer-1 tuning, observer-2 tuning, mean-observer tuning, and maximin tuning of the per-image lower observer Dice. For random forests, maximin changes the threshold in 19 of 21 fits, but worst-observer Dice decreases from 70.53 percent to 70.45 percent. The paired difference is -0.073 percentage points, with a conditional subject-bootstrap 95 percent interval of [-0.384, 0.238]. Extra Trees shows the same direction. Identical observer-1-tuned random-forest masks score 73.66 percent against observer 1 and 71.06 percent against observer 2. The results support explicit reporting of both the threshold-selection reference and evaluation reference; they do not support an accuracy benefit from maximin tuning in this cohort. All splits, raw predictions, metrics and code are supplied. AI assistance is disclosed.
...
Abstract (英文): Interactive segmentation of 3D medical images supports quantitative analysis of anatomical structures and disease while allowing users to specify and refine their targets.
...
Abstract (英文): This paper introduces GRIP, a pose-conditioned refinement framework for pixel-to-point matching and 2D to 3D registration. Given an initial coarse pose estimate, GRIP addresses the structural mismatch between grid based image descriptors and unordered point cloud descriptors by softly rendering learned 3D point features onto the image grid through Gaussian feature splatting. The rendered point derived feature map is then fused with image features by a pixel aligned transformer, enabling visual semantic and geometric cues to interact in a shared 2D representation. The refined features are decoded and propagated to finer resolutions for dense correspondence estimation and final pose refinement. Experiments on RGB D Scenes V2 and 7 Scenes demonstrate state of the art inlier ratio and competitive registration recall, with stronger performance under stricter evaluation thresholds.
...
Abstract (英文): Lean Pool is a repository of formalized mathematics. It is grown, maintained and optimized by AI agents.
...
Abstract (英文): Finding circuits in language models usually means running many careful interventions. We try something simpler: treat attention as a routing map from one forward pass, keep a small set of routes that point toward the answer, and ask whether those routes actually matter.
...
Abstract (英文): On repeated work, agents are inconsistent. We ran 42 tasks three times each and found that, depending on the model, 38% to 74% returned answers that did not agree. Consistency is what a buyer, an auditor, or a regulator requires, and agents do not have it. They are wasteful too: 95.3% to 97.2% of what an agent generates goes to re-deriving a plan the system already knows.
...
Abstract (英文): In this note, we show that the problem of checking, whether a sequence of bits is ordered, is not doable by 1-head 1-layer transformers but is doable by a 2-head 1-layer transformer. Unlike similar previous results, our results assume the model where transformers have an output MLP.
...
Abstract (英文): Accurately forecasting river runoff is key to managing water resources, controlling floods, and planning agriculture. This study examines the Ajichay River in northwest Iran, a major tributary of Lake Urmia that has experienced increasing water-related stress in recent years. We introduce a daily runoff prediction model based on Long Short-Term Memory (LSTM) networks. The model combines five LSTM units, each trained on different time intervals ranging from 2 to 6 days, to better capture variations in river flow patterns. To improve performance, each model was fine-tuned using Particle Swarm Optimization (PSO), a population-based optimization algorithm. The proposed approach was evaluated on unseen data from 2017-2018 using $R^2$, RMSE, and MSE as performance metrics. The results showed strong predictive accuracy, with $R^2$ values ranging from 74.95% to 91.42%. In addition, multiple feature-importance methods were applied to identify the most influential variables, providing further insight into the factors that drive runoff variations.
...
### [210] [Enhanced early warning of extreme river discharge events in the Yangtze River Basin using atmospheric circulation signals](https://doi.org/10.1016/j.jag.2026.105590)
...
### [211] [Context-aware quantile baselines for disturbance detection in non-stationary short-rotation forest plantations](https://doi.org/10.1016/j.jag.2026.105555)
...
### [212] [An RTE-guided mixture-of-experts framework for AMSR2 passive microwave soil moisture retrieval](https://doi.org/10.1016/j.jag.2026.105597)
...
### [213] [Multimodal loess cave extraction using LMUNet-RCAM: Integrating hillshade and RGB features with local attention mechanisms](https://doi.org/10.1016/j.jag.2026.105596)
...
### [214] [Learning instance-level semantic Gaussian representations for occlusion-aware urban scene reconstruction from UAV imagery](https://doi.org/10.1016/j.jag.2026.105574)
...
### [215] [Vegetation canopy height retrieval in complex mountainous regions based on data calibration and CNN model](https://doi.org/10.1016/j.jag.2026.105598)
...
### [216] [Automated conversion of unstructured mesh models to structured parametric CAD models for architectural applications](https://doi.org/10.1016/j.jag.2026.105583)
...
### [217] [Challenges in heating demand modelling based on the Kleczków district in Wrocław, Poland](https://doi.org/10.1016/j.jag.2026.105581)
...
### [218] [Biophysical effects of afforestation on local land surface temperature in china: A comprehensive analysis incorporating forest age and afforestation intensity](https://doi.org/10.1016/j.jag.2026.105593)
...
### [219] [A two-stage transformer framework for reconstructing hourly 1 km all-weather land surface temperature using cross-scale transfer learning](https://doi.org/10.1016/j.jag.2026.105602)
...
### [220] [AdaGeo-SR: Geo-semantic-guided super-resolution of sentinel-2 imagery with AlphaEarth foundation priors](https://doi.org/10.1016/j.jag.2026.105573)
...
### [221] [A spatiotemporal reconstruction framework for GRACE/GRACE-FO-derived groundwater storage anomalies based on historical phase-constrained gap filling and physics-guided global–local downscaling](https://doi.org/10.1016/j.jag.2026.105603)
...
### [222] [Adaptive background suppression-based plume identification, emission estimation, and ground validation of coal mine methane in Changzhi, Shanxi, China](https://doi.org/10.1016/j.jag.2026.105587)
...
### [223] [Kinematic pattern interpretation of railway-corridor deformation from InSAR time series with transfer learning-based TCN](https://doi.org/10.1016/j.jag.2026.105594)
...
### [224] [Relaxing the clear-sky assumption: Cloud-tolerant spatiotemporal fusion via mask-guided feature modulation and temporal-memory collaboration](https://doi.org/10.1016/j.jag.2026.105571)
...
### [225] [Estimating supraglacial lake bottom ablation rates by integrating ICESat-2 repeat track data and multi-temporal sentinel-2 imagery](https://doi.org/10.1016/j.jag.2026.105605)
...
### [226] [Analysis of intraday urban thermal anisotropy and downward shortwave radiation through UAV experiment and coupled model](https://doi.org/10.1016/j.jag.2026.105582)
...
### [227] [Remote-sensing-assisted and in situ-constrained reconstruction of 4-hourly chlorophyll-a dynamics in inland lakes using vision Mamba and temporal attention](https://doi.org/10.1016/j.jag.2026.105588)
...
### [228] [Reference-anchored envelope modelling for predicting tree mortality hotspots driven by drought](https://doi.org/10.1016/j.jag.2026.105609)
...
### [229] [A Targeted–Refined Framework for wind turbine detection integrating AlphaEarth Foundations and high-resolution multispectral satellite imagery](https://doi.org/10.1016/j.jag.2026.105604)
...
### [230] [Multi-angular assessment of early-season defoliation: Evaluating the sensitivity of vegetation indices to NPV visibility in temperate broadleaved forests](https://doi.org/10.1016/j.jag.2026.105575)
...
### [231] [Unsupervised soft-min fusion of multi-sensor turbidity products using Gaussian mixture models](https://doi.org/10.1016/j.jag.2026.105584)
...
### [232] [Downscaling of SMAP Soil Moisture Based on the Transformer Algorithm in Anhui Province](https://doi.org/10.3390/rs18193272)
...
### [233] [Retrieval of Optically Active and Inactive Water Quality Parameters Using Remote Sensing and Machine Learning: Evidence from Water Hyacinth-Infested Lake Tana, Ethiopia](https://doi.org/10.3390/rs18183185)
...
### [234] [Spatially Constrained Grassland Aboveground Biomass Estimation by Identifying and Masking Achnatherum splendens: Integrating UAV Remote Sensing and Deep Learning](https://doi.org/10.3390/rs18183177)
...
### [235] [Regional Marine Gravity Field Refinement by Integrating Region-Adaptive Fusion and Multi-Relational Graph Residual Learning](https://doi.org/10.3390/rs18183182)
...
### [236] [Quantifying the Spatio-Ecological Context of Urban Nature-Based Solutions Across European Cities](https://doi.org/10.3390/rs18183184)
*Andrew Ikingura,Barbara Natalia Sowińska-Świerkosz,Dagmara Kociuba,Hai-Ying Liu,Elīna Dāce,Fabiano Lemes de Oliveira,Raimund Kemper,Anna Giulia Castaldo*
...
### [237] [Deep Learning-Based Monitoring of Photovoltaic Power Plant Expansion and Assessment of Albedo-Driven Shortwave Energy Changes](https://doi.org/10.3390/rs18183183)
...
### [238] [A Canopy-Scale Fluorescence Approach for Evaluating Seasonal Photochemical Dynamics Across Different U.S. Forests](https://doi.org/10.3390/rs18183181)
...
### [239] [Class-Specific Interpretation and Validation of Optical and SAR Sensor Contributions in Multi-Sensor Land-Cover Classification Using SHAP and Leave-One-Sensor-Out Ablation](https://doi.org/10.3390/rs18183186)
...
### [240] [Red-Edge Information in Agricultural Remote Sensing: From Spectral Theory to Explainable Machine Learning](https://doi.org/10.3390/rs18183180)
...
### [241] [Dynamic Reconstruction of Vegetation Earth Observation Time Series: Beyond Gap-Filling in Level-3 Products](https://doi.org/10.3390/rs18183178)
*Jochem Verrelst*
...
### [242] [Gas Plume Detection from Infrared Hyperspectral Remote Sensing Data Based on Deep Learning Algorithms](https://doi.org/10.3390/rs18183179)
...
### [243] [Unsupervised Scale-Conditioned Hyperspectral and Multispectral Image Fusion via a Frequency–Spatial Dual-Domain Network](https://doi.org/10.3390/rs18183187)
*Peng Tang,Ke Hong Zheng,Jiaxin Li,Haoyang Yu,Xu Sun*
...
### [244] [The Evolution of Land Subsidence Under the New Water Regime in the North China Plain: A TS-InSAR and Spatiotemporal Pattern Analysis](https://doi.org/10.3390/rs18183188)
...
### [245] [Evaluating In Situ and OCO-3 CO2 Observations in the Sichuan Basin Using WRF-Chem Simulations and Footprint Analysis](https://doi.org/10.3390/rs18183193)
...
### [246] [Sea Surface Current Vector Reconstruction from Multitemporal Sentinel-1 Doppler Observations: A Trajectory-Crossing Approach with Spatial Registration](https://doi.org/10.3390/rs18183192)
...
### [247] [Unpacking the Fragmentation–Ecology Nexus: Spatial Heterogeneity, Nonlinear Responses, and Associated Ecological Factors Across Urban Agglomerations in the Yellow River Basin](https://doi.org/10.3390/rs18183191)
...
### [248] [Low-Cost Geological Reconnaissance for Artisanal and Small-Scale Mining: RGB–HSV Analysis of Rendered Google Earth Imagery in Arid Copper-Prospective Terrains of Chile and Balochistan](https://doi.org/10.3390/rs18183189)
...
### [249] [Multi-Scale Validation of Satellite-Based Precipitation Products and Their Impacts on Hydrological Simulation in a Humid Mountainous Basin](https://doi.org/10.3390/rs18183190)
...
### [250] [TOTMSeg: A Texture-Aware Octree-Based Transformer-Mamba Framework for Large-Scale Urban Mesh Semantic Segmentation](https://doi.org/10.3390/rs18183198)
...
### [251] [Reading the Void: A Reconstructive Hypothesis for a Service Staircase at the Infantado Palace in Guadalajara Using TLS](https://doi.org/10.3390/rs18183200)
...
### [252] [Spatial Prediction and Performance Comparison of Soil Organic Carbon in High-Latitude Forest Soils Based on Multiple Feature Selection Methods and Machine Learning Models](https://doi.org/10.3390/rs18183199)
...
### [253] [Calibration of Event-Based Camera for High-Speed Observations of Lightning and Transient Luminous Events](https://doi.org/10.3390/rs18183201)
...
### [254] [Reconstructed XCO2 Reveals Seasonal Moisture Limitation Sensitivity Across Typical Steppe, Forest, and Gobi Desert in Mongolia](https://doi.org/10.3390/rs18183196)
...
### [255] [HSRA-Net: Hierarchical Season-Year Representation Alignment for Tree Species Classification from Long-Term Sentinel-2 Observations](https://doi.org/10.3390/rs18183195)
...
### [256] [New Insights into Aristarchus Plateau and Its Vicinity from Chang’e-2 Microwave Radiometer Data](https://doi.org/10.3390/rs18183194)
...
### [257] [Uncertainty-Based Quality-Control Prioritization for High-Resolution Land-Cover Mapping Under Spatially Disjoint Evaluation](https://doi.org/10.3390/rs18183197)
...
### [258] [Cross-Regional Transfer Learning for Radar Echo Extrapolation in Northwestern Xinjiang, Western China](https://doi.org/10.3390/rs18183209)
...
### [260] [HAF-Net: A Hierarchical Adaptive Fusion Network for Satellite-Based Radar Reflectivity Reconstruction](https://doi.org/10.3390/rs18183202)
...
### [262] [Assessment of Sea Surface Wind Measurements from Wave Gliders in Tropical Cyclones](https://doi.org/10.3390/rs18183213)
...
### [264] [Motion-Guided Multi-Offset Detector-Native ReID Readout for Efficient UAV Multi-Object Tracking](https://doi.org/10.3390/rs18183238)
...
### [265] [The Five-Decade Landsat Legacy in Inland Water Quality: A Systematic Review of Technological and Methodological Evolution](https://doi.org/10.3390/rs18183237)
...
### [266] [Fine-Grained Tree Species Classification in Urban Forests via Phenology–Structure Synergistic Fusion of Multi-Source Remote Sensing Data](https://doi.org/10.3390/rs18183246)
...
### [267] [LiDAR-Based Sub-Canopy Surface Water Detection in a Post-Mining Environment](https://doi.org/10.3390/rs18183222)
...
### [268] [Reliability-Aware Dual-Stream Self-Training for Semi-Supervised Semantic Segmentation of High-Resolution Remote Sensing Imagery](https://doi.org/10.3390/rs18183257)
...
### [269] [Cross-Year Crop and Land-Cover Classification with Limited Labels](https://doi.org/10.3390/rs18183210)
...
### [271] [A Multi-Modal Deep Learning Framework for High-Resolution Alpine Land Use/Land Cover](https://doi.org/10.3390/rs18183203)
...
### [273] [Assessing and Improving Geolocation of InSAR Scatterers with LiDAR Data](https://doi.org/10.3390/rs18183249)
...
### [274] [UAV-Based Prediction of Cover Crop Biomass in Winter Peas Across Planting Dates and Cropping Systems](https://doi.org/10.3390/rs18183208)
*Raksha K. Thapa,Lisa Kissing Kucek,Steve Zwinger,Steven Lee,Jamie Crawford,Kelly Robbins,Virginia Mae Moore*
...
Abstract (英文): Winter pea grown as a cover crop in monoculture and mixture with grasses poses remote sensing challenges, including spectral mixing among peas, weeds and intercrop canopies and wide variation in canopy establishment and biomass across planting dates and years. The effect of these factors, cropping system, and model choice on UAV-based biomass prediction and whether such models transfer across years remain untested. Over two seasons (2022–2024) in Freeville, NY, we evaluated winter pea across four planting dates and two cropping systems, combining UAV multispectral imagery with four machine learning models to predict biomass from vegetation indices. Spring imagery achieved the highest cross-validation accuracy (R2 up to 0.894), with early plantings far more predictable than later ones. Crop growth stage at acquisition influenced accuracy more than model choice: a single well-timed spring flight matched or exceeded multi-date combinations. In the evaluation of an independent test set, models ranked plots reliably but did not generalize in absolute magnitude. Low-cost UAV imagery is thus practical for cover crop biomass monitoring, provided accuracy is interpreted relative to the biomass range and recalibrated across years.

</details>


### [275] [Multi-Temporal Satellite Observations and Machine Learning-Based Flood Susceptibility Assessment of the 2025 Punjab Flood](https://doi.org/10.3390/rs18183204)
...
### [278] [FuzzyQSM: A Novel Tree Structure Reconstruction Approach Using Uncertainty](https://doi.org/10.3390/rs18183244)
...
### [279] [Dual-Branch GCN-Mamba Network with Semantic-Guided Meta-Learning for Multimodal Remote Sensing Classification](https://doi.org/10.3390/rs18183218)
...
### [280] [Evaluating Remote Sensing Foundation Model Embeddings for Cross-City Thematic Mapping of Eucalyptus Plantations in Guangxi, China](https://doi.org/10.3390/rs18183248)
...
### [281] [Generating Annual 10 m Land Cover Maps for 37 Chinese Metropolises Using Google Satellite Embeddings](https://doi.org/10.3390/rs18183233)
...
### [282] [Surface Soil Moisture from Sentinel-2 Imagery: A Systematic Review Complemented by a Case Study in Sardinia, Italy](https://doi.org/10.3390/rs18183262)
...
### [283] [Nonlinear Latent-Space Data Assimilation for Sea Surface Height Reconstruction from Sparse Observations](https://doi.org/10.3390/rs18183221)
...
### [284] [Kinematics of the Haiyuan Fault (Northwest China) Prior to the 2022 Menyuan Mw 6.7 Earthquake](https://doi.org/10.3390/rs18183232)
...
### [285] [HiSGD: High–Low Spectral Gradient Decoupling for Heterogeneous Distillation in Infrared Small-Target Detection](https://doi.org/10.3390/rs18183215)
*Wujiao He,Weixing Li,Chengjin An,Boyang Li,Chao Xiao,Jun Chen,R. H. Ni,Siyi Deng*
...
### [287] [SDR-YOLO: Scale-Selective Detail Residual Enhanced YOLO for Visible–Thermal Object Detection](https://doi.org/10.3390/rs18183216)
...
### [288] [Improving the Quality of the Surface Heat Flux Data over the Tibetan Plateau by Using an Optuna–CatBoost–Shapley Additive exPlanation Method](https://doi.org/10.3390/rs18183242)
...
### [289] [A Physics-Guided Diffusion Framework for Mars Rover Image Restoration Under Atmospheric Dust](https://doi.org/10.3390/rs18183224)
...
### [290] [SDFA-Net: Urban Manhole Cover Detection via Fisheye Image and LiDAR Point Cloud Fusion](https://doi.org/10.3390/rs18183206)
*Qiuping Lan,Shuwen Hu,Jia Li,Yijie Huang,Zikuan Li,Qiang Fan*
...
### [293] [SAR-Based Monitoring of Maize Phenology in the High-Yielding Mpumalanga Province of South Africa](https://doi.org/10.3390/rs18183231)
*Reiji Kimura,Masao Moriyama,Mitsuru Tsubo,Mokhele Edmond Moeletsi,Zaid A. Bello*
...
### [294] [A Scene Classification Method for Dead-End Road Association Zones in Mountainous Areas Using Optical Remote Sensing](https://doi.org/10.3390/rs18183214)
*Xin Jin,Shuo Zhang,Rixing He,Yue Deng,Yitong Wang,Yupeng Luan,Yanyan Li*
...
### [296] [Acquisition-Level Optimization of Split-Spectrum TEC Observability in L-Band InSAR](https://doi.org/10.3390/rs18183217)
...
### [297] [SHALA: Sparse Hierarchical Alignment for Visible-Infrared Vehicle Re-Identification Under Aerial-Ground Viewpoint Gaps](https://doi.org/10.3390/rs18183219)
...
### [298] [Label-Efficient Semi-Supervised Building Semantic Segmentation for High-Resolution UAV Imagery](https://doi.org/10.3390/rs18183250)
...
### [299] [Atmospheric Water Vapor Monitoring on Horseshoe Island, Antarctica: GNSS Observations at the Permanent TUR1 and TUR2 Stations Using a Regional Tm Calibration](https://doi.org/10.3390/rs18183226)
...
### [300] [SASI-Net: A Sparse Adaptive SAR Imaging Network with Complex-Valued Mask-Aware Completion and Adaptive Truncated Penalty](https://doi.org/10.3390/rs18183255)
...
### [301] [Case Studies of Environmental Monitoring Based on the Integrated Use of Bioindicators and Remote Sensing](https://doi.org/10.3390/rs18183227)
...
### [302] [Temporal Lag and Response Characteristics of Runoff and Its Components in a Glacial Basin](https://doi.org/10.3390/rs18183235)
...
### [303] [WSF-YOLO: Wavelet-Guided Spatial–Frequency Joint Modeling for Aircraft Detection in SAR Images](https://doi.org/10.3390/rs18183254)
...
### [304] [An Ensemble-Based Transfer Learning Framework Using EfficientNetV2 and MobileNetV2 for Satellite Image Classification of Wildfires](https://doi.org/10.3390/rs18183212)
...
### [306] [Early Detection of Pine Wilt Disease at the Pre-Visual Stage Using UAV-Based Hyperspectral Imagery](https://doi.org/10.3390/rs18183229)
...
### [307] [Dynamic Monthly Population-Exposure-Based Flash-Flood Risk Mapping Using an Explainable XGBoost Framework](https://doi.org/10.3390/rs18183253)
...
### [308] [Bridging Temporal Gaps in Thermal Remote Sensing: A Generative Adversarial Network Approach to Land-Cover-Stratified Diurnal Heat Retention in Peri-Urban Landscapes](https://doi.org/10.3390/rs18183259)
...
### [309] [Assessing the Sensitivity of Sentinel-1 and Sentinel-2 Time Series to Wheat Yellow Rust Using Feature Evaluation, Separability, and Random Forest Classification](https://doi.org/10.3390/rs18183252)
...
### [310] [A Vegetation Growth Pattern-Constrained Interpolation Method for High-Resolution Daily FPAR/LAI Reconstruction and NPP Spatial Disaggregation](https://doi.org/10.3390/rs18183211)
*Meiheng Zhong,Zi Ye,Yihao Liu,Siqi Long,Rixiu Zhou,Dehua Zhao*
...
### [312] [MSFFusion: Multi-Level Shallow Feature Fusion Network for Pansharpening](https://doi.org/10.3390/rs18183236)
...
### [313] [Boundary-Aware Attention-Enhanced DeepLabV3+ for Landslide Segmentation Along the Sichuan–Tibet Transportation Corridor](https://doi.org/10.3390/rs18183220)
...
### [314] [Experimental Assessment of Microwave Penetration Depth in Layered Soil by Wideband (0.3–6 GHz) Transmissometry and Reflectometry](https://doi.org/10.3390/rs18183258)
...
### [315] [Region and Boundary Accuracy in Zero-Shot Shoreline Extraction: A Benchmark of Segmentation-Native Vision-Language Models and Prompted SAM Pipelines](https://doi.org/10.3390/rs18183228)
...
### [316] [Data Processing and Quality Control of the CW193 Sun Photometer Network and Evaluation of Satellite Aerosol Products](https://doi.org/10.3390/rs18183225)
...
### [317] [Optimization of Farmland Management Zoning in the Black Soil Region: A Climate Adaptability Assessment Considering Crop Growth Response and Topographic Characteristics](https://doi.org/10.3390/rs18183260)
...
### [318] [FCD-Mamba: A Frequency-Enhanced and Center-Pixel-Guided Dual-Branch Mamba Network for Hyperspectral Image Classification](https://doi.org/10.3390/rs18183234)
...
### [319] [National-Scale Flood Susceptibility Mapping of Nigeria Using Statistical and Machine Learning Models with Satellite-Driven Validation for Data-Sparse Environments](https://doi.org/10.3390/rs18193264)
...
### [320] [Estimating Ground-Level PM2.5 over Beijing–Tianjin–Hebei from DQ-1 Wide-Swath Imager Signals and ERA5 Meteorology Using Physics-Guided Feature Engineering](https://doi.org/10.3390/rs18193266)
...
### [321] [CYGNSS Soil Moisture Performance in Guinea Savanna Region: Extended and Quadruple Collocation Evidence from Benue State, Nigeria](https://doi.org/10.3390/rs18193267)
...
### [322] [From Pixel Receptive Fields to Ground Spans: GSD-Conditioned Cross-Resolution Feature Alignment for Photovoltaic Detection and Segmentation](https://doi.org/10.3390/rs18193263)
...
### [323] [Satellite-Based Seasonal Monitoring of PM2.5-Related Trace Gases and Aerosol Loading over the Lazio Region](https://doi.org/10.3390/rs18193265)
...
### [324] [Triple-Level Topology Awareness Using Hypergraph for Marine Ship Surveillance from SAR Imagery](https://doi.org/10.3390/rs18193268)
...
### [325] [Daily Lake-Surface NDVI Reconstruction Using Multi-Source Machine Learning Under Incomplete Optical Observations](https://doi.org/10.3390/rs18193269)
...
### [326] [MHBA-TransUNet: Shallow–Deep Collaborative RGB–DSM Fusion with Hybrid Bidirectional Attention for High-Resolution Remote Sensing Semantic Segmentation](https://doi.org/10.3390/rs18193271)
...
### [327] [Simulation of Equatorial Plasma Bubble Signatures in GNSS TEC](https://doi.org/10.3390/rs18193273)
...
### [328] [SWOT Wide-Swath Altimetry for Inland-Water Remote Sensing: Applications, Challenges, and Prospects—A Systematic Bibliometric and Thematic Review](https://doi.org/10.3390/rs18193274)
...
### [329] [Expert knowledge-guided decision calibration for accurate fine-grained tree species classification](https://doi.org/10.1016/j.isprsjprs.2026.09.013)
...
### [330] [DirPA: Addressing prior shift in imbalanced few-shot crop-type classification](https://doi.org/10.1016/j.isprsjprs.2026.08.016)
...
### [331] [BlinkChange: Active perception bi-temporal reliable representation for disaster impact understanding from remote sensing imagery](https://doi.org/10.1016/j.isprsjprs.2026.09.014)
...
### [332] [Structure-preserving feature disentanglement with an adaptive transformer for unsupervised cross-domain object detection in heterogeneous SAR imagery](https://doi.org/10.1016/j.isprsjprs.2026.08.044)
...
### [333] [Toward scalable organ-level 3D plant segmentation: A systematic and quantitative review through the lens of the data-algorithm-computing triangle](https://doi.org/10.1016/j.isprsjprs.2026.09.003)
...
### [334] [A novel change homogenizing flood index and an expedited locally tuned Markov random field model for unsupervised rural floodwater detection in sentinel-1 data: An application to flood impact assessment](https://doi.org/10.1016/j.isprsjprs.2026.08.021)
...
### [335] [Spot the differences: a registration-robust object-level change detection framework for remote sensing images](https://doi.org/10.1016/j.isprsjprs.2026.09.001)
...
### [336] [Explaining discrepancies between ground-based and spaceborne nighttime light measurements using urban morphology: a multi-city study](https://doi.org/10.1016/j.isprsjprs.2026.09.015)
...
### [337] [SAF-CropNet: Spatially adaptive fusion of SAR and optical imagery for semantic segmentation in operational cropland mapping across regions](https://doi.org/10.1016/j.isprsjprs.2026.08.029)
*minghui chang,Shuaifeng Peng,Tao Xu,Yi Lin Yuan,洋一 馬目,Jie Bai,Fugui Luo,Jingyu Zhang,Xiaoyu Xiao,Yu Mu,Yong Wang,Shihua Li*
...
### [338] [Sketch and paint: A structural–temporal aware refinement network for fine-grained crop mapping on satellite image time series](https://doi.org/10.1016/j.isprsjprs.2026.09.009)
...
### [339] [An Adaptive Dynamic AI-driven ParameTerization (ADAPT) framework for global hourly gross primary productivity estimation](https://doi.org/10.1016/j.isprsjprs.2026.09.024)
...
### [340] [Spectral unmixing of hyperspectral lidar signals considering laser beam profile](https://doi.org/10.1016/j.isprsjprs.2026.09.011)
...
### [341] [Nested generative downscaling of passive microwave satellite data for navigation-scale Arctic sea ice forecasting](https://doi.org/10.1016/j.isprsjprs.2026.09.021)
...
### [342] [HELIOS-Net: Hourly daytime estimation of land surface temperature with integration of observational solar-cloud-satellite geometry and deep neural network](https://doi.org/10.1016/j.isprsjprs.2026.09.028)
...
### [343] [PatchFusion: Resilient LiDAR-camera fusion for point cloud semantic segmentation in autonomous driving](https://doi.org/10.1016/j.isprsjprs.2026.09.008)
...
### [344] [SNAIL-Radar-Indoor:A large-scale handheld LiDAR–4D radar–IMU dataset for robust SLAM in degraded environments](https://doi.org/10.1016/j.isprsjprs.2026.09.004)
...
### [345] [SaKD: Lightweight infrared small target detection via Spatial-Aware Knowledge Distillation for edge deployment](https://doi.org/10.1016/j.isprsjprs.2026.09.018)
...
### [346] [Resampling-free jitter-aware bundle adjustment for HiRISE multi-CCD stereo mapping on Mars](https://doi.org/10.1016/j.isprsjprs.2026.09.020)
...
### [347] [Point-Cloud-Native L-Band Augmentation of SWOT KaRIn: Recovering Canopy-Shadowed Dark Water with NISAR](https://doi.org/10.5281/zenodo.22818803)
...
### [348] [Point-Cloud-Native L-Band Augmentation of SWOT KaRIn: Recovering Canopy-Shadowed Dark Water with NISAR](https://doi.org/10.5281/zenodo.22818804)
*Waleed Shoaib,Bakhtawar Ismail,Muhammad Salik*
...
### [349] [Intercomparison and evaluation of global land surface phenology (LSP) products](https://doi.org/10.1016/j.rse.2026.115680)
...
### [350] [Using ICESat-2 ATL08 heights and Landsat time series to assess the age-height relationship of southern yellow pines in the US](https://doi.org/10.1016/j.rse.2026.115677)
...
### [351] [Urban building height mapping in Beijing-Tianjin-Hebei megalopolis by synergy of spaceborne LiDAR and multisource geospatial datasets](https://doi.org/10.1016/j.rse.2026.115658)
...
### [352] [Time and tide: Mapping the changing 3D shape of Australia's dynamic intertidal zone using time series satellite data](https://doi.org/10.1016/j.rse.2026.115663)
...
### [353] [Direct quantification of solar-induced chlorophyll fluorescence using compact solar-blind optical radiometers](https://doi.org/10.1016/j.rse.2026.115669)
*Jonas Kuhn,J. Stutz*
...
### [354] [3DSO: Quantifying forest three-dimensional structural organization beyond the canopy surface using LiDAR data](https://doi.org/10.1016/j.rse.2026.115668)
...
### [355] [An uncertainty workflow for landsat TIRS split window surface temperature products incorporating atmospheric water vapor](https://doi.org/10.1016/j.rse.2026.115660)
...
### [356] [Observation-constrained clear-sky direct radiative forcing of black carbon containing aerosols reveals distinct radiative regimes across urban Asia](https://doi.org/10.1016/j.rse.2026.115673)
...
### [357] [CEDA-Net: A cascade empirical–deep network for mitigating InSAR tropospheric delay in complex atmospheric and topographic conditions](https://doi.org/10.1016/j.rse.2026.115659)
...
### [358] [Detecting diurnal dynamics of cotton leaf inclination angle under water-salt stress](https://doi.org/10.1016/j.rse.2026.115674)
...
### [359] [Diurnal coupling of total column nitrogen dioxide and aerosol optical depth: Insights from the Pandora Asia network](https://doi.org/10.1016/j.rse.2026.115682)
...
### [360] [Bias-variance trade-off in radiative transfer model inversion drives uncertainty in leaf area index estimation](https://doi.org/10.1016/j.rse.2026.115662)
...
### [361] [Optically consistent multi-parameter aerosol retrieval from dual-satellite polarimetric observations enabled by physics-guided deep learning](https://doi.org/10.1016/j.rse.2026.115685)
...
### [362] [Spatiotemporal extension of all-sky downward shortwave radiation over mountainous area](https://doi.org/10.1016/j.rse.2026.115678)
...
### [363] [High-resolution land cover mapping from coarse labels via a noisy label learning-guided cross-scale framework](https://doi.org/10.1080/15481603.2026.2726002)
...
### [364] [A source–medium–receiver framework for modeling wind-driven vegetation cooling in urban landscapes](https://doi.org/10.1080/15481603.2026.2733792)
...
### [365] [Correction](https://doi.org/10.1080/15481603.2026.2736374)
*Unknown Author*
...
### [366] [Reliable Sar oil-spill segmentation for marine monitoring Via self-supervised denoising and lightweight wavelet–state-space modeling](https://doi.org/10.1080/15481603.2026.2727781)
...
### [367] [Global daily joint reconstruction of TROPOMI tropospheric No 2 columns and averaging kernels with temporally aware deep learning](https://doi.org/10.1080/15481603.2026.2732650)
